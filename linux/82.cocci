...
position p;
@@
 unsigned long cocci_id@p [ _KERNEL_CAPABILITY_U32S ] ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct subprocess_info * info , struct cred * new ) ;
...
position p;
@@
 void ( * cocci_id@p ) ( struct subprocess_info * info ) ;
...
position p;
@@
 enum umh_disable_depth cocci_id@p ;
