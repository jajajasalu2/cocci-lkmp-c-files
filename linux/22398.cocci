...
position p;
@@
 void cocci_id@p ( struct ct_mixer * mixer , enum CT_AMIXER_CTL type ) ;
...
position p;
@@
 struct snd_kcontrol * cocci_id@p [ 2 ] ;
...
position p;
@@
 struct ct_kcontrol_init {  unsigned char ctl ;  char * name ; } cocci_id@p ;
...
position p;
@@
 struct ct_kcontrol_init cocci_id@p [ NUM_CTALSA_MIXERS ] ;
...
position p;
@@
 enum CTALSA_MIXER_CTL cocci_id@p ;
...
position p;
@@
 enum CT_SUM_CTL { SUM_IN_F , SUM_IN_R , SUM_IN_C , SUM_IN_S , SUM_IN_F_C , NUM_CT_SUMS , } cocci_id@p ;
