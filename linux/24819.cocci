...
position p;
@@
 const struct nla_policy cocci_id@p [ NFACCT_FILTER_MAX + 1 ] ;
...
position p;
@@
 const struct nfacct_filter * cocci_id@p ;
...
position p;
@@
 struct nfacct_filter cocci_id@p ;
...
position p;
@@
 const struct nfnl_callback cocci_id@p [ NFNL_MSG_ACCT_MAX ] ;
...
position p;
@@
 struct nlattr * cocci_id@p [ NFACCT_FILTER_MAX + 1 ] ;
...
symbol cocci_id;
position p;
typedef atomic64_t,refcount_t;
@@
 struct nf_acct {  atomic64_t pkts ;  atomic64_t bytes ;  unsigned long flags ;  struct list_head head ;  refcount_t refcnt ;  char name [ NFACCT_NAME_MAX ] ;  struct rcu_head rcu_head ;  char data [ 0 ] ; } cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ NFACCT_MAX + 1 ] ;
...
typedef u32;
@@
 struct nfacct_filter {  u32 value ;  u32 mask ; } cocci_id@p ;
