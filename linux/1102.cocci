...
position p;
@@
 struct global_pstate_info * cocci_id@p ;
...
typedef u8;
@@
 struct pstate_idx_revmap_data {  u8 pstate_id ;  unsigned int cpufreq_table_idx ;  struct hlist_node hentry ; } cocci_id@p ;
...
position p;
@@
 struct powernv_smp_call_data * cocci_id@p ;
...
typedef u8;
@@
 struct powernv_smp_call_data {  unsigned int freq ;  u8 pstate_id ;  u8 gpstate_id ; } cocci_id@p ;
...
position p;
@@
 struct cpufreq_frequency_table cocci_id@p [ POWERNV_MAX_PSTATES + 1 ] ;
...
symbol cocci_id;
position p;
typedef u8,bool,cpumask_t;
@@
 struct chip {  unsigned int id ;  bool throttled ;  bool restore ;  u8 throttle_reason ;  cpumask_t mask ;  struct work_struct throttle ;  int throttle_turbo ;  int throttle_sub_turbo ;  int reason [ OCC_MAX_REASON ] ; } * cocci_id@p ;
...
position p;
@@
 enum throttle_reason_type { NO_THROTTLE = 0 , POWERCAP , CPU_OVERTEMP , POWER_SUPPLY_FAILURE , OVERCURRENT , OCC_RESET_THROTTLE , OCC_MAX_REASON , } cocci_id@p ;
...
position p;
@@
 struct opal_occ_msg cocci_id@p ;
...
typedef spinlock_t;
@@
 struct global_pstate_info {  int highest_lpstate_idx ;  unsigned int elapsed_time ;  unsigned int last_sampled_time ;  int last_lpstate_idx ;  int last_gpstate_idx ;  spinlock_t gpstate_lock ;  struct timer_list timer ;  struct cpufreq_policy * policy ; } cocci_id@p ;
...
position p;
@@
 struct pstate_idx_revmap_data * cocci_id@p ;
...
typedef bool;
@@
 struct powernv_pstate_info {  unsigned int min ;  unsigned int max ;  unsigned int nominal ;  unsigned int nr_pstates ;  bool wof_enabled ; } cocci_id@p ;
