...
position p;
@@
 struct seccomp_knotif * cocci_id@p ;
...
typedef u32;
@@
 struct seccomp_log_name {  u32 log ;  const char * name ; } cocci_id@p ;
...
position p;
@@
 struct seccomp_filter * * cocci_id@p ;
...
position p;
@@
 const struct seccomp_log_name * cocci_id@p ;
...
position p;
@@
 struct seccomp_data * cocci_id@p ;
...
typedef u64,u32;
@@
 struct seccomp_knotif {  struct task_struct * task ;  u64 id ;  const struct seccomp_data * data ;  enum notify_state state ;  int error ;  long val ;  u32 flags ;  struct completion ready ;  struct list_head list ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,refcount_t;
@@
 struct seccomp_filter {  refcount_t usage ;  bool log ;  struct seccomp_filter * prev ;  struct bpf_prog * prog ;  struct notification * notif ;  struct mutex notify_lock ; } cocci_id@p ;
...
position p;
@@
 struct seccomp_metadata cocci_id@p ;
...
position p;
@@
 struct seccomp_notif_resp cocci_id@p ;
...
position p;
@@
 const struct seccomp_data * cocci_id@p ;
...
position p;
@@
 struct seccomp_notif_sizes cocci_id@p ;
...
position p;
@@
 struct seccomp_notif cocci_id@p ;
...
position p;
@@
 enum notify_state { SECCOMP_NOTIFY_INIT , SECCOMP_NOTIFY_SENT , SECCOMP_NOTIFY_REPLIED , } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u64,wait_queue_head_t;
@@
 struct notification {  struct semaphore request ;  u64 next_id ;  struct list_head notifications ;  wait_queue_head_t wqh ; } cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ sizeof ( seccomp_actions_avail ) ] ;
...
position p;
@@
 struct compat_sock_fprog cocci_id@p ;
