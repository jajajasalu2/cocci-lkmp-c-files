...
symbol cocci_id;
position p;
typedef s8,cpumask_t,raw_spinlock_t;
@@
 struct dsu_pmu {  struct pmu pmu ;  struct device * dev ;  raw_spinlock_t pmu_lock ;  struct dsu_hw_events hw_events ;  cpumask_t associated_cpus ;  cpumask_t active_cpu ;  struct hlist_node cpuhp_node ;  s8 num_counters ;  int irq ; 
...
position p;
@@
 struct dsu_hw_events * cocci_id@p ;
...
@@
 struct dsu_hw_events { 
	 DECLARE_BITMAP ( used_mask , DSU_PMU_MAX_HW_CNTRS ) 
		 ;  struct perf_event * events [ DSU_PMU_MAX_HW_CNTRS ] ; } cocci_id@p ;
