...
position p;
@@
 struct kvp_file_state cocci_id@p [ KVP_POOL_COUNT ] ;
...
position p;
@@
 struct kvp_file_state {  int fd ;  int num_blocks ;  struct kvp_record * records ;  int num_records ;  char fname [ MAX_FILE_NAME ] ; } cocci_id@p ;
...
position p;
@@
 struct kvp_record {  char key [ HV_KVP_EXCHANGE_MAX_KEY_SIZE ] ;  char value [ HV_KVP_EXCHANGE_MAX_VALUE_SIZE ] ; } cocci_id@p ;
...
position p;
@@
 struct hv_kvp_ipaddr_value * cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ HV_KVP_EXCHANGE_MAX_VALUE_SIZE ] ;
...
position p;
@@
 enum { IPADDR = 0 , NETMASK , GATEWAY , DNS , } cocci_id@p ;
...
position p;
@@
 enum key_index { FullyQualifiedDomainName = 0 , IntegrationServicesVersion , NetworkAddressIPv4 , NetworkAddressIPv6 , OSBuildNumber , OSName , OSMajorVersion , OSMinorVersion , OSVersion , ProcessorArchitecture , } cocci_id@p ;
...
position p;
@@
 struct hv_kvp_msg cocci_id@p [ 1 ] ;
