...
position p;
@@
 struct target_backend_ops cocci_id@p ;
...
position p;
@@
 struct tcmu_cmd_entry cocci_id@p ;
...
position p;
@@
 struct tcmu_mailbox * cocci_id@p ;
...
typedef u32;
@@
 struct tcmu_hba {  u32 host_id ; } cocci_id@p ;
...
position p;
@@
 enum { Opt_dev_config , Opt_dev_size , Opt_hw_block_size , Opt_hw_max_sectors , Opt_nl_reply_supported , Opt_max_data_area_mb , Opt_err , } cocci_id@p ;
...
position p;
@@
 struct nla_policy cocci_id@p [ TCMU_ATTR_MAX + 1 ] ;
...
position p;
@@
 struct tcmu_nl_cmd {  struct completion complete ;  struct list_head nl_list ;  struct tcmu_dev * udev ;  int cmd ;  int status ; } cocci_id@p ;
...
position p;
@@
 enum tcmu_multicast_groups { TCMU_MCGRP_CONFIG , } cocci_id@p ;
