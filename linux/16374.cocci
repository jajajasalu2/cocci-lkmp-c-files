...
position p;
@@
 struct roccat_reader {  struct list_head node ;  struct roccat_device * device ;  int cbuf_start ; } cocci_id@p ;
...
typedef uint8_t;
@@
 struct roccat_report {  uint8_t * value ; } cocci_id@p ;
...
position p;
@@
 struct roccat_device cocci_id@p ;
...
position p;
@@
 struct roccat_reader cocci_id@p ;
...
position p;
@@
 struct roccat_report * cocci_id@p ;
...
@@
 struct roccat_device {  unsigned int minor ;  int report_size ;  int open ;  int exist ;  wait_queue_head_t wait ;  struct device * dev ;  struct hid_device * hid ;  struct list_head readers ;  struct mutex readers_lock ;  struct roccat_report cbuf [ ROCCAT_CBUF_SIZE ] ;  int cbuf_end ;  struct mutex cbuf_lock ; } cocci_id@p ;
...
position p;
@@
 struct roccat_device * cocci_id@p [ ROCCAT_MAX_DEVICES ] ;
