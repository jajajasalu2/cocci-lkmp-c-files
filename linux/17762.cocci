...
position p;
@@
 const struct kvm_irq_routing_entry * cocci_id@p ;
...
position p;
@@
 struct irq_queue {  unsigned long queue [ BITS_TO_LONGS ( ( MAX_IRQ + 63 ) & ~ 63 ) ] ;  int next ;  int priority ; } cocci_id@p ;
...
position p;
@@
 enum irq_type { IRQ_TYPE_NORMAL = 0 , IRQ_TYPE_FSLINT , IRQ_TYPE_FSLSPECIAL , } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef uint32_t,spinlock_t,gpa_t;
@@
 struct openpic {  struct kvm * kvm ;  struct kvm_device * dev ;  struct kvm_io_device mmio ;  const struct mem_reg * mmio_regions [ MAX_MMIO_REGIONS ] ;  int num_mmio_regions ;  gpa_t reg_base ;  spinlock_t lock ;  struct fsl_mpic_info * fsl ;  uint32_t model ;  uint32_t flags ;  uint32_t nb_irqs ;  uint32_t vid ;  uint32_t vir ;  uint32_t vector_mask ;  uint32_t tfrr_reset ;  uint32_t ivpr_reset ;  uint32_t idr_reset ;  uint32_t brr1 ;  uint32_t mpic_mode_mask ;  uint32_t frr ;  uint32_t gcr ;  uint32_t pir ;  uint32_t spve ;  uint32_t tfrr ;  struct irq_source src [ MAX_IRQ ] ;  struct irq_dest dst [ MAX_CPU ] ;  uint32_t nb_cpus ;  struct {  uint32_t tccr ;  uint32_t tbcr ; } timers [ MAX_TMR ] ;  struct {  uint32_t msir ; } msi [ MAX_MSI ] ;  uint32_t max_irq ;  uint32_t irq_ipi0 ;  uint32_t irq_tim0 ;  uint32_t irq_msi ; } cocci_id@p ;
...
@@
 struct irq_source {  uint32_t ivpr ;  uint32_t idr ;  uint32_t destmask ;  int last_cpu ;  int output ;  int pending ;  enum irq_type type ;  bool level : 1 ;  bool nomask : 1 ; } cocci_id@p ;
...
@@
 struct irq_dest {  struct kvm_vcpu * vcpu ;  int32_t ctpr ;  struct irq_queue raised ;  struct irq_queue servicing ;  uint32_t outputs_active [ NUM_OUTPUTS ] ; } cocci_id@p ;
...
position p;
@@
 const struct mem_reg cocci_id@p ;
...
position p;
@@
 struct kvm_device_ops cocci_id@p ;
...
position p;
@@
 const struct kvm_io_device_ops cocci_id@p ;
...
position p;
@@
 struct fsl_mpic_info {  int max_ext ; } cocci_id@p ;
...
position p;
@@
 struct fsl_mpic_info cocci_id@p ;
...
typedef u8,u32;
@@
 union {  u32 val ;  u8 bytes [ 4 ] ; } cocci_id@p ;
