...
position p;
@@
 struct ssif_addr_info * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct ssif_info * ssif_info , int result , unsigned char * data , unsigned int len ) ;
...
position p;
@@
 enum ssif_intf_state { SSIF_NORMAL , SSIF_GETTING_FLAGS , SSIF_GETTING_EVENTS , SSIF_CLEARING_FLAGS , SSIF_GETTING_MESSAGES , } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct ssif_info * ssif_info ) ;
...
position p;
@@
 void cocci_id@p ( struct ssif_info * ssif_info , struct ipmi_smi_msg * msg ) ;
...
position p;
@@
 enum i2c_alert_protocol cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct ssif_info * ssif_info , unsigned long * flags ) ;
...
position p;
@@
 int cocci_id@p ( struct ssif_info * ssif_info , unsigned char * data , unsigned int len ) ;
...
position p;
@@
 unsigned short cocci_id@p [ MAX_SSIF_BMCS ] ;
...
position p;
@@
 struct ssif_addr_info {  struct i2c_board_info binfo ;  char * adapter_name ;  int debug ;  int slave_addr ;  enum ipmi_addr_src addr_src ;  union ipmi_smi_info_union addr_info ;  struct device * dev ;  struct i2c_client * client ;  struct i2c_client * added_client ;  struct mutex clients_mutex ;  struct list_head clients ;  struct list_head link ; } cocci_id@p ;
