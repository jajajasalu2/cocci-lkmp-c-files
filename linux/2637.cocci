...
position p;
@@
 const struct meson_i2c_data * cocci_id@p ;
...
@@
 struct meson_i2c {  struct i2c_adapter adap ;  struct device * dev ;  void __iomem * regs ;  struct clk * clk ;  struct i2c_msg * msg ;  int state ;  bool last ;  int count ;  int pos ;  int error ;  spinlock_t lock ;  struct completion done ;  u32 tokens [ 2 ] ;  int num_tokens ;  const struct meson_i2c_data * data ; } cocci_id@p ;
...
position p;
@@
 enum { STATE_IDLE , STATE_READ , STATE_WRITE , } cocci_id@p ;
...
position p;
@@
 struct meson_i2c_data {  unsigned char div_factor ; } cocci_id@p ;
...
position p;
@@
 enum { TOKEN_END = 0 , TOKEN_START , TOKEN_SLAVE_ADDR_WRITE , TOKEN_SLAVE_ADDR_READ , TOKEN_DATA , TOKEN_DATA_LAST , TOKEN_STOP , } cocci_id@p ;
