...
position p;
@@
 char cocci_id@p [ ( sizeof ( * guid ) + 1 ) * 2 ] ;
...
position p;
@@
 const struct devtable * cocci_id@p ;
...
position p;
@@
 struct devtable {  const char * device_id ;  unsigned long id_size ;  int ( * do_entry ) ( const char * filename , void * symval , char * alias ) ; } cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( const char * filename , void * symval , char * alias ) ;
...
position p;
@@
 const struct dmifield {  const char * prefix ;  int field ; } cocci_id@p [ ] ;
