...
position p;
@@
 struct w1_family_ops cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct device * device ) ;
...
symbol cocci_id;
position p;
typedef atomic_t,uint8_t;
@@
 struct w1_therm_family_data {  uint8_t rom [ 9 ] ;  atomic_t refcnt ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct device * device , int val ) ;
...
position p;
@@
 struct w1_therm_family_converter cocci_id@p [ ] ;
...
typedef u8,u16;
@@
 struct w1_therm_family_converter {  u8 broken ;  u16 reserved ;  struct w1_family * f ;  int ( * convert ) ( u8 rom [ 9 ] ) ;  int ( * precision ) ( struct device * device , int val ) ;  int ( * eeprom ) ( struct device * device ) ; } cocci_id@p ;
...
position p;
@@
 struct w1_therm_family_data cocci_id@p ;
...
typedef u32;
@@
 int cocci_id@p ( struct device * dev , u32 attr , int channel , long * val ) ;
...
typedef u8;
@@
 struct therm_info {  u8 rom [ 9 ] ;  u8 crc ;  u8 verdict ; } cocci_id@p ;
...
position p;
@@
 ssize_t cocci_id@p ( struct device * device , struct device_attribute * attr , char * buf ) ;
