...
position p;
@@
 struct arm_ccn_pmu_event * cocci_id@p ;
...
position p;
@@
 struct arm_ccn {  struct device * dev ;  void __iomem * base ;  unsigned int irq ;  unsigned sbas_present : 1 ;  unsigned sbsx_present : 1 ;  int num_nodes ;  struct arm_ccn_component * node ;  int num_xps ;  struct arm_ccn_component * xp ;  struct arm_ccn_dt dt ;  int mn_id ; } cocci_id@p ;
...
typedef u32;
@@
 struct arm_ccn_pmu_event {  struct device_attribute attr ;  u32 type ;  u32 event ;  int num_ports ;  int num_vcs ;  const char * def ;  int mask ; } cocci_id@p ;
...
typedef u32;
@@
 struct arm_ccn_component {  void __iomem * base ;  u32 type ; 
...
position p;
@@
 struct attribute * cocci_id@p [ ARRAY_SIZE ( arm_ccn_pmu_events ) + 1 ] ;
...
position p;
@@
 struct arm_ccn_component * cocci_id@p ;
...
typedef u32;
@@
 int ( * cocci_id@p ) ( struct arm_ccn * ccn , int region , void __iomem * base , u32 type , u32 id ) ;
...
@@
 struct arm_ccn_dt {  int id ;  void __iomem * base ;  spinlock_t config_lock ; 
	 DECLARE_BITMAP ( pmu_counters_mask , CCN_NUM_PMU_EVENT_COUNTERS + 1 ) 
		 ;  struct {  struct arm_ccn_component * source ;  struct perf_event * event ; } pmu_counters [ CCN_NUM_PMU_EVENT_COUNTERS + 1 ] ;  struct {  u64 l , h ; } cmp_mask [ CCN_NUM_PMU_EVENT_COUNTERS + CCN_NUM_PREDEFINED_MASKS ] ;  struct hrtimer hrtimer ;  unsigned int cpu ;  struct hlist_node node ;  struct pmu pmu ; } cocci_id@p ;
