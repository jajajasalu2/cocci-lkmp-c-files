...
position p;
@@
 const struct pkt_desc * cocci_id@p ;
...
position p;
@@
 const unsigned int cocci_id@p [ CIP_SFC_COUNT ] ;
...
position p;
@@
 enum amdtp_stream_direction cocci_id@p ;
...
position p;
@@
 struct amdtp_domain * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef amdtp_stream_process_ctx_payloads_t;
@@
 amdtp_stream_process_ctx_payloads_t cocci_id@p ;
...
position p;
@@
 struct amdtp_stream * cocci_id@p ;
...
position p;
@@
 const struct {  unsigned int data_block ;  unsigned int syt_offset ; } * cocci_id@p ;
...
typedef __be32;
@@
 struct {  struct fw_iso_packet params ;  __be32 header [ IT_PKT_HEADER_SIZE_CIP / sizeof ( __be32 ) ] ; } cocci_id@p ;
