...
position p;
@@
 struct prefetch_mr_work * cocci_id@p ;
...
position p;
@@
 struct mlx5_base_av * cocci_id@p ;
...
position p;
@@
 struct mlx5_ib_pf_eq * cocci_id@p ;
...
typedef u64,u32;
@@
 struct pf_frame {  struct pf_frame * next ;  u32 key ;  u64 io_virt ;  size_t bcnt ;  int depth ; } cocci_id@p ;
...
position p;
@@
 struct mlx5_eq_param cocci_id@p ;
...
position p;
@@
 enum { MLX5_IB_NUM_PF_EQE = 0x1000 , MLX5_IB_NUM_PF_DRAIN = 64 , } cocci_id@p ;
...
position p;
@@
 struct mlx5_wqe_srq_next_seg cocci_id@p ;
...
typedef u64,u32;
@@
 struct prefetch_mr_work {  struct work_struct work ;  u32 pf_flags ;  u32 num_sge ;  struct {  u64 io_virt ;  struct mlx5_ib_mr * mr ;  size_t length ; } frags [ ] ; } cocci_id@p ;
...
position p;
@@
 const struct mmu_interval_notifier_ops cocci_id@p ;
...
position p;
@@
 struct mlx5_pagefault cocci_id@p ;
...
@@
 struct mlx5_pagefault {  u32 bytes_committed ;  u32 token ;  u8 event_subtype ;  u8 type ;  union {  struct {  u32 packet_size ;  u32 wq_num ;  u16 wqe_index ; } wqe ;  struct {  u32 r_key ;  u32 packet_size ;  u32 rdma_op_len ;  u64 rdma_va ; } rdma ; } ;  struct mlx5_ib_pf_eq * eq ;  struct work_struct work ; } cocci_id@p ;
...
position p;
@@
 struct mmu_interval_notifier * cocci_id@p ;
...
typedef u32;
@@
 u32 cocci_id@p [ MLX5_ST_SZ_DW ( page_fault_resume_in ) ] ;
...
position p;
@@
 struct mlx5_cache_ent * cocci_id@p ;
...
position p;
@@
 struct mlx5_core_rsc_common * cocci_id@p ;
...
typedef u32;
@@
 u32 cocci_id@p [ MLX5_ST_SZ_DW ( page_fault_resume_out ) ] ;
...
position p;
@@
 struct mlx5_eqe_page_fault * cocci_id@p ;
...
position p;
@@
 enum ib_uverbs_advise_mr_advice cocci_id@p ;
