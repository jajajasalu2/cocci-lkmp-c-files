...
position p;
@@
 enum { MTHCA_ICM_ALLOC_SIZE = 1 << 18 , MTHCA_TABLE_CHUNK_SIZE = 1 << 18 , } cocci_id@p ;
...
position p;
@@
 struct mthca_icm_table * cocci_id@p ;
...
position p;
@@
 struct mthca_user_db_table * cocci_id@p ;
...
typedef u64;
@@
 struct mthca_user_db_table {  struct mutex mutex ;  struct {  u64 uvirt ;  struct scatterlist mem ;  int refcount ; } page [ 0 ] ; } cocci_id@p ;
...
position p;
@@
 struct mthca_db_page * cocci_id@p ;
...
position p;
@@
 struct mthca_icm_chunk * cocci_id@p ;
