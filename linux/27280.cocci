...
typedef u32;
@@
 struct nfs4_cb_compound_hdr {  u32 ident ;  u32 nops ;  __be32 * nops_p ;  u32 minorversion ;  int status ; } cocci_id@p ;
...
position p;
@@
 struct rpc_create_args cocci_id@p ;
...
position p;
@@
 const struct nfsd4_copy * cocci_id@p ;
...
position p;
@@
 const struct nfsd4_blocked_lock * cocci_id@p ;
...
position p;
@@
 const struct rpc_program cocci_id@p ;
...
position p;
@@
 const struct nfs4_delegation * cocci_id@p ;
...
position p;
@@
 const struct {  int stat ;  int errno ; } cocci_id@p [ ] ;
...
position p;
@@
 const struct rpc_version * cocci_id@p [ 2 ] ;
...
position p;
@@
 struct nfs4_cb_compound_hdr * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct nfs4_client * , int reason ) ;
...
position p;
@@
 const struct nfsd4_callback * cocci_id@p ;
...
position p;
@@
 const struct nfsd4_session * cocci_id@p ;
...
position p;
@@
 const struct nfs4_layout_stateid * cocci_id@p ;
...
position p;
@@
 enum nfs_cb_opnum4 { OP_CB_GETATTR = 3 , OP_CB_RECALL = 4 , OP_CB_LAYOUTRECALL = 5 , OP_CB_NOTIFY = 6 , OP_CB_PUSH_DELEG = 7 , OP_CB_RECALL_ANY = 8 , OP_CB_RECALLABLE_OBJ_AVAIL = 9 , OP_CB_RECALL_SLOT = 10 , OP_CB_SEQUENCE = 11 , OP_CB_WANTS_CANCELLED = 12 , OP_CB_NOTIFY_LOCK = 13 , OP_CB_NOTIFY_DEVICEID = 14 , OP_CB_OFFLOAD = 15 , OP_CB_ILLEGAL = 10044 , } cocci_id@p ;
...
position p;
@@
 unsigned int cocci_id@p [ ARRAY_SIZE ( nfs4_cb_procedures ) ] ;
...
position p;
@@
 const struct knfsd_fh * cocci_id@p ;
