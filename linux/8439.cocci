...
position p;
@@
 struct de_desc {  __le32 opts1 ;  __le32 opts2 ;  __le32 addr1 ;  __le32 addr2 ; #if DSL  __le32 skip [ DSL ] ; #endif } cocci_id@p ;
...
typedef u8;
@@
 u8 cocci_id@p [ DE_EEPROM_SIZE + 6 ] ;
...
typedef u32;
@@
 unsigned int cocci_id@p ( struct de_private * de , u32 new_media ) ;
...
position p;
@@
 struct de_srom_media_block * cocci_id@p ;
...
typedef dma_addr_t;
@@
 struct ring_info {  struct sk_buff * skb ;  dma_addr_t mapping ; } cocci_id@p ;
...
position p;
@@
 struct de_srom_info_leaf * cocci_id@p ;
...
typedef u32;
@@
 void cocci_id@p ( struct de_private * de , u32 status ) ;
...
position p;
@@
 const char * const cocci_id@p [ DE_MAX_MEDIA ] ;
...
position p;
@@
 void cocci_id@p ( struct de_private * de ) ;
...
typedef u16;
@@
 struct media_info {  u16 type ;  u16 csr13 ;  u16 csr14 ;  u16 csr15 ; } cocci_id@p ;
