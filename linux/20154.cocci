...
position p;
@@
 enum au1300_multifunc_pins cocci_id@p ;
...
position p;
@@
 const struct alchemy_irqmap * cocci_id@p ;
...
position p;
@@
 unsigned long cocci_id@p [ ALCHEMY_GPIC_INT_NUM + 6 ] ;
...
position p;
@@
 struct alchemy_irqmap {  int irq ;  int type ;  int prio ;  int internal ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct irq_data * d , unsigned int type ) ;
...
position p;
@@
 struct alchemy_irqmap * cocci_id@p ;
