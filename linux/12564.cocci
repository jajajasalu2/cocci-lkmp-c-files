...
position p;
@@
 enum mipi_dsi_pixel_format cocci_id@p ;
...
position p;
@@
 struct msm_dsi_phy_shared_timings * cocci_id@p ;
...
position p;
@@
 enum dsi_cmd_dst_format cocci_id@p ;
...
position p;
@@
 const struct dsi_reg_entry * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct msm_dsi_host * msm_host ) ;
...
position p;
@@
 enum dsi_traffic_mode cocci_id@p ;
...
position p;
@@
 struct msm_dsi_phy_clk_request * cocci_id@p ;
...
position p;
@@
 const enum mipi_dsi_pixel_format cocci_id@p ;
...
position p;
@@
 struct msm_dsi_host cocci_id@p ;
...
position p;
@@
 struct mipi_dsi_host_ops cocci_id@p ;
...
position p;
@@
 enum dsi_vid_dst_format cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct msm_dsi_host * msm_host ) ;
...
position p;
@@
 const struct msm_dsi_config * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,bool,u32,spinlock_t,dma_addr_t;
@@
 struct msm_dsi_host {  struct mipi_dsi_host base ;  struct platform_device * pdev ;  struct drm_device * dev ;  int id ;  void __iomem * ctrl_base ;  struct regulator_bulk_data supplies [ DSI_DEV_REGULATOR_MAX ] ;  struct clk * bus_clks [ DSI_BUS_CLK_MAX ] ;  struct clk * byte_clk ;  struct clk * esc_clk ;  struct clk * pixel_clk ;  struct clk * byte_clk_src ;  struct clk * pixel_clk_src ;  struct clk * byte_intf_clk ;  u32 byte_clk_rate ;  u32 pixel_clk_rate ;  u32 esc_clk_rate ;  struct clk * src_clk ;  struct clk * esc_clk_src ;  struct clk * dsi_clk_src ;  u32 src_clk_rate ;  struct gpio_desc * disp_en_gpio ;  struct gpio_desc * te_gpio ;  const struct msm_dsi_cfg_handler * cfg_hnd ;  struct completion dma_comp ;  struct completion video_comp ;  struct mutex dev_mutex ;  struct mutex cmd_mutex ;  spinlock_t intr_lock ;  u32 err_work_state ;  struct work_struct err_work ;  struct work_struct hpd_work ;  struct workqueue_struct * workqueue ;  struct drm_gem_object * tx_gem_obj ;  void * tx_buf ;  dma_addr_t tx_buf_paddr ;  int tx_size ;  u8 * rx_buf ;  struct regmap * sfpb ;  struct drm_display_mode * mode ;  struct device_node * device_node ;  unsigned int channel ;  unsigned int lanes ;  enum mipi_dsi_pixel_format format ;  unsigned long mode_flags ;  int dlane_swap ;  int num_data_lanes ;  u32 dma_cmd_ctrl_restore ;  bool registered ;  bool power_on ;  bool enabled ;  int irq ; } cocci_id@p ;
...
position p;
@@
 enum sfpb_ahb_arb_master_port_en cocci_id@p ;
...
position p;
@@
 struct mipi_dsi_msg cocci_id@p ;
...
position p;
@@
 const struct msm_dsi_cfg_handler * cocci_id@p ;
