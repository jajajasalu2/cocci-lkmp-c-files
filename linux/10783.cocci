...
typedef u64,bool;
@@
 struct pin_mode {  u64 size ;  u64 flags ;  bool ( * assert ) ( const struct i915_vma * , const struct pin_mode * mode , int result ) ;  const char * string ; } cocci_id@p ;
...
position p;
@@
 const struct pin_mode * cocci_id@p ;
...
position p;
@@
 enum i915_ggtt_view_type cocci_id@p [ ] ;
...
position p;
@@
 const struct intel_remapped_plane_info cocci_id@p [ ] ;
...
position p;
@@
 const struct phase {  const char * name ; } cocci_id@p [ ] ;
