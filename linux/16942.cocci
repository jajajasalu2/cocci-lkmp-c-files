...
position p;
@@
 enum elants_iap_mode { ELAN_IAP_OPERATIONAL , ELAN_IAP_RECOVERY , } cocci_id@p ;
...
position p;
@@
 struct elants_version_attribute cocci_id@p ;
...
position p;
@@
 struct elants_version_attribute {  struct device_attribute dattr ;  size_t field_offset ;  size_t field_size ; } cocci_id@p ;
...
typedef u8,bool,u16;
@@
 struct elants_data {  struct i2c_client * client ;  struct input_dev * input ;  struct regulator * vcc33 ;  struct regulator * vccio ;  struct gpio_desc * reset_gpio ;  u16 fw_version ;  u8 test_version ;  u8 solution_version ;  u8 bc_version ;  u8 iap_version ;  u16 hw_version ;  unsigned int x_res ;  unsigned int y_res ;  unsigned int x_max ;  unsigned int y_max ;  enum elants_state state ;  enum elants_iap_mode iap_mode ;  struct mutex sysfs_mutex ;  u8 cmd_resp [ HEADER_SIZE ] ;  struct completion cmd_done ;  bool wake_irq_enabled ;  bool keep_power_in_suspend ;  u8 buf [ MAX_PACKET_SIZE ] ____cacheline_aligned ; } cocci_id@p ;
...
position p;
@@
 enum elants_state { ELAN_STATE_NORMAL , ELAN_WAIT_QUEUE_HEADER , ELAN_WAIT_RECALIBRATION , } cocci_id@p ;
