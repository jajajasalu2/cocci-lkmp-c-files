...
position p;
@@
 const struct watchdog_ops cocci_id@p ;
...
position p;
@@
 struct mei_wdt_start_response cocci_id@p ;
...
position p;
@@
 struct mei_wdt_stop_request cocci_id@p ;
...
position p;
@@
 struct mei_wdt_start_request cocci_id@p ;
...
typedef u8;
@@
 struct mei_mc_hdr {  u8 command ;  u8 bytecount ;  u8 subcommand ;  u8 versionnumber ; } cocci_id@p ;
...
position p;
@@
 enum mei_wdt_state { MEI_WDT_PROBE , MEI_WDT_IDLE , MEI_WDT_START , MEI_WDT_RUNNING , MEI_WDT_STOPPING , MEI_WDT_NOT_REQUIRED , } cocci_id@p ;
...
position p;
@@
 struct watchdog_device * cocci_id@p ;
...
typedef bool,u16;
@@
 struct mei_wdt {  struct watchdog_device wdd ;  struct mei_cl_device * cldev ;  enum mei_wdt_state state ;  bool resp_required ;  struct completion response ;  struct work_struct unregister ;  struct mutex reg_lock ;  u16 timeout ; #if IS_ENABLED(CONFIG_DEBUG_FS)  struct dentry * dbgfs_dir ; #endif } cocci_id@p ;
