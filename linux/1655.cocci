...
position p;
@@
 const struct rescale_cfg cocci_id@p [ ] ;
...
position p;
@@
 enum rescale_variant { CURRENT_SENSE_AMPLIFIER , CURRENT_SENSE_SHUNT , VOLTAGE_DIVIDER , } cocci_id@p ;
...
typedef s32;
@@
 struct rescale {  const struct rescale_cfg * cfg ;  struct iio_channel * source ;  struct iio_chan_spec chan ;  struct iio_chan_spec_ext_info * ext_info ;  s32 numerator ;  s32 denominator ; } cocci_id@p ;
...
position p;
@@
 struct rescale_cfg {  enum iio_chan_type type ;  int ( * props ) ( struct device * dev , struct rescale * rescale ) ; } cocci_id@p ;
