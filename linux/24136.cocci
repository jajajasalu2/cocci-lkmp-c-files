...
typedef u8,u32,u16;
@@
 struct cgw_job {  struct hlist_node list ;  struct rcu_head rcu ;  u32 handled_frames ;  u32 dropped_frames ;  u32 deleted_frames ;  struct cf_mod mod ;  union {  struct net_device * dev ; } src ;  union {  struct net_device * dev ; } dst ;  union {  struct can_can_gw ccgw ; } ;  u8 gwtype ;  u8 limit_hops ;  u16 flags ; } cocci_id@p ;
...
position p;
@@
 struct cgw_csum_xor cocci_id@p ;
...
position p;
@@
 struct can_can_gw {  struct can_filter filter ;  int src_idx ;  int dst_idx ; } cocci_id@p ;
...
position p;
@@
 struct cgw_frame_mod cocci_id@p ;
...
position p;
@@
 struct cgw_fdframe_mod cocci_id@p ;
...
position p;
@@
 struct cgw_csum_crc8 cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ CGW_MAX + 1 ] ;
...
position p;
@@
 struct nlattr * cocci_id@p [ CGW_MAX + 1 ] ;
