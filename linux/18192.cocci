...
position p;
@@
 struct clk_spc {  struct clk_hw hw ;  int cluster ; } cocci_id@p ;
...
position p;
@@
 struct ve_spc_drvdata * cocci_id@p ;
...
position p;
@@
 struct ve_spc_opp {  unsigned long freq ;  unsigned long u_volt ; } cocci_id@p ;
...
@@
 struct ve_spc_drvdata {  void __iomem * baseaddr ;  u32 a15_clusid ;  uint32_t cur_rsp_mask ;  uint32_t cur_rsp_stat ;  struct semaphore sem ;  struct completion done ;  struct ve_spc_opp * opps [ MAX_CLUSTERS ] ;  int num_opps [ MAX_CLUSTERS ] ; } cocci_id@p ;
