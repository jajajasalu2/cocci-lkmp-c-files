...
position p;
@@
 struct dn_rt_cache_iter_state cocci_id@p ;
...
position p;
@@
 const struct flowidn * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct sk_buff * ) ;
...
position p;
@@
 struct dn_rt_hash_bucket cocci_id@p ;
...
typedef spinlock_t;
@@
 struct dn_rt_hash_bucket {  struct dn_route __rcu * chain ;  spinlock_t lock ; } cocci_id@p ;
...
position p;
@@
 struct dn_rt_cache_iter_state {  int bucket ; } cocci_id@p ;
...
typedef __u32;
@@
 struct dst_entry * cocci_id@p ( struct dst_entry * , __u32 ) ;
