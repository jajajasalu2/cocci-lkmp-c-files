...
position p;
@@
 struct initialValues {  unsigned short reg ;  unsigned short value ; } cocci_id@p ;
...
typedef u32;
@@
 struct nm256_stream {  struct nm256 * chip ;  struct snd_pcm_substream * substream ;  int running ;  int suspended ;  u32 buf ;  int bufsize ;  void __iomem * bufptr ;  unsigned long bufptr_addr ;  int dma_size ;  int period_size ;  int periods ;  int shift ;  int cur_period ; } cocci_id@p ;
...
position p;
@@
 struct nm256_stream * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u32,spinlock_t,irq_handler_t;
@@
 struct nm256 {  struct snd_card * card ;  void __iomem * cport ;  struct resource * res_cport ;  unsigned long cport_addr ;  void __iomem * buffer ;  struct resource * res_buffer ;  unsigned long buffer_addr ;  u32 buffer_start ;  u32 buffer_end ;  u32 buffer_size ;  u32 all_coeff_buf ;  u32 coeff_buf [ 2 ] ;  unsigned int coeffs_current : 1 ;  unsigned int use_cache : 1 ;  unsigned int reset_workaround : 1 ;  unsigned int reset_workaround_2 : 1 ;  unsigned int in_resume : 1 ;  int mixer_base ;  int mixer_status_offset ;  int mixer_status_mask ;  int irq ;  int irq_acks ;  irq_handler_t interrupt ;  int badintrcount ;  struct mutex irq_mutex ;  struct nm256_stream streams [ 2 ] ;  struct snd_ac97 * ac97 ;  unsigned short * ac97_regs ;  struct snd_pcm * pcm ;  struct pci_dev * pci ;  spinlock_t reg_lock ; } cocci_id@p ;
...
position p;
@@
 struct initialValues cocci_id@p [ ] ;
...
position p;
@@
 const struct snd_ac97_res_table cocci_id@p [ ] ;
...
position p;
@@
 enum { NM_BLACKLISTED , NM_RESET_WORKAROUND , NM_RESET_WORKAROUND_2 , } cocci_id@p ;
...
position p;
@@
 const unsigned int cocci_id@p [ 8 ] ;
