...
position p;
@@
 struct fam15h_power_data * cocci_id@p ;
...
typedef u64;
@@
 struct fam15h_power_data {  struct pci_dev * pdev ;  unsigned int tdp_to_watts ;  unsigned int base_tdp ;  unsigned int processor_pwr_watts ;  unsigned int cpu_pwr_sample_ratio ;  const struct attribute_group * groups [ FAM15H_NUM_GROUPS ] ;  struct attribute_group group ;  u64 max_cu_acc_power ;  u64 cu_acc_power [ MAX_CUS ] ;  u64 cpu_sw_pwr_ptsc [ MAX_CUS ] ;  int cu_on [ MAX_CUS ] ;  unsigned long power_period ; } cocci_id@p ;
