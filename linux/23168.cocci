...
position p;
@@
 struct alchemy_pcm_ctx {  struct audio_stream stream [ 2 ] ; } cocci_id@p ;
...
position p;
@@
 struct audio_stream * cocci_id@p ;
...
position p;
@@
 struct alchemy_pcm_ctx * cocci_id@p ;
...
position p;
@@
 struct audio_stream {  struct snd_pcm_substream * substream ;  int dma ;  struct pcm_period * buffer ;  unsigned int period_size ;  unsigned int periods ; } cocci_id@p ;
...
typedef u32;
@@
 struct pcm_period {  u32 start ;  u32 relative_end ;  struct pcm_period * next ; } cocci_id@p ;
