...
position p;
@@
 struct rcu_torture {  struct rcu_head rtort_rcu ;  int rtort_pipe_count ;  struct list_head rtort_free ;  int rtort_mbtest ; } cocci_id@p ;
...
position p;
@@
 struct task_struct * cocci_id@p [ NR_CPUS ] ;
...
position p;
@@
 struct rcu_boost_inflight cocci_id@p ;
...
position p;
@@
 struct rcu_boost_inflight {  struct rcu_head rcu ;  int inflight ; } cocci_id@p ;
...
position p;
@@
 struct fwd_cb_state {  struct rcu_head rh ;  int stop ; } cocci_id@p ;
...
position p;
@@
 struct fwd_cb_state * cocci_id@p ;
...
position p;
@@
 struct rcu_launder_hist {  long n_launders ;  unsigned long launder_gp_seq ; } cocci_id@p ;
...
position p;
@@
 struct rcu_launder_hist cocci_id@p [ N_LAUNDERS_HIST ] ;
...
position p;
@@
 struct rcu_torture_ops * cocci_id@p ;
...
position p;
@@
 long cocci_id@p [ RCU_TORTURE_PIPE_LEN + 1 ] ;
...
position p;
@@
 struct rcu_fwd_cb {  struct rcu_head rh ;  struct rcu_fwd_cb * rfc_next ;  int rfc_gps ; } cocci_id@p ;
...
typedef bool;
@@
 struct rt_read_seg {  int rt_readstate ;  unsigned long rt_delay_jiffies ;  unsigned long rt_delay_ms ;  unsigned long rt_delay_us ;  bool rt_preempted ; } cocci_id@p ;
...
position p;
@@
 struct rt_read_seg cocci_id@p [ RCUTORTURE_RDR_MAX_SEGS ] ;
...
symbol cocci_id;
position p;
typedef rcu_callback_t;
@@
 rcu_callback_t cocci_id@p ;
...
typedef atomic_t;
@@
 atomic_t cocci_id@p [ RCU_TORTURE_PIPE_LEN + 1 ] ;
...
position p;
@@
 struct torture_random_state * cocci_id@p ;
...
position p;
@@
 struct rcu_torture cocci_id@p [ 10 * RCU_TORTURE_PIPE_LEN ] ;
