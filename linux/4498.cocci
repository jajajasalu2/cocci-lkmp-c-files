...
position p;
@@
 const struct memdev * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct file * file , unsigned long addr ) ;
...
symbol cocci_id;
position p;
typedef fmode_t,umode_t;
@@
 const struct memdev {  const char * name ;  umode_t mode ;  const struct file_operations * fops ;  fmode_t fmode ; } cocci_id@p [ ] ;
