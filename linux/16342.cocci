...
position p;
@@
 const struct {  int most_ch_data_type ;  const char * name ; } cocci_id@p [ ] ;
...
position p;
@@
 struct mostcore {  struct device dev ;  struct device_driver drv ;  struct bus_type bus ;  struct list_head comp_list ; } cocci_id@p ;
...
position p;
@@
 struct most_interface * cocci_id@p ;
...
position p;
@@
 struct most_channel cocci_id@p ;
...
position p;
@@
 struct interface_private {  int dev_id ;  char name [ STRING_SIZE ] ;  struct most_channel * channel [ MAX_CHANNELS ] ;  struct list_head channel_list ; } cocci_id@p ;
...
position p;
@@
 typeof ( c -> iface -> enqueue ) cocci_id@p ;
...
position p;
@@
 struct show_links_data cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,u16,wait_queue_head_t,spinlock_t,atomic_t;
@@
 struct most_channel {  struct device dev ;  struct completion cleanup ;  atomic_t mbo_ref ;  atomic_t mbo_nq_level ;  u16 channel_id ;  char name [ STRING_SIZE ] ;  bool is_poisoned ;  struct mutex start_mutex ;  struct mutex nq_mutex ;  int is_starving ;  struct most_interface * iface ;  struct most_channel_config cfg ;  bool keep_mbo ;  bool enqueue_halt ;  struct list_head fifo ;  spinlock_t fifo_lock ;  struct list_head halt_fifo ;  struct list_head list ;  struct pipe pipe0 ;  struct pipe pipe1 ;  struct list_head trash_fifo ;  struct task_struct * hdm_enqueue_task ;  wait_queue_head_t hdm_fifo_wq ; } cocci_id@p ;
...
position p;
@@
 struct pipe {  struct core_component * comp ;  int refs ;  int num_buffers ; } cocci_id@p ;
...
position p;
@@
 struct core_component * cocci_id@p ;
...
position p;
@@
 struct show_links_data {  int offs ;  char * buf ; } cocci_id@p ;
