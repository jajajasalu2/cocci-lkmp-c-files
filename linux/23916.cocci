...
position p;
@@
 void cocci_id@p ( struct in_device * in_dev , struct in_ifaddr __rcu * * ifap , int destroy ) ;
...
position p;
@@
 void cocci_id@p ( struct in_device * idev ) ;
...
position p;
@@
 struct inet_fill_args cocci_id@p ;
...
position p;
@@
 struct devinet_sysctl_table * cocci_id@p ;
...
typedef u32;
@@
 struct inet_fill_args {  u32 portid ;  u32 seq ;  int event ;  unsigned int flags ;  int netnsid ;  int ifindex ; } cocci_id@p ;
...
typedef u32;
@@
 void cocci_id@p ( int event , struct in_ifaddr * , struct nlmsghdr * , u32 ) ;
...
position p;
@@
 struct ipv4_devconf cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct in_device * idev ) ;
...
position p;
@@
 char cocci_id@p [ sizeof ( "net/ipv4/conf/" ) + IFNAMSIZ ] ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ IFLA_INET_MAX + 1 ] ;
...
position p;
@@
 struct hlist_head cocci_id@p [ IN4_ADDR_HSIZE ] ;
...
position p;
@@
 struct devinet_sysctl_table {  struct ctl_table_header * sysctl_header ;  struct ctl_table devinet_vars [ __IPV4_DEVCONF_MAX ] ; } cocci_id@p ;
