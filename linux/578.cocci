...
position p;
@@
 unsigned char cocci_id@p [ STREEBOG512_DIGEST_SIZE ] ;
...
position p;
@@
 const struct ecc_curve * cocci_id@p ;
...
position p;
@@
 unsigned char cocci_id@p [ ECRDSA_MAX_SIG_SIZE ] ;
...
typedef u64;
@@
 u64 cocci_id@p [ ECRDSA_MAX_DIGITS ] ;
...
typedef u64;
@@
 struct ecrdsa_ctx {  enum OID algo_oid ;  enum OID curve_oid ;  enum OID digest_oid ;  const struct ecc_curve * curve ;  unsigned int digest_len ;  const char * digest ;  unsigned int key_len ;  const char * key ;  struct ecc_point pub_key ;  u64 _pubp [ 2 ] [ ECRDSA_MAX_DIGITS ] ; } cocci_id@p ;
