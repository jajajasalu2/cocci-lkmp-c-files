...
position p;
@@
 struct sess_data {  unsigned int xid ;  struct cifs_ses * ses ;  struct nls_table * nls_cp ;  void ( * func ) ( struct sess_data * ) ;  int result ;  int buf0_type ;  struct kvec iov [ 3 ] ; } cocci_id@p ;
...
position p;
@@
 AUTHENTICATE_MESSAGE * cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ sizeof ( unc_fmt ) + SERVER_NAME_LEN_WITH_NULL ] ;
...
position p;
@@
 void cocci_id@p ( struct sess_data * sess_data ) ;
...
position p;
@@
 char cocci_id@p [ CIFS_AUTH_RESP_SIZE ] ;
