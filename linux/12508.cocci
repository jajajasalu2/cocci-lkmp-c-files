...
position p;
@@
 struct drm_mm_node cocci_id@p [ 2 ] ;
...
position p;
@@
 const struct insert_mode {  const char * name ;  enum drm_mm_insert_mode mode ; } cocci_id@p [ ] ;
...
position p;
@@
 enum { BEST , BOTTOMUP , TOPDOWN , EVICT , } cocci_id@p ;
...
position p;
@@
 struct evict_node {  struct drm_mm_node node ;  struct list_head link ; } cocci_id@p ;
...
position p;
@@
 const struct drm_mm * cocci_id@p ;
...
position p;
@@
 const struct insert_mode * cocci_id@p ;
...
position p;
@@
 const struct drm_mm_scan * cocci_id@p ;
