...
position p;
@@
 struct target_core_file_cmd cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ FD_MAX_DEV_PROT_NAME ] ;
...
position p;
@@
 const struct target_backend_ops cocci_id@p ;
...
position p;
@@
 enum { Opt_fd_dev_name , Opt_fd_dev_size , Opt_fd_buffered_io , Opt_fd_async_io , Opt_err , } cocci_id@p ;
...
position p;
@@
 struct target_core_file_cmd {  unsigned long len ;  struct se_cmd * cmd ;  struct kiocb iocb ; } cocci_id@p ;
