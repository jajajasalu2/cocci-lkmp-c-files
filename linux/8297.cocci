...
typedef u8;
@@
 int cocci_id@p ( struct net_device * dev , struct ethtool_eeprom * eeprom , u8 * data ) ;
...
position p;
@@
 void cocci_id@p ( struct cp_private * cp ) ;
...
symbol cocci_id;
position p;
typedef __le32,__le64;
@@
 struct cp_desc {  __le32 opts1 ;  __le32 opts2 ;  __le64 addr ; } cocci_id@p ;
...
position p;
@@
 struct cp_extra_stats {  unsigned long rx_frags ; } cocci_id@p ;
...
position p;
@@
 struct cp_dma_stats * cocci_id@p ;
...
position p;
@@
 const struct ethtool_wolinfo * cocci_id@p ;
