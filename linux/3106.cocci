...
symbol cocci_id;
position p;
typedef u16,uint,spinlock_t,resource_size_t;
@@
 struct ace_device {  int id ;  int media_change ;  int users ;  struct list_head list ;  struct tasklet_struct fsm_tasklet ;  uint fsm_task ;  uint fsm_state ;  uint fsm_continue_flag ;  uint fsm_iter_num ;  struct timer_list stall_timer ;  struct request * req ;  void * data_ptr ;  int data_count ;  int data_result ;  int id_req_count ;  int id_result ;  struct completion id_completion ;  int in_irq ;  resource_size_t physaddr ;  void __iomem * baseaddr ;  int irq ;  int bus_width ;  struct ace_reg_ops * reg_ops ;  int lock_count ;  spinlock_t lock ;  struct device * dev ;  struct request_queue * queue ;  struct gendisk * gd ;  struct blk_mq_tag_set tag_set ;  struct list_head rq_list ;  u16 cf_id [ ATA_ID_WORDS ] ; } cocci_id@p ;
