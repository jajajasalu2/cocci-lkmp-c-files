...
position p;
@@
 void cocci_id@p ( struct request_queue * q , struct blk_flush_queue * fq , unsigned int flags ) ;
...
position p;
@@
 enum { REQ_FSEQ_PREFLUSH = ( 1 << 0 ) , REQ_FSEQ_DATA = ( 1 << 1 ) , REQ_FSEQ_POSTFLUSH = ( 1 << 2 ) , REQ_FSEQ_DONE = ( 1 << 3 ) , REQ_FSEQ_ACTIONS = REQ_FSEQ_PREFLUSH | REQ_FSEQ_DATA | REQ_FSEQ_POSTFLUSH , FLUSH_PENDING_TIMEOUT = 5 * HZ , } cocci_id@p ;
...
position p;
@@
 struct blk_flush_queue * cocci_id@p ;
