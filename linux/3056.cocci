...
position p;
@@
 struct acpi_battery_reader cocci_id@p [ ] ;
...
typedef u8,s16,u16;
@@
 struct acpi_battery {  struct power_supply * bat ;  struct power_supply_desc bat_desc ;  struct acpi_sbs * sbs ;  unsigned long update_time ;  char name [ 8 ] ;  char manufacturer_name [ ACPI_SBS_BLOCK_MAX ] ;  char device_name [ ACPI_SBS_BLOCK_MAX ] ;  char device_chemistry [ ACPI_SBS_BLOCK_MAX ] ;  u16 alarm_capacity ;  u16 full_charge_capacity ;  u16 design_capacity ;  u16 design_voltage ;  u16 serial_number ;  u16 cycle_count ;  u16 temp_now ;  u16 voltage_now ;  s16 rate_now ;  s16 rate_avg ;  u16 capacity_now ;  u16 state_of_charge ;  u16 state ;  u16 mode ;  u16 spec ;  u8 id ;  u8 present : 1 ;  u8 have_sysfs_alarm : 1 ; } cocci_id@p ;
...
typedef u8;
@@
 struct acpi_sbs {  struct power_supply * charger ;  struct acpi_device * device ;  struct acpi_smb_hc * hc ;  struct mutex lock ;  struct acpi_battery battery [ MAX_SBS_BAT ] ;  u8 batteries_supported : 4 ;  u8 manager_present : 1 ;  u8 charger_present : 1 ;  u8 charger_exists : 1 ; } cocci_id@p ;
...
typedef u8;
@@
 struct acpi_battery_reader {  u8 command ;  u8 mode ;  size_t offset ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct acpi_battery * battery ) ;
...
position p;
@@
 struct acpi_battery cocci_id@p ;
