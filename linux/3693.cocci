...
position p;
@@
 const struct mtk_sha_reqctx * cocci_id@p ;
...
position p;
@@
 struct mtk_sha_reqctx cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u64,__le32,u8,u32,dma_addr_t;
@@
 struct mtk_sha_reqctx {  struct mtk_sha_info info ;  unsigned long flags ;  unsigned long op ;  u64 digcnt ;  size_t bufcnt ;  dma_addr_t dma_addr ;  __le32 ct_hdr ;  u32 ct_size ;  dma_addr_t ct_dma ;  dma_addr_t tfm_dma ;  struct scatterlist * sg ;  u32 offset ;  u32 total ;  size_t ds ;  size_t bs ;  u8 * buffer ; } cocci_id@p ;
...
typedef spinlock_t;
@@
 struct mtk_sha_drv {  struct list_head dev_list ;  spinlock_t lock ; } cocci_id@p ;
...
typedef __le32;
@@
 struct mtk_sha_info {  __le32 ctrl [ 2 ] ;  __le32 cmd [ 3 ] ;  __le32 tfm [ 2 ] ;  __le32 digest [ SHA_MAX_DIGEST_BUF_SIZE ] ; } cocci_id@p ;
...
typedef u8;
@@
 int cocci_id@p ( struct mtk_cryp * cryp , u8 id , struct ahash_request * req ) ;
...
position p;
@@
 struct mtk_sha_hmac_ctx * cocci_id@p ;
...
position p;
@@
 struct mtk_sha_info * cocci_id@p ;
