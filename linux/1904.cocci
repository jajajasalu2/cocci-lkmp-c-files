...
position p;
@@
 enum ads124s_id { ADS124S08_ID , ADS124S06_ID , } cocci_id@p ;
...
position p;
@@
 struct ads124s_chip_info {  const struct iio_chan_spec * channels ;  unsigned int num_channels ; } cocci_id@p ;
...
typedef u32,u16,s64;
@@
 u32 cocci_id@p [ ADS124S08_MAX_CHANNELS + sizeof ( s64 ) / sizeof ( u16 ) ] ;
...
typedef u8;
@@
 struct ads124s_private {  const struct ads124s_chip_info * chip_info ;  struct gpio_desc * reset_gpio ;  struct spi_device * spi ;  struct mutex lock ;  u8 data [ 5 ] ____cacheline_aligned ; } cocci_id@p ;
...
position p;
@@
 struct ads124s_private * cocci_id@p ;
...
position p;
@@
 const struct ads124s_chip_info cocci_id@p [ ] ;
