...
position p;
@@
 struct serial_uart_config {  char * name ;  int dfl_xmit_fifo_size ;  int flags ; } cocci_id@p ;
...
position p;
@@
 const struct serial_uart_config cocci_id@p [ ] ;
...
position p;
@@
 enum su_type { SU_PORT_NONE , SU_PORT_MS , SU_PORT_KBD , SU_PORT_PORT , } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct uart_port * port , unsigned int cflag , unsigned int iflag , unsigned int quot ) ;
...
position p;
@@
 struct uart_sunsu_port cocci_id@p ;
...
position p;
@@
 struct uart_sunsu_port cocci_id@p [ UART_NR ] ;
