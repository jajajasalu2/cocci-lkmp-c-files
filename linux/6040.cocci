...
position p;
@@
 struct pmic_gpio_state * cocci_id@p ;
...
position p;
@@
 struct pmic_gpio_pad * cocci_id@p ;
...
position p;
@@
 const struct pin_config_item cocci_id@p [ ARRAY_SIZE ( pmic_gpio_bindings ) ] ;
...
position p;
@@
 struct pmic_gpio_state {  struct device * dev ;  struct regmap * map ;  struct pinctrl_dev * ctrl ;  struct gpio_chip chip ; } cocci_id@p ;
