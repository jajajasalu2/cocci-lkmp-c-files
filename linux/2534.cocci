...
position p;
@@
 const struct {  enum dm_uevent_type type ;  enum kobject_action action ;  char * name ; } cocci_id@p [ ] ;
...
position p;
@@
 struct dm_uevent {  struct mapped_device * md ;  enum kobject_action action ;  struct kobj_uevent_env ku_env ;  struct list_head elist ;  char name [ DM_NAME_LEN ] ;  char uuid [ DM_UUID_LEN ] ; } cocci_id@p ;
...
position p;
@@
 enum dm_uevent_type cocci_id@p ;
