...
position p;
@@
 const struct dbg_off * const cocci_id@p ;
...
position p;
@@
 struct wil_debugfs_iomem_data * cocci_id@p ;
...
position p;
@@
 struct wil_tx_enhanced_desc * cocci_id@p ;
...
position p;
@@
 unsigned char cocci_id@p [ MAX_MBOXITEM_SIZE ] ;
...
position p;
@@
 const struct dbg_off cocci_id@p [ ] ;
...
position p;
@@
 struct wil_rx_enhanced_desc * cocci_id@p ;
...
position p;
@@
 enum dbg_off_type { doff_u32 = 0 , doff_x32 = 1 , doff_ulong = 2 , doff_io32 = 3 , doff_u8 = 4 , } cocci_id@p ;
...
position p;
@@
 volatile struct vring_tx_desc * cocci_id@p ;
...
typedef umode_t;
@@
 const struct {  const char * name ;  umode_t mode ;  const struct file_operations * fops ; } cocci_id@p [ ] ;
...
position p;
@@
 enum { max_count = 4096 , } cocci_id@p ;
...
typedef u32;
@@
 const struct {  const char * name ;  u32 icr_off ; } cocci_id@p [ ] ;
...
position p;
@@
 struct wil_rx_buff_mgmt * cocci_id@p ;
...
position p;
@@
 struct wil_tid_crypto_rx * cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ sizeof ( run_command ) + 1 ] ;
...
position p;
@@
 struct wil_blob_wrapper * cocci_id@p ;
...
typedef umode_t;
@@
 struct dbg_off {  const char * name ;  umode_t mode ;  ulong off ;  enum dbg_off_type type ; } cocci_id@p ;
