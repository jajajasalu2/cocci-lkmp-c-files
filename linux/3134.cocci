...
position p;
@@
 const struct nla_policy cocci_id@p [ NBD_SOCK_MAX + 1 ] ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ NBD_ATTR_MAX + 1 ] ;
...
position p;
@@
 struct link_dead_args cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u64,u32,wait_queue_head_t,loff_t,atomic_t;
@@
 struct nbd_config {  u32 flags ;  unsigned long runtime_flags ;  u64 dead_conn_timeout ;  struct nbd_sock * * socks ;  int num_connections ;  atomic_t live_connections ;  wait_queue_head_t conn_wait ;  atomic_t recv_threads ;  wait_queue_head_t recv_wq ;  loff_t blksize ;  loff_t bytesize ; #if IS_ENABLED(CONFIG_DEBUG_FS)  struct dentry * dbg_dir ; #endif } cocci_id@p ;
...
position p;
@@
 struct recv_thread_args * cocci_id@p ;
...
position p;
@@
 struct link_dead_args {  struct work_struct work ;  int index ; } cocci_id@p ;
...
position p;
@@
 const struct device_attribute cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct nbd_device * nbd ) ;
...
position p;
@@
 struct recv_thread_args {  struct work_struct work ;  struct nbd_device * nbd ;  int index ; } cocci_id@p ;
...
position p;
@@
 struct nlattr * cocci_id@p [ NBD_SOCK_MAX + 1 ] ;
...
position p;
@@
 int cocci_id@p ( struct nbd_device * nbd ) ;
...
@@
 struct nbd_cmd {  struct nbd_device * nbd ;  struct mutex lock ;  int index ;  int cookie ;  int retries ;  blk_status_t status ;  unsigned long flags ;  u32 cmd_cookie ; } cocci_id@p ;
...
typedef bool;
@@
 struct nbd_sock {  struct socket * sock ;  struct mutex tx_lock ;  struct request * pending ;  int sent ;  bool dead ;  int fallback_index ;  int cookie ; } cocci_id@p ;
...
typedef refcount_t;
@@
 struct nbd_device {  struct blk_mq_tag_set tag_set ;  int index ;  refcount_t config_refs ;  refcount_t refs ;  struct nbd_config * config ;  struct mutex config_lock ;  struct gendisk * disk ;  struct workqueue_struct * recv_workq ;  struct list_head list ;  struct task_struct * task_recv ;  struct task_struct * task_setup ;  struct completion * destroy_complete ;  unsigned long flags ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct genl_info * info , int index ) ;
