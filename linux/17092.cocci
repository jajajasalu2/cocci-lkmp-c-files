...
position p;
@@
 struct dt_cpu_feature * cocci_id@p ;
...
position p;
@@
 long cocci_id@p ( struct pt_regs * regs ) ;
...
typedef u64;
@@
 struct dt_cpu_feature_match {  const char * name ;  int ( * enable ) ( struct dt_cpu_feature * f ) ;  u64 cpu_ftr_bit_mask ; } cocci_id@p ;
...
typedef u64;
@@
 struct {  u64 lpcr ;  u64 lpcr_clear ;  u64 hfscr ;  u64 fscr ; } cocci_id@p ;
...
position p;
@@
 const struct dt_cpu_feature_match * cocci_id@p ;
...
typedef uint32_t;
@@
 struct dt_cpu_feature {  const char * name ;  uint32_t isa ;  uint32_t usable_privilege ;  uint32_t hv_support ;  uint32_t os_support ;  uint32_t hfscr_bit_nr ;  uint32_t fscr_bit_nr ;  uint32_t hwcap_bit_nr ;  unsigned long node ;  int enabled ;  int disabled ; } cocci_id@p ;
