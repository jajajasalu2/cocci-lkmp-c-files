...
typedef uint32_t;
@@
 struct dpu_media_color_map {  uint32_t format ;  uint32_t color ; } cocci_id@p ;
...
position p;
@@
 const struct msm_kms * cocci_id@p ;
...
position p;
@@
 const struct dpu_media_color_map cocci_id@p [ ] ;
...
typedef uint32_t;
@@
 uint32_t cocci_id@p [ DPU_MAX_PLANES ] ;
...
position p;
@@
 enum dpu_chroma_samp_type cocci_id@p ;
