...
position p;
@@
 enum dealloc_states { DEALLOC_MP_FULL = 0 , DEALLOC_MP_LOWER = 1 , DEALLOC_FILL_MP = 2 , DEALLOC_DONE = 3 , } cocci_id@p ;
...
position p;
@@
 enum walker_status { WALK_STOP , WALK_FOLLOW , WALK_CONTINUE , } cocci_id@p ;
...
position p;
@@
 struct gfs2_journal_extent * cocci_id@p ;
...
position p;
@@
 gfs2_metadata_walker cocci_id@p ;
...
typedef __u16;
@@
 struct metapath {  struct buffer_head * mp_bh [ GFS2_MAX_META_HEIGHT ] ;  __u16 mp_list [ GFS2_MAX_META_HEIGHT ] ;  int mp_fheight ;  int mp_aheight ; } cocci_id@p ;
...
position p;
@@
 enum walker_status ( * cocci_id@p ) ( struct metapath * mp , unsigned int ptrs ) ;
...
position p;
@@
 const struct metapath * cocci_id@p ;
...
position p;
@@
 const struct iomap_page_ops cocci_id@p ;
...
typedef u64;
@@
 int cocci_id@p ( struct gfs2_inode * ip , u64 offset , u64 length ) ;
...
position p;
@@
 const struct buffer_head * cocci_id@p ;
...
position p;
@@
 enum alloc_state { ALLOC_DATA = 0 , ALLOC_GROW_DEPTH = 1 , ALLOC_GROW_HEIGHT = 2 , } cocci_id@p ;
