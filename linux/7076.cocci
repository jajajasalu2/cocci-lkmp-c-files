...
symbol cocci_id;
position p;
typedef idetape_tape_t;
@@
 idetape_tape_t cocci_id@p ;
...
position p;
@@
 struct idetape_config {  int dsc_rw_frequency ;  int dsc_media_access_frequency ;  int nr_stages ; } cocci_id@p ;
...
position p;
@@
 struct ide_tape_obj * cocci_id@p [ MAX_HWIFS * MAX_DRIVES ] ;
...
position p;
@@
 enum { IDETAPE_DIR_NONE = ( 1 << 0 ) , IDETAPE_DIR_READ = ( 1 << 1 ) , IDETAPE_DIR_WRITE = ( 1 << 2 ) , } cocci_id@p ;
...
position p;
@@
 struct ide_atapi_pc * cocci_id@p ;
...
position p;
@@
 struct ide_tape_obj * cocci_id@p ;
