...
typedef u8;
@@
 struct s390_aes_ctx {  u8 key [ AES_MAX_KEY_SIZE ] ;  int key_len ;  unsigned long fc ;  union {  struct crypto_skcipher * skcipher ;  struct crypto_cipher * cip ; } fallback ; } cocci_id@p ;
...
typedef u8;
@@
 struct {  u8 iv [ AES_BLOCK_SIZE ] ;  u8 key [ AES_MAX_KEY_SIZE ] ; } cocci_id@p ;
...
position p;
@@
 struct s390_xts_ctx * cocci_id@p ;
...
typedef u8;
@@
 struct s390_xts_ctx {  u8 key [ 32 ] ;  u8 pcc_key [ 32 ] ;  int key_len ;  unsigned long fc ;  struct crypto_skcipher * fallback ; } cocci_id@p ;
...
typedef u8;
@@
 u8 cocci_id@p [ GHASH_DIGEST_SIZE ] ;
...
position p;
@@
 struct s390_aes_ctx cocci_id@p ;
...
typedef u8;
@@
 struct gcm_sg_walk {  struct scatter_walk walk ;  unsigned int walk_bytes ;  u8 * walk_ptr ;  unsigned int walk_bytes_remain ;  u8 buf [ AES_BLOCK_SIZE ] ;  unsigned int buf_bytes ;  u8 * ptr ;  unsigned int nbytes ; } cocci_id@p ;
...
position p;
@@
 struct skcipher_alg * cocci_id@p [ 4 ] ;
...
typedef u8;
@@
 struct {  u8 key [ 32 ] ;  u8 tweak [ 16 ] ;  u8 block [ 16 ] ;  u8 bit [ 16 ] ;  u8 xts [ 16 ] ; } cocci_id@p ;
...
typedef u8,u64,u32;
@@
 struct {  u32 _ [ 3 ] ;  u32 cv ;  u8 t [ GHASH_DIGEST_SIZE ] ;  u8 h [ AES_BLOCK_SIZE ] ;  u64 taadl ;  u64 tpcl ;  u8 j0 [ GHASH_BLOCK_SIZE ] ;  u8 k [ AES_MAX_KEY_SIZE ] ; } cocci_id@p ;
...
typedef u8;
@@
 struct {  u8 key [ 32 ] ;  u8 init [ 16 ] ; } cocci_id@p ;
