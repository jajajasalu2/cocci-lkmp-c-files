...
typedef bool;
@@
 struct thunk {  load_mapping_fn fn ;  void * context ;  struct dm_cache_metadata * cmd ;  bool respect_dirty_flags ;  bool hints_valid ; } cocci_id@p ;
...
position p;
@@
 struct cache_disk_superblock * cocci_id@p ;
...
position p;
@@
 struct dm_bitset_cursor cocci_id@p ;
...
position p;
@@
 enum superblock_flag_bits { CLEAN_SHUTDOWN , NEEDS_CHECK , } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,__u8,dm_dblock_t,refcount_t,dm_block_t,dm_cblock_t,sector_t;
@@
 struct dm_cache_metadata {  refcount_t ref_count ;  struct list_head list ;  unsigned version ;  struct block_device * bdev ;  struct dm_block_manager * bm ;  struct dm_space_map * metadata_sm ;  struct dm_transaction_manager * tm ;  struct dm_array_info info ;  struct dm_array_info hint_info ;  struct dm_disk_bitset discard_info ;  struct rw_semaphore root_lock ;  unsigned long flags ;  dm_block_t root ;  dm_block_t hint_root ;  dm_block_t discard_root ;  sector_t discard_block_size ;  dm_dblock_t discard_nr_blocks ;  sector_t data_block_size ;  dm_cblock_t cache_blocks ;  bool changed : 1 ;  bool clean_when_opened : 1 ;  char policy_name [ CACHE_POLICY_NAME_SIZE ] ;  unsigned policy_version [ CACHE_POLICY_VERSION_SIZE ] ;  size_t policy_hint_size ;  struct dm_cache_statistics stats ;  __u8 metadata_space_map_root [ SPACE_MAP_ROOT_SIZE ] ;  bool fail_io : 1 ;  dm_block_t dirty_root ;  struct dm_disk_bitset dirty_info ;  struct dm_array_cursor mapping_cursor ;  struct dm_array_cursor hint_cursor ;  struct dm_bitset_cursor dirty_cursor ; } cocci_id@p ;
...
position p;
@@
 struct dm_array_cursor * cocci_id@p ;
...
position p;
@@
 unsigned long ( * cocci_id@p ) ( unsigned long ) ;
...
position p;
@@
 enum mapping_bits { M_VALID = 1 , M_DIRTY = 2 , } cocci_id@p ;
