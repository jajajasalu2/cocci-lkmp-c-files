...
typedef u32;
@@
 struct urbp {  struct urb * urb ;  struct list_head urbp_list ;  struct sg_mapping_iter miter ;  u32 miter_started ; } cocci_id@p ;
...
typedef bool;
@@
 struct dummy_hcd_module_parameters {  bool is_super_speed ;  bool is_high_speed ;  unsigned int num ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct usb_gadget * g , struct usb_gadget_driver * driver ) ;
...
position p;
@@
 struct dummy_ep {  struct list_head queue ;  unsigned long last_io ;  struct usb_gadget * gadget ;  const struct usb_endpoint_descriptor * desc ;  struct usb_ep ep ;  unsigned halted : 1 ;  unsigned wedged : 1 ;  unsigned already_seen : 1 ;  unsigned setup_stage : 1 ;  unsigned stream_en : 1 ; } cocci_id@p ;
...
position p;
@@
 const struct usb_gadget_ops cocci_id@p ;
...
position p;
@@
 enum dummy_rh_state { DUMMY_RH_RESET , DUMMY_RH_SUSPENDED , DUMMY_RH_RUNNING , } cocci_id@p ;
...
position p;
@@
 const struct {  const char * name ;  const struct usb_ep_caps caps ; } cocci_id@p [ ] ;
...
position p;
@@
 int cocci_id@p ( struct usb_gadget * g ) ;
...
position p;
@@
 struct dummy_request cocci_id@p ;
...
position p;
@@
 struct dummy_request {  struct list_head queue ;  struct usb_request req ; } cocci_id@p ;
...
position p;
@@
 struct usb_gadget_driver * cocci_id@p ;
...
@@
 struct dummy {  spinlock_t lock ;  struct dummy_ep ep [ DUMMY_ENDPOINTS ] ;  int address ;  int callback_usage ;  struct usb_gadget gadget ;  struct usb_gadget_driver * driver ;  struct dummy_request fifo_req ;  u8 fifo_buf [ FIFO_SIZE ] ;  u16 devstatus ;  unsigned ints_enabled : 1 ;  unsigned udc_suspended : 1 ;  unsigned pullup : 1 ;  struct dummy_hcd * hs_hcd ;  struct dummy_hcd * ss_hcd ; } cocci_id@p ;
...
position p;
@@
 const struct usb_ep_ops cocci_id@p ;
...
position p;
@@
 struct dummy_hcd_module_parameters cocci_id@p ;
...
position p;
@@
 struct dummy * cocci_id@p [ MAX_NUM_UDC ] ;
...
position p;
@@
 enum usb_device_speed cocci_id@p ;
...
position p;
@@
 struct platform_device * cocci_id@p [ MAX_NUM_UDC ] ;
...
typedef u8,u32;
@@
 struct dummy_hcd {  struct dummy * dum ;  enum dummy_rh_state rh_state ;  struct timer_list timer ;  u32 port_status ;  u32 old_status ;  unsigned long re_timeout ;  struct usb_device * udev ;  struct list_head urbp_list ;  struct urbp * next_frame_urbp ;  u32 stream_en_ep ;  u8 num_stream [ 30 / 2 ] ;  unsigned active : 1 ;  unsigned old_active : 1 ;  unsigned resuming : 1 ; } cocci_id@p ;
