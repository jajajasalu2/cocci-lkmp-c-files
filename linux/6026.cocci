...
@@
 struct msm_pinctrl {  struct device * dev ;  struct pinctrl_dev * pctrl ;  struct gpio_chip chip ;  struct pinctrl_desc desc ;  struct notifier_block restart_nb ;  struct irq_chip irq_chip ;  int irq ;  raw_spinlock_t lock ; 
	 DECLARE_BITMAP ( dual_edge_irqs , MAX_NR_GPIO ) 
		 ; 
	 DECLARE_BITMAP ( enabled_irqs , MAX_NR_GPIO ) 
		 ; 
	 DECLARE_BITMAP ( skip_wake_irqs , MAX_NR_GPIO ) 
		 ;  const struct msm_pinctrl_soc_data * soc ;  void __iomem * regs [ MAX_NR_TILES ] ; } cocci_id@p ;
...
position p;
@@
 const struct msm_pinctrl_soc_data * cocci_id@p ;
...
position p;
@@
 const struct msm_function * cocci_id@p ;
...
position p;
@@
 const struct msm_pingroup * cocci_id@p ;
...
position p;
@@
 const struct msm_gpio_wakeirq_map * cocci_id@p ;
