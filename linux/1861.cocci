...
position p;
@@
 struct cpcap_adc_conversion_tbl cocci_id@p [ ] ;
...
position p;
@@
 struct cpcap_adc_conversion_tbl {  enum iio_chan_info_enum conv_type ;  int align_offset ;  int conv_offset ;  int cal_offset ;  int multiplier ;  int divider ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,u16,wait_queue_head_t;
@@
 struct cpcap_adc {  struct regmap * reg ;  struct device * dev ;  u16 vendor ;  int irq ;  struct mutex lock ;  const struct cpcap_adc_ato * ato ;  wait_queue_head_t wq_data_avail ;  bool done ; } cocci_id@p ;
...
position p;
@@
 struct cpcap_adc_request {  int channel ;  const struct cpcap_adc_phasing_tbl * phase_tbl ;  const struct cpcap_adc_conversion_tbl * conv_tbl ;  int bank_index ;  enum cpcap_adc_timing timing ;  int result ; } cocci_id@p ;
...
position p;
@@
 enum cpcap_adc_channel cocci_id@p ;
...
position p;
@@
 struct cpcap_adc_request cocci_id@p ;
...
position p;
@@
 const struct cpcap_adc_ato * cocci_id@p ;
...
position p;
@@
 struct cpcap_adc_phasing_tbl {  short offset ;  unsigned short multiplier ;  unsigned short divider ;  short min ;  short max ; } cocci_id@p ;
...
position p;
@@
 const struct cpcap_adc_phasing_tbl * cocci_id@p ;
...
position p;
@@
 struct cpcap_adc_ato {  unsigned short ato_in ;  unsigned short atox_in ;  unsigned short adc_ps_factor_in ;  unsigned short atox_ps_factor_in ;  unsigned short ato_out ;  unsigned short atox_out ;  unsigned short adc_ps_factor_out ;  unsigned short atox_ps_factor_out ; } cocci_id@p ;
...
position p;
@@
 enum cpcap_adc_timing { CPCAP_ADC_TIMING_IMM , CPCAP_ADC_TIMING_IN , CPCAP_ADC_TIMING_OUT , } cocci_id@p ;
...
position p;
@@
 const struct cpcap_adc_conversion_tbl * cocci_id@p ;
...
position p;
@@
 const int cocci_id@p [ CPCAP_MAX_TEMP_LVL ] [ 2 ] ;
