...
typedef spinlock_t;
@@
 struct iocq_ktio {  struct list_head head ;  spinlock_t lock ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct frame * , struct sk_buff * ) ;
...
position p;
@@
 struct buf * cocci_id@p ( struct aoedev * ) ;
...
position p;
@@
 int cocci_id@p ( struct aoedev * d , int * untainted ) ;
