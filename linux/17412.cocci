...
position p;
@@
 long ( * cocci_id@p ) ( struct device_node * node , long param , long value ) ;
...
position p;
@@
 struct pmac_mb_def {  const char * model_string ;  const char * model_name ;  int model_id ;  struct feature_table_entry * features ;  unsigned long board_flags ; } cocci_id@p ;
...
typedef u8;
@@
 u8 cocci_id@p [ KEYLARGO_GPIO_CNT ] ;
...
position p;
@@
 struct feature_table_entry cocci_id@p [ ] ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct pci_dev * bridge ) ;
...
position p;
@@
 struct dbdma_regs cocci_id@p [ 13 ] ;
...
typedef u8;
@@
 u8 cocci_id@p [ KEYLARGO_GPIO_EXTINT_CNT ] ;
...
position p;
@@
 struct feature_table_entry {  unsigned int selector ;  feature_call function ; } cocci_id@p ;
...
position p;
@@
 const struct slot_names_prop {  int count ;  char name [ 1 ] ; } * cocci_id@p ;
...
position p;
@@
 struct macio_chip cocci_id@p [ MAX_MACIO_CHIPS ] ;
