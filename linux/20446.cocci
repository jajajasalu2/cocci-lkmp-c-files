...
typedef bool;
@@
 int ( * cocci_id@p ) ( struct mce * m , int tolerant , char * * msg , bool is_excp ) ;
...
position p;
@@
 enum context { IN_KERNEL = 1 , IN_USER = 2 , IN_KERNEL_RECOV = 3 , } cocci_id@p ;
...
position p;
@@
 enum exception { EXCP_CONTEXT = 1 , NO_EXCP = 2 , } cocci_id@p ;
...
position p;
@@
 enum ser { SER_REQUIRED = 1 , NO_SER = 2 , } cocci_id@p ;
...
typedef u64;
@@
 struct severity {  u64 mask ;  u64 result ;  unsigned char sev ;  unsigned char mcgmask ;  unsigned char mcgres ;  unsigned char ser ;  unsigned char context ;  unsigned char excp ;  unsigned char covered ;  char * msg ; } cocci_id@p [ ] ;
