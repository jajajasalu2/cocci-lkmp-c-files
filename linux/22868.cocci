...
position p;
@@
 struct fcp_transaction * cocci_id@p ;
...
position p;
@@
 enum fcp_state { STATE_PENDING , STATE_BUS_RESET , STATE_COMPLETE , STATE_DEFERRED , } cocci_id@p ;
...
@@
 struct fcp_transaction {  struct list_head list ;  struct fw_unit * unit ;  void * response_buffer ;  unsigned int response_size ;  unsigned int response_match_bytes ;  enum fcp_state state ;  wait_queue_head_t wait ;  bool deferrable ; } cocci_id@p ;
...
position p;
@@
 enum avc_general_plug_dir cocci_id@p ;
...
typedef u8;
@@
 u8 cocci_id@p [ AVC_PLUG_INFO_BUF_BYTES ] ;
