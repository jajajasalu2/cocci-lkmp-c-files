...
position p;
@@
 struct stack_record * cocci_id@p [ STACK_HASH_SIZE ] ;
...
typedef u32;
@@
 struct stack_record {  struct stack_record * next ;  u32 hash ;  u32 size ;  union handle_parts handle ;  unsigned long entries [ 1 ] ; } cocci_id@p ;
...
position p;
@@
 struct stack_record * cocci_id@p ;
...
@@
 union handle_parts {  depot_stack_handle_t handle ;  struct {  u32 slabindex : STACK_ALLOC_INDEX_BITS ;  u32 offset : STACK_ALLOC_OFFSET_BITS ;  u32 valid : STACK_ALLOC_NULL_PROTECTION_BITS ; } ; } cocci_id@p ;
...
position p;
@@
 void * cocci_id@p [ STACK_ALLOC_MAX_SLABS ] ;
