...
position p;
@@
 void cocci_id@p ( struct usb_xpad * xpad ) ;
...
position p;
@@
 const struct xboxone_init_packet cocci_id@p [ ] ;
...
position p;
@@
 int cocci_id@p ( struct usb_xpad * xpad ) ;
...
position p;
@@
 struct xpad_output_packet * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,bool,spinlock_t,dma_addr_t;
@@
 struct usb_xpad {  struct input_dev * dev ;  struct input_dev __rcu * x360w_dev ;  struct usb_device * udev ;  struct usb_interface * intf ;  bool pad_present ;  bool input_created ;  struct urb * irq_in ;  unsigned char * idata ;  dma_addr_t idata_dma ;  struct urb * irq_out ;  struct usb_anchor irq_out_anchor ;  bool irq_out_active ;  u8 odata_serial ;  unsigned char * odata ;  dma_addr_t odata_dma ;  spinlock_t odata_lock ;  struct xpad_output_packet out_packets [ XPAD_NUM_OUT_PACKETS ] ;  int last_out_packet ;  int init_seq ; #if defined(CONFIG_JOYSTICK_XPAD_LEDS)  struct xpad_led * led ; #endif  char phys [ 64 ] ;  int mapping ;  int xtype ;  int pad_nr ;  const char * name ;  struct work_struct work ; } cocci_id@p ;
...
typedef u8,u16;
@@
 struct xboxone_init_packet {  u16 idVendor ;  u16 idProduct ;  const u8 * data ;  u8 len ; } cocci_id@p ;
...
typedef u8,u16;
@@
 const struct xpad_device {  u16 idVendor ;  u16 idProduct ;  char * name ;  u8 mapping ;  u8 xtype ; } cocci_id@p [ ] ;
...
position p;
@@
 struct xpad_led {  char name [ 16 ] ;  struct led_classdev led_cdev ;  struct usb_xpad * xpad ; } cocci_id@p ;
...
typedef u8;
@@
 void cocci_id@p ( struct usb_xpad * xpad , u8 seq_num ) ;
...
typedef u8,bool;
@@
 struct xpad_output_packet {  u8 data [ XPAD_PKT_LEN ] ;  u8 len ;  bool pending ; } cocci_id@p ;
