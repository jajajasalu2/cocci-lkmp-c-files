...
position p;
@@
 struct metric_event * cocci_id@p ;
...
position p;
@@
 enum grc_type { GRC_STALLED_CYCLES_FE , GRC_STALLED_CYCLES_BE , GRC_CACHE_MISSES , GRC_MAX_NR , } cocci_id@p ;
...
position p;
@@
 const double cocci_id@p [ GRC_MAX_NR ] [ 3 ] ;
...
position p;
@@
 const struct saved_value * cocci_id@p ;
...
typedef u64;
@@
 struct saved_value {  struct rb_node rb_node ;  struct evsel * evsel ;  enum stat_type type ;  int ctx ;  int cpu ;  struct runtime_stat * stat ;  struct stats stats ;  u64 metric_total ;  int metric_other ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef print_metric_t;
@@
 print_metric_t cocci_id@p ;
