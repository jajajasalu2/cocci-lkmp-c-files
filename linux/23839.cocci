...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_CBS_MAX + 1 ] ;
...
position p;
@@
 const struct tc_cbs_qopt * cocci_id@p ;
...
position p;
@@
 struct cbs_sched_data cocci_id@p ;
...
symbol cocci_id;
position p;
typedef s32,bool,s64,atomic64_t;
@@
 struct cbs_sched_data {  bool offload ;  int queue ;  atomic64_t port_rate ;  s64 last ;  s64 credits ;  s32 locredit ;  s32 hicredit ;  s64 sendslope ;  s64 idleslope ;  struct qdisc_watchdog watchdog ;  int ( * enqueue ) ( struct sk_buff * skb , struct Qdisc * sch , struct sk_buff * * to_free ) ;  struct sk_buff * ( * dequeue ) ( struct Qdisc * sch ) ;  struct Qdisc * qdisc ;  struct list_head cbs_list ; } cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_CBS_MAX + 1 ] ;
