...
position p;
@@
 void cocci_id@p ( struct timespec * ts , int argc , const char * * argv ) ;
...
position p;
@@
 enum counter_recovery { COUNTER_SKIP , COUNTER_RETRY , COUNTER_FATAL , } cocci_id@p ;
...
position p;
@@
 struct runtime_stat cocci_id@p ;
...
position p;
@@
 enum counter_recovery cocci_id@p ;
...
typedef u64,bool;
@@
 struct perf_stat {  bool record ;  struct perf_data data ;  struct perf_session * session ;  u64 bytes_written ;  struct perf_tool tool ;  bool maps_allocated ;  struct perf_cpu_map * cpus ;  struct perf_thread_map * threads ;  enum aggr_mode aggr_mode ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef aggr_get_id_t;
@@
 aggr_get_id_t cocci_id@p ;
