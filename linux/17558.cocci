...
position p;
@@
 struct spu_buffer cocci_id@p [ MAX_NUMNODES * SPUS_PER_NODE ] ;
...
position p;
@@
 struct cached_info * cocci_id@p [ MAX_NUMNODES * 8 ] ;
...
position p;
@@
 struct vma_to_fileoffset_map * cocci_id@p ;
...
position p;
@@
 struct cached_info {  struct vma_to_fileoffset_map * map ;  struct spu * the_spu ;  struct kref cache_ref ; } cocci_id@p ;
