...
position p;
@@
 struct seq_rw_config cocci_id@p [ ] ;
...
position p;
@@
 const struct stfsm_seq * cocci_id@p ;
...
@@
 struct seq_rw_config {  uint32_t flags ;  uint8_t cmd ;  int write ;  uint8_t addr_pads ;  uint8_t data_pads ;  uint8_t mode_data ;  uint8_t mode_cycles ;  uint8_t dummy_cycles ; } cocci_id@p ;
...
typedef u32,u16;
@@
 struct flash_info {  char * name ;  u32 jedec_id ;  u16 ext_id ;  unsigned sector_size ;  u16 n_sectors ;  u32 flags ;  u32 max_freq ;  int ( * config ) ( struct stfsm * ) ; } cocci_id@p ;
...
typedef uint32_t;
@@
 uint32_t cocci_id@p [ FLASH_PAGESIZE_32 ] ;
...
position p;
@@
 int cocci_id@p ( struct stfsm * fsm ) ;
