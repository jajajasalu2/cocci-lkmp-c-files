...
position p;
@@
 char cocci_id@p [ BRCMF_FW_NVRAM_PCIEDEV_LEN ] ;
...
position p;
@@
 void ( * cocci_id@p ) ( struct device * dev , int err , struct brcmf_fw_request * req ) ;
...
position p;
@@
 struct brcmf_fw_item * cocci_id@p ;
...
position p;
@@
 struct nvram_parser * cocci_id@p ;
...
position p;
@@
 enum nvram_parser_state ( * cocci_id@p [ ] ) ( struct nvram_parser * nvp ) ;
...
typedef u32;
@@
 struct brcmf_fw {  struct device * dev ;  struct brcmf_fw_request * req ;  u32 curpos ;  void ( * done ) ( struct device * dev , int err , struct brcmf_fw_request * req ) ; } cocci_id@p ;
...
position p;
@@
 enum nvram_parser_state { IDLE , KEY , VALUE , COMMENT , END , } cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ BRCMF_FW_NAME_LEN ] ;
...
typedef u8,bool,u32;
@@
 struct nvram_parser {  enum nvram_parser_state state ;  const u8 * data ;  u8 * nvram ;  u32 nvram_len ;  u32 line ;  u32 column ;  u32 pos ;  u32 entry ;  bool multi_dev_v1 ;  bool multi_dev_v2 ;  bool boardrev_found ; } cocci_id@p ;
...
position p;
@@
 enum nvram_parser_state cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( const struct firmware * fw , void * ctx ) ;
