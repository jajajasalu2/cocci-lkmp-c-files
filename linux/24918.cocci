...
position p;
@@
 struct udp_media_addr cocci_id@p ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TIPC_NLA_UDP_MAX + 1 ] ;
...
position p;
@@
 struct udp_replicast * cocci_id@p ;
...
typedef u32;
@@
 struct udp_bearer {  struct tipc_bearer __rcu * bearer ;  struct socket * ubsock ;  u32 ifindex ;  struct work_struct work ;  struct udp_replicast rcast ; } cocci_id@p ;
...
position p;
@@
 struct udp_media_addr {  __be16 proto ;  __be16 port ;  union {  struct in_addr ipv4 ;  struct in6_addr ipv6 ; } ; } cocci_id@p ;
...
position p;
@@
 struct udp_replicast {  struct udp_media_addr addr ;  struct dst_cache dst_cache ;  struct rcu_head rcu ;  struct list_head list ; } cocci_id@p ;
