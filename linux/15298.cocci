...
position p;
@@
 struct sbefifo_user {  struct sbefifo * sbefifo ;  struct mutex file_lock ;  void * cmd_page ;  void * pending_cmd ;  size_t pending_len ; } cocci_id@p ;
...
position p;
@@
 enum sbe_state { SBE_STATE_UNKNOWN = 0x0 , SBE_STATE_IPLING = 0x1 , SBE_STATE_ISTEP = 0x2 , SBE_STATE_MPIPL = 0x3 , SBE_STATE_RUNTIME = 0x4 , SBE_STATE_DMT = 0x5 , SBE_STATE_DUMP = 0x6 , SBE_STATE_FAILURE = 0x7 , SBE_STATE_QUIESCE = 0x8 , } cocci_id@p ;
...
position p;
@@
 struct sbefifo_user * cocci_id@p ;
...
position p;
@@
 struct fsi_device_id cocci_id@p [ ] ;
