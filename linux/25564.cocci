...
position p;
@@
 struct rb_node * const * cocci_id@p ;
...
position p;
@@
 enum { COMP_ID__NONE = 0 , } cocci_id@p ;
...
typedef bool;
@@
 const struct {  const char * fmt ;  int ( * decompress ) ( const char * input , int output ) ;  bool ( * is_compressed ) ( const char * input ) ; } cocci_id@p [ ] ;
...
position p;
@@
 enum dso_data_status_seen cocci_id@p ;
