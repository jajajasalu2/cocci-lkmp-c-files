...
position p;
@@
 struct cci_pmu_model * cocci_id@p ;
...
position p;
@@
 enum cci400_perf_events { CCI400_PMU_CYCLES = 0xff , } cocci_id@p ;
...
typedef u32;
@@
 struct event_range {  u32 min ;  u32 max ; } cocci_id@p ;
...
position p;
@@
 const struct cci_pmu_model * cocci_id@p ;
...
position p;
@@
 unsigned long cocci_id@p [ BITS_TO_LONGS ( HW_CNTRS_MAX ) ] ;
...
position p;
@@
 void cocci_id@p ( struct cci_pmu * cci_pmu , unsigned long * mask ) ;
...
@@
 struct cci_pmu_hw_events {  struct perf_event * * events ;  unsigned long * used_mask ;  raw_spinlock_t pmu_lock ; } cocci_id@p ;
...
position p;
@@
 struct cci_pmu_hw_events * cocci_id@p ;
