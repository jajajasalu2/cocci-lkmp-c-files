...
position p;
@@
 const struct clock_topology * cocci_id@p ;
...
typedef u32;
@@
 struct zynqmp_clock {  char clk_name [ MAX_NAME_LEN ] ;  u32 valid ;  enum clk_type type ;  struct clock_topology node [ MAX_NODES ] ;  u32 num_nodes ;  struct clock_parent parent [ MAX_PARENT ] ;  u32 num_parents ;  u32 clk_id ; } cocci_id@p ;
...
typedef u8,u32;
@@
 struct clk_hw * ( * cocci_id@p [ ] ) ( const char * name , u32 clk_id , const char * const * parents , u8 num_parents , const struct clock_topology * nodes ) ;
...
position p;
@@
 struct name_resp {  char name [ CLK_GET_NAME_RESP_LEN ] ; } cocci_id@p ;
...
position p;
@@
 struct topology_resp * cocci_id@p ;
...
position p;
@@
 struct parents_resp cocci_id@p ;
...
position p;
@@
 struct zynqmp_clock * cocci_id@p ;
...
position p;
@@
 struct zynqmp_pm_query_data cocci_id@p ;
...
typedef u32;
@@
 struct clock_parent {  char name [ MAX_NAME_LEN ] ;  int id ;  u32 flag ; } cocci_id@p ;
...
position p;
@@
 enum clk_type { CLK_TYPE_OUTPUT , CLK_TYPE_EXTERNAL , } cocci_id@p ;
...
typedef u32;
@@
 u32 cocci_id@p [ PAYLOAD_ARG_CNT ] ;
...
position p;
@@
 const char * cocci_id@p [ MAX_PARENT ] ;
...
position p;
@@
 struct clock_topology * cocci_id@p ;
...
position p;
@@
 struct clock_parent * cocci_id@p ;
