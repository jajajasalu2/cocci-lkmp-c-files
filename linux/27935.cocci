...
position p;
@@
 int cocci_id@p ( struct super_block * sb , int is_mount ) ;
...
position p;
@@
 enum { Opt_err_cont , Opt_err_panic , Opt_err_ro , Opt_barrier , Opt_nobarrier , Opt_snapshot , Opt_order , Opt_norecovery , Opt_discard , Opt_nodiscard , Opt_err , } cocci_id@p ;
...
position p;
@@
 struct nilfs_btree_path cocci_id@p ;
...
position p;
@@
 struct nilfs_super_data * cocci_id@p ;
...
typedef __u64;
@@
 struct nilfs_super_data {  struct block_device * bdev ;  __u64 cno ;  int flags ; } cocci_id@p ;
