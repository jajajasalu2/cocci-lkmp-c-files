...
position p;
@@
 struct db9_config {  int args [ 2 ] ;  unsigned int nargs ; } cocci_id@p ;
...
position p;
@@
 struct db9_mode_data {  const char * name ;  const short * buttons ;  int n_buttons ;  int n_pads ;  int n_axis ;  int bidirectional ;  int reverse ; } cocci_id@p ;
...
position p;
@@
 const struct db9_mode_data * cocci_id@p ;
...
position p;
@@
 struct db9_config cocci_id@p [ DB9_MAX_PORTS ] ;
...
position p;
@@
 struct db9 {  struct input_dev * dev [ DB9_MAX_DEVICES ] ;  struct timer_list timer ;  struct pardevice * pd ;  int mode ;  int used ;  int parportno ;  struct mutex mutex ;  char phys [ DB9_MAX_DEVICES ] [ 32 ] ; } cocci_id@p ;
