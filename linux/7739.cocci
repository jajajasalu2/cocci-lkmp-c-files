...
position p;
@@
 struct uart_8250_port cocci_id@p [ UART_NR ] ;
...
typedef u32;
@@
 void ( * cocci_id@p ) ( int port , struct uart_port * up , u32 * capabilities ) ;
...
position p;
@@
 const struct old_serial_port cocci_id@p [ ] ;
...
position p;
@@
 unsigned long cocci_id@p [ PORT_RSA_MAX ] ;
...
position p;
@@
 struct hlist_head cocci_id@p [ NR_IRQ_HASH ] ;
...
typedef spinlock_t;
@@
 struct irq_info {  struct hlist_node node ;  int irq ;  spinlock_t lock ;  struct list_head * head ; } cocci_id@p ;
...
position p;
@@
 const struct uart_8250_ops cocci_id@p ;
