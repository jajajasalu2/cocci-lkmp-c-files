...
position p;
@@
 struct dn_dev_parms cocci_id@p [ ] ;
...
position p;
@@
 struct dn_dev_sysctl_table * cocci_id@p ;
...
position p;
@@
 struct dn_dev_sysctl_table {  struct ctl_table_header * sysctl_header ;  struct ctl_table dn_dev_vars [ 5 ] ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( int event , struct dn_ifaddr * ifa ) ;
...
position p;
@@
 void cocci_id@p ( struct net_device * dev , struct dn_ifaddr * ifa ) ;
...
position p;
@@
 struct dn_dev * cocci_id@p ( struct net_device * dev , int * err ) ;
...
position p;
@@
 char cocci_id@p [ sizeof ( "net/decnet/conf/" ) + IFNAMSIZ ] ;
...
position p;
@@
 struct endnode_hello_message * cocci_id@p ;
