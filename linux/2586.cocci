...
typedef uint16_t;
@@
 struct dirty_io {  struct closure cl ;  struct cached_dev * dc ;  uint16_t sequence ;  struct bio bio ; } cocci_id@p ;
...
position p;
@@
 struct sectors_dirty_init * cocci_id@p ;
...
position p;
@@
 struct sectors_dirty_init {  struct btree_op op ;  unsigned int inode ;  size_t count ;  struct bkey start ; } cocci_id@p ;
