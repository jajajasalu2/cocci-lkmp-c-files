...
position p;
@@
 struct image_desc cocci_id@p [ VME_DEVS ] ;
...
position p;
@@
 struct vme_user_vma_priv * cocci_id@p ;
...
typedef dma_addr_t;
@@
 struct image_desc {  void * kern_buf ;  dma_addr_t pci_buf ;  unsigned long long size_buf ;  struct mutex mutex ;  struct device * device ;  struct vme_resource * resource ;  int mmap_count ; } cocci_id@p ;
...
typedef refcount_t;
@@
 struct vme_user_vma_priv {  unsigned int minor ;  refcount_t refcnt ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p [ VME_USER_BUS_MAX ] ;
