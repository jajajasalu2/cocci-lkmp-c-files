...
position p;
@@
 struct swap_map_page * cocci_id@p ;
...
@@
 struct cmp_data {  struct task_struct * thr ;  atomic_t ready ;  atomic_t stop ;  int ret ;  wait_queue_head_t go ;  wait_queue_head_t done ;  size_t unc_len ;  size_t cmp_len ;  unsigned char unc [ LZO_UNC_SIZE ] ;  unsigned char cmp [ LZO_CMP_SIZE ] ;  unsigned char wrk [ LZO1X_1_MEM_COMPRESS ] ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u32,wait_queue_head_t,atomic_t;
@@
 struct crc_data {  struct task_struct * thr ;  atomic_t ready ;  atomic_t stop ;  unsigned run_threads ;  wait_queue_head_t go ;  wait_queue_head_t done ;  u32 * crc32 ;  size_t * unc_len [ LZO_THREADS ] ;  unsigned char * unc [ LZO_THREADS ] ; } cocci_id@p ;
...
position p;
@@
 struct swsusp_extent cocci_id@p ;
...
@@
 struct swap_map_handle {  struct swap_map_page * cur ;  struct swap_map_page_list * maps ;  sector_t cur_swap ;  sector_t first_sector ;  unsigned int k ;  unsigned long reqd_free_pages ;  u32 crc32 ; } cocci_id@p ;
...
position p;
@@
 struct swsusp_header * cocci_id@p ;
...
position p;
@@
 struct swap_map_page_list * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef wait_queue_head_t,atomic_t,blk_status_t;
@@
 struct hib_bio_batch {  atomic_t count ;  wait_queue_head_t wait ;  blk_status_t error ; } cocci_id@p ;
...
position p;
@@
 struct swsusp_extent {  struct rb_node node ;  unsigned long start ;  unsigned long end ; } cocci_id@p ;
...
position p;
@@
 struct snapshot_handle * cocci_id@p ;
...
position p;
@@
 struct swap_map_handle cocci_id@p ;
...
position p;
@@
 struct hib_bio_batch cocci_id@p ;
...
typedef sector_t;
@@
 struct swap_map_page {  sector_t entries [ MAP_PAGE_ENTRIES ] ;  sector_t next_swap ; } cocci_id@p ;
...
position p;
@@
 struct swap_map_page_list {  struct swap_map_page * map ;  struct swap_map_page_list * next ; } cocci_id@p ;
...
typedef wait_queue_head_t,atomic_t;
@@
 struct dec_data {  struct task_struct * thr ;  atomic_t ready ;  atomic_t stop ;  int ret ;  wait_queue_head_t go ;  wait_queue_head_t done ;  size_t unc_len ;  size_t cmp_len ;  unsigned char unc [ LZO_UNC_SIZE ] ;  unsigned char cmp [ LZO_CMP_SIZE ] ; } cocci_id@p ;
