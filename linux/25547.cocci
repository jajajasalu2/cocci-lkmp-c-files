...
position p;
@@
 enum acquire_flags { TRY_LOCK = 1 , READ_LOCK = 2 , } cocci_id@p ;
...
typedef u64;
@@
 struct lock_stat {  struct list_head hash_entry ;  struct rb_node rb ;  void * addr ;  char * name ;  unsigned int nr_acquire ;  unsigned int nr_acquired ;  unsigned int nr_contended ;  unsigned int nr_release ;  unsigned int nr_readlock ;  unsigned int nr_trylock ;  u64 avg_wait_time ;  u64 wait_time_total ;  u64 wait_time_min ;  u64 wait_time_max ;  int discard ; } cocci_id@p ;
...
typedef u32;
@@
 struct thread_stat * cocci_id@p ( u32 tid ) ;
...
typedef u32;
@@
 struct thread_stat {  struct rb_node rb ;  u32 tid ;  struct list_head seq_list ; } cocci_id@p ;
...
position p;
@@
 struct lock_key {  const char * name ;  int ( * key ) ( struct lock_stat * , struct lock_stat * ) ; } cocci_id@p ;
...
position p;
@@
 enum broken_state { BROKEN_ACQUIRE , BROKEN_ACQUIRED , BROKEN_CONTENDED , BROKEN_RELEASE , BROKEN_MAX , } cocci_id@p ;
...
position p;
@@
 struct list_head cocci_id@p [ LOCKHASH_SIZE ] ;
...
position p;
@@
 struct trace_lock_handler cocci_id@p ;
...
position p;
@@
 struct trace_lock_handler {  int ( * acquire_event ) ( struct evsel * evsel , struct perf_sample * sample ) ;  int ( * acquired_event ) ( struct evsel * evsel , struct perf_sample * sample ) ;  int ( * contended_event ) ( struct evsel * evsel , struct perf_sample * sample ) ;  int ( * release_event ) ( struct evsel * evsel , struct perf_sample * sample ) ; } cocci_id@p ;
...
position p;
@@
 struct lock_seq_stat * cocci_id@p ;
...
typedef u64;
@@
 struct lock_seq_stat {  struct list_head list ;  int state ;  u64 prev_event_time ;  void * addr ;  int read_count ; } cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct lock_stat * , struct lock_stat * ) ;
