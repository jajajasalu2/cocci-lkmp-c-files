...
position p;
@@
 enum rtmutex_chainwalk cocci_id@p ;
...
typedef bool;
@@
 bool ( * cocci_id@p ) ( struct rt_mutex * lock , struct wake_q_head * wqh ) ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct rt_mutex * lock ) ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct rt_mutex * lock , int state , struct hrtimer_sleeper * timeout , enum rtmutex_chainwalk chwalk ) ;
