...
position p;
@@
 unsigned long cocci_id@p [ AT_VECTOR_SIZE ] ;
...
position p;
@@
 struct oldold_utsname cocci_id@p ;
...
position p;
@@
 struct getcpu_cache * cocci_id@p ;
...
position p;
@@
 struct prctl_mm_map cocci_id@p ;
...
position p;
@@
 const struct rlimit64 * cocci_id@p ;
...
position p;
@@
 const struct rlimit * cocci_id@p ;
...
typedef u32,u16,s32;
@@
 struct compat_sysinfo {  s32 uptime ;  u32 loads [ 3 ] ;  u32 totalram ;  u32 freeram ;  u32 sharedram ;  u32 bufferram ;  u32 totalswap ;  u32 freeswap ;  u16 procs ;  u16 pad ;  u32 totalhigh ;  u32 freehigh ;  u32 mem_unit ;  char _f [ 20 - 2 * sizeof ( u32 ) - sizeof ( int ) ] ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef compat_clock_t;
@@
 compat_clock_t cocci_id@p ;
...
position p;
@@
 unsigned char cocci_id@p [ sizeof ( me -> comm ) ] ;
