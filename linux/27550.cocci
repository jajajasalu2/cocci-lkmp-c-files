...
typedef u16;
@@
 const struct btrfs_csums {  u16 size ;  const char * name ;  const char * driver ; } cocci_id@p [ ] ;
...
position p;
@@
 void cocci_id@p ( struct btrfs_root * root , struct btrfs_path * path , int level , int slot ) ;
...
typedef u64;
@@
 struct tree_mod_elem {  struct rb_node node ;  u64 logical ;  u64 seq ;  enum mod_log_op op ;  int slot ;  u64 generation ;  struct btrfs_disk_key key ;  u64 blockptr ;  struct {  int dst_slot ;  int nr_items ; } move ;  struct tree_mod_root old_root ; } cocci_id@p ;
...
position p;
@@
 struct tree_mod_root * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct btrfs_trans_handle * trans , struct extent_buffer * dst_buf , struct extent_buffer * src_buf ) ;
...
position p;
@@
 struct tree_mod_elem * * cocci_id@p ;
...
typedef u64,u8;
@@
 struct tree_mod_root {  u64 logical ;  u8 level ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct btrfs_trans_handle * trans , struct btrfs_root * root , struct btrfs_path * path , int level ) ;
...
position p;
@@
 int cocci_id@p ( struct btrfs_trans_handle * trans , struct btrfs_root * root , const struct btrfs_key * ins_key , struct btrfs_path * path , int data_size , int extend ) ;
...
position p;
@@
 enum mod_log_op { MOD_LOG_KEY_REPLACE , MOD_LOG_KEY_ADD , MOD_LOG_KEY_REMOVE , MOD_LOG_KEY_REMOVE_WHILE_FREEING , MOD_LOG_KEY_REMOVE_WHILE_MOVING , MOD_LOG_MOVE_KEYS , MOD_LOG_ROOT_REPLACE , } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct btrfs_trans_handle * trans , struct extent_buffer * dst , struct extent_buffer * src , int empty ) ;
...
position p;
@@
 const struct btrfs_super_block * cocci_id@p ;
