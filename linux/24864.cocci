...
position p;
@@
 const struct nla_policy cocci_id@p [ CTA_PROTOINFO_SCTP_MAX + 1 ] ;
...
position p;
@@
 struct nlattr * cocci_id@p [ CTA_PROTOINFO_SCTP_MAX + 1 ] ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ CTA_TIMEOUT_SCTP_MAX + 1 ] ;
...
position p;
@@
 unsigned long cocci_id@p [ 256 / sizeof ( unsigned long ) ] ;
...
position p;
@@
 const struct sctp_chunkhdr * cocci_id@p ;
...
typedef u8;
@@
 const u8 cocci_id@p [ 2 ] [ 11 ] [ SCTP_CONNTRACK_MAX ] ;
...
position p;
@@
 enum sctp_conntrack cocci_id@p ;
...
position p;
@@
 const struct sctphdr * cocci_id@p ;
