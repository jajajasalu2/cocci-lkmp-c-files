...
position p;
@@
 enum rd_sect_type { RD_NONE , RD_TEST , RD_CMD , RD_GPUADDR , RD_CONTEXT , RD_CMDSTREAM , RD_CMDSTREAM_ADDR , RD_PARAM , RD_FLUSH , RD_PROGRAM , RD_VERT_SHADER , RD_FRAG_SHADER , RD_BUFFER_CONTENTS , RD_GPU_ID , } cocci_id@p ;
...
position p;
@@
 struct msm_rd_state * cocci_id@p ;
...
@@
 struct msm_rd_state {  struct drm_device * dev ;  bool open ;  struct msm_gem_submit * submit ;  struct mutex read_lock ;  wait_queue_head_t fifo_event ;  struct circ_buf fifo ;  char buf [ BUF_SZ ] ; } cocci_id@p ;
