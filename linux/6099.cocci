...
@@
 struct l2cache_pmu {  struct hlist_node node ;  u32 num_pmus ;  struct pmu pmu ;  int num_counters ;  cpumask_t cpumask ;  struct platform_device * pdev ;  struct cluster_pmu * __percpu * pmu_cluster ;  struct list_head clusters ; } cocci_id@p ;
...
@@
 struct cluster_pmu {  struct list_head next ;  struct perf_event * events [ MAX_L2_CTRS ] ;  struct l2cache_pmu * l2cache_pmu ; 
	 ...
	 DECLARE_BITMAP ( used_groups , L2_EVT_GROUP_MAX + 1 ) 
		 ;  int irq ;  int cluster_id ;  int on_cpu ;  cpumask_t cluster_cpus ;  spinlock_t pmu_lock ; } cocci_id@p ;
