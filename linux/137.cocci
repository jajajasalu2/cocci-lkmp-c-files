...
position p;
@@
 enum map_err_types { MAP_ERR_CHECK_NOT_APPLICABLE , MAP_ERR_NOT_CHECKED , MAP_ERR_CHECKED , } cocci_id@p ;
...
position p;
@@
 struct dma_debug_entry cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ NAME_MAX_LEN + 1 ] ;
...
typedef spinlock_t;
@@
 struct hash_bucket {  struct list_head list ;  spinlock_t lock ; } cocci_id@p ;
...
typedef bool;
@@
 bool ( * cocci_id@p ) ( struct dma_debug_entry * , struct dma_debug_entry * ) ;
...
position p;
@@
 struct hash_bucket cocci_id@p [ HASH_SIZE ] ;
...
position p;
@@
 struct dma_debug_entry * cocci_id@p [ CACHELINES_PER_PAGE ] ;
...
position p;
@@
 enum { dma_debug_single , dma_debug_sg , dma_debug_coherent , dma_debug_resource , } cocci_id@p ;
