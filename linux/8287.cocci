...
typedef u32,ktime_t;
@@
 struct efx_ptp_event_rx {  struct list_head link ;  u32 seq0 ;  u32 seq1 ;  ktime_t hwtimestamp ;  unsigned long expiry ; } cocci_id@p ;
...
position p;
@@
 enum ptp_packet_state cocci_id@p ;
...
position p;
@@
 struct efx_ptp_data * cocci_id@p ;
...
position p;
@@
 struct pps_event_time * cocci_id@p ;
...
typedef s64;
@@
 int cocci_id@p ( struct ptp_clock_info * ptp , s64 delta ) ;
...
position p;
@@
 int cocci_id@p ( struct ptp_clock_info * ptp , const struct timespec64 * e_ts ) ;
...
position p;
@@
 int cocci_id@p ( struct ptp_clock_info * ptp , struct timespec64 * ts ) ;
...
position p;
@@
 struct efx_ptp_event_rx * cocci_id@p ;
...
position p;
@@
 struct efx_ptp_match * cocci_id@p ;
...
position p;
@@
 enum ptp_packet_state { PTP_PACKET_STATE_UNMATCHED = 0 , PTP_PACKET_STATE_MATCHED , PTP_PACKET_STATE_TIMED_OUT , PTP_PACKET_STATE_MATCH_UNWANTED , } cocci_id@p ;
...
typedef s32;
@@
 int cocci_id@p ( struct ptp_clock_info * ptp , s32 delta ) ;
...
typedef u32;
@@
 struct efx_ptp_timeset {  u32 host_start ;  u32 major ;  u32 minor ;  u32 host_end ;  u32 wait ;  u32 window ; } cocci_id@p ;
...
position p;
@@
 const struct efx_hw_stat_desc cocci_id@p [ ] ;
...
position p;
@@
 int cocci_id@p ( struct ptp_clock_info * ptp , struct ptp_clock_request * request , int on ) ;
