...
symbol cocci_id;
position p;
typedef cpuset_filetype_t;
@@
 cpuset_filetype_t cocci_id@p ;
...
position p;
@@
 struct sched_domain_attr * cocci_id@p ;
...
position p;
@@
 enum { CS_ONLINE , CS_CPU_EXCLUSIVE , CS_MEM_EXCLUSIVE , CS_MEM_HARDWALL , CS_MEMORY_MIGRATE , CS_SCHED_LOAD_BALANCE , CS_SPREAD_PAGE , CS_SPREAD_SLAB , } cocci_id@p ;
...
position p;
@@
 enum subparts_cmd { partcmd_enable , partcmd_disable , partcmd_update , } cocci_id@p ;
...
position p;
@@
 const struct cpuset * cocci_id@p ;
...
position p;
@@
 struct cpuset_migrate_mm_work cocci_id@p ;
...
typedef nodemask_t;
@@
 struct cpuset_migrate_mm_work {  struct work_struct work ;  struct mm_struct * mm ;  nodemask_t from ;  nodemask_t to ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef cpuset_flagbits_t;
@@
 cpuset_flagbits_t cocci_id@p ;
...
symbol cocci_id;
position p;
typedef time64_t,spinlock_t;
@@
 struct fmeter {  int cnt ;  int val ;  time64_t time ;  spinlock_t lock ; } cocci_id@p ;
