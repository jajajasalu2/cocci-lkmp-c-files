...
position p;
@@
 int cocci_id@p ( struct pg * dev , int log ) ;
...
position p;
@@
 struct pg {  struct pi_adapter pia ;  struct pi_adapter * pi ;  int busy ;  int start ;  int dlen ;  unsigned long timeout ;  int status ;  int drive ;  unsigned long access ;  int present ;  char * bufptr ;  char name [ PG_NAMELEN ] ; } cocci_id@p ;
...
position p;
@@
 struct pg_write_hdr cocci_id@p ;
