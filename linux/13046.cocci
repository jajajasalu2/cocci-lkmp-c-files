...
typedef uint8_t;
@@
 struct apr_rx_buf {  struct list_head node ;  int len ;  uint8_t buf [ ] ; } cocci_id@p ;
...
typedef spinlock_t;
@@
 struct apr {  struct rpmsg_endpoint * ch ;  struct device * dev ;  spinlock_t svcs_lock ;  spinlock_t rx_lock ;  struct idr svcs_idr ;  int dest_domain_id ;  struct workqueue_struct * rxwq ;  struct work_struct rx_work ;  struct list_head rx_list ; } cocci_id@p ;
...
position p;
@@
 const struct apr_device_id * cocci_id@p ;
...
position p;
@@
 struct apr_device_id cocci_id@p ;
