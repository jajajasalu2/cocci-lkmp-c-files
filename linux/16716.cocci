...
position p;
@@
 struct gpio_keys_polled_dev {  struct input_dev * input ;  struct device * dev ;  const struct gpio_keys_platform_data * pdata ;  unsigned long rel_axis_seen [ BITS_TO_LONGS ( REL_CNT ) ] ;  unsigned long abs_axis_seen [ BITS_TO_LONGS ( ABS_CNT ) ] ;  struct gpio_keys_button_data data [ 0 ] ; } cocci_id@p ;
...
position p;
@@
 struct gpio_keys_polled_dev * cocci_id@p ;
...
position p;
@@
 struct gpio_keys_button_data * cocci_id@p ;
...
position p;
@@
 struct gpio_keys_button_data {  struct gpio_desc * gpiod ;  int last_state ;  int count ;  int threshold ; } cocci_id@p ;
