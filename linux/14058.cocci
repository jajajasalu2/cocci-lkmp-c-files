...
position p;
@@
 struct vdec_h264_inst * cocci_id@p ;
...
position p;
@@
 struct h264_ring_fb_list * cocci_id@p ;
...
position p;
@@
 struct vdec_h264_vsi * cocci_id@p ;
...
position p;
@@
 struct h264_ring_fb_list {  struct h264_fb fb_list [ H264_MAX_FB_NUM ] ;  unsigned int read_idx ;  unsigned int write_idx ;  unsigned int count ;  unsigned int reserved ; } cocci_id@p ;
...
@@
 struct vdec_h264_dec_info {  uint32_t dpb_sz ;  uint32_t resolution_changed ;  uint32_t realloc_mv_buf ;  uint32_t reserved ;  uint64_t bs_dma ;  uint64_t y_fb_dma ;  uint64_t c_fb_dma ;  uint64_t vdec_fb_va ; } cocci_id@p ;
...
typedef uint64_t;
@@
 struct vdec_h264_vsi {  unsigned char hdr_buf [ HDR_PARSING_BUF_SZ ] ;  uint64_t pred_buf_dma ;  uint64_t mv_buf_dma [ H264_MAX_FB_NUM ] ;  struct h264_ring_fb_list list_free ;  struct h264_ring_fb_list list_disp ;  struct vdec_h264_dec_info dec ;  struct vdec_pic_info pic ;  struct v4l2_rect crop ; } cocci_id@p ;
...
position p;
@@
 struct vdec_h264_inst {  unsigned int num_nalu ;  struct mtk_vcodec_ctx * ctx ;  struct mtk_vcodec_mem pred_buf ;  struct mtk_vcodec_mem mv_buf [ H264_MAX_FB_NUM ] ;  struct vdec_vpu_inst vpu ;  struct vdec_h264_vsi * vsi ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef int32_t,uint64_t,uint32_t;
@@
 struct h264_fb {  uint64_t vdec_fb_va ;  uint64_t y_fb_dma ;  uint64_t c_fb_dma ;  int32_t poc ;  uint32_t reserved ; } cocci_id@p ;
