...
position p;
@@
 struct vdic_pipeline_ops cocci_id@p ;
...
typedef bool,u32;
@@
 struct vdic_priv {  struct device * ipu_dev ;  struct ipu_soc * ipu ;  struct v4l2_subdev sd ;  struct media_pad pad [ VDIC_NUM_PADS ] ;  struct mutex lock ;  struct ipu_vdi * vdi ;  int active_input_pad ;  struct ipuv3_channel * vdi_in_ch_p ;  struct ipuv3_channel * vdi_in_ch ;  struct ipuv3_channel * vdi_in_ch_n ;  struct vdic_pipeline_ops * ops ;  struct imx_media_buffer * curr_in_buf ;  struct imx_media_buffer * prev_in_buf ;  u32 fieldtype ;  u32 in_stride ;  u32 field_size ;  struct media_entity * src ;  struct v4l2_subdev * sink_sd ;  struct v4l2_mbus_framefmt format_mbus [ VDIC_NUM_PADS ] ;  const struct imx_media_pixfmt * cc [ VDIC_NUM_PADS ] ;  struct v4l2_fract frame_interval [ VDIC_NUM_PADS ] ;  struct imx_media_video_dev * vdev ;  bool csi_direct ;  struct v4l2_ctrl_handler ctrl_hdlr ;  enum ipu_motion_sel motion ;  int stream_count ; } cocci_id@p ;
...
position p;
@@
 enum ipu_motion_sel cocci_id@p ;
...
position p;
@@
 struct vdic_pipeline_ops {  int ( * setup ) ( struct vdic_priv * priv ) ;  void ( * start ) ( struct vdic_priv * priv ) ;  void ( * stop ) ( struct vdic_priv * priv ) ;  void ( * disable ) ( struct vdic_priv * priv ) ; } cocci_id@p ;
