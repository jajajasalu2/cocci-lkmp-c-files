...
position p;
@@
 unsigned int cocci_id@p [ VERSION_ELEMENTS ] ;
...
position p;
@@
 enum { PCODE = 0 , SUBSTRATE , DVFS_MAX_REGFIELDS , } cocci_id@p ;
...
position p;
@@
 struct sti_cpufreq_ddata {  struct device * cpu ;  struct regmap * syscfg_eng ;  struct regmap * syscfg ; } cocci_id@p ;
...
position p;
@@
 const struct reg_field cocci_id@p [ DVFS_MAX_REGFIELDS ] ;
...
position p;
@@
 char cocci_id@p [ MAX_PCODE_NAME_LEN ] ;
