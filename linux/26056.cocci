...
position p;
@@
 struct prog_test_def cocci_id@p [ ] ;
...
typedef bool;
@@
 struct prog_test_def {  const char * test_name ;  int test_num ;  void ( * run_test ) ( void ) ;  bool force_log ;  int error_cnt ;  int skip_cnt ;  bool tested ;  bool need_cgroup_cleanup ;  char * subtest_name ;  int subtest_num ;  int old_error_cnt ; } cocci_id@p ;
...
position p;
@@
 struct test_selector * cocci_id@p ;
...
position p;
@@
 const struct prog_test_def * cocci_id@p ;
...
position p;
@@
 enum ARG_KEYS { ARG_TEST_NUM = 'n' , ARG_TEST_NAME = 't' , ARG_VERIFIER_STATS = 's' , ARG_VERBOSE = 'v' , } cocci_id@p ;
...
position p;
@@
 const struct argp_option cocci_id@p [ ] ;
...
position p;
@@
 char cocci_id@p [ PERF_MAX_STACK_DEPTH * sizeof ( struct bpf_stack_build_id ) ] ;
