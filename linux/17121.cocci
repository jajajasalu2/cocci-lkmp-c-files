...
position p;
@@
 struct cpu_messages {  long messages ; } cocci_id@p ;
...
typedef u32;
@@
 u32 cocci_id@p [ 3 + MAX_THREAD_LIST_SIZE ] ;
...
position p;
@@
 struct thread_groups cocci_id@p ;
...
position p;
@@
 struct thread_groups {  unsigned int property ;  unsigned int nr_groups ;  unsigned int threads_per_group ;  unsigned int thread_list [ MAX_THREAD_LIST_SIZE ] ; } cocci_id@p ;
...
position p;
@@
 struct cpu_messages cocci_id@p ;
...
position p;
@@
 volatile unsigned int cocci_id@p [ NR_CPUS ] ;
