...
position p;
@@
 struct segment_info * * cocci_id@p ;
...
position p;
@@
 struct dcssblk_dev_info cocci_id@p ;
...
typedef atomic_t;
@@
 struct dcssblk_dev_info {  struct list_head lh ;  struct device dev ;  char segment_name [ DCSS_BUS_ID_SIZE ] ;  atomic_t use_count ;  struct gendisk * gd ;  unsigned long start ;  unsigned long end ;  int segment_type ;  unsigned char save_pending ;  unsigned char is_shared ;  struct request_queue * dcssblk_queue ;  int num_of_segments ;  struct list_head seg_list ;  struct dax_device * dax_dev ; } cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ DCSSBLK_PARM_LEN + 1 ] ;
...
position p;
@@
 char cocci_id@p [ DCSSBLK_PARM_LEN ] ;
...
position p;
@@
 struct segment_info {  struct list_head lh ;  char segment_name [ DCSS_BUS_ID_SIZE ] ;  unsigned long start ;  unsigned long end ;  int segment_type ; } cocci_id@p ;
...
typedef blk_qc_t;
@@
 blk_qc_t cocci_id@p ( struct request_queue * q , struct bio * bio ) ;
