...
position p;
@@
 const struct scp_ctrl_reg * cocci_id@p ;
...
position p;
@@
 const struct scp_subdomain * cocci_id@p ;
...
typedef bool;
@@
 struct scp_soc_data {  const struct scp_domain_data * domains ;  int num_domains ;  const struct scp_subdomain * subdomains ;  int num_subdomains ;  const struct scp_ctrl_reg regs ;  bool bus_prot_reg_update ; } cocci_id@p ;
...
typedef bool;
@@
 struct scp {  struct scp_domain * domains ;  struct genpd_onecell_data pd_data ;  struct device * dev ;  void __iomem * base ;  struct regmap * infracfg ;  struct scp_ctrl_reg ctrl_reg ;  bool bus_prot_reg_update ; } cocci_id@p ;
...
position p;
@@
 struct scp_ctrl_reg {  int pwr_sta_offs ;  int pwr_sta2nd_offs ; } cocci_id@p ;
...
position p;
@@
 struct genpd_onecell_data * cocci_id@p ;
...
position p;
@@
 const struct scp_domain_data * cocci_id@p ;
...
position p;
@@
 enum clk_id { CLK_NONE , CLK_MM , CLK_MFG , CLK_VENC , CLK_VENC_LT , CLK_ETHIF , CLK_VDEC , CLK_HIFSEL , CLK_JPGDEC , CLK_AUDIO , CLK_MAX , } cocci_id@p ;
...
typedef u8,u32;
@@
 struct scp_domain_data {  const char * name ;  u32 sta_mask ;  int ctl_offs ;  u32 sram_pdn_bits ;  u32 sram_pdn_ack_bits ;  u32 bus_prot_mask ;  enum clk_id clk_id [ MAX_CLKS ] ;  u8 caps ; } cocci_id@p ;
...
position p;
@@
 const struct scp_soc_data cocci_id@p ;
...
position p;
@@
 struct scp_subdomain {  int origin ;  int subdomain ; } cocci_id@p ;
...
position p;
@@
 struct scp_domain {  struct generic_pm_domain genpd ;  struct scp * scp ;  struct clk * clk [ MAX_CLKS ] ;  const struct scp_domain_data * data ;  struct regulator * supply ; } cocci_id@p ;
