...
typedef u64,u32,u16;
@@
 struct mqprio_sched {  struct Qdisc * * qdiscs ;  u16 mode ;  u16 shaper ;  int hw_offload ;  u32 flags ;  u64 min_rate [ TC_QOPT_MAX_QUEUE ] ;  u64 max_rate [ TC_QOPT_MAX_QUEUE ] ; } cocci_id@p ;
...
position p;
@@
 struct netdev_tc_txq cocci_id@p ;
...
position p;
@@
 struct mqprio_sched * cocci_id@p ;
...
position p;
@@
 struct gnet_stats_basic_packed cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_MQPRIO_MAX + 1 ] ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_MQPRIO_MAX + 1 ] ;
