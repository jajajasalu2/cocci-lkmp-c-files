...
position p;
@@
 enum { ACPI_BATTERY_ALARM_PRESENT , ACPI_BATTERY_XINFO_PRESENT , ACPI_BATTERY_QUIRK_PERCENTAGE_CAPACITY , ACPI_BATTERY_QUIRK_THINKPAD_MAH , ACPI_BATTERY_QUIRK_DEGRADED_FULL_CHARGE , } cocci_id@p ;
...
position p;
@@
 const struct acpi_offsets cocci_id@p [ ] ;
...
position p;
@@
 const struct acpi_battery * cocci_id@p ;
...
typedef u8;
@@
 struct acpi_offsets {  size_t offset ;  u8 mode ; } cocci_id@p ;
...
position p;
@@
 struct proc_dir_entry * cocci_id@p ( void ) ;
...
position p;
@@
 void * cocci_id@p ( struct proc_dir_entry * acpi_battery_dir ) ;
