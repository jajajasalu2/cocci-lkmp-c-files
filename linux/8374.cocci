...
position p;
@@
 struct nfp_cpp_area_cache cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,u32,u16,wait_queue_head_t,rwlock_t;
@@
 struct nfp_cpp {  struct device dev ;  void * priv ;  u32 model ;  u16 interface ;  u8 serial [ NFP_SERIAL_LEN ] ;  const struct nfp_cpp_operations * op ;  struct list_head resource_list ;  rwlock_t resource_lock ;  wait_queue_head_t waitq ;  u32 imb_cat_table [ 16 ] ;  unsigned int mu_locality_lsb ;  struct mutex area_cache_mutex ;  struct list_head area_cache_list ; } cocci_id@p ;
...
position p;
@@
 struct nfp_cpp_explicit {  struct nfp_cpp * cpp ;  struct nfp_cpp_explicit_command cmd ; } cocci_id@p ;
...
position p;
@@
 enum nfp_cpp_explicit_signal_mode cocci_id@p ;
...
position p;
@@
 struct nfp_cpp_resource * cocci_id@p ;
...
typedef atomic_t;
@@
 struct nfp_cpp_area {  struct nfp_cpp * cpp ;  struct kref kref ;  atomic_t refcount ;  struct mutex mutex ;  unsigned long long offset ;  unsigned long size ;  struct nfp_cpp_resource resource ;  void __iomem * iomem ; } cocci_id@p ;
...
typedef u64,u32;
@@
 struct nfp_cpp_area_cache {  struct list_head entry ;  u32 id ;  u64 addr ;  u32 size ;  struct nfp_cpp_area * area ; } cocci_id@p ;
...
typedef u64,u32;
@@
 struct nfp_cpp_resource {  struct list_head list ;  const char * name ;  u32 cpp_id ;  u64 start ;  u64 end ; } cocci_id@p ;
