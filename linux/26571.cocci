...
typedef u8,bool,u16;
@@
 struct serial_port {  u8 uart_reg [ 8 ] ;  struct rxtx rxtx ;  bool dlab ;  bool overrun ;  u16 divisor ;  u8 fcr ;  u8 max_fifo_size ;  u8 intr_trigger_level ; } cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ MTTY_STRING_LEN ] ;
...
typedef dev_t;
@@
 struct mtty_dev {  dev_t vd_devt ;  struct class * vd_class ;  struct cdev vd_cdev ;  struct idr vd_idr ;  struct device dev ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct mdev_state * mdev_state ) ;
...
typedef u64,u32;
@@
 struct mdev_region_info {  u64 start ;  u64 phys_start ;  u32 size ;  u64 vfio_offset ; } cocci_id@p ;
...
typedef u8,u32;
@@
 struct mdev_state {  int irq_fd ;  struct eventfd_ctx * intx_evtfd ;  struct eventfd_ctx * msi_evtfd ;  int irq_index ;  u8 * vconfig ;  struct mutex ops_lock ;  struct mdev_device * mdev ;  struct mdev_region_info region_info [ VFIO_PCI_NUM_REGIONS ] ;  u32 bar_mask [ VFIO_PCI_NUM_REGIONS ] ;  struct list_head next ;  struct serial_port s [ 2 ] ;  struct mutex rxtx_lock ;  struct vfio_device_info dev_info ;  int nr_ports ; } cocci_id@p ;
