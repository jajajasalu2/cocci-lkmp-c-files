...
symbol cocci_id;
position p;
typedef wait_queue_head_t,dev_t,spinlock_t;
@@
 struct axis_fifo {  int irq ;  void __iomem * base_addr ;  unsigned int rx_fifo_depth ;  unsigned int tx_fifo_depth ;  int has_rx_fifo ;  int has_tx_fifo ;  wait_queue_head_t read_queue ;  spinlock_t read_queue_lock ;  wait_queue_head_t write_queue ;  spinlock_t write_queue_lock ;  unsigned int write_flags ;  unsigned int read_flags ;  struct device * dt_device ;  struct device * device ;  dev_t devt ;  struct cdev char_device ; } cocci_id@p ;
