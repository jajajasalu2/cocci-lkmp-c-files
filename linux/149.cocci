...
position p;
@@
 struct hlist_head cocci_id@p [ CHAINHASH_SIZE ] ;
...
position p;
@@
 int cocci_id@p ( const struct lockdep_map * lock , int read ) ;
...
position p;
@@
 int cocci_id@p ( struct task_struct * curr , struct held_lock * this , enum lock_usage_bit new_bit ) ;
...
position p;
@@
 const struct lock_class_key * cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct task_struct * , struct held_lock * , enum lock_usage_bit bit , const char * name ) ;
...
position p;
@@
 const struct lockdep_subclass_key * cocci_id@p ;
...
position p;
@@
 struct lock_class_stats cocci_id@p ;
...
position p;
@@
 struct hlist_head cocci_id@p [ STACK_TRACE_HASH_SIZE ] ;
...
position p;
@@
 struct circular_queue cocci_id@p ;
...
position p;
@@
 struct hlist_head cocci_id@p [ CLASSHASH_SIZE ] ;
...
position p;
@@
 struct lock_class cocci_id@p [ MAX_LOCKDEP_KEYS ] ;
...
position p;
@@
 const struct lock_class * cocci_id@p ;
...
position p;
@@
 struct lock_list cocci_id@p [ MAX_LOCKDEP_ENTRIES ] ;
...
position p;
@@
 char cocci_id@p [ LOCK_USAGE_CHARS ] ;
...
position p;
@@
 struct lock_chain cocci_id@p [ MAX_LOCKDEP_CHAINS ] ;
...
position p;
@@
 unsigned long cocci_id@p [ MAX_STACK_TRACE_ENTRIES ] ;
...
position p;
@@
 struct pending_free * cocci_id@p ;
...
position p;
@@
 enum lock_usage_bit cocci_id@p ;
...
position p;
@@
 struct hlist_head cocci_id@p [ KEYHASH_SIZE ] ;
...
position p;
@@
 const struct lock_trace * cocci_id@p ;
...
position p;
@@
 struct lock_trace * * const cocci_id@p ;
...
typedef u16;
@@
 u16 cocci_id@p [ MAX_LOCKDEP_CHAIN_HLOCKS ] ;
...
position p;
@@
 struct lockdep_subclass_key * cocci_id@p ;
...
position p;
@@
 struct delayed_free {  struct rcu_head rcu_head ;  int index ;  int scheduled ;  struct pending_free pf [ 2 ] ; } cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct lock_list * entry , void * data ) ;
...
position p;
@@
 const struct held_lock * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct rcu_head * cb ) ;
...
position p;
@@
 const struct lockdep_map * cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p [ ] ) ( struct lock_class * class ) ;
