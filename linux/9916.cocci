...
position p;
@@
 struct ath6kl_usb_pipe * cocci_id@p ;
...
position p;
@@
 struct ath6kl_usb_ctrl_diag_resp_read * cocci_id@p ;
...
position p;
@@
 struct ath6kl_usb_ctrl_diag_cmd_write * cocci_id@p ;
...
typedef u8,u32,u16;
@@
 struct ath6kl_usb_pipe {  struct list_head urb_list_head ;  struct usb_anchor urb_submitted ;  u32 urb_alloc ;  u32 urb_cnt ;  u32 urb_cnt_thresh ;  unsigned int usb_pipe_handle ;  u32 flags ;  u8 ep_address ;  u8 logical_pipe_num ;  struct ath6kl_usb * ar_usb ;  u16 max_packet_size ;  struct work_struct io_complete_work ;  struct sk_buff_head io_comp_queue ;  struct usb_endpoint_descriptor * ep_desc ; } cocci_id@p ;
...
position p;
@@
 struct ath6kl_urb_context * cocci_id@p ;
...
@@
 struct ath6kl_usb {  spinlock_t cs_lock ;  struct usb_device * udev ;  struct usb_interface * interface ;  struct ath6kl_usb_pipe pipes [ ATH6KL_USB_PIPE_MAX ] ;  u8 * diag_cmd_buffer ;  u8 * diag_resp_buffer ;  struct ath6kl * ar ; } cocci_id@p ;
...
position p;
@@
 struct ath6kl_usb_ctrl_diag_cmd_read * cocci_id@p ;
...
position p;
@@
 struct ath6kl_urb_context {  struct list_head link ;  struct ath6kl_usb_pipe * pipe ;  struct sk_buff * skb ;  struct ath6kl * ar ; } cocci_id@p ;
...
position p;
@@
 enum ATH6KL_USB_PIPE_ID { ATH6KL_USB_PIPE_TX_CTRL = 0 , ATH6KL_USB_PIPE_TX_DATA_LP , ATH6KL_USB_PIPE_TX_DATA_MP , ATH6KL_USB_PIPE_TX_DATA_HP , ATH6KL_USB_PIPE_RX_CTRL , ATH6KL_USB_PIPE_RX_DATA , ATH6KL_USB_PIPE_RX_DATA2 , ATH6KL_USB_PIPE_RX_INT , ATH6KL_USB_PIPE_MAX , } cocci_id@p ;
...
position p;
@@
 const struct ath6kl_hif_ops cocci_id@p ;
