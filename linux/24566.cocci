...
position p;
@@
 void cocci_id@p ( struct cflayer * layr , enum caif_ctrlcmd ctrl , int phyid ) ;
...
typedef u16;
@@
 struct cflayer * cocci_id@p ( struct cfmuxl * muxl , u16 id ) ;
...
position p;
@@
 int cocci_id@p ( struct cflayer * layr , struct cfpkt * pkt ) ;
...
typedef spinlock_t;
@@
 struct cfmuxl {  struct cflayer layer ;  struct list_head srvl_list ;  struct list_head frml_list ;  struct cflayer * up_cache [ UP_CACHE_SIZE ] ;  struct cflayer * dn_cache [ DN_CACHE_SIZE ] ;  spinlock_t transmit_lock ;  spinlock_t receive_lock ; } cocci_id@p ;
