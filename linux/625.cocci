...
position p;
@@
 struct adiantum_tfm_ctx {  struct crypto_skcipher * streamcipher ;  struct crypto_cipher * blockcipher ;  struct crypto_shash * hash ;  struct poly1305_key header_hash_key ; } cocci_id@p ;
...
position p;
@@
 struct adiantum_request_ctx * cocci_id@p ;
...
position p;
@@
 struct adiantum_tfm_ctx cocci_id@p ;
...
typedef u8;
@@
 struct {  u8 iv [ XCHACHA_IV_SIZE ] ;  u8 derived_keys [ BLOCKCIPHER_KEY_SIZE + HASH_KEY_SIZE ] ;  struct scatterlist sg ;  struct crypto_wait wait ;  struct skcipher_request req ; } * cocci_id@p ;
...
typedef __le64;
@@
 struct {  __le64 message_bits ;  __le64 padding ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef __le32,u8,le128,bool;
@@
 struct adiantum_request_ctx {  union {  u8 bytes [ XCHACHA_IV_SIZE ] ;  __le32 words [ XCHACHA_IV_SIZE / sizeof ( __le32 ) ] ;  le128 bignum ; } rbuf ;  bool enc ;  le128 header_hash ;  union {  struct shash_desc hash_desc ;  struct skcipher_request streamcipher_req ; } u ; } cocci_id@p ;
...
position p;
@@
 struct adiantum_instance_ctx * cocci_id@p ;
...
position p;
@@
 struct poly1305_state cocci_id@p ;
...
position p;
@@
 struct adiantum_instance_ctx {  struct crypto_skcipher_spawn streamcipher_spawn ;  struct crypto_spawn blockcipher_spawn ;  struct crypto_shash_spawn hash_spawn ; } cocci_id@p ;
...
position p;
@@
 const struct adiantum_tfm_ctx * cocci_id@p ;
