...
position p;
@@
 struct xfs_iext_leaf {  struct xfs_iext_rec recs [ RECS_PER_LEAF ] ;  struct xfs_iext_leaf * prev ;  struct xfs_iext_leaf * next ; } cocci_id@p ;
...
position p;
@@
 struct xfs_iext_node * cocci_id@p ;
...
position p;
@@
 struct xfs_iext_rec cocci_id@p ;
...
typedef uint64_t;
@@
 struct xfs_iext_rec {  uint64_t lo ;  uint64_t hi ; } cocci_id@p ;
...
position p;
@@
 struct xfs_iext_leaf * cocci_id@p ;
...
typedef uint64_t;
@@
 enum { NODE_SIZE = 256 , KEYS_PER_NODE = NODE_SIZE / ( sizeof ( uint64_t ) + sizeof ( void * ) ) , RECS_PER_LEAF = ( NODE_SIZE - ( 2 * sizeof ( struct xfs_iext_leaf * ) ) ) / sizeof ( struct xfs_iext_rec ) , } cocci_id@p ;
