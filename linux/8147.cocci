...
position p;
@@
 struct pcmcia_device_id cocci_id@p [ ] ;
...
typedef u8;
@@
 struct pcan_pccard {  struct pcmcia_device * pdev ;  int chan_count ;  struct pcan_channel channel [ PCC_CHAN_MAX ] ;  u8 ccr ;  u8 fw_major ;  u8 fw_minor ;  void __iomem * ioport_addr ;  struct timer_list led_timer ; } cocci_id@p ;
...
typedef u8;
@@
 void cocci_id@p ( struct pcan_pccard * card , u8 mask , u8 state ) ;
...
position p;
@@
 struct pcan_channel {  struct net_device * netdev ;  unsigned long prev_rx_bytes ;  unsigned long prev_tx_bytes ; } cocci_id@p ;
