...
position p;
@@
 const struct fsl_mmdc_devtype_data cocci_id@p ;
...
position p;
@@
 struct fsl_mmdc_devtype_data {  unsigned int flags ; } cocci_id@p ;
...
position p;
@@
 struct fsl_mmdc_devtype_data * cocci_id@p ;
...
typedef cpumask_t;
@@
 struct mmdc_pmu {  struct pmu pmu ;  void __iomem * mmdc_base ;  cpumask_t cpu ;  struct hrtimer hrtimer ;  unsigned int active_events ;  struct device * dev ;  struct perf_event * mmdc_events [ MMDC_NUM_COUNTERS ] ;  struct hlist_node node ;  struct fsl_mmdc_devtype_data * devtype_data ; } cocci_id@p ;
