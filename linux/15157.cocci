...
position p;
@@
 struct ssi_protocol * cocci_id@p ;
...
position p;
@@
 enum { INIT , HANDSHAKE , ACTIVE , } cocci_id@p ;
...
@@
 struct ssi_protocol {  unsigned int main_state ;  unsigned int send_state ;  unsigned int recv_state ;  unsigned long flags ;  u8 rxid ;  u8 txid ;  unsigned int txqueue_len ;  struct timer_list tx_wd ;  struct timer_list rx_wd ;  struct timer_list keep_alive ;  spinlock_t lock ;  struct net_device * netdev ;  struct list_head txqueue ;  struct list_head cmdqueue ;  struct work_struct work ;  struct hsi_client * cl ;  struct list_head link ;  atomic_t tx_usecnt ;  int channel_id_cmd ;  int channel_id_data ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct hsi_msg * msg ) ;
...
position p;
@@
 void cocci_id@p ( struct hsi_client * cl , unsigned int enable ) ;
...
position p;
@@
 enum { SEND_IDLE , WAIT4READY , SEND_READY , SENDING , SENDING_SWBREAK , } cocci_id@p ;
...
position p;
@@
 enum { RECV_IDLE , RECV_READY , RECEIVING , } cocci_id@p ;
