...
position p;
@@
 struct cio_dac_iio * const cocci_id@p ;
...
position p;
@@
 const struct iio_chan_spec cocci_id@p [ CIO_DAC_NUM_CHAN ] ;
...
position p;
@@
 unsigned int cocci_id@p [ max_num_isa_dev ( CIO_DAC_EXTENT ) ] ;
...
position p;
@@
 struct cio_dac_iio {  int chan_out_states [ CIO_DAC_NUM_CHAN ] ;  unsigned int base ; } cocci_id@p ;
