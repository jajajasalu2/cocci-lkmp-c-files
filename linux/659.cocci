...
position p;
@@
 int cocci_id@p ( struct skcipher_walk * walk ) ;
...
position p;
@@
 struct skcipher_ctx_simple cocci_id@p ;
...
position p;
@@
 struct crypto_report_blkcipher cocci_id@p ;
...
position p;
@@
 struct skcipher_walk_buffer * cocci_id@p ;
...
typedef u8;
@@
 struct skcipher_walk_buffer {  struct list_head entry ;  struct scatter_walk dst ;  unsigned int len ;  u8 * data ;  u8 buffer [ ] ; } cocci_id@p ;
...
position p;
@@
 enum { SKCIPHER_WALK_PHYS = 1 << 0 , SKCIPHER_WALK_SLOW = 1 << 1 , SKCIPHER_WALK_COPY = 1 << 2 , SKCIPHER_WALK_DIFF = 1 << 3 , SKCIPHER_WALK_SLEEP = 1 << 4 , } cocci_id@p ;
...
position p;
@@
 const struct crypto_type cocci_id@p ;
