...
position p;
@@
 enum con_flush_mode cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ LOG_LINE_MAX + PREFIX_MAX ] ;
...
position p;
@@
 const struct printk_log * cocci_id@p ;
...
position p;
@@
 enum devkmsg_log_masks { DEVKMSG_LOG_MASK_ON = BIT ( __DEVKMSG_LOG_BIT_ON ) , DEVKMSG_LOG_MASK_OFF = BIT ( __DEVKMSG_LOG_BIT_OFF ) , DEVKMSG_LOG_MASK_LOCK = BIT ( __DEVKMSG_LOG_BIT_LOCK ) , } cocci_id@p ;
...
position p;
@@
 enum devkmsg_log_bits { __DEVKMSG_LOG_BIT_ON = 0 , __DEVKMSG_LOG_BIT_OFF , __DEVKMSG_LOG_BIT_LOCK , } cocci_id@p ;
...
typedef u64,u8,u32;
@@
 struct cont {  char buf [ LOG_LINE_MAX ] ;  size_t len ;  u32 caller_id ;  u64 ts_nsec ;  u8 level ;  u8 facility ;  enum log_flags flags ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef initcall_entry_t;
@@
 initcall_entry_t * cocci_id@p ;
...
position p;
@@
 struct devkmsg_user * cocci_id@p ;
...
position p;
@@
 struct console_cmdline * cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ DEVKMSG_STR_MAX_SIZE ] ;
...
position p;
@@
 char cocci_id@p [ CONSOLE_EXT_LOG_MAX ] ;
...
typedef u64,u32;
@@
 struct devkmsg_user {  u64 seq ;  u32 idx ;  struct ratelimit_state rs ;  struct mutex lock ;  char buf [ CONSOLE_EXT_LOG_MAX ] ; } cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ sizeof ( console_cmdline [ 0 ] . name ) + 4 ] ;
...
position p;
@@
 struct console_cmdline cocci_id@p [ MAX_CMDLINECONSOLES ] ;
...
position p;
@@
 enum log_flags { LOG_NEWLINE = 2 , LOG_CONT = 8 , } cocci_id@p ;
...
position p;
@@
 enum con_msg_format_flags { MSG_FORMAT_DEFAULT = 0 , MSG_FORMAT_SYSLOG = ( 1 << 0 ) , } cocci_id@p ;
