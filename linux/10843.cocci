...
position p;
@@
 struct amdgpu_sdma_instance cocci_id@p ;
...
position p;
@@
 const struct amdgpu_buffer_funcs cocci_id@p ;
...
typedef u32;
@@
 u32 cocci_id@p [ AMDGPU_MAX_SDMA_INSTANCES ] ;
...
position p;
@@
 int cocci_id@p ( struct amdgpu_device * adev , void * err_data , struct amdgpu_iv_entry * entry ) ;
...
position p;
@@
 const struct amdgpu_vm_pte_funcs cocci_id@p ;
...
position p;
@@
 struct amdgpu_ring * cocci_id@p [ AMDGPU_MAX_SDMA_INSTANCES ] ;
...
position p;
@@
 struct drm_gpu_scheduler * cocci_id@p ;
