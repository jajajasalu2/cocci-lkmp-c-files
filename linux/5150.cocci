...
position p;
@@
 struct gpiomm_gpio * const cocci_id@p ;
...
position p;
@@
 unsigned int cocci_id@p [ MAX_NUM_GPIOMM ] ;
...
typedef spinlock_t;
@@
 struct gpiomm_gpio {  struct gpio_chip chip ;  unsigned char io_state [ 6 ] ;  unsigned char out_state [ 6 ] ;  unsigned char control [ 2 ] ;  spinlock_t lock ;  unsigned int base ; } cocci_id@p ;
...
position p;
@@
 const char * cocci_id@p [ GPIOMM_NGPIO ] ;
