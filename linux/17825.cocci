...
symbol cocci_id;
position p;
typedef fp_state_low_volatile_t;
@@
 fp_state_low_volatile_t * cocci_id@p ;
...
position p;
@@
 struct illegal_op_return cocci_id@p ( struct pt_regs * , unsigned long ) ;
...
position p;
@@
 struct fpu_swa_msg {  unsigned long count ;  unsigned long time ; } cocci_id@p ;
...
@@
 struct {  spinlock_t lock ;  u32 lock_owner ;  int lock_owner_depth ; } cocci_id@p ;
...
position p;
@@
 struct illegal_op_return cocci_id@p ;
