...
@@
 struct vdec_vp8_vpu_inst {  wait_queue_head_t wq_hd ;  int signaled ;  int failure ;  uint32_t inst_addr ; } cocci_id@p ;
...
position p;
@@
 struct vdec_pic_info * cocci_id@p ;
...
@@
 struct vdec_vp8_dec_info {  uint64_t working_buf_dma ;  uint64_t prev_y_dma ;  uint64_t cur_y_fb_dma ;  uint64_t cur_c_fb_dma ;  uint64_t bs_dma ;  uint32_t bs_sz ;  uint32_t resolution_changed ;  uint32_t show_frame ;  uint32_t wait_key_frame ; } cocci_id@p ;
...
position p;
@@
 struct vdec_fb_node * cocci_id@p ;
...
position p;
@@
 const struct vdec_common_if cocci_id@p ;
...
position p;
@@
 struct mtk_vcodec_mem * cocci_id@p ;
...
position p;
@@
 struct vdec_vp8_inst * cocci_id@p ;
...
position p;
@@
 struct vdec_vp8_inst {  struct vdec_fb * cur_fb ;  struct vdec_fb_node dec_fb [ VP8_MAX_FRM_BUF_NODE_NUM ] ;  struct list_head available_fb_node_list ;  struct list_head fb_use_list ;  struct list_head fb_free_list ;  struct list_head fb_disp_list ;  struct mtk_vcodec_mem working_buf ;  struct vdec_vp8_hw_reg_base reg_base ;  unsigned int frm_cnt ;  struct mtk_vcodec_ctx * ctx ;  struct vdec_vpu_inst vpu ;  struct vdec_vp8_vsi * vsi ; } cocci_id@p ;
...
position p;
@@
 enum vdec_get_param_type cocci_id@p ;
...
position p;
@@
 struct vdec_vp8_dec_info * cocci_id@p ;
...
position p;
@@
 struct vdec_vp8_hw_reg_base {  void __iomem * sys ;  void __iomem * misc ;  void __iomem * ld ;  void __iomem * top ;  void __iomem * cm ;  void __iomem * hwd ;  void __iomem * hwb ; } cocci_id@p ;
...
position p;
@@
 struct mtk_vcodec_ctx * cocci_id@p ;
...
typedef uint32_t;
@@
 struct vdec_vp8_vsi {  struct vdec_vp8_dec_info dec ;  struct vdec_pic_info pic ;  uint32_t dec_table [ VP8_DEC_TABLE_SZ ] ;  uint32_t segment_buf [ VP8_HW_SEGMENT_DATA_SZ ] [ VP8_HW_SEGMENT_UINT ] ;  uint32_t load_data ; } cocci_id@p ;
...
position p;
@@
 struct vdec_vp8_vsi * cocci_id@p ;
...
position p;
@@
 struct vdec_vpu_inst * cocci_id@p ;
