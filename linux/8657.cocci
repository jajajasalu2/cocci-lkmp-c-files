...
position p;
@@
 const struct ixgbe_reg_test * cocci_id@p ;
...
position p;
@@
 struct ixgbe_stats {  char stat_string [ ETH_GSTRING_LEN ] ;  int type ;  int sizeof_stat ;  int stat_offset ; } cocci_id@p ;
...
typedef u8,u32,u16;
@@
 struct ixgbe_reg_test {  u16 reg ;  u8 array_len ;  u8 test_type ;  u32 mask ;  u32 write ; } cocci_id@p ;
...
position p;
@@
 struct ixgbe_fc_info cocci_id@p ;
...
typedef u32;
@@
 u32 cocci_id@p [ FW_PHY_ACT_DATA_COUNT ] ;
...
position p;
@@
 struct ixgbe_queue_stats cocci_id@p ;
...
typedef u32;
@@
 const struct {  ixgbe_link_speed mac_speed ;  u32 supported ; } cocci_id@p [ ] ;
...
position p;
@@
 enum { NETDEV_STATS , IXGBE_STATS , } cocci_id@p ;
...
typedef u32;
@@
 const struct {  u32 lp_advertised ;  u32 mac_speed ; } cocci_id@p [ ] ;
...
position p;
@@
 const struct ixgbe_stats cocci_id@p [ ] ;
