...
position p;
@@
 struct bfa_fcs_driver_info_s cocci_id@p ;
...
position p;
@@
 struct bfa_lport_cfg_s * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct bfad_s * bfad , enum bfad_sm_event event ) ;
...
typedef u32;
@@
 u32 * cocci_id@p ( struct pci_dev * pdev ) ;
...
position p;
@@
 struct msix_entry cocci_id@p [ MAX_MSIX_ENTRY ] ;
...
typedef u32;
@@
 void cocci_id@p ( struct pci_dev * pdev , u32 * * bfi_image , u32 * bfi_image_size , char * fw_name ) ;
