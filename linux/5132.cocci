...
position p;
@@
 const struct aspeed_sgpio_bank cocci_id@p [ ] ;
...
@@
 struct aspeed_sgpio {  struct gpio_chip chip ;  struct clk * pclk ;  spinlock_t lock ;  void __iomem * base ;  uint32_t dir_in [ 3 ] ;  int irq ; } cocci_id@p ;
...
position p;
@@
 enum aspeed_sgpio_reg cocci_id@p ;
...
position p;
@@
 struct aspeed_sgpio * cocci_id@p ;
...
position p;
@@
 const enum aspeed_sgpio_reg cocci_id@p ;
...
typedef uint16_t;
@@
 struct aspeed_sgpio_bank {  uint16_t val_regs ;  uint16_t rdata_reg ;  uint16_t irq_regs ;  const char names [ 4 ] [ 3 ] ; } cocci_id@p ;
