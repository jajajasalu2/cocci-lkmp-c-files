...
position p;
@@
 int cocci_id@p ( struct usb_interface * interface ) ;
...
position p;
@@
 enum { ATI_REMOTE2_AUX1 , ATI_REMOTE2_AUX2 , ATI_REMOTE2_AUX3 , ATI_REMOTE2_AUX4 , ATI_REMOTE2_PC , ATI_REMOTE2_MODES , } cocci_id@p ;
...
position p;
@@
 enum { ATI_REMOTE2_OPENED = 0x1 , ATI_REMOTE2_SUSPENDED = 0x2 , } cocci_id@p ;
...
position p;
@@
 const struct input_keymap_entry * cocci_id@p ;
...
position p;
@@
 struct input_keymap_entry * cocci_id@p ;
...
position p;
@@
 enum { ATI_REMOTE2_MAX_CHANNEL_MASK = 0xFFFF , ATI_REMOTE2_MAX_MODE_MASK = 0x1F , } cocci_id@p ;
...
typedef u8,u16;
@@
 const struct {  u8 hw_code ;  u16 keycode ; } cocci_id@p [ ] ;
...
@@
 struct ati_remote2 {  struct input_dev * idev ;  struct usb_device * udev ;  struct usb_interface * intf [ 2 ] ;  struct usb_endpoint_descriptor * ep [ 2 ] ;  struct urb * urb [ 2 ] ;  void * buf [ 2 ] ;  dma_addr_t buf_dma [ 2 ] ;  unsigned long jiffies ;  int mode ;  char name [ 64 ] ;  char phys [ 64 ] ;  u16 keycode [ ATI_REMOTE2_MODES ] [ ARRAY_SIZE ( ati_remote2_key_table ) ] ;  unsigned int flags ;  unsigned int channel_mask ;  unsigned int mode_mask ; } cocci_id@p ;
