...
position p;
@@
 const struct msdos_sb_info * cocci_id@p ;
...
position p;
@@
 enum { PARSE_INVALID = 1 , PARSE_NOT_LONGNAME , PARSE_EOF , } cocci_id@p ;
...
position p;
@@
 struct buffer_head * cocci_id@p [ MAX_BUF_PER_PAGE ] ;
...
position p;
@@
 struct fat_ioctl_filldir_callback * cocci_id@p ;
...
position p;
@@
 unsigned char cocci_id@p [ FAT_MAX_SHORT_SIZE ] ;
...
position p;
@@
 struct fat_slot_info * cocci_id@p ;
...
position p;
@@
 struct compat_dirent cocci_id@p [ 2 ] ;
...
position p;
@@
 const struct msdos_dir_entry * cocci_id@p ;
...
position p;
@@
 struct msdos_dir_slot * cocci_id@p ;
...
position p;
@@
 struct fat_ioctl_filldir_callback {  struct dir_context ctx ;  void __user * dirent ;  int result ;  const char * longname ;  int long_len ;  const char * shortname ;  int short_len ; } cocci_id@p ;
