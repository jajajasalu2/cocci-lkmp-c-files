...
position p;
@@
 enum mcast_group_state { MCAST_IDLE , MCAST_JOIN_SENT , MCAST_LEAVE_SENT , MCAST_RESP_READY , } cocci_id@p ;
...
position p;
@@
 struct ib_sa_mcmember_data * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef __be64,atomic_t;
@@
 struct mcast_group {  struct ib_sa_mcmember_data rec ;  struct rb_node node ;  struct list_head mgid0_list ;  struct mlx4_ib_demux_ctx * demux ;  struct mcast_member func [ MAX_VFS ] ;  struct mutex lock ;  struct work_struct work ;  struct list_head pending_list ;  int members [ 3 ] ;  enum mcast_group_state state ;  enum mcast_group_state prev_state ;  struct ib_sa_mad response_sa_mad ;  __be64 last_req_tid ;  char name [ 33 ] ;  struct device_attribute dentry ;  atomic_t refcount ;  struct delayed_work timeout_work ;  struct list_head cleanup_list ; } cocci_id@p ;
...
typedef uint8_t;
@@
 struct mcast_member {  enum mcast_state state ;  uint8_t join_state ;  int num_pend_reqs ;  struct list_head pending ; } cocci_id@p ;
...
position p;
@@
 struct mcast_req {  int func ;  struct ib_sa_mad sa_mad ;  struct list_head group_list ;  struct list_head func_list ;  struct mcast_group * group ;  int clean ; } cocci_id@p ;
...
position p;
@@
 struct clean_work {  struct work_struct work ;  struct mlx4_ib_demux_ctx * ctx ;  int destroy_wq ; } cocci_id@p ;
...
position p;
@@
 enum mcast_state { MCAST_NOT_MEMBER = 0 , MCAST_MEMBER , } cocci_id@p ;
