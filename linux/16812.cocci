...
position p;
@@
 struct keyspan_message cocci_id@p ;
...
typedef u8,u16;
@@
 struct keyspan_message {  u16 system ;  u8 button ;  u8 toggle ; } cocci_id@p ;
...
typedef dma_addr_t;
@@
 struct usb_keyspan {  char name [ 128 ] ;  char phys [ 64 ] ;  unsigned short keymap [ ARRAY_SIZE ( keyspan_key_table ) ] ;  struct usb_device * udev ;  struct input_dev * input ;  struct usb_interface * interface ;  struct usb_endpoint_descriptor * in_endpoint ;  struct urb * irq_urb ;  int open ;  dma_addr_t in_dma ;  unsigned char * in_buffer ;  struct bit_tester data ;  int stage ;  int toggle ; } cocci_id@p ;
...
typedef u8,u32;
@@
 struct bit_tester {  u32 tester ;  int len ;  int pos ;  int bits_left ;  u8 buffer [ 32 ] ; } cocci_id@p ;
