...
position p;
@@
 enum sbs_battery_mode { BATTERY_MODE_AMPS = 0 , BATTERY_MODE_WATTS = 0x8000 , } cocci_id@p ;
...
position p;
@@
 enum sbs_battery_mode cocci_id@p ;
...
typedef u8;
@@
 const struct chip_data {  enum power_supply_property psp ;  u8 addr ;  int min_value ;  int max_value ; } cocci_id@p [ ] ;
...
typedef u8;
@@
 u8 cocci_id@p [ I2C_SMBUS_BLOCK_MAX + 1 ] ;
...
position p;
@@
 struct sbs_platform_data * cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ I2C_SMBUS_BLOCK_MAX + 1 ] ;
...
typedef bool,u32;
@@
 struct sbs_info {  struct i2c_client * client ;  struct power_supply * power_supply ;  bool is_present ;  struct gpio_desc * gpio_detect ;  bool enable_detection ;  int last_state ;  int poll_time ;  u32 i2c_retry_count ;  u32 poll_retry_count ;  struct delayed_work work ;  struct mutex mode_lock ;  u32 flags ; } cocci_id@p ;
