...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_HTB_MAX + 1 ] ;
...
position p;
@@
 struct htb_level {  struct rb_root wait_pq ;  struct htb_prio hprio [ TC_HTB_NUMPRIO ] ; } cocci_id@p ;
...
position p;
@@
 struct {  struct nlattr nla ;  struct gnet_estimator opt ; } cocci_id@p ;
...
position p;
@@
 enum htb_cmode { HTB_CANT_SEND , HTB_MAY_BORROW , HTB_CAN_SEND , } cocci_id@p ;
...
typedef u32;
@@
 struct {  struct rb_node * root ;  struct rb_node * * pptr ;  u32 * pid ; } cocci_id@p [ TC_HTB_MAXDEPTH ] ;
...
position p;
@@
 const struct htb_class * cocci_id@p ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_HTB_MAX + 1 ] ;
...
typedef u32;
@@
 struct htb_prio {  union {  struct rb_root row ;  struct rb_root feed ; } ;  struct rb_node * ptr ;  u32 last_ptr_id ; } cocci_id@p ;
