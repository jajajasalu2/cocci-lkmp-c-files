...
position p;
@@
 struct channel_dev {  struct cdev cdev ;  struct device * dev ; } cocci_id@p ;
...
position p;
@@
 enum rio_cm_pkt_type { RIO_CM_SYS = 0xaa , RIO_CM_CHAN = 0x55 , } cocci_id@p ;
...
position p;
@@
 struct rio_cm_work {  struct work_struct work ;  struct cm_dev * cm ;  void * data ; } cocci_id@p ;
...
position p;
@@
 enum rio_cm_state { RIO_CM_IDLE , RIO_CM_CONNECT , RIO_CM_CONNECTED , RIO_CM_DISCONNECT , RIO_CM_CHAN_BOUND , RIO_CM_LISTEN , RIO_CM_DESTROYING , } cocci_id@p ;
...
position p;
@@
 struct rio_cm_accept cocci_id@p ;
...
position p;
@@
 struct subsys_interface cocci_id@p ;
...
typedef u16;
@@
 struct rio_channel * cocci_id@p ( u16 ch_num ) ;
...
position p;
@@
 struct chan_rx_ring {  void * buf [ RIOCM_RX_RING_SIZE ] ;  int head ;  int tail ;  int count ;  void * inuse [ RIOCM_RX_RING_SIZE ] ;  int inuse_cnt ; } cocci_id@p ;
...
position p;
@@
 struct rio_ch_chan_hdr * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct rio_channel * ch ) ;
...
typedef u32,u16;
@@
 struct conn_req {  struct list_head node ;  u32 destid ;  u16 chan ;  struct cm_dev * cmdev ; } cocci_id@p ;
...
position p;
@@
 enum rio_cm_chop { CM_CONN_REQ , CM_CONN_ACK , CM_CONN_CLOSE , CM_DATA_MSG , } cocci_id@p ;
...
position p;
@@
 enum { DBG_NONE = 0 , DBG_INIT = BIT ( 0 ) , DBG_EXIT = BIT ( 1 ) , DBG_MPORT = BIT ( 2 ) , DBG_RDEV = BIT ( 3 ) , DBG_CHOP = BIT ( 4 ) , DBG_WAIT = BIT ( 5 ) , DBG_TX = BIT ( 6 ) , DBG_TX_EVENT = BIT ( 7 ) , DBG_RX_DATA = BIT ( 8 ) , DBG_RX_CMD = BIT ( 9 ) , DBG_ALL = ~ 0 , } cocci_id@p ;
...
position p;
@@
 struct cm_peer {  struct list_head node ;  struct rio_dev * rdev ; } cocci_id@p ;
...
position p;
@@
 struct tx_req {  struct list_head node ;  struct rio_dev * rdev ;  void * buffer ;  size_t len ; } cocci_id@p ;
...
position p;
@@
 struct rio_cm_channel cocci_id@p ;
...
@@
 struct rio_channel {  u16 id ;  struct kref ref ;  struct file * filp ;  struct cm_dev * cmdev ;  struct rio_dev * rdev ;  enum rio_cm_state state ;  int error ;  spinlock_t lock ;  void * context ;  u32 loc_destid ;  u32 rem_destid ;  u16 rem_channel ;  struct list_head accept_queue ;  struct list_head ch_node ;  struct completion comp ;  struct completion comp_close ;  struct chan_rx_ring rx_ring ; } cocci_id@p ;
