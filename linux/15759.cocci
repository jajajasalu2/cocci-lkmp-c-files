...
position p;
@@
 long cocci_id@p ( struct file * , struct vsoc_device_region __user * ) ;
...
typedef __u32;
@@
 ssize_t cocci_id@p ( struct file * filp , __u32 * perm_off ) ;
...
position p;
@@
 struct vsoc_region_data * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct vsoc_device_region * region_p , struct fd_scoped_permission_node * np , struct fd_scoped_permission_arg __user * arg ) ;
...
position p;
@@
 struct vsoc_device_region * cocci_id@p ;
...
position p;
@@
 struct hrtimer_sleeper cocci_id@p ;
...
position p;
@@
 struct vsoc_cond_wait * cocci_id@p ;
...
position p;
@@
 struct vsoc_private_data * cocci_id@p ;
...
position p;
@@
 struct fd_scoped_permission_node {  struct fd_scoped_permission permission ;  struct list_head list ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct vsoc_device_region * owner_region_p , struct fd_scoped_permission * perm ) ;
...
position p;
@@
 const struct vsoc_device_region * cocci_id@p ;
...
position p;
@@
 struct vsoc_private_data {  struct fd_scoped_permission_node * fd_scoped_permission_node ; } cocci_id@p ;
...
position p;
@@
 struct fd_scoped_permission_node * cocci_id@p ;
...
position p;
@@
 struct fd_scoped_permission * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,phys_addr_t;
@@
 struct vsoc_device {  void __iomem * regs ;  phys_addr_t shm_phys_start ;  void __iomem * kernel_mapped_shm ;  size_t shm_size ;  struct vsoc_shm_layout_descriptor * layout ;  struct vsoc_device_region * regions ;  struct list_head permissions ;  struct pci_dev * dev ;  struct vsoc_region_data * regions_data ;  struct msix_entry * msix_entries ;  struct mutex mtx ;  int major ;  struct cdev cdev ;  struct class * class ;  bool enabled_device ;  bool requested_regions ;  bool cdev_added ;  bool class_added ;  bool msix_enabled ; } cocci_id@p ;
...
position p;
@@
 enum { INTR_MASK = 0x00 , INTR_STATUS = 0x04 , IV_POSITION = 0x08 , DOORBELL = 0x0c , } cocci_id@p ;
...
@@
 struct vsoc_region_data {  char name [ VSOC_DEVICE_NAME_SZ + 1 ] ;  wait_queue_head_t interrupt_wait_queue ;  wait_queue_head_t futex_wait_queue ;  atomic_t * incoming_signalled ;  atomic_t * outgoing_signalled ;  bool irq_requested ;  bool device_created ; } cocci_id@p ;
...
position p;
@@
 const struct vsoc_signal_table_layout * cocci_id@p ;
