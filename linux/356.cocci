...
position p;
@@
 enum bpf_type { BPF_TYPE_UNSPEC = 0 , BPF_TYPE_PROG , BPF_TYPE_MAP , } cocci_id@p ;
...
typedef umode_t;
@@
 struct bpf_mount_opts {  umode_t mode ; } cocci_id@p ;
...
position p;
@@
 const struct filename * cocci_id@p ;
...
position p;
@@
 struct bpf_mount_opts cocci_id@p ;
...
typedef bool;
@@
 struct map_iter {  void * key ;  bool done ; } cocci_id@p ;
