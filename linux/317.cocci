...
position p;
@@
 struct futex_pi_state * cocci_id@p ;
...
typedef bool;
@@
 struct {  struct fault_attr attr ;  bool ignore_private ; } cocci_id@p ;
...
position p;
@@
 long cocci_id@p ( struct restart_block * restart ) ;
...
position p;
@@
 struct futex_hash_bucket * cocci_id@p ;
...
@@
 struct futex_q {  struct plist_node list ;  struct task_struct * task ;  spinlock_t * lock_ptr ;  union futex_key key ;  struct futex_pi_state * pi_state ;  struct rt_mutex_waiter * rt_waiter ;  union futex_key * requeue_pi_key ;  u32 bitset ; } cocci_id@p ;
...
position p;
@@
 enum futex_access { FUTEX_READ , FUTEX_WRITE , } cocci_id@p ;
...
position p;
@@
 const struct futex_q cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct task_struct * curr ) ;
...
typedef refcount_t;
@@
 struct futex_pi_state {  struct list_head list ;  struct rt_mutex pi_mutex ;  struct task_struct * owner ;  refcount_t refcount ;  union futex_key key ; } cocci_id@p ;
...
position p;
@@
 struct robust_list_head * cocci_id@p ;
...
position p;
@@
 struct rt_mutex_waiter cocci_id@p ;
...
position p;
@@
 struct restart_block * cocci_id@p ;
...
position p;
@@
 struct {  struct futex_hash_bucket * queues ;  unsigned long hashsize ; } cocci_id@p ;
