...
position p;
@@
 struct nlattr * cocci_id@p [ CTA_PROTOINFO_TCP_MAX + 1 ] ;
...
position p;
@@
 struct nf_ct_tcp_flags cocci_id@p ;
...
typedef u8;
@@
 const u8 cocci_id@p [ 2 ] [ 6 ] [ TCP_CONNTRACK_MAX ] ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ CTA_PROTOINFO_TCP_MAX + 1 ] ;
...
typedef u8;
@@
 const u8 cocci_id@p [ ( TCPHDR_FIN | TCPHDR_SYN | TCPHDR_RST | TCPHDR_ACK | TCPHDR_URG ) + 1 ] ;
...
position p;
@@
 const struct nf_tcp_net * cocci_id@p ;
...
position p;
@@
 struct ip_ct_tcp_state * cocci_id@p ;
...
position p;
@@
 const struct ip_ct_tcp_state * cocci_id@p ;
...
position p;
@@
 enum tcp_bit_set { TCP_SYN_SET , TCP_SYNACK_SET , TCP_FIN_SET , TCP_ACK_SET , TCP_RST_SET , TCP_NONE_SET , } cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ CTA_TIMEOUT_TCP_MAX + 1 ] ;
...
position p;
@@
 unsigned char cocci_id@p [ ( 15 * 4 ) - sizeof ( struct tcphdr ) ] ;
