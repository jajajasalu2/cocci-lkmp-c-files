...
position p;
@@
 int ( * cocci_id@p ) ( struct chcr_dev * dev , unsigned char * input ) ;
...
position p;
@@
 struct chcr_driver_data cocci_id@p ;
...
position p;
@@
 const struct cpl_fw6_pld * cocci_id@p ;
...
position p;
@@
 chcr_handler_func cocci_id@p [ NUM_CPL_CMDS ] ;
...
position p;
@@
 int cocci_id@p ( void * handle , enum cxgb4_state state ) ;
...
position p;
@@
 void * cocci_id@p ( const struct cxgb4_lld_info * lld ) ;
