...
position p;
@@
 struct {  struct sockaddr_ll sa ;  char buf [ MAX_ADDR_LEN ] ; } cocci_id@p ;
...
position p;
@@
 enum { VHOST_NET_BACKEND_FEATURES = ( 1ULL << VHOST_BACKEND_F_IOTLB_MSG_V2 ) , } cocci_id@p ;
...
position p;
@@
 struct vhost_net_virtqueue * cocci_id@p ;
...
position p;
@@
 struct vring_used_elem * cocci_id@p ;
...
position p;
@@
 struct vhost_vring_file cocci_id@p ;
...
position p;
@@
 struct vhost_net_buf {  void * * queue ;  int tail ;  int head ; } cocci_id@p ;
...
position p;
@@
 enum { VHOST_NET_VQ_RX = 0 , VHOST_NET_VQ_TX = 1 , VHOST_NET_VQ_MAX = 2 , } cocci_id@p ;
...
typedef bool;
@@
 struct vhost_net {  struct vhost_dev dev ;  struct vhost_net_virtqueue vqs [ VHOST_NET_VQ_MAX ] ;  struct vhost_poll poll [ VHOST_NET_VQ_MAX ] ;  unsigned tx_packets ;  unsigned tx_zcopy_err ;  bool tx_flush ;  struct page_frag page_frag ;  int refcnt_bias ; } cocci_id@p ;
...
position p;
@@
 enum { VHOST_NET_FEATURES = VHOST_FEATURES | ( 1ULL << VHOST_NET_F_VIRTIO_NET_HDR ) | ( 1ULL << VIRTIO_NET_F_MRG_RXBUF ) | ( 1ULL << VIRTIO_F_IOMMU_PLATFORM ) , } cocci_id@p ;
...
position p;
@@
 struct vhost_net_virtqueue {  struct vhost_virtqueue vq ;  size_t vhost_hlen ;  size_t sock_hlen ;  int upend_idx ;  int done_idx ;  int batched_xdp ;  struct ubuf_info * ubuf_info ;  struct vhost_net_ubuf_ref * ubufs ;  struct ptr_ring * rx_ring ;  struct vhost_net_buf rxq ;  struct xdp_buff * xdp ; } cocci_id@p ;
...
position p;
@@
 struct vhost_net_buf * cocci_id@p ;
...
position p;
@@
 struct vhost_net_ubuf_ref * cocci_id@p ;
...
@@
 struct vhost_net_ubuf_ref {  atomic_t refcount ;  wait_queue_head_t wait ;  struct vhost_virtqueue * vq ; } cocci_id@p ;
