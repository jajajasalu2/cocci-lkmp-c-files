...
position p;
@@
 void cocci_id@p ( struct vb2_queue * vq ) ;
...
position p;
@@
 struct s2255_bufferi {  unsigned long dwFrames ;  struct s2255_framei frame [ SYS_FRAMES ] ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct s2255_vc * vc , struct s2255_mode * mode ) ;
...
position p;
@@
 int cocci_id@p ( struct vb2_queue * vq , unsigned int count ) ;
...
position p;
@@
 struct s2255_framei * cocci_id@p ;
...
position p;
@@
 struct s2255_buffer * cocci_id@p ;
...
position p;
@@
 const struct s2255_fmt cocci_id@p [ ] ;
...
position p;
@@
 struct v4l2_frmivalenum * cocci_id@p ;
...
position p;
@@
 unsigned long cocci_id@p [ MAX_CHANNELS ] ;
...
position p;
@@
 void cocci_id@p ( struct s2255_dev * dev ) ;
...
@@
 struct s2255_vc {  struct s2255_dev * dev ;  struct video_device vdev ;  struct v4l2_ctrl_handler hdl ;  struct v4l2_ctrl * jpegqual_ctrl ;  int resources ;  struct list_head buf_list ;  struct s2255_bufferi buffer ;  struct s2255_mode mode ;  v4l2_std_id std ;  unsigned jpegqual ;  struct v4l2_captureparm cap_parm ;  int cur_frame ;  int last_frame ;  unsigned long req_image_size ;  unsigned long pkt_size ;  int bad_payload ;  unsigned long frame_count ;  int jpg_size ;  int configured ;  wait_queue_head_t wait_setmode ;  int setmode_ready ;  int vidstatus ;  wait_queue_head_t wait_vidstatus ;  int vidstatus_ready ;  unsigned int width ;  unsigned int height ;  enum v4l2_field field ;  const struct s2255_fmt * fmt ;  int idx ;  struct vb2_queue vb_vidq ;  struct mutex vb_lock ;  spinlock_t qlock ; } cocci_id@p ;
...
position p;
@@
 const struct v4l2_frmsize_discrete * cocci_id@p ;
...
position p;
@@
 struct s2255_buffer {  struct vb2_v4l2_buffer vb ;  struct list_head list ; } cocci_id@p ;
...
position p;
@@
 struct s2255_pipeinfo * cocci_id@p ;
...
typedef u8,u32;
@@
 struct s2255_pipeinfo {  u32 max_transfer_size ;  u32 cur_transfer_size ;  u8 * transfer_buffer ;  u32 state ;  void * stream_urb ;  void * dev ;  u32 err_count ;  u32 idx ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct s2255_vc * vc , struct s2255_buffer * buf , int jpgsize ) ;
...
typedef u32;
@@
 struct s2255_mode {  u32 format ;  u32 scale ;  u32 color ;  u32 fdec ;  u32 bright ;  u32 contrast ;  u32 saturation ;  u32 hue ;  u32 single ;  u32 usb_block ;  u32 restart ; } cocci_id@p ;
...
typedef s32,u16;
@@
 long cocci_id@p ( struct s2255_dev * dev , unsigned char req , u16 index , u16 value , void * buf , s32 buf_len , int bOut ) ;
...
typedef u32;
@@
 struct s2255_fmt {  u32 fourcc ;  int depth ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p [ NUM_FRAME_ENUMS ] ;
...
symbol cocci_id;
position p;
typedef wait_queue_head_t,atomic_t;
@@
 struct s2255_fw {  int fw_loaded ;  int fw_size ;  struct urb * fw_urb ;  atomic_t fw_state ;  void * pfw_data ;  wait_queue_head_t wait_fw ;  const struct firmware * fw ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct s2255_vc * vc ) ;
...
position p;
@@
 struct s2255_framei {  unsigned long size ;  unsigned long ulState ;  void * lpvbits ;  unsigned long cur_size ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct s2255_dev * dev ) ;
