...
position p;
@@
 struct sfq_sched_data * cocci_id@p ;
...
position p;
@@
 struct tc_sfq_qopt_v1 * cocci_id@p ;
...
position p;
@@
 struct sfq_slot {  struct sk_buff * skblist_next ;  struct sk_buff * skblist_prev ;  sfq_index qlen ;  sfq_index next ;  struct sfq_head dep ;  unsigned short hash ;  short allot ;  unsigned int backlog ;  struct red_vars vars ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,siphash_key_t;
@@
 struct sfq_sched_data {  int limit ;  unsigned int divisor ;  u8 headdrop ;  u8 maxdepth ;  siphash_key_t perturbation ;  u8 cur_depth ;  u8 flags ;  unsigned short scaled_quantum ;  struct tcf_proto __rcu * filter_list ;  struct tcf_block * block ;  sfq_index * ht ;  struct sfq_slot * slots ;  struct red_parms * red_parms ;  struct tc_sfqred_stats stats ;  struct sfq_slot * tail ;  struct sfq_head dep [ SFQ_MAX_DEPTH + 1 ] ;  unsigned int maxflows ;  int perturb_period ;  unsigned int quantum ;  struct timer_list perturb_timer ;  struct Qdisc * sch ; } cocci_id@p ;
...
position p;
@@
 struct sfq_head {  sfq_index next ;  sfq_index prev ; } cocci_id@p ;
...
position p;
@@
 const struct sfq_sched_data * cocci_id@p ;
...
position p;
@@
 const struct sfq_slot * cocci_id@p ;
...
position p;
@@
 struct tc_sfq_xstats cocci_id@p ;
