...
position p;
@@
 struct lan78xx_statstage * cocci_id@p ;
...
position p;
@@
 enum skb_state { illegal = 0 , tx_start , tx_done , rx_start , rx_done , rx_cleanup , unlink_start , } cocci_id@p ;
...
position p;
@@
 struct statstage {  struct mutex access_lock ;  struct lan78xx_statstage saved ;  struct lan78xx_statstage rollover_count ;  struct lan78xx_statstage rollover_max ;  struct lan78xx_statstage64 curr_stat ; } cocci_id@p ;
...
position p;
@@
 struct skb_data {  struct urb * urb ;  struct lan78xx_net * dev ;  enum skb_state state ;  size_t length ;  int num_of_packet ; } cocci_id@p ;
...
position p;
@@
 struct usb_context {  struct usb_ctrlrequest req ;  struct lan78xx_net * dev ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u32,irq_flow_handler_t;
@@
 struct irq_domain_data {  struct irq_domain * irqdomain ;  unsigned int phyirq ;  struct irq_chip * irqchip ;  irq_flow_handler_t irq_handler ;  u32 irqenable ;  struct mutex irq_lock ; } cocci_id@p ;
...
position p;
@@
 struct lan78xx_priv cocci_id@p ;
...
position p;
@@
 struct irq_domain_data * cocci_id@p ;
...
@@
 struct lan78xx_priv {  struct lan78xx_net * dev ;  u32 rfe_ctl ;  u32 mchash_table [ DP_SEL_VHF_HASH_LEN ] ;  u32 pfilter_table [ NUM_OF_MAF ] [ 2 ] ;  u32 vlan_table [ DP_SEL_VHF_VLAN_LEN ] ;  struct mutex dataport_mutex ;  spinlock_t rfe_ctl_lock ;  struct work_struct set_multicast ;  struct work_struct set_vlan ;  u32 wol ; } cocci_id@p ;
