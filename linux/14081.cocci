...
position p;
@@
 const struct csis_drvdata * cocci_id@p ;
...
typedef bool;
@@
 int cocci_id@p ( struct device * dev , bool runtime ) ;
...
position p;
@@
 enum { ST_POWERED = 1 , ST_STREAMING = 2 , ST_SUSPENDED = 4 , } cocci_id@p ;
...
@@
 struct csis_state {  struct mutex lock ;  struct media_pad pads [ CSIS_PADS_NUM ] ;  struct v4l2_subdev sd ;  u8 index ;  struct platform_device * pdev ;  struct phy * phy ;  void __iomem * regs ;  struct regulator_bulk_data supplies [ CSIS_NUM_SUPPLIES ] ;  struct clk * clock [ NUM_CSIS_CLOCKS ] ;  int irq ;  u32 interrupt_mask ;  u32 flags ;  u32 clk_frequency ;  u32 hs_settle ;  u32 num_lanes ;  u32 max_num_lanes ;  u8 wclk_ext ;  const struct csis_pix_format * csis_fmt ;  struct v4l2_mbus_framefmt format ;  spinlock_t slock ;  struct csis_pktbuf pkt_buf ;  struct s5pcsis_event events [ S5PCSIS_NUM_EVENTS ] ; } cocci_id@p ;
...
position p;
@@
 const struct s5pcsis_event cocci_id@p [ ] ;
...
typedef u32;
@@
 struct s5pcsis_event {  u32 mask ;  const char * const name ;  unsigned int counter ; } cocci_id@p ;
...
position p;
@@
 const struct csis_pix_format * cocci_id@p ;
...
typedef u32;
@@
 struct csis_pktbuf {  u32 * data ;  unsigned int len ; } cocci_id@p ;
...
position p;
@@
 enum { CSIS_CLK_MUX , CSIS_CLK_GATE , } cocci_id@p ;
...
typedef u8,u32;
@@
 struct csis_pix_format {  unsigned int pix_width_alignment ;  u32 code ;  u32 fmt_reg ;  u8 data_alignment ; } cocci_id@p ;
...
typedef u32;
@@
 struct csis_drvdata {  u32 interrupt_mask ; } cocci_id@p ;
...
typedef u32;
@@
 struct {  u32 offset ;  const char * const name ; } cocci_id@p [ ] ;
