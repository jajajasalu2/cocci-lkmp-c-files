...
typedef u64,u16;
@@
 struct fwnet_peer {  struct list_head peer_link ;  struct fwnet_device * dev ;  u64 guid ;  struct list_head pd_list ;  unsigned pdg_size ;  u16 datagram_label ;  u16 max_payload ;  int node_id ;  int generation ;  unsigned speed ; } cocci_id@p ;
...
typedef u8,u64,u16;
@@
 struct fwnet_packet_task {  struct fw_transaction transaction ;  struct rfc2734_header hdr ;  struct sk_buff * skb ;  struct fwnet_device * dev ;  int outstanding_pkts ;  u64 fifo_addr ;  u16 dest_node ;  u16 max_payload ;  u8 generation ;  u8 speed ;  u8 enqueued ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct fwnet_packet_task * ptask ) ;
...
typedef u16;
@@
 struct fwnet_partial_datagram {  struct list_head pd_link ;  struct list_head fi_list ;  struct sk_buff * skb ;  char * pbuf ;  u16 datagram_label ;  u16 ether_type ;  u16 datagram_size ; } cocci_id@p ;
...
position p;
@@
 struct fwnet_device * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u64,u16,spinlock_t;
@@
 struct fwnet_device {  struct list_head dev_link ;  spinlock_t lock ;  enum { FWNET_BROADCAST_ERROR , FWNET_BROADCAST_RUNNING , FWNET_BROADCAST_STOPPED , } broadcast_state ;  struct fw_iso_context * broadcast_rcv_context ;  struct fw_iso_buffer broadcast_rcv_buffer ;  void * * broadcast_rcv_buffer_ptrs ;  unsigned broadcast_rcv_next_ptr ;  unsigned num_broadcast_rcv_ptrs ;  unsigned rcv_buffer_size ;  unsigned broadcast_xmt_max_payload ;  u16 broadcast_xmt_datagramlabel ;  struct fw_address_handler handler ;  u64 local_fifo ;  int queued_datagrams ;  int peer_count ;  struct list_head peer_list ;  struct fw_card * card ;  struct net_device * netdev ; } cocci_id@p ;
...
position p;
@@
 struct fwnet_header * cocci_id@p ;
...
position p;
@@
 struct fwnet_fragment_info cocci_id@p ;
...
typedef u32;
@@
 struct rfc2734_header {  u32 w0 ;  u32 w1 ; } cocci_id@p ;
...
position p;
@@
 struct fwnet_packet_task cocci_id@p ;
...
typedef u16;
@@
 struct fwnet_fragment_info {  struct list_head fi_link ;  u16 offset ;  u16 len ; } cocci_id@p ;
...
position p;
@@
 struct rfc2734_header cocci_id@p ;
...
position p;
@@
 const struct ieee1394_device_id cocci_id@p [ ] ;
...
position p;
@@
 struct fwnet_partial_datagram * cocci_id@p ;
...
position p;
@@
 struct fw_descriptor cocci_id@p ;
