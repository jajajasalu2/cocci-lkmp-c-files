...
position p;
@@
 struct tgfx {  struct pardevice * pd ;  struct timer_list timer ;  struct input_dev * dev [ TGFX_MAX_DEVICES ] ;  char name [ TGFX_MAX_DEVICES ] [ 64 ] ;  char phys [ TGFX_MAX_DEVICES ] [ 32 ] ;  int sticks ;  int used ;  int parportno ;  struct mutex sem ; } * cocci_id@p [ TGFX_MAX_PORTS ] ;
...
position p;
@@
 struct tgfx_config cocci_id@p [ TGFX_MAX_PORTS ] ;
...
position p;
@@
 struct tgfx_config {  int args [ TGFX_MAX_DEVICES + 1 ] ;  unsigned int nargs ; } cocci_id@p ;
