...
symbol cocci_id;
position p;
typedef port_status_t;
@@
 port_status_t * cocci_id@p ;
...
@@
 struct card {  int n_ports ;  u8 irq ;  u8 __iomem * plx ;  struct pci_dev * pdev ;  int rx_in ;  struct sk_buff * rx_skbs [ RX_QUEUE_LENGTH ] ;  struct card_status * status ;  dma_addr_t status_address ;  struct port ports [ 0 ] ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef desc_t,port_status_t;
@@
 struct card_status {  desc_t rx_descs [ RX_QUEUE_LENGTH ] ;  port_status_t port_status [ 4 ] ; } cocci_id@p ;
