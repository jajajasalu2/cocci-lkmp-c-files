...
position p;
@@
 struct {  long jiffies ;  unsigned cycles ;  unsigned num ; } cocci_id@p ;
...
position p;
@@
 struct timer_rand_state * cocci_id@p ;
...
position p;
@@
 struct {  unsigned long now ;  struct timer_list timer ; } cocci_id@p ;
...
@@
 struct batched_entropy {  union {  u64 entropy_u64 [ CHACHA_BLOCK_SIZE / sizeof ( u64 ) ] ;  u32 entropy_u32 [ CHACHA_BLOCK_SIZE / sizeof ( u32 ) ] ; } ;  unsigned int position ;  spinlock_t batch_lock ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef __u32,spinlock_t;
@@
 struct crng_state {  __u32 state [ 16 ] ;  unsigned long init_time ;  spinlock_t lock ; } cocci_id@p ;
...
typedef __u32,__u8;
@@
 union {  __u8 block [ CHACHA_BLOCK_SIZE ] ;  __u32 key [ 8 ] ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef __u32,__u8,spinlock_t;
@@
 struct entropy_store {  const struct poolinfo * poolinfo ;  __u32 * pool ;  const char * name ;  struct entropy_store * pull ;  struct work_struct push_work ;  unsigned long last_pulled ;  spinlock_t lock ;  unsigned short add_ptr ;  unsigned short input_rotate ;  int entropy_count ;  unsigned int initialized : 1 ;  unsigned int last_data_init : 1 ;  __u8 last_data [ EXTRACT_SIZE ] ; } cocci_id@p ;
...
typedef __u32;
@@
 struct fast_pool {  __u32 pool [ 4 ] ;  unsigned long last ;  unsigned short reg_idx ;  unsigned char count ; } cocci_id@p ;
...
typedef __u32;
@@
 union {  __u32 w [ 5 ] ;  unsigned long l [ LONGS ( 20 ) ] ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( void * buf , int nbytes ) ;
...
position p;
@@
 struct random_ready_callback * cocci_id@p ;
...
position p;
@@
 struct batched_entropy * cocci_id@p ;
...
typedef __u8;
@@
 void cocci_id@p ( struct crng_state * crng , __u8 tmp [ CHACHA_BLOCK_SIZE ] , int used ) ;
...
position p;
@@
 struct entropy_store cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct crng_state * crng , struct entropy_store * r ) ;
...
typedef __u8;
@@
 void cocci_id@p ( struct crng_state * crng , __u8 out [ CHACHA_BLOCK_SIZE ] ) ;
