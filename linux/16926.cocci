...
position p;
@@
 struct pixcir_report_data * cocci_id@p ;
...
typedef bool;
@@
 struct pixcir_i2c_ts_data {  struct i2c_client * client ;  struct input_dev * input ;  struct gpio_desc * gpio_attb ;  struct gpio_desc * gpio_reset ;  struct gpio_desc * gpio_enable ;  struct gpio_desc * gpio_wake ;  const struct pixcir_i2c_chip_data * chip ;  struct touchscreen_properties prop ;  bool running ; } cocci_id@p ;
...
position p;
@@
 const struct pixcir_i2c_chip_data cocci_id@p ;
...
position p;
@@
 int cocci_id@p [ PIXCIR_MAX_SLOTS ] ;
...
position p;
@@
 struct pixcir_i2c_ts_data * cocci_id@p ;
...
position p;
@@
 enum pixcir_power_mode cocci_id@p ;
...
position p;
@@
 enum pixcir_power_mode { PIXCIR_POWER_ACTIVE , PIXCIR_POWER_IDLE , PIXCIR_POWER_HALT , } cocci_id@p ;
...
typedef u8;
@@
 u8 cocci_id@p [ 2 + PIXCIR_MAX_SLOTS * 5 ] ;
...
position p;
@@
 enum pixcir_int_mode cocci_id@p ;
...
typedef u8,bool;
@@
 struct pixcir_i2c_chip_data {  u8 max_fingers ;  bool has_hw_ids ; } cocci_id@p ;
...
position p;
@@
 enum pixcir_int_mode { PIXCIR_INT_PERIODICAL , PIXCIR_INT_DIFF_COORD , PIXCIR_INT_LEVEL_TOUCH , PIXCIR_INT_PULSE_TOUCH , } cocci_id@p ;
...
position p;
@@
 struct pixcir_report_data {  int num_touches ;  struct input_mt_pos pos [ PIXCIR_MAX_SLOTS ] ;  int ids [ PIXCIR_MAX_SLOTS ] ; } cocci_id@p ;
