...
symbol cocci_id;
position p;
typedef wait_queue_head_t,domid_t;
@@
 struct tpm_private {  struct tpm_chip * chip ;  struct xenbus_device * dev ;  struct vtpm_shared_page * shr ;  unsigned int evtchn ;  int ring_ref ;  domid_t backend_id ;  int irq ;  wait_queue_head_t read_queue ; } cocci_id@p ;
...
position p;
@@
 enum status_bits { VTPM_STATUS_RUNNING = 0x1 , VTPM_STATUS_IDLE = 0x2 , VTPM_STATUS_RESULT = 0x4 , VTPM_STATUS_CANCELED = 0x8 , } cocci_id@p ;
...
position p;
@@
 struct vtpm_shared_page * cocci_id@p ;
