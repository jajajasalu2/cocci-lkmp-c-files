...
position p;
@@
 int cocci_id@p ( struct notifier_block * mpoa_notifier , unsigned long event , void * dev ) ;
...
position p;
@@
 void cocci_id@p ( const char * addr , struct mpoa_client * mpc ) ;
...
position p;
@@
 void cocci_id@p ( struct k_message * msg , struct mpoa_client * mpc ) ;
...
position p;
@@
 void cocci_id@p ( struct atm_vcc * vcc , eg_cache_entry * entry ) ;
...
typedef uint8_t;
@@
 uint8_t cocci_id@p [ 4 + 1 + 1 + 1 + ATM_ESA_LEN ] ;
...
position p;
@@
 struct atm_mpoa_qos cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct k_message * msg , struct mpoa_client * mpc , int action ) ;
...
position p;
@@
 struct llc_snap_hdr cocci_id@p ;
...
typedef __be32;
@@
 struct {  struct llc_snap_hdr hdr ;  __be32 tag ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct k_message * mesg , struct mpoa_client * mpc ) ;
