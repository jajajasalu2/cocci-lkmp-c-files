...
position p;
@@
 struct ichxrom_map_info {  struct list_head list ;  struct map_info map ;  struct mtd_info * mtd ;  struct resource rsrc ;  char map_name [ sizeof ( MOD_NAME ) + 2 + ADDRESS_NAME_LEN ] ; } cocci_id@p ;
...
position p;
@@
 struct ichxrom_window {  void __iomem * virt ;  unsigned long phys ;  unsigned long size ;  struct list_head maps ;  struct resource rsrc ;  struct pci_dev * pdev ; } cocci_id@p ;
...
position p;
@@
 struct ichxrom_window cocci_id@p ;
...
position p;
@@
 struct ichxrom_map_info * cocci_id@p ;
