...
typedef u_char,bool;
@@
 struct omap_nand_info {  struct nand_chip nand ;  struct platform_device * pdev ;  int gpmc_cs ;  bool dev_ready ;  enum nand_io xfer_type ;  int devsize ;  enum omap_ecc ecc_opt ;  struct device_node * elm_of_node ;  unsigned long phys_base ;  struct completion comp ;  struct dma_chan * dma ;  int gpmc_irq_fifo ;  int gpmc_irq_count ;  enum { OMAP_NAND_IO_READ = 0 , OMAP_NAND_IO_WRITE , } iomode ;  u_char * buf ;  int buf_len ;  struct gpmc_nand_regs reg ;  struct gpmc_nand_ops * ops ;  bool flash_bbt ;  struct device * elm_dev ;  struct gpio_desc * ready_gpiod ; } cocci_id@p ;
...
position p;
@@
 struct omap_nand_info * cocci_id@p ;
...
position p;
@@
 struct elm_errorvec cocci_id@p [ ERROR_VECTOR_MAX ] ;
