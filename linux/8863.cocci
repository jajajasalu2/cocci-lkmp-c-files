...
position p;
@@
 enum hinic_offload_type { TX_OFFLOAD_TSO = BIT ( 0 ) , TX_OFFLOAD_CSUM = BIT ( 1 ) , TX_OFFLOAD_VLAN = BIT ( 2 ) , TX_OFFLOAD_INVALID = BIT ( 3 ) , } cocci_id@p ;
...
position p;
@@
 union hinic_l3 {  struct iphdr * v4 ;  struct ipv6hdr * v6 ;  unsigned char * hdr ; } cocci_id@p ;
...
position p;
@@
 union hinic_l4 {  struct tcphdr * tcp ;  struct udphdr * udp ;  unsigned char * hdr ; } cocci_id@p ;
...
position p;
@@
 enum hinic_offload_type cocci_id@p ;
...
position p;
@@
 struct hinic_txq_stats * cocci_id@p ;
