...
position p;
@@
 const struct csi2tx_fmt * cocci_id@p ;
...
position p;
@@
 struct csi2tx_vops {  void ( * dphy_setup ) ( struct csi2tx_priv * csi2tx ) ; } cocci_id@p ;
...
position p;
@@
 enum csi2tx_pads { CSI2TX_PAD_SOURCE , CSI2TX_PAD_SINK_STREAM0 , CSI2TX_PAD_SINK_STREAM1 , CSI2TX_PAD_SINK_STREAM2 , CSI2TX_PAD_SINK_STREAM3 , CSI2TX_PAD_MAX , } cocci_id@p ;
...
position p;
@@
 const struct csi2tx_vops cocci_id@p ;
...
typedef u8,bool;
@@
 struct csi2tx_priv {  struct device * dev ;  unsigned int count ;  struct mutex lock ;  void __iomem * base ;  struct csi2tx_vops * vops ;  struct clk * esc_clk ;  struct clk * p_clk ;  struct clk * pixel_clk [ CSI2TX_STREAMS_MAX ] ;  struct v4l2_subdev subdev ;  struct media_pad pads [ CSI2TX_PAD_MAX ] ;  struct v4l2_mbus_framefmt pad_fmts [ CSI2TX_PAD_MAX ] ;  bool has_internal_dphy ;  u8 lanes [ CSI2TX_LANES_MAX ] ;  unsigned int num_lanes ;  unsigned int max_lanes ;  unsigned int max_streams ; } cocci_id@p ;
...
typedef u32;
@@
 struct csi2tx_fmt {  u32 mbus ;  u32 dt ;  u32 bpp ; } cocci_id@p ;
