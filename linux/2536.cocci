...
typedef spinlock_t;
@@
 struct dmz_target {  struct dm_dev * ddev ;  unsigned long flags ;  struct dmz_dev * dev ;  struct dmz_metadata * metadata ;  struct dmz_reclaim * reclaim ;  struct radix_tree_root chunk_rxtree ;  struct workqueue_struct * chunk_wq ;  struct mutex chunk_lock ;  struct bio_set bio_set ;  spinlock_t flush_lock ;  struct bio_list flush_list ;  struct delayed_work flush_work ;  struct workqueue_struct * flush_wq ; } cocci_id@p ;
...
position p;
@@
 struct dm_chunk_work cocci_id@p ;
...
typedef refcount_t;
@@
 struct dm_chunk_work {  struct work_struct work ;  refcount_t refcount ;  struct dmz_target * target ;  unsigned int chunk ;  struct bio_list bio_list ; } cocci_id@p ;
...
typedef refcount_t;
@@
 struct dmz_bioctx {  struct dmz_target * target ;  struct dm_zone * zone ;  struct bio * bio ;  refcount_t ref ; } cocci_id@p ;
