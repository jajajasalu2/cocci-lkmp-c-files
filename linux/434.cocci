...
symbol cocci_id;
position p;
typedef __u32,__u64,compat_uptr_t;
@@
 struct compat_kvm_dirty_log {  __u32 slot ;  __u32 padding1 ;  union {  compat_uptr_t dirty_bitmap ;  __u64 padding2 ; } ; } cocci_id@p ;
...
position p;
@@
 struct compat_kvm_dirty_log cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( unsigned int type , struct kvm * kvm ) ;
...
position p;
@@
 struct kvm_coalesced_mmio_zone cocci_id@p ;
...
position p;
@@
 struct kvm_irq_routing_entry * cocci_id@p ;
...
position p;
@@
 struct kvm_ioeventfd cocci_id@p ;
...
symbol cocci_id;
position p;
typedef kvm_vm_thread_fn_t,uintptr_t;
@@
 struct kvm_vm_worker_thread_context {  struct kvm * kvm ;  struct task_struct * parent ;  struct completion init_done ;  kvm_vm_thread_fn_t thread_fn ;  uintptr_t data ;  int err ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct kvm_io_bus * bus ) ;
...
position p;
@@
 const struct mmu_notifier_range * cocci_id@p ;
...
position p;
@@
 struct swait_queue_head * cocci_id@p ;
...
position p;
@@
 struct kvm_signal_mask cocci_id@p ;
...
position p;
@@
 struct kvm_stat_data cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct kvm_device * dev , struct kvm_device_attr * attr ) ;
...
position p;
@@
 struct mmu_notifier * cocci_id@p ;
...
position p;
@@
 struct kvm_io_device * cocci_id@p ;
...
position p;
@@
 struct kvm_irq_routing cocci_id@p ;
...
position p;
@@
 struct kvm_io_range cocci_id@p ;
...
position p;
@@
 const struct kvm_io_range * cocci_id@p ;
...
position p;
@@
 const struct kvm_device_ops * cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ 8 + 1 + ITOA_MAX_LEN + 1 ] ;
...
position p;
@@
 struct kvm_create_device cocci_id@p ;
...
position p;
@@
 struct gfn_to_hva_cache * cocci_id@p ;
...
position p;
@@
 struct file_operations cocci_id@p ;
...
position p;
@@
 const struct kvm_device_ops * cocci_id@p [ KVM_DEV_TYPE_MAX ] ;
...
position p;
@@
 struct kvm_vm_worker_thread_context cocci_id@p ;
...
position p;
@@
 long cocci_id@p ( struct file * file , unsigned int ioctl , unsigned long arg ) ;
...
symbol cocci_id;
position p;
typedef kvm_vm_thread_fn_t;
@@
 kvm_vm_thread_fn_t cocci_id@p ;
...
position p;
@@
 const struct mmu_notifier_ops cocci_id@p ;
...
position p;
@@
 struct kobj_uevent_env * cocci_id@p ;
...
position p;
@@
 struct preempt_notifier * cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ ITOA_MAX_LEN * 2 ] ;
...
position p;
@@
 struct kvm_device_attr cocci_id@p ;
