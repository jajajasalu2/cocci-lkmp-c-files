...
position p;
@@
 const struct cpcap_battery_config cocci_id@p ;
...
position p;
@@
 const char * const cocci_id@p [ CPCAP_BATTERY_IIO_NR ] ;
...
typedef s16,s32;
@@
 struct cpcap_coulomb_counter_data {  s32 sample ;  s32 accumulator ;  s16 offset ;  s16 integrator ; } cocci_id@p ;
...
position p;
@@
 struct cpcap_coulomb_counter_data * cocci_id@p ;
...
position p;
@@
 struct cpcap_battery_state_data cocci_id@p ;
...
position p;
@@
 struct cpcap_battery_config {  int cd_factor ;  struct power_supply_info info ;  struct power_supply_battery_info bat ; } cocci_id@p ;
...
position p;
@@
 enum cpcap_battery_irq_action { CPCAP_BATTERY_IRQ_ACTION_NONE , CPCAP_BATTERY_IRQ_ACTION_CC_CAL_DONE , CPCAP_BATTERY_IRQ_ACTION_BATTERY_LOW , CPCAP_BATTERY_IRQ_ACTION_POWEROFF , } cocci_id@p ;
...
position p;
@@
 struct cpcap_interrupt_desc {  const char * name ;  struct list_head node ;  int irq ;  enum cpcap_battery_irq_action action ; } cocci_id@p ;
...
position p;
@@
 enum cpcap_battery_state { CPCAP_BATTERY_STATE_PREVIOUS , CPCAP_BATTERY_STATE_LATEST , CPCAP_BATTERY_STATE_NR , } cocci_id@p ;
...
position p;
@@
 struct cpcap_battery_ddata * cocci_id@p ;
...
position p;
@@
 enum { CPCAP_BATTERY_IIO_BATTDET , CPCAP_BATTERY_IIO_VOLTAGE , CPCAP_BATTERY_IIO_CHRG_CURRENT , CPCAP_BATTERY_IIO_BATT_CURRENT , CPCAP_BATTERY_IIO_NR , } cocci_id@p ;
...
position p;
@@
 struct cpcap_interrupt_desc * cocci_id@p ;
...
position p;
@@
 enum cpcap_battery_state cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u32,u16,atomic_t;
@@
 struct cpcap_battery_ddata {  struct device * dev ;  struct regmap * reg ;  struct list_head irq_list ;  struct iio_channel * channels [ CPCAP_BATTERY_IIO_NR ] ;  struct power_supply * psy ;  struct cpcap_battery_config config ;  struct cpcap_battery_state_data state [ CPCAP_BATTERY_STATE_NR ] ;  u32 cc_lsb ;  atomic_t active ;  int status ;  u16 vendor ; } cocci_id@p ;
...
typedef ktime_t;
@@
 struct cpcap_battery_state_data {  int voltage ;  int current_ua ;  int counter_uah ;  int temperature ;  ktime_t time ;  struct cpcap_coulomb_counter_data cc ; } cocci_id@p ;
