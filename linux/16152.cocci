...
position p;
@@
 enum { SAWTOOTH_CHAN , SQUARE_CHAN , } cocci_id@p ;
...
typedef u64;
@@
 struct waveform_private {  struct timer_list ai_timer ;  u64 ai_convert_time ;  unsigned int wf_amplitude ;  unsigned int wf_period ;  unsigned int wf_current ;  unsigned int ai_scan_period ;  unsigned int ai_convert_period ;  struct timer_list ao_timer ;  struct comedi_device * dev ;  u64 ao_last_scan_time ;  unsigned int ao_scan_period ;  unsigned short ao_loopbacks [ N_CHANS ] ; } cocci_id@p ;
...
position p;
@@
 struct waveform_private * cocci_id@p ;
