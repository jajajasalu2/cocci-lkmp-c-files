...
position p;
@@
 enum snd_rme_domain { SND_RME_DOMAIN_SYSTEM , SND_RME_DOMAIN_AES , SND_RME_DOMAIN_SPDIF , } cocci_id@p ;
...
typedef u8,u32;
@@
 const struct rc_config {  u32 usb_id ;  u8 offset ;  u8 length ;  u8 packet_length ;  u8 min_packet_length ;  u8 mute_mixer_id ;  u32 mute_code ; } cocci_id@p [ ] ;
...
position p;
@@
 enum snd_rme_clock_status { SND_RME_CLOCK_NOLOCK , SND_RME_CLOCK_LOCK , SND_RME_CLOCK_SYNC , } cocci_id@p ;
...
position p;
@@
 struct std_mono_table cocci_id@p [ ] ;
...
position p;
@@
 const struct sb_jack {  int unitid ;  const char * name ; } cocci_id@p [ ] ;
...
position p;
@@
 const struct sb_jack * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef snd_kcontrol_tlv_rw_t;
@@
 snd_kcontrol_tlv_rw_t * cocci_id@p ;
...
position p;
@@
 const struct rc_config * cocci_id@p ;
