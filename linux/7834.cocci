...
typedef bool,u32;
@@
 struct hwsim_phy {  struct ieee802154_hw * hw ;  u32 idx ;  struct hwsim_pib __rcu * pib ;  bool suspended ;  struct list_head edges ;  struct list_head list ; } cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ MAC802154_HWSIM_ATTR_MAX + 1 ] ;
...
typedef u8;
@@
 struct hwsim_edge_info {  u8 lqi ;  struct rcu_head rcu ; } cocci_id@p ;
...
typedef bool;
@@
 int cocci_id@p ( struct genl_info * info , struct device * dev , bool init ) ;
...
position p;
@@
 struct hwsim_edge {  struct hwsim_phy * endpoint ;  struct hwsim_edge_info __rcu * info ;  struct list_head list ;  struct rcu_head rcu ; } cocci_id@p ;
...
position p;
@@
 struct hwsim_edge_info * cocci_id@p ;
...
position p;
@@
 struct nlattr * cocci_id@p [ MAC802154_HWSIM_EDGE_ATTR_MAX + 1 ] ;
...
typedef u8;
@@
 struct hwsim_pib {  u8 page ;  u8 channel ;  struct rcu_head rcu ; } cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ MAC802154_HWSIM_EDGE_ATTR_MAX + 1 ] ;
...
position p;
@@
 void cocci_id@p ( struct hwsim_phy * phy ) ;
