...
position p;
@@
 const struct bsg_ops * cocci_id@p ;
...
position p;
@@
 struct bsg_class_device * cocci_id@p ;
...
position p;
@@
 struct hlist_head cocci_id@p [ BSG_LIST_ARRAY_SIZE ] ;
...
@@
 struct bsg_device {  struct request_queue * queue ;  spinlock_t lock ;  struct hlist_node dev_list ;  refcount_t ref_count ;  char name [ 20 ] ;  int max_queue ; } cocci_id@p ;
