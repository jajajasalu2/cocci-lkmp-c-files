...
position p;
@@
 struct ipu_flow {  struct ipu_dp foreground ;  struct ipu_dp background ;  enum ipu_color_space out_cs ;  void __iomem * base ;  struct ipu_dp_priv * priv ; } cocci_id@p ;
...
typedef bool,u32;
@@
 struct ipu_dp {  u32 flow ;  bool in_use ;  bool foreground ;  enum ipu_color_space in_cs ; } cocci_id@p ;
...
position p;
@@
 struct ipu_dp_priv {  struct ipu_soc * ipu ;  struct device * dev ;  void __iomem * base ;  struct ipu_flow flow [ IPUV3_NUM_FLOWS ] ;  struct mutex mutex ;  int use_count ; } cocci_id@p ;
