...
typedef u32;
@@
 const struct {  u32 reg ;  u32 val ; } cocci_id@p [ ] ;
...
position p;
@@
 struct xgene_phy_ctx * cocci_id@p ;
...
typedef clk_type_t;
@@
 enum clk_type_t { CLK_EXT_DIFF = 0 , CLK_INT_DIFF = 1 , CLK_INT_SING = 2 , } cocci_id@p ;
...
position p;
@@
 enum xgene_phy_mode { MODE_SATA = 0 , MODE_SGMII = 1 , MODE_PCIE = 2 , MODE_USB = 3 , MODE_XFI = 4 , MODE_MAX , } cocci_id@p ;
...
typedef cmu_type_t;
@@
 enum cmu_type_t { REF_CMU = 0 , PHY_CMU = 1 , } cocci_id@p ;
...
typedef mux_type_t;
@@
 enum mux_type_t { MUX_SELECT_ATA = 0 , MUX_SELECT_SGMMII = 0 , } cocci_id@p ;
...
typedef clk_type_t;
@@
 struct xgene_phy_ctx {  struct device * dev ;  struct phy * phy ;  enum xgene_phy_mode mode ;  enum clk_type_t clk_type ;  void __iomem * sds_base ;  struct clk * clk ;  struct xgene_sata_override_param sata_param ; } cocci_id@p ;
...
typedef u32;
@@
 struct xgene_sata_override_param {  u32 speed [ MAX_LANE ] ;  u32 txspeed [ 3 ] ;  u32 txboostgain [ MAX_LANE * 3 ] ;  u32 txeyetuning [ MAX_LANE * 3 ] ;  u32 txeyedirection [ MAX_LANE * 3 ] ;  u32 txamplitude [ MAX_LANE * 3 ] ;  u32 txprecursor_cn1 [ MAX_LANE * 3 ] ;  u32 txprecursor_cn2 [ MAX_LANE * 3 ] ;  u32 txpostcursor_cp1 [ MAX_LANE * 3 ] ; } cocci_id@p ;
