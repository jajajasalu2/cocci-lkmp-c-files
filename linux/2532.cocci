...
position p;
@@
 struct dm_report_zones_args cocci_id@p ;
...
position p;
@@
 struct dm_rq_clone_bio_info cocci_id@p ;
...
position p;
@@
 const struct dax_operations cocci_id@p ;
...
position p;
@@
 struct dm_target_io cocci_id@p ;
...
symbol cocci_id;
position p;
typedef spinlock_t,atomic_t,blk_status_t;
@@
 struct dm_io {  unsigned magic ;  struct mapped_device * md ;  blk_status_t status ;  atomic_t io_count ;  struct bio * orig_bio ;  unsigned long start_time ;  spinlock_t endio_lock ;  struct dm_stats_aux stats_aux ;  struct dm_target_io tio ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct dm_io * io ) ;
...
position p;
@@
 struct dm_md_mempools {  struct bio_set bs ;  struct bio_set io_bs ; } cocci_id@p ;
...
position p;
@@
 struct dm_md_mempools * cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ DM_COOKIE_LENGTH ] ;
...
typedef sector_t;
@@
 struct clone_info {  struct dm_table * map ;  struct bio * bio ;  struct dm_io * io ;  sector_t sector ;  unsigned sector_count ; } cocci_id@p ;
...
position p;
@@
 struct table_device * cocci_id@p ;
...
typedef refcount_t;
@@
 struct table_device {  struct list_head list ;  refcount_t count ;  struct dm_dev dm_dev ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct mapped_device * md ) ;
...
position p;
@@
 enum kobject_action cocci_id@p ;
...
typedef bool;
@@
 struct dm_target_io {  unsigned magic ;  struct dm_io * io ;  struct dm_target * ti ;  unsigned target_bio_nr ;  unsigned * len_ptr ;  bool inside_dm_io ;  struct bio clone ; } cocci_id@p ;
...
@@
 struct dm_pr {  u64 old_key ;  u64 new_key ;  u32 flags ;  bool fail_early ; } cocci_id@p ;
...
position p;
@@
 unsigned ( * cocci_id@p ) ( struct dm_target * ti ) ;
