...
typedef bool;
@@
 bool cocci_id@p ( struct dm_table * t ) ;
...
position p;
@@
 enum blk_zoned_model * cocci_id@p ;
...
position p;
@@
 enum suspend_mode { PRESUSPEND , PRESUSPEND_UNDO , POSTSUSPEND , } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,fmode_t,sector_t;
@@
 struct dm_table {  struct mapped_device * md ;  enum dm_queue_mode type ;  unsigned int depth ;  unsigned int counts [ MAX_DEPTH ] ;  sector_t * index [ MAX_DEPTH ] ;  unsigned int num_targets ;  unsigned int num_allocated ;  sector_t * highs ;  struct dm_target * targets ;  struct target_type * immutable_target_type ;  bool integrity_supported : 1 ;  bool singleton : 1 ;  unsigned integrity_added : 1 ;  fmode_t mode ;  struct list_head devices ;  void ( * event_fn ) ( void * ) ;  void * event_context ;  struct dm_md_mempools * mempools ;  struct list_head target_callbacks ; } cocci_id@p ;
