...
position p;
@@
 enum gs_can_state { GS_CAN_STATE_ERROR_ACTIVE = 0 , GS_CAN_STATE_ERROR_WARNING , GS_CAN_STATE_ERROR_PASSIVE , GS_CAN_STATE_BUS_OFF , GS_CAN_STATE_STOPPED , GS_CAN_STATE_SLEEPING , } cocci_id@p ;
...
position p;
@@
 enum gs_can_identify_mode { GS_CAN_IDENTIFY_OFF = 0 , GS_CAN_IDENTIFY_ON , } cocci_id@p ;
...
position p;
@@
 struct gs_host_frame cocci_id@p ;
...
typedef atomic_t;
@@
 struct gs_usb {  struct gs_can * canch [ GS_MAX_INTF ] ;  struct usb_anchor rx_submitted ;  atomic_t active_channels ;  struct usb_device * udev ; } cocci_id@p ;
...
position p;
@@
 struct gs_identify_mode * cocci_id@p ;
...
position p;
@@
 enum gs_can_mode { GS_CAN_MODE_RESET = 0 , GS_CAN_MODE_START , } cocci_id@p ;
...
position p;
@@
 struct gs_tx_context {  struct gs_can * dev ;  unsigned int echo_id ; } cocci_id@p ;
...
position p;
@@
 struct gs_tx_context * cocci_id@p ;
...
position p;
@@
 struct gs_device_bittiming * cocci_id@p ;
...
position p;
@@
 struct gs_host_config * cocci_id@p ;
...
position p;
@@
 struct gs_device_config * cocci_id@p ;
...
position p;
@@
 struct gs_device_mode * cocci_id@p ;
...
position p;
@@
 enum gs_usb_breq { GS_USB_BREQ_HOST_FORMAT = 0 , GS_USB_BREQ_BITTIMING , GS_USB_BREQ_MODE , GS_USB_BREQ_BERR , GS_USB_BREQ_BT_CONST , GS_USB_BREQ_DEVICE_CONFIG , GS_USB_BREQ_TIMESTAMP , GS_USB_BREQ_IDENTIFY , } cocci_id@p ;
...
@@
 struct gs_can {  struct can_priv can ;  struct gs_usb * parent ;  struct net_device * netdev ;  struct usb_device * udev ;  struct usb_interface * iface ;  struct can_bittiming_const bt_const ;  unsigned int channel ;  spinlock_t tx_ctx_lock ;  struct gs_tx_context tx_context [ GS_MAX_TX_URBS ] ;  struct usb_anchor tx_submitted ;  atomic_t active_tx_urbs ; } cocci_id@p ;
...
position p;
@@
 struct gs_device_bt_const * cocci_id@p ;
