...
position p;
@@
 struct aspeed_gpio_config {  unsigned int nr_gpios ;  const struct aspeed_bank_props * props ; } cocci_id@p ;
...
position p;
@@
 const unsigned long int cocci_id@p ;
...
typedef uint16_t;
@@
 struct aspeed_gpio_bank {  uint16_t val_regs ;  uint16_t rdata_reg ;  uint16_t irq_regs ;  uint16_t debounce_regs ;  uint16_t tolerance_regs ;  uint16_t cmdsrc_regs ;  const char names [ 4 ] [ 3 ] ; } cocci_id@p ;
...
position p;
@@
 const struct aspeed_gpio_bank * cocci_id@p ;
...
position p;
@@
 const struct aspeed_bank_props * cocci_id@p ;
...
position p;
@@
 const struct aspeed_gpio_config cocci_id@p ;
...
position p;
@@
 const enum aspeed_gpio_reg cocci_id@p ;
...
@@
 struct aspeed_gpio {  struct gpio_chip chip ;  struct irq_chip irqc ;  spinlock_t lock ;  void __iomem * base ;  int irq ;  const struct aspeed_gpio_config * config ;  u8 * offset_timer ;  unsigned int timer_users [ 4 ] ;  struct clk * clk ;  u32 * dcache ;  u8 * cf_copro_bankmap ; } cocci_id@p ;
...
typedef u32;
@@
 struct aspeed_bank_props {  unsigned int bank ;  u32 input ;  u32 output ; } cocci_id@p ;
...
position p;
@@
 const struct aspeed_gpio_copro_ops * cocci_id@p ;
