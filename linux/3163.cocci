...
position p;
@@
 struct attribute * cocci_id@p [ 6 ] [ 5 ] ;
...
typedef u8,u16;
@@
 struct vt8231_data {  unsigned short addr ;  const char * name ;  struct mutex update_lock ;  struct device * hwmon_dev ;  char valid ;  unsigned long last_updated ;  u8 in [ 6 ] ;  u8 in_max [ 6 ] ;  u8 in_min [ 6 ] ;  u16 temp [ 6 ] ;  u8 temp_max [ 6 ] ;  u8 temp_min [ 6 ] ;  u8 fan [ 2 ] ;  u8 fan_min [ 2 ] ;  u8 fan_div [ 2 ] ;  u16 alarms ;  u8 uch_config ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct pci_dev * dev , const struct pci_device_id * id ) ;
...
position p;
@@
 struct vt8231_data * cocci_id@p ( struct device * dev ) ;
...
position p;
@@
 void cocci_id@p ( struct vt8231_data * data ) ;
