...
position p;
@@
 const struct isi_port * cocci_id@p ;
...
position p;
@@
 struct isi_board cocci_id@p [ BOARD_COUNT ] ;
...
typedef spinlock_t;
@@
 struct isi_board {  unsigned long base ;  int irq ;  unsigned char port_count ;  unsigned short status ;  unsigned short port_status ;  unsigned short shift_count ;  struct isi_port * ports ;  signed char count ;  spinlock_t card_lock ;  unsigned long flags ;  unsigned int index ; } cocci_id@p ;
...
typedef u8,u16;
@@
 struct stframe {  u16 addr ;  u16 count ;  u8 data [ 0 ] ; } * cocci_id@p ;
...
typedef u16;
@@
 struct isi_port {  unsigned short magic ;  struct tty_port port ;  u16 channel ;  u16 status ;  struct isi_board * card ;  unsigned char * xmit_buf ;  int xmit_head ;  int xmit_tail ;  int xmit_cnt ; } cocci_id@p ;
...
position p;
@@
 struct isi_port cocci_id@p [ PORT_COUNT ] ;
