...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_ETF_MAX + 1 ] ;
...
position p;
@@
 struct etf_sched_data * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef s32,bool,ktime_t;
@@
 struct etf_sched_data {  bool offload ;  bool deadline_mode ;  bool skip_sock_check ;  int clockid ;  int queue ;  s32 delta ;  ktime_t last ;  struct rb_root_cached head ;  struct qdisc_watchdog watchdog ;  ktime_t ( * get_time ) ( void ) ; } cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_ETF_MAX + 1 ] ;
