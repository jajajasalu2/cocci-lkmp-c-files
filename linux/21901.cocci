...
@@
 struct skb_segment_test {  const char * descr ;  struct sk_buff * ( * build_skb ) ( void ) ;  netdev_features_t features ; } cocci_id@p ;
...
position p;
@@
 const struct bpf_test * cocci_id@p ;
...
position p;
@@
 const struct skb_segment_test * cocci_id@p ;
...
position p;
@@
 enum { CLASSIC = BIT ( 6 ) , INTERNAL = BIT ( 7 ) , } cocci_id@p ;
