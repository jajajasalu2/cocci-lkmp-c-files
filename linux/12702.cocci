...
symbol cocci_id;
position p;
typedef __le32,spinlock_t,dma_addr_t;
@@
 struct pcilynx {  struct pci_dev * pci_device ; 
	 __iomem 
		  char * registers ;  struct pcl * rcv_start_pcl , * rcv_pcl ;  __le32 * rcv_buffer ;  dma_addr_t rcv_start_pcl_bus , rcv_pcl_bus , rcv_buffer_bus ;  spinlock_t client_list_lock ;  struct list_head client_list ;  struct miscdevice misc ;  struct list_head link ;  struct kref kref ; } cocci_id@p ;
...
position p;
@@
 struct packet {  unsigned int length ;  char data [ 0 ] ; } cocci_id@p ;
...
typedef __le32,u32;
@@
 struct pcl {  __le32 next ;  __le32 async_error_next ;  u32 user_data ;  __le32 pcl_status ;  __le32 remaining_transfer_count ;  __le32 next_data_buffer ;  struct {  __le32 control ;  __le32 pointer ; } buffer [ 13 ] ; } cocci_id@p ;
...
position p;
@@
 struct packet_buffer * cocci_id@p ;
...
typedef u32;
@@
 struct client {  struct pcilynx * lynx ;  u32 tcode_mask ;  struct packet_buffer buffer ;  struct list_head link ; } cocci_id@p ;
