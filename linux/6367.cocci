...
position p;
@@
 struct rpmh_vreg_hw_data {  enum rpmh_regulator_type regulator_type ;  const struct regulator_ops * ops ;  const struct regulator_linear_range voltage_range ;  int n_voltages ;  int hpm_min_load_uA ;  const int * pmic_mode_map ;  unsigned int ( * of_map_mode ) ( unsigned int mode ) ; } cocci_id@p ;
...
position p;
@@
 struct rpmh_vreg_init_data {  const char * name ;  const char * resource_name ;  const char * supply_name ;  const struct rpmh_vreg_hw_data * hw_data ; } cocci_id@p ;
...
position p;
@@
 const int cocci_id@p [ REGULATOR_MODE_STANDBY + 1 ] ;
...
position p;
@@
 enum rpmh_regulator_type { VRM , XOB , } cocci_id@p ;
...
typedef bool,u32;
@@
 struct rpmh_vreg {  struct device * dev ;  u32 addr ;  struct regulator_desc rdesc ;  const struct rpmh_vreg_hw_data * hw_data ;  bool always_wait_for_ack ;  int enabled ;  bool bypassed ;  int voltage_selector ;  unsigned int mode ; } cocci_id@p ;
...
position p;
@@
 const struct rpmh_vreg_init_data cocci_id@p [ ] ;
...
position p;
@@
 const struct rpmh_vreg_hw_data cocci_id@p ;
