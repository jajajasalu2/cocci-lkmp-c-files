...
position p;
@@
 struct bpf_lwt_prog * cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ LWT_BPF_PROG_MAX + 1 ] ;
...
position p;
@@
 struct nlattr * cocci_id@p [ LWT_BPF_MAX + 1 ] ;
...
position p;
@@
 struct nlattr * cocci_id@p [ LWT_BPF_PROG_MAX + 1 ] ;
...
position p;
@@
 struct bpf_lwt {  struct bpf_lwt_prog in ;  struct bpf_lwt_prog out ;  struct bpf_lwt_prog xmit ;  int family ; } cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ LWT_BPF_MAX + 1 ] ;
