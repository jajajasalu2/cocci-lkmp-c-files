...
position p;
@@
 struct i2c_hid_platform_data * cocci_id@p ;
...
position p;
@@
 const struct i2c_hid_cmd cocci_id@p ;
...
position p;
@@
 struct i2c_hid_desc cocci_id@p ;
...
typedef __u32,__u16;
@@
 const struct i2c_hid_quirks {  __u16 idVendor ;  __u16 idProduct ;  __u32 quirks ; } cocci_id@p [ ] ;
...
symbol cocci_id;
position p;
typedef u8,bool,__u8,wait_queue_head_t;
@@
 struct i2c_hid {  struct i2c_client * client ;  struct hid_device * hid ;  union {  __u8 hdesc_buffer [ sizeof ( struct i2c_hid_desc ) ] ;  struct i2c_hid_desc hdesc ; } ;  __le16 wHIDDescRegister ;  unsigned int bufsize ;  u8 * inbuf ;  u8 * rawbuf ;  u8 * cmdbuf ;  u8 * argsbuf ;  unsigned long flags ;  unsigned long quirks ;  wait_queue_head_t wait ;  struct i2c_hid_platform_data pdata ;  bool irq_wake_enabled ;  struct mutex reset_lock ; } cocci_id@p ;
...
typedef bool,__u8;
@@
 struct i2c_hid_cmd {  unsigned int registerIndex ;  __u8 opcode ;  unsigned int length ;  bool wait ; } cocci_id@p ;
...
typedef u8,__u8;
@@
 union command {  u8 data [ 0 ] ;  struct cmd {  __le16 reg ;  __u8 reportTypeID ;  __u8 opcode ; } __packed c ; } cocci_id@p ;
