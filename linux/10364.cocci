...
symbol cocci_id;
position p;
typedef bool,u16,wait_queue_head_t,ktime_t,spinlock_t,dma_addr_t;
@@
 struct tilcdc_crtc {  struct drm_crtc base ;  struct drm_plane primary ;  const struct tilcdc_panel_info * info ;  struct drm_pending_vblank_event * event ;  struct mutex enable_lock ;  bool enabled ;  bool shutdown ;  wait_queue_head_t frame_done_wq ;  bool frame_done ;  spinlock_t irq_lock ;  unsigned int lcd_fck_rate ;  ktime_t last_vblank ;  unsigned int hvtotal_us ;  struct drm_framebuffer * next_fb ;  bool simulate_vesa_sync ;  int sync_lost_count ;  bool frame_intact ;  struct work_struct recover_work ;  dma_addr_t palette_dma_handle ;  u16 * palette_base ;  struct completion palette_loaded ; } cocci_id@p ;
...
position p;
@@
 struct tilcdc_drm_private * cocci_id@p ;
...
position p;
@@
 const struct tilcdc_panel_info * cocci_id@p ;
