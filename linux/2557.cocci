...
symbol cocci_id;
position p;
typedef wait_queue_head_t,spinlock_t,mempool_t,atomic_t;
@@
 struct dm_kcopyd_client {  struct page_list * pages ;  unsigned nr_reserved_pages ;  unsigned nr_free_pages ;  unsigned sub_job_size ;  struct dm_io_client * io_client ;  wait_queue_head_t destroyq ;  mempool_t job_pool ;  struct workqueue_struct * kcopyd_wq ;  struct work_struct kcopyd_work ;  struct dm_kcopyd_throttle * throttle ;  atomic_t nr_jobs ;  spinlock_t job_lock ;  struct list_head callback_jobs ;  struct list_head complete_jobs ;  struct list_head io_jobs ;  struct list_head pages_jobs ; } cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct kcopyd_job * ) ;
...
position p;
@@
 dm_kcopyd_notify_fn cocci_id@p ;
...
position p;
@@
 struct dm_kcopyd_client * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef sector_t,atomic_t;
@@
 struct kcopyd_job {  struct dm_kcopyd_client * kc ;  struct list_head list ;  unsigned long flags ;  int read_err ;  unsigned long write_err ;  int rw ;  struct dm_io_region source ;  unsigned int num_dests ;  struct dm_io_region dests [ DM_KCOPYD_MAX_REGIONS ] ;  struct page_list * pages ;  dm_kcopyd_notify_fn fn ;  void * context ;  struct mutex lock ;  atomic_t sub_jobs ;  sector_t progress ;  sector_t write_offset ;  struct kcopyd_job * master_job ; } cocci_id@p ;
...
position p;
@@
 struct dm_kcopyd_throttle * cocci_id@p ;
