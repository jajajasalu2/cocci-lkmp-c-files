...
position p;
@@
 struct platform_data * cocci_id@p ;
...
typedef u32;
@@
 struct sensor_data {  u32 id ;  u32 hwmon_index ;  u32 opal_index ;  enum sensors type ;  char label [ MAX_LABEL_LEN ] ;  char name [ MAX_ATTR_LEN ] ;  struct device_attribute dev_attr ;  struct sensor_group_data * sgrp_data ; } cocci_id@p ;
...
typedef bool,u32;
@@
 struct sensor_group_data {  struct mutex mutex ;  u32 gid ;  bool enable ; } cocci_id@p ;
...
typedef u32;
@@
 struct platform_data {  const struct attribute_group * attr_groups [ MAX_SENSOR_TYPE + 1 ] ;  struct sensor_group_data * sgrp_data ;  u32 sensors_count ;  u32 nr_sensor_groups ; } cocci_id@p ;
...
typedef u32;
@@
 struct sensor_group {  const char * name ;  struct attribute_group group ;  u32 attr_count ;  u32 hwmon_index ; } cocci_id@p [ ] ;
...
typedef u32;
@@
 u32 cocci_id@p [ MAX_SENSOR_TYPE ] ;
...
position p;
@@
 struct sensor_group_data * cocci_id@p ;
...
position p;
@@
 enum sensors { FAN , TEMP , POWER_SUPPLY , POWER_INPUT , CURRENT , ENERGY , MAX_SENSOR_TYPE , } cocci_id@p ;
