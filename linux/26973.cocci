...
position p;
@@
 char cocci_id@p [ INET6_ADDRSTRLEN + IPV6_SCOPE_ID_LEN + 1 ] ;
...
position p;
@@
 const struct nfs_dns_ent * cocci_id@p ;
...
position p;
@@
 struct nfs_dns_ent {  struct cache_head h ;  char * hostname ;  size_t namelen ;  struct sockaddr_storage addr ;  size_t addrlen ;  struct rcu_head rcu_head ; } cocci_id@p ;
...
position p;
@@
 struct nfs_cache_defer_req * cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ NFS_DNS_HOSTNAME_MAXLEN + 1 ] ;
