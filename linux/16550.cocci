...
symbol cocci_id;
position p;
typedef bool,__u32,__u8,wait_queue_head_t,spinlock_t;
@@
 struct virtio_ccw_device {  struct virtio_device vdev ;  __u8 config [ VIRTIO_CCW_CONFIG_SIZE ] ;  struct ccw_device * cdev ;  __u32 curr_io ;  int err ;  unsigned int revision ;  wait_queue_head_t wait_q ;  spinlock_t lock ;  struct mutex io_lock ;  struct list_head virtqueues ;  bool is_thinint ;  bool going_away ;  bool device_lost ;  unsigned int config_ready ;  void * airq_info ;  struct vcdev_dma_area * dma_area ; } cocci_id@p ;
...
position p;
@@
 struct virtio_ccw_vq_info {  struct virtqueue * vq ;  int num ;  union {  struct vq_info_block s ;  struct vq_info_block_legacy l ; } * info_block ;  int bit_nr ;  struct list_head node ;  long cookie ; } cocci_id@p ;
...
typedef __u8;
@@
 struct vcdev_dma_area {  unsigned long indicators ;  unsigned long indicators2 ;  struct vq_config_block config_block ;  __u8 status ; } cocci_id@p ;
...
position p;
@@
 struct virtio_ccw_vq_info cocci_id@p ;
...
position p;
@@
 struct virtio_ccw_device * cocci_id@p ;
...
position p;
@@
 struct virtio_rev_info * cocci_id@p ;
...
position p;
@@
 unsigned long cocci_id@p [ __MAX_SSID + 1 ] [ __DEV_WORDS ] ;
...
typedef __u16,__u8;
@@
 struct virtio_rev_info {  __u16 revision ;  __u16 length ;  __u8 data [ ] ; } cocci_id@p ;
...
position p;
@@
 struct virtio_feature_desc * cocci_id@p ;
...
position p;
@@
 const struct virtio_config_ops cocci_id@p ;
...
position p;
@@
 struct vq_config_block cocci_id@p ;
...
position p;
@@
 struct airq_info * cocci_id@p [ MAX_AIRQ_AREAS ] ;
...
typedef u8,rwlock_t;
@@
 struct airq_info {  rwlock_t lock ;  u8 summary_indicator_idx ;  struct airq_struct airq ;  struct airq_iv * aiv ; } cocci_id@p ;
...
position p;
@@
 struct virtio_thinint_area * cocci_id@p ;
