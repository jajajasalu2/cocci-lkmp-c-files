...
position p;
@@
 struct drm_gem_cma_object * cocci_id@p [ ARRAY_SIZE ( shader_reloc_offsets ) + 8 ] ;
...
position p;
@@
 enum vc4_texture_data_type cocci_id@p ;
...
position p;
@@
 struct vc4_validated_shader_info * cocci_id@p ;
...
position p;
@@
 struct vc4_texture_sample_info * cocci_id@p ;
...
typedef uint16_t;
@@
 const struct cmd_info {  uint16_t len ;  const char * name ;  int ( * func ) ( struct vc4_exec_info * exec , void * validated , void * untrusted ) ; } cocci_id@p [ ] ;
