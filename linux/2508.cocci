...
position p;
@@
 struct bop_ring_buffer {  unsigned begin ;  unsigned end ;  struct block_op bops [ MAX_RECURSIVE_ALLOCATIONS + 1 ] ; } cocci_id@p ;
...
position p;
@@
 enum allocation_event cocci_id@p ;
...
position p;
@@
 const struct dm_space_map cocci_id@p ;
...
typedef dm_block_t;
@@
 struct sm_metadata {  struct dm_space_map sm ;  struct ll_disk ll ;  struct ll_disk old_ll ;  dm_block_t begin ;  unsigned recursion_count ;  unsigned allocated_this_transaction ;  struct bop_ring_buffer uncommitted ;  struct threshold threshold ; } cocci_id@p ;
...
typedef dm_block_t;
@@
 struct block_op {  enum block_op_type type ;  dm_block_t block ; } cocci_id@p ;
...
position p;
@@
 struct disk_sm_root cocci_id@p ;
...
position p;
@@
 enum block_op_type { BOP_INC , BOP_DEC , } cocci_id@p ;
...
position p;
@@
 struct dm_transaction_manager * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,dm_block_t;
@@
 struct threshold {  bool threshold_set ;  bool value_set ;  dm_block_t threshold ;  dm_block_t current_value ;  dm_sm_threshold_fn fn ;  void * context ; } cocci_id@p ;
...
position p;
@@
 struct bop_ring_buffer * cocci_id@p ;
