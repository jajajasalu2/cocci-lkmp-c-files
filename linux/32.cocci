...
position p;
@@
 enum { SJ_SETJMP = 0 , SJ_FAIL , SJ_SUCCEED , } cocci_id@p ;
...
typedef uint64_t;
@@
 uint64_t ( * cocci_id@p ) ( const uint64_t * ) ;
...
typedef uint16_t;
@@
 uint16_t ( * cocci_id@p ) ( const uint16_t * ) ;
...
typedef uint32_t;
@@
 uint32_t ( * cocci_id@p ) ( const uint32_t * ) ;
...
symbol cocci_id;
position p;
typedef table_sort_t;
@@
 table_sort_t cocci_id@p ;
