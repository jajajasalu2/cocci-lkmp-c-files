...
position p;
@@
 struct tc_taprio_qopt_offload * cocci_id@p ;
...
position p;
@@
 const struct sched_gate_list * cocci_id@p ;
...
typedef s64,ktime_t;
@@
 struct sched_gate_list {  struct rcu_head rcu ;  struct list_head entries ;  size_t num_entries ;  ktime_t cycle_close_time ;  s64 cycle_time ;  s64 cycle_time_extension ;  s64 base_time ; } cocci_id@p ;
...
position p;
@@
 struct sched_gate_list * * cocci_id@p ;
...
position p;
@@
 struct tc_taprio_sched_entry * cocci_id@p ;
...
position p;
@@
 const struct sched_entry * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,u32,atomic_t,ktime_t;
@@
 struct sched_entry {  struct list_head list ;  ktime_t close_time ;  ktime_t next_txtime ;  atomic_t budget ;  int index ;  u32 gate_mask ;  u32 interval ;  u8 command ; } cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_TAPRIO_ATTR_MAX + 1 ] ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_TAPRIO_SCHED_ENTRY_MAX + 1 ] ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_TAPRIO_SCHED_ENTRY_MAX + 1 ] ;
...
position p;
@@
 struct taprio_sched * cocci_id@p ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_TAPRIO_ATTR_MAX + 1 ] ;
...
typedef refcount_t;
@@
 struct __tc_taprio_qopt_offload {  refcount_t users ;  struct tc_taprio_qopt_offload offload ; } cocci_id@p ;
...
position p;
@@
 const struct tc_mqprio_qopt * cocci_id@p ;
