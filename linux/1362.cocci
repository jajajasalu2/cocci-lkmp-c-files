...
symbol cocci_id;
position p;
typedef u16,wait_queue_head_t,atomic_t;
@@
 struct iowarrior {  struct mutex mutex ;  struct usb_device * udev ;  struct usb_interface * interface ;  unsigned char minor ;  struct usb_endpoint_descriptor * int_out_endpoint ;  struct usb_endpoint_descriptor * int_in_endpoint ;  struct urb * int_in_urb ;  unsigned char * int_in_buffer ;  unsigned char serial_number ;  unsigned char * read_queue ;  wait_queue_head_t read_wait ;  wait_queue_head_t write_wait ;  atomic_t write_busy ;  atomic_t read_idx ;  atomic_t intr_idx ;  atomic_t overflow_flag ;  int present ;  int opened ;  char chip_serial [ 9 ] ;  int report_size ;  u16 product_id ;  struct usb_anchor submitted ; } cocci_id@p ;
...
position p;
@@
 struct iowarrior_info cocci_id@p ;
