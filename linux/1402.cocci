...
position p;
@@
 struct s3c24xx_hsudc_platdata * cocci_id@p ;
...
typedef u8;
@@
 struct s3c_hsudc_ep {  struct usb_ep ep ;  char name [ 20 ] ;  struct s3c_hsudc * dev ;  struct list_head queue ;  u8 stopped ;  u8 wedge ;  u8 bEndpointAddress ;  void __iomem * fifo ; } cocci_id@p ;
...
position p;
@@
 struct s3c_hsudc_ep cocci_id@p ;
...
position p;
@@
 struct s3c_hsudc_req * cocci_id@p ;
...
position p;
@@
 struct s3c_hsudc_req {  struct usb_request req ;  struct list_head queue ; } cocci_id@p ;
...
typedef spinlock_t;
@@
 struct s3c_hsudc {  struct usb_gadget gadget ;  struct usb_gadget_driver * driver ;  struct device * dev ;  struct s3c24xx_hsudc_platdata * pd ;  struct usb_phy * transceiver ;  struct regulator_bulk_data supplies [ ARRAY_SIZE ( s3c_hsudc_supply_names ) ] ;  spinlock_t lock ;  void __iomem * regs ;  int irq ;  struct clk * uclk ;  int ep0state ;  struct s3c_hsudc_ep ep [ ] ; } cocci_id@p ;
