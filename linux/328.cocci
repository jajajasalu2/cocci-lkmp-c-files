...
typedef pid_t;
@@
 struct cgroup_pidlist {  struct {  enum cgroup_filetype type ;  struct pid_namespace * ns ; } key ;  pid_t * list ;  int length ;  struct list_head links ;  struct cgroup * owner ;  struct delayed_work destroy_dwork ; } cocci_id@p ;
...
position p;
@@
 struct cgroup_pidlist * * cocci_id@p ;
...
position p;
@@
 enum cgroup_filetype { CGROUP_FILE_PROCS , CGROUP_FILE_TASKS , } cocci_id@p ;
...
position p;
@@
 enum cgroup1_param { Opt_all , Opt_clone_children , Opt_cpuset_v2_mode , Opt_name , Opt_none , Opt_noprefix , Opt_release_agent , Opt_xattr , } cocci_id@p ;
...
position p;
@@
 enum cgroup_filetype cocci_id@p ;
