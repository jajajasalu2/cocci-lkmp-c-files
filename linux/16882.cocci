...
position p;
@@
 struct atp_info {  int xsensors ;  int xsensors_17 ;  int ysensors ;  int xfact ;  int yfact ;  int datalen ;  void ( * callback ) ( struct urb * ) ;  int fuzz ; } cocci_id@p ;
...
position p;
@@
 const struct atp_info * cocci_id@p ;
...
typedef u8,bool;
@@
 struct atp {  char phys [ 64 ] ;  struct usb_device * udev ;  struct usb_interface * intf ;  struct urb * urb ;  u8 * data ;  struct input_dev * input ;  const struct atp_info * info ;  bool open ;  bool valid ;  bool size_detect_done ;  bool overflow_warned ;  int fingers_old ;  int x_old ;  int y_old ;  signed char xy_cur [ ATP_XSENSORS + ATP_YSENSORS ] ;  signed char xy_old [ ATP_XSENSORS + ATP_YSENSORS ] ;  int xy_acc [ ATP_XSENSORS + ATP_YSENSORS ] ;  int smooth [ ATP_SMOOTHSIZE ] ;  int smooth_tmp [ ATP_SMOOTHSIZE ] ;  int idlecount ;  struct work_struct work ; } cocci_id@p ;
...
position p;
@@
 enum atp_status_bits { ATP_STATUS_BUTTON = BIT ( 0 ) , ATP_STATUS_BASE_UPDATE = BIT ( 2 ) , ATP_STATUS_FROM_RESET = BIT ( 4 ) , } cocci_id@p ;
