...
typedef u64,u32;
@@
 union {  u64 full ;  struct {  u32 low ;  u32 high ; } ; } cocci_id@p ;
...
position p;
@@
 struct dca_provider * cocci_id@p ;
...
position p;
@@
 struct ioat_dca_priv * cocci_id@p ;
...
typedef u8;
@@
 struct ioat_dca_priv {  void __iomem * iobase ;  void __iomem * dca_base ;  int max_requesters ;  int requester_count ;  u8 tag_map [ IOAT_TAG_MAP_LEN ] ;  struct ioat_dca_slot req_slots [ 0 ] ; } cocci_id@p ;
...
position p;
@@
 const struct dca_ops cocci_id@p ;
...
typedef u16;
@@
 struct ioat_dca_slot {  struct pci_dev * pdev ;  u16 rid ; } cocci_id@p ;
