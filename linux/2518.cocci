...
position p;
@@
 void cocci_id@p ( struct dmz_metadata * zmd , struct dm_zone * zone ) ;
...
@@
 struct dmz_super {  __le32 magic ;  __le32 version ;  __le64 gen ;  __le64 sb_block ;  __le32 nr_meta_blocks ;  __le32 nr_reserved_seq ;  __le32 nr_chunks ;  __le32 nr_map_blocks ;  __le32 nr_bitmap_blocks ;  __le32 crc ;  u8 reserved [ 464 ] ; } cocci_id@p ;
...
typedef __le32;
@@
 struct dmz_map {  __le32 dzone_id ;  __le32 bzone_id ; } cocci_id@p ;
...
position p;
@@
 enum { DMZ_META_DIRTY , DMZ_META_READING , DMZ_META_WRITING , DMZ_META_ERROR , } cocci_id@p ;
...
typedef sector_t;
@@
 struct dmz_mblock {  struct rb_node node ;  struct list_head link ;  sector_t no ;  unsigned int ref ;  unsigned long state ;  struct page * page ;  void * data ; } cocci_id@p ;
...
typedef sector_t;
@@
 struct dmz_sb {  sector_t block ;  struct dmz_mblock * mblk ;  struct dmz_super * sb ; } cocci_id@p ;
...
position p;
@@
 struct dmz_metadata cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u64,wait_queue_head_t,sector_t,spinlock_t,atomic_t;
@@
 struct dmz_metadata {  struct dmz_dev * dev ;  sector_t zone_bitmap_size ;  unsigned int zone_nr_bitmap_blocks ;  unsigned int nr_bitmap_blocks ;  unsigned int nr_map_blocks ;  unsigned int nr_useable_zones ;  unsigned int nr_meta_blocks ;  unsigned int nr_meta_zones ;  unsigned int nr_data_zones ;  unsigned int nr_rnd_zones ;  unsigned int nr_reserved_seq ;  unsigned int nr_chunks ;  struct dm_zone * zones ;  struct dm_zone * sb_zone ;  struct dmz_sb sb [ 2 ] ;  unsigned int mblk_primary ;  u64 sb_gen ;  unsigned int min_nr_mblks ;  unsigned int max_nr_mblks ;  atomic_t nr_mblks ;  struct rw_semaphore mblk_sem ;  struct mutex mblk_flush_lock ;  spinlock_t mblk_lock ;  struct rb_root mblk_rbtree ;  struct list_head mblk_lru_list ;  struct list_head mblk_dirty_list ;  struct shrinker mblk_shrinker ;  struct mutex map_lock ;  struct dmz_mblock * * map_mblk ;  unsigned int nr_rnd ;  atomic_t unmap_nr_rnd ;  struct list_head unmap_rnd_list ;  struct list_head map_rnd_list ;  unsigned int nr_seq ;  atomic_t unmap_nr_seq ;  struct list_head unmap_seq_list ;  struct list_head map_seq_list ;  atomic_t nr_reserved_seq_zones ;  struct list_head reserved_seq_zones_list ;  wait_queue_head_t free_wq ; } cocci_id@p ;
