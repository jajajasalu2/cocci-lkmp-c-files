...
position p;
@@
 struct compute_test * cocci_id@p ;
...
position p;
@@
 union {  float a ;  int b ; } cocci_id@p ;
...
position p;
@@
 struct compute_test {  char * mnemonic ;  struct {  char * descr ;  unsigned long flags ;  unsigned int instr ;  struct pt_regs regs ; } subtests [ MAX_SUBTESTS + 1 ] ; } cocci_id@p ;
...
typedef u32;
@@
 union {  vector128 a ;  u32 b [ 4 ] ; } cocci_id@p ;
...
position p;
@@
 union {  double a ;  long b ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct pt_regs * regs ) ;
