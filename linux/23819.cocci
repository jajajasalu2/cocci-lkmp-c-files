...
position p;
@@
 const struct netdev_queue * cocci_id@p ;
...
typedef u8;
@@
 const u8 cocci_id@p [ TC_PRIO_MAX + 1 ] ;
...
position p;
@@
 struct pfifo_fast_priv {  struct skb_array q [ PFIFO_FAST_BANDS ] ; } cocci_id@p ;
...
position p;
@@
 struct pfifo_fast_priv * cocci_id@p ;
...
position p;
@@
 const struct tc_ratespec * cocci_id@p ;
...
position p;
@@
 struct skb_array * cocci_id@p [ PFIFO_FAST_BANDS ] ;
