...
position p;
@@
 struct z3fold_ops {  int ( * evict ) ( struct z3fold_pool * pool , unsigned long handle ) ; } cocci_id@p ;
...
position p;
@@
 struct z3fold_header * cocci_id@p ;
...
position p;
@@
 enum z3fold_page_flags { PAGE_HEADLESS = 0 , MIDDLE_CHUNK_MAPPED , NEEDS_COMPACTING , PAGE_STALE , PAGE_CLAIMED , } cocci_id@p ;
...
position p;
@@
 enum z3fold_handle_flags { HANDLES_ORPHANED = 0 , } cocci_id@p ;
...
position p;
@@
 enum buddy { HEADLESS = 0 , FIRST , MIDDLE , LAST , BUDDIES_MAX = LAST , } cocci_id@p ;
...
position p;
@@
 const struct z3fold_ops cocci_id@p ;
...
symbol cocci_id;
position p;
typedef atomic64_t,spinlock_t;
@@
 struct z3fold_pool {  const char * name ;  spinlock_t lock ;  spinlock_t stale_lock ;  struct list_head * unbuddied ;  struct list_head lru ;  struct list_head stale ;  atomic64_t pages_nr ;  struct kmem_cache * c_handle ;  const struct z3fold_ops * ops ;  struct zpool * zpool ;  const struct zpool_ops * zpool_ops ;  struct workqueue_struct * compact_wq ;  struct workqueue_struct * release_wq ;  struct work_struct work ;  struct inode * inode ; } cocci_id@p ;
...
position p;
@@
 struct z3fold_buddy_slots * cocci_id@p ;
...
typedef rwlock_t;
@@
 struct z3fold_buddy_slots {  unsigned long slot [ BUDDY_MASK + 1 ] ;  unsigned long pool ;  rwlock_t lock ; } cocci_id@p ;
