...
position p;
@@
 const struct nlm_lock * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef __be32,wait_queue_head_t;
@@
 struct nlm_wait {  struct list_head b_list ;  wait_queue_head_t b_wait ;  struct nlm_host * b_host ;  struct file_lock * b_lock ;  unsigned short b_reclaim ;  __be32 b_status ; } cocci_id@p ;
...
position p;
@@
 const struct nlmclnt_initdata * cocci_id@p ;
