...
typedef u32;
@@
 struct net_vrf {  struct rtable __rcu * rth ;  struct rt6_info __rcu * rt6 ; #if IS_ENABLED(CONFIG_IPV6)  struct fib6_table * fib6_table ; #endif  u32 tb_id ; } cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ IFLA_VRF_MAX + 1 ] ;
...
position p;
@@
 const struct l3mdev_ops cocci_id@p ;
...
position p;
@@
 const struct pcpu_dstats * cocci_id@p ;
...
typedef u64;
@@
 struct pcpu_dstats {  u64 tx_pkts ;  u64 tx_bytes ;  u64 tx_drps ;  u64 rx_pkts ;  u64 rx_bytes ;  u64 rx_drps ;  struct u64_stats_sync syncp ; } cocci_id@p ;
