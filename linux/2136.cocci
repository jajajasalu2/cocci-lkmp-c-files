...
typedef bool;
@@
 struct cpcap_charger_ints_state {  bool chrg_det ;  bool rvrs_chrg ;  bool vbusov ;  bool chrg_se1b ;  bool rvrs_mode ;  bool chrgcurr2 ;  bool chrgcurr1 ;  bool vbusvld ;  bool battdetb ; } cocci_id@p ;
...
position p;
@@
 struct cpcap_charger_ddata cocci_id@p ;
...
position p;
@@
 struct phy_companion * cocci_id@p ;
...
position p;
@@
 enum { CPCAP_CHARGER_DISCONNECTED , CPCAP_CHARGER_DETECTING , CPCAP_CHARGER_CHARGING , CPCAP_CHARGER_DONE , } cocci_id@p ;
...
position p;
@@
 const char * const cocci_id@p [ CPCAP_CHARGER_IIO_NR ] ;
...
position p;
@@
 struct cpcap_interrupt_desc {  int irq ;  struct list_head node ;  const char * name ; } cocci_id@p ;
...
typedef atomic_t;
@@
 struct cpcap_charger_ddata {  struct device * dev ;  struct regmap * reg ;  struct list_head irq_list ;  struct delayed_work detect_work ;  struct delayed_work vbus_work ;  struct gpio_desc * gpio [ 2 ] ;  struct iio_channel * channels [ CPCAP_CHARGER_IIO_NR ] ;  struct power_supply * usb ;  struct phy_companion comparator ;  unsigned int vbus_enabled : 1 ;  unsigned int feeding_vbus : 1 ;  atomic_t active ;  int status ;  int state ;  int voltage ; } cocci_id@p ;
...
position p;
@@
 enum { CPCAP_CHARGER_IIO_BATTDET , CPCAP_CHARGER_IIO_VOLTAGE , CPCAP_CHARGER_IIO_VBUS , CPCAP_CHARGER_IIO_CHRG_CURRENT , CPCAP_CHARGER_IIO_BATT_CURRENT , CPCAP_CHARGER_IIO_NR , } cocci_id@p ;
...
position p;
@@
 struct cpcap_charger_ints_state cocci_id@p ;
