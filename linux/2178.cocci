...
position p;
@@
 struct gab_platform_data * cocci_id@p ;
...
position p;
@@
 enum gab_chan_type { GAB_VOLTAGE = 0 , GAB_CURRENT , GAB_POWER , GAB_MAX_CHAN_TYPE , } cocci_id@p ;
...
position p;
@@
 const enum power_supply_property cocci_id@p [ ] ;
...
typedef bool;
@@
 struct gab {  struct power_supply * psy ;  struct power_supply_desc psy_desc ;  struct iio_channel * channel [ GAB_MAX_CHAN_TYPE ] ;  struct gab_platform_data * pdata ;  struct delayed_work bat_work ;  int level ;  int status ;  bool cable_plugged ; } cocci_id@p ;
