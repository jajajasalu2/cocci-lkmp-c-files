...
typedef u8;
@@
 u8 cocci_id@p [ ST_NCI_SPI_MAX_SIZE + NCI_DATA_HDR_SIZE + ST_NCI_FRAME_HEADROOM + ST_NCI_FRAME_TAILROOM ] ;
...
position p;
@@
 struct spi_device_id cocci_id@p [ ] ;
...
position p;
@@
 struct st_nci_spi_phy * cocci_id@p ;
...
typedef bool;
@@
 struct st_nci_spi_phy {  struct spi_device * spi_dev ;  struct llt_ndlc * ndlc ;  bool irq_active ;  struct gpio_desc * gpiod_reset ;  struct st_nci_se_status se_status ; } cocci_id@p ;
...
typedef u8;
@@
 u8 cocci_id@p [ ST_NCI_SPI_MAX_SIZE ] ;
