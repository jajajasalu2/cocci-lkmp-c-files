...
position p;
@@
 struct xts_instance_ctx {  struct crypto_skcipher_spawn spawn ;  char name [ CRYPTO_MAX_ALG_NAME ] ; } cocci_id@p ;
...
position p;
@@
 struct rctx {  le128 t ;  struct scatterlist * tail ;  struct scatterlist sg [ 2 ] ;  struct skcipher_request subreq ; } cocci_id@p ;
...
position p;
@@
 struct priv {  struct crypto_skcipher * child ;  struct crypto_cipher * tweak ; } cocci_id@p ;
...
position p;
@@
 struct xts_instance_ctx * cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct skcipher_request * req ) ;
