...
position p;
@@
 const struct hdmi_msm_audio_arcs cocci_id@p [ ] ;
...
position p;
@@
 struct hdmi_msm_audio_arcs {  unsigned long int pixclock ;  struct hdmi_msm_audio_acr lut [ MSM_HDMI_SAMPLE_RATE_MAX ] ; } cocci_id@p ;
...
typedef uint32_t;
@@
 struct hdmi_msm_audio_acr {  uint32_t n ;  uint32_t cts ; } cocci_id@p ;
