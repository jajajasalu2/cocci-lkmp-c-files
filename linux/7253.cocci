...
typedef u8;
@@
 struct h5 {  struct hci_uart serdev_hu ;  struct sk_buff_head unack ;  struct sk_buff_head rel ;  struct sk_buff_head unrel ;  unsigned long flags ;  struct sk_buff * rx_skb ;  size_t rx_pending ;  u8 rx_ack ;  int ( * rx_func ) ( struct hci_uart * hu , u8 c ) ;  struct timer_list timer ;  struct hci_uart * hu ;  u8 tx_seq ;  u8 tx_ack ;  u8 tx_win ;  enum { H5_UNINITIALIZED , H5_INITIALIZED , H5_ACTIVE , } state ;  enum { H5_AWAKE , H5_SLEEPING , H5_WAKING_UP , } sleep ;  const struct h5_vnd * vnd ;  const char * id ;  struct gpio_desc * enable_gpio ;  struct gpio_desc * device_wake_gpio ; } cocci_id@p ;
...
position p;
@@
 struct btrtl_device_info * cocci_id@p ;
...
position p;
@@
 const struct h5_vnd * cocci_id@p ;
...
position p;
@@
 struct h5_btrtl_reprobe {  struct device * dev ;  struct work_struct work ; } cocci_id@p ;
...
position p;
@@
 struct h5_vnd {  int ( * setup ) ( struct h5 * h5 ) ;  void ( * open ) ( struct h5 * h5 ) ;  void ( * close ) ( struct h5 * h5 ) ;  int ( * suspend ) ( struct h5 * h5 ) ;  int ( * resume ) ( struct h5 * h5 ) ;  const struct acpi_gpio_mapping * acpi_gpio_map ; } cocci_id@p ;
...
position p;
@@
 enum { H5_RX_ESC , H5_TX_ACK_REQ , } cocci_id@p ;
...
position p;
@@
 struct h5_btrtl_reprobe * cocci_id@p ;
