...
position p;
@@
 char * cocci_id@p ( const char * , int ) ;
...
position p;
@@
 struct kcore {  int fd ;  int elfclass ;  Elf * elf ;  GElf_Ehdr ehdr ; } cocci_id@p ;
...
typedef u64,off_t;
@@
 struct phdr_data {  off_t offset ;  off_t rel ;  u64 addr ;  u64 len ;  struct list_head node ;  struct phdr_data * remaps ; } cocci_id@p ;
...
position p;
@@
 union {  Elf64_Addr a64 [ NR_ADDR ] ;  Elf32_Addr a32 [ NR_ADDR ] ; } cocci_id@p ;
...
typedef u64;
@@
 struct kcore_copy_info {  u64 stext ;  u64 etext ;  u64 first_symbol ;  u64 last_symbol ;  u64 first_module ;  u64 last_module_symbol ;  size_t phnum ;  struct list_head phdrs ;  struct list_head syms ; } cocci_id@p ;
...
typedef u64;
@@
 struct sym_data {  u64 addr ;  struct list_head node ; } cocci_id@p ;
...
position p;
@@
 struct kcore_copy_info * cocci_id@p ;
