...
position p;
@@
 const struct venc_common_if cocci_id@p ;
...
typedef u32;
@@
 struct venc_h264_vpu_buf {  u32 iova ;  u32 vpua ;  u32 size ; } cocci_id@p ;
...
position p;
@@
 struct venc_enc_param * cocci_id@p ;
...
position p;
@@
 struct venc_h264_vsi * cocci_id@p ;
...
typedef u32;
@@
 struct venc_h264_vpu_config {  u32 input_fourcc ;  u32 bitrate ;  u32 pic_w ;  u32 pic_h ;  u32 buf_w ;  u32 buf_h ;  u32 gop_size ;  u32 intra_period ;  u32 framerate ;  u32 profile ;  u32 level ;  u32 wfd ; } cocci_id@p ;
...
position p;
@@
 enum venc_set_param_type cocci_id@p ;
...
typedef bool;
@@
 struct venc_h264_inst {  void __iomem * hw_base ;  struct mtk_vcodec_mem work_bufs [ VENC_H264_VPU_WORK_BUF_MAX ] ;  struct mtk_vcodec_mem pps_buf ;  bool work_buf_allocated ;  unsigned int frm_cnt ;  unsigned int prepend_hdr ;  struct venc_vpu_inst vpu_inst ;  struct venc_h264_vsi * vsi ;  struct mtk_vcodec_ctx * ctx ; } cocci_id@p ;
...
position p;
@@
 struct venc_done_result * cocci_id@p ;
...
position p;
@@
 struct venc_h264_inst * cocci_id@p ;
...
position p;
@@
 struct venc_frm_buf * cocci_id@p ;
...
position p;
@@
 struct venc_h264_vsi {  struct venc_h264_vpu_config config ;  struct venc_h264_vpu_buf work_bufs [ VENC_H264_VPU_WORK_BUF_MAX ] ; } cocci_id@p ;
...
position p;
@@
 struct venc_h264_vpu_buf * cocci_id@p ;
...
position p;
@@
 enum venc_h264_bs_mode { H264_BS_MODE_SPS , H264_BS_MODE_PPS , H264_BS_MODE_FRAME , } cocci_id@p ;
...
position p;
@@
 enum venc_start_opt cocci_id@p ;
