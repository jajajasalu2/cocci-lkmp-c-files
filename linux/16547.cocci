...
position p;
@@
 char cocci_id@p [ DASD_BUS_ID_SIZE + 1 ] ;
...
position p;
@@
 struct list_head cocci_id@p [ 256 ] ;
...
position p;
@@
 char * cocci_id@p [ DASD_MAX_PARAMS ] ;
...
position p;
@@
 struct dasd_devmap {  struct list_head list ;  char bus_id [ DASD_BUS_ID_SIZE ] ;  unsigned int devindex ;  unsigned short features ;  struct dasd_device * device ; } cocci_id@p ;
...
position p;
@@
 struct dasd_devmap * cocci_id@p ( const char * , int ) ;
