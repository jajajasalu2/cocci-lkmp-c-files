...
position p;
@@
 const struct stedma40_chan_cfg cocci_id@p ;
...
position p;
@@
 struct stedma40_chan_cfg cocci_id@p ;
...
position p;
@@
 enum d40_events { D40_DEACTIVATE_EVENTLINE = 0 , D40_ACTIVATE_EVENTLINE = 1 , D40_SUSPEND_REQ_EVENTLINE = 2 , D40_ROUND_EVENTLINE = 3 , } cocci_id@p ;
...
position p;
@@
 struct d40_log_lli_bidir * cocci_id@p ;
...
typedef u32;
@@
 struct d40_gen_dmac {  u32 * backup ;  u32 backup_size ;  u32 realtime_en ;  u32 realtime_clear ;  u32 high_prio_en ;  u32 high_prio_clear ;  u32 interrupt_en ;  u32 interrupt_clear ;  struct d40_interrupt_lookup * il ;  u32 il_size ;  struct d40_reg_val * init_reg ;  u32 init_reg_size ; } cocci_id@p ;
...
position p;
@@
 struct d40_lcla_pool * cocci_id@p ;
...
@@
 struct d40_lli_pool {  void * base ;  int size ;  dma_addr_t dma_addr ;  u8 pre_alloc_lli [ 3 * sizeof ( struct d40_phy_lli ) ] ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct dma_chan * chan , struct dma_slave_config * config , enum dma_transfer_direction direction ) ;
...
symbol cocci_id;
position p;
typedef u8,u32,u16,phys_addr_t,resource_size_t,spinlock_t,dma_addr_t;
@@
...
symbol cocci_id;
position p;
typedef bool,u32,spinlock_t;
@@
 struct d40_phy_res {  spinlock_t lock ;  bool reserved ;  int num ;  u32 allocated_src ;  u32 allocated_dst ;  bool use_soft_lli ; } cocci_id@p ;
...
@@
 struct d40_lcla_pool {  void * base ;  dma_addr_t dma_addr ;  void * base_unaligned ;  int pages ;  spinlock_t lock ;  struct d40_desc * * alloc_map ; } cocci_id@p ;
...
position p;
@@
 enum d40_command { D40_DMA_STOP = 0 , D40_DMA_RUN = 1 , D40_DMA_SUSPEND_REQ = 2 , D40_DMA_SUSPENDED = 3 , } cocci_id@p ;
...
position p;
@@
 struct d40_interrupt_lookup cocci_id@p [ ] ;
...
typedef bool,u32;
@@
 struct d40_interrupt_lookup {  u32 src ;  u32 clr ;  bool is_error ;  int offset ; } cocci_id@p ;
...
position p;
@@
 struct stedma40_platform_data * cocci_id@p ;
...
position p;
@@
 struct d40_reg_val {  unsigned int reg ;  unsigned int val ; } cocci_id@p ;
...
position p;
@@
 struct stedma40_half_channel_info * cocci_id@p ;
...
typedef bool;
@@
 struct d40_desc {  struct d40_phy_lli_bidir lli_phy ;  struct d40_log_lli_bidir lli_log ;  struct d40_lli_pool lli_pool ;  int lli_len ;  int lli_current ;  int lcla_alloc ;  struct dma_async_tx_descriptor txd ;  struct list_head node ;  bool is_in_client_list ;  bool cyclic ; } cocci_id@p ;
...
position p;
@@
 struct d40_gen_dmac * cocci_id@p ;
