...
position p;
@@
 struct dm_bio_details cocci_id@p ;
...
typedef bool;
@@
 struct priority_group {  struct list_head list ;  struct multipath * m ;  struct path_selector ps ;  unsigned pg_num ;  unsigned nr_pgpaths ;  struct list_head pgpaths ;  bool bypassed : 1 ; } cocci_id@p ;
...
@@
 struct multipath {  unsigned long flags ;  spinlock_t lock ;  enum dm_queue_mode queue_mode ;  struct pgpath * current_pgpath ;  struct priority_group * current_pg ;  struct priority_group * next_pg ;  atomic_t nr_valid_paths ;  unsigned nr_priority_groups ;  struct list_head priority_groups ;  const char * hw_handler_name ;  char * hw_handler_params ;  wait_queue_head_t pg_init_wait ;  unsigned pg_init_retries ;  unsigned pg_init_delay_msecs ;  atomic_t pg_init_in_progress ;  atomic_t pg_init_count ;  struct mutex work_mutex ;  struct work_struct trigger_event ;  struct dm_target * ti ;  struct work_struct process_queued_bios ;  struct bio_list queued_bios ; } cocci_id@p ;
...
position p;
@@
 struct path_selector_type * cocci_id@p ;
...
typedef bool;
@@
 struct pgpath {  struct list_head list ;  struct priority_group * pg ;  unsigned fail_count ;  struct dm_path path ;  struct delayed_work activate_path ;  bool is_active : 1 ; } cocci_id@p ;
...
position p;
@@
 struct dm_mpath_io {  struct pgpath * pgpath ;  size_t nr_bytes ; } cocci_id@p ;
...
position p;
@@
 struct path_selector * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct pgpath * pgpath ) ;
...
position p;
@@
 struct priority_group * cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct pgpath * pgpath ) ;
