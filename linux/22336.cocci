...
position p;
@@
 unsigned char cocci_id@p [ MAX_KEY_SIZE ] ;
...
position p;
@@
 const struct evm_ima_xattr_data * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef __u32,gid_t,umode_t,uid_t;
@@
 struct h_misc {  unsigned long ino ;  __u32 generation ;  uid_t uid ;  gid_t gid ;  umode_t mode ; } cocci_id@p ;
...
position p;
@@
 struct crypto_shash * cocci_id@p [ HASH_ALGO__LAST ] ;
