...
position p;
@@
 struct stx104_iio {  unsigned int chan_out_states [ STX104_NUM_OUT_CHAN ] ;  unsigned int base ; } cocci_id@p ;
...
position p;
@@
 struct stx104_iio * const cocci_id@p ;
...
position p;
@@
 struct stx104_gpio * const cocci_id@p ;
...
typedef spinlock_t;
@@
 struct stx104_gpio {  struct gpio_chip chip ;  spinlock_t lock ;  unsigned int base ;  unsigned int out_state ; } cocci_id@p ;
...
position p;
@@
 const char * cocci_id@p [ STX104_NGPIO ] ;
...
position p;
@@
 unsigned int cocci_id@p [ max_num_isa_dev ( STX104_EXTENT ) ] ;
