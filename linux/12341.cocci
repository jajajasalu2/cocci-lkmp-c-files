...
position p;
@@
 struct vc4_shader_validation_state cocci_id@p ;
...
@@
 struct vc4_shader_validation_state {  uint32_t ip ;  uint32_t max_ip ;  uint64_t * shader ;  struct vc4_texture_sample_info tmu_setup [ 2 ] ;  int tmu_write_count [ 2 ] ;  uint32_t live_min_clamp_offsets [ LIVE_REG_COUNT ] ;  bool live_max_clamp_regs [ LIVE_REG_COUNT ] ;  uint32_t live_immediates [ LIVE_REG_COUNT ] ;  unsigned long * branch_targets ;  bool needs_uniform_address_update ;  bool needs_uniform_address_for_loop ;  bool all_registers_used ; } cocci_id@p ;
