...
position p;
@@
 enum { Opt_cache_loose , Opt_cache_strict , Opt_cache_none , Opt_cache_ro , Opt_cache_rw , Opt_cache_err , } cocci_id@p ;
...
position p;
@@
 struct mid_q_entry * cocci_id@p [ MAX_COMPOUND ] ;
...
position p;
@@
 const struct dfs_cache_tgt_iterator * cocci_id@p ;
...
position p;
@@
 struct cifs_mnt_data * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct TCP_Server_Info * server ) ;
...
position p;
@@
 const struct smb_vol * cocci_id@p ;
...
position p;
@@
 char * cocci_id@p ( const char * unc ) ;
...
position p;
@@
 char cocci_id@p [ SERVER_NAME_LENGTH + sizeof ( "//x/IPC$" ) ] ;
...
position p;
@@
 struct super_cb_data {  struct TCP_Server_Info * server ;  struct super_block * sb ; } cocci_id@p ;
...
position p;
@@
 struct lock_class_key cocci_id@p [ 2 ] ;
...
position p;
@@
 enum { Opt_sec_krb5 , Opt_sec_krb5i , Opt_sec_krb5p , Opt_sec_ntlmsspi , Opt_sec_ntlmssp , Opt_ntlm , Opt_sec_ntlmi , Opt_sec_ntlmv2 , Opt_sec_ntlmv2i , Opt_sec_lanman , Opt_sec_none , Opt_sec_err , } cocci_id@p ;
...
position p;
@@
 struct rfc1002_session_packet cocci_id@p ;
...
position p;
@@
 struct super_cb_data cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct rb_root * root , struct tcon_link * new_tlink ) ;
...
position p;
@@
 const struct cifs_sb_info * cocci_id@p ;
