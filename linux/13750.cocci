...
typedef u16;
@@
 struct CHIPSTATE {  struct v4l2_subdev sd ;  struct v4l2_ctrl_handler hdl ;  struct {  struct v4l2_ctrl * volume ;  struct v4l2_ctrl * balance ; } ;  struct CHIPDESC * desc ;  audiocmd shadow ;  u16 muted ;  int prevmode ;  int radio ;  int input ;  struct task_struct * thread ;  struct timer_list wt ;  int audmode ; } cocci_id@p ;
...
position p;
@@
 void ( * cocci_id@p ) ( struct CHIPSTATE * , int mode ) ;
...
position p;
@@
 struct AUDIOCMD {  int count ;  unsigned char bytes [ MAXREGS + 1 ] ; } cocci_id@p ;
...
position p;
@@
 struct tda9874a_MODES {  char * name ;  audiocmd cmd ; } cocci_id@p [ 9 ] ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct CHIPSTATE * ) ;
