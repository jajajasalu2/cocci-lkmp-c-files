...
position p;
@@
 struct arc_callchain_trace cocci_id@p ;
...
typedef u32;
@@
 union cc_name {  struct {  u32 word0 , word1 ;  char sentinel ; } indiv ;  char str [ ARCPMU_EVENT_NAME_LEN ] ; } cocci_id@p ;
...
position p;
@@
 struct arc_pmu_raw_event_entry {  char name [ ARCPMU_EVENT_NAME_LEN ] ; } cocci_id@p ;
...
position p;
@@
 struct arc_pmu_cpu {  unsigned long used_mask [ BITS_TO_LONGS ( ARC_PERF_MAX_COUNTERS ) ] ;  struct perf_event * act_counter [ ARC_PERF_MAX_COUNTERS ] ; } cocci_id@p ;
...
position p;
@@
 struct arc_callchain_trace {  int depth ;  void * perf_stuff ; } cocci_id@p ;
...
position p;
@@
 struct arc_reg_cc_build cocci_id@p ;
...
position p;
@@
 struct arc_reg_pct_build cocci_id@p ;
...
position p;
@@
 enum arc_pmu_attr_groups { ARCPMU_ATTR_GR_EVENTS , ARCPMU_ATTR_GR_FORMATS , ARCPMU_NR_ATTR_GR , } cocci_id@p ;
...
typedef u64;
@@
 struct arc_pmu {  struct pmu pmu ;  unsigned int irq ;  int n_counters ;  int n_events ;  u64 max_period ;  int ev_hw_idx [ PERF_COUNT_ARC_HW_MAX ] ;  struct arc_pmu_raw_event_entry * raw_entry ;  struct attribute * * attrs ;  struct perf_pmu_events_attr * attr ;  const struct attribute_group * attr_groups [ ARCPMU_NR_ATTR_GR + 1 ] ; } cocci_id@p ;
