...
position p;
@@
 struct hv_host_device {  struct hv_device * dev ;  unsigned int port ;  unsigned char path ;  unsigned char target ;  struct workqueue_struct * handle_error_wq ;  struct work_struct host_scan_work ;  struct Scsi_Host * host ; } cocci_id@p ;
...
position p;
@@
 struct vmbus_packet_mpb_array * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u64,bool,u32,u16,wait_queue_head_t,atomic_t;
@@
 struct storvsc_device {  struct hv_device * device ;  bool destroy ;  bool drain_notify ;  atomic_t num_outstanding_req ;  struct Scsi_Host * host ;  wait_queue_head_t waiting_to_drain ;  unsigned int port_number ;  unsigned char path_id ;  unsigned char target_id ;  u32 max_transfer_bytes ;  u16 num_sc ;  struct vmbus_channel * * stor_chns ;  struct cpumask alloced_cpus ;  struct storvsc_cmd_request init_request ;  struct storvsc_cmd_request reset_request ;  u64 node_name ;  u64 port_name ; #if IS_ENABLED(CONFIG_SCSI_FC_ATTRS)  struct fc_rport * rport ; #endif } cocci_id@p ;
...
position p;
@@
 struct vmscsi_win8_extension cocci_id@p ;
...
position p;
@@
 struct vstor_packet cocci_id@p ;
...
position p;
@@
 struct storvsc_cmd_request * cocci_id@p ;
...
position p;
@@
 struct vmstor_protocol {  int protocol_version ;  int sense_buffer_size ;  int vmscsi_size_delta ; } cocci_id@p ;
...
position p;
@@
 struct vmscsi_request * cocci_id@p ;
...
position p;
@@
 enum storvsc_request_type { WRITE_TYPE = 0 , READ_TYPE , UNKNOWN_TYPE , } cocci_id@p ;
...
position p;
@@
 struct vmbus_channel_packet_multipage_buffer cocci_id@p ;
...
position p;
@@
 struct vmstorage_channel_properties cocci_id@p ;
...
position p;
@@
 const struct vmstor_protocol cocci_id@p [ ] ;
...
position p;
@@
 struct hv_host_device cocci_id@p ;
...
position p;
@@
 enum { SCSI_GUID , IDE_GUID , SFC_GUID , } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct scsi_device * sdev , int queue_depth ) ;
...
position p;
@@
 struct fc_function_template cocci_id@p ;
...
typedef u8;
@@
 struct hv_fc_wwn_packet {  u8 primary_active ;  u8 reserved1 [ 3 ] ;  u8 primary_port_wwn [ 8 ] ;  u8 primary_node_wwn [ 8 ] ;  u8 secondary_port_wwn [ 8 ] ;  u8 secondary_node_wwn [ 8 ] ; } cocci_id@p ;
...
position p;
@@
 struct storvsc_device cocci_id@p ;
...
position p;
@@
 struct storvsc_scan_work * cocci_id@p ;
...
position p;
@@
 struct vmbus_channel * cocci_id@p ;
...
typedef u32;
@@
 struct storvsc_cmd_request {  struct scsi_cmnd * cmd ;  struct hv_device * device ;  struct completion wait_event ;  struct vmbus_channel_packet_multipage_buffer mpb ;  struct vmbus_packet_mpb_array * payload ;  u32 payload_sz ;  struct vstor_packet vstor_packet ; } cocci_id@p ;
...
typedef guid_t;
@@
 const struct {  guid_t guid ; } cocci_id@p ;
...
position p;
@@
 struct fc_rport_identifiers cocci_id@p ;
...
typedef u8;
@@
 struct storvsc_scan_work {  struct work_struct work ;  struct Scsi_Host * host ;  u8 lun ;  u8 tgt_id ; } cocci_id@p ;
...
position p;
@@
 const struct vmpacket_descriptor * cocci_id@p ;
