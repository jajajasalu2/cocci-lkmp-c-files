...
position p;
@@
 struct ata_scsi_args * cocci_id@p ;
...
typedef u8;
@@
 const u8 cocci_id@p [ CACHE_MPAGE_LEN ] ;
...
typedef u8;
@@
 const u8 cocci_id@p [ RW_RECOVERY_MPAGE_LEN ] ;
...
typedef u8;
@@
 const u8 cocci_id@p [ CONTROL_MPAGE_LEN ] ;
...
typedef u8;
@@
 u8 cocci_id@p [ SCSI_SENSE_BUFFERSIZE ] ;
...
typedef u8;
@@
 u8 cocci_id@p [ ATA_SCSI_RBUF_SIZE ] ;
...
typedef u8;
@@
 unsigned int ( * cocci_id@p ) ( struct ata_scsi_args * args , u8 * rbuf ) ;
...
typedef u16;
@@
 struct ata_scsi_args {  struct ata_device * dev ;  u16 * id ;  struct scsi_cmnd * cmd ; } cocci_id@p ;
...
typedef u8;
@@
 u8 cocci_id@p [ CONTROL_MPAGE_LEN ] ;
...
position p;
@@
 struct ata_device * cocci_id@p ( struct ata_port * ap , const struct scsi_device * scsidev ) ;
...
position p;
@@
 const struct scsi_device * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef ata_xlat_func_t;
@@
 ata_xlat_func_t cocci_id@p ;
