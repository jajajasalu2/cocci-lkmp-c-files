...
position p;
@@
 struct team_option_inst_info * cocci_id@p ;
...
typedef u64;
@@
 struct lb_stats {  u64 tx_bytes ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef lb_select_tx_port_func_t;
@@
 struct lb_priv {  struct bpf_prog __rcu * fp ;  lb_select_tx_port_func_t __rcu * select_tx_port_func ;  struct lb_pcpu_stats __percpu * pcpu_stats ;  struct lb_priv_ex * ex ; } cocci_id@p ;
...
position p;
@@
 const struct team_option cocci_id@p [ ] ;
...
position p;
@@
 struct lb_pcpu_stats cocci_id@p ;
...
position p;
@@
 struct lb_stats_info * cocci_id@p ;
...
position p;
@@
 struct lb_stats_info {  struct lb_stats stats ;  struct lb_stats last_stats ;  struct team_option_inst_info * opt_inst_info ; } cocci_id@p ;
...
typedef lb_select_tx_port_func_t;
@@
 lb_select_tx_port_func_t * cocci_id@p ;
...
position p;
@@
 struct lb_port_mapping {  struct team_port __rcu * port ;  struct team_option_inst_info * opt_inst_info ; } cocci_id@p ;
...
position p;
@@
 struct team_gsetter_ctx * cocci_id@p ;
...
position p;
@@
 struct u64_stats_sync * cocci_id@p ;
...
position p;
@@
 struct lb_port_priv * cocci_id@p ;
...
position p;
@@
 const struct team_mode_ops cocci_id@p ;
...
position p;
@@
 const struct team_mode cocci_id@p ;
...
position p;
@@
 struct lb_port_priv {  struct lb_stats __percpu * pcpu_stats ;  struct lb_stats_info stats_info ; } cocci_id@p ;
...
position p;
@@
 struct lb_priv_ex {  struct team * team ;  struct lb_port_mapping tx_hash_to_port_mapping [ LB_TX_HASHTABLE_SIZE ] ;  struct sock_fprog_kern * orig_fprog ;  struct {  unsigned int refresh_interval ;  struct delayed_work refresh_dw ;  struct lb_stats_info info [ LB_TX_HASHTABLE_SIZE ] ; } stats ; } cocci_id@p ;
...
position p;
@@
 struct lb_pcpu_stats {  struct lb_stats hash_stats [ LB_TX_HASHTABLE_SIZE ] ;  struct u64_stats_sync syncp ; } cocci_id@p ;
...
position p;
@@
 const struct lb_select_tx_port * cocci_id@p ;
...
typedef lb_select_tx_port_func_t;
@@
 struct lb_select_tx_port {  char * name ;  lb_select_tx_port_func_t * func ; } cocci_id@p ;
...
position p;
@@
 struct lb_port_mapping * cocci_id@p ;
...
position p;
@@
 struct team_port * cocci_id@p ( struct team * , struct lb_priv * , struct sk_buff * , unsigned char ) ;
