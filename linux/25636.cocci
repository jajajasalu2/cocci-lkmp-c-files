...
position p;
@@
 struct perf_tsc_conversion cocci_id@p ;
...
position p;
@@
 struct debug_line_info {  unsigned long vma ;  unsigned int lineno ;  const char filename [ 0 ] ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u64,FILE,bool,uint64_t,uint32_t;
@@
 struct jit_buf_desc {  struct perf_data * output ;  struct perf_session * session ;  struct machine * machine ;  union jr_entry * entry ;  void * buf ;  uint64_t sample_type ;  size_t bufsize ;  FILE * in ;  bool needs_bswap ;  bool use_arch_timestamp ;  void * debug_data ;  void * unwinding_data ;  uint64_t unwinding_size ;  uint64_t unwinding_mapped_size ;  uint64_t eh_frame_hdr_size ;  size_t nr_debug_entries ;  uint32_t code_load_count ;  u64 bytes_written ;  struct rb_root code_root ;  char dir [ PATH_MAX ] ; } cocci_id@p ;
...
typedef u64;
@@
 struct jit_tool {  struct perf_tool tool ;  struct perf_data output ;  struct perf_data input ;  u64 bytes_written ; } cocci_id@p ;
...
position p;
@@
 struct jit_buf_desc * cocci_id@p ;
...
typedef u64,u32;
@@
 struct {  u32 pid , tid ;  u64 time ; } * cocci_id@p ;
