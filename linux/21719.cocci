...
position p;
@@
 struct sclp_core_info * cocci_id@p ;
...
position p;
@@
 struct ec_creg_mask_parms {  unsigned long orval ;  unsigned long andval ;  int cr ; } cocci_id@p ;
...
position p;
@@
 struct pcpu cocci_id@p [ NR_CPUS ] ;
...
position p;
@@
 enum { CPU_STATE_STANDBY , CPU_STATE_CONFIGURED , } cocci_id@p ;
...
position p;
@@
 enum { ec_schedule = 0 , ec_call_function_single , ec_stop_cpu , } cocci_id@p ;
...
position p;
@@
 struct sclp_core_entry * cocci_id@p ;
...
position p;
@@
 struct ec_creg_mask_parms * cocci_id@p ;
...
typedef u16;
@@
 struct pcpu {  struct lowcore * lowcore ;  unsigned long ec_mask ;  unsigned long ec_clk ;  signed char state ;  signed char polarization ;  u16 address ; } cocci_id@p ;
