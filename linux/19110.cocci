...
position p;
@@
 struct rt_sigframe {  char __user * pretcode ;  int sig ;  struct siginfo __user * pinfo ;  void __user * puc ;  char retcode [ 8 ] ;  struct siginfo info ;  struct ucontext uc ; } cocci_id@p ;
...
position p;
@@
 unsigned char cocci_id@p [ FPCONTEXT_SIZE ] ;
...
position p;
@@
 unsigned long cocci_id@p [ sizeof ( ( ( struct frame * ) 0 ) -> un ) / 2 ] ;
...
position p;
@@
 struct sigframe {  char __user * pretcode ;  int sig ;  int code ;  struct sigcontext __user * psc ;  char retcode [ 8 ] ;  unsigned long extramask [ _NSIG_WORDS - 1 ] ;  struct sigcontext sc ; } cocci_id@p ;
