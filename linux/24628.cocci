...
position p;
@@
 void cocci_id@p ( struct neigh_parms * parms ) ;
...
position p;
@@
 struct neigh_dump_filter * cocci_id@p ;
...
position p;
@@
 void ( * cocci_id@p ) ( struct hh_cache * , const struct net_device * , const unsigned char * ) ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ NDTPA_MAX + 1 ] ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ NDTA_MAX + 1 ] ;
...
position p;
@@
 char cocci_id@p [ sizeof ( "net//neigh/" ) + IFNAMSIZ + IFNAMSIZ ] ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct neighbour * ) ;
...
typedef u32;
@@
 void cocci_id@p ( struct neighbour * neigh , u32 nlmsg_pid ) ;
...
position p;
@@
 struct neigh_statistics * cocci_id@p ;
...
typedef u32;
@@
 void cocci_id@p ( struct neighbour * n , int type , int flags , u32 pid ) ;
...
position p;
@@
 struct neigh_seq_state * cocci_id@p ;
...
position p;
@@
 struct pneigh_entry * cocci_id@p ;
...
position p;
@@
 struct neigh_dump_filter {  int master_idx ;  int dev_idx ; } cocci_id@p ;
...
position p;
@@
 void ( * cocci_id@p ) ( struct neighbour * , void * ) ;
...
position p;
@@
 struct neigh_sysctl_table * cocci_id@p ;
...
position p;
@@
 struct nlattr * cocci_id@p [ NDTPA_MAX + 1 ] ;
...
position p;
@@
 struct neigh_hash_table cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct neigh_table * tbl , struct net_device * dev ) ;
...
position p;
@@
 struct neigh_sysctl_table {  struct ctl_table_header * sysctl_header ;  struct ctl_table neigh_vars [ NEIGH_VAR_MAX + 1 ] ; } cocci_id@p ;
...
position p;
@@
 struct nlattr * cocci_id@p [ NDTA_MAX + 1 ] ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ NDA_MAX + 1 ] ;
