...
typedef u8;
@@
 int cocci_id@p ( struct in6_addr * addr , u8 dad_count , const struct inet6_dev * idev ) ;
...
typedef u8;
@@
 int cocci_id@p ( u8 * eui , struct net_device * dev ) ;
...
position p;
@@
 struct prefix_cacheinfo cocci_id@p ;
...
position p;
@@
 struct ipv6_saddr_dst {  const struct in6_addr * addr ;  int ifindex ;  int scope ;  int label ;  unsigned int prefs ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( int event , struct inet6_dev * idev , struct prefix_info * pinfo ) ;
...
position p;
@@
 struct ipv6_saddr_score * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef cleanup_prefix_rt_t;
@@
 enum cleanup_prefix_rt_t cocci_id@p ;
...
position p;
@@
 struct inet6_fill_args * cocci_id@p ;
...
typedef u64;
@@
 u64 cocci_id@p [ IPSTATS_MIB_MAX ] ;
...
position p;
@@
 const struct prefix_info * cocci_id@p ;
...
position p;
@@
 struct ip_tunnel_parm cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct inet6_dev * idev ) ;
...
position p;
@@
 const struct ctl_table cocci_id@p [ ] ;
...
position p;
@@
 void cocci_id@p ( struct net_device * dev , unsigned long event ) ;
...
position p;
@@
 struct nlattr * cocci_id@p [ NETCONFA_MAX + 1 ] ;
...
position p;
@@
 struct ipv6_stable_secret * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u32,addr_type_t;
@@
 struct inet6_fill_args {  u32 portid ;  u32 seq ;  int event ;  unsigned int flags ;  int netnsid ;  int ifindex ;  enum addr_type_t type ; } cocci_id@p ;
...
typedef cleanup_prefix_rt_t;
@@
 enum cleanup_prefix_rt_t { CLEANUP_PREFIX_RT_NOP , CLEANUP_PREFIX_RT_DEL , CLEANUP_PREFIX_RT_EXPIRE , } cocci_id@p ;
...
typedef bool,u32;
@@
 struct fib6_info * cocci_id@p ( const struct in6_addr * pfx , int plen , const struct net_device * dev , u32 flags , u32 noflags , bool no_gw ) ;
...
position p;
@@
 struct nlattr * cocci_id@p [ IFLA_INET6_MAX + 1 ] ;
...
typedef u8,__be32;
@@
 union {  char __data [ SHA_MESSAGE_BYTES ] ;  struct {  struct in6_addr secret ;  __be32 prefix [ 2 ] ;  unsigned char hwaddr [ MAX_ADDR_LEN ] ;  u8 dad_count ; } __packed ; } cocci_id@p ;
...
position p;
@@
 struct icmpv6_mib_device cocci_id@p ;
...
position p;
@@
 struct hlist_head cocci_id@p [ IN6_ADDR_HSIZE ] ;
...
position p;
@@
 struct nlattr * cocci_id@p [ IFA_MAX + 1 ] ;
...
position p;
@@
 struct ipv6_saddr_dst cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct inet6_dev * idev ) ;
...
position p;
@@
 struct ipv6_devconf * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct net_device * dev , int how ) ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ NETCONFA_MAX + 1 ] ;
...
position p;
@@
 void cocci_id@p ( int event , struct inet6_ifaddr * ifa ) ;
...
position p;
@@
 char cocci_id@p [ sizeof ( "net/ipv6/conf/" ) + IFNAMSIZ ] ;
...
position p;
@@
 const struct inet6_dev * cocci_id@p ;
...
typedef bool;
@@
 void cocci_id@p ( struct inet6_dev * idev , bool restart ) ;
...
position p;
@@
 struct if6_iter_state {  struct seq_net_private p ;  int bucket ;  int offset ; } cocci_id@p ;
...
typedef addr_type_t;
@@
 enum addr_type_t { UNICAST_ADDR , MULTICAST_ADDR , ANYCAST_ADDR , } cocci_id@p ;
...
position p;
@@
 struct ipv6_saddr_score cocci_id@p [ 2 ] ;
...
position p;
@@
 const struct inet6_ifaddr * cocci_id@p ;
...
@@
 struct ipv6_saddr_score {  int rule ;  int addr_type ;  struct inet6_ifaddr * ifa ; 
	 DECLARE_BITMAP ( scorebits , IPV6_SADDR_RULE_MAX ) 
		 ;  int scopedist ;  int matchlen ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( const struct inet6_dev * idev ) ;
...
position p;
@@
 struct if6_iter_state cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ IFLA_INET6_MAX + 1 ] ;
...
position p;
@@
 char cocci_id@p [ IPV6_MAX_STRLEN ] ;
...
position p;
@@
 void cocci_id@p ( struct inet6_ifaddr * ifp ) ;
...
position p;
@@
 struct icmpv6msg_mib_device cocci_id@p ;
...
typedef bool;
@@
 void cocci_id@p ( struct inet6_ifaddr * ifp , bool bump_id , bool send_na ) ;
...
position p;
@@
 struct ifa_cacheinfo * cocci_id@p ;
...
position p;
@@
 struct netdev_notifier_change_info * cocci_id@p ;
...
position p;
@@
 enum { DAD_PROCESS , DAD_BEGIN , DAD_ABORT , } cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ IFA_MAX + 1 ] ;
...
position p;
@@
 struct ifla_cacheinfo cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct inet6_dev * idev , struct in6_addr * tmpaddr ) ;
