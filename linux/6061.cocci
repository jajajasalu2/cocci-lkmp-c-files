...
position p;
@@
 const struct stm32_desc_function * cocci_id@p ;
...
position p;
@@
 const struct pin_desc * cocci_id@p ;
...
position p;
@@
 struct stm32_gpio_bank cocci_id@p ;
...
@@
 struct stm32_pinctrl {  struct device * dev ;  struct pinctrl_dev * pctl_dev ;  struct pinctrl_desc pctl_desc ;  struct stm32_pinctrl_group * groups ;  unsigned ngroups ;  const char * * grp_names ;  struct stm32_gpio_bank * banks ;  unsigned nbanks ;  const struct stm32_pinctrl_match_data * match_data ;  struct irq_domain * domain ;  struct regmap * regmap ;  struct regmap_field * irqmux [ STM32_GPIO_PINS_PER_BANK ] ;  struct hwspinlock * hwlock ;  struct stm32_desc_pin * pins ;  u32 npins ;  u32 pkg ;  u16 irqmux_map ;  spinlock_t irqmux_lock ; } cocci_id@p ;
...
position p;
@@
 const struct stm32_desc_pin * cocci_id@p ;
...
@@
 struct stm32_gpio_bank {  void __iomem * base ;  struct clk * clk ;  spinlock_t lock ;  struct gpio_chip gpio_chip ;  struct pinctrl_gpio_range range ;  struct fwnode_handle * fwnode ;  struct irq_domain * domain ;  u32 bank_nr ;  u32 bank_ioport_nr ;  u32 pin_backup [ STM32_GPIO_PINS_PER_BANK ] ; } cocci_id@p ;
...
position p;
@@
 struct stm32_desc_pin * cocci_id@p ;
...
position p;
@@
 struct stm32_pinctrl_group * cocci_id@p ;
...
position p;
@@
 struct stm32_pinctrl_group {  const char * name ;  unsigned long config ;  unsigned pin ; } cocci_id@p ;
...
position p;
@@
 struct stm32_pinctrl * cocci_id@p ;
