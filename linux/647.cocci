...
position p;
@@
 const union aegis_block cocci_id@p [ 2 ] ;
...
position p;
@@
 void cocci_id@p ( struct aegis_state * state , const void * msg ) ;
...
position p;
@@
 struct aegis_state {  union aegis_block blocks [ AEGIS128_STATE_BLOCKS ] ; } cocci_id@p ;
...
typedef u8;
@@
 void ( * cocci_id@p ) ( struct aegis_state * state , u8 * dst , const u8 * src , unsigned int size ) ;
...
typedef u8;
@@
 void cocci_id@p ( struct aegis_state * state , const union aegis_block * key , const u8 * iv ) ;
...
typedef u8;
@@
 const u8 cocci_id@p [ AEGIS128_MAX_AUTH_SIZE ] ;
...
position p;
@@
 const union aegis_block * cocci_id@p ;
...
typedef u64;
@@
 void cocci_id@p ( struct aegis_state * state , union aegis_block * tag_xor , u64 assoclen , u64 cryptlen ) ;
...
position p;
@@
 struct aegis_ctx {  union aegis_block key ; } cocci_id@p ;
