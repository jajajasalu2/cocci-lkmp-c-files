...
typedef pid_t;
@@
 struct listener {  struct list_head list ;  pid_t pid ;  char valid ; } cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TASKSTATS_CMD_ATTR_MAX + 1 ] ;
...
position p;
@@
 struct listener_list {  struct rw_semaphore sem ;  struct list_head list ; } cocci_id@p ;
...
position p;
@@
 enum actions { REGISTER , DEREGISTER , CPU_DONT_CARE , } cocci_id@p ;
...
position p;
@@
 struct listener_list * cocci_id@p ;
