...
position p;
@@
 struct paravirt_patch_site * cocci_id@p ;
...
position p;
@@
 struct text_poke_loc * cocci_id@p ;
...
position p;
@@
 struct bp_patching_desc {  struct text_poke_loc * vec ;  int nr_entries ; } cocci_id@p ;
...
position p;
@@
 void __init_or_module noinline cocci_id@p ;
...
position p;
@@
 const unsigned char * constcocci_id [ ASM_NOP_MAX + 2 ] ;
...
symbol cocci_id;
position p;
typedef temp_mm_state_t;
@@
 temp_mm_state_t cocci_id@p ;
...
position p;
@@
 struct smp_alt_module * cocci_id@p ;
...
typedef u8,s32;
@@
 struct smp_alt_module {  struct module * mod ;  char * name ;  const s32 * locks ;  const s32 * locks_end ;  u8 * text ;  u8 * text_end ;  struct list_head next ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( unsigned int * ptr ) ;
...
position p;
@@
 const unsigned char * const * cocci_id@p ;
