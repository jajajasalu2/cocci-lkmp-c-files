...
position p;
@@
 enum { KYBER_TOTAL_LATENCY , KYBER_IO_LATENCY , } cocci_id@p ;
...
typedef atomic_t;
@@
 struct kyber_cpu_latency {  atomic_t buckets [ KYBER_OTHER ] [ 2 ] [ KYBER_LATENCY_BUCKETS ] ; } cocci_id@p ;
...
position p;
@@
 struct kyber_cpu_latency cocci_id@p ;
...
@@
 int cocci_id@p ( wait_queue_entry_t * wait , unsigned mode , int flags , void * key ) ;
...
position p;
@@
 struct kyber_hctx_data * cocci_id@p ;
...
position p;
@@
 enum { KYBER_ASYNC_PERCENT = 75 , } cocci_id@p ;
...
position p;
@@
 struct kyber_queue_data * cocci_id@p ;
...
position p;
@@
 enum { KYBER_LATENCY_SHIFT = 2 , KYBER_GOOD_BUCKETS = 1 << KYBER_LATENCY_SHIFT , KYBER_LATENCY_BUCKETS = 2 << KYBER_LATENCY_SHIFT , } cocci_id@p ;
...
typedef u64;
@@
 struct kyber_queue_data {  struct request_queue * q ;  struct sbitmap_queue domain_tokens [ KYBER_NUM_DOMAINS ] ;  unsigned int async_depth ;  struct kyber_cpu_latency __percpu * cpu_latency ;  struct timer_list timer ;  unsigned int latency_buckets [ KYBER_OTHER ] [ 2 ] [ KYBER_LATENCY_BUCKETS ] ;  unsigned long latency_timeout [ KYBER_OTHER ] ;  int domain_p99 [ KYBER_OTHER ] ;  u64 latency_targets [ KYBER_OTHER ] ; } cocci_id@p ;
...
position p;
@@
 struct kyber_ctx_queue cocci_id@p ;
...
position p;
@@
 struct blk_mq_alloc_data * cocci_id@p ;
...
position p;
@@
 const struct blk_mq_debugfs_attr cocci_id@p [ ] ;
...
position p;
@@
 struct elevator_type cocci_id@p ;
...
position p;
@@
 struct flush_kcq_data cocci_id@p ;
...
position p;
@@
 struct sbq_wait_state * cocci_id@p ;
...
position p;
@@
 struct sbitmap_queue * cocci_id@p ;
...
position p;
@@
 struct elevator_queue * cocci_id@p ;
...
position p;
@@
 struct flush_kcq_data {  struct kyber_hctx_data * khd ;  unsigned int sched_domain ;  struct list_head * list ; } cocci_id@p ;
...
position p;
@@
 enum { KYBER_READ , KYBER_WRITE , KYBER_DISCARD , KYBER_OTHER , KYBER_NUM_DOMAINS , } cocci_id@p ;
...
position p;
@@
 struct elv_fs_entry cocci_id@p [ ] ;
