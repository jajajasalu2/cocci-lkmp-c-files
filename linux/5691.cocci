...
position p;
@@
 struct charlcd_priv * cocci_id@p ;
...
position p;
@@
 void ( * cocci_id@p ) ( struct charlcd * lcd , int cmd ) ;
...
typedef bool;
@@
 struct charlcd_priv {  struct charlcd lcd ;  struct delayed_work bl_work ;  struct mutex bl_tempo_lock ;  bool bl_tempo ;  bool must_clear ;  unsigned long int flags ;  struct {  unsigned long int x ;  unsigned long int y ; } addr ;  struct {  char buf [ LCD_ESCAPE_LEN + 1 ] ;  int len ; } esc_seq ;  unsigned long long drvdata [ 0 ] ; } cocci_id@p ;
