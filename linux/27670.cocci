...
position p;
@@
 void cocci_id@p ( struct nlm_block * block ) ;
...
position p;
@@
 void cocci_id@p ( struct nlm_block * block , unsigned long ) ;
...
position p;
@@
 struct nlm_lockowner * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef nlm_host_match_fn_t;
@@
 nlm_host_match_fn_t cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct nlm_rqst * call , struct nlm_lock * lock ) ;
...
position p;
@@
 char cocci_id@p [ 2 * NLM_MAXCOOKIELEN + 1 ] ;
...
position p;
@@
 const struct nlm_cookie * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct nlm_rqst * call ) ;
...
position p;
@@
 const struct file_lock_operations cocci_id@p ;
