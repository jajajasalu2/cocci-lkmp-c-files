...
position p;
@@
 struct {  unsigned char value ;  unsigned char * name ; } cocci_id@p [ ] ;
...
position p;
@@
 unsigned short cocci_id@p [ LK_NUM_KEYCODES ] ;
...
position p;
@@
 struct lkkbd {  unsigned short keycode [ LK_NUM_KEYCODES ] ;  int ignore_bytes ;  unsigned char id [ LK_NUM_IGNORE_BYTES ] ;  struct input_dev * dev ;  struct serio * serio ;  struct work_struct tq ;  char name [ 64 ] ;  char phys [ 32 ] ;  char type ;  int bell_volume ;  int keyclick_volume ;  int ctrlclick_volume ; } cocci_id@p ;
