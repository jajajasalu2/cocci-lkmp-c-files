...
typedef pid_t;
@@
 struct fgraph_cpu_data {  pid_t last_pid ;  int depth ;  int depth_irq ;  int ignore ;  unsigned long enter_funcs [ FTRACE_RETFUNC_DEPTH ] ; } cocci_id@p ;
...
position p;
@@
 struct ftrace_graph_ent_entry * cocci_id@p ;
...
typedef u32;
@@
 void cocci_id@p ( struct trace_array * tr , unsigned long long duration , struct trace_seq * s , u32 flags ) ;
...
position p;
@@
 enum { FLAGS_FILL_FULL = 1 << TRACE_GRAPH_PRINT_FILL_SHIFT , FLAGS_FILL_START = 2 << TRACE_GRAPH_PRINT_FILL_SHIFT , FLAGS_FILL_END = 3 << TRACE_GRAPH_PRINT_FILL_SHIFT , } cocci_id@p ;
...
position p;
@@
 struct fgraph_cpu_data * cocci_id@p ;
...
position p;
@@
 struct ftrace_graph_ret_entry * cocci_id@p ;
...
position p;
@@
 struct fgraph_data {  struct fgraph_cpu_data __percpu * cpu_data ;  struct ftrace_graph_ent_entry ent ;  struct ftrace_graph_ret_entry ret ;  int failed ;  int cpu ; } cocci_id@p ;
