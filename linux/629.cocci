...
position p;
@@
 const struct vmac_tfm_ctx * cocci_id@p ;
...
position p;
@@
 struct vmac_tfm_ctx cocci_id@p ;
...
position p;
@@
 struct vmac_desc_ctx cocci_id@p ;
...
typedef u64;
@@
 struct vmac_tfm_ctx {  struct crypto_cipher * cipher ;  u64 nhkey [ ( VMAC_NHBYTES / 8 ) + 2 * ( VMAC_TAG_LEN / 64 - 1 ) ] ;  u64 polykey [ 2 * VMAC_TAG_LEN / 64 ] ;  u64 l3key [ 2 * VMAC_TAG_LEN / 64 ] ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u64,u8,bool,__le64,__be64;
@@
 struct vmac_desc_ctx {  union {  u8 partial [ VMAC_NHBYTES ] ;  __le64 partial_words [ VMAC_NHBYTES / 8 ] ; } ;  unsigned int partial_size ;  bool first_block_processed ;  u64 polytmp [ 2 * VMAC_TAG_LEN / 64 ] ;  union {  u8 bytes [ VMAC_NONCEBYTES ] ;  __be64 pads [ VMAC_NONCEBYTES / 8 ] ; } nonce ;  unsigned int nonce_size ; } cocci_id@p ;
