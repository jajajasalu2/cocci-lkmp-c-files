...
typedef u32,__be32;
@@
 struct talitos_alg_template {  u32 type ;  u32 priority ;  union {  struct skcipher_alg skcipher ;  struct ahash_alg hash ;  struct aead_alg aead ; } alg ;  __be32 desc_hdr_template ; } cocci_id@p ;
...
position p;
@@
 struct talitos_private cocci_id@p ;
...
position p;
@@
 struct talitos_edesc * cocci_id@p ;
...
position p;
@@
 struct talitos_crypto_alg {  struct list_head entry ;  struct device * dev ;  struct talitos_alg_template algt ; } cocci_id@p ;
...
typedef u8,u32;
@@
 struct talitos_export_state {  u32 hw_context [ TALITOS_MDEU_MAX_CONTEXT_SIZE / sizeof ( u32 ) ] ;  u8 buf [ HASH_MAX_BLOCK_SIZE ] ;  unsigned int swinit ;  unsigned int first ;  unsigned int last ;  unsigned int to_hash_later ;  unsigned int nbuf ; } cocci_id@p ;
...
position p;
@@
 struct talitos_ahash_req_ctx * cocci_id@p ;
...
position p;
@@
 struct talitos_export_state * cocci_id@p ;
...
position p;
@@
 void ( * cocci_id@p ) ( struct device * dev , struct talitos_desc * desc , void * context , int error ) ;
...
typedef u8;
@@
 u8 cocci_id@p [ SHA512_DIGEST_SIZE ] ;
...
position p;
@@
 struct talitos_request cocci_id@p ;
...
position p;
@@
 struct talitos_crypto_alg cocci_id@p ;
...
typedef u8,u32;
@@
 struct talitos_ahash_req_ctx {  u32 hw_context [ TALITOS_MDEU_MAX_CONTEXT_SIZE / sizeof ( u32 ) ] ;  unsigned int hw_context_size ;  u8 buf [ 2 ] [ HASH_MAX_BLOCK_SIZE ] ;  int buf_idx ;  unsigned int swinit ;  unsigned int first ;  unsigned int last ;  unsigned int to_hash_later ;  unsigned int nbuf ;  struct scatterlist bufsl [ 2 ] ;  struct scatterlist * psrc ; } cocci_id@p ;
...
position p;
@@
 struct talitos_desc cocci_id@p ;
...
position p;
@@
 struct talitos_channel cocci_id@p ;
...
position p;
@@
 const struct talitos_export_state * cocci_id@p ;
...
position p;
@@
 struct talitos_alg_template cocci_id@p [ ] ;
...
symbol cocci_id;
position p;
typedef u8,__be32,dma_addr_t;
@@
 struct talitos_ctx {  struct device * dev ;  int ch ;  __be32 desc_hdr_template ;  u8 key [ TALITOS_MAX_KEY_SIZE ] ;  u8 iv [ TALITOS_MAX_IV_LENGTH ] ;  dma_addr_t dma_key ;  unsigned int keylen ;  unsigned int enckeylen ;  unsigned int authkeylen ; } cocci_id@p ;
