...
@@
 struct {  int64_t is_recovering ;  uint64_t in_sync_hint ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef region_t,int64_t;
@@
 struct {  region_t r ;  int64_t i ; } cocci_id@p ;
...
typedef region_t;
@@
 struct dm_dirty_log_flush_entry {  int type ;  region_t region ;  struct list_head list ; } cocci_id@p ;
...
typedef region_t,int64_t;
@@
 struct {  int64_t i ;  region_t r ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef mempool_t,region_t,atomic_t,uint32_t,uint64_t,spinlock_t;
@@
 struct log_c {  struct dm_target * ti ;  struct dm_dev * log_dev ;  char * usr_argv_str ;  uint32_t usr_argc ;  uint32_t region_size ;  region_t region_count ;  uint64_t luid ;  char uuid [ DM_UUID_LEN ] ;  spinlock_t flush_lock ;  struct list_head mark_list ;  struct list_head clear_list ;  uint64_t in_sync_hint ;  struct workqueue_struct * dmlog_wq ;  struct delayed_work flush_log_work ;  atomic_t sched_flush ;  uint32_t integrated_flush ;  mempool_t flush_entry_pool ; } cocci_id@p ;
...
typedef uint64_t;
@@
 uint64_t cocci_id@p [ MAX_FLUSH_GROUP_COUNT ] ;
...
position p;
@@
 struct dm_dirty_log_flush_entry * cocci_id@p ;
