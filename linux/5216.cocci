...
typedef u32;
@@
 int cocci_id@p ( struct ata_link * link , unsigned int scr , u32 val ) ;
...
typedef bool;
@@
 struct svia_priv {  bool wd_workaround ; } cocci_id@p ;
...
position p;
@@
 enum { SATA_CHAN_ENAB = 0x40 , SATA_INT_GATE = 0x41 , SATA_NATIVE_MODE = 0x42 , SVIA_MISC_3 = 0x46 , PATA_UDMA_TIMING = 0xB3 , PATA_PIO_TIMING = 0xAB , PORT0 = ( 1 << 1 ) , PORT1 = ( 1 << 0 ) , ALL_PORTS = PORT0 | PORT1 , NATIVE_MODE_ALL = ( 1 << 7 ) | ( 1 << 6 ) | ( 1 << 5 ) | ( 1 << 4 ) , SATA_EXT_PHY = ( 1 << 6 ) , SATA_HOTPLUG = ( 1 << 5 ) , } cocci_id@p ;
...
position p;
@@
 enum board_ids_enum { vt6420 , vt6421 , vt8251 , } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct ata_port * ap , struct ata_device * adev ) ;
