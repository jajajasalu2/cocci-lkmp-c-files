...
position p;
@@
 struct evm_ima_xattr_data * cocci_id@p ;
...
position p;
@@
 const int cocci_id@p [ READING_MAX_ID ] ;
...
position p;
@@
 struct ima_template_entry * cocci_id@p ;
...
position p;
@@
 struct {  struct ima_digest_data hdr ;  char digest [ IMA_MAX_DIGEST_SIZE ] ; } cocci_id@p ;
...
position p;
@@
 struct ima_event_data cocci_id@p ;
