...
position p;
@@
 const struct intc_irqpin_config * cocci_id@p ;
...
position p;
@@
 void ( * cocci_id@p ) ( struct irq_data * d ) ;
...
position p;
@@
 struct intc_irqpin_iomem * cocci_id@p ;
...
typedef u8,atomic_t;
@@
 struct intc_irqpin_priv {  struct intc_irqpin_iomem iomem [ INTC_IRQPIN_REG_NR ] ;  struct intc_irqpin_irq irq [ INTC_IRQPIN_MAX ] ;  unsigned int sense_bitfield_width ;  struct platform_device * pdev ;  struct irq_chip irq_chip ;  struct irq_domain * irq_domain ;  atomic_t wakeup_path ;  unsigned shared_irqs : 1 ;  u8 shared_irq_mask ; } cocci_id@p ;
...
position p;
@@
 struct intc_irqpin_irq * cocci_id@p ;
...
position p;
@@
 struct intc_irqpin_config {  unsigned int irlm_bit ;  unsigned needs_irlm : 1 ; } cocci_id@p ;
...
position p;
@@
 struct intc_irqpin_priv * cocci_id@p ;
...
position p;
@@
 struct resource * cocci_id@p [ INTC_IRQPIN_REG_NR ] ;
...
position p;
@@
 struct intc_irqpin_irq {  int hw_irq ;  int requested_irq ;  int domain_irq ;  struct intc_irqpin_priv * p ; } cocci_id@p ;
...
position p;
@@
 unsigned char cocci_id@p [ IRQ_TYPE_SENSE_MASK + 1 ] ;
