...
typedef spinlock_t;
@@
 struct cops_local {  int board ;  int nodeid ;  unsigned char node_acquire ;  struct atalk_addr node_addr ;  spinlock_t lock ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( int ioaddr , int board ) ;
...
position p;
@@
 void cocci_id@p ( struct net_device * dev , int sleep ) ;
...
position p;
@@
 int cocci_id@p ( struct net_device * dev , int nodeid ) ;
