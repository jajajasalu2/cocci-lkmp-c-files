...
position p;
@@
 const struct nfp_cpp_operations cocci_id@p ;
...
position p;
@@
 const struct nfp_bar * cocci_id@p ;
...
position p;
@@
 const struct nfp_cpp_explicit_command * cocci_id@p ;
...
position p;
@@
 struct nfp6000_area_priv cocci_id@p ;
...
position p;
@@
 struct nfp6000_pcie cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct nfp6000_pcie * nfp ) ;
...
position p;
@@
 struct nfp6000_explicit_priv {  struct nfp6000_pcie * nfp ;  struct {  int group ;  int area ; } bar ;  int bitsize ;  void __iomem * data ;  void __iomem * addr ; } cocci_id@p ;
...
position p;
@@
 struct nfp_cpp_explicit * cocci_id@p ;
...
position p;
@@
 struct nfp6000_explicit_priv cocci_id@p ;
...
position p;
@@
 const struct nfp6000_pcie * cocci_id@p ;
...
@@
 struct nfp6000_pcie {  struct pci_dev * pdev ;  struct device * dev ;  spinlock_t bar_lock ;  int bars ;  struct nfp_bar bar [ NFP_PCI_BAR_MAX ] ;  wait_queue_head_t bar_waiters ;  struct {  void __iomem * csr ;  void __iomem * em ;  void __iomem * expl [ 4 ] ; } iomem ;  struct {  struct mutex mutex ;  u8 master_id ;  u8 signal_ref ;  void __iomem * data ;  struct {  void __iomem * addr ;  int bitsize ;  int free [ 4 ] ; } group [ 4 ] ; } expl ; } cocci_id@p ;
...
position p;
@@
 struct nfp_cpp_area * cocci_id@p ;
...
@@
 struct nfp_bar {  struct nfp6000_pcie * nfp ;  u32 barcfg ;  u64 base ;  u64 mask ;  u32 bitsize ;  int index ;  atomic_t refcnt ;  void __iomem * iomem ;  struct resource * resource ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u64,u32,phys_addr_t,atomic_t;
@@
 struct nfp6000_area_priv {  atomic_t refcnt ;  struct nfp_bar * bar ;  u32 bar_offset ;  u32 target ;  u32 action ;  u32 token ;  u64 offset ;  struct {  int read ;  int write ;  int bar ; } width ;  size_t size ;  void __iomem * iomem ;  phys_addr_t phys ;  struct resource resource ; } cocci_id@p ;
