...
position p;
@@
 struct cls_mall_head cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_MATCHALL_MAX + 1 ] ;
...
position p;
@@
 struct tc_matchall_pcnt * cocci_id@p ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_MATCHALL_MAX + 1 ] ;
...
typedef bool,u32;
@@
 struct cls_mall_head {  struct tcf_exts exts ;  struct tcf_result res ;  u32 handle ;  u32 flags ;  unsigned int in_hw_count ;  struct tc_matchall_pcnt __percpu * pf ;  struct rcu_work rwork ;  bool deleting ; } cocci_id@p ;
