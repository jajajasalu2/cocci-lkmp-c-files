...
position p;
@@
 enum dspi_trans_mode { DSPI_EOQ_MODE = 0 , DSPI_TCFQ_MODE , DSPI_DMA_MODE , } cocci_id@p ;
...
position p;
@@
 const struct fsl_dspi_devtype_data cocci_id@p ;
...
typedef u8,bool;
@@
 struct fsl_dspi_devtype_data {  enum dspi_trans_mode trans_mode ;  u8 max_clock_factor ;  bool ptp_sts_supported ;  bool xspi_mode ; } cocci_id@p ;
...
@@
 struct fsl_dspi {  struct spi_controller * ctlr ;  struct platform_device * pdev ;  struct regmap * regmap ;  struct regmap * regmap_pushr ;  int irq ;  struct clk * clk ;  struct spi_transfer * cur_transfer ;  struct spi_message * cur_msg ;  struct chip_data * cur_chip ;  size_t len ;  const void * tx ;  void * rx ;  void * rx_end ;  u16 void_write_data ;  u16 tx_cmd ;  u8 bits_per_word ;  u8 bytes_per_word ;  const struct fsl_dspi_devtype_data * devtype_data ;  wait_queue_head_t waitq ;  u32 waitflags ;  struct fsl_dspi_dma * dma ; } cocci_id@p ;
...
typedef u32,u16;
@@
 struct chip_data {  u32 ctar_val ;  u16 void_write_data ; } cocci_id@p ;
...
position p;
@@
 struct fsl_dspi_platform_data * cocci_id@p ;
...
position p;
@@
 enum dspi_trans_mode cocci_id@p ;
...
position p;
@@
 struct fsl_dspi_dma * cocci_id@p ;
