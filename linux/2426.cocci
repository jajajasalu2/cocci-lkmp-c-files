...
typedef bool;
@@
 struct s3c24xx_dma_chan {  int id ;  const char * name ;  struct virt_dma_chan vc ;  struct s3c24xx_dma_phy * phy ;  struct dma_slave_config cfg ;  struct s3c24xx_txd * at ;  struct s3c24xx_dma_engine * host ;  enum s3c24xx_dma_chan_state state ;  bool slave ; } cocci_id@p ;
...
position p;
@@
 const struct s3c24xx_dma_platdata * cocci_id@p ;
...
position p;
@@
 struct s3c24xx_dma_channel * cocci_id@p ;
...
@@
 struct s3c24xx_dma_phy {  unsigned int id ;  bool valid ;  void __iomem * base ;  int irq ;  struct clk * clk ;  spinlock_t lock ;  struct s3c24xx_dma_chan * serving ;  struct s3c24xx_dma_engine * host ; } cocci_id@p ;
...
typedef u8,bool,u32;
@@
 struct s3c24xx_txd {  struct virt_dma_desc vd ;  struct list_head dsg_list ;  struct list_head * at ;  u8 width ;  u32 disrcc ;  u32 didstc ;  u32 dcon ;  bool cyclic ; } cocci_id@p ;
...
position p;
@@
 enum s3c24xx_dma_chan_state { S3C24XX_DMA_CHAN_IDLE , S3C24XX_DMA_CHAN_RUNNING , S3C24XX_DMA_CHAN_WAITING , } cocci_id@p ;
...
position p;
@@
 struct s3c24xx_dma_chan cocci_id@p ;
...
typedef bool;
@@
 struct soc_data {  int stride ;  bool has_reqsel ;  bool has_clocks ; } cocci_id@p ;
...
position p;
@@
 struct s3c24xx_dma_engine * cocci_id@p ;
...
position p;
@@
 struct s3c24xx_dma_phy cocci_id@p ;
...
position p;
@@
 struct s3c24xx_dma_engine {  struct platform_device * pdev ;  const struct s3c24xx_dma_platdata * pdata ;  struct soc_data * sdata ;  void __iomem * base ;  struct dma_device slave ;  struct dma_device memcpy ;  struct s3c24xx_dma_phy * phy_chans ; } cocci_id@p ;
...
typedef dma_addr_t;
@@
 struct s3c24xx_sg {  dma_addr_t src_addr ;  dma_addr_t dst_addr ;  size_t len ;  struct list_head node ; } cocci_id@p ;
