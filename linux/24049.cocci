...
position p;
@@
 const struct action_fifo * cocci_id@p ;
...
position p;
@@
 struct action_flow_keys cocci_id@p ;
...
position p;
@@
 struct ovs_frag_data * cocci_id@p ;
...
position p;
@@
 const struct nshhdr * cocci_id@p ;
...
position p;
@@
 struct action_fifo {  int head ;  int tail ;  struct deferred_action fifo [ DEFERRED_ACTION_FIFO_SIZE ] ; } cocci_id@p ;
...
position p;
@@
 const struct ovs_action_push_eth * cocci_id@p ;
...
position p;
@@
 struct dp_upcall_info cocci_id@p ;
...
typedef bool,u32;
@@
 int cocci_id@p ( struct datapath * dp , struct sk_buff * skb , struct sw_flow_key * key , u32 recirc_id , const struct nlattr * actions , int len , bool last , bool clone_flow_key ) ;
...
position p;
@@
 const struct sw_flow_actions * cocci_id@p ;
...
typedef u8,u16;
@@
 struct ovs_frag_data {  unsigned long dst ;  struct vport * vport ;  struct ovs_skb_cb cb ;  __be16 inner_protocol ;  u16 network_offset ;  u16 vlan_tci ;  __be16 vlan_proto ;  unsigned int l2_len ;  u8 mac_proto ;  u8 l2_data [ MAX_L2_LEN ] ; } cocci_id@p ;
...
position p;
@@
 struct mpls_shim_hdr * cocci_id@p ;
...
position p;
@@
 struct action_flow_keys {  struct sw_flow_key key [ OVS_DEFERRED_ACTION_THRESHOLD ] ; } cocci_id@p ;
...
position p;
@@
 struct deferred_action {  struct sk_buff * skb ;  const struct nlattr * actions ;  int actions_len ;  struct sw_flow_key pkt_key ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct datapath * dp , struct sk_buff * skb , struct sw_flow_key * key , const struct nlattr * attr , int len ) ;
...
position p;
@@
 struct deferred_action * cocci_id@p ;
...
position p;
@@
 const struct nf_ipv6_ops * cocci_id@p ;
