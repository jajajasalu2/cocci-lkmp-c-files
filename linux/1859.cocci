...
position p;
@@
 struct tiadc_device * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,dma_cookie_t,dma_addr_t;
@@
 struct tiadc_dma {  struct dma_slave_config conf ;  struct dma_chan * chan ;  dma_addr_t addr ;  dma_cookie_t cookie ;  u8 * buf ;  int current_period ;  int period_size ;  u8 fifo_thresh ; } cocci_id@p ;
...
typedef irqreturn_t;
@@
 irqreturn_t ( * cocci_id@p ) ( int irq , void * p ) ;
...
typedef u8,u32,u16;
@@
 struct tiadc_device {  struct ti_tscadc_dev * mfd_tscadc ;  struct tiadc_dma dma ;  struct mutex fifo1_lock ;  int channels ;  int total_ch_enabled ;  u8 channel_line [ 8 ] ;  u8 channel_step [ 8 ] ;  int buffer_en_ch_steps ;  u16 data [ 8 ] ;  u32 open_delay [ 8 ] , sample_delay [ 8 ] , step_avg [ 8 ] ; } cocci_id@p ;
