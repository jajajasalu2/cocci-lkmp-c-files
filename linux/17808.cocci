...
position p;
@@
 struct salinfo_data * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,u64,wait_queue_head_t,cpumask_t;
@@
 struct salinfo_data {  cpumask_t cpu_event ;  wait_queue_head_t read_wait ;  u8 * log_buffer ;  u64 log_size ;  u8 * oemdata ;  u64 oemdata_size ;  int open ;  u8 type ;  u8 saved_num ;  enum salinfo_state state : 8 ;  u8 padding ;  int cpu_check ;  struct salinfo_data_saved data_saved [ 5 ] ; } cocci_id@p ;
...
position p;
@@
 struct {  const char * name ;  unsigned long feature ;  struct proc_dir_entry * entry ; } cocci_id@p ;
...
position p;
@@
 struct salinfo_data cocci_id@p [ ARRAY_SIZE ( salinfo_log_name ) ] ;
...
position p;
@@
 struct salinfo_data_saved * cocci_id@p ;
...
position p;
@@
 struct proc_dir_entry * cocci_id@p [ ARRAY_SIZE ( salinfo_entries ) + ARRAY_SIZE ( salinfo_log_name ) + ( 2 * ARRAY_SIZE ( salinfo_log_name ) ) + 1 ] ;
...
typedef u8,u64;
@@
 int ( * cocci_id@p ) ( const u8 * sect_header , u8 * * oemdata , u64 * oemdata_size ) ;
...
symbol cocci_id;
position p;
typedef salinfo_entry_t;
@@
 const salinfo_entry_t cocci_id@p [ ] ;
...
position p;
@@
 enum salinfo_state { STATE_NO_DATA , STATE_LOG_RECORD , STATE_OEMDATA , } cocci_id@p ;
...
position p;
@@
 struct salinfo_platform_oemdata_parms * cocci_id@p ;
...
typedef u8,u64;
@@
 struct salinfo_data_saved {  u8 * buffer ;  u64 size ;  u64 id ;  int cpu ; } cocci_id@p ;
...
typedef u8,u64;
@@
 struct salinfo_platform_oemdata_parms {  const u8 * efi_guid ;  u8 * * oemdata ;  u64 * oemdata_size ; } cocci_id@p ;
