...
position p;
@@
 struct iq_post_status cocci_id@p ;
...
position p;
@@
 void * cocci_id@p [ MAX_OCTEON_DEVICES ] [ REQTYPE_LAST + 1 ] ( void * ) ;
...
position p;
@@
 struct octeon_instr_rdp * cocci_id@p ;
...
position p;
@@
 struct octeon_response_list * cocci_id@p ;
...
typedef u64;
@@
 void cocci_id@p ( struct octeon_device * oct , u64 iq_no ) ;
...
position p;
@@
 struct iq_post_status {  int status ;  int index ; } cocci_id@p ;
...
position p;
@@
 struct octeon_iq_config * cocci_id@p ;
