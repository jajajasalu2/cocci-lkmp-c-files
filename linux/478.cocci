...
symbol cocci_id;
position p;
typedef u64,atomic_t,spinlock_t;
@@
 struct child_latency_info {  spinlock_t lock ;  u64 last_scale_event ;  u64 scale_lat ;  u64 nr_samples ;  struct iolatency_grp * scale_grp ;  atomic_t scale_cookie ; } cocci_id@p ;
...
position p;
@@
 struct blk_iolatency * cocci_id@p ;
...
position p;
@@
 struct latency_stat {  union {  struct percentile_stats ps ;  struct blk_rq_stat rqs ; } ; } cocci_id@p ;
...
typedef atomic_t;
@@
 struct blk_iolatency {  struct rq_qos rqos ;  struct timer_list timer ;  atomic_t enabled ; } cocci_id@p ;
...
typedef u64;
@@
 const u64 cocci_id@p [ BLKIOLATENCY_NR_EXP_FACTORS ] ;
...
position p;
@@
 struct iolatency_grp * cocci_id@p ;
...
typedef u64;
@@
 struct percentile_stats {  u64 total ;  u64 missed ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u64,bool,atomic64_t,atomic_t;
@@
 struct iolatency_grp {  struct blkg_policy_data pd ;  struct latency_stat __percpu * stats ;  struct latency_stat cur_stat ;  struct blk_iolatency * blkiolat ;  struct rq_depth rq_depth ;  struct rq_wait rq_wait ;  atomic64_t window_start ;  atomic_t scale_cookie ;  u64 min_lat_nsec ;  u64 cur_win_nsec ;  u64 lat_avg ;  u64 nr_samples ;  bool ssd ;  struct child_latency_info child_lat ; } cocci_id@p ;
...
position p;
@@
 struct child_latency_info * cocci_id@p ;
...
position p;
@@
 struct latency_stat cocci_id@p ;
