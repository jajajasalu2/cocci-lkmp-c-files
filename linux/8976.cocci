...
position p;
@@
 enum { MAX_TXQ_ENTRIES = 16384 , MAX_CTRL_TXQ_ENTRIES = 1024 , MAX_RSPQ_ENTRIES = 16384 , MAX_RX_BUFFERS = 16384 , MAX_RX_JUMBO_BUFFERS = 16384 , MIN_TXQ_ENTRIES = 4 , MIN_CTRL_TXQ_ENTRIES = 4 , MIN_RSPQ_ENTRIES = 32 , MIN_FL_ENTRIES = 32 , } cocci_id@p ;
...
position p;
@@
 struct cpl_smt_write_req * cocci_id@p ;
...
position p;
@@
 struct cpl_l2t_write_req * cocci_id@p ;
...
typedef u16;
@@
 u16 cocci_id@p [ RSS_TABLE_SIZE + 1 ] ;
...
position p;
@@
 struct cpl_rte_write_req * cocci_id@p ;
...
position p;
@@
 ssize_t ( * cocci_id@p ) ( struct net_device * , char * ) ;
...
position p;
@@
 const struct mac_stats * cocci_id@p ;
...
position p;
@@
 ssize_t ( * cocci_id@p ) ( struct net_device * , unsigned int ) ;
...
position p;
@@
 struct ch_qset_params cocci_id@p ;
...
position p;
@@
 const struct adapter_info * cocci_id@p ;
...
position p;
@@
 struct mngt_pktsched_wr * cocci_id@p ;
...
position p;
@@
 struct msix_entry cocci_id@p [ SGE_QSETS + 1 ] ;
...
position p;
@@
 const struct adapter_params * cocci_id@p ;
