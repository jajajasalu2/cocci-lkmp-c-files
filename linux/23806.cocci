...
position p;
@@
 struct flow_head {  struct list_head filters ;  struct rcu_head rcu ; } cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_FLOW_MAX + 1 ] ;
...
typedef u32;
@@
 struct flow_filter {  struct list_head list ;  struct tcf_exts exts ;  struct tcf_ematch_tree ematches ;  struct tcf_proto * tp ;  struct timer_list perturb_timer ;  u32 perturb_period ;  u32 handle ;  u32 nkeys ;  u32 keymask ;  u32 mode ;  u32 mask ;  u32 xor ;  u32 rshift ;  u32 addend ;  u32 divisor ;  u32 baseclass ;  u32 hashrnd ;  struct rcu_work rwork ; } cocci_id@p ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_FLOW_MAX + 1 ] ;
