...
position p;
@@
 struct svc_cacherep * cocci_id@p ;
...
typedef spinlock_t;
@@
 struct nfsd_drc_bucket {  struct rb_root rb_head ;  struct list_head lru_head ;  spinlock_t cache_lock ; } cocci_id@p ;
...
position p;
@@
 unsigned long cocci_id@p ( struct shrinker * shrink , struct shrink_control * sc ) ;
...
position p;
@@
 int cocci_id@p ( struct svc_rqst * rqstp , struct kvec * vec ) ;
...
position p;
@@
 struct nfsd_drc_bucket * cocci_id@p ;
...
position p;
@@
 const struct svc_cacherep * cocci_id@p ;
