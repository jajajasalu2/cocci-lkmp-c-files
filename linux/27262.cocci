...
position p;
@@
 struct nfs4_dir_ctx {  struct dir_context ctx ;  struct list_head names ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct dentry * , struct dentry * , struct nfsd_net * ) ;
...
typedef uint8_t;
@@
 struct nfsd4_client_tracking_ops {  int ( * init ) ( struct net * ) ;  void ( * exit ) ( struct net * ) ;  void ( * create ) ( struct nfs4_client * ) ;  void ( * remove ) ( struct nfs4_client * ) ;  int ( * check ) ( struct nfs4_client * ) ;  void ( * grace_done ) ( struct nfsd_net * ) ;  uint8_t version ;  size_t msglen ; } cocci_id@p ;
...
position p;
@@
 struct nfs4_dir_ctx cocci_id@p ;
...
@@
 struct cld_net {  struct rpc_pipe * cn_pipe ;  spinlock_t cn_lock ;  struct list_head cn_list ;  unsigned int cn_xid ;  bool cn_has_legacy ;  struct crypto_shash * cn_tfm ; } cocci_id@p ;
...
position p;
@@
 const struct nfsd4_client_tracking_ops cocci_id@p ;
...
position p;
@@
 struct cld_upcall {  struct list_head cu_list ;  struct cld_net * cu_net ;  struct completion cu_done ;  union {  struct cld_msg_hdr cu_hdr ;  struct cld_msg cu_msg ;  struct cld_msg_v2 cu_msg_v2 ; } cu_u ; } cocci_id@p ;
...
position p;
@@
 struct name_list {  char name [ HEXDIR_LEN ] ;  struct list_head list ; } cocci_id@p ;
