...
position p;
@@
 unsigned long * const cocci_id@p [ DATA_MODE_LIMIT ] ;
...
position p;
@@
 enum new_flag { NF_FRESH = 0 , NF_READ = 1 , NF_GET = 2 , NF_PREFETCH = 3 , } cocci_id@p ;
...
position p;
@@
 void ( * cocci_id@p ) ( struct dm_buffer * ) ;
...
position p;
@@
 enum data_mode { DATA_MODE_SLAB = 0 , DATA_MODE_GET_FREE_PAGES = 1 , DATA_MODE_VMALLOC = 2 , DATA_MODE_LIMIT = 3 , } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef s8,wait_queue_head_t,sector_t;
@@
 struct dm_bufio_client {  struct mutex lock ;  struct list_head lru [ LIST_SIZE ] ;  unsigned long n_buffers [ LIST_SIZE ] ;  struct block_device * bdev ;  unsigned block_size ;  s8 sectors_per_block_bits ;  void ( * alloc_callback ) ( struct dm_buffer * ) ;  void ( * write_callback ) ( struct dm_buffer * ) ;  struct kmem_cache * slab_buffer ;  struct kmem_cache * slab_cache ;  struct dm_io_client * dm_io ;  struct list_head reserved_buffers ;  unsigned need_reserved_buffers ;  unsigned minimum_buffers ;  struct rb_root buffer_tree ;  wait_queue_head_t free_buffer_wait ;  sector_t start ;  int async_write_error ;  struct list_head client_list ;  struct shrinker shrinker ; } cocci_id@p ;
