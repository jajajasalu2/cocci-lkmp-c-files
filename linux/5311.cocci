...
symbol cocci_id;
position p;
typedef ata_postreset_fn_t;
@@
 ata_postreset_fn_t cocci_id@p ;
...
symbol cocci_id;
position p;
typedef ata_reset_fn_t;
@@
 ata_reset_fn_t cocci_id@p ;
...
symbol cocci_id;
position p;
typedef ata_prereset_fn_t;
@@
 ata_prereset_fn_t cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct ata_ering_entry * , void * ) ;
...
position p;
@@
 struct speed_down_verdict_arg cocci_id@p ;
...
typedef u8;
@@
 const struct {  u8 command ;  const char * text ; } cocci_id@p [ ] ;
...
position p;
@@
 const struct ata_eh_cmd_timeout_ent cocci_id@p [ ATA_EH_CMD_TIMEOUT_TABLE_SIZE ] ;
...
typedef u8;
@@
 struct ata_eh_cmd_timeout_ent {  const u8 * commands ;  const unsigned long * timeouts ; } cocci_id@p ;
...
typedef u64;
@@
 struct speed_down_verdict_arg {  u64 since ;  int xfer_ok ;  int nr_errors [ ATA_ECAT_NR ] ; } cocci_id@p ;
...
position p;
@@
 struct ata_ering_entry * cocci_id@p ;
