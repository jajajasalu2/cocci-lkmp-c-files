...
typedef u64;
@@
 struct ghash_key {  u64 h [ 2 ] ;  u64 h2 [ 2 ] ;  u64 h3 [ 2 ] ;  u64 h4 [ 2 ] ;  be128 k ; } cocci_id@p ;
...
typedef u8,u64,u32;
@@
 struct ghash_desc_ctx {  u64 digest [ GHASH_DIGEST_SIZE / sizeof ( u64 ) ] ;  u8 buf [ GHASH_BLOCK_SIZE ] ;  u32 count ; } cocci_id@p ;
...
typedef u8,u64,u32;
@@
 void cocci_id@p ( int bytes , u8 dst [ ] , const u8 src [ ] , const struct ghash_key * k , u64 dg [ ] , u8 ctr [ ] , const u32 rk [ ] , int rounds , u8 tag [ ] ) ;
...
position p;
@@
 struct gcm_aes_ctx {  struct crypto_aes_ctx aes_key ;  struct ghash_key ghash_key ; } cocci_id@p ;
...
typedef u64;
@@
 void cocci_id@p ( int blocks , u64 dg [ ] , const char * src , const struct ghash_key * k , const char * head ) ;
...
position p;
@@
 const struct cpu_feature cocci_id@p [ ] ;
...
position p;
@@
 struct ghash_desc_ctx * cocci_id@p ;
...
typedef u8;
@@
 u8 cocci_id@p [ GHASH_BLOCK_SIZE ] ;
