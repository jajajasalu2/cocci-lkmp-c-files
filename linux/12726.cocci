...
position p;
@@
 struct rk_iommudata * cocci_id@p ;
...
typedef bool;
@@
 struct rk_iommu {  struct device * dev ;  void __iomem * * bases ;  int num_mmu ;  int num_irq ;  struct clk_bulk_data * clocks ;  int num_clocks ;  bool reset_disabled ;  struct iommu_device iommu ;  struct list_head node ;  struct iommu_domain * domain ;  struct iommu_group * group ; } cocci_id@p ;
...
position p;
@@
 struct rk_iommu_domain cocci_id@p ;
...
@@
 struct rk_iommu_domain {  struct list_head iommus ;  u32 * dt ;  dma_addr_t dt_dma ;  spinlock_t iommus_lock ;  spinlock_t dt_lock ;  struct iommu_domain domain ; } cocci_id@p ;
...
position p;
@@
 struct rk_iommudata {  struct device_link * link ;  struct rk_iommu * iommu ; } cocci_id@p ;
