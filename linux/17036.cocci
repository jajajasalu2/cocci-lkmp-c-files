...
position p;
@@
 unsigned char cocci_id@p [ GC_MAX_DEVICES ] ;
...
position p;
@@
 unsigned char cocci_id@p [ GC_MULTI2_LENGTH ] ;
...
position p;
@@
 enum gc_type { GC_NONE = 0 , GC_SNES , GC_NES , GC_NES4 , GC_MULTI , GC_MULTI2 , GC_N64 , GC_PSX , GC_DDR , GC_SNESMOUSE , GC_MAX , } cocci_id@p ;
...
position p;
@@
 unsigned char cocci_id@p [ GC_MAX_DEVICES ] [ GC_PSX_BYTES ] ;
...
position p;
@@
 struct gc {  struct pardevice * pd ;  struct gc_pad pads [ GC_MAX_DEVICES ] ;  struct timer_list timer ;  int pad_count [ GC_MAX ] ;  int used ;  int parportno ;  struct mutex mutex ; } cocci_id@p ;
...
position p;
@@
 unsigned char cocci_id@p [ GC_SNESMOUSE_LENGTH ] ;
...
position p;
@@
 struct ff_rumble_effect * cocci_id@p ;
...
position p;
@@
 struct gc_pad {  struct input_dev * dev ;  enum gc_type type ;  char phys [ 32 ] ; } cocci_id@p ;
...
position p;
@@
 struct gc_subdev {  unsigned int idx ; } cocci_id@p ;
...
position p;
@@
 unsigned char cocci_id@p [ GC_N64_LENGTH ] ;
...
position p;
@@
 struct gc_config cocci_id@p [ GC_MAX_PORTS ] ;
...
position p;
@@
 struct gc_config {  int args [ GC_MAX_DEVICES + 1 ] ;  unsigned int nargs ; } cocci_id@p ;
