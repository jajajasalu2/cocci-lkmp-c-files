...
typedef spinlock_t;
@@
 struct dm_transaction_manager {  int is_clone ;  struct dm_transaction_manager * real ;  struct dm_block_manager * bm ;  struct dm_space_map * sm ;  spinlock_t lock ;  struct hlist_head buckets [ DM_HASH_SIZE ] ;  struct prefetch_set prefetches ; } cocci_id@p ;
...
position p;
@@
 struct prefetch_set * cocci_id@p ;
...
typedef dm_block_t;
@@
 struct shadow_info {  struct hlist_node hlist ;  dm_block_t where ; } cocci_id@p ;
...
typedef dm_block_t;
@@
 struct prefetch_set {  struct mutex lock ;  dm_block_t blocks [ PREFETCH_SIZE ] ; } cocci_id@p ;
