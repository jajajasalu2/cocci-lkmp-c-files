...
position p;
@@
 void cocci_id@p ( struct ufx_data * dev ) ;
...
typedef u32;
@@
 struct pll_values {  u32 div_r0 ;  u32 div_f0 ;  u32 div_q0 ;  u32 range0 ;  u32 div_r1 ;  u32 div_f1 ;  u32 div_q1 ;  u32 range1 ; } cocci_id@p ;
...
position p;
@@
 struct urb_node {  struct list_head entry ;  struct ufx_data * dev ;  struct delayed_work release_urb_work ;  struct urb * urb ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,bool,u32,atomic_t;
@@
 struct ufx_data {  struct usb_device * udev ;  struct device * gdev ;  struct fb_info * info ;  struct urb_list urbs ;  struct kref kref ;  int fb_count ;  bool virtualized ;  struct delayed_work free_framebuffer_work ;  atomic_t usb_active ;  atomic_t lost_pixels ;  u8 * edid ;  size_t edid_size ;  u32 pseudo_palette [ 256 ] ; } cocci_id@p ;
...
position p;
@@
 const struct fb_bitfield cocci_id@p ;
...
typedef spinlock_t;
@@
 struct urb_list {  struct list_head list ;  spinlock_t lock ;  struct semaphore limit_sem ;  int available ;  int count ;  size_t size ; } cocci_id@p ;
...
position p;
@@
 struct urb * cocci_id@p ( struct ufx_data * dev ) ;
