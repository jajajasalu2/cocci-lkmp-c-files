...
symbol cocci_id;
position p;
typedef FILE,uint32_t;
@@
 int ( * cocci_id@p ) ( uint32_t , FILE * ) ;
...
typedef Elf_Sym;
@@
 int ( * cocci_id@p ) ( struct section * sec , Elf_Rel * rel , Elf_Sym * sym , const char * symname ) ;
...
position p;
@@
 const char * constcocci_id [ S_NSYMTYPES ] ;
...
symbol cocci_id;
position p;
typedef Elf_Sym,Elf_Shdr;
@@
 struct section {  Elf_Shdr shdr ;  struct section * link ;  Elf_Sym * symtab ;  Elf_Rel * reltab ;  char * strtab ; } cocci_id@p ;
...
typedef uint32_t;
@@
 struct relocs {  uint32_t * offset ;  unsigned long count ;  unsigned long size ; } cocci_id@p ;
...
typedef regex_t;
@@
 regex_t cocci_id@p [ S_NSYMTYPES ] ;
