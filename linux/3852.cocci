...
position p;
@@
 struct sahara_aes_reqctx cocci_id@p ;
...
typedef u32;
@@
 struct sahara_hw_link {  u32 len ;  u32 p ;  u32 next ; } cocci_id@p ;
...
position p;
@@
 struct sahara_sha_reqctx * cocci_id@p ;
...
position p;
@@
 struct sahara_hw_desc cocci_id@p ;
...
typedef u8;
@@
 struct sahara_sha_reqctx {  u8 buf [ SAHARA_MAX_SHA_BLOCK_SIZE ] ;  u8 rembuf [ SAHARA_MAX_SHA_BLOCK_SIZE ] ;  u8 context [ SHA256_DIGEST_SIZE + 4 ] ;  unsigned int mode ;  unsigned int digest_size ;  unsigned int context_size ;  unsigned int buf_cnt ;  unsigned int sg_in_idx ;  struct scatterlist * in_sg ;  struct scatterlist in_sg_chain [ 2 ] ;  size_t total ;  unsigned int last ;  unsigned int first ;  unsigned int active ; } cocci_id@p ;
...
typedef u8;
@@
 struct sahara_ctx {  unsigned long flags ;  int keylen ;  u8 key [ AES_KEYSIZE_128 ] ;  struct crypto_sync_skcipher * fallback ; } cocci_id@p ;
...
position p;
@@
 struct sahara_aes_reqctx {  unsigned long mode ; } cocci_id@p ;
...
typedef u32;
@@
 struct sahara_hw_desc {  u32 hdr ;  u32 len1 ;  u32 p1 ;  u32 len2 ;  u32 p2 ;  u32 next ; } cocci_id@p ;
...
position p;
@@
 struct sahara_hw_link cocci_id@p ;
