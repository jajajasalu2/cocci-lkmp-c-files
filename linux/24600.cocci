...
position p;
@@
 struct old_timeval32 * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct net * net , int val ) ;
...
position p;
@@
 struct sock_exterr_skb * cocci_id@p ;
...
position p;
@@
 struct prot_inuse {  int val [ PROTO_INUSE_NR ] ; } cocci_id@p ;
...
typedef u32;
@@
 u32 cocci_id@p [ SK_MEMINFO_VARS ] ;
...
position p;
@@
 const char * const cocci_id@p [ AF_MAX + 1 ] ;
...
typedef u64;
@@
 union {  int val ;  u64 val64 ;  unsigned long ulval ;  struct linger ling ;  struct old_timeval32 tm32 ;  struct __kernel_old_timeval tm ;  struct __kernel_sock_timeval stm ;  struct sock_txtime txtime ; } cocci_id@p ;
...
position p;
@@
 struct lock_class_key cocci_id@p [ AF_MAX ] ;
...
position p;
@@
 const struct group_info * cocci_id@p ;
