...
position p;
@@
 const struct vz89x_chip_data * cocci_id@p ;
...
typedef u8,bool;
@@
 struct vz89x_data {  struct i2c_client * client ;  const struct vz89x_chip_data * chip ;  struct mutex lock ;  int ( * xfer ) ( struct vz89x_data * data , u8 cmd ) ;  bool is_valid ;  unsigned long last_update ;  u8 buffer [ VZ89TE_REG_MEASUREMENT_RD_SIZE ] ; } cocci_id@p ;
...
position p;
@@
 struct vz89x_chip_data cocci_id@p ;
...
typedef u8,bool;
@@
 struct vz89x_chip_data {  bool ( * valid ) ( struct vz89x_data * data ) ;  const struct iio_chan_spec * channels ;  u8 num_channels ;  u8 cmd ;  u8 read_size ;  u8 write_size ; } cocci_id@p ;
