...
symbol cocci_id;
position p;
typedef u32,hv_protocol_t,spinlock_t;
@@
 struct hvterm_priv {  u32 termno ;  hv_protocol_t proto ;  struct hvsi_priv hvsi ;  spinlock_t buf_lock ;  char buf [ SIZE_VIO_GET_CHARS ] ;  int left ;  int offset ; } cocci_id@p ;
...
position p;
@@
 struct hvterm_priv * cocci_id@p [ MAX_NR_HVC_CONSOLES ] ;
...
position p;
@@
 enum hv_protocol { HV_PROTOCOL_RAW , HV_PROTOCOL_HVSI , } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef hv_protocol_t;
@@
 hv_protocol_t cocci_id@p ;
