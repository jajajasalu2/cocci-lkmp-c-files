...
position p;
@@
 char cocci_id@p [ ACPIHID_UID_LEN + ACPIHID_HID_LEN ] ;
...
position p;
@@
 struct amd_iommu * cocci_id@p [ MAX_IOMMUS ] ;
...
position p;
@@
 enum iommu_init_state cocci_id@p ;
...
typedef u8,u64,bool;
@@
 int cocci_id@p ( struct amd_iommu * iommu , u8 bank , u8 cntr , u8 fxn , u64 * value , bool is_write ) ;
...
position p;
@@
 enum iommu_init_state { IOMMU_START_STATE , IOMMU_IVRS_DETECTED , IOMMU_ACPI_FINISHED , IOMMU_ENABLED , IOMMU_PCI_INIT , IOMMU_INTERRUPTS_EN , IOMMU_DMA_OPS , IOMMU_INITIALIZED , IOMMU_NOT_FOUND , IOMMU_INIT_ERROR , IOMMU_CMDLINE_DISABLED , } cocci_id@p ;
