...
position p;
@@
 struct ovs_ct_limit * cocci_id@p ;
...
position p;
@@
 enum ovs_ct_nat { OVS_CT_NAT = 1 << 0 , OVS_CT_SRC_NAT = 1 << 1 , OVS_CT_DST_NAT = 1 << 2 , } cocci_id@p ;
...
position p;
@@
 struct ovs_conntrack_info * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct ovs_conntrack_info * ct_info ) ;
...
position p;
@@
 const struct nf_conntrack_helper * cocci_id@p ;
...
position p;
@@
 struct inet_skb_parm cocci_id@p ;
...
position p;
@@
 struct ovs_zone_limit cocci_id@p ;
...
position p;
@@
 const struct ovs_conntrack_info * cocci_id@p ;
...
position p;
@@
 struct nf_conncount_data * cocci_id@p ;
...
position p;
@@
 const struct ovs_ct_limit_info * cocci_id@p ;
...
position p;
@@
 struct md_labels {  struct ovs_key_ct_labels value ;  struct ovs_key_ct_labels mask ; } cocci_id@p ;
...
typedef u8,u32,u16;
@@
 struct ovs_conntrack_info {  struct nf_conntrack_helper * helper ;  struct nf_conntrack_zone zone ;  struct nf_conn * ct ;  u8 commit : 1 ;  u8 nat : 3 ;  u8 force : 1 ;  u8 have_eventmask : 1 ;  u16 family ;  u32 eventmask ;  struct md_mark mark ;  struct md_labels labels ;  char timeout [ CTNL_TIMEOUT_NAME_MAX ] ;  struct nf_ct_timeout * nf_ct_timeout ; #if IS_ENABLED(CONFIG_NF_NAT)  struct nf_nat_range2 range ; #endif } cocci_id@p ;
...
position p;
@@
 struct nf_conntrack_ecache * cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ OVS_CT_LIMIT_ATTR_MAX + 1 ] ;
...
typedef u32;
@@
 struct ovs_ct_limit_info {  u32 default_limit ;  struct hlist_head * limits ;  struct nf_conncount_data * data ; } cocci_id@p ;
...
position p;
@@
 const struct nf_nat_range2 * cocci_id@p ;
...
position p;
@@
 struct nf_hook_state cocci_id@p ;
...
position p;
@@
 enum ip_defrag_users cocci_id@p ;
...
typedef u32,u16;
@@
 struct ovs_ct_limit {  struct hlist_node hlist_node ;  struct rcu_head rcu ;  u16 zone ;  u32 limit ; } cocci_id@p ;
...
position p;
@@
 struct inet6_skb_parm cocci_id@p ;
...
position p;
@@
 enum ip6_defrag_users cocci_id@p ;
...
typedef u32;
@@
 struct md_mark {  u32 value ;  u32 mask ; } cocci_id@p ;
...
position p;
@@
 const struct ovs_ct_len_tbl cocci_id@p [ OVS_CT_ATTR_MAX + 1 ] ;
...
position p;
@@
 const struct nf_conntrack_tuple_hash * cocci_id@p ;
...
typedef bool;
@@
 bool cocci_id@p ( const struct ovs_key_ct_labels * labels ) ;
...
position p;
@@
 struct ovs_ct_limit_info * cocci_id@p ;
...
position p;
@@
 struct ovs_ct_len_tbl {  int maxlen ;  int minlen ; } cocci_id@p ;
...
position p;
@@
 struct nf_conn_timeout * cocci_id@p ;
...
position p;
@@
 const int cocci_id@p [ OVS_NAT_ATTR_MAX + 1 ] [ 2 ] ;
