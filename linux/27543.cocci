...
position p;
@@
 struct reada_machine_work * cocci_id@p ;
...
@@
 struct reada_zone {  u64 start ;  u64 end ;  u64 elems ;  struct list_head list ;  spinlock_t lock ;  int locked ;  struct btrfs_device * device ;  struct btrfs_device * devs [ BTRFS_MAX_MIRRORS ] ;  int ndevs ;  struct kref refcnt ; } cocci_id@p ;
...
position p;
@@
 struct reada_extctl cocci_id@p ;
...
position p;
@@
 struct reada_extent * cocci_id@p ;
...
typedef u64,spinlock_t;
@@
 struct reada_extent {  u64 logical ;  struct btrfs_key top ;  struct list_head extctl ;  int refcnt ;  spinlock_t lock ;  struct reada_zone * zones [ BTRFS_MAX_MIRRORS ] ;  int nzones ;  int scheduled ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct btrfs_fs_info * , struct reada_extent * ) ;
...
position p;
@@
 struct reada_machine_work {  struct btrfs_work work ;  struct btrfs_fs_info * fs_info ; } cocci_id@p ;
...
typedef u64;
@@
 struct reada_extctl {  struct list_head list ;  struct reada_control * rc ;  u64 generation ; } cocci_id@p ;
...
typedef u64;
@@
 int cocci_id@p ( struct reada_control * rc , u64 logical , struct btrfs_key * top , u64 generation ) ;
