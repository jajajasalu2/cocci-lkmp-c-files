...
position p;
@@
 int cocci_id@p [ KNL_MAX_CHANNELS ] ;
...
position p;
@@
 const struct pci_id_descr * cocci_id@p ;
...
position p;
@@
 struct pci_id_descr {  int dev_id ;  int optional ;  enum domain dom ; } cocci_id@p ;
...
position p;
@@
 enum type { SANDY_BRIDGE , IVY_BRIDGE , HASWELL , BROADWELL , KNIGHTS_LANDING , } cocci_id@p ;
...
position p;
@@
 const struct pci_id_table * cocci_id@p ;
...
position p;
@@
 const struct sbridge_pvt * cocci_id@p ;
...
position p;
@@
 struct pci_id_table {  const struct pci_id_descr * descr ;  int n_devs_per_imc ;  int n_devs_per_sock ;  int n_imcs_per_sock ;  enum type type ; } cocci_id@p ;
...
position p;
@@
 unsigned int cocci_id@p [ MAX_INTERLEAVE ] ;
...
position p;
@@
 const struct interleave_pkg * cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ KNL_MAX_CHANNELS * 2 ] ;
...
typedef u32;
@@
 struct sbridge_channel {  u32 ranks ;  u32 dimms ; } cocci_id@p ;
...
position p;
@@
 struct knl_pvt {  struct pci_dev * pci_cha [ KNL_MAX_CHAS ] ;  struct pci_dev * pci_channel [ KNL_MAX_CHANNELS ] ;  struct pci_dev * pci_mc0 ;  struct pci_dev * pci_mc1 ;  struct pci_dev * pci_mc0_misc ;  struct pci_dev * pci_mc1_misc ;  struct pci_dev * pci_mc_info ; } cocci_id@p ;
...
position p;
@@
 enum mirroring_mode { NON_MIRRORING , ADDR_RANGE_MIRRORING , FULL_MIRRORING , } cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ KNL_MAX_EDCS * 2 ] ;
...
position p;
@@
 struct interleave_pkg {  unsigned char start ;  unsigned char end ; } cocci_id@p ;
...
typedef u32;
@@
 const u32 cocci_id@p [ MAX_RIR_RANGES ] [ MAX_RIR_WAY ] ;
...
position p;
@@
 enum domain { IMC0 = 0 , IMC1 , SOCK , } cocci_id@p ;
...
typedef u64;
@@
 u64 cocci_id@p [ KNL_MAX_CHANNELS ] ;
...
position p;
@@
 const struct mem_ctl_info * cocci_id@p ;
...
position p;
@@
 struct pci_id_table * cocci_id@p ;
