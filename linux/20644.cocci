...
position p;
@@
 enum { BTS_STATE_STOPPED = 0 , BTS_STATE_INACTIVE , BTS_STATE_ACTIVE , } cocci_id@p ;
...
position p;
@@
 struct bts_phys {  struct page * page ;  unsigned long size ;  unsigned long offset ;  unsigned long displacement ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct bts_buffer * buf , struct perf_output_handle * handle ) ;
...
symbol cocci_id;
position p;
typedef bool,local_t;
@@
 struct bts_buffer {  size_t real_size ;  unsigned int nr_pages ;  unsigned int nr_bufs ;  unsigned int cur_buf ;  bool snapshot ;  local_t data_size ;  local_t head ;  unsigned long end ;  void * * data_pages ;  struct bts_phys buf [ 0 ] ; } cocci_id@p ;
...
position p;
@@
 struct bts_ctx {  struct perf_output_handle handle ;  struct debug_store ds_back ;  int state ; } cocci_id@p ;
