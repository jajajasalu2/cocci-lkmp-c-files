...
symbol cocci_id;
position p;
typedef cpu_stop_fn_t,atomic_t;
@@
 struct multi_stop_data {  cpu_stop_fn_t fn ;  void * data ;  unsigned int num_threads ;  const struct cpumask * active_cpus ;  enum multi_stop_state state ;  atomic_t thread_ack ; } cocci_id@p ;
...
position p;
@@
 struct cpu_stop_done * cocci_id@p ;
...
position p;
@@
 enum multi_stop_state { MULTI_STOP_NONE , MULTI_STOP_PREPARE , MULTI_STOP_DISABLE_IRQ , MULTI_STOP_RUN , MULTI_STOP_EXIT , } cocci_id@p ;
...
position p;
@@
 enum multi_stop_state cocci_id@p ;
...
@@
 struct cpu_stopper {  struct task_struct * thread ;  raw_spinlock_t lock ;  bool enabled ;  struct list_head works ;  struct cpu_stop_work stop_work ; } cocci_id@p ;
...
position p;
@@
 struct multi_stop_data * cocci_id@p ;
...
position p;
@@
 struct cpu_stop_work cocci_id@p ;
...
symbol cocci_id;
position p;
typedef cpu_stop_fn_t;
@@
 cpu_stop_fn_t cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( int cpu , struct task_struct * stop ) ;
...
typedef atomic_t;
@@
 struct cpu_stop_done {  atomic_t nr_todo ;  int ret ;  struct completion completion ; } cocci_id@p ;
...
position p;
@@
 struct smp_hotplug_thread cocci_id@p ;
