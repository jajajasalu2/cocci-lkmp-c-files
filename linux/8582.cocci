...
position p;
@@
 struct sh_eth_rxdesc cocci_id@p ;
...
position p;
@@
 struct net_device_stats * cocci_id@p ( struct net_device * ndev ) ;
...
position p;
@@
 struct sh_eth_cpu_data * cocci_id@p ;
...
position p;
@@
 struct sh_eth_private cocci_id@p ;
...
typedef u16;
@@
 const u16 cocci_id@p [ SH_ETH_MAX_REGISTER_OFFSET ] ;
...
position p;
@@
 struct sh_eth_txdesc cocci_id@p ;
...
position p;
@@
 struct bb_info {  void ( * set_gate ) ( void * addr ) ;  struct mdiobb_ctrl ctrl ;  void * addr ; } cocci_id@p ;
