...
typedef bool;
@@
 struct veth_rq {  struct napi_struct xdp_napi ;  struct net_device * dev ;  struct bpf_prog __rcu * xdp_prog ;  struct xdp_mem_info xdp_mem ;  struct veth_rq_stats stats ;  bool rx_notify_masked ;  struct ptr_ring xdp_ring ;  struct xdp_rxq_info xdp_rxq ; } cocci_id@p ;
...
typedef u64;
@@
 struct veth_rq_stats {  u64 xdp_packets ;  u64 xdp_bytes ;  u64 xdp_drops ;  struct u64_stats_sync syncp ; } cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ VETH_INFO_MAX + 1 ] ;
...
position p;
@@
 const struct veth_rq_stats * cocci_id@p ;
...
position p;
@@
 struct veth_xdp_tx_bq {  struct xdp_frame * q [ VETH_XDP_TX_BULK_SIZE ] ;  unsigned int count ; } cocci_id@p ;
...
position p;
@@
 struct veth_xdp_tx_bq cocci_id@p ;
...
position p;
@@
 struct {  const char string [ ETH_GSTRING_LEN ] ; } cocci_id@p [ ] ;
...
typedef atomic64_t;
@@
 struct veth_priv {  struct net_device __rcu * peer ;  atomic64_t dropped ;  struct bpf_prog * _xdp_prog ;  struct veth_rq * rq ;  unsigned int requested_headroom ; } cocci_id@p ;
...
position p;
@@
 struct veth_rq_stats cocci_id@p ;
...
position p;
@@
 const struct veth_q_stat_desc cocci_id@p [ ] ;
...
position p;
@@
 struct veth_q_stat_desc {  char desc [ ETH_GSTRING_LEN ] ;  size_t offset ; } cocci_id@p ;
