...
position p;
@@
 unsigned long cocci_id@p [ SLAVE + 1 ] ;
...
position p;
@@
 struct {  long rt ;  long master ;  long diff ;  long lat ; } cocci_id@p [ NUM_ROUNDS ] ;
...
position p;
@@
 struct tlb_pending_info {  unsigned long ctx ;  unsigned long nr ;  unsigned long * vaddrs ; } cocci_id@p ;
...
position p;
@@
 struct tlb_pending_info cocci_id@p ;
...
position p;
@@
 void ( * cocci_id@p ) ( struct trap_per_cpu * , int ) ;
...
position p;
@@
 struct hvtramp_descr * cocci_id@p ;
...
position p;
@@
 struct hvtramp_mapping cocci_id@p ;
