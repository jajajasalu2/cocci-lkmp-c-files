...
position p;
@@
 struct psp_memory_training_context * cocci_id@p ;
...
position p;
@@
 struct amdgpu_ttm_tt * cocci_id@p ;
...
position p;
@@
 const struct ttm_place * cocci_id@p ;
...
position p;
@@
 struct amdgpu_copy_mem * cocci_id@p ;
...
position p;
@@
 struct ttm_mem_type_manager * cocci_id@p ;
...
position p;
@@
 struct ttm_bo_device * cocci_id@p ;
...
position p;
@@
 struct dma_resv_list * cocci_id@p ;
...
position p;
@@
 const struct {  char * name ;  const struct file_operations * fops ;  int domain ; } cocci_id@p [ ] ;
...
typedef uint64_t;
@@
 const uint64_t cocci_id@p [ HMM_PFN_VALUE_MAX ] ;
...
position p;
@@
 struct drm_sched_rq * cocci_id@p ;
...
position p;
@@
 struct ttm_backend_func cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u64,uint64_t,uint32_t;
@@
 struct amdgpu_ttm_tt {  struct ttm_dma_tt ttm ;  struct drm_gem_object * gobj ;  u64 offset ;  uint64_t userptr ;  struct task_struct * usertask ;  uint32_t userflags ; #if IS_ENABLED(CONFIG_DRM_AMDGPU_USERPTR)  struct hmm_range * range ; #endif } cocci_id@p ;
...
typedef uint64_t;
@@
 const uint64_t cocci_id@p [ HMM_PFN_FLAG_MAX ] ;
...
position p;
@@
 struct ttm_bo_driver cocci_id@p ;
