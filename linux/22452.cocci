...
position p;
@@
 struct viadev {  unsigned int reg_offset ;  unsigned long port ;  int direction ;  struct snd_pcm_substream * substream ;  int running ;  unsigned int tbl_entries ;  struct snd_dma_buffer table ;  struct snd_via_sg_table * idx_table ;  unsigned int lastpos ;  unsigned int fragsize ;  unsigned int bufsize ;  unsigned int bufsize2 ;  int hwptr_done ;  int in_interrupt ;  int shadow_shift ; } cocci_id@p ;
...
position p;
@@
 struct snd_via_sg_table {  unsigned int offset ;  unsigned int size ; } cocci_id@p ;
...
position p;
@@
 struct snd_ctl_elem_id cocci_id@p ;
...
position p;
@@
 struct via823x_info cocci_id@p [ ] ;
...
position p;
@@
 struct via_rate_lock * cocci_id@p ;
...
position p;
@@
 enum { TYPE_CARD_VIA686 = 1 , TYPE_CARD_VIA8233 , } cocci_id@p ;
...
position p;
@@
 struct via823x_info {  int revision ;  char * name ;  int type ; } cocci_id@p ;
...
position p;
@@
 enum { TYPE_VIA686 , TYPE_VIA8233 , TYPE_VIA8233A , } cocci_id@p ;
...
position p;
@@
 enum { VIA_REG_ ## name ## _STATUS = ( val ) , VIA_REG_ ## name ## _CONTROL = ( val ) + 0x01 , VIA_REG_ ## name ## _TYPE = ( val ) + 0x02 , VIA_REG_ ## name ## _TABLE_PTR = ( val ) + 0x04 , VIA_REG_ ## name ## _CURR_PTR = ( val ) + 0x04 , VIA_REG_ ## name ## _STOP_IDX = ( val ) + 0x08 , VIA_REG_ ## name ## _CURR_COUNT = ( val ) + 0x0c , } cocci_id@p ;
...
typedef spinlock_t;
@@
 struct via_rate_lock {  spinlock_t lock ;  int rate ;  int used ; } cocci_id@p ;
