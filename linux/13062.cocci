...
typedef __le32;
@@
 struct do_cleanup_msg {  __le32 version ;  __le32 command ;  __le32 seq_num ;  __le32 name_len ;  char name [ 32 ] ; } cocci_id@p ;
...
position p;
@@
 struct do_cleanup_msg cocci_id@p ;
...
typedef __le32;
@@
 struct cleanup_done_msg {  __le32 version ;  __le32 response ;  __le32 seq_num ; } cocci_id@p ;
...
typedef u32;
@@
 struct glink_ssr {  struct device * dev ;  struct rpmsg_endpoint * ept ;  struct notifier_block nb ;  u32 seq_num ;  struct completion completion ; } cocci_id@p ;
...
position p;
@@
 struct cleanup_done_msg * cocci_id@p ;
