...
position p;
@@
 struct optee_call_ctx cocci_id@p ;
...
position p;
@@
 struct optee_call_waiter {  struct list_head list_node ;  struct completion c ; } cocci_id@p ;
...
position p;
@@
 union {  struct arm_smccc_res smccc ;  struct optee_smc_disable_shm_cache_result result ; } cocci_id@p ;
...
position p;
@@
 struct optee_call_queue * cocci_id@p ;
...
typedef u64;
@@
 struct {  u64 pages_list [ PAGELIST_ENTRIES_PER_PAGE ] ;  u64 next_page_data ; } * cocci_id@p ;
...
position p;
@@
 struct optee_call_waiter cocci_id@p ;
...
position p;
@@
 struct optee_rpc_param cocci_id@p ;
