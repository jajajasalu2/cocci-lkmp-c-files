...
position p;
@@
 struct rmi_transport_dev * cocci_id@p ;
...
position p;
@@
 const struct rmi_transport_ops cocci_id@p ;
...
typedef u8;
@@
 int cocci_id@p ( struct hid_device * hdev , u8 * report , int len ) ;
...
position p;
@@
 enum rmi_mode_type { RMI_MODE_OFF = 0 , RMI_MODE_ATTN_REPORTS = 1 , RMI_MODE_NO_PACKED_ATTN_REPORTS = 2 , } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,u32,wait_queue_head_t;
@@
 struct rmi_data {  struct mutex page_mutex ;  int page ;  struct rmi_transport_dev xport ;  wait_queue_head_t wait ;  u8 * writeReport ;  u8 * readReport ;  u32 input_report_size ;  u32 output_report_size ;  unsigned long flags ;  struct work_struct reset_work ;  struct hid_device * hdev ;  unsigned long device_flags ;  struct irq_domain * domain ;  int rmi_irq ; } cocci_id@p ;
