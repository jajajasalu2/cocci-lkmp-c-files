...
position p;
@@
 struct joydev_client * cocci_id@p ;
...
position p;
@@
 struct JS_DATA_TYPE cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,__u16,__u8,wait_queue_head_t,spinlock_t;
@@
 struct joydev {  int open ;  struct input_handle handle ;  wait_queue_head_t wait ;  struct list_head client_list ;  spinlock_t client_lock ;  struct mutex mutex ;  struct device dev ;  struct cdev cdev ;  bool exist ;  struct js_corr corr [ ABS_CNT ] ;  struct JS_DATA_SAVE_TYPE glue ;  int nabs ;  int nkey ;  __u16 keymap [ KEY_MAX - BTN_MISC + 1 ] ;  __u16 keypam [ KEY_MAX - BTN_MISC + 1 ] ;  __u8 absmap [ ABS_CNT ] ;  __u8 abspam [ ABS_CNT ] ;  __s16 abs [ ABS_CNT ] ; } cocci_id@p ;
...
typedef spinlock_t;
@@
 struct joydev_client {  struct js_event buffer [ JOYDEV_BUFFER_SIZE ] ;  int head ;  int tail ;  int startup ;  spinlock_t buffer_lock ;  struct fasync_struct * fasync ;  struct joydev * joydev ;  struct list_head node ; } cocci_id@p ;
...
position p;
@@
 struct JS_DATA_SAVE_TYPE_32 cocci_id@p ;
