...
position p;
@@
 struct tcp_metrics_block * cocci_id@p ( const struct inetpeer_addr * saddr , const struct inetpeer_addr * daddr , struct net * net , unsigned int hash ) ;
...
position p;
@@
 struct tcpm_hash_bucket cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCP_METRICS_ATTR_MAX + 1 ] ;
...
position p;
@@
 struct tcp_metrics_block * cocci_id@p ;
...
position p;
@@
 const struct inetpeer_addr * cocci_id@p ;
...
position p;
@@
 struct inetpeer_addr * cocci_id@p ;
...
position p;
@@
 struct tcp_fastopen_metrics cocci_id@p [ 1 ] ;
...
@@
 struct tcp_metrics_block {  struct tcp_metrics_block __rcu * tcpm_next ;  possible_net_t tcpm_net ;  struct inetpeer_addr tcpm_saddr ;  struct inetpeer_addr tcpm_daddr ;  unsigned long tcpm_stamp ;  u32 tcpm_lock ;  u32 tcpm_vals [ TCP_METRIC_MAX_KERNEL + 1 ] ;  struct tcp_fastopen_metrics tcpm_fastopen ;  struct rcu_head rcu_head ; } cocci_id@p ;
...
position p;
@@
 struct tcpm_hash_bucket {  struct tcp_metrics_block __rcu * chain ; } cocci_id@p ;
...
position p;
@@
 enum tcp_metric_index cocci_id@p ;
...
position p;
@@
 struct tcp_fastopen_metrics * cocci_id@p ;
