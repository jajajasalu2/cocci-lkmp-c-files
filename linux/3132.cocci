...
position p;
@@
 struct macio_driver cocci_id@p ;
...
position p;
@@
 struct swim3 {  REG ( data ) ;  REG ( timer ) ;  REG ( error ) ;  REG ( mode ) ;  REG ( select ) ;  REG ( setup ) ;  REG ( control ) ;  REG ( status ) ;  REG ( intr ) ;  REG ( nseek ) ;  REG ( ctrack ) ;  REG ( csect ) ;  REG ( gap3 ) ;  REG ( sector ) ;  REG ( nsect ) ;  REG ( intr_enable ) ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct dbdma_cmd * cp , int cmd , void * buf , int count ) ;
...
position p;
@@
 struct floppy_state cocci_id@p [ MAX_FLOPPIES ] ;
...
position p;
@@
 int cocci_id@p ( struct floppy_state * fs ) ;
...
position p;
@@
 void cocci_id@p ( struct floppy_state * fs , int n ) ;
...
position p;
@@
 struct gendisk * cocci_id@p [ MAX_FLOPPIES ] ;
...
position p;
@@
 struct floppy_state * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct floppy_state * fs ) ;
...
@@
 struct floppy_state {  enum swim_state state ;  struct swim3 __iomem * swim3 ;  struct dbdma_regs __iomem * dma ;  int swim3_intr ;  int dma_intr ;  int cur_cyl ;  int cur_sector ;  int req_cyl ;  int head ;  int req_sector ;  int scount ;  int retries ;  int settle_time ;  int secpercyl ;  int secpertrack ;  int total_secs ;  int write_prot ;  struct dbdma_cmd * dma_cmd ;  int ref_count ;  int expect_cyl ;  struct timer_list timeout ;  int timeout_pending ;  int ejected ;  wait_queue_head_t wait ;  int wanted ;  struct macio_dev * mdev ;  char dbdma_cmd_space [ 5 * sizeof ( struct dbdma_cmd ) ] ;  int index ;  struct request * cur_req ;  struct blk_mq_tag_set tag_set ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct floppy_state * fs , enum swim_state state , int interruptible ) ;
...
position p;
@@
 unsigned int cocci_id@p ( struct gendisk * disk , unsigned int clearing ) ;
