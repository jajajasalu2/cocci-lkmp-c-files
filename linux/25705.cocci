...
position p;
@@
 struct perf_thread_map * cocci_id@p ;
...
position p;
@@
 struct sort_dimension * cocci_id@p ;
...
position p;
@@
 struct perf_session * cocci_id@p ;
...
position p;
@@
 const struct option * cocci_id@p ;
...
position p;
@@
 struct addr_location cocci_id@p ;
...
position p;
@@
 struct callchain_cursor * cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct perf_tool * tool , struct evsel * evsel , struct perf_sample * sample , struct machine * machine ) ;
...
position p;
@@
 struct callchain_cursor_node * cocci_id@p ;
...
position p;
@@
 struct evsel_runtime cocci_id@p ;
...
typedef u64;
@@
 struct work_atom {  struct list_head list ;  enum thread_state state ;  u64 sched_out_time ;  u64 wake_up_time ;  u64 sched_in_time ;  u64 runtime ; } cocci_id@p ;
...
position p;
@@
 struct callchain_node cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u64,pthread_t,sem_t;
@@
 struct task_desc {  unsigned long nr ;  unsigned long pid ;  char comm [ COMM_LEN ] ;  unsigned long nr_events ;  unsigned long curr_event ;  struct sched_atom * * atoms ;  pthread_t thread ;  sem_t sleep_sem ;  sem_t ready_for_work ;  sem_t work_done_sem ;  u64 cpu_usage ; } cocci_id@p ;
...
typedef bool;
@@
 union map_priv {  void * ptr ;  bool color ; } cocci_id@p ;
...
typedef u64,bool;
@@
 struct thread_runtime {  u64 last_time ;  u64 dt_run ;  u64 dt_sleep ;  u64 dt_iowait ;  u64 dt_preempt ;  u64 dt_delay ;  u64 ready_to_run ;  struct stats run_stats ;  u64 total_run_time ;  u64 total_sleep_time ;  u64 total_iowait_time ;  u64 total_preempt_time ;  u64 total_delay_time ;  int last_state ;  char shortname [ 3 ] ;  bool comm_changed ;  u64 migrations ; } cocci_id@p ;
...
position p;
@@
 struct rb_root_cached * cocci_id@p ;
...
position p;
@@
 struct sched_thread_parms * cocci_id@p ;
...
position p;
@@
 struct perf_cpu_map * cocci_id@p ;
...
typedef u64,u32;
@@
 struct evsel_runtime {  u64 * last_time ;  u32 ncpu ; } cocci_id@p ;
...
position p;
@@
 struct perf_time_interval * cocci_id@p ;
...
@@
 struct perf_sched_map { 
	 DECLARE_BITMAP ( comp_cpus_mask , MAX_CPUS ) 
		 ;  int * comp_cpus ;  bool comp ;  struct perf_thread_map * color_pids ;  const char * color_pids_str ;  struct perf_cpu_map * color_cpus ;  const char * color_cpus_str ;  struct perf_cpu_map * cpus ;  const char * cpus_str ; } cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct work_atoms * , struct work_atoms * ) ;
...
position p;
@@
 struct callchain_list * cocci_id@p ;
...
position p;
@@
 enum sched_event_type { SCHED_EVENT_RUN , SCHED_EVENT_SLEEP , SCHED_EVENT_WAKEUP , SCHED_EVENT_MIGRATION , } cocci_id@p ;
...
typedef u64,sem_t;
@@
 struct sched_atom {  enum sched_event_type type ;  int specific_wait ;  u64 timestamp ;  u64 duration ;  unsigned long nr ;  sem_t * wait_sem ;  struct task_desc * wakee ; } cocci_id@p ;
...
position p;
@@
 struct thread_runtime cocci_id@p ;
...
position p;
@@
 struct total_run_stats cocci_id@p ;
...
position p;
@@
 struct trace_sched_handler cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct perf_tool * tool , union perf_event * event , struct evsel * evsel , struct perf_sample * sample , struct machine * machine ) ;
...
position p;
@@
 enum thread_state { THREAD_SLEEPING = 0 , THREAD_WAIT_CPU , THREAD_SCHED_IN , THREAD_IGNORE , } cocci_id@p ;
...
position p;
@@
 struct idle_thread_runtime * cocci_id@p ;
...
typedef sort_fn_t;
@@
 struct sort_dimension {  const char * name ;  sort_fn_t cmp ;  struct list_head list ; } cocci_id@p ;
...
position p;
@@
 const struct evsel_str_handler cocci_id@p [ ] ;
...
typedef u64;
@@
 struct work_atoms {  struct list_head work_list ;  struct thread * thread ;  struct rb_node node ;  u64 max_lat ;  u64 max_lat_at ;  u64 total_lat ;  u64 nb_atoms ;  u64 total_runtime ;  int num_merged ; } cocci_id@p ;
...
position p;
@@
 struct idle_thread_runtime {  struct thread_runtime tr ;  struct thread * last_thread ;  struct rb_root_cached sorted_root ;  struct callchain_root callchain ;  struct callchain_cursor cursor ; } cocci_id@p ;
...
typedef u64;
@@
 struct total_run_stats {  struct perf_sched * sched ;  u64 sched_count ;  u64 task_count ;  u64 total_run_time ; } cocci_id@p ;
...
position p;
@@
 struct sched_thread_parms {  struct task_desc * task ;  struct perf_sched * sched ;  int fd ; } cocci_id@p ;
