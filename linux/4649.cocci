...
position p;
@@
 union {  unsigned short shorts [ 4 ] ;  unsigned long * ptr ; } cocci_id@p ;
...
position p;
@@
 struct lkdtm_list {  struct list_head node ; } cocci_id@p ;
...
position p;
@@
 void ( * cocci_id@p ) ( unsigned long val ) ;
...
position p;
@@
 volatile char cocci_id@p [ REC_STACK_SIZE ] ;
