...
position p;
@@
 int cocci_id@p ( struct zpool * pool , unsigned long handle ) ;
...
typedef pgoff_t;
@@
 struct zswap_entry {  struct rb_node rbnode ;  pgoff_t offset ;  int refcount ;  unsigned int length ;  struct zswap_pool * pool ;  union {  unsigned long handle ;  unsigned long value ; } ; } cocci_id@p ;
...
position p;
@@
 struct zswap_header cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct zswap_pool * pool ) ;
...
position p;
@@
 struct zswap_tree * cocci_id@p [ MAX_SWAPFILES ] ;
...
position p;
@@
 enum zswap_get_swap_ret { ZSWAP_SWAPCACHE_NEW , ZSWAP_SWAPCACHE_EXIST , ZSWAP_SWAPCACHE_FAIL , } cocci_id@p ;
...
position p;
@@
 struct zswap_pool {  struct zpool * zpool ;  struct crypto_comp * __percpu * tfm ;  struct kref kref ;  struct list_head list ;  struct work_struct work ;  struct hlist_node node ;  char tfm_name [ CRYPTO_MAX_ALG_NAME ] ; } cocci_id@p ;
...
typedef swp_entry_t;
@@
 struct zswap_header {  swp_entry_t swpentry ; } cocci_id@p ;
...
position p;
@@
 struct frontswap_ops cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct zswap_pool * pool ) ;
...
typedef spinlock_t;
@@
 struct zswap_tree {  struct rb_root rbroot ;  spinlock_t lock ; } cocci_id@p ;
