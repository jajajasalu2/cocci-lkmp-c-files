...
position p;
@@
 struct shell_cmd {  const char * name ;  int ( * func ) ( char * args ) ; } cocci_id@p ;
...
position p;
@@
 const struct shell_cmd cocci_id@p [ ] ;
...
position p;
@@
 enum { CMD_OK , CMD_ERR , CMD_EX , } cocci_id@p ;
...
position p;
@@
 struct pcap_filehdr * cocci_id@p ;
...
position p;
@@
 struct bpf_regs cocci_id@p [ BPF_MAXINSNS + 1 ] ;
...
typedef int32_t;
@@
 struct pcap_timeval {  int32_t tv_sec ;  int32_t tv_usec ; } cocci_id@p ;
...
typedef uint32_t;
@@
 struct pcap_pkthdr {  struct pcap_timeval ts ;  uint32_t caplen ;  uint32_t len ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef int32_t,uint16_t,uint32_t;
@@
 struct pcap_filehdr {  uint32_t magic ;  uint16_t version_major ;  uint16_t version_minor ;  int32_t thiszone ;  uint32_t sigfigs ;  uint32_t snaplen ;  uint32_t linktype ; } cocci_id@p ;
...
position p;
@@
 struct sock_filter cocci_id@p [ BPF_MAXINSNS + 1 ] ;
...
symbol cocci_id;
position p;
typedef bool,uint16_t,uint32_t;
@@
 struct bpf_regs {  uint32_t A ;  uint32_t X ;  uint32_t M [ BPF_MEMWORDS ] ;  uint32_t R ;  bool Rs ;  uint16_t Pc ; } cocci_id@p ;
