...
position p;
@@
 enum page_cache_mode cocci_id@p ;
...
position p;
@@
 struct map_range cocci_id@p [ NR_RANGE_MR ] ;
...
position p;
@@
 struct range cocci_id@p [ E820_MAX_ENTRIES ] ;
...
typedef uint16_t;
@@
 uint16_t cocci_id@p [ _PAGE_CACHE_MODE_NUM ] ;
...
position p;
@@
 struct map_range {  unsigned long start ;  unsigned long end ;  unsigned page_size_mask ; } cocci_id@p ;
