...
position p;
@@
 struct cpuhp_cpu_state * cocci_id@p ;
...
position p;
@@
 enum cpu_mitigations { CPU_MITIGATIONS_OFF , CPU_MITIGATIONS_AUTO , CPU_MITIGATIONS_AUTO_NOSMT , } cocci_id@p ;
...
position p;
@@
 enum cpu_mitigations cocci_id@p ;
...
position p;
@@
 enum cpuhp_smt_control cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( unsigned int cpu ) ;
...
position p;
@@
 int ( * cocci_id@p ) ( unsigned int cpu , struct hlist_node * node ) ;
...
typedef bool;
@@
 struct cpuhp_step {  const char * name ;  union {  int ( * single ) ( unsigned int cpu ) ;  int ( * multi ) ( unsigned int cpu , struct hlist_node * node ) ; } startup ;  union {  int ( * single ) ( unsigned int cpu ) ;  int ( * multi ) ( unsigned int cpu , struct hlist_node * node ) ; } teardown ;  struct hlist_head list ;  bool cant_stop ;  bool multi_instance ; } cocci_id@p ;
...
position p;
@@
 const unsigned long cocci_id@p [ BITS_PER_LONG + 1 ] [ BITS_TO_LONGS ( NR_CPUS ) ] ;
