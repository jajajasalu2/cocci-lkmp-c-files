...
symbol cocci_id;
position p;
typedef xtregs_opt_t;
@@
 xtregs_opt_t cocci_id@p ;
...
symbol cocci_id;
position p;
typedef xtregs_opt_t,xtregs_coprocessor_t,xtregs_user_t;
@@
 struct rt_sigframe {  struct siginfo info ;  struct ucontext uc ;  struct {  xtregs_opt_t opt ;  xtregs_user_t user ; #if XTENSA_HAVE_COPROCESSORS  xtregs_coprocessor_t cp ; #endif } xtregs ;  unsigned char retcode [ 6 ] ;  unsigned int window [ 4 ] ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef xtregs_user_t;
@@
 xtregs_user_t cocci_id@p ;
