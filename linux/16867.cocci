...
@@
 struct evdev {  int open ;  struct input_handle handle ;  wait_queue_head_t wait ;  struct evdev_client __rcu * grab ;  struct list_head client_list ;  spinlock_t client_lock ;  struct mutex mutex ;  struct device dev ;  struct cdev cdev ;  bool exist ; } cocci_id@p ;
...
position p;
@@
 const size_t cocci_id@p [ EV_CNT ] ;
...
position p;
@@
 const struct input_mt * cocci_id@p ;
...
position p;
@@
 enum input_clock_type cocci_id@p ;
...
position p;
@@
 struct evdev_client * cocci_id@p ;
...
position p;
@@
 const struct input_event * cocci_id@p ;
