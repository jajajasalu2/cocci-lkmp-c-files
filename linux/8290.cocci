...
position p;
@@
 int cocci_id@p ( struct net_device * dev , struct netdev_bpf * xdp ) ;
...
position p;
@@
 struct efx_mcdi_iface * cocci_id@p ;
...
typedef u32;
@@
 int cocci_id@p ( struct net_device * dev , int n , struct xdp_frame * * xdpfs , u32 flags ) ;
...
position p;
@@
 const struct efx_phy_operations cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct efx_nic * efx , struct bpf_prog * prog ) ;
...
position p;
@@
 struct efx_channel * cocci_id@p [ EFX_MAX_CHANNELS ] ;
...
position p;
@@
 struct efx_rx_page_state cocci_id@p ;
...
position p;
@@
 struct msix_entry cocci_id@p [ EFX_MAX_CHANNELS ] ;
...
position p;
@@
 void cocci_id@p ( struct efx_channel * channel ) ;
...
position p;
@@
 const struct efx_channel * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct efx_nic * efx ) ;
...
position p;
@@
 struct efx_link_state * cocci_id@p ;
...
position p;
@@
 enum udp_parsable_tunnel_type cocci_id@p ;
