...
@@
 sense_reason_t cocci_id@p ( struct xcopy_op * xop ) ;
...
position p;
@@
 unsigned char cocci_id@p [ XCOPY_NAA_IEEE_REGEX_LEN ] ;
...
position p;
@@
 struct xcopy_pt_cmd {  struct se_cmd se_cmd ;  struct completion xpt_passthrough_sem ;  unsigned char sense_buffer [ TRANSPORT_SENSE_BUFFER ] ; } cocci_id@p ;
...
position p;
@@
 struct xcopy_pt_cmd cocci_id@p ;
...
position p;
@@
 struct xcopy_dev_search_info * cocci_id@p ;
...
position p;
@@
 struct xcopy_dev_search_info {  const unsigned char * dev_wwn ;  struct se_device * found_dev ; } cocci_id@p ;
