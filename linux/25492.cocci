...
typedef u64;
@@
 struct alloc_func {  u64 start ;  u64 end ;  char * name ; } cocci_id@p ;
...
typedef u64,u32;
@@
 struct alloc_stat {  u64 call_site ;  u64 ptr ;  u64 bytes_req ;  u64 bytes_alloc ;  u64 last_alloc ;  u32 hit ;  u32 pingpong ;  short alloc_cpu ;  struct rb_node node ; } cocci_id@p ;
...
position p;
@@
 const struct alloc_func * cocci_id@p ;
...
typedef sort_fn_t;
@@
 struct sort_dimension {  const char name [ 20 ] ;  sort_fn_t cmp ;  struct list_head list ; } cocci_id@p ;
...
position p;
@@
 enum { KMEM_SLAB , KMEM_PAGE , } cocci_id@p ;
...
position p;
@@
 const struct {  const char * original ;  const char * compact ; } cocci_id@p [ ] ;
...
position p;
@@
 const struct gfp_flag * cocci_id@p ;
...
position p;
@@
 int cocci_id@p [ MAX_PAGE_ORDER ] [ MAX_MIGRATE_TYPES ] ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct evsel * evsel , struct perf_sample * sample ) ;
...
position p;
@@
 struct gfp_flag {  unsigned int flags ;  char * compact_str ;  char * human_readable ; } cocci_id@p ;
...
typedef u64;
@@
 struct page_stat {  struct rb_node node ;  u64 page ;  u64 callsite ;  int order ;  unsigned gfp_flags ;  unsigned migrate_type ;  u64 alloc_bytes ;  u64 free_bytes ;  int nr_alloc ;  int nr_free ; } cocci_id@p ;
