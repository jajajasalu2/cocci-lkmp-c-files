...
position p;
@@
 struct if_spi_packet cocci_id@p ;
...
position p;
@@
 const struct lbs_fw_table cocci_id@p [ ] ;
...
@@
 struct if_spi_card {  struct spi_device * spi ;  struct lbs_private * priv ;  struct libertas_spi_platform_data * pdata ;  u16 card_id ;  u8 card_rev ;  unsigned long prev_xfer_time ;  int use_dummy_writes ;  unsigned long spu_port_delay ;  unsigned long spu_reg_delay ;  struct workqueue_struct * workqueue ;  struct work_struct packet_work ;  struct work_struct resume_work ;  u8 cmd_buffer [ IF_SPI_CMD_BUF_SIZE ] ;  struct list_head cmd_packet_list ;  struct list_head data_packet_list ;  spinlock_t buffer_lock ;  u8 suspended ; } cocci_id@p ;
...
typedef u8;
@@
 u8 cocci_id@p [ HELPER_FW_LOAD_CHUNK_SZ ] ;
