...
typedef refcount_t;
@@
 struct io_wq {  struct io_wqe * * wqes ;  unsigned long state ;  get_work_fn * get_work ;  put_work_fn * put_work ;  struct task_struct * manager ;  struct user_struct * user ;  const struct cred * creds ;  struct mm_struct * mm ;  refcount_t refs ;  struct completion done ; } cocci_id@p ;
...
position p;
@@
 enum { IO_WQ_ACCT_BOUND , IO_WQ_ACCT_UNBOUND , } cocci_id@p ;
...
position p;
@@
 struct io_wq_flush_data * cocci_id@p ;
...
typedef atomic_t;
@@
 struct io_wqe_acct {  unsigned nr_workers ;  unsigned max_workers ;  atomic_t nr_running ; } cocci_id@p ;
...
position p;
@@
 struct io_cb_cancel_data * cocci_id@p ;
...
typedef spinlock_t;
@@
 struct io_wqe {  struct {  spinlock_t lock ;  struct io_wq_work_list work_list ;  unsigned long hash_map ;  unsigned flags ; } ____cacheline_aligned_in_smp ;  int node ;  struct io_wqe_acct acct [ 2 ] ;  struct hlist_nulls_head free_list ;  struct list_head all_list ;  struct io_wq * wq ; } cocci_id@p ;
...
position p;
@@
 struct io_cb_cancel_data {  struct io_wqe * wqe ;  work_cancel_fn * cancel ;  void * caller_data ; } cocci_id@p ;
...
position p;
@@
 enum { IO_WQ_BIT_EXIT = 0 , IO_WQ_BIT_CANCEL = 1 , IO_WQ_BIT_ERROR = 2 , } cocci_id@p ;
...
typedef bool;
@@
 bool ( * cocci_id@p ) ( struct io_worker * , void * ) ;
...
position p;
@@
 enum { IO_WQE_FLAG_STALLED = 1 , } cocci_id@p ;
...
position p;
@@
 enum { IO_WORKER_F_UP = 1 , IO_WORKER_F_RUNNING = 2 , IO_WORKER_F_FREE = 4 , IO_WORKER_F_EXITING = 8 , IO_WORKER_F_FIXED = 16 , IO_WORKER_F_BOUND = 32 , } cocci_id@p ;
...
position p;
@@
 struct io_wq_work_node * cocci_id@p ;
...
position p;
@@
 struct io_wq_flush_data {  struct io_wq_work work ;  struct completion done ; } cocci_id@p ;
