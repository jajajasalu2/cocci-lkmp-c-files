...
typedef u8;
@@
 int cocci_id@p ( struct acpi_ec * ec , u8 * data ) ;
...
symbol cocci_id;
position p;
typedef acpi_handle,u8;
@@
 struct acpi_ec_query_handler {  struct list_head node ;  acpi_ec_query_func func ;  acpi_handle handle ;  void * data ;  u8 query_bit ;  struct kref kref ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct acpi_ec * ec ) ;
...
position p;
@@
 struct acpi_ec_query_handler cocci_id@p ;
...
position p;
@@
 struct acpi_ec_query {  struct transaction transaction ;  struct work_struct work ;  struct acpi_ec_query_handler * handler ; } cocci_id@p ;
...
position p;
@@
 acpi_physical_address cocci_id@p ;
...
typedef acpi_status;
@@
 acpi_status cocci_id@p ( struct acpi_resource * resource , void * context ) ;
...
position p;
@@
 struct acpi_table_ecdt * cocci_id@p ;
...
position p;
@@
 enum { EC_FLAGS_QUERY_ENABLED , EC_FLAGS_QUERY_PENDING , EC_FLAGS_QUERY_GUARDING , EC_FLAGS_EVENT_HANDLER_INSTALLED , EC_FLAGS_EC_HANDLER_INSTALLED , EC_FLAGS_QUERY_METHODS_INSTALLED , EC_FLAGS_STARTED , EC_FLAGS_STOPPED , EC_FLAGS_EVENTS_MASKED , } cocci_id@p ;
...
position p;
@@
 struct acpi_ec_query cocci_id@p ;
...
position p;
@@
 enum ec_command { ACPI_EC_COMMAND_READ = 0x80 , ACPI_EC_COMMAND_WRITE = 0x81 , ACPI_EC_BURST_ENABLE = 0x82 , ACPI_EC_BURST_DISABLE = 0x83 , ACPI_EC_COMMAND_QUERY = 0x84 , } cocci_id@p ;
...
typedef u8;
@@
 struct transaction {  const u8 * wdata ;  u8 * rdata ;  unsigned short irq_count ;  u8 command ;  u8 wi ;  u8 ri ;  u8 wlen ;  u8 rlen ;  u8 flags ; } cocci_id@p ;
