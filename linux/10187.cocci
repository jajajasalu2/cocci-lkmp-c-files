...
@@
 struct fritzcard {  struct list_head list ;  struct pci_dev * pdev ;  char name [ MISDN_MAX_IDLEN ] ;  u8 type ;  u8 ctrlreg ;  u16 irq ;  u32 irqcnt ;  u32 addr ;  spinlock_t lock ;  struct isac_hw isac ;  struct hdlc_hw hdlc [ 2 ] ;  struct bchannel bch [ 2 ] ;  char log [ LOG_SIZE + 1 ] ; } cocci_id@p ;
...
position p;
@@
 enum { AVM_FRITZ_PCI , AVM_FRITZ_PCIV2 , } cocci_id@p ;
...
typedef u32;
@@
 struct hdlc_hw {  union {  u32 ctrl ;  struct hdlc_stat_reg sr ; } ctrl ;  u32 stat ; } cocci_id@p ;
