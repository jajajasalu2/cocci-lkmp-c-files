...
position p;
@@
 enum genl_validate_flags cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( int event , const struct genl_family * family , const struct genl_multicast_group * grp , int grp_id ) ;
...
position p;
@@
 enum netlink_validation cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ CTRL_ATTR_MAX + 1 ] ;
...
position p;
@@
 const struct genl_family * cocci_id@p ;
