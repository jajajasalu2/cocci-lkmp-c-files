...
@@
 struct log_header_core {  uint32_t magic ;  uint32_t version ;  uint64_t nr_regions ; } cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct dm_target * ti ) ;
...
position p;
@@
 struct dm_dirty_log_type * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef region_t,uint32_t;
@@
 struct log_c {  struct dm_target * ti ;  int touched_dirtied ;  int touched_cleaned ;  int flush_failed ;  uint32_t region_size ;  unsigned int region_count ;  region_t sync_count ;  unsigned bitset_uint32_count ;  uint32_t * clean_bits ;  uint32_t * sync_bits ;  uint32_t * recovering_bits ;  int sync_search ;  enum sync { DEFAULTSYNC , NOSYNC , FORCESYNC , } sync ;  struct dm_io_request io_req ;  int log_dev_failed ;  int log_dev_flush_failed ;  struct dm_dev * log_dev ;  struct log_header_core header ;  struct dm_io_region header_location ;  struct log_header_disk * disk_header ; } cocci_id@p ;
...
position p;
@@
 struct log_header_core * cocci_id@p ;
...
position p;
@@
 struct log_header_disk * cocci_id@p ;
