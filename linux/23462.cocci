...
position p;
@@
 struct hdmi_codec_cea_spk_alloc {  const int ca_id ;  unsigned int n_ch ;  unsigned long mask ; } cocci_id@p ;
...
position p;
@@
 enum hdmi_codec_cea_spk_placement { FL = BIT ( 0 ) , FC = BIT ( 1 ) , FR = BIT ( 2 ) , FLC = BIT ( 3 ) , FRC = BIT ( 4 ) , RL = BIT ( 5 ) , RC = BIT ( 6 ) , RR = BIT ( 7 ) , RLC = BIT ( 8 ) , RRC = BIT ( 9 ) , LFE = BIT ( 10 ) , } cocci_id@p ;
...
position p;
@@
 struct hdmi_codec_priv * cocci_id@p ;
...
position p;
@@
 const struct hdmi_codec_cea_spk_alloc cocci_id@p [ ] ;
...
typedef uint8_t;
@@
 struct hdmi_codec_priv {  struct hdmi_codec_pdata hcd ;  uint8_t eld [ MAX_ELD_BYTES ] ;  struct snd_pcm_chmap * chmap_info ;  unsigned int chmap_idx ;  unsigned long busy ;  struct snd_soc_jack * jack ;  unsigned int jack_status ; } cocci_id@p ;
...
position p;
@@
 const struct snd_soc_dai_driver cocci_id@p ;
...
position p;
@@
 struct hdmi_codec_channel_map_table {  unsigned char map ;  unsigned long spk_mask ; } cocci_id@p ;
...
position p;
@@
 enum { DAI_ID_I2S = 0 , DAI_ID_SPDIF , } cocci_id@p ;
