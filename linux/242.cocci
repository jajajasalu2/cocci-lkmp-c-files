...
position p;
@@
 struct event_probe_data * cocci_id@p ;
...
position p;
@@
 struct ftrace_module_file_ops cocci_id@p ;
...
position p;
@@
 enum { FORMAT_HEADER = 1 , FORMAT_FIELD_SEPERATOR = 2 , FORMAT_PRINTFMT = 3 , } cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct trace_seq * s ) ;
...
typedef bool;
@@
 struct event_probe_data {  struct trace_event_file * file ;  unsigned long count ;  int ref ;  bool enable ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct trace_event_call * call ) ;
