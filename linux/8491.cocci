...
position p;
@@
 volatile struct TDRE * cocci_id@p ;
...
typedef u_short;
@@
 struct lancedata {  struct TDRE tx_ring [ TX_RING_SIZE ] ;  struct RDRE rx_ring [ RX_RING_SIZE ] ;  u_short tx_buff [ TX_RING_SIZE ] [ PKT_BUF_SIZE / sizeof ( u_short ) ] ;  u_short rx_buff [ RX_RING_SIZE ] [ PKT_BUF_SIZE / sizeof ( u_short ) ] ; } cocci_id@p ;
...
position p;
@@
 struct ariadne_private cocci_id@p ;
...
position p;
@@
 volatile struct lancedata * cocci_id@p ;
...
position p;
@@
 volatile struct Am79C960 * cocci_id@p ;
...
position p;
@@
 volatile struct RDRE * cocci_id@p ;
