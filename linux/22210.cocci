...
position p;
@@
 void cocci_id@p ( const void * , int ) ;
...
position p;
@@
 struct vmap_block_queue * cocci_id@p ;
...
typedef spinlock_t;
@@
 struct vmap_block_queue {  spinlock_t lock ;  struct list_head free ; } cocci_id@p ;
...
position p;
@@
 struct vfree_deferred {  struct llist_head list ;  struct work_struct wq ; } cocci_id@p ;
...
position p;
@@
 enum fit_type { NOTHING_FIT = 0 , FL_FIT_TYPE = 1 , LE_FIT_TYPE = 2 , RE_FIT_TYPE = 3 , NE_FIT_TYPE = 4 , } cocci_id@p ;
...
position p;
@@
 struct vfree_deferred * cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct page * page ) ;
