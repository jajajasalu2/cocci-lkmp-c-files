...
position p;
@@
 void cocci_id@p ( struct mmc_host * mmc ) ;
...
position p;
@@
 enum dbg_channels { dbg_err = ( 1 << 0 ) , dbg_debug = ( 1 << 1 ) , dbg_info = ( 1 << 2 ) , dbg_irq = ( 1 << 3 ) , dbg_sg = ( 1 << 4 ) , dbg_dma = ( 1 << 5 ) , dbg_pio = ( 1 << 6 ) , dbg_fail = ( 1 << 7 ) , dbg_conf = ( 1 << 8 ) , } cocci_id@p ;
...
position p;
@@
 const struct s3cmci_reg cocci_id@p [ ] ;
...
position p;
@@
 void cocci_id@p ( struct s3cmci_host * host ) ;
...
position p;
@@
 struct s3cmci_reg {  unsigned short addr ;  unsigned char * name ; } cocci_id@p ;
