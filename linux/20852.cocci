...
position p;
@@
 enum { AUDIT_PRE_PAGE_FAULT , AUDIT_POST_PAGE_FAULT , AUDIT_PRE_PTE_WRITE , AUDIT_POST_PTE_WRITE , AUDIT_PRE_SYNC , AUDIT_POST_SYNC , } cocci_id@p ;
...
position p;
@@
 struct kernel_param_ops cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct kvm * kvm , struct list_head * invalid_list ) ;
...
position p;
@@
 struct rmap_iterator {  struct pte_list_desc * desc ;  int pos ; } cocci_id@p ;
...
position p;
@@
 struct kvm_tlb_range * cocci_id@p ;
...
position p;
@@
 struct kvm_page_track_notifier_node * cocci_id@p ;
...
position p;
@@
 struct kvm_mmu_pages * cocci_id@p ;
...
position p;
@@
 struct kvm_mmu_page cocci_id@p ;
...
typedef u64;
@@
 struct pte_list_desc {  u64 * sptes [ PTE_LIST_EXT ] ;  struct pte_list_desc * more ; } cocci_id@p ;
...
typedef u64,hpa_t;
@@
 struct kvm_shadow_walk_iterator {  u64 addr ;  hpa_t shadow_addr ;  u64 * sptep ;  int level ;  unsigned index ; } cocci_id@p ;
...
position p;
@@
 struct shrink_control * cocci_id@p ;
...
position p;
@@
 struct kvm_arch_async_pf cocci_id@p ;
...
position p;
@@
 union kvm_mmu_page_role cocci_id@p ( struct kvm_vcpu * vcpu ) ;
...
position p;
@@
 struct pte_list_desc * cocci_id@p ;
...
position p;
@@
 const union kvm_mmu_page_role cocci_id@p ;
...
position p;
@@
 struct kvm_mmu_pages {  struct mmu_page_and_offset {  struct kvm_mmu_page * sp ;  unsigned int idx ; } page [ KVM_PAGE_ARRAY_NR ] ;  unsigned int nr ; } cocci_id@p ;
...
position p;
@@
 struct mmu_page_path * cocci_id@p ;
...
position p;
@@
 union kvm_mmu_page_role cocci_id@p ;
...
typedef gfn_t;
@@
 struct slot_rmap_walk_iterator {  struct kvm_memory_slot * slot ;  gfn_t start_gfn ;  gfn_t end_gfn ;  int start_level ;  int end_level ;  gfn_t gfn ;  struct kvm_rmap_head * rmap ;  int level ;  struct kvm_rmap_head * end_rmap ; } cocci_id@p ;
...
position p;
@@
 enum { RET_PF_RETRY = 0 , RET_PF_EMULATE = 1 , RET_PF_INVALID = 2 , } cocci_id@p ;
...
position p;
@@
 struct kvm_rmap_head * cocci_id@p ;
...
position p;
@@
 struct kvm_mmu_root_info cocci_id@p ;
...
typedef u64;
@@
 void cocci_id@p ( u64 * sptep , u64 spte ) ;
...
typedef bool;
@@
 bool cocci_id@p ( struct kvm * kvm , struct kvm_mmu_page * sp , struct list_head * invalid_list ) ;
...
position p;
@@
 struct slot_rmap_walk_iterator cocci_id@p ;
...
position p;
@@
 struct kvm_shadow_walk_iterator cocci_id@p ;
...
position p;
@@
 struct rsvd_bits_validate * cocci_id@p ;
...
typedef u64;
@@
 u64 cocci_id@p [ PT64_ROOT_MAX_LEVEL ] ;
...
position p;
@@
 struct rmap_iterator * cocci_id@p ;
...
typedef bool;
@@
 bool ( * cocci_id@p ) ( struct kvm * kvm , struct kvm_rmap_head * rmap_head ) ;
...
position p;
@@
 struct page * cocci_id@p [ PTE_PREFETCH_NUM ] ;
...
typedef u64,u32;
@@
 union split_spte {  struct {  u32 spte_low ;  u32 spte_high ; } ;  u64 spte ; } cocci_id@p ;
...
position p;
@@
 struct mmu_page_path {  struct kvm_mmu_page * parent [ PT64_ROOT_MAX_LEVEL ] ;  unsigned int idx [ PT64_ROOT_MAX_LEVEL ] ; } cocci_id@p ;
...
position p;
@@
 struct kvm_mmu_memory_cache * cocci_id@p ;
...
position p;
@@
 union kvm_mmu_extended_role cocci_id@p ;
