...
position p;
@@
 struct iucv_tty_buffer * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef iucv_state_t;
@@
 enum iucv_state_t { IUCV_DISCONN = 0 , IUCV_CONNECTED = 1 , IUCV_SEVERED = 2 , } cocci_id@p ;
...
typedef tty_state_t;
@@
 enum tty_state_t { TTY_CLOSED = 0 , TTY_OPENED = 1 , } cocci_id@p ;
...
position p;
@@
 struct hvc_iucv_private * cocci_id@p [ MAX_HVC_IUCV_LINES ] ;
...
position p;
@@
 struct hvc_iucv_private cocci_id@p ;
...
position p;
@@
 struct iucv_tty_msg cocci_id@p ;
...
position p;
@@
 struct iucv_tty_buffer {  struct list_head list ;  struct iucv_message msg ;  size_t offset ;  struct iucv_tty_msg * mbuf ; } cocci_id@p ;
