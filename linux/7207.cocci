...
position p;
@@
 struct nvme_loop_iod cocci_id@p ;
...
position p;
@@
 struct nvme_loop_queue {  struct nvmet_cq nvme_cq ;  struct nvmet_sq nvme_sq ;  struct nvme_loop_ctrl * ctrl ;  unsigned long flags ; } cocci_id@p ;
...
position p;
@@
 struct nvme_loop_iod {  struct nvme_request nvme_req ;  struct nvme_command cmd ;  struct nvme_completion cqe ;  struct nvmet_req req ;  struct nvme_loop_queue * queue ;  struct work_struct work ;  struct sg_table sg_table ;  struct scatterlist first_sgl [ ] ; } cocci_id@p ;
...
position p;
@@
 struct nvme_loop_ctrl {  struct nvme_loop_queue * queues ;  struct blk_mq_tag_set admin_tag_set ;  struct list_head list ;  struct blk_mq_tag_set tag_set ;  struct nvme_loop_iod async_event_iod ;  struct nvme_ctrl ctrl ;  struct nvmet_ctrl * target_ctrl ;  struct nvmet_port * port ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct nvmet_ctrl * ctrl ) ;
...
position p;
@@
 enum nvme_loop_queue_flags { NVME_LOOP_Q_LIVE = 0 , } cocci_id@p ;
...
position p;
@@
 struct nvme_loop_ctrl cocci_id@p ;
...
position p;
@@
 struct nvme_loop_queue cocci_id@p ;
