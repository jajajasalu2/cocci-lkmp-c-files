...
position p;
@@
 struct xenvif_queue cocci_id@p ;
...
position p;
@@
 struct pending_tx_info cocci_id@p ;
...
position p;
@@
 struct xenvif_tx_cb * cocci_id@p ;
...
position p;
@@
 const struct ubuf_info * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct xenvif_queue * queue ) ;
...
position p;
@@
 struct xen_netif_tx_request cocci_id@p [ XEN_NETBK_LEGACY_SLOTS_MAX ] ;
...
typedef s8;
@@
 void cocci_id@p ( struct xenvif_queue * queue , struct xen_netif_tx_request * txp , unsigned int extra_count , s8 st ) ;
...
position p;
@@
 struct xenvif_mcast_addr * cocci_id@p ;
...
typedef u8,u16;
@@
 void cocci_id@p ( struct xenvif_queue * queue , u16 pending_idx , u8 status ) ;
...
position p;
@@
 struct gnttab_unmap_grant_ref * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct xenvif_queue * queue ) ;
...
position p;
@@
 const struct xen_netif_ctrl_request * cocci_id@p ;
...
typedef skb_frag_t;
@@
 skb_frag_t cocci_id@p [ MAX_SKB_FRAGS ] ;
...
symbol cocci_id;
position p;
typedef pending_ring_idx_t;
@@
 pending_ring_idx_t cocci_id@p ;
...
position p;
@@
 struct xen_netif_extra_info cocci_id@p [ XEN_NETIF_EXTRA_TYPE_MAX - 1 ] ;
...
typedef u16;
@@
 struct xenvif_tx_cb {  u16 pending_idx ; } cocci_id@p ;
...
position p;
@@
 struct xen_netif_ctrl_request cocci_id@p ;
...
position p;
@@
 struct xen_netif_ctrl_response cocci_id@p ;
