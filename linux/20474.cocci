...
position p;
@@
 struct tboot_mac_region * cocci_id@p ;
...
typedef u8;
@@
 struct sha1_hash {  u8 hash [ SHA1_SIZE ] ; } cocci_id@p ;
...
typedef u32;
@@
 u32 cocci_id@p [ ACPI_S_STATE_COUNT ] ;
...
position p;
@@
 struct sinit_mle_data * cocci_id@p ;
...
position p;
@@
 const struct acpi_table_fadt * cocci_id@p ;
