...
position p;
@@
 enum perf_rapl_events { PERF_RAPL_PP0 = 0 , PERF_RAPL_PKG , PERF_RAPL_RAM , PERF_RAPL_PP1 , PERF_RAPL_PSYS , PERF_RAPL_MAX , NR_RAPL_DOMAINS = PERF_RAPL_MAX , } cocci_id@p ;
...
typedef bool;
@@
 struct rapl_model {  unsigned long events ;  bool apply_quirk ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef ktime_t,raw_spinlock_t;
@@
 struct rapl_pmu {  raw_spinlock_t lock ;  int n_active ;  int cpu ;  struct list_head active_list ;  struct pmu * pmu ;  ktime_t timer_interval ;  struct hrtimer hrtimer ; } cocci_id@p ;
...
position p;
@@
 struct rapl_pmus {  struct pmu pmu ;  unsigned int maxdie ;  struct rapl_pmu * pmus [ ] ; } cocci_id@p ;
