...
position p;
@@
 const struct fw_card_driver cocci_id@p ;
...
position p;
@@
 struct descriptor_buffer cocci_id@p ;
...
position p;
@@
 struct fw_iso_buffer * cocci_id@p ;
...
typedef u8;
@@
 struct driver_data {  u8 inline_data [ 8 ] ;  struct fw_packet * packet ; } cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct context * ctx , struct descriptor * d , struct descriptor * last ) ;
...
symbol cocci_id;
position p;
typedef descriptor_callback_t;
@@
 descriptor_callback_t cocci_id@p ;
...
position p;
@@
 const struct {  unsigned short vendor , device , revision , flags ; } cocci_id@p [ ] ;
...
position p;
@@
 struct fw_iso_packet * cocci_id@p ;
...
@@
 struct ar_context {  struct fw_ohci * ohci ;  struct page * pages [ AR_BUFFERS ] ;  void * buffer ;  struct descriptor * descriptors ;  dma_addr_t descriptors_bus ;  void * pointer ;  unsigned int last_buffer_index ;  u32 regs ;  struct tasklet_struct tasklet ; } cocci_id@p ;
...
position p;
@@
 struct page * cocci_id@p [ AR_BUFFERS + AR_WRAPAROUND_PAGES ] ;
...
typedef u8,u32,u16;
@@
 struct iso_context {  struct fw_iso_context base ;  struct context context ;  void * header ;  size_t header_length ;  unsigned long flushing_completions ;  u32 mc_buffer_bus ;  u16 mc_completed ;  u16 last_timestamp ;  u8 sync ;  u8 tags ; } cocci_id@p ;
...
position p;
@@
 const struct fw_ohci * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u64,__le32,bool,u32,__be32,spinlock_t,dma_addr_t;
@@
 struct fw_ohci {  struct fw_card card ; 
	 __iomem 
		  char * registers ;  int node_id ;  int generation ;  int request_generation ;  unsigned quirks ;  unsigned int pri_req_max ;  u32 bus_time ;  bool bus_time_running ;  bool is_root ;  bool csr_state_setclear_abdicate ;  int n_ir ;  int n_it ;  spinlock_t lock ;  struct mutex phy_reg_mutex ;  void * misc_buffer ;  dma_addr_t misc_buffer_bus ;  struct ar_context ar_request_ctx ;  struct ar_context ar_response_ctx ;  struct context at_request_ctx ;  struct context at_response_ctx ;  u32 it_context_support ;  u32 it_context_mask ;  struct iso_context * it_context_list ;  u64 ir_context_channels ;  u32 ir_context_support ;  u32 ir_context_mask ;  struct iso_context * ir_context_list ;  u64 mc_channels ;  bool mc_allocated ;  __be32 * config_rom ;  dma_addr_t config_rom_bus ;  __be32 * next_config_rom ;  dma_addr_t next_config_rom_bus ;  __be32 next_header ;  __le32 * self_id ;  dma_addr_t self_id_bus ;  struct work_struct bus_reset_work ;  u32 self_id_buffer [ 512 ] ; } cocci_id@p ;
...
typedef dma_addr_t;
@@
 struct descriptor_buffer {  struct list_head list ;  dma_addr_t buffer_bus ;  size_t buffer_size ;  size_t used ;  struct descriptor buffer [ 0 ] ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,u32,descriptor_callback_t;
@@
 struct context {  struct fw_ohci * ohci ;  u32 regs ;  int total_allocation ;  u32 current_bus ;  bool running ;  bool flushing ;  struct list_head buffer_list ;  struct descriptor_buffer * buffer_tail ;  struct descriptor * last ;  struct descriptor * prev ;  int prev_z ;  descriptor_callback_t callback ;  struct tasklet_struct tasklet ; } cocci_id@p ;
