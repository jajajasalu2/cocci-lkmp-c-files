...
typedef spinlock_t;
@@
 struct qcom_qspi {  void __iomem * base ;  struct device * dev ;  struct clk_bulk_data clks [ QSPI_NUM_CLKS ] ;  struct qspi_xfer xfer ;  spinlock_t lock ; } cocci_id@p ;
...
typedef bool;
@@
 struct qspi_xfer {  union {  const void * tx_buf ;  void * rx_buf ; } ;  unsigned int rem_bytes ;  unsigned int buswidth ;  enum qspi_dir dir ;  bool is_last ; } cocci_id@p ;
...
position p;
@@
 enum qspi_dir { QSPI_READ , QSPI_WRITE , } cocci_id@p ;
...
position p;
@@
 enum qspi_clocks { QSPI_CLK_CORE , QSPI_CLK_IFACE , QSPI_NUM_CLKS , } cocci_id@p ;
...
position p;
@@
 const struct qspi_xfer * cocci_id@p ;
