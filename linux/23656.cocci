...
symbol cocci_id;
position p;
typedef phys_addr_t,uint16_t,spinlock_t;
@@
 struct q6asm_dai_rtd {  struct snd_pcm_substream * substream ;  struct snd_compr_stream * cstream ;  struct snd_compr_params codec_param ;  struct snd_dma_buffer dma_buffer ;  spinlock_t lock ;  phys_addr_t phys ;  unsigned int pcm_size ;  unsigned int pcm_count ;  unsigned int pcm_irq_pos ;  unsigned int periods ;  unsigned int bytes_sent ;  unsigned int bytes_received ;  unsigned int copied_total ;  uint16_t bits_per_sample ;  uint16_t source ;  struct audio_client * audio_client ;  uint16_t session_id ;  enum stream_state state ; } cocci_id@p ;
...
position p;
@@
 struct q6asm_dai_rtd * cocci_id@p ;
...
position p;
@@
 enum stream_state { Q6ASM_STREAM_IDLE = 0 , Q6ASM_STREAM_STOPPED , Q6ASM_STREAM_RUNNING , } cocci_id@p ;
...
position p;
@@
 const struct snd_compr_codec_caps cocci_id@p ;
...
position p;
@@
 struct snd_compr_runtime * cocci_id@p ;
...
position p;
@@
 struct snd_dec_flac * cocci_id@p ;
...
position p;
@@
 struct snd_compr_codec_caps * cocci_id@p ;
...
position p;
@@
 struct q6asm_dai_data {  long long int sid ; } cocci_id@p ;
...
position p;
@@
 union snd_codec_options * cocci_id@p ;
...
position p;
@@
 struct q6asm_dai_data * cocci_id@p ;
...
position p;
@@
 struct snd_compr_ops cocci_id@p ;
