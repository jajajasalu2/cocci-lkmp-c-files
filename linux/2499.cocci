...
@@
 struct dm_thin_device {  struct list_head list ;  struct dm_pool_metadata * pmd ;  dm_thin_id id ;  int open_count ;  bool changed : 1 ;  bool aborted_with_changes : 1 ;  uint64_t mapped_blocks ;  uint64_t transaction_id ;  uint32_t creation_time ;  uint32_t snapshotted_time ; } cocci_id@p ;
...
position p;
@@
 dm_pool_pre_commit_fn cocci_id@p ;
...
position p;
@@
 struct thin_disk_superblock cocci_id@p ;
...
position p;
@@
 struct dm_space_map * cocci_id@p ;
...
position p;
@@
 struct dm_btree_info * cocci_id@p ;
...
position p;
@@
 struct disk_device_details cocci_id@p ;
