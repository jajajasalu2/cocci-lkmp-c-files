...
position p;
@@
 const struct pt_address_range {  unsigned long msr_a ;  unsigned long msr_b ;  unsigned int reg_off ; } cocci_id@p [ ] ;
...
position p;
@@
 enum pt_capabilities cocci_id@p ;
...
position p;
@@
 struct topa_page {  struct topa_entry table [ TENTS_PER_PAGE ] ;  struct topa topa ; } cocci_id@p ;
...
typedef u64;
@@
 struct topa {  struct list_head list ;  u64 offset ;  size_t size ;  int last ;  unsigned int z_count ; } cocci_id@p ;
...
typedef u8,u32;
@@
 struct pt_cap_desc {  const char * name ;  u32 leaf ;  u8 reg ;  u32 mask ; } cocci_id@p [ ] ;
