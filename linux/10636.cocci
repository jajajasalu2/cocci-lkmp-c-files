...
position p;
@@
 struct i915_page_directory * const cocci_id@p ;
...
position p;
@@
 struct execlist_ring_context * cocci_id@p ;
...
position p;
@@
 struct workload_thread_param {  struct intel_gvt * gvt ;  int ring_id ; } cocci_id@p ;
...
typedef u64;
@@
 u64 cocci_id@p [ GVT_RING_CTX_NR_PDPS ] ;
...
position p;
@@
 struct intel_gvt_workload_scheduler * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct intel_vgpu_workload * workload ) ;
...
position p;
@@
 struct workload_thread_param * cocci_id@p ;
...
position p;
@@
 struct execlist_ctx_descriptor_format * cocci_id@p ;
...
position p;
@@
 enum intel_gvt_gtt_type cocci_id@p ;
...
position p;
@@
 const struct intel_vgpu_submission_ops * cocci_id@p [ ] ;
