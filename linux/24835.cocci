...
position p;
@@
 char cocci_id@p [ NFLOGGER_NAME_LEN ] ;
...
position p;
@@
 struct ctl_table cocci_id@p [ NFPROTO_NUMPROTO + 1 ] ;
...
position p;
@@
 struct nf_log_buf {  unsigned int count ;  char buf [ S_SIZE + 1 ] ; } cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ NF_LOG_PREFIXLEN ] ;
...
position p;
@@
 const struct nf_logger * cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ NFPROTO_NUMPROTO - NFPROTO_UNSPEC ] [ 3 ] ;
