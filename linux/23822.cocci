...
position p;
@@
 struct choke_skb_cb * cocci_id@p ;
...
position p;
@@
 const struct tc_red_qopt * cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_CHOKE_MAX + 1 ] ;
...
typedef u32;
@@
 struct choke_sched_data {  u32 limit ;  unsigned char flags ;  struct red_parms parms ;  struct red_vars vars ;  struct {  u32 prob_drop ;  u32 prob_mark ;  u32 forced_drop ;  u32 forced_mark ;  u32 pdrop ;  u32 other ;  u32 matched ; } stats ;  unsigned int head ;  unsigned int tail ;  unsigned int tab_mask ;  struct sk_buff * * tab ; } cocci_id@p ;
...
position p;
@@
 const struct choke_sched_data * cocci_id@p ;
...
position p;
@@
 struct choke_sched_data cocci_id@p ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_CHOKE_MAX + 1 ] ;
...
typedef u8,u16;
@@
 struct choke_skb_cb {  u16 classid ;  u8 keys_valid ;  struct flow_keys_digest keys ; } cocci_id@p ;
...
position p;
@@
 const struct red_parms * cocci_id@p ;
...
position p;
@@
 struct tc_choke_xstats cocci_id@p ;
