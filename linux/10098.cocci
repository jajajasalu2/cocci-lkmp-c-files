...
position p;
@@
 struct gtp_net {  struct list_head gtp_dev_list ; } cocci_id@p ;
...
position p;
@@
 struct gtp_pktinfo {  struct sock * sk ;  struct iphdr * iph ;  struct flowi4 fl4 ;  struct rtable * rt ;  struct pdp_ctx * pctx ;  struct net_device * dev ;  __be16 gtph_port ; } cocci_id@p ;
...
position p;
@@
 struct gtp_dev {  struct list_head list ;  struct sock * sk0 ;  struct sock * sk1u ;  struct net_device * dev ;  unsigned int role ;  unsigned int hash_size ;  struct hlist_head * tid_hash ;  struct hlist_head * addr_hash ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct pdp_ctx * pctx ) ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ GTPA_MAX + 1 ] ;
...
position p;
@@
 int cocci_id@p ( struct gtp_dev * gtp , int hsize ) ;
...
position p;
@@
 int cocci_id@p ( struct gtp_dev * gtp , struct nlattr * data [ ] ) ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ IFLA_GTP_MAX + 1 ] ;
...
symbol cocci_id;
position p;
typedef u64,u8,u32,u16,atomic_t;
@@
 struct pdp_ctx {  struct hlist_node hlist_tid ;  struct hlist_node hlist_addr ;  union {  struct {  u64 tid ;  u16 flow ; } v0 ;  struct {  u32 i_tei ;  u32 o_tei ; } v1 ; } u ;  u8 gtp_version ;  u16 af ;  struct in_addr ms_addr_ip4 ;  struct in_addr peer_addr_ip4 ;  struct sock * sk ;  struct net_device * dev ;  atomic_t tx_seq ;  struct rcu_head rcu_head ; } cocci_id@p ;
