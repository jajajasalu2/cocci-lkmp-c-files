...
symbol cocci_id;
position p;
typedef erase_unit_header_t,xfer_info_t,uint32_t,eun_info_t,partition_t,uint16_t;
@@
 struct partition_t {  struct mtd_blktrans_dev mbd ;  uint32_t state ;  uint32_t * VirtualBlockMap ;  uint32_t FreeTotal ;  struct eun_info_t {  uint32_t Offset ;  uint32_t EraseCount ;  uint32_t Free ;  uint32_t Deleted ; } * EUNInfo ;  struct xfer_info_t {  uint32_t Offset ;  uint32_t EraseCount ;  uint16_t state ; } * XferInfo ;  uint16_t bam_index ;  uint32_t * bam_cache ;  uint16_t DataUnits ;  uint32_t BlocksPerUnit ;  erase_unit_header_t header ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef erase_unit_header_t;
@@
 erase_unit_header_t cocci_id@p ;
