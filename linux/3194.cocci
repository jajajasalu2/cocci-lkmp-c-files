...
position p;
@@
 struct tjmax {  const char * id ;  int tjmax ; } cocci_id@p ;
...
typedef bool,u32;
@@
 struct temp_data {  int temp ;  int ttarget ;  int tjmax ;  unsigned long last_updated ;  unsigned int cpu ;  u32 cpu_core_id ;  u32 status_reg ;  int attr_size ;  bool is_pkg_data ;  bool valid ;  struct sensor_device_attribute sd_attrs [ TOTAL_ATTRS ] ;  char attr_name [ TOTAL_ATTRS ] [ CORETEMP_NAME_LENGTH ] ;  struct attribute * attrs [ TOTAL_ATTRS + 1 ] ;  struct attribute_group attr_group ;  struct mutex update_lock ; } cocci_id@p ;
...
position p;
@@
 const struct tjmax_model * cocci_id@p ;
...
position p;
@@
 const struct tjmax_pci cocci_id@p [ ] ;
...
position p;
@@
 const char * const cocci_id@p [ TOTAL_ATTRS ] ;
...
position p;
@@
 ssize_t ( * cocci_id@p [ TOTAL_ATTRS ] ) ( struct device * dev , struct device_attribute * devattr , char * buf ) ;
...
typedef u16;
@@
 struct platform_data {  struct device * hwmon_dev ;  u16 pkg_id ;  struct cpumask cpumask ;  struct temp_data * core_data [ MAX_CORE_DATA ] ;  struct device_attribute name_attr ; } cocci_id@p ;
...
position p;
@@
 struct tjmax_pci {  unsigned int device ;  int tjmax ; } cocci_id@p ;
...
typedef u8;
@@
 struct tjmax_model {  u8 model ;  u8 mask ;  int tjmax ; } cocci_id@p ;
