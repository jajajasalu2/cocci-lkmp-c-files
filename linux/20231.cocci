...
typedef alpha_pmu_t;
@@
 const struct alpha_pmu_t * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef ev67_mapping_t;
@@
 struct ev67_mapping_t {  int config ;  int idx ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u64,alpha_pmu_t;
@@
 struct alpha_pmu_t {  const int * event_map ;  int max_events ;  int num_pmcs ;  int pmc_count_shift [ MAX_HWEVENTS ] ;  unsigned long pmc_count_mask [ MAX_HWEVENTS ] ;  unsigned long pmc_max_period [ MAX_HWEVENTS ] ;  long pmc_left [ 3 ] ;  int ( * check_constraints ) ( struct perf_event * * , unsigned long * , int ) ;  int ( * raw_event_valid ) ( u64 config ) ; } cocci_id@p ;
...
position p;
@@
 enum ev67_pmc_event_type { EV67_CYCLES = 1 , EV67_INSTRUCTIONS , EV67_BCACHEMISS , EV67_MBOXREPLAY , EV67_LAST_ET , } cocci_id@p ;
...
typedef ev67_mapping_t;
@@
 const struct ev67_mapping_t cocci_id@p [ ] ;
...
position p;
@@
 struct cpu_hw_events {  int enabled ;  int n_events ;  int n_added ;  struct perf_event * event [ MAX_HWEVENTS ] ;  unsigned long evtype [ MAX_HWEVENTS ] ;  int current_idx [ MAX_HWEVENTS ] ;  unsigned long config ;  unsigned long idx_mask ; } cocci_id@p ;
