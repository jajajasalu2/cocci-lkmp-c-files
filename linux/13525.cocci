...
position p;
@@
 struct {  signed int x : 14 ; } cocci_id@p ;
...
position p;
@@
 struct msi2500_format cocci_id@p [ ] ;
...
position p;
@@
 struct msi2500_frame_buf * cocci_id@p ;
...
typedef u32;
@@
 struct msi2500_format {  u32 pixelformat ;  u32 buffersize ; } cocci_id@p ;
...
@@
 struct msi2500_dev {  struct device * dev ;  struct video_device vdev ;  struct v4l2_device v4l2_dev ;  struct v4l2_subdev * v4l2_subdev ;  struct spi_master * master ;  struct vb2_queue vb_queue ;  struct list_head queued_bufs ;  spinlock_t queued_bufs_lock ;  struct mutex v4l2_lock ;  struct mutex vb_queue_lock ;  struct usb_device * udev ;  unsigned int f_adc ;  u32 pixelformat ;  u32 buffersize ;  unsigned int num_formats ;  unsigned int isoc_errors ;  unsigned int vb_full ;  struct urb * urbs [ MAX_ISO_BUFS ] ;  struct v4l2_ctrl_handler hdl ;  u32 next_sample ;  u32 sample ;  unsigned long jiffies_next ; } cocci_id@p ;
...
position p;
@@
 struct msi2500_frame_buf {  struct vb2_v4l2_buffer vb ;  struct list_head list ; } cocci_id@p ;
