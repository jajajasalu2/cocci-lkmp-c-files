...
position p;
@@
 struct disasm_line_samples {  double percent ;  struct sym_hist_entry he ; } cocci_id@p ;
...
position p;
@@
 struct annotate_browser cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ SYM_TITLE_MAX_SIZE ] ;
...
typedef bool;
@@
 struct annotate_browser {  struct ui_browser b ;  struct rb_root entries ;  struct rb_node * curr_hot ;  struct annotation_line * selection ;  struct arch * arch ;  struct annotation_options * opts ;  bool searching_backwards ;  char search_bf [ 128 ] ; } cocci_id@p ;
