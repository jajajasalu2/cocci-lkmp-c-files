...
symbol cocci_id;
position p;
typedef dbg_reg_def_t;
@@
 struct dbg_reg_def_t cocci_id@p [ DBG_MAX_REG_NUM ] ;
...
position p;
@@
 struct inactive_task_frame * cocci_id@p ;
...
position p;
@@
 const struct kgdb_arch cocci_id@p ;
...
position p;
@@
 struct hw_breakpoint {  unsigned enabled ;  unsigned long addr ;  int len ;  int type ;  struct perf_event * __percpu * pev ; } cocci_id@p [ HBP_NUM ] ;
