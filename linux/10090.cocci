...
position p;
@@
 void cocci_id@p ( struct asyncppp * ap , unsigned char * data , int len , int inbound ) ;
...
symbol cocci_id;
position p;
typedef u32,u16,spinlock_t,refcount_t;
@@
 struct asyncppp {  struct tty_struct * tty ;  unsigned int flags ;  unsigned int state ;  unsigned int rbits ;  int mru ;  spinlock_t xmit_lock ;  spinlock_t recv_lock ;  unsigned long xmit_flags ;  u32 xaccm [ 8 ] ;  u32 raccm ;  unsigned int bytes_sent ;  unsigned int bytes_rcvd ;  struct sk_buff * tpkt ;  int tpkt_pos ;  u16 tfcs ;  unsigned char * optr ;  unsigned char * olim ;  unsigned long last_xmit ;  struct sk_buff * rpkt ;  int lcp_fcs ;  struct sk_buff_head rqueue ;  struct tasklet_struct tsk ;  refcount_t refcnt ;  struct completion dead ;  struct ppp_channel chan ;  unsigned char obuf [ OBUFSIZE ] ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct asyncppp * ap , const unsigned char * buf , char * flags , int count ) ;
...
position p;
@@
 void cocci_id@p ( struct asyncppp * ap ) ;
...
position p;
@@
 int cocci_id@p ( struct asyncppp * ap ) ;
...
position p;
@@
 int cocci_id@p ( struct ppp_channel * chan , unsigned int cmd , unsigned long arg ) ;
...
position p;
@@
 void cocci_id@p ( unsigned long arg ) ;
