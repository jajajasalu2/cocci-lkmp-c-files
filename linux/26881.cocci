...
position p;
@@
 unsigned long cocci_id@p [ DIV_ROUND_UP ( LZ4_MAX_DISTANCE_PAGES , BITS_PER_LONG ) ] ;
...
position p;
@@
 const struct z_erofs_decompressor * cocci_id@p ;
...
position p;
@@
 struct z_erofs_decompressor cocci_id@p [ ] ;
...
position p;
@@
 const struct z_erofs_decompress_req * cocci_id@p ;
...
typedef u8;
@@
 struct z_erofs_decompressor {  int ( * prepare_destpages ) ( struct z_erofs_decompress_req * rq , struct list_head * pagepool ) ;  int ( * decompress ) ( struct z_erofs_decompress_req * rq , u8 * out ) ;  char * name ; } cocci_id@p ;
...
position p;
@@
 struct page * cocci_id@p [ LZ4_MAX_DISTANCE_PAGES ] ;
