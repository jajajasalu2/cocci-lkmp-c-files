...
position p;
@@
 struct xdp_bulk_queue {  void * q [ CPU_MAP_BULK_SIZE ] ;  struct list_head flush_node ;  struct bpf_cpu_map_entry * obj ;  unsigned int count ; } cocci_id@p ;
...
position p;
@@
 struct bpf_cpu_map {  struct bpf_map map ;  struct bpf_cpu_map_entry * * cpu_map ;  struct list_head __percpu * flush_list ; } cocci_id@p ;
...
@@
 struct bpf_cpu_map_entry {  u32 cpu ;  int map_id ;  u32 qsize ;  struct xdp_bulk_queue __percpu * bulkq ;  struct bpf_cpu_map * cmap ;  struct ptr_ring * queue ;  struct task_struct * kthread ;  struct work_struct kthread_stop_wq ;  atomic_t refcnt ;  struct rcu_head rcu ; } cocci_id@p ;
...
typedef bool;
@@
 int cocci_id@p ( struct xdp_bulk_queue * bq , bool in_napi_ctx ) ;
...
position p;
@@
 struct xdp_bulk_queue * cocci_id@p ;
