...
position p;
@@
 const struct rsnd_regmap_field_conf cocci_id@p [ ] ;
...
typedef phys_addr_t;
@@
 struct rsnd_gen {  struct rsnd_gen_ops * ops ;  void __iomem * base [ RSND_BASE_MAX ] ;  phys_addr_t res [ RSND_BASE_MAX ] ;  struct regmap * regmap [ RSND_BASE_MAX ] ;  struct regmap_field * regs [ REG_MAX ] ;  const char * reg_name [ REG_MAX ] ; } cocci_id@p ;
...
position p;
@@
 struct rsnd_regmap_field_conf {  int idx ;  unsigned int reg_offset ;  unsigned int id_offset ;  const char * reg_name ; } cocci_id@p ;
