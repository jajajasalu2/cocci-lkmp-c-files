...
position p;
@@
 enum plip_packet_state { PLIP_PK_DONE = 0 , PLIP_PK_TRIGGER , PLIP_PK_LENGTH_LSB , PLIP_PK_LENGTH_MSB , PLIP_PK_DATA , PLIP_PK_CHECKSUM , } cocci_id@p ;
...
position p;
@@
 struct net_device * cocci_id@p [ PLIP_MAX ] ;
...
position p;
@@
 int cocci_id@p ( struct net_device * dev , struct ifreq * ifr , int cmd ) ;
...
position p;
@@
 const struct neighbour * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct net_device * , struct net_local * , struct plip_local * , struct plip_local * ) ;
...
position p;
@@
 int cocci_id@p ( struct sk_buff * skb , struct net_device * dev , unsigned short type , const void * daddr , const void * saddr , unsigned len ) ;
...
position p;
@@
 struct plip_local {  enum plip_packet_state state ;  enum plip_nibble_state nibble ;  union {  struct { #if defined(__LITTLE_ENDIAN)  unsigned char lsb ;  unsigned char msb ; #elif defined(__BIG_ENDIAN)  unsigned char msb ;  unsigned char lsb ; #else #error	"Please fix the endianness defines in <asm/byteorder.h>"
 #endif } b ;  unsigned short h ; } length ;  unsigned short byte ;  unsigned char checksum ;  unsigned char data ;  struct sk_buff * skb ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct sk_buff * skb , struct net_device * dev ) ;
...
position p;
@@
 enum plip_nibble_state { PLIP_NB_BEGIN , PLIP_NB_1 , PLIP_NB_2 , } cocci_id@p ;
...
@@
 struct net_local {  struct net_device * dev ;  struct work_struct immediate ;  struct delayed_work deferred ;  struct delayed_work timer ;  struct plip_local snd_data ;  struct plip_local rcv_data ;  struct pardevice * pardev ;  unsigned long trigger ;  unsigned long nibble ;  enum plip_connection_state connection ;  unsigned short timeout_count ;  int is_deferred ;  int port_owner ;  int should_relinquish ;  spinlock_t lock ;  atomic_t kill_timer ;  struct completion killed_timer_cmp ; } cocci_id@p ;
...
position p;
@@
 enum plip_nibble_state * cocci_id@p ;
...
position p;
@@
 enum plip_connection_state { PLIP_CN_NONE = 0 , PLIP_CN_RECEIVE , PLIP_CN_SEND , PLIP_CN_CLOSING , PLIP_CN_ERROR , } cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct net_device * dev , struct net_local * nl , struct plip_local * snd , struct plip_local * rcv ) ;
...
position p;
@@
 const struct in_ifaddr * cocci_id@p ;
...
position p;
@@
 struct parport_driver cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct net_device * dev , struct net_local * nl , struct plip_local * snd , struct plip_local * rcv , int error ) ;
...
position p;
@@
 const struct in_device * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( const struct neighbour * neigh , struct hh_cache * hh , __be16 type ) ;
