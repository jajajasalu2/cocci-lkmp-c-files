...
position p;
@@
 enum vuart_param { PARAM_TX_TRIGGER = 0 , PARAM_RX_TRIGGER = 1 , PARAM_INTERRUPT_MASK = 2 , PARAM_RX_BUF_SIZE = 3 , PARAM_RX_BYTES = 4 , PARAM_TX_BUF_SIZE = 5 , PARAM_TX_BYTES = 6 , PARAM_INTERRUPT_STATUS = 7 , } cocci_id@p ;
...
position p;
@@
 struct vuart_bus_priv {  struct ports_bmp * bmp ;  unsigned int virq ;  struct mutex probe_mutex ;  int use_count ;  struct ps3_system_bus_device * devices [ PORT_COUNT ] ; } cocci_id@p ;
...
position p;
@@
 const struct ports_bmp * cocci_id@p ;
...
position p;
@@
 struct vuart_bus_priv * cocci_id@p ;
...
@@
 struct ps3_vuart_port_priv {  u64 interrupt_mask ;  struct {  spinlock_t lock ;  struct list_head head ; } tx_list ;  struct {  struct ps3_vuart_work work ;  unsigned long bytes_held ;  spinlock_t lock ;  struct list_head head ; } rx_list ;  struct ps3_vuart_stats stats ; } cocci_id@p ;
...
position p;
@@
 struct list_buffer {  struct list_head link ;  const unsigned char * head ;  const unsigned char * tail ;  unsigned long dbg_number ;  unsigned char data [ ] ; } cocci_id@p ;
...
position p;
@@
 enum vuart_interrupt_bit { INTERRUPT_BIT_TX = 0 , INTERRUPT_BIT_RX = 1 , INTERRUPT_BIT_DISCONNECT = 2 , } cocci_id@p ;
...
position p;
@@
 struct ps3_vuart_port_priv cocci_id@p ;
...
position p;
@@
 enum vuart_interrupt_mask { INTERRUPT_MASK_TX = 1 , INTERRUPT_MASK_RX = 2 , INTERRUPT_MASK_DISCONNECT = 4 , } cocci_id@p ;
...
position p;
@@
 struct vuart_triggers * cocci_id@p ;
