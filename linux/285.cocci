...
position p;
@@
 struct process_timer {  struct timer_list timer ;  struct task_struct * task ; } cocci_id@p ;
...
position p;
@@
 struct hlist_head cocci_id@p [ LVL_DEPTH ] ;
...
position p;
@@
 struct process_timer cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct timer_list * timer , void ( * func ) ( struct timer_list * ) , unsigned int flags , const char * name , struct lock_class_key * key ) ;
