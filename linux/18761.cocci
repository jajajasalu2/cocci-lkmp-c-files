...
symbol cocci_id;
position p;
typedef pmdval_t,pteval_t;
@@
 struct cachepolicy {  const char policy [ 16 ] ;  unsigned int cr_mask ;  pmdval_t pmd ;  pteval_t pte ;  pteval_t pte_s2 ; } cocci_id@p ;
...
position p;
@@
 void * ( * cocci_id@p ) ( unsigned long sz ) ;
...
position p;
@@
 void cocci_id@p ( long long offset , unsigned long pgd , void * bdata ) ;
...
typedef pmd_t,pte_t;
@@
 pte_t * ( * cocci_id@p ) ( pmd_t * dir , unsigned long addr ) ;
...
position p;
@@
 const struct mem_type * cocci_id@p ;
