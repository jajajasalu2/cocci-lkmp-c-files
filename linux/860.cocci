...
position p;
@@
 enum mtk_ecc_operation cocci_id@p ;
...
position p;
@@
 struct mtk_ecc_config * cocci_id@p ;
...
position p;
@@
 const struct mtk_ecc_caps cocci_id@p ;
...
typedef u8,u32;
@@
 struct mtk_ecc {  struct device * dev ;  const struct mtk_ecc_caps * caps ;  void __iomem * regs ;  struct clk * clk ;  struct completion done ;  struct mutex lock ;  u32 sectors ;  u8 * eccdata ; } cocci_id@p ;
...
position p;
@@
 enum mtk_ecc_regs { ECC_ENCPAR00 , ECC_ENCIRQ_EN , ECC_ENCIRQ_STA , ECC_DECDONE , ECC_DECIRQ_EN , ECC_DECIRQ_STA , } cocci_id@p ;
...
typedef u8,u32;
@@
 struct mtk_ecc_caps {  u32 err_mask ;  const u8 * ecc_strength ;  const u32 * ecc_regs ;  u8 num_ecc_strength ;  u8 ecc_mode_shift ;  u32 parity_bits ;  int pg_irq_sel ; } cocci_id@p ;
