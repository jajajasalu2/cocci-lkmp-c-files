...
position p;
@@
 struct ov965x_interval cocci_id@p [ ] ;
...
position p;
@@
 struct ov965x_ctrls * cocci_id@p ;
...
typedef u8;
@@
 const u8 cocci_id@p [ NUM_FMT_REGS ] ;
...
typedef u8;
@@
 struct i2c_rv {  u8 addr ;  u8 value ; } cocci_id@p ;
...
typedef u8;
@@
 const u8 cocci_id@p [ NUM_SAT_LEVELS ] [ NUM_SAT_REGS ] ;
...
typedef u8;
@@
 const u8 cocci_id@p [ NUM_BR_LEVELS + 1 ] [ NUM_BR_REGS ] ;
...
typedef u8,u16;
@@
 struct ov965x_framesize {  u16 width ;  u16 height ;  u16 max_exp_lines ;  const u8 * regs ; } cocci_id@p ;
...
position p;
@@
 const struct i2c_rv * cocci_id@p ;
...
position p;
@@
 enum gpio_id { GPIO_PWDN , GPIO_RST , NUM_GPIOS , } cocci_id@p ;
...
position p;
@@
 const struct ov9650_platform_data * cocci_id@p ;
...
position p;
@@
 const struct ov965x_interval * cocci_id@p ;
...
position p;
@@
 const struct ov965x_framesize * cocci_id@p ;
...
typedef u8;
@@
 struct ov965x_interval {  struct v4l2_fract interval ;  struct v4l2_frmsize_discrete size ;  u8 clkrc_div ; } cocci_id@p ;
...
position p;
@@
 const struct ov965x_pixfmt cocci_id@p [ ] ;
...
typedef u8;
@@
 struct ov965x_ctrls {  struct v4l2_ctrl_handler handler ;  struct {  struct v4l2_ctrl * auto_exp ;  struct v4l2_ctrl * exposure ; } ;  struct {  struct v4l2_ctrl * auto_wb ;  struct v4l2_ctrl * blue_balance ;  struct v4l2_ctrl * red_balance ; } ;  struct {  struct v4l2_ctrl * hflip ;  struct v4l2_ctrl * vflip ; } ;  struct {  struct v4l2_ctrl * auto_gain ;  struct v4l2_ctrl * gain ; } ;  struct v4l2_ctrl * brightness ;  struct v4l2_ctrl * saturation ;  struct v4l2_ctrl * sharpness ;  struct v4l2_ctrl * light_freq ;  u8 update ; } cocci_id@p ;
...
typedef u8,u32;
@@
 struct ov965x_pixfmt {  u32 code ;  u32 colorspace ;  u8 tslb_reg ; } cocci_id@p ;
...
typedef u8;
@@
 struct ov965x {  struct v4l2_subdev sd ;  struct media_pad pad ;  enum v4l2_mbus_type bus_type ;  struct gpio_desc * gpios [ NUM_GPIOS ] ;  unsigned long mclk_frequency ;  struct clk * clk ;  struct mutex lock ;  struct regmap * regmap ;  unsigned int exp_row_interval ;  unsigned short id ;  const struct ov965x_framesize * frame_size ;  u8 tslb_reg ;  struct v4l2_mbus_framefmt format ;  struct ov965x_ctrls ctrls ;  const struct ov965x_interval * fiv ;  int streaming ;  int power ;  u8 apply_frame_fmt ; } cocci_id@p ;
