...
typedef bool;
@@
 struct xoadc_variant {  const char name [ 16 ] ;  const struct xoadc_channel * channels ;  bool broken_ratiometric ;  bool prescaling ;  bool second_level_mux ; } cocci_id@p ;
...
position p;
@@
 struct pm8xxx_xoadc {  struct device * dev ;  struct regmap * map ;  const struct xoadc_variant * variant ;  struct regulator * vref ;  unsigned int nchans ;  struct pm8xxx_chan_info * chans ;  struct iio_chan_spec * iio_chans ;  struct vadc_linear_graph graph [ 2 ] ;  struct completion complete ;  struct mutex lock ; } cocci_id@p ;
...
position p;
@@
 struct pm8xxx_chan_info * cocci_id@p ;
...
typedef u8;
@@
 struct xoadc_channel {  const char * datasheet_name ;  u8 pre_scale_mux : 2 ;  u8 amux_channel : 4 ;  const struct vadc_prescale_ratio prescale ;  enum iio_chan_type type ;  enum vadc_scale_fn_type scale_fn_type ;  u8 amux_ip_rsv : 3 ; } cocci_id@p ;
...
position p;
@@
 const struct xoadc_channel * cocci_id@p ;
...
position p;
@@
 const struct xoadc_variant * cocci_id@p ;
...
position p;
@@
 struct pm8xxx_xoadc * cocci_id@p ;
...
position p;
@@
 const struct pm8xxx_chan_info * cocci_id@p ;
...
typedef u8;
@@
 struct pm8xxx_chan_info {  const char * name ;  const struct xoadc_channel * hwchan ;  enum vadc_calibration calibration ;  u8 decimation : 2 ;  u8 amux_ip_rsv : 3 ; } cocci_id@p ;
