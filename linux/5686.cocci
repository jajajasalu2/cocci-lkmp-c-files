...
position p;
@@
 struct ht16k33_fbdev cocci_id@p ;
...
@@
 struct ht16k33_fbdev {  struct fb_info * info ;  uint32_t refresh_rate ;  uint8_t * buffer ;  uint8_t * cache ;  struct delayed_work work ; } cocci_id@p ;
...
position p;
@@
 struct ht16k33_priv * cocci_id@p ;
...
typedef u16;
@@
 u16 cocci_id@p [ HT16K33_MATRIX_KEYPAD_MAX_COLS ] ;
...
position p;
@@
 struct ht16k33_priv {  struct i2c_client * client ;  struct ht16k33_keypad keypad ;  struct ht16k33_fbdev fbdev ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,wait_queue_head_t,uint16_t,uint32_t;
@@
 struct ht16k33_keypad {  struct i2c_client * client ;  struct input_dev * dev ;  uint32_t cols ;  uint32_t rows ;  uint32_t row_shift ;  uint32_t debounce_ms ;  uint16_t last_key_state [ HT16K33_MATRIX_KEYPAD_MAX_COLS ] ;  wait_queue_head_t wait ;  bool stopped ; } cocci_id@p ;
...
position p;
@@
 struct ht16k33_keypad * cocci_id@p ;
...
typedef uint8_t;
@@
 uint8_t cocci_id@p [ HT16K33_MATRIX_LED_MAX_COLS * 2 ] ;
