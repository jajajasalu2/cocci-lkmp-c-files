...
position p;
@@
 struct tipc_bclink_entry {  struct tipc_link * link ;  struct sk_buff_head inputq1 ;  struct sk_buff_head arrvq ;  struct sk_buff_head inputq2 ;  struct sk_buff_head namedq ; } cocci_id@p ;
...
position p;
@@
 struct tipc_sock_conn * cocci_id@p ;
...
typedef u8;
@@
 struct tipc_node * cocci_id@p ( struct net * net , u8 * id ) ;
...
typedef u32;
@@
 struct tipc_node * cocci_id@p ( struct net * net , u32 addr ) ;
...
position p;
@@
 struct tipc_link_entry * cocci_id@p ;
...
@@
 struct tipc_link_entry {  struct tipc_link * link ;  spinlock_t lock ;  u32 mtu ;  struct sk_buff_head inputq ;  struct tipc_media_addr maddr ; } cocci_id@p ;
...
position p;
@@
 struct tipc_bclink_entry * cocci_id@p ;
...
typedef bool;
@@
 void cocci_id@p ( struct tipc_node * n , int bearer_id , bool delete ) ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TIPC_NLA_PROP_MAX + 1 ] ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TIPC_NLA_MON_MAX + 1 ] ;
...
position p;
@@
 void cocci_id@p ( struct tipc_node * n , struct sk_buff_head * inputq ) ;
...
typedef bool;
@@
 bool cocci_id@p ( struct tipc_node * n ) ;
...
typedef u32;
@@
 struct tipc_sock_conn {  u32 port ;  u32 peer_port ;  u32 peer_node ;  struct list_head list ; } cocci_id@p ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TIPC_NLA_NET_MAX + 1 ] ;
...
position p;
@@
 void cocci_id@p ( struct tipc_node * n , int * bearer_id , struct sk_buff_head * xmitq , struct tipc_media_addr * * maddr ) ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TIPC_NLA_NODE_MAX + 1 ] ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TIPC_NLA_LINK_MAX + 1 ] ;
...
position p;
@@
 void cocci_id@p ( struct tipc_node * node ) ;
...
position p;
@@
 enum { TIPC_NOTIFY_NODE_DOWN = ( 1 << 3 ) , TIPC_NOTIFY_NODE_UP = ( 1 << 4 ) , TIPC_NOTIFY_LINK_UP = ( 1 << 6 ) , TIPC_NOTIFY_LINK_DOWN = ( 1 << 7 ) , } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct tipc_node * n , int evt ) ;
...
position p;
@@
 enum { SELF_DOWN_PEER_DOWN = 0xdd , SELF_UP_PEER_UP = 0xaa , SELF_DOWN_PEER_LEAVING = 0xd1 , SELF_UP_PEER_COMING = 0xac , SELF_COMING_PEER_UP = 0xca , SELF_LEAVING_PEER_DOWN = 0x1d , NODE_FAILINGOVER = 0xf0 , NODE_SYNCHING = 0xcc , } cocci_id@p ;
...
position p;
@@
 enum { SELF_ESTABL_CONTACT_EVT = 0xece , SELF_LOST_CONTACT_EVT = 0x1ce , PEER_ESTABL_CONTACT_EVT = 0x9ece , PEER_LOST_CONTACT_EVT = 0x91ce , NODE_FAILOVER_BEGIN_EVT = 0xfbe , NODE_FAILOVER_END_EVT = 0xfee , NODE_SYNCH_BEGIN_EVT = 0xcbe , NODE_SYNCH_END_EVT = 0xcee , } cocci_id@p ;
