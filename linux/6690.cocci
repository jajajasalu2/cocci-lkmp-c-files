...
position p;
@@
 struct sysfs_entry {  char * name ;  struct bin_attribute * attr ;  int type ; } cocci_id@p [ ] ;
...
typedef bool;
@@
 int cocci_id@p ( struct fc_vport * , bool ) ;
...
typedef uint8_t;
@@
 const uint8_t cocci_id@p [ WWN_SIZE ] ;
...
position p;
@@
 enum { NO_ACTION , MODE_CHANGE_ACCEPT , MODE_CHANGE_NO_ACTION , TARGET_STILL_ACTIVE , } cocci_id@p ;
