...
position p;
@@
 struct geos_gpio_attr cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct solos_card * ) ;
...
typedef uint32_t;
@@
 uint32_t cocci_id@p ( struct solos_card * ) ;
...
position p;
@@
 int cocci_id@p ( struct sk_buff * buf ) ;
...
typedef uint32_t;
@@
 struct solos_skb_cb {  struct atm_vcc * vcc ;  uint32_t dma_addr ; } cocci_id@p ;
...
position p;
@@
 struct atm_vcc * cocci_id@p ( struct atm_dev * dev , short vpi , int vci ) ;
...
position p;
@@
 struct geos_gpio_attr {  struct device_attribute attr ;  int offset ; } cocci_id@p ;
...
position p;
@@
 struct solos_skb_cb cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct solos_card * card , int port , struct sk_buff * skb , struct atm_vcc * vcc ) ;
...
typedef pid_t;
@@
 struct solos_param {  struct list_head list ;  pid_t pid ;  int port ;  struct sk_buff * response ; } cocci_id@p ;
...
position p;
@@
 struct pkt_hdr {  __le16 size ;  __le16 vpi ;  __le16 vci ;  __le16 type ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct solos_card * , struct device * ) ;
