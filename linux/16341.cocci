...
position p;
@@
 struct dim_channel * cocci_id@p [ DMA_CHANNELS + 1 ] ;
...
symbol cocci_id;
position p;
typedef dim_ch_state_t;
@@
 struct dim_ch_state_t cocci_id@p ;
...
position p;
@@
 enum dim2_platforms { FSL_MX6 , RCAR_H2 , RCAR_M3 , } cocci_id@p ;
...
position p;
@@
 struct hdm_channel * constcocci_id ;
...
position p;
@@
 enum { MLB_INT_IDX , AHB0_INT_IDX , } cocci_id@p ;
...
position p;
@@
 struct dim2_platform_data {  int ( * enable ) ( struct platform_device * ) ;  void ( * disable ) ( struct platform_device * ) ; } cocci_id@p ;
...
typedef bool,u16;
@@
 struct hdm_channel {  char name [ sizeof "caNNN" ] ;  bool is_initialized ;  struct dim_channel ch ;  u16 * reset_dbr_size ;  struct list_head pending_list ;  struct list_head started_list ;  enum most_channel_direction direction ;  enum most_channel_data_type data_type ; } cocci_id@p ;
...
position p;
@@
 struct dim2_platform_data cocci_id@p [ ] ;
...
position p;
@@
 const struct dim2_platform_data * cocci_id@p ;
...
typedef u8;
@@
 struct {  const char * clock_speed ;  u8 clk_speed ; } cocci_id@p [ ] ;
