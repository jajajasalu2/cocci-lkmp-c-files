...
position p;
@@
 struct vfio_region_gfx_edid * cocci_id@p ;
...
typedef u32;
@@
 const struct mbochs_type {  const char * name ;  u32 mbytes ;  u32 max_x ;  u32 max_y ; } cocci_id@p [ ] ;
...
symbol cocci_id;
position p;
typedef u8,u64,u32,u16,pgoff_t;
@@
 struct mdev_state {  u8 * vconfig ;  u64 bar_mask [ 3 ] ;  u32 memory_bar_mask ;  struct mutex ops_lock ;  struct mdev_device * mdev ;  const struct mbochs_type * type ;  u16 vbe [ VBE_DISPI_INDEX_COUNT ] ;  u64 memsize ;  struct page * * pages ;  pgoff_t pagecount ;  struct vfio_region_gfx_edid edid_regs ;  u8 edid_blob [ 0x400 ] ;  struct list_head dmabufs ;  u32 active_id ;  u32 next_id ; } cocci_id@p ;
...
position p;
@@
 const struct mbochs_type * cocci_id@p ;
...
position p;
@@
 struct vfio_region_info_ext * cocci_id@p ;
...
position p;
@@
 struct mbochs_dmabuf * cocci_id@p ;
...
position p;
@@
 struct vfio_device_gfx_plane_info cocci_id@p ;
...
typedef u64,u32;
@@
 struct mbochs_mode {  u32 drm_format ;  u32 bytepp ;  u32 width ;  u32 height ;  u32 stride ;  u32 __pad ;  u64 offset ;  u64 size ; } cocci_id@p ;
...
position p;
@@
 const char * cocci_id@p [ VBE_DISPI_INDEX_COUNT ] ;
...
symbol cocci_id;
position p;
typedef bool,u32,pgoff_t;
@@
 struct mbochs_dmabuf {  struct mbochs_mode mode ;  u32 id ;  struct page * * pages ;  pgoff_t pagecount ;  struct dma_buf * buf ;  struct mdev_state * mdev_state ;  struct list_head next ;  bool unlinked ; } cocci_id@p ;
...
position p;
@@
 struct vfio_region_info_ext {  struct vfio_region_info base ;  struct vfio_region_info_cap_type type ; } cocci_id@p ;
...
position p;
@@
 const struct mdev_parent_ops cocci_id@p ;
