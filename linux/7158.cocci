...
typedef phys_addr_t;
@@
 struct map_ring_valloc_hvm {  unsigned int idx ;  phys_addr_t phys_addrs [ XENBUS_MAX_RING_GRANTS ] ;  unsigned long addrs [ XENBUS_MAX_RING_GRANTS ] ; } cocci_id@p ;
...
position p;
@@
 struct evtchn_close cocci_id@p ;
...
position p;
@@
 struct evtchn_alloc_unbound cocci_id@p ;
...
position p;
@@
 struct unmap_ring_vfree_hvm cocci_id@p ;
...
position p;
@@
 struct xenbus_map_node * cocci_id@p ;
...
position p;
@@
 struct gnttab_map_grant_ref cocci_id@p [ XENBUS_MAX_RING_GRANTS ] ;
...
position p;
@@
 void ( * cocci_id@p ) ( struct xenbus_watch * , const char * , const char * ) ;
...
@@
 struct xenbus_map_node {  struct list_head next ;  union {  struct {  struct vm_struct * area ; } pv ;  struct {  struct page * pages [ XENBUS_MAX_RING_PAGES ] ;  unsigned long addrs [ XENBUS_MAX_RING_GRANTS ] ;  void * addr ; } hvm ; } ;  grant_handle_t handles [ XENBUS_MAX_RING_GRANTS ] ;  unsigned int nr_handles ; } cocci_id@p ;
...
position p;
@@
 struct gnttab_unmap_grant_ref cocci_id@p [ XENBUS_MAX_RING_GRANTS ] ;
...
position p;
@@
 struct map_ring_valloc_hvm * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct xenbus_device * , int , int , const char * , ... ) ;
...
typedef phys_addr_t;
@@
 phys_addr_t cocci_id@p [ XENBUS_MAX_RING_GRANTS ] ;
...
position p;
@@
 const struct xenbus_ring_ops * cocci_id@p ;
...
position p;
@@
 struct xenbus_watch * cocci_id@p ;
...
position p;
@@
 struct unmap_ring_vfree_hvm {  unsigned int idx ;  unsigned long addrs [ XENBUS_MAX_RING_GRANTS ] ; } cocci_id@p ;
...
typedef pte_t;
@@
 pte_t * cocci_id@p [ XENBUS_MAX_RING_GRANTS ] ;
