...
position p;
@@
 void cocci_id@p ( struct igorplugusb * ir , int cmd ) ;
...
typedef uint8_t;
@@
 struct igorplugusb {  struct rc_dev * rc ;  struct device * dev ;  struct urb * urb ;  struct usb_ctrlrequest request ;  struct timer_list timer ;  uint8_t buf_in [ MAX_PACKET ] ;  char phys [ 64 ] ; } cocci_id@p ;
