...
position p;
@@
 int cocci_id@p ( struct net_device * dev , int ioaddr , int irq , int options ) ;
...
typedef s16,s32;
@@
 struct lance_rx_head {  s32 base ;  s16 buf_length ;  s16 msg_length ; } cocci_id@p ;
...
position p;
@@
 struct net_device * cocci_id@p [ MAX_CARDS ] ;
...
position p;
@@
 struct lance_chip_type {  int id_number ;  const char * name ;  int flags ; } cocci_id@p [ ] ;
...
typedef u8,u32,u16;
@@
 struct lance_init_block {  u16 mode ;  u8 phys_addr [ 6 ] ;  u32 filter [ 2 ] ;  u32 rx_ring ;  u32 tx_ring ; } cocci_id@p ;
...
typedef s16,s32;
@@
 struct lance_tx_head {  s32 base ;  s16 length ;  s16 misc ; } cocci_id@p ;
...
position p;
@@
 struct card {  char id_offset14 ;  char id_offset15 ; } cocci_id@p [ ] ;
...
position p;
@@
 enum { OLD_LANCE = 0 , PCNET_ISA = 1 , PCNET_ISAP = 2 , PCNET_PCI = 3 , PCNET_VLB = 4 , PCNET_PCI_II = 5 , LANCE_UNKNOWN = 6 , } cocci_id@p ;
