...
typedef u16;
@@
 struct riptide_firmware {  u16 ASIC ;  u16 CODEC ;  u16 AUXDSP ;  u16 PROG ; } cocci_id@p ;
...
position p;
@@
 enum FIRMWARE { DATA_REC = 0 , EXT_END_OF_FILE , EXT_SEG_ADDR_REC , EXT_GOTO_CMD_REC , EXT_LIN_ADDR_REC , } cocci_id@p ;
...
typedef u32;
@@
 int cocci_id@p ( struct cmdif * cif , u32 flags , u32 cmd , u32 parm , union cmdret * ret ) ;
...
position p;
@@
 int cocci_id@p ( struct cmdif * cif , unsigned char source , unsigned char sink , unsigned char * a , unsigned char * b ) ;
...
position p;
@@
 union firmware_version {  union cmdret ret ;  struct riptide_firmware firmware ; } cocci_id@p ;
...
position p;
@@
 unsigned char cocci_id@p [ E2SINK_MAX + 1 ] [ 2 ] ;
...
typedef u32;
@@
 struct riptideport {  u32 audio_control ;  u32 audio_status ;  u32 pad [ 2 ] ;  struct cmdport port [ 2 ] ; } cocci_id@p ;
...
position p;
@@
 enum RT_CHANNEL_IDS { M0TX = 0 , M1TX , TAMTX , HSSPKR , PDAC , DSNDTX0 , DSNDTX1 , DSNDTX2 , DSNDTX3 , DSNDTX4 , DSNDTX5 , DSNDTX6 , DSNDTX7 , WVSTRTX , COP3DTX , SPARE , M0RX , HSMIC , M1RX , CLEANRX , MICADC , PADC , COPRX1 , COPRX2 , CHANNEL_ID_COUNTER , } cocci_id@p ;
...
typedef u32;
@@
 struct cmdport {  u32 data1 ;  u32 data2 ;  u32 stat ;  u32 pad [ 5 ] ; } cocci_id@p ;
...
position p;
@@
 enum { SB_CMD = 0 , MODEM_CMD , I2S_CMD0 , I2S_CMD1 , FM_CMD , MAX_CMD , } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct cmdif * cif , struct snd_riptide * chip ) ;
...
typedef u8,u32,u16;
@@
 union cmdret {  u8 retbytes [ 8 ] ;  u16 retwords [ 4 ] ;  u32 retlongs [ 2 ] ; } cocci_id@p ;
...
typedef spinlock_t;
@@
 struct cmdif {  struct riptideport * hwport ;  spinlock_t lock ;  unsigned int cmdcnt ;  unsigned int cmdtime ;  unsigned int cmdtimemax ;  unsigned int cmdtimemin ;  unsigned int errcnt ;  int is_reset ; } cocci_id@p ;
...
position p;
@@
 union firmware_version cocci_id@p [ ] ;
...
position p;
@@
 struct snd_dma_buffer * cocci_id@p ;
...
position p;
@@
 struct snd_ac97_template cocci_id@p ;
...
position p;
@@
 unsigned char cocci_id@p [ E2SINK_MAX ] ;
...
typedef __le32;
@@
 struct sgd {  __le32 dwNextLink ;  __le32 dwSegPtrPhys ;  __le32 dwSegLen ;  __le32 dwStat_Ctl ; } cocci_id@p ;
...
position p;
@@
 struct lbuspath {  unsigned char * noconv ;  unsigned char * stereo ;  unsigned char * mono ; } cocci_id@p ;
...
position p;
@@
 struct snd_ac97_bus_ops cocci_id@p ;
...
position p;
@@
 struct snd_pcm_substream * cocci_id@p [ PLAYBACK_SUBSTREAMS + 1 ] ;
...
position p;
@@
 struct snd_ac97_bus * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct snd_riptide * chip ) ;
...
position p;
@@
 enum LBUS_SINK { LS_SRC_INTERPOLATOR = 0 , LS_SRC_INTERPOLATORM , LS_SRC_DECIMATOR , LS_SRC_DECIMATORM , LS_MIXER_IN , LS_MIXER_GAIN_FUNCTION , LS_SRC_SPLITTER , LS_SRC_MERGER , LS_NONE1 , LS_NONE2 , } cocci_id@p ;
...
position p;
@@
 enum CMDS { GETV = 0x00 , GETC , GUNS , SCID , RMEM = 0x10 , SMEM , WMEM , SDTM , GOTO , SSTR = 0x20 , PSTR , KSTR , KDMA , GPOS , SETF , GSTS , NGPOS , PSEL = 0x30 , PCLR , PLST , RSSV , LSEL , SSRC = 0x40 , SLST , RSRC , SSRB , SDGV = 0x50 , RDGV , DLST , SACR = 0x60 , RACR , ALST , TXAC , RXAC , SI2S = 0x70 , ARM_SETDPLL = 0x72 , } cocci_id@p ;
...
position p;
@@
 const struct lbuspath cocci_id@p ;
