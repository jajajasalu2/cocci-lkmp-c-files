...
typedef u8;
@@
 struct pwm_data {  u8 pwm ;  u8 enable ;  u8 auto_pwm_min ; } cocci_id@p ;
...
position p;
@@
 struct adm1026_data cocci_id@p ;
...
typedef s8,u8;
@@
 struct adm1026_data {  struct i2c_client * client ;  const struct attribute_group * groups [ 3 ] ;  struct mutex update_lock ;  int valid ;  unsigned long last_reading ;  unsigned long last_config ;  u8 in [ 17 ] ;  u8 in_max [ 17 ] ;  u8 in_min [ 17 ] ;  s8 temp [ 3 ] ;  s8 temp_min [ 3 ] ;  s8 temp_max [ 3 ] ;  s8 temp_tmin [ 3 ] ;  s8 temp_crit [ 3 ] ;  s8 temp_offset [ 3 ] ;  u8 fan [ 8 ] ;  u8 fan_min [ 8 ] ;  u8 fan_div [ 8 ] ;  struct pwm_data pwm1 ;  u8 vrm ;  u8 analog_out ;  long alarms ;  long alarm_mask ;  long gpio ;  long gpio_mask ;  u8 gpio_config [ 17 ] ;  u8 config1 ;  u8 config2 ;  u8 config3 ; } cocci_id@p ;
