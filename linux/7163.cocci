...
position p;
@@
 enum shutdown_state { SHUTDOWN_INVALID = - 1 , SHUTDOWN_POWEROFF = 0 , SHUTDOWN_SUSPEND = 2 , SHUTDOWN_HALT = 4 , } cocci_id@p ;
...
position p;
@@
 struct suspend_info cocci_id@p ;
...
position p;
@@
 enum shutdown_state cocci_id@p ;
...
position p;
@@
 struct suspend_info {  int cancelled ; } cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ FEATURE_PATH_SIZE ] ;
...
position p;
@@
 struct shutdown_handler cocci_id@p [ ] ;
