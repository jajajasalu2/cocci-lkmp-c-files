...
typedef u32;
@@
 int cocci_id@p ( struct ata_link * link , unsigned int sc_reg , u32 * val ) ;
...
position p;
@@
 struct nv_adma_port_priv * cocci_id@p ;
...
position p;
@@
 struct nv_host_priv * cocci_id@p ;
...
position p;
@@
 struct ata_bmdma_prd * cocci_id@p ;
...
@@
 struct nv_pi_priv {  irq_handler_t irq_handler ;  struct scsi_host_template * sht ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct ata_host * host ) ;
...
typedef u8,__le64;
@@
 struct nv_adma_cpb {  u8 resp_flags ;  u8 reserved1 ;  u8 ctl_flags ;  u8 len ;  u8 tag ;  u8 next_cpb_idx ;  __le16 reserved2 ;  __le16 tf [ 12 ] ;  struct nv_adma_prd aprd [ 5 ] ;  __le64 next_aprd ;  __le64 reserved3 ; } cocci_id@p ;
...
typedef u32;
@@
 struct defer_queue {  u32 defer_bits ;  unsigned int head ;  unsigned int tail ;  unsigned int tag [ ATA_MAX_QUEUE ] ; } cocci_id@p ;
...
position p;
@@
 struct ata_eh_context * cocci_id@p ;
...
position p;
@@
 enum ncq_saw_flag_list { ncq_saw_d2h = ( 1U << 0 ) , ncq_saw_dmas = ( 1U << 1 ) , ncq_saw_sdb = ( 1U << 2 ) , ncq_saw_backout = ( 1U << 3 ) , } cocci_id@p ;
...
position p;
@@
 enum nv_host_type { GENERIC , NFORCE2 , NFORCE3 = NFORCE2 , CK804 , ADMA , MCP5x , SWNCQ , } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef __le32,u8,__le64;
@@
 struct nv_adma_prd {  __le64 addr ;  __le32 len ;  u8 flags ;  u8 packet_len ;  __le16 reserved ; } cocci_id@p ;
...
position p;
@@
 unsigned char cocci_id@p [ ATA_ID_PROD_LEN + 1 ] ;
...
typedef u16;
@@
 void cocci_id@p ( struct ata_port * ap , u16 fis ) ;
...
position p;
@@
 struct nv_host_priv {  unsigned long type ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct ata_port * ap , struct ata_taskfile * tf ) ;
...
symbol cocci_id;
position p;
typedef u64,u8,dma_addr_t;
@@
 struct nv_adma_port_priv {  struct nv_adma_cpb * cpb ;  dma_addr_t cpb_dma ;  struct nv_adma_prd * aprd ;  dma_addr_t aprd_dma ;  void __iomem * ctl_block ;  void __iomem * gen_block ;  void __iomem * notifier_clear_block ;  u64 adma_dma_mask ;  u8 flags ;  int last_issue_ncq ; } cocci_id@p ;
...
@@
 struct nv_swncq_port_priv {  struct ata_bmdma_prd * prd ;  dma_addr_t prd_dma ;  void __iomem * sactive_block ;  void __iomem * irq_block ;  void __iomem * tag_block ;  u32 qc_active ;  unsigned int last_issue_tag ;  struct defer_queue defer_queue ;  u32 dhfis_bits ;  u32 dmafis_bits ;  u32 sdbfis_bits ;  unsigned int ncq_flags ; } cocci_id@p ;
...
position p;
@@
 struct nv_swncq_port_priv * cocci_id@p ;
...
position p;
@@
 enum nv_adma_regbits { CMDEND = ( 1 << 15 ) , WNB = ( 1 << 14 ) , IGN = ( 1 << 13 ) , CS1n = ( 1 << ( 4 + 8 ) ) , DA2 = ( 1 << ( 2 + 8 ) ) , DA1 = ( 1 << ( 1 + 8 ) ) , DA0 = ( 1 << ( 0 + 8 ) ) , } cocci_id@p ;
