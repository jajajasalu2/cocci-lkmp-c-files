...
position p;
@@
 void cocci_id@p ( struct hvcs_partner_info * pi , struct hvcs_struct * hvcsd ) ;
...
position p;
@@
 const struct ktermios cocci_id@p ;
...
position p;
@@
 struct hvcs_partner_info * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct hvcs_struct * hvcsd ) ;
...
position p;
@@
 int cocci_id@p ( struct vio_dev * dev ) ;
...
position p;
@@
 void cocci_id@p ( struct hvcs_struct * hvcsd ) ;
...
position p;
@@
 int cocci_id@p ( struct vio_dev * dev , const struct vio_device_id * id ) ;
...
@@
 struct hvcs_struct {  struct tty_port port ;  spinlock_t lock ;  unsigned int index ;  int todo_mask ;  char buffer [ HVCS_BUFF_LEN ] ;  int chars_in_buffer ;  int connected ;  uint32_t p_unit_address ;  uint32_t p_partition_ID ;  char p_location_code [ HVCS_CLC_LENGTH + 1 ] ;  struct list_head next ;  struct vio_dev * vdev ; } cocci_id@p ;
