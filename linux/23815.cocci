...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_ACT_BPF_MAX + 1 ] ;
...
position p;
@@
 const struct tcf_bpf_cfg * cocci_id@p ;
...
typedef bool,u16;
@@
 struct tcf_bpf_cfg {  struct bpf_prog * filter ;  struct sock_filter * bpf_ops ;  const char * bpf_name ;  u16 bpf_num_ops ;  bool is_ebpf ; } cocci_id@p ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_ACT_BPF_MAX + 1 ] ;
...
position p;
@@
 const struct tcf_bpf * cocci_id@p ;
