...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_FW_MAX + 1 ] ;
...
typedef u32;
@@
 struct fw_head {  u32 mask ;  struct fw_filter __rcu * ht [ HTSIZE ] ;  struct rcu_head rcu ; } cocci_id@p ;
...
typedef u32;
@@
 struct fw_filter {  struct fw_filter __rcu * next ;  u32 id ;  struct tcf_result res ;  int ifindex ;  struct tcf_exts exts ;  struct tcf_proto * tp ;  struct rcu_work rwork ; } cocci_id@p ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_FW_MAX + 1 ] ;
