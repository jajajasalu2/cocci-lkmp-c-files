...
position p;
@@
 volatile const unsigned long * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef wait_queue_head_t,sector_t,spinlock_t;
@@
 struct drbd_bitmap {  struct page * * bm_pages ;  spinlock_t bm_lock ;  unsigned int n_bitmap_hints ;  unsigned int al_bitmap_hints [ AL_UPDATES_PER_TRANSACTION ] ;  unsigned long bm_set ;  unsigned long bm_bits ;  size_t bm_words ;  size_t bm_number_of_pages ;  sector_t bm_dev_capacity ;  struct mutex bm_change ;  wait_queue_head_t bm_io_wait ;  enum bm_flag bm_flags ;  char * bm_why ;  struct task_struct * bm_task ; } cocci_id@p ;
