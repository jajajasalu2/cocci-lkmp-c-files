...
position p;
@@
 struct writeback_struct cocci_id@p ;
...
position p;
@@
 struct writeback_list * cocci_id@p ;
...
typedef atomic_t;
@@
 struct io_notify {  struct dm_writecache * wc ;  struct completion c ;  atomic_t count ; } cocci_id@p ;
...
position p;
@@
 struct writeback_list {  struct list_head list ;  size_t size ; } cocci_id@p ;
...
position p;
@@
 struct wc_memory_entry cocci_id@p ;
...
typedef __le64;
@@
 struct wc_memory_entry {  __le64 original_sector ;  __le64 seq_count ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct dm_writecache * wc , struct dm_dev * dev ) ;
...
@@
 struct wc_memory_superblock {  union {  struct {  __le32 magic ;  __le32 version ;  __le32 block_size ;  __le32 pad ;  __le64 n_blocks ;  __le64 seq_count ; } ;  __le64 padding [ 8 ] ; } ;  struct wc_memory_entry entries [ 0 ] ; } cocci_id@p ;
...
position p;
@@
 struct copy_struct {  struct list_head endio_entry ;  struct dm_writecache * wc ;  struct wc_entry * e ;  unsigned n_entries ;  int error ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,uint64_t,mempool_t,raw_spinlock_t,atomic_t,sector_t;
@@
 struct dm_writecache {  struct mutex lock ;  struct list_head lru ;  union {  struct list_head freelist ;  struct {  struct rb_root freetree ;  struct wc_entry * current_free ; } ; } ;  struct rb_root tree ;  size_t freelist_size ;  size_t writeback_size ;  size_t freelist_high_watermark ;  size_t freelist_low_watermark ;  unsigned uncommitted_blocks ;  unsigned autocommit_blocks ;  unsigned max_writeback_jobs ;  int error ;  unsigned long autocommit_jiffies ;  struct timer_list autocommit_timer ;  struct wait_queue_head freelist_wait ;  atomic_t bio_in_progress [ 2 ] ;  struct wait_queue_head bio_in_progress_wait [ 2 ] ;  struct dm_target * ti ;  struct dm_dev * dev ;  struct dm_dev * ssd_dev ;  sector_t start_sector ;  void * memory_map ;  uint64_t memory_map_size ;  size_t metadata_sectors ;  size_t n_blocks ;  uint64_t seq_count ;  void * block_start ;  struct wc_entry * entries ;  unsigned block_size ;  unsigned char block_size_bits ;  bool pmem_mode : 1 ;  bool writeback_fua : 1 ;  bool overwrote_committed : 1 ;  bool memory_vmapped : 1 ;  bool high_wm_percent_set : 1 ;  bool low_wm_percent_set : 1 ;  bool max_writeback_jobs_set : 1 ;  bool autocommit_blocks_set : 1 ;  bool autocommit_time_set : 1 ;  bool writeback_fua_set : 1 ;  bool flush_on_suspend : 1 ;  unsigned writeback_all ;  struct workqueue_struct * writeback_wq ;  struct work_struct writeback_work ;  struct work_struct flush_work ;  struct dm_io_client * dm_io ;  raw_spinlock_t endio_list_lock ;  struct list_head endio_list ;  struct task_struct * endio_thread ;  struct task_struct * flush_thread ;  struct bio_list flush_list ;  struct dm_kcopyd_client * dm_kcopyd ;  unsigned long * dirty_bitmap ;  unsigned dirty_bitmap_size ;  struct bio_set bio_set ;  mempool_t copy_pool ; } cocci_id@p ;
...
position p;
@@
 struct wc_memory_superblock * cocci_id@p ;
...
position p;
@@
 struct dm_writecache cocci_id@p ;
...
position p;
@@
 struct writeback_struct {  struct list_head endio_entry ;  struct dm_writecache * wc ;  struct wc_entry * * wc_list ;  unsigned wc_list_n ;  struct wc_entry * wc_list_inline [ WB_LIST_INLINE ] ;  struct bio bio ; } cocci_id@p ;
