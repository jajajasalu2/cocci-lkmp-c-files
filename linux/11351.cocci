...
position p;
@@
 enum ttm_caching_state cocci_id@p ;
...
position p;
@@
 struct device_pools {  struct list_head pools ;  struct device * dev ;  struct dma_pool * pool ; } cocci_id@p ;
...
position p;
@@
 struct ttm_pool_manager cocci_id@p ;
...
symbol cocci_id;
position p;
typedef gfp_t,spinlock_t;
@@
 struct dma_pool {  struct list_head pools ;  enum pool_type type ;  spinlock_t lock ;  struct list_head free_list ;  struct device * dev ;  unsigned size ;  unsigned npages_free ;  unsigned npages_in_use ;  unsigned long nfrees ;  unsigned long nrefills ;  gfp_t gfp_flags ;  char name [ 13 ] ;  char dev_name [ 64 ] ; } cocci_id@p ;
...
position p;
@@
 struct page * cocci_id@p [ NUM_PAGES_TO_ALLOC ] ;
...
position p;
@@
 struct device_pools cocci_id@p ;
...
position p;
@@
 struct ttm_pool_opts {  unsigned alloc_size ;  unsigned max_size ;  unsigned small ; } cocci_id@p ;
...
typedef dma_addr_t;
@@
 struct dma_page {  struct list_head page_list ;  unsigned long vaddr ;  struct page * p ;  dma_addr_t dma ; } cocci_id@p ;
...
position p;
@@
 enum pool_type { IS_UNDEFINED = 0 , IS_WC = 1 << 1 , IS_UC = 1 << 2 , IS_CACHED = 1 << 3 , IS_DMA32 = 1 << 4 , IS_HUGE = 1 << 5 , } cocci_id@p ;
...
position p;
@@
 struct ttm_pool_manager {  struct mutex lock ;  struct list_head pools ;  struct ttm_pool_opts options ;  unsigned npools ;  struct shrinker mm_shrink ;  struct kobject kobj ; } cocci_id@p ;
