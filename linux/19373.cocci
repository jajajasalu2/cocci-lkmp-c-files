...
position p;
@@
 struct xtensa_pmu_events {  struct perf_event * event [ XCHAL_NUM_PERF_COUNTERS ] ;  unsigned long used_mask [ BITS_TO_LONGS ( XCHAL_NUM_PERF_COUNTERS ) ] ; } cocci_id@p ;
...
position p;
@@
 struct xtensa_pmu_events * cocci_id@p ;
...
typedef u32;
@@
 const u32 cocci_id@p [ ] [ C ( OP_MAX ) ] [ C ( RESULT_MAX ) ] ;
