...
position p;
@@
 struct mpc_dma_desc * cocci_id@p ;
...
position p;
@@
 struct mpc_dma_chan * cocci_id@p ;
...
typedef dma_addr_t;
@@
 struct mpc_dma_desc {  struct dma_async_tx_descriptor desc ;  struct mpc_dma_tcd * tcd ;  dma_addr_t tcd_paddr ;  int error ;  struct list_head node ;  int will_access_peripheral ; } cocci_id@p ;
...
@@
 struct mpc_dma {  struct dma_device dma ;  struct tasklet_struct tasklet ;  struct mpc_dma_chan channels [ MPC_DMA_CHANNELS ] ;  struct mpc_dma_regs __iomem * regs ;  struct mpc_dma_tcd __iomem * tcd ;  int irq ;  int irq2 ;  uint error_status ;  int is_mpc8308 ;  spinlock_t error_status_lock ; } cocci_id@p ;
...
@@
 struct mpc_dma_chan {  struct dma_chan chan ;  struct list_head free ;  struct list_head prepared ;  struct list_head queued ;  struct list_head active ;  struct list_head completed ;  struct mpc_dma_tcd * tcd ;  dma_addr_t tcd_paddr ;  dma_addr_t src_per_paddr ;  u32 src_tcd_nunits ;  u8 swidth ;  dma_addr_t dst_per_paddr ;  u32 dst_tcd_nunits ;  u8 dwidth ;  spinlock_t lock ; } cocci_id@p ;
