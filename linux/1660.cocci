...
position p;
@@
 enum asahi_compass_chipset { AK8975 , AK8963 , AK09911 , AK09912 , AK_MAX_TYPE , } cocci_id@p ;
...
position p;
@@
 enum asahi_compass_chipset cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,wait_queue_head_t;
@@
 struct ak8975_data {  struct i2c_client * client ;  const struct ak_def * def ;  struct mutex lock ;  u8 asa [ 3 ] ;  long raw_to_gauss [ 3 ] ;  int eoc_gpio ;  int eoc_irq ;  wait_queue_head_t data_ready_queue ;  unsigned long flags ;  u8 cntl_cache ;  struct iio_mount_matrix orientation ;  struct regulator * vdd ;  struct regulator * vid ; } cocci_id@p ;
...
position p;
@@
 enum ak_ctrl_reg_mask { ST1_DRDY , ST2_HOFL , ST2_DERR , CNTL_MODE , MASK_END , } cocci_id@p ;
...
position p;
@@
 const struct ak8975_data * cocci_id@p ;
...
typedef u8,u16;
@@
 struct ak_def {  enum asahi_compass_chipset type ;  long ( * raw_to_gauss ) ( u16 data ) ;  u16 range ;  u8 ctrl_regs [ REGS_END ] ;  u8 ctrl_masks [ MASK_END ] ;  u8 ctrl_modes [ MODE_END ] ;  u8 data_regs [ 3 ] ; } cocci_id@p ;
...
position p;
@@
 const struct ak8975_platform_data * cocci_id@p ;
...
position p;
@@
 const struct iio_poll_func * cocci_id@p ;
...
position p;
@@
 enum ak_ctrl_mode { POWER_DOWN , MODE_ONCE , SELF_TEST , FUSE_ROM , MODE_END , } cocci_id@p ;
...
position p;
@@
 const struct ak_def cocci_id@p [ AK_MAX_TYPE ] ;
...
position p;
@@
 const struct ak_def * cocci_id@p ;
...
position p;
@@
 enum ak_ctrl_reg_addr { ST1 , ST2 , CNTL , ASA_BASE , MAX_REGS , REGS_END , } cocci_id@p ;
