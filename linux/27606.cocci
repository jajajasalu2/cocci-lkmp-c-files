...
position p;
@@
 struct bio_vec cocci_id@p [ DIO_INLINE_BIO_VECS ] ;
...
position p;
@@
 struct bd_holder_disk * cocci_id@p ;
...
position p;
@@
 struct bd_holder_disk {  struct list_head list ;  struct gendisk * disk ;  int refcnt ; } cocci_id@p ;
...
position p;
@@
 struct bdev_inode {  struct block_device bdev ;  struct inode vfs_inode ; } cocci_id@p ;
...
position p;
@@
 void ( * cocci_id@p ) ( struct block_device * , void * ) ;
...
@@
 struct blkdev_dio {  union {  struct kiocb * iocb ;  struct task_struct * waiter ; } ;  size_t size ;  atomic_t ref ;  bool multi_bio : 1 ;  bool should_dirty : 1 ;  bool is_sync : 1 ;  struct bio bio ; } cocci_id@p ;
