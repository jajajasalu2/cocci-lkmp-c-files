...
symbol cocci_id;
position p;
typedef bool,spinlock_t,refcount_t;
@@
 struct kcov {  refcount_t refcount ;  spinlock_t lock ;  enum kcov_mode mode ;  unsigned int size ;  void * area ;  struct task_struct * t ;  bool remote ;  unsigned int remote_size ;  int sequence ; } cocci_id@p ;
...
typedef u64;
@@
 struct kcov_remote {  u64 handle ;  struct kcov * kcov ;  struct hlist_node hnode ; } cocci_id@p ;
...
position p;
@@
 struct kcov_remote_area * cocci_id@p ;
...
position p;
@@
 struct kcov_remote_arg cocci_id@p ;
...
position p;
@@
 struct kcov_remote_area {  struct list_head list ;  unsigned int size ; } cocci_id@p ;
