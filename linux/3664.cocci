...
position p;
@@
 struct scmi_chan_info {  struct mbox_client cl ;  struct mbox_chan * chan ;  void __iomem * payload ;  struct device * dev ;  struct scmi_handle * handle ; } cocci_id@p ;
...
position p;
@@
 struct scmi_msg_hdr * cocci_id@p ;
...
typedef spinlock_t;
@@
 struct scmi_xfers_info {  struct scmi_xfer * xfer_block ;  unsigned long * xfer_alloc_table ;  spinlock_t xfer_lock ; } cocci_id@p ;
...
position p;
@@
 const struct scmi_desc * cocci_id@p ;
...
typedef u8;
@@
 struct scmi_info {  struct device * dev ;  const struct scmi_desc * desc ;  struct scmi_revision_info version ;  struct scmi_handle handle ;  struct scmi_xfers_info tx_minfo ;  struct idr tx_idr ;  struct idr rx_idr ;  u8 * protocols_imp ;  struct list_head node ;  int users ; } cocci_id@p ;
...
position p;
@@
 struct scmi_xfers_info * cocci_id@p ;
...
position p;
@@
 const struct scmi_handle * cocci_id@p ;
...
position p;
@@
 struct scmi_chan_info * cocci_id@p ;
...
position p;
@@
 enum scmi_error_codes { SCMI_SUCCESS = 0 , SCMI_ERR_SUPPORT = - 1 , SCMI_ERR_PARAMS = - 2 , SCMI_ERR_ACCESS = - 3 , SCMI_ERR_ENTRY = - 4 , SCMI_ERR_RANGE = - 5 , SCMI_ERR_BUSY = - 6 , SCMI_ERR_COMMS = - 7 , SCMI_ERR_GENERIC = - 8 , SCMI_ERR_HARDWARE = - 9 , SCMI_ERR_PROTOCOL = - 10 , SCMI_ERR_MAX , } cocci_id@p ;
...
position p;
@@
 const struct scmi_chan_info * cocci_id@p ;
...
position p;
@@
 struct scmi_desc {  int max_rx_timeout_ms ;  int max_msg ;  int max_msg_size ; } cocci_id@p ;
