...
typedef bool;
@@
 struct outqueue_entry {  bool inactive ;  struct tipc_event evt ;  struct list_head list ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct net * net , struct tipc_event * evt ) ;
...
position p;
@@
 void cocci_id@p ( struct tipc_conn * con , struct tipc_subscr * s ) ;
...
position p;
@@
 struct outqueue_entry cocci_id@p ;
...
typedef spinlock_t;
@@
 struct tipc_topsrv {  struct idr conn_idr ;  spinlock_t idr_lock ;  int idr_in_use ;  struct net * net ;  struct work_struct awork ;  struct workqueue_struct * rcv_wq ;  struct workqueue_struct * send_wq ;  struct socket * listener ;  char name [ TIPC_SERVER_NAME_LEN ] ; } cocci_id@p ;
...
typedef spinlock_t;
@@
 struct tipc_conn {  struct kref kref ;  int conid ;  struct socket * sock ;  unsigned long flags ;  struct tipc_topsrv * server ;  struct list_head sub_list ;  spinlock_t sub_lock ;  struct work_struct rwork ;  struct list_head outqueue ;  spinlock_t outqueue_lock ;  struct work_struct swork ; } cocci_id@p ;
...
position p;
@@
 struct tipc_subscription * cocci_id@p ;
