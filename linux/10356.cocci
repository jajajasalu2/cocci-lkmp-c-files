...
position p;
@@
 struct tda998x_priv cocci_id@p ;
...
position p;
@@
 const struct hdmi_audio_infoframe * cocci_id@p ;
...
typedef u8;
@@
 struct tda998x_audio_settings {  const struct tda998x_audio_route * route ;  struct hdmi_audio_infoframe cea ;  unsigned int sample_rate ;  u8 status [ 5 ] ;  u8 ena_ap ;  u8 i2s_format ;  u8 cts_n ; } cocci_id@p ;
...
typedef u8;
@@
 u8 cocci_id@p [ MAX_WRITE_RANGE_BUF + 1 ] ;
...
position p;
@@
 const struct tda998x_audio_settings * cocci_id@p ;
...
position p;
@@
 struct tda998x_audio_settings * cocci_id@p ;
...
position p;
@@
 enum { AUDIO_ROUTE_I2S , AUDIO_ROUTE_SPDIF , AUDIO_ROUTE_NUM , } cocci_id@p ;
...
typedef u8;
@@
 u8 cocci_id@p [ MAX_WRITE_RANGE_BUF ] ;
...
symbol cocci_id;
position p;
typedef u8,bool,u16,wait_queue_head_t;
@@
 struct tda998x_priv {  struct i2c_client * cec ;  struct i2c_client * hdmi ;  struct mutex mutex ;  u16 rev ;  u8 cec_addr ;  u8 current_page ;  bool is_on ;  bool supports_infoframes ;  bool sink_has_audio ;  enum hdmi_quantization_range rgb_quant_range ;  u8 vip_cntrl_0 ;  u8 vip_cntrl_1 ;  u8 vip_cntrl_2 ;  unsigned long tmds_clock ;  struct tda998x_audio_settings audio ;  struct platform_device * audio_pdev ;  struct mutex audio_mutex ;  struct mutex edid_mutex ;  wait_queue_head_t wq_edid ;  volatile int wq_edid_wait ;  struct work_struct detect_work ;  struct timer_list edid_delay_timer ;  wait_queue_head_t edid_delay_waitq ;  bool edid_delay_active ;  struct drm_encoder encoder ;  struct drm_bridge bridge ;  struct drm_connector connector ;  u8 audio_port_enable [ AUDIO_ROUTE_NUM ] ;  struct tda9950_glue cec_glue ;  struct gpio_desc * calib ;  struct cec_notifier * cec_notify ; } cocci_id@p ;
...
position p;
@@
 const struct tda998x_encoder_params * cocci_id@p ;
...
position p;
@@
 const struct tda998x_audio_route cocci_id@p [ AUDIO_ROUTE_NUM ] ;
...
typedef u8;
@@
 struct tda998x_audio_route {  u8 ena_aclk ;  u8 mux_ap ;  u8 aip_clksel ; } cocci_id@p ;
