...
typedef u32;
@@
 const struct {  const char * name ;  u32 vsync_irq ;  u32 framedone_irq ;  u32 sync_lost_irq ;  struct dispc_reg_field reg_desc [ DISPC_MGR_FLD_NUM ] ; } cocci_id@p [ ] ;
...
position p;
@@
 const struct color_conv_coef * cocci_id@p ;
...
position p;
@@
 unsigned long cocci_id@p ( enum omap_plane plane ) ;
...
position p;
@@
 unsigned long cocci_id@p ( enum omap_channel channel ) ;
...
position p;
@@
 enum omap_dss_signal_edge cocci_id@p ;
...
position p;
@@
 enum omap_dss_signal_level cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,u32,spinlock_t,irq_handler_t;
@@
 struct {  struct platform_device * pdev ;  void __iomem * base ;  int irq ;  irq_handler_t user_handler ;  void * user_data ;  unsigned long core_clk_rate ;  unsigned long tv_pclk_rate ;  u32 fifo_size [ DISPC_MAX_NR_FIFOS ] ;  int fifo_assignment [ DISPC_MAX_NR_FIFOS ] ;  bool ctx_valid ;  u32 ctx [ DISPC_SZ_REGS / sizeof ( u32 ) ] ;  const struct dispc_features * feat ;  bool is_enabled ;  struct regmap * syscon_pol ;  u32 syscon_pol_offset ;  spinlock_t control_lock ; } cocci_id@p ;
...
position p;
@@
 enum omap_color_mode cocci_id@p ;
