...
position p;
@@
 void ( * cocci_id@p ) ( const struct cpumask * , unsigned int ) ;
...
position p;
@@
 enum ipi_msg_type { IPI_WAKEUP , IPI_TIMER , IPI_RESCHEDULE , IPI_CALL_FUNC , IPI_CPU_STOP , IPI_IRQ_WORK , IPI_COMPLETION , IPI_CPU_BACKTRACE , } cocci_id@p ;
...
position p;
@@
 const struct smp_operations * cocci_id@p ;
...
position p;
@@
 struct smp_operations cocci_id@p ;
...
position p;
@@
 struct secondary_data cocci_id@p ;
