...
typedef bool;
@@
 struct bitmap_iterator {  unsigned long * stats_bitmap ;  unsigned int count ;  unsigned int iterator ;  bool advance_array ; } cocci_id@p ;
...
position p;
@@
 const struct mlx4_en_priv * cocci_id@p ;
...
position p;
@@
 struct bitmap_iterator * cocci_id@p ;
...
position p;
@@
 struct ptys2ethtool_config cocci_id@p [ MLX4_LINK_MODES_SZ ] ;
...
position p;
@@
 enum ethtool_report cocci_id@p ;
...
position p;
@@
 enum ethtool_report { SUPPORTED = 0 , ADVERTISED = 1 , } cocci_id@p ;
...
typedef u32;
@@
 struct ptys2ethtool_config {  __ETHTOOL_DECLARE_LINK_MODE_MASK ( supported ) ;  __ETHTOOL_DECLARE_LINK_MODE_MASK ( advertised ) ;  u32 speed ; } cocci_id@p ;
