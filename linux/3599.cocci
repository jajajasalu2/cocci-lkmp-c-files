...
position p;
@@
 struct tee_client_driver cocci_id@p ;
...
position p;
@@
 struct tee_param cocci_id@p [ MAX_TEE_PARAM_ARRY_MEMB ] ;
...
position p;
@@
 struct tee_bnxt_fw_private cocci_id@p ;
...
typedef u32;
@@
 struct tee_bnxt_fw_private {  struct device * dev ;  struct tee_context * ctx ;  u32 session_id ;  struct tee_shm * fw_shm_pool ; } cocci_id@p ;
...
position p;
@@
 enum ta_cmd { TA_CMD_BNXT_FASTBOOT = 0 , TA_CMD_BNXT_COPY_COREDUMP = 3 , } cocci_id@p ;
