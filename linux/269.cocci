...
position p;
@@
 struct delayed_uprobe * cocci_id@p ;
...
@@
 struct xol_area {  wait_queue_head_t wq ;  atomic_t slot_count ;  unsigned long * bitmap ;  struct vm_special_mapping xol_mapping ;  struct page * pages [ 2 ] ;  unsigned long vaddr ; } cocci_id@p ;
...
position p;
@@
 struct map_info {  struct map_info * next ;  struct mm_struct * mm ;  unsigned long vaddr ; } cocci_id@p ;
...
position p;
@@
 struct return_instance * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef uprobe_opcode_t;
@@
 uprobe_opcode_t * cocci_id@p ;
...
position p;
@@
 struct page_vma_mapped_walk cocci_id@p ;
...
position p;
@@
 struct mutex cocci_id@p [ UPROBES_HASH_SZ ] ;
...
symbol cocci_id;
position p;
typedef loff_t,refcount_t;
@@
 struct uprobe {  struct rb_node rb_node ;  refcount_t ref ;  struct rw_semaphore register_rwsem ;  struct rw_semaphore consumer_rwsem ;  struct list_head pending_list ;  struct uprobe_consumer * consumers ;  struct inode * inode ;  loff_t offset ;  loff_t ref_ctr_offset ;  unsigned long flags ;  struct arch_uprobe arch ; } cocci_id@p ;
...
position p;
@@
 struct delayed_uprobe {  struct list_head list ;  struct uprobe * uprobe ;  struct mm_struct * mm ; } cocci_id@p ;
