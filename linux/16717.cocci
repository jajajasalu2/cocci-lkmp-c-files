...
position p;
@@
 struct pxa27x_keypad * cocci_id@p ;
...
position p;
@@
 const struct pxa27x_keypad_platform_data * cocci_id@p ;
...
typedef uint32_t;
@@
 uint32_t cocci_id@p [ MAX_MATRIX_KEY_COLS ] ;
...
typedef uint32_t;
@@
 struct pxa27x_keypad {  const struct pxa27x_keypad_platform_data * pdata ;  struct clk * clk ;  struct input_dev * input_dev ;  void __iomem * mmio_base ;  int irq ;  unsigned short keycodes [ MAX_KEYPAD_KEYS ] ;  int rotary_rel_code [ 2 ] ;  unsigned int row_shift ;  uint32_t matrix_key_state [ MAX_MATRIX_KEY_COLS ] ;  uint32_t direct_key_state ;  unsigned int direct_key_mask ; } cocci_id@p ;
