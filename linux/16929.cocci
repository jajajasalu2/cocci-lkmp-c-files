...
typedef u16;
@@
 struct goodix_chip_data {  u16 config_addr ;  int config_len ;  int ( * check_config ) ( struct goodix_ts_data * , const struct firmware * ) ; } cocci_id@p ;
...
typedef u16;
@@
 struct goodix_ts_data {  struct i2c_client * client ;  struct input_dev * input_dev ;  const struct goodix_chip_data * chip ;  struct touchscreen_properties prop ;  unsigned int max_touch_num ;  unsigned int int_trigger_type ;  struct regulator * avdd28 ;  struct regulator * vddio ;  struct gpio_desc * gpiod_int ;  struct gpio_desc * gpiod_rst ;  u16 id ;  u16 version ;  const char * cfg_name ;  struct completion firmware_loading_complete ;  unsigned long irq_flags ;  unsigned int contact_size ; } cocci_id@p ;
...
typedef u8;
@@
 u8 cocci_id@p [ GOODIX_CONFIG_MAX_LENGTH ] ;
...
position p;
@@
 const struct goodix_chip_data cocci_id@p ;
...
position p;
@@
 struct goodix_ts_data cocci_id@p ;
...
typedef u8;
@@
 u8 cocci_id@p [ 1 + GOODIX_MAX_CONTACT_SIZE * GOODIX_MAX_CONTACTS ] ;
...
position p;
@@
 int cocci_id@p ( struct goodix_ts_data * ts , const struct firmware * cfg ) ;
