...
typedef u32;
@@
 struct i2c_pnx_algo_data {  void __iomem * ioaddr ;  struct i2c_pnx_mif mif ;  int last ;  struct clk * clk ;  struct i2c_adapter adapter ;  int irq ;  u32 timeout ; } cocci_id@p ;
...
position p;
@@
 struct i2c_pnx_algo_data * cocci_id@p ;
...
typedef u8;
@@
 struct i2c_pnx_mif {  int ret ;  int mode ;  struct completion complete ;  struct timer_list timer ;  u8 * buf ;  int len ;  int order ; } cocci_id@p ;
...
position p;
@@
 enum { rw_bit = 1 << 0 , start_bit = 1 << 8 , stop_bit = 1 << 9 , } cocci_id@p ;
