...
typedef uint32_t;
@@
 const struct rdr_tbl_ent * cocci_id@p ( uint32_t rdr_num ) ;
...
position p;
@@
 const struct rdr_tbl_ent cocci_id@p [ ] ;
...
@@
 struct rdr_tbl_ent {  uint16_t width ;  uint8_t num_words ;  uint8_t write_control ; } cocci_id@p ;
...
typedef uint64_t;
@@
 uint64_t cocci_id@p [ MAX_RDR_WORDS ] ;
...
typedef uint32_t;
@@
 int cocci_id@p ( uint32_t rdr_num ) ;
...
typedef uint32_t;
@@
 int cocci_id@p ( uint32_t * image_ptr ) ;
...
typedef uint64_t;
@@
 int cocci_id@p ( uint64_t * memaddr ) ;
