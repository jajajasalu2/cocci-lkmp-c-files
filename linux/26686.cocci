...
typedef __u64;
@@
 struct record {  __u64 timestamp ;  struct datarec total ;  struct datarec * cpu ; } cocci_id@p ;
...
position p;
@@
 struct datarec cocci_id@p [ nr_cpus ] ;
...
position p;
@@
 enum { REDIR_SUCCESS = 0 , REDIR_ERROR = 1 , } cocci_id@p ;
...
position p;
@@
 const char * cocci_id@p [ XDP_ACTION_MAX ] ;
...
position p;
@@
 struct stats_record {  struct record_u64 xdp_redirect [ REDIR_RES_MAX ] ;  struct record_u64 xdp_exception [ XDP_ACTION_MAX ] ;  struct record xdp_cpumap_kthread ;  struct record xdp_cpumap_enqueue [ MAX_CPUS ] ;  struct record xdp_devmap_xmit ; } cocci_id@p ;
...
position p;
@@
 struct stats_record * * cocci_id@p ;
...
typedef __u64;
@@
 struct record_u64 {  __u64 timestamp ;  struct u64rec total ;  struct u64rec * cpu ; } cocci_id@p ;
...
position p;
@@
 struct u64rec cocci_id@p [ nr_cpus ] ;
...
typedef __u64;
@@
 struct u64rec {  __u64 processed ; } cocci_id@p ;
...
typedef __u64;
@@
 struct datarec {  __u64 processed ;  __u64 dropped ;  __u64 info ;  __u64 err ; } cocci_id@p ;
...
position p;
@@
 const char * cocci_id@p [ REDIR_RES_MAX ] ;
