...
@@
 struct pmu_private {  struct list_head list ;  int rb_get ;  int rb_put ;  struct rb_entry {  unsigned short len ;  unsigned char data [ 16 ] ; } rb_buf [ RB_SIZE ] ;  wait_queue_head_t wait ;  spinlock_t lock ; #if defined(CONFIG_INPUT_ADBHID) && defined(CONFIG_PMAC_BACKLIGHT)  int backlight_locker ; #endif } cocci_id@p ;
...
position p;
@@
 struct proc_dir_entry * cocci_id@p [ PMU_MAX_BATTERIES ] ;
...
position p;
@@
 const char * cocci_id@p [ NUM_IRQ_STATS ] ;
...
position p;
@@
 void cocci_id@p ( unsigned char * data , int len ) ;
...
position p;
@@
 const struct platform_suspend_ops cocci_id@p ;
...
position p;
@@
 const struct adb_driver cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct adb_request * req , int sync ) ;
...
position p;
@@
 struct pmu_battery_info cocci_id@p [ PMU_MAX_BATTERIES ] ;
...
position p;
@@
 volatile enum pmu_state { uninitialized = 0 , idle , sending , intack , reading , reading_intr , locked , } cocci_id@p ;
...
position p;
@@
 void ( * cocci_id@p ) ( struct adb_request * ) ;
...
position p;
@@
 int cocci_id@p ( struct adb_request * req ) ;
...
position p;
@@
 volatile enum int_data_state { int_data_empty , int_data_fill , int_data_ready , int_data_flush , } cocci_id@p [ 2 ] ;
...
position p;
@@
 unsigned int cocci_id@p [ NUM_IRQ_STATS ] ;
