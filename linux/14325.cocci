...
position p;
@@
 const struct dibx000_bandwidth_config * cocci_id@p ;
...
position p;
@@
 enum timeout_mode { SYMBOL_DEPENDENT_OFF = 0 , SYMBOL_DEPENDENT_ON , } cocci_id@p ;
...
position p;
@@
 struct linear_segments cocci_id@p [ ] ;
...
position p;
@@
 int cocci_id@p ( struct dvb_frontend * fe , enum fe_status * stat ) ;
...
position p;
@@
 enum dibx000_adc_states cocci_id@p ;
...
position p;
@@
 enum dib8000_autosearch_step { AS_START = 0 , AS_SEARCHING_FFT , AS_SEARCHING_GUARD , AS_DONE = 100 , } cocci_id@p ;
...
typedef u32;
@@
 int cocci_id@p ( struct dvb_frontend * fe , u32 * unc ) ;
...
position p;
@@
 struct dib8000_state * cocci_id@p ;
...
position p;
@@
 enum param_loop_step cocci_id@p ;
...
position p;
@@
 const struct per_layer_regs cocci_id@p [ ] ;
...
position p;
@@
 struct linear_segments {  unsigned x ;  signed y ; } cocci_id@p ;
...
position p;
@@
 enum dib8000_power_mode { DIB8000_POWER_ALL = 0 , DIB8000_POWER_INTERFACE_ONLY , } cocci_id@p ;
...
position p;
@@
 enum dib8000_power_mode cocci_id@p ;
...
position p;
@@
 enum param_loop_step { LOOP_TUNE_1 , LOOP_TUNE_2 , } cocci_id@p ;
...
typedef u8;
@@
 struct i2c_device {  struct i2c_adapter * adap ;  u8 addr ;  u8 * i2c_write_buffer ;  u8 * i2c_read_buffer ;  struct mutex * i2c_buffer_lock ; } cocci_id@p ;
...
position p;
@@
 enum dibx000_i2c_interface cocci_id@p ;
