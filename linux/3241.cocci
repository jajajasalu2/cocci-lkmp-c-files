...
position p;
@@
 struct ibmpex_bmc_data * cocci_id@p ;
...
position p;
@@
 struct ibmpex_bmc_data {  struct list_head list ;  struct device * hwmon_dev ;  struct device * bmc_device ;  struct mutex lock ;  char valid ;  unsigned long last_updated ;  struct ipmi_addr address ;  struct completion read_complete ;  struct ipmi_user * user ;  int interface ;  struct kernel_ipmi_msg tx_message ;  unsigned char tx_msg_data [ IPMI_MAX_MSG_LENGTH ] ;  long tx_msgid ;  unsigned char rx_msg_data [ IPMI_MAX_MSG_LENGTH ] ;  unsigned long rx_msg_len ;  unsigned char rx_result ;  int rx_recv_type ;  unsigned char sensor_major ;  unsigned char sensor_minor ;  unsigned char num_sensors ;  struct ibmpex_sensor_data * sensors ; } cocci_id@p ;
...
typedef s16;
@@
 struct ibmpex_sensor_data {  int in_use ;  s16 values [ PEX_NUM_SENSOR_FUNCS ] ;  int multiplier ;  struct sensor_device_attribute_2 attr [ PEX_NUM_SENSOR_FUNCS ] ; } cocci_id@p ;
...
position p;
@@
 struct ibmpex_driver_data {  struct list_head bmc_data ;  struct ipmi_smi_watcher bmc_events ;  struct ipmi_user_hndl ipmi_hndlrs ; } cocci_id@p ;
...
position p;
@@
 struct ibmpex_driver_data cocci_id@p ;
