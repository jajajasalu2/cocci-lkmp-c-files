...
position p;
@@
 enum { ZSTDds_getFrameHeaderSize , ZSTDds_decodeFrameHeader , ZSTDds_decodeBlockHeader , ZSTDds_decompressBlock , ZSTDds_decompressLastBlock , ZSTDds_checkChecksum , ZSTDds_decodeSkippableHeader , ZSTDds_skipFrame , } cocci_id@p ;
...
position p;
@@
 enum { zdss_init , zdss_loadHeader , zdss_read , zdss_load , zdss_flush , } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef ZSTD_entropyTables_t;
@@
 ZSTD_entropyTables_t * cocci_id@p ;
...
position p;
@@
 ZSTD_nextInputType_e cocci_id@p ;
...
position p;
@@
 struct {  FSE_DTable LLTable [ FSE_DTABLE_SIZE_U32 ( LLFSELog ) ] ;  FSE_DTable OFTable [ FSE_DTABLE_SIZE_U32 ( OffFSELog ) ] ;  FSE_DTable MLTable [ FSE_DTABLE_SIZE_U32 ( MLFSELog ) ] ;  HUF_DTable hufTable [ HUF_DTABLE_SIZE ( HufLog ) ] ;  U64 workspace [ HUF_DECOMPRESS_WORKSPACE_SIZE_U32 / 2 ] ;  U32 rep [ ZSTD_REP_NUM ] ; } cocci_id@p ;
...
position p;
@@
 const symbolEncodingType_e cocci_id@p ;
...
symbol cocci_id;
position p;
typedef BIT_DStream_t,FSE_DState_t;
@@
 struct {  BIT_DStream_t DStream ;  FSE_DState_t stateLL ;  FSE_DState_t stateOffb ;  FSE_DState_t stateML ;  size_t prevOffset [ ZSTD_REP_NUM ] ;  const BYTE * base ;  size_t pos ;  uPtrDiff gotoDict ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef blockProperties_t;
@@
 blockProperties_t cocci_id@p ;
...
position p;
@@
 const void * cocci_id@p ( const ZSTD_DDict * ddict ) ;
...
position p;
@@
 symbolEncodingType_e cocci_id@p ;
...
typedef ZSTD_entropyTables_t;
@@
 struct ZSTD_DCtx_s {  const FSE_DTable * LLTptr ;  const FSE_DTable * MLTptr ;  const FSE_DTable * OFTptr ;  const HUF_DTable * HUFptr ;  ZSTD_entropyTables_t entropy ;  const void * previousDstEnd ;  const void * base ;  const void * vBase ;  const void * dictEnd ;  size_t expected ;  ZSTD_frameParams fParams ;  blockType_e bType ;  ZSTD_dStage stage ;  U32 litEntropy ;  U32 fseEntropy ;  struct xxh64_state xxhState ;  size_t headerSize ;  U32 dictID ;  const BYTE * litPtr ;  ZSTD_customMem customMem ;  size_t litSize ;  size_t rleSize ;  BYTE litBuffer [ ZSTD_BLOCKSIZE_ABSOLUTEMAX + WILDCOPY_OVERLENGTH ] ;  BYTE headerBuffer [ ZSTD_FRAMEHEADERSIZE_MAX ] ; } cocci_id@p ;
...
position p;
@@
 const FSE_decode_t4 * cocci_id@p ;
...
position p;
@@
 const ZSTD_customMem cocci_id@p ;
...
position p;
@@
 struct {  size_t litLength ;  size_t matchLength ;  size_t offset ;  const BYTE * match ; } cocci_id@p ;
...
position p;
@@
 const unsigned long long cocci_id@p ;
...
position p;
@@
 const FSE_decode_t4 cocci_id@p [ ( 1 << OF_DEFAULTNORMLOG ) + 1 ] ;
...
position p;
@@
 struct ZSTD_DStream_s {  ZSTD_DCtx * dctx ;  ZSTD_DDict * ddictLocal ;  const ZSTD_DDict * ddict ;  ZSTD_frameParams fParams ;  ZSTD_dStreamStage stage ;  char * inBuff ;  size_t inBuffSize ;  size_t inPos ;  size_t maxWindowSize ;  char * outBuff ;  size_t outBuffSize ;  size_t outStart ;  size_t outEnd ;  size_t blockSize ;  BYTE headerBuffer [ ZSTD_FRAMEHEADERSIZE_MAX ] ;  size_t lhSize ;  ZSTD_customMem customMem ;  void * legacyContext ;  U32 previousLegacyVersion ;  U32 legacyVersion ;  U32 hostageByte ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( ZSTD_DCtx * dstDCtx , const ZSTD_DDict * ddict ) ;
...
typedef ZSTD_entropyTables_t;
@@
 struct ZSTD_DDict_s {  void * dictBuffer ;  const void * dictContent ;  size_t dictSize ;  ZSTD_entropyTables_t entropy ;  U32 dictID ;  U32 entropyPresent ;  ZSTD_customMem cMem ; } cocci_id@p ;
...
position p;
@@
 struct {  blockType_e blockType ;  U32 lastBlock ;  U32 origSize ; } cocci_id@p ;
...
position p;
@@
 const FSE_decode_t4 cocci_id@p [ ( 1 << LL_DEFAULTNORMLOG ) + 1 ] ;
...
position p;
@@
 size_t cocci_id@p ( const ZSTD_DDict * ddict ) ;
...
position p;
@@
 const FSE_decode_t4 cocci_id@p [ ( 1 << ML_DEFAULTNORMLOG ) + 1 ] ;
...
symbol cocci_id;
position p;
typedef FSE_decode_t;
@@
 union {  FSE_decode_t realData ;  U32 alignedBy4 ; } cocci_id@p ;
