...
position p;
@@
 const struct branch_stack * cocci_id@p ;
...
position p;
@@
 unsigned char cocci_id@p [ CHASHSZ ] ;
...
typedef bool;
@@
 void cocci_id@p ( struct machine * machine , struct thread * th , bool lock ) ;
...
position p;
@@
 struct extra_kernel_map * cocci_id@p ;
...
position p;
@@
 struct branch_flags * cocci_id@p ;
...
position p;
@@
 struct branch_entry cocci_id@p [ nr ] ;
...
typedef u64;
@@
 struct process_args {  u64 start ; } cocci_id@p ;
...
position p;
@@
 struct unwind_entry * cocci_id@p ;
...
typedef u64;
@@
 struct iterations {  int nr_loop_iter ;  u64 cycles ; } cocci_id@p ;
...
position p;
@@
 struct iterations cocci_id@p [ nr ] ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct thread * thread , void * p ) ;
