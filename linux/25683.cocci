...
position p;
@@
 struct intel_pt_pkt_info * cocci_id@p ;
...
position p;
@@
 struct fast_forward_data cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct intel_pt_pkt_info * pkt_info ) ;
...
position p;
@@
 struct intel_pt_stack {  struct intel_pt_blk * blk ;  struct intel_pt_blk * spare ;  int pos ; } cocci_id@p ;
...
typedef uint64_t;
@@
 struct fast_forward_data {  uint64_t timestamp ;  uint64_t buf_timestamp ; } cocci_id@p ;
...
position p;
@@
 struct intel_pt_calc_cyc_to_tsc_info cocci_id@p ;
...
position p;
@@
 struct intel_pt_decoder cocci_id@p ;
...
symbol cocci_id;
position p;
typedef intel_pt_pkt_cb_t;
@@
 intel_pt_pkt_cb_t cocci_id@p ;
...
position p;
@@
 enum intel_pt_pkt_state cocci_id@p ;
...
position p;
@@
 const struct intel_pt_pkt * cocci_id@p ;
...
position p;
@@
 struct intel_pt_blk cocci_id@p ;
...
typedef uint64_t;
@@
 struct intel_pt_pkt_info {  struct intel_pt_decoder * decoder ;  struct intel_pt_pkt packet ;  uint64_t pos ;  int pkt_len ;  int last_packet_type ;  void * data ; } cocci_id@p ;
...
position p;
@@
 struct intel_pt_stack * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,uint64_t,uint32_t;
@@
 struct intel_pt_calc_cyc_to_tsc_info {  uint64_t cycle_cnt ;  unsigned int cbr ;  uint32_t last_mtc ;  uint64_t ctc_timestamp ;  uint64_t ctc_delta ;  uint64_t tsc_timestamp ;  uint64_t timestamp ;  bool have_tma ;  bool fixup_last_mtc ;  bool from_mtc ;  double cbr_cyc_to_tsc ; } cocci_id@p ;
...
position p;
@@
 enum intel_pt_pkt_state { INTEL_PT_STATE_NO_PSB , INTEL_PT_STATE_NO_IP , INTEL_PT_STATE_ERR_RESYNC , INTEL_PT_STATE_IN_SYNC , INTEL_PT_STATE_TNT_CONT , INTEL_PT_STATE_TNT , INTEL_PT_STATE_TIP , INTEL_PT_STATE_TIP_PGD , INTEL_PT_STATE_FUP , INTEL_PT_STATE_FUP_NO_TIP , } cocci_id@p ;
...
typedef uint64_t;
@@
 struct intel_pt_blk {  struct intel_pt_blk * prev ;  uint64_t ip [ INTEL_PT_BLK_SIZE ] ; } cocci_id@p ;
