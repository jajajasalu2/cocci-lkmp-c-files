...
position p;
@@
 const struct sys64_hook * cocci_id@p ;
...
typedef u32;
@@
 int ( * cocci_id@p ) ( struct pt_regs * regs , u32 instr ) ;
...
position p;
@@
 char cocci_id@p [ sizeof ( "00000000 " ) * 5 + 2 + 1 ] ;
...
position p;
@@
 struct sys64_hook {  unsigned int esr_mask ;  unsigned int esr_val ;  void ( * handler ) ( unsigned int esr , struct pt_regs * regs ) ; } cocci_id@p ;
