...
position p;
@@
 struct red_sched_data * cocci_id@p ;
...
position p;
@@
 struct tc_red_xstats cocci_id@p ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_RED_MAX + 1 ] ;
...
typedef u32;
@@
 struct red_sched_data {  u32 limit ;  unsigned char flags ;  struct timer_list adapt_timer ;  struct Qdisc * sch ;  struct red_parms parms ;  struct red_vars vars ;  struct red_stats stats ;  struct Qdisc * qdisc ; } cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_RED_MAX + 1 ] ;
