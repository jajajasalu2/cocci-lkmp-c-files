...
position p;
@@
 struct tc_drr_stats cocci_id@p ;
...
typedef u32;
@@
 struct drr_class {  struct Qdisc_class_common common ;  unsigned int filter_cnt ;  struct gnet_stats_basic_packed bstats ;  struct gnet_stats_queue qstats ;  struct net_rate_estimator __rcu * rate_est ;  struct list_head alist ;  struct Qdisc * qdisc ;  u32 quantum ;  u32 deficit ; } cocci_id@p ;
...
position p;
@@
 struct drr_sched {  struct list_head active ;  struct tcf_proto __rcu * filter_list ;  struct tcf_block * block ;  struct Qdisc_class_hash clhash ; } cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_DRR_MAX + 1 ] ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_DRR_MAX + 1 ] ;
