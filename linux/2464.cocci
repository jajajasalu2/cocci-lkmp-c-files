...
@@
 struct sba_device {  struct device * dev ;  enum sba_version ver ;  u32 max_req ;  u32 hw_buf_size ;  u32 hw_resp_size ;  u32 max_pq_coefs ;  u32 max_pq_srcs ;  u32 max_cmd_per_req ;  u32 max_xor_srcs ;  u32 max_resp_pool_size ;  u32 max_cmds_pool_size ;  struct mbox_client client ;  struct mbox_chan * mchan ;  struct device * mbox_dev ;  struct dma_device dma_dev ;  struct dma_chan dma_chan ;  void * resp_base ;  dma_addr_t resp_dma_base ;  void * cmds_base ;  dma_addr_t cmds_dma_base ;  spinlock_t reqs_lock ;  bool reqs_fence ;  struct list_head reqs_alloc_list ;  struct list_head reqs_pending_list ;  struct list_head reqs_active_list ;  struct list_head reqs_aborted_list ;  struct list_head reqs_free_list ;  struct dentry * root ; } cocci_id@p ;
...
position p;
@@
 struct brcm_sba_command * cocci_id@p ;
...
position p;
@@
 enum sba_version { SBA_VER_1 = 0 , SBA_VER_2 , } cocci_id@p ;
...
@@
 struct sba_request {  struct list_head node ;  struct sba_device * sba ;  u32 flags ;  struct sba_request * first ;  struct list_head next ;  atomic_t next_pending_count ;  struct brcm_message msg ;  struct dma_async_tx_descriptor tx ;  struct brcm_sba_command cmds [ 0 ] ; } cocci_id@p ;
...
position p;
@@
 enum sba_request_flags { SBA_REQUEST_STATE_FREE = 0x001 , SBA_REQUEST_STATE_ALLOCED = 0x002 , SBA_REQUEST_STATE_PENDING = 0x004 , SBA_REQUEST_STATE_ACTIVE = 0x008 , SBA_REQUEST_STATE_ABORTED = 0x010 , SBA_REQUEST_STATE_MASK = 0x0ff , SBA_REQUEST_FENCE = 0x100 , } cocci_id@p ;
