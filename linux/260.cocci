...
position p;
@@
 struct kprobe_insn_page cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct kprobe * p , struct pt_regs * regs ) ;
...
@@
 struct kprobe_insn_page {  struct list_head list ;  kprobe_opcode_t * insns ;  struct kprobe_insn_cache * cache ;  int nused ;  int ngarbage ;  char slot_used [ ] ; } cocci_id@p ;
...
position p;
@@
 struct optimized_kprobe * cocci_id@p ;
...
@@
 struct {  raw_spinlock_t lock ____cacheline_aligned_in_smp ; } cocci_id@p [ KPROBE_TABLE_SIZE ] ;
...
position p;
@@
 enum kprobe_slot_state { SLOT_CLEAN = 0 , SLOT_DIRTY = 1 , SLOT_USED = 2 , } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct kprobe * ap , struct kprobe * p ) ;
...
position p;
@@
 int cocci_id@p ( struct kprobe_insn_cache * c ) ;
...
position p;
@@
 struct kprobe_blacklist_entry * cocci_id@p ;
...
position p;
@@
 struct arch_specific_insn cocci_id@p ;
...
position p;
@@
 struct kprobe_insn_cache cocci_id@p ;
...
position p;
@@
 struct hlist_head cocci_id@p [ KPROBE_TABLE_SIZE ] ;
