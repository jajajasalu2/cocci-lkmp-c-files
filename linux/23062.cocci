...
position p;
@@
 struct mtk_btcvsd_snd_time_buffer_info cocci_id@p ;
...
position p;
@@
 struct mtk_btcvsd_snd_time_buffer_info {  unsigned long long data_count_equi_time ;  unsigned long long time_stamp_us ; } cocci_id@p ;
...
position p;
@@
 enum BT_SCO_BAND { BT_SCO_NB , BT_SCO_WB , } cocci_id@p ;
...
position p;
@@
 enum bt_sco_direct { BT_SCO_DIRECT_BT2ARM , BT_SCO_DIRECT_ARM2BT , } cocci_id@p ;
...
position p;
@@
 const unsigned int cocci_id@p [ BT_SCO_CVSD_MAX ] [ 4 ] ;
...
typedef u8;
@@
 const u8 cocci_id@p [ SCO_PACKET_180 ] ;
...
position p;
@@
 struct mtk_btcvsd_snd_hw_info {  unsigned int num_valid_addr ;  unsigned long bt_sram_addr [ 20 ] ;  unsigned int packet_length ;  unsigned int packet_num ; } cocci_id@p ;
...
position p;
@@
 struct mtk_btcvsd_snd_stream * cocci_id@p ;
...
position p;
@@
 enum bt_sco_state { BT_SCO_STATE_IDLE , BT_SCO_STATE_RUNNING , BT_SCO_STATE_ENDING , BT_SCO_STATE_LOOPBACK , } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,u32,wait_queue_head_t,spinlock_t;
@@
 struct mtk_btcvsd_snd {  struct device * dev ;  int irq_id ;  struct regmap * infra ;  void __iomem * bt_pkv_base ;  void __iomem * bt_sram_bank2_base ;  unsigned int infra_misc_offset ;  unsigned int conn_bt_cvsd_mask ;  unsigned int cvsd_mcu_read_offset ;  unsigned int cvsd_mcu_write_offset ;  unsigned int cvsd_packet_indicator ;  u32 * bt_reg_pkt_r ;  u32 * bt_reg_pkt_w ;  u32 * bt_reg_ctl ;  unsigned int irq_disabled : 1 ;  spinlock_t tx_lock ;  spinlock_t rx_lock ;  wait_queue_head_t tx_wait ;  wait_queue_head_t rx_wait ;  struct mtk_btcvsd_snd_stream * tx ;  struct mtk_btcvsd_snd_stream * rx ;  u8 tx_packet_buf [ BTCVSD_TX_BUF_SIZE ] ;  u8 rx_packet_buf [ BTCVSD_RX_BUF_SIZE ] ;  enum BT_SCO_BAND band ; } cocci_id@p ;
...
position p;
@@
 enum bt_sco_packet_len { BT_SCO_CVSD_30 = 0 , BT_SCO_CVSD_60 , BT_SCO_CVSD_90 , BT_SCO_CVSD_120 , BT_SCO_CVSD_10 , BT_SCO_CVSD_20 , BT_SCO_CVSD_MAX , } cocci_id@p ;
...
position p;
@@
 struct mtk_btcvsd_snd * cocci_id@p ;
...
position p;
@@
 const unsigned int cocci_id@p [ BT_SCO_CVSD_MAX ] [ 6 ] ;
...
position p;
@@
 enum bt_sco_packet_len cocci_id@p ;
