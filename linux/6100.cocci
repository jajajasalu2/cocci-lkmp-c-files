...
position p;
@@
 struct fsl_ddr_devtype_data {  unsigned int quirks ; } cocci_id@p ;
...
position p;
@@
 struct ddr_pmu {  struct pmu pmu ;  void __iomem * base ;  unsigned int cpu ;  struct hlist_node node ;  struct device * dev ;  struct perf_event * events [ NUM_COUNTERS ] ;  int active_events ;  enum cpuhp_state cpuhp_state ;  const struct fsl_ddr_devtype_data * devtype_data ;  int irq ;  int id ; } cocci_id@p ;
...
position p;
@@
 enum ddr_perf_filter_capabilities { PERF_CAP_AXI_ID_FILTER = 0 , PERF_CAP_AXI_ID_FILTER_ENHANCED , PERF_CAP_AXI_ID_FEAT_MAX , } cocci_id@p ;
...
position p;
@@
 const struct fsl_ddr_devtype_data cocci_id@p ;
