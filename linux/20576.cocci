...
position p;
@@
 struct mid_pwr_device_info {  int ( * set_initial_state ) ( struct mid_pwr * pwr ) ; } cocci_id@p ;
...
position p;
@@
 struct mid_pwr_device_info * cocci_id@p ;
...
position p;
@@
 const struct mid_pwr_device_info cocci_id@p ;
...
typedef pci_power_t;
@@
 struct mid_pwr_dev {  struct pci_dev * pdev ;  pci_power_t state ; } cocci_id@p ;
...
typedef bool;
@@
 struct mid_pwr {  struct device * dev ;  void __iomem * regs ;  int irq ;  bool available ;  struct mutex lock ;  struct mid_pwr_dev lss [ LSS_MAX_DEVS ] [ LSS_MAX_SHARED_DEVS ] ; } cocci_id@p ;
