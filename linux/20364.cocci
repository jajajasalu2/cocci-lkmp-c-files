...
@@
 struct nmi_desc {  raw_spinlock_t lock ;  struct list_head head ; } cocci_id@p ;
...
position p;
@@
 struct nmi_desc cocci_id@p [ NMI_MAX ] ;
...
position p;
@@
 struct nmi_stats {  unsigned int normal ;  unsigned int unknown ;  unsigned int external ;  unsigned int swallow ; } cocci_id@p ;
...
position p;
@@
 struct cea_exception_stacks * cocci_id@p ;
...
position p;
@@
 enum nmi_states { NMI_NOT_RUNNING = 0 , NMI_EXECUTING , NMI_LATCHED , } cocci_id@p ;
