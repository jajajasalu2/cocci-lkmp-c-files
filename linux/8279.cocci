...
position p;
@@
 enum { SIENA_RESET_PORT = ( ETH_RESET_DMA | ETH_RESET_FILTER | ETH_RESET_OFFLOAD | ETH_RESET_MAC | ETH_RESET_PHY ) , SIENA_RESET_MC = ( SIENA_RESET_PORT | ETH_RESET_MGMT << ETH_RESET_SHARED_SHIFT ) , } cocci_id@p ;
...
position p;
@@
 struct siena_nvram_type_info {  int port ;  const char * name ; } cocci_id@p ;
...
position p;
@@
 const struct siena_nvram_type_info cocci_id@p [ ] ;
...
typedef uint16_t;
@@
 uint16_t cocci_id@p [ MC_CMD_GET_BOARD_CFG_OUT_FW_SUBTYPE_LIST_MAXNUM ] ;
...
position p;
@@
 const struct efx_hw_stat_desc cocci_id@p [ SIENA_STAT_COUNT ] ;
