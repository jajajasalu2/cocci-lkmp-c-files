...
position p;
@@
 void ( * cocci_id@p ) ( void * arg , int mode ) ;
...
position p;
@@
 struct md_cluster_info cocci_id@p ;
...
position p;
@@
 enum msg_type { METADATA_UPDATED = 0 , RESYNCING , NEWDISK , REMOVE , RE_ADD , BITMAP_NEEDS_SYNC , CHANGE_CAPACITY , BITMAP_RESIZE , } cocci_id@p ;
...
position p;
@@
 struct bitmap_counts * cocci_id@p ;
...
typedef __le64;
@@
 struct resync_info {  __le64 lo ;  __le64 hi ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bitmap_super_t;
@@
 bitmap_super_t * cocci_id@p ;
...
@@
 struct cluster_msg {  __le32 type ;  __le32 slot ;  __le64 low ;  __le64 high ;  char uuid [ 16 ] ;  __le32 raid_slot ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef wait_queue_head_t,sector_t,spinlock_t,dlm_lockspace_t;
@@
 struct md_cluster_info {  struct mddev * mddev ;  dlm_lockspace_t * lockspace ;  int slot_number ;  struct completion completion ;  struct mutex recv_mutex ;  struct dlm_lock_resource * bitmap_lockres ;  struct dlm_lock_resource * * other_bitmap_lockres ;  struct dlm_lock_resource * resync_lockres ;  struct list_head suspend_list ;  spinlock_t suspend_lock ;  sector_t suspend_lo ;  sector_t suspend_hi ;  int suspend_from ;  struct md_thread * recovery_thread ;  unsigned long recovery_map ;  struct dlm_lock_resource * ack_lockres ;  struct dlm_lock_resource * message_lockres ;  struct dlm_lock_resource * token_lockres ;  struct dlm_lock_resource * no_new_dev_lockres ;  struct md_thread * recv_thread ;  struct completion newdisk_completion ;  wait_queue_head_t wait ;  unsigned long state ;  sector_t sync_low ;  sector_t sync_hi ; } cocci_id@p ;
