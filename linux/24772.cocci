...
position p;
@@
 unsigned char cocci_id@p [ MAX_IPOPTLEN ] ;
...
position p;
@@
 struct nf_osf_hdr_ctx * cocci_id@p ;
...
position p;
@@
 struct nf_osf_finger cocci_id@p ;
...
position p;
@@
 struct nf_osf_user_finger cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ OSF_ATTR_MAX + 1 ] ;
...
position p;
@@
 const struct nf_osf_info * cocci_id@p ;
...
position p;
@@
 const struct nfnl_callback cocci_id@p [ OSF_MSG_MAX ] ;
...
typedef bool,u16;
@@
 struct nf_osf_hdr_ctx {  bool df ;  u16 window ;  u16 totlen ;  const unsigned char * optp ;  unsigned int optsize ; } cocci_id@p ;
...
position p;
@@
 const struct nf_osf_finger * cocci_id@p ;
...
position p;
@@
 const struct nf_osf_user_finger * cocci_id@p ;
