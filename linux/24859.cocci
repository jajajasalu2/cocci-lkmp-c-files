...
position p;
@@
 const int cocci_id@p [ NFNLGRP_MAX + 1 ] ;
...
position p;
@@
 struct nlattr * cocci_id@p [ NFNL_MAX_ATTR_COUNT + 1 ] ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ NFNL_BATCH_MAX + 1 ] ;
...
position p;
@@
 struct {  struct mutex mutex ;  const struct nfnetlink_subsystem __rcu * subsys ; } cocci_id@p [ NFNL_SUBSYS_COUNT ] ;
...
position p;
@@
 struct nlattr * cocci_id@p [ NFNL_BATCH_MAX + 1 ] ;
...
position p;
@@
 struct nfnl_err {  struct list_head head ;  struct nlmsghdr * nlh ;  int err ;  struct netlink_ext_ack extack ; } cocci_id@p ;
...
position p;
@@
 enum { NFNL_BATCH_FAILURE = ( 1 << 0 ) , NFNL_BATCH_DONE = ( 1 << 1 ) , NFNL_BATCH_REPLAY = ( 1 << 2 ) , } cocci_id@p ;
...
position p;
@@
 const struct nfnl_callback * cocci_id@p ;
