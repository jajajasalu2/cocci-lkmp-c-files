...
typedef u64,u32;
@@
 struct imon_panel_key_table {  u64 hw_code ;  u32 keycode ; } cocci_id@p ;
...
position p;
@@
 const struct imon_usb_dev_descr cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct usb_interface * interface ) ;
...
position p;
@@
 struct imon_usb_dev_descr * cocci_id@p ;
...
position p;
@@
 struct usb_class_driver cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct usb_interface * interface , const struct usb_device_id * id ) ;
...
position p;
@@
 struct imon_context * cocci_id@p ;
...
position p;
@@
 const struct imon_panel_key_table * cocci_id@p ;
...
position p;
@@
 enum { IMON_KEY_IMON = 0 , IMON_KEY_MCE = 1 , IMON_KEY_PANEL = 2 , } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,u64,bool,u32,u16,wait_queue_head_t,tx_t,spinlock_t;
@@
 struct imon_context {  struct device * dev ;  struct usb_device * usbdev_intf0 ;  struct usb_device * usbdev_intf1 ;  bool display_supported ;  bool display_isopen ;  bool rf_device ;  bool rf_isassociating ;  bool dev_present_intf0 ;  bool dev_present_intf1 ;  struct mutex lock ;  wait_queue_head_t remove_ok ;  struct usb_endpoint_descriptor * rx_endpoint_intf0 ;  struct usb_endpoint_descriptor * rx_endpoint_intf1 ;  struct usb_endpoint_descriptor * tx_endpoint ;  struct urb * rx_urb_intf0 ;  struct urb * rx_urb_intf1 ;  struct urb * tx_urb ;  bool tx_control ;  unsigned char usb_rx_buf [ 8 ] ;  unsigned char usb_tx_buf [ 8 ] ;  unsigned int send_packet_delay ;  struct tx_t {  unsigned char data_buf [ 35 ] ;  struct completion finished ;  bool busy ;  int status ; } tx ;  u16 vendor ;  u16 product ;  struct rc_dev * rdev ;  struct input_dev * idev ;  struct input_dev * touch ;  spinlock_t kc_lock ;  u32 kc ;  u32 last_keycode ;  u32 rc_scancode ;  u8 rc_toggle ;  u64 rc_proto ;  bool release_code ;  u8 display_type ;  bool pad_mouse ;  char name_rdev [ 128 ] ;  char phys_rdev [ 64 ] ;  char name_idev [ 128 ] ;  char phys_idev [ 64 ] ;  char name_touch [ 128 ] ;  char phys_touch [ 64 ] ;  struct timer_list ttimer ;  int touch_x ;  int touch_y ;  const struct imon_usb_dev_descr * dev_descr ; } cocci_id@p ;
...
position p;
@@
 enum { IMON_DISPLAY_TYPE_AUTO = 0 , IMON_DISPLAY_TYPE_VFD = 1 , IMON_DISPLAY_TYPE_LCD = 2 , IMON_DISPLAY_TYPE_VGA = 3 , IMON_DISPLAY_TYPE_NONE = 4 , } cocci_id@p ;
...
position p;
@@
 unsigned char cocci_id@p [ IMON_CLOCK_ENABLE_PACKETS ] [ 8 ] ;
...
position p;
@@
 int cocci_id@p ( struct usb_interface * intf ) ;
