...
position p;
@@
 const  cocci_id@p ( adc_pga_gain_tlv , 
							   0 , 0 , 
...
typedef bool;
@@
 struct es8316_priv {  struct mutex lock ;  struct clk * mclk ;  struct regmap * regmap ;  struct snd_soc_component * component ;  struct snd_soc_jack * jack ;  int irq ;  unsigned int sysclk ;  unsigned int allowed_rates [ NR_SUPPORTED_MCLK_LRCK_RATIOS ] ;  struct snd_pcm_hw_constraint_list sysclk_constraints ;  bool jd_inverted ; } cocci_id@p ;
...
position p;
@@
 const  cocci_id@p ( hpmixer_gain_tlv , 
							   0 , 4 , 
...
position p;
@@
 const  cocci_id@p ( hpout_vol_tlv , 
							   0 , 0 , 
