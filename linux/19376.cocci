...
position p;
@@
 const struct {  const char * short_text ;  const char * long_text ; } cocci_id@p [ ] ;
...
position p;
@@
 struct flush_data {  struct vm_area_struct * vma ;  unsigned long addr1 ;  unsigned long addr2 ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( unsigned long start , unsigned long size ) ;
...
position p;
@@
 struct ipi_data {  unsigned long ipi_count [ IPI_MAX ] ; } cocci_id@p ;
...
position p;
@@
 enum ipi_msg_type { IPI_RESCHEDULE = 0 , IPI_CALL_FUNC , IPI_CPU_STOP , IPI_MAX , } cocci_id@p ;
