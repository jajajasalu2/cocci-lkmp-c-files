...
position p;
@@
 struct kvm_task_sleep_node * cocci_id@p ;
...
@@
 struct kvm_task_sleep_head {  raw_spinlock_t lock ;  struct hlist_head list ; } cocci_id@p [ KVM_TASK_SLEEP_HASHSIZE ] ;
...
position p;
@@
 struct kvm_vcpu_pv_apf_data cocci_id@p ;
...
position p;
@@
 struct kvm_task_sleep_head * cocci_id@p ;
...
typedef bool,u32;
@@
 struct kvm_task_sleep_node {  struct hlist_node link ;  struct swait_queue_head wq ;  u32 token ;  int cpu ;  bool halted ; } cocci_id@p ;
