...
position p;
@@
 struct tcindex_data cocci_id@p ;
...
position p;
@@
 struct tcindex_filter_result {  struct tcf_exts exts ;  struct tcf_result res ;  struct rcu_work rwork ; } cocci_id@p ;
...
position p;
@@
 struct tcindex_filter_result * cocci_id@p ;
...
position p;
@@
 struct tcindex_filter * cocci_id@p ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_TCINDEX_MAX + 1 ] ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_TCINDEX_MAX + 1 ] ;
...
typedef u16;
@@
 struct tcindex_filter {  u16 key ;  struct tcindex_filter_result result ;  struct tcindex_filter __rcu * next ;  struct rcu_work rwork ; } cocci_id@p ;
...
typedef u32,u16;
@@
 struct tcindex_data {  struct tcindex_filter_result * perfect ;  struct tcindex_filter __rcu * * h ;  struct tcf_proto * tp ;  u16 mask ;  u32 shift ;  u32 hash ;  u32 alloc_hash ;  u32 fall_through ;  struct rcu_work rwork ; } cocci_id@p ;
