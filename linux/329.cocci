...
position p;
@@
 void cocci_id@p ( struct percpu_ref * ref ) ;
...
position p;
@@
 char cocci_id@p [ CGROUP_FILE_NAME_MAX ] ;
...
position p;
@@
 struct cgroup_subsys_state * cocci_id@p ( struct cgroup * cgrp , struct cgroup_subsys * ss ) ;
...
position p;
@@
 struct cgrp_cset_link cocci_id@p ;
...
position p;
@@
 const struct cftype * cocci_id@p ;
...
position p;
@@
 struct kernfs_syscall_ops cocci_id@p ;
...
position p;
@@
 struct cgroup_namespace * cocci_id@p ;
...
position p;
@@
 struct kobj_attribute * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct cgroup * cgrp ) ;
...
position p;
@@
 struct sock_cgroup_data * cocci_id@p ;
...
position p;
@@
 struct cgroup_subsys_state * cocci_id@p [ CGROUP_SUBSYS_COUNT ] ;
...
position p;
@@
 struct cgroup_mgctx * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct cgroup * cgrp , int ret ) ;
...
position p;
@@
 char cocci_id@p [ TRACE_CGROUP_PATH_LEN ] ;
...
position p;
@@
 enum cgroup2_param { Opt_nsdelegate , Opt_memory_localevents , nr__cgroup2_params , } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct css_task_iter * it , struct task_struct * task ) ;
...
typedef bool;
@@
 int cocci_id@p ( struct cgroup_subsys_state * css , struct cgroup * cgrp , struct cftype cfts [ ] , bool is_add ) ;
...
position p;
@@
 struct cgroup_fs_context * cocci_id@p ;
...
position p;
@@
 const struct cgroup * cocci_id@p ;
