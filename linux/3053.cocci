...
position p;
@@
 struct prt_quirk {  const struct dmi_system_id * system ;  unsigned int segment ;  unsigned int bus ;  unsigned int device ;  unsigned char pin ;  const char * source ;  const char * actual_source ; } cocci_id@p ;
...
position p;
@@
 const struct prt_quirk cocci_id@p [ ] ;
...
position p;
@@
 struct acpi_prt_entry * * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,u32,acpi_handle;
@@
 struct acpi_prt_entry {  struct acpi_pci_id id ;  u8 pin ;  acpi_handle link ;  u32 index ; } cocci_id@p ;
