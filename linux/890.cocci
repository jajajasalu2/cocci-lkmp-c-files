...
position p;
@@
 struct NFTLMediaHeader * cocci_id@p ;
...
position p;
@@
 struct INFTLPartition * cocci_id@p ;
...
position p;
@@
 struct INFTLMediaHeader * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct nand_chip * this , int chip ) ;
...
@@
 union {  uint32_t dword ;  uint8_t byte [ 4 ] ; } cocci_id@p ;
...
typedef u_char;
@@
 struct doc_priv {  void __iomem * virtadr ;  unsigned long physadr ;  u_char ChipID ;  u_char CDSNControl ;  int chips_per_floor ;  int curfloor ;  int curchip ;  int mh0_page ;  int mh1_page ;  struct rs_control * rs_decoder ;  struct mtd_info * nextdoc ;  int ( * late_init ) ( struct mtd_info * mtd ) ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct nand_chip * this , int cmd , unsigned int bitmask ) ;
...
position p;
@@
 struct mtd_partition cocci_id@p [ 5 ] ;
