...
position p;
@@
 struct smu_sdbp_header * cocci_id@p ;
...
@@
 struct smu_device {  spinlock_t lock ;  struct device_node * of_node ;  struct platform_device * of_dev ;  int doorbell ;  u32 __iomem * db_buf ;  struct device_node * db_node ;  unsigned int db_irq ;  int msg ;  struct device_node * msg_node ;  unsigned int msg_irq ;  struct smu_cmd_buf * cmd_buf ;  u32 cmd_buf_abs ;  struct list_head cmd_list ;  struct smu_cmd * cmd_cur ;  int broken_nap ;  struct list_head cmd_i2c_list ;  struct smu_i2c_cmd * cmd_i2c_cur ;  struct timer_list i2c_timer ; } cocci_id@p ;
...
position p;
@@
 void ( * cocci_id@p ) ( struct smu_cmd * cmd , void * misc ) ;
...
position p;
@@
 const struct smu_sdbp_header * cocci_id@p ;
...
position p;
@@
 struct smu_user_reply_hdr cocci_id@p ;
...
position p;
@@
 void ( * cocci_id@p ) ( struct smu_i2c_cmd * cmd , void * misc ) ;
...
typedef u8;
@@
 struct smu_cmd_buf {  u8 cmd ;  u8 length ;  u8 data [ SMU_MAX_DATA ] ; } cocci_id@p ;
...
position p;
@@
 enum smu_file_mode { smu_file_commands , smu_file_events , smu_file_closing , } cocci_id@p ;
...
@@
 struct smu_private {  struct list_head list ;  enum smu_file_mode mode ;  int busy ;  struct smu_cmd cmd ;  spinlock_t lock ;  wait_queue_head_t wait ;  u8 buffer [ SMU_MAX_DATA ] ; } cocci_id@p ;
...
position p;
@@
 struct smu_user_cmd_hdr cocci_id@p ;
...
position p;
@@
 struct smu_simple_cmd * cocci_id@p ;
