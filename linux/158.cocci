...
position p;
@@
 struct lock_stress_stats {  long n_lock_fail ;  long n_lock_acquired ; } cocci_id@p ;
...
@@
 struct lock_torture_cxt {  int nrealwriters_stress ;  int nrealreaders_stress ;  bool debug_lock ;  atomic_t n_lock_torture_errors ;  struct lock_torture_ops * cur_ops ;  struct lock_stress_stats * lwsa ;  struct lock_stress_stats * lrsa ; } cocci_id@p ;
...
position p;
@@
 struct lock_torture_ops * cocci_id@p ;
...
position p;
@@
 struct lock_torture_ops {  void ( * init ) ( void ) ;  int ( * writelock ) ( void ) ;  void ( * write_delay ) ( struct torture_random_state * trsp ) ;  void ( * task_boost ) ( struct torture_random_state * trsp ) ;  void ( * writeunlock ) ( void ) ;  int ( * readlock ) ( void ) ;  void ( * read_delay ) ( struct torture_random_state * trsp ) ;  void ( * readunlock ) ( void ) ;  unsigned long flags ;  const char * name ; } cocci_id@p ;
...
position p;
@@
 struct lock_stress_stats * cocci_id@p ;
...
position p;
@@
 struct reorder_lock {  struct list_head link ;  struct ww_mutex * lock ; } cocci_id@p [ 3 ] ;
...
position p;
@@
 struct lock_torture_cxt cocci_id@p ;
...
position p;
@@
 struct percpu_rw_semaphore cocci_id@p ;
