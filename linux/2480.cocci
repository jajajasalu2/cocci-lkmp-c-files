...
@@
 struct bam_chan {  struct virt_dma_chan vc ;  struct bam_device * bdev ;  u32 id ;  struct dma_slave_config slave ;  struct bam_desc_hw * fifo_virt ;  dma_addr_t fifo_phys ;  unsigned short head ;  unsigned short tail ;  unsigned int initialized ;  unsigned int paused ;  unsigned int reconfigure ;  struct list_head desc_list ;  struct list_head node ; } cocci_id@p ;
...
position p;
@@
 struct bam_async_desc cocci_id@p ;
...
typedef u32,u16;
@@
 struct bam_async_desc {  struct virt_dma_desc vd ;  u32 num_desc ;  u32 xfer_len ;  u16 flags ;  struct bam_desc_hw * curr_desc ;  struct list_head desc_node ;  enum dma_transfer_direction dir ;  size_t length ;  struct bam_desc_hw desc [ 0 ] ; } cocci_id@p ;
...
typedef __le32;
@@
 struct bam_desc_hw {  __le32 addr ;  __le16 size ;  __le16 flags ; } cocci_id@p ;
...
position p;
@@
 const struct reg_offset_data cocci_id@p ;
...
typedef bool,u32;
@@
 struct bam_device {  void __iomem * regs ;  struct device * dev ;  struct dma_device common ;  struct device_dma_parameters dma_parms ;  struct bam_chan * channels ;  u32 num_channels ;  u32 num_ees ;  u32 ee ;  bool controlled_remotely ;  const struct reg_offset_data * layout ;  struct clk * bamclk ;  int irq ;  struct tasklet_struct task ; } cocci_id@p ;
