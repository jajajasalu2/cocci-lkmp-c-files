...
position p;
@@
 struct rionet_private * cocci_id@p ;
...
position p;
@@
 struct rionet_net cocci_id@p [ RIONET_MAX_NETS ] ;
...
@@
 struct rionet_private {  struct rio_mport * mport ;  struct sk_buff * rx_skb [ RIONET_RX_RING_SIZE ] ;  struct sk_buff * tx_skb [ RIONET_TX_RING_SIZE ] ;  int rx_slot ;  int tx_slot ;  int tx_cnt ;  int ack_slot ;  spinlock_t lock ;  spinlock_t tx_lock ;  u32 msg_enable ;  bool open ; } cocci_id@p ;
...
typedef spinlock_t;
@@
 struct rionet_net {  struct net_device * ndev ;  struct list_head peers ;  spinlock_t lock ;  struct rio_dev * * active ;  int nact ; } cocci_id@p ;
...
position p;
@@
 struct rio_device_id cocci_id@p [ ] ;
...
position p;
@@
 struct rionet_peer {  struct list_head node ;  struct rio_dev * rdev ;  struct resource * res ; } cocci_id@p ;
