...
position p;
@@
 unsigned long cocci_id@p [ PLPAR_HCALL9_BUFSIZE ] ;
...
position p;
@@
 struct mmu_psize_def * cocci_id@p ;
...
position p;
@@
 struct hpt_resize_state {  unsigned long shift ;  int commit_rc ; } cocci_id@p ;
...
position p;
@@
 struct vcpu_dispatch_data * cocci_id@p ;
...
position p;
@@
 struct ppc64_tlb_batch * cocci_id@p ;
...
position p;
@@
 struct vcpu_dispatch_data {  int last_disp_cpu ;  int total_disp ;  int same_cpu_disp ;  int same_chip_disp ;  int diff_chip_disp ;  int far_chip_disp ;  int numa_home_disp ;  int numa_remote_disp ;  int numa_far_disp ; } cocci_id@p ;
...
position p;
@@
 struct {  unsigned long pteh ;  unsigned long ptel ; } cocci_id@p [ 4 ] ;
...
position p;
@@
 struct hpt_resize_state cocci_id@p ;
...
position p;
@@
 int cocci_id@p [ MMU_PAGE_COUNT ] [ MMU_PAGE_COUNT ] ;
...
position p;
@@
 unsigned char cocci_id@p [ SPLPAR_TLB_BIC_MAXLENGTH ] ;
...
position p;
@@
 struct hvcall_mpp_data * cocci_id@p ;
...
position p;
@@
 unsigned long cocci_id@p [ PPC64_HUGE_HPTE_BATCH ] ;
...
position p;
@@
 struct hvcall_mpp_x_data * cocci_id@p ;
...
position p;
@@
 struct dtl_worker {  struct delayed_work work ;  int cpu ; } cocci_id@p ;
