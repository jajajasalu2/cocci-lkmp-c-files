...
symbol cocci_id;
position p;
typedef uint64_t,uint8_t;
@@
 struct fsi_msg {  uint64_t msg ;  uint8_t bits ; } cocci_id@p ;
...
position p;
@@
 struct genpool_data_fixed cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,uint16_t,uint32_t,uint8_t;
@@
 struct fsi_master_acf {  struct fsi_master master ;  struct device * dev ;  struct regmap * scu ;  struct mutex lock ;  struct gpio_desc * gpio_clk ;  struct gpio_desc * gpio_data ;  struct gpio_desc * gpio_trans ;  struct gpio_desc * gpio_enable ;  struct gpio_desc * gpio_mux ;  uint16_t gpio_clk_vreg ;  uint16_t gpio_clk_dreg ;  uint16_t gpio_dat_vreg ;  uint16_t gpio_dat_dreg ;  uint16_t gpio_tra_vreg ;  uint16_t gpio_tra_dreg ;  uint8_t gpio_clk_bit ;  uint8_t gpio_dat_bit ;  uint8_t gpio_tra_bit ;  uint32_t cf_mem_addr ;  size_t cf_mem_size ;  void __iomem * cf_mem ;  void __iomem * cvic ;  struct gen_pool * sram_pool ;  void __iomem * sram ;  bool is_ast2500 ;  bool external_mode ;  bool trace_enabled ;  uint32_t last_addr ;  uint8_t t_send_delay ;  uint8_t t_echo_delay ;  uint32_t cvic_sw_irq ; } cocci_id@p ;
...
position p;
@@
 struct fsi_master_acf * cocci_id@p ;
