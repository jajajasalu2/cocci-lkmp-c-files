...
typedef dma_addr_t;
@@
 struct isi_dma_desc {  struct list_head list ;  struct fbd * p_fbd ;  dma_addr_t fbd_phys ; } cocci_id@p ;
...
position p;
@@
 const struct isi_format cocci_id@p [ ] ;
...
position p;
@@
 const struct isi_format * cocci_id@p [ ARRAY_SIZE ( isi_formats ) ] ;
...
position p;
@@
 struct frame_buffer * cocci_id@p ;
...
position p;
@@
 struct isi_graph_entity {  struct device_node * node ;  struct v4l2_async_subdev asd ;  struct v4l2_subdev * subdev ; } cocci_id@p ;
...
position p;
@@
 struct isi_dma_desc * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,u16,spinlock_t,dma_addr_t;
@@
 struct atmel_isi {  spinlock_t irqlock ;  struct device * dev ;  void __iomem * regs ;  int sequence ;  struct fbd * p_fb_descriptors ;  dma_addr_t fb_descriptors_phys ;  struct list_head dma_desc_head ;  struct isi_dma_desc dma_desc [ VIDEO_MAX_FRAME ] ;  bool enable_preview_path ;  struct completion complete ;  struct clk * pclk ;  unsigned int irq ;  struct isi_platform_data pdata ;  u16 width_flags ;  struct list_head video_buffer_list ;  struct frame_buffer * active ;  struct v4l2_device v4l2_dev ;  struct video_device * vdev ;  struct v4l2_async_notifier notifier ;  struct isi_graph_entity entity ;  struct v4l2_format fmt ;  const struct isi_format * * user_formats ;  unsigned int num_user_formats ;  const struct isi_format * current_fmt ;  struct mutex lock ;  struct vb2_queue queue ; } cocci_id@p ;
...
typedef u32;
@@
 struct fbd {  u32 fb_address ;  u32 dma_ctrl ;  u32 next_fbd_address ; } cocci_id@p ;
...
position p;
@@
 struct frame_buffer {  struct vb2_v4l2_buffer vb ;  struct isi_dma_desc * p_dma_desc ;  struct list_head list ; } cocci_id@p ;
...
typedef u8,u32;
@@
 struct isi_format {  u32 fourcc ;  u32 mbus_code ;  u8 bpp ;  u32 swap ; } cocci_id@p ;
