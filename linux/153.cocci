...
position p;
@@
 enum writer_wait_state cocci_id@p ;
...
position p;
@@
 enum rwsem_wake_type { RWSEM_WAKE_ANY , RWSEM_WAKE_READERS , RWSEM_WAKE_READ_OWNED , } cocci_id@p ;
...
position p;
@@
 enum rwsem_waiter_type { RWSEM_WAITING_FOR_WRITE , RWSEM_WAITING_FOR_READ , } cocci_id@p ;
...
position p;
@@
 struct rwsem_waiter cocci_id@p ;
...
position p;
@@
 enum writer_wait_state { WRITER_NOT_FIRST , WRITER_FIRST , WRITER_HANDOFF , } cocci_id@p ;
...
position p;
@@
 struct rwsem_waiter {  struct list_head list ;  struct task_struct * task ;  enum rwsem_waiter_type type ;  unsigned long timeout ;  unsigned long last_rowner ; } cocci_id@p ;
...
position p;
@@
 enum rwsem_wake_type cocci_id@p ;
...
position p;
@@
 enum owner_state { OWNER_NULL = 1 << 0 , OWNER_WRITER = 1 << 1 , OWNER_READER = 1 << 2 , OWNER_NONSPINNABLE = 1 << 3 , } cocci_id@p ;
