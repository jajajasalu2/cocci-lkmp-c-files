...
@@
 struct rb_irq_work {  struct irq_work work ;  wait_queue_head_t waiters ;  wait_queue_head_t full_waiters ;  bool waiters_pending ;  bool full_waiters_pending ;  bool wakeup_full ; } cocci_id@p ;
...
position p;
@@
 struct rb_event_info * cocci_id@p ;
...
typedef u64;
@@
 struct rb_event_info {  u64 ts ;  u64 delta ;  unsigned long length ;  struct buffer_page * tail_page ;  int add_timestamp ; } cocci_id@p ;
...
position p;
@@
 struct buffer_data_page * cocci_id@p ;
...
position p;
@@
 enum { RB_CTX_NMI , RB_CTX_IRQ , RB_CTX_SOFTIRQ , RB_CTX_NORMAL , RB_CTX_MAX , } cocci_id@p ;
...
position p;
@@
 struct rb_test_data * cocci_id@p ;
...
position p;
@@
 struct ring_buffer_per_cpu * cocci_id@p ;
...
typedef bool;
@@
 bool cocci_id@p ( struct ring_buffer_per_cpu * cpu_buffer , struct ring_buffer_event * event ) ;
...
typedef local_t;
@@
 struct buffer_page {  struct list_head list ;  local_t write ;  unsigned read ;  local_t entries ;  unsigned long real_end ;  struct buffer_data_page * page ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct ring_buffer_per_cpu * cpu_buffer ) ;
...
typedef u64;
@@
 struct ring_buffer_iter {  struct ring_buffer_per_cpu * cpu_buffer ;  unsigned long head ;  struct buffer_page * head_page ;  struct buffer_page * cache_reader_page ;  unsigned long cache_read ;  u64 read_stamp ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u64,arch_spinlock_t,local_t,atomic_t,raw_spinlock_t;
@@
 struct ring_buffer_per_cpu {  int cpu ;  atomic_t record_disabled ;  struct ring_buffer * buffer ;  raw_spinlock_t reader_lock ;  arch_spinlock_t lock ;  struct lock_class_key lock_key ;  struct buffer_data_page * free_page ;  unsigned long nr_pages ;  unsigned int current_context ;  struct list_head * pages ;  struct buffer_page * head_page ;  struct buffer_page * tail_page ;  struct buffer_page * commit_page ;  struct buffer_page * reader_page ;  unsigned long lost_events ;  unsigned long last_overrun ;  unsigned long nest ;  local_t entries_bytes ;  local_t entries ;  local_t overrun ;  local_t commit_overrun ;  local_t dropped_events ;  local_t committing ;  local_t commits ;  local_t pages_touched ;  local_t pages_read ;  long last_pages_touch ;  size_t shortest_full ;  unsigned long read ;  unsigned long read_bytes ;  u64 write_stamp ;  u64 read_stamp ;  long nr_pages_to_update ;  struct list_head new_pages ;  struct work_struct update_pages_work ;  struct completion update_done ;  struct rb_irq_work irq_work ; } cocci_id@p ;
...
position p;
@@
 struct rb_item {  int size ;  char str [ ] ; } cocci_id@p ;
...
position p;
@@
 enum { RB_LEN_TIME_EXTEND = 8 , RB_LEN_TIME_STAMP = 8 , } cocci_id@p ;
