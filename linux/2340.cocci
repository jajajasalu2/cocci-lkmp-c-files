...
position p;
@@
 struct node_hash {  struct hlist_node node ;  struct device_node * np ; } cocci_id@p ;
...
position p;
@@
 struct overlay_info * cocci_id@p ;
...
position p;
@@
 struct unittest_results {  int passed ;  int failed ; } cocci_id@p ;
...
position p;
@@
 struct unittest_i2c_bus_data {  struct platform_device * pdev ;  struct i2c_adapter adap ; } cocci_id@p ;
...
position p;
@@
 struct {  const char * path ;  const char * data ; } cocci_id@p [ ] ;
...
typedef uint8_t;
@@
 struct overlay_info {  uint8_t * dtb_begin ;  uint8_t * dtb_end ;  int expected_result ;  int overlay_id ;  char * name ; } cocci_id@p ;
...
position p;
@@
 struct unittest_i2c_bus_data * cocci_id@p ;
...
position p;
@@
 unsigned long cocci_id@p [ BITS_TO_LONGS ( MAX_UNITTEST_OVERLAYS ) ] ;
...
position p;
@@
 enum overlay_type { PDEV_OVERLAY , I2C_OVERLAY , } cocci_id@p ;
...
position p;
@@
 struct of_changeset cocci_id@p ;
