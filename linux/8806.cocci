...
typedef u_char;
@@
 int cocci_id@p ( struct pcmcia_device * link , u_char * node_id ) ;
...
position p;
@@
 enum cardtype { MBH10302 , MBH10304 , TDK , CONTEC , LA501 , UNGERMANN , XXX10304 , NEC , KME , } cocci_id@p ;
...
position p;
@@
 const unsigned int cocci_id@p [ 5 ] ;
...
symbol cocci_id;
position p;
typedef u_short,u_char,uint;
@@
 struct local_info {  struct pcmcia_device * p_dev ;  long open_time ;  uint tx_started : 1 ;  uint tx_queue ;  u_short tx_queue_len ;  enum cardtype cardtype ;  u_short sent ;  u_char __iomem * base ; } cocci_id@p ;
