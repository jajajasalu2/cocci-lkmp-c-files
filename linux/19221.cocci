...
position p;
@@
 struct keyboard_state {  unsigned char buf [ 6 ] ;  int len ;  KB_STATE_T state ; } cocci_id@p ;
...
position p;
@@
 void ( * cocci_id@p ) ( unsigned char , char ) ;
...
typedef kb_state_t;
@@
 enum kb_state_t { KEYBOARD , AMOUSE , RMOUSE , JOYSTICK , CLOCK , RESYNC , } cocci_id@p ;
...
position p;
@@
 unsigned long cocci_id@p [ 128 / ( sizeof ( unsigned long ) * 8 ) ] ;
