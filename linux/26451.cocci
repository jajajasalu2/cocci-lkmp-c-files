...
position p;
@@
 struct avc_opcode_info {  const char * name ;  struct avc_field fields [ 8 ] ; } cocci_id@p ;
...
position p;
@@
 const struct avc_opcode_info cocci_id@p [ 256 ] ;
...
position p;
@@
 struct avc_field {  const char * name ;  int offset ;  int width ;  struct avc_enum * names ; } cocci_id@p ;
...
position p;
@@
 struct avc_enum {  int value ;  const char * name ; } cocci_id@p ;
...
position p;
@@
 const struct avc_opcode_info * cocci_id@p ;
...
typedef uint32_t;
@@
 struct avc_frame {  uint32_t operand0 : 8 ;  uint32_t opcode : 8 ;  uint32_t subunit_id : 3 ;  uint32_t subunit_type : 5 ;  uint32_t ctype : 4 ;  uint32_t cts : 4 ; } cocci_id@p ;
