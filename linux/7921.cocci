...
position p;
@@
 struct phy_device * cocci_id@p [ PHY_MAX_ADDR ] ;
...
typedef u16;
@@
 struct dsa_loop_priv {  struct mii_bus * bus ;  unsigned int port_base ;  struct dsa_loop_vlan vlans [ DSA_LOOP_VLANS ] ;  struct net_device * netdev ;  struct dsa_loop_port ports [ DSA_MAX_PORTS ] ;  u16 pvid ; } cocci_id@p ;
...
position p;
@@
 struct dsa_loop_mib_entry cocci_id@p [ ] ;
...
typedef u16;
@@
 struct dsa_loop_vlan {  u16 members ;  u16 untagged ; } cocci_id@p ;
...
position p;
@@
 struct dsa_loop_mib_entry {  char name [ ETH_GSTRING_LEN ] ;  unsigned long val ; } cocci_id@p ;
...
position p;
@@
 enum dsa_loop_mib_counters { DSA_LOOP_PHY_READ_OK , DSA_LOOP_PHY_READ_ERR , DSA_LOOP_PHY_WRITE_OK , DSA_LOOP_PHY_WRITE_ERR , __DSA_LOOP_CNT_MAX , } cocci_id@p ;
...
position p;
@@
 struct dsa_loop_vlan * cocci_id@p ;
...
position p;
@@
 struct dsa_loop_pdata * cocci_id@p ;
...
position p;
@@
 struct dsa_loop_port {  struct dsa_loop_mib_entry mib [ __DSA_LOOP_CNT_MAX ] ; } cocci_id@p ;
...
position p;
@@
 struct dsa_loop_priv * cocci_id@p ;
