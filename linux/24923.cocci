...
position p;
@@
 struct nlattr * cocci_id@p [ TIPC_NLA_SOCK_MAX + 1 ] ;
...
typedef bool;
@@
 int cocci_id@p ( struct socket * sock , struct socket * new_sock , int flags , bool kern ) ;
...
position p;
@@
 struct sockaddr_tipc * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct tipc_sock * tsk ) ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct sk_buff * skb , struct netlink_callback * cb , struct tipc_sock * tsk ) ;
...
position p;
@@
 struct tipc_mc_method * cocci_id@p ;
...
position p;
@@
 struct sockaddr_pair {  struct sockaddr_tipc sock ;  struct sockaddr_tipc member ; } cocci_id@p ;
...
position p;
@@
 enum { TIPC_LISTEN = TCP_LISTEN , TIPC_ESTABLISHED = TCP_ESTABLISHED , TIPC_OPEN = TCP_CLOSE , TIPC_DISCONNECTING = TCP_CLOSE_WAIT , TIPC_CONNECTING = TCP_SYN_SENT , } cocci_id@p ;
...
position p;
@@
 struct tipc_sioc_nodeid_req cocci_id@p ;
...
typedef uint;
@@
 int cocci_id@p ( struct tipc_sock * tsk , uint scope , const struct tipc_name_seq * seq ) ;
...
position p;
@@
 void cocci_id@p ( struct tipc_sock * tsk ) ;
...
position p;
@@
 int cocci_id@p ( struct socket * sock ) ;
...
position p;
@@
 struct tipc_sioc_ln_req cocci_id@p ;
...
typedef u32;
@@
 struct tipc_sock * cocci_id@p ( struct net * net , u32 portid ) ;
...
position p;
@@
 struct tipc_name_seq cocci_id@p ;
...
position p;
@@
 const struct tipc_name_seq * cocci_id@p ;
...
position p;
@@
 struct tipc_group_req * cocci_id@p ;
...
position p;
@@
 struct sockaddr_pair * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct sock * sk , struct sk_buff * skb ) ;
