...
position p;
@@
 struct work_queue_wrapper cocci_id@p ;
...
position p;
@@
 struct spi_internal * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct transport_container * tc , struct device * dev , struct device * cdev ) ;
...
position p;
@@
 struct {  enum spi_signal_type value ;  char * name ; } cocci_id@p [ ] ;
...
position p;
@@
 struct spi_host_attrs cocci_id@p ;
...
position p;
@@
 struct work_queue_wrapper {  struct work_struct work ;  struct scsi_device * sdev ; } cocci_id@p ;
...
position p;
@@
 struct spi_transport_attrs * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef blist_flags_t;
@@
 blist_flags_t cocci_id@p ;
...
typedef u8;
@@
 enum spi_compare_returns ( * cocci_id@p ) ( struct scsi_device * , u8 * , u8 * , int ) ;
...
position p;
@@
 enum spi_compare_returns { SPI_COMPARE_SUCCESS , SPI_COMPARE_FAILURE , SPI_COMPARE_SKIP_TEST , } cocci_id@p ;
...
position p;
@@
 struct spi_internal {  struct scsi_transport_template t ;  struct spi_function_template * f ; } cocci_id@p ;
...
position p;
@@
 enum spi_compare_returns cocci_id@p ;
...
typedef blist_flags_t;
@@
 enum { SPI_BLIST_NOIUS = ( __force blist_flags_t ) 0x1 , } cocci_id@p ;
