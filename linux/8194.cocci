...
position p;
@@
 const struct orion_mdio_ops cocci_id@p ;
...
position p;
@@
 struct orion_mdio_ops {  int ( * is_done ) ( struct orion_mdio_dev * ) ;  unsigned int poll_interval_min ;  unsigned int poll_interval_max ; } cocci_id@p ;
...
position p;
@@
 struct orion_mdio_dev * cocci_id@p ;
...
position p;
@@
 enum orion_mdio_bus_type cocci_id@p ;
...
position p;
@@
 enum orion_mdio_bus_type { BUS_TYPE_SMI , BUS_TYPE_XSMI , } cocci_id@p ;
...
@@
 struct orion_mdio_dev {  void __iomem * regs ;  struct clk * clk [ 4 ] ;  int err_interrupt ;  wait_queue_head_t smi_busy_wait ; } cocci_id@p ;
