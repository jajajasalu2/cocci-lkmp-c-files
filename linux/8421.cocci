...
position p;
@@
 struct ptp_skb_ring * cocci_id@p ;
...
position p;
@@
 const struct aq_hw_ops * cocci_id@p ;
...
position p;
@@
 const struct hw_atl_ptp_offset * cocci_id@p ;
...
position p;
@@
 struct hw_atl_utils_mbox cocci_id@p ;
...
position p;
@@
 struct ptp_pin_desc cocci_id@p [ MAX_PTP_GPIO_COUNT ] ;
...
position p;
@@
 enum ptp_pin_function cocci_id@p ;
...
position p;
@@
 struct ptp_tm_offset {  unsigned int mbps ;  int egress ;  int ingress ; } cocci_id@p ;
...
@@
 struct ptp_tx_timeout {  spinlock_t lock ;  bool active ;  unsigned long tx_start ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u64,bool,u32,atomic_t,spinlock_t;
@@
 struct aq_ptp_s {  struct aq_nic_s * aq_nic ;  struct hwtstamp_config hwtstamp_config ;  spinlock_t ptp_lock ;  spinlock_t ptp_ring_lock ;  struct ptp_clock * ptp_clock ;  struct ptp_clock_info ptp_info ;  atomic_t offset_egress ;  atomic_t offset_ingress ;  struct aq_ring_param_s ptp_ring_param ;  struct ptp_tx_timeout ptp_tx_timeout ;  unsigned int idx_vector ;  struct napi_struct napi ;  struct aq_ring_s ptp_tx ;  struct aq_ring_s ptp_rx ;  struct aq_ring_s hwts_rx ;  struct ptp_skb_ring skb_ring ;  struct aq_rx_filter_l3l4 udp_filter ;  struct aq_rx_filter_l2 eth_type_filter ;  struct delayed_work poll_sync ;  u32 poll_timeout_ms ;  bool extts_pin_enabled ;  u64 last_sync1588_ts ; } cocci_id@p ;
...
position p;
@@
 struct ptp_tx_timeout * cocci_id@p ;
...
position p;
@@
 struct ptp_pin_desc cocci_id@p ;
...
position p;
@@
 struct ptp_tm_offset cocci_id@p [ 6 ] ;
...
typedef spinlock_t;
@@
 struct ptp_skb_ring {  struct sk_buff * * buff ;  spinlock_t lock ;  unsigned int size ;  unsigned int head ;  unsigned int tail ; } cocci_id@p ;
...
position p;
@@
 struct ptp_clock_time * cocci_id@p ;
...
position p;
@@
 enum ptp_speed_offsets { ptp_offset_idx_10 = 0 , ptp_offset_idx_100 , ptp_offset_idx_1000 , ptp_offset_idx_2500 , ptp_offset_idx_5000 , ptp_offset_idx_10000 , } cocci_id@p ;
