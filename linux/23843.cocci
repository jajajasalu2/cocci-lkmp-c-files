...
typedef u32;
@@
 struct route4_fastmap {  struct route4_filter * filter ;  u32 id ;  int iif ; } cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_ROUTE4_MAX + 1 ] ;
...
position p;
@@
 struct route4_bucket {  struct route4_filter __rcu * ht [ 16 + 16 + 1 ] ;  struct rcu_head rcu ; } cocci_id@p ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_ROUTE4_MAX + 1 ] ;
...
typedef u32;
@@
 struct route4_filter {  struct route4_filter __rcu * next ;  u32 id ;  int iif ;  struct tcf_result res ;  struct tcf_exts exts ;  u32 handle ;  struct route4_bucket * bkt ;  struct tcf_proto * tp ;  struct rcu_work rwork ; } cocci_id@p ;
...
position p;
@@
 struct route4_bucket cocci_id@p ;
...
position p;
@@
 struct route4_head {  struct route4_fastmap fastmap [ 16 ] ;  struct route4_bucket __rcu * table [ 256 + 1 ] ;  struct rcu_head rcu ; } cocci_id@p ;
...
position p;
@@
 struct route4_filter * cocci_id@p ;
