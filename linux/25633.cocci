...
position p;
@@
 struct symbol_name_rb_node * cocci_id@p ;
...
position p;
@@
 enum symbol_tag_include cocci_id@p ;
...
position p;
@@
 struct kcore_mapfn_data {  struct dso * dso ;  struct list_head maps ; } cocci_id@p ;
...
position p;
@@
 struct kcore_mapfn_data cocci_id@p ;
...
position p;
@@
 unsigned char cocci_id@p [ BUILD_ID_SIZE ] ;
...
typedef bool;
@@
 bool cocci_id@p ( const char * name ) ;
...
position p;
@@
 int cocci_id@p ( struct dso * dso , struct map * map ) ;
...
typedef u64;
@@
 struct module_info {  struct rb_node rb_node ;  char * name ;  u64 start ; } cocci_id@p ;
...
typedef u64;
@@
 int ( * cocci_id@p ) ( void * arg , const char * name , u64 start , u64 size ) ;
