...
position p;
@@
 struct fis_image_desc * cocci_id@p ;
...
typedef uint32_t;
@@
 struct fis_image_desc {  unsigned char name [ 16 ] ;  uint32_t flash_base ;  uint32_t mem_base ;  uint32_t size ;  uint32_t entry_point ;  uint32_t data_length ;  unsigned char _pad [ 256 - ( 16 + 7 * sizeof ( uint32_t ) ) ] ;  uint32_t desc_cksum ;  uint32_t file_cksum ; } cocci_id@p ;
...
position p;
@@
 struct fis_list {  struct fis_image_desc * img ;  struct fis_list * next ; } cocci_id@p ;
