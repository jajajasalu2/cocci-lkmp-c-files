...
position p;
@@
 struct vscsifrnt_shadow cocci_id@p ;
...
position p;
@@
 struct vscsifrnt_info * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef wait_queue_head_t,spinlock_t,grant_ref_t;
@@
 struct vscsifrnt_info {  struct xenbus_device * dev ;  struct Scsi_Host * host ;  int host_active ;  unsigned int evtchn ;  unsigned int irq ;  grant_ref_t ring_ref ;  struct vscsiif_front_ring ring ;  struct vscsiif_response ring_rsp ;  spinlock_t shadow_lock ; 
	 DECLARE_BITMAP ( shadow_free_bitmap , VSCSIIF_MAX_REQS ) 
		 ;  struct vscsifrnt_shadow * shadow [ VSCSIIF_MAX_REQS ] ;  wait_queue_head_t wq_sync ;  wait_queue_head_t wq_pause ;  unsigned int wait_ring_available : 1 ;  unsigned int waiting_pause : 1 ;  unsigned int pause : 1 ;  unsigned callers ;  char dev_state_path [ 64 ] ;  struct task_struct * curr ; } cocci_id@p ;
...
position p;
@@
 struct vscsiif_front_ring * cocci_id@p ;
