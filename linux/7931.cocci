...
position p;
@@
 enum qmi_wwan_flags { QMI_WWAN_FLAG_RAWIP = 1 << 0 , QMI_WWAN_FLAG_MUX = 1 << 1 , } cocci_id@p ;
...
typedef u8;
@@
 struct qmimux_priv {  struct net_device * real_dev ;  u8 mux_id ;  struct pcpu_sw_netstats __percpu * stats64 ; } cocci_id@p ;
...
position p;
@@
 enum qmi_wwan_quirks { QMI_WWAN_QUIRK_DTR = 1 << 0 , QMI_WWAN_QUIRK_QUECTEL_DYNCFG = 1 << 1 , } cocci_id@p ;
...
position p;
@@
 struct qmi_wwan_state cocci_id@p ;
...
typedef u8;
@@
 struct qmimux_hdr {  u8 pad ;  u8 mux_id ;  __be16 pkt_len ; } cocci_id@p ;
...
typedef atomic_t;
@@
 struct qmi_wwan_state {  struct usb_driver * subdriver ;  atomic_t pmcount ;  unsigned long flags ;  struct usb_interface * control ;  struct usb_interface * data ; } cocci_id@p ;
