...
position p;
@@
 char cocci_id@p [ MAX_WORD_SIZE + 1 ] ;
...
typedef u8;
@@
 struct wilco_ec_debugfs {  struct wilco_ec_device * ec ;  struct dentry * dir ;  size_t response_size ;  u8 raw_data [ EC_MAILBOX_DATA_SIZE ] ;  u8 formatted_data [ FORMATTED_BUFFER_SIZE ] ; } cocci_id@p ;
...
position p;
@@
 struct wilco_ec_debugfs * cocci_id@p ;
...
typedef u8;
@@
 u8 cocci_id@p [ TYPE_AND_DATA_SIZE ] ;
