...
position p;
@@
 struct tegra_actmon_emc_ratio {  unsigned long cpu_freq ;  unsigned long emc_freq ; } cocci_id@p ;
...
typedef u32;
@@
 struct tegra_devfreq_device_config {  u32 offset ;  u32 irq_mask ;  unsigned int boost_up_coeff ;  unsigned int boost_down_coeff ;  unsigned int boost_up_threshold ;  unsigned int boost_down_threshold ;  u32 avg_dependency_threshold ; } cocci_id@p ;
...
position p;
@@
 const struct tegra_actmon_emc_ratio * cocci_id@p ;
...
position p;
@@
 struct tegra_devfreq * cocci_id@p ;
...
typedef u32;
@@
 struct tegra_devfreq_device {  const struct tegra_devfreq_device_config * config ;  void __iomem * regs ;  u32 avg_count ;  unsigned long boost_freq ;  unsigned long target_freq ; } cocci_id@p ;
...
position p;
@@
 const struct tegra_devfreq_device_config cocci_id@p [ ] ;
...
position p;
@@
 struct tegra_devfreq_device * cocci_id@p ;
...
typedef bool;
@@
 struct tegra_devfreq {  struct devfreq * devfreq ;  struct reset_control * reset ;  struct clk * clock ;  void __iomem * regs ;  struct clk * emc_clock ;  unsigned long max_freq ;  unsigned long cur_freq ;  struct notifier_block clk_rate_change_nb ;  struct delayed_work cpufreq_update_work ;  struct notifier_block cpu_rate_change_nb ;  struct tegra_devfreq_device devices [ ARRAY_SIZE ( actmon_device_configs ) ] ;  unsigned int irq ;  bool started ; } cocci_id@p ;
...
position p;
@@
 enum tegra_actmon_device { MCALL = 0 , MCCPU , } cocci_id@p ;
...
position p;
@@
 struct devfreq_governor cocci_id@p ;
