...
position p;
@@
 enum dfw_cmd { DFW_CMD_FUSE = 0x01 , DFW_CMD_CODE_HDR , DFW_CMD_CODE_DATA , DFW_CMD_PLL , DFW_CMD_INFO = 0xff , } cocci_id@p ;
...
@@
 struct wm0010_priv {  struct snd_soc_component * component ;  struct mutex lock ;  struct device * dev ;  struct wm0010_pdata pdata ;  int gpio_reset ;  int gpio_reset_value ;  struct regulator_bulk_data core_supplies [ 2 ] ;  struct regulator * dbvdd ;  int sysclk ;  enum wm0010_state state ;  bool boot_failed ;  bool ready ;  bool pll_running ;  int max_spi_freq ;  int board_max_spi_speed ;  u32 pll_clkctrl1 ;  spinlock_t irq_lock ;  int irq ;  struct completion boot_completion ; } cocci_id@p ;
...
position p;
@@
 struct wm0010_boot_xfer {  struct list_head list ;  struct snd_soc_component * component ;  struct completion * done ;  struct spi_message m ;  struct spi_transfer t ; } cocci_id@p ;
...
typedef u8;
@@
 struct dfw_inforec {  u8 info_version ;  u8 tool_major_version ;  u8 tool_minor_version ;  u8 dsp_target ; } cocci_id@p ;
...
typedef u8;
@@
 struct wm0010_spi_msg {  struct spi_message m ;  struct spi_transfer t ;  u8 * tx_buf ;  u8 * rx_buf ;  size_t len ; } cocci_id@p ;
...
position p;
@@
 const struct dfw_binrec * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct snd_soc_component * component ) ;
...
position p;
@@
 const struct dfw_inforec * cocci_id@p ;
...
position p;
@@
 struct wm0010_boot_xfer * cocci_id@p ;
...
typedef u32;
@@
 struct pll_clock_map {  int max_sysclk ;  int max_pll_spi_speed ;  u32 pll_clkctrl1 ; } cocci_id@p [ ] ;
...
position p;
@@
 enum wm0010_state { WM0010_POWER_OFF , WM0010_OUT_OF_RESET , WM0010_BOOTROM , WM0010_STAGE2 , WM0010_FIRMWARE , } cocci_id@p ;
