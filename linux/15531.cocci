...
position p;
@@
 struct dw_i3c_xfer {  struct list_head node ;  struct completion comp ;  int ret ;  unsigned int ncmds ;  struct dw_i3c_cmd cmds [ 0 ] ; } cocci_id@p ;
...
position p;
@@
 struct dw_i3c_master cocci_id@p ;
...
typedef u8;
@@
 struct dw_i3c_master_caps {  u8 cmdfifodepth ;  u8 datafifodepth ; } cocci_id@p ;
...
typedef u8;
@@
 struct dw_i3c_i2c_dev_data {  u8 index ; } cocci_id@p ;
...
@@
 struct dw_i3c_master {  struct i3c_master_controller base ;  u16 maxdevs ;  u16 datstartaddr ;  u32 free_pos ;  struct {  struct list_head list ;  struct dw_i3c_xfer * cur ;  spinlock_t lock ; } xferqueue ;  struct dw_i3c_master_caps caps ;  void __iomem * regs ;  struct reset_control * core_rst ;  struct clk * core_clk ;  char version [ 5 ] ;  char type [ 5 ] ;  u8 addrs [ MAX_DEVS ] ; } cocci_id@p ;
...
typedef u8,u32,u16;
@@
 struct dw_i3c_cmd {  u32 cmd_lo ;  u32 cmd_hi ;  u16 tx_len ;  const void * tx_buf ;  u16 rx_len ;  void * rx_buf ;  u8 error ; } cocci_id@p ;
...
position p;
@@
 struct dw_i3c_i2c_dev_data * cocci_id@p ;
