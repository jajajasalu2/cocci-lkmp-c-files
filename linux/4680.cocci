...
position p;
@@
 char cocci_id@p [ BREAK_INSTR_SIZE ] ;
...
position p;
@@
 unsigned long cocci_id@p [ ( NUMREGBYTES + sizeof ( unsigned long ) - 1 ) / sizeof ( unsigned long ) ] ;
...
position p;
@@
 struct test_state {  char * name ;  struct test_struct * tst ;  int idx ;  int ( * run_test ) ( int , int ) ;  int ( * validate_put ) ( char * ) ; } cocci_id@p ;
...
position p;
@@
 struct test_struct {  char * get ;  char * put ;  void ( * get_handler ) ( char * ) ;  int ( * put_handler ) ( char * , char * ) ; } cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ MAX_CONFIG_LEN ] ;
