...
position p;
@@
 struct test_case_data cocci_id@p [ NR_CPUS ] [ ARRAY_SIZE ( test_case_array ) ] ;
...
position p;
@@
 struct test_driver {  struct task_struct * task ;  unsigned long start ;  unsigned long stop ;  int cpu ; } cocci_id@p [ NR_CPUS ] ;
...
position p;
@@
 struct test_case_desc {  const char * test_name ;  int ( * test_func ) ( void ) ; } cocci_id@p ;
...
typedef u64;
@@
 struct test_case_data {  int test_failed ;  int test_passed ;  u64 time ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p [ ARRAY_SIZE ( test_case_array ) ] ;
...
position p;
@@
 struct test_case_desc cocci_id@p [ ] ;
