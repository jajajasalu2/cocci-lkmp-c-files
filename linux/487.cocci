...
position p;
@@
 struct bdi_writeback_congested * cocci_id@p ;
...
position p;
@@
 struct cgroup_subsys_state * const cocci_id@p ;
...
position p;
@@
 struct blkg_iostat_set cocci_id@p ;
...
position p;
@@
 const struct blkcg_policy * cocci_id@p ;
...
typedef u64;
@@
 u64 ( * cocci_id@p ) ( struct seq_file * , struct blkg_policy_data * , int ) ;
...
position p;
@@
 struct blkcg_policy * cocci_id@p [ BLKCG_MAX_POLS ] ;
