...
position p;
@@
 struct basic_head {  struct list_head flist ;  struct idr handle_idr ;  struct rcu_head rcu ; } cocci_id@p ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_BASIC_MAX + 1 ] ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_BASIC_MAX + 1 ] ;
...
position p;
@@
 struct basic_filter * cocci_id@p ;
...
typedef u32;
@@
 struct basic_filter {  u32 handle ;  struct tcf_exts exts ;  struct tcf_ematch_tree ematches ;  struct tcf_result res ;  struct tcf_proto * tp ;  struct list_head link ;  struct tc_basic_pcnt __percpu * pf ;  struct rcu_work rwork ; } cocci_id@p ;
...
position p;
@@
 struct tc_basic_pcnt cocci_id@p ;
