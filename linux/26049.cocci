...
position p;
@@
 union {  char buf [ CMSG_SPACE ( sizeof ( struct in6_pktinfo ) ) ] ;  struct cmsghdr align ; } cocci_id@p ;
...
position p;
@@
 struct sock_addr_test {  const char * descr ;  load_fn loadfn ;  enum bpf_attach_type expected_attach_type ;  enum bpf_attach_type attach_type ;  int domain ;  int type ;  const char * requested_ip ;  unsigned short requested_port ;  const char * expected_ip ;  unsigned short expected_port ;  const char * expected_src_ip ;  enum { LOAD_REJECT , ATTACH_REJECT , ATTACH_OKAY , SYSCALL_EPERM , SYSCALL_ENOTSUPP , SUCCESS , } expected_result ; } cocci_id@p ;
...
position p;
@@
 const struct sockaddr_storage * cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ BPF_LOG_BUF_SIZE ] ;
...
position p;
@@
 int cocci_id@p ( const struct sock_addr_test * test ) ;
...
@@
 union {  uint8_t u4_addr8 [ 4 ] ;  uint16_t u4_addr16 [ 2 ] ;  uint32_t u4_addr32 ; } cocci_id@p ;
...
position p;
@@
 struct sock_addr_test cocci_id@p ;
...
position p;
@@
 union {  char buf [ CMSG_SPACE ( sizeof ( struct in_pktinfo ) ) ] ;  struct cmsghdr align ; } cocci_id@p ;
...
position p;
@@
 const struct sock_addr_test * cocci_id@p ;
