...
position p;
@@
 union aqm_grp_execmsk_hi cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct pci_dev * pdev , int num_vfs ) ;
...
position p;
@@
 union aqm_grp_execmsk_lo cocci_id@p ;
...
@@
 struct ucode {  u8 id ;  char version [ VERSION_LEN - 1 ] ;  __be32 code_size ;  u8 raz [ 12 ] ;  u64 code [ 0 ] ; } cocci_id@p ;
...
position p;
@@
 union ucd_core_eid_ucode_block_num cocci_id@p ;
