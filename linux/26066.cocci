...
position p;
@@
 struct sock_test {  const char * descr ;  struct bpf_insn insns [ MAX_INSNS ] ;  enum bpf_attach_type expected_attach_type ;  enum bpf_attach_type attach_type ;  int domain ;  int type ;  const char * ip ;  unsigned short port ;  enum { LOAD_REJECT , ATTACH_REJECT , BIND_REJECT , SUCCESS , } result ; } cocci_id@p ;
...
position p;
@@
 const struct sock_test * cocci_id@p ;
