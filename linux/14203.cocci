...
typedef u32;
@@
 struct rcar_drif_format {  u32 pixelformat ;  u32 buffersize ;  u32 bitlen ;  u32 wdcnt ;  u32 num_ch ; } cocci_id@p ;
...
position p;
@@
 struct rcar_drif_frame_buf cocci_id@p ;
...
position p;
@@
 struct rcar_drif_hwbuf * cocci_id@p [ RCAR_DRIF_MAX_CHANNEL ] ;
...
position p;
@@
 struct rcar_drif_hwbuf {  void * addr ;  unsigned int status ; } cocci_id@p ;
...
position p;
@@
 struct rcar_drif_frame_buf {  struct vb2_v4l2_buffer vb ;  struct list_head list ; } cocci_id@p ;
...
position p;
@@
 struct rcar_drif_sdr cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,resource_size_t,dma_addr_t;
@@
 struct rcar_drif {  struct rcar_drif_sdr * sdr ;  struct platform_device * pdev ;  void __iomem * base ;  resource_size_t start ;  struct dma_chan * dmach ;  struct clk * clk ;  struct rcar_drif_hwbuf buf [ RCAR_DRIF_NUM_HWBUFS ] ;  dma_addr_t dma_handle ;  unsigned int num ;  bool acting_sdr ; } cocci_id@p ;
...
position p;
@@
 struct rcar_drif_graph_ep {  struct v4l2_subdev * subdev ;  struct v4l2_async_subdev asd ; } cocci_id@p ;
...
@@
 struct rcar_drif_sdr {  struct device * dev ;  struct video_device * vdev ;  struct v4l2_device v4l2_dev ;  struct vb2_queue vb_queue ;  struct list_head queued_bufs ;  spinlock_t queued_bufs_lock ;  spinlock_t dma_lock ;  struct mutex v4l2_mutex ;  struct mutex vb_queue_mutex ;  struct v4l2_ctrl_handler ctrl_hdl ;  struct v4l2_async_notifier notifier ;  struct rcar_drif_graph_ep ep ;  const struct rcar_drif_format * fmt ;  u32 mdr1 ;  struct rcar_drif * ch [ RCAR_DRIF_MAX_CHANNEL ] ;  unsigned long hw_ch_mask ;  unsigned long cur_ch_mask ;  u32 num_hw_ch ;  u32 num_cur_ch ;  u32 hwbuf_size ;  u32 produced ; } cocci_id@p ;
...
position p;
@@
 struct rcar_drif_hwbuf * * cocci_id@p ;
...
position p;
@@
 const struct rcar_drif_format cocci_id@p [ ] ;
