...
symbol cocci_id;
position p;
typedef cfi_cmdset_fn_t;
@@
 cfi_cmdset_fn_t cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct map_info * map , struct chip_probe * cp , struct cfi_private * cfi ) ;
...
position p;
@@
 struct cfi_private * cocci_id@p ( struct map_info * map , struct chip_probe * cp ) ;
