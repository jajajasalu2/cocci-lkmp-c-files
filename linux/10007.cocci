...
typedef u8,u16;
@@
 void cocci_id@p ( struct net_device * dev , u16 type_subtype , char * body , int body_len , u8 * addr , u16 tx_cb_idx ) ;
...
typedef u8;
@@
 struct sta_info * cocci_id@p ( struct ap_data * ap , u8 * sta ) ;
...
position p;
@@
 struct wds_oper_data * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct net_device * dev , struct sta_info * sta ) ;
...
position p;
@@
 struct hostap_tx_data * cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ 8 + WLAN_AUTH_CHALLENGE_LEN ] ;
...
position p;
@@
 struct add_sta_proc_data * cocci_id@p ;
...
position p;
@@
 struct prism2_hostapd_param * cocci_id@p ;
...
position p;
@@
 struct mac_restrictions * cocci_id@p ;
