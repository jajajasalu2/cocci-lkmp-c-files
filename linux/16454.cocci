...
position p;
@@
 const char * cocci_id@p [ WIIMOTE_EXT_NUM ] ;
...
position p;
@@
 const char * cocci_id@p [ WIIMOTE_DEV_NUM ] ;
...
typedef __u8;
@@
 struct wiiproto_handler {  __u8 id ;  size_t size ;  void ( * func ) ( struct wiimote_data * wdata , const __u8 * payload ) ; } cocci_id@p ;
...
position p;
@@
 struct wiimote_queue * cocci_id@p ;
...
typedef __u8;
@@
 const __u8 * const cocci_id@p [ WIIMOTE_DEV_NUM ] ;
...
position p;
@@
 struct wiiproto_handler cocci_id@p [ ] ;
