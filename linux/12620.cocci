...
position p;
@@
 struct mdp4_lvds_pll cocci_id@p ;
...
position p;
@@
 const struct pll_rate cocci_id@p [ ] ;
...
position p;
@@
 struct mdp4_lvds_pll {  struct clk_hw pll_hw ;  struct drm_device * dev ;  unsigned long pixclk ; } cocci_id@p ;
...
typedef uint32_t;
@@
 struct pll_rate {  unsigned long rate ;  struct {  uint32_t val ;  uint32_t reg ; } conf [ 32 ] ; } cocci_id@p ;
