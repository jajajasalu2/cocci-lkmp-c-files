...
position p;
@@
 struct grave_page {  struct list_head list ;  unsigned int page_no ;  unsigned int max_reads ;  unsigned int reads_done ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u_char,uint64_t,uint16_t,uint32_t;
@@
 struct nandsim {  struct nand_chip chip ;  struct nand_controller base ;  struct mtd_partition partitions [ CONFIG_NANDSIM_MAX_PARTS ] ;  unsigned int nbparts ;  uint busw ;  u_char ids [ 8 ] ;  uint32_t options ;  uint32_t state ;  uint32_t nxstate ;  uint32_t * op ;  uint32_t pstates [ NS_MAX_PREVSTATES ] ;  uint16_t npstates ;  uint16_t stateidx ;  union ns_mem * pages ;  struct kmem_cache * nand_pages_slab ;  union ns_mem buf ;  struct {  uint64_t totsz ;  uint32_t secsz ;  uint pgsz ;  uint oobsz ;  uint64_t totszoob ;  uint pgszoob ;  uint secszoob ;  uint pgnum ;  uint pgsec ;  uint secshift ;  uint pgshift ;  uint pgaddrbytes ;  uint secaddrbytes ;  uint idbytes ; } geom ;  struct {  unsigned command ;  u_char status ;  uint row ;  uint column ;  uint count ;  uint num ;  uint off ; } regs ;  struct {  int ce ;  int cle ;  int ale ;  int wp ; } lines ;  struct file * cfile ;  unsigned long * pages_written ;  void * file_buf ;  struct page * held_pages [ NS_MAX_HELD_PAGES ] ;  int held_cnt ; } cocci_id@p ;
...
position p;
@@
 struct nand_memory_organization * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u_char,uint16_t;
@@
 union ns_mem {  u_char * byte ;  uint16_t * word ; } cocci_id@p ;
...
position p;
@@
 struct weak_block {  struct list_head list ;  unsigned int erase_block_no ;  unsigned int max_erases ;  unsigned int erases_done ; } cocci_id@p ;
...
typedef uint32_t;
@@
 struct nandsim_operations {  uint32_t reqopts ;  uint32_t states [ NS_OPER_STATES ] ; } cocci_id@p [ NS_OPER_NUM ] ;
...
position p;
@@
 struct weak_page {  struct list_head list ;  unsigned int page_no ;  unsigned int max_writes ;  unsigned int writes_done ; } cocci_id@p ;
...
position p;
@@
 unsigned long cocci_id@p [ CONFIG_NANDSIM_MAX_PARTS ] ;
