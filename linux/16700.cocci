...
position p;
@@
 struct omap4_keypad * cocci_id@p ;
...
position p;
@@
 enum { KBD_REVISION_OMAP4 = 0 , KBD_REVISION_OMAP5 , } cocci_id@p ;
...
position p;
@@
 unsigned char cocci_id@p [ ARRAY_SIZE ( keypad_data -> key_state ) ] ;
...
typedef bool,u32;
@@
 struct omap4_keypad {  struct input_dev * input ;  void __iomem * base ;  bool irq_wake_enabled ;  unsigned int irq ;  unsigned int rows ;  unsigned int cols ;  u32 reg_offset ;  u32 irqreg_offset ;  unsigned int row_shift ;  bool no_autorepeat ;  unsigned char key_state [ 8 ] ;  unsigned short * keymap ; } cocci_id@p ;
