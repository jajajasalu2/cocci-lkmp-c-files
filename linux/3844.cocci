...
position p;
@@
 enum cc_req_dma_buf_type cocci_id@p ;
...
@@
 struct cc_xcbc_s {  u8 * xcbc_keys ;  dma_addr_t xcbc_keys_dma_addr ; } cocci_id@p ;
...
position p;
@@
 struct aead_req_ctx cocci_id@p ;
...
position p;
@@
 struct cc_crypto_alg cocci_id@p ;
...
position p;
@@
 struct cc_alg_template cocci_id@p [ ] ;
...
typedef u8,dma_addr_t;
@@
 struct cc_aead_ctx {  struct cc_drvdata * drvdata ;  u8 ctr_nonce [ MAX_NONCE_SIZE ] ;  u8 * enckey ;  dma_addr_t enckey_dma_addr ;  union {  struct cc_hmac_s hmac ;  struct cc_xcbc_s xcbc ; } auth_state ;  unsigned int enc_keylen ;  unsigned int auth_keylen ;  unsigned int authsize ;  unsigned int hash_len ;  enum drv_cipher_mode cipher_mode ;  enum cc_flow_mode flow_mode ;  enum drv_hash_mode auth_mode ; } cocci_id@p ;
...
typedef u8,dma_addr_t;
@@
 struct cc_hmac_s {  u8 * padded_authkey ;  u8 * ipad_opad ;  dma_addr_t padded_authkey_dma_addr ;  dma_addr_t ipad_opad_dma_addr ; } cocci_id@p ;
...
position p;
@@
 enum drv_crypto_direction cocci_id@p ;
...
position p;
@@
 struct cc_hw_desc cocci_id@p [ MAX_AEAD_PROCESS_SEQ ] ;
...
position p;
@@
 struct cc_aead_handle * cocci_id@p ;
...
typedef cc_sram_addr_t;
@@
 struct cc_aead_handle {  cc_sram_addr_t sram_workspace_addr ;  struct list_head aead_list ; } cocci_id@p ;
...
position p;
@@
 struct cc_hw_desc cocci_id@p [ MAX_AEAD_SETKEY_SEQ ] ;
