...
position p;
@@
 struct spi_fiq_code * cocci_id@p ;
...
position p;
@@
 enum spi_fiq_mode { FIQ_MODE_NONE = 0 , FIQ_MODE_TX = 1 , FIQ_MODE_RX = 2 , FIQ_MODE_TXRX = 3 , } cocci_id@p ;
...
typedef u8,u32;
@@
 struct spi_fiq_code {  u32 length ;  u32 ack_offset ;  u8 data [ 0 ] ; } cocci_id@p ;
...
position p;
@@
 struct s3c24xx_spi_devstate * cocci_id@p ;
...
position p;
@@
 struct s3c2410_spi_info * cocci_id@p ;
...
typedef u8;
@@
 struct s3c24xx_spi_devstate {  unsigned int hz ;  unsigned int mode ;  u8 spcon ;  u8 sppre ; } cocci_id@p ;
