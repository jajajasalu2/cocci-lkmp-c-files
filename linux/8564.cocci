...
typedef u8;
@@
 int cocci_id@p ( struct link_params * params , struct link_vars * vars , u8 notify ) ;
...
typedef u16;
@@
 u16 cocci_id@p [ PHY848xx_CMDHDLR_MAX_ARGS ] ;
...
position p;
@@
 int cocci_id@p ( struct bnx2x_phy * phy , struct link_params * params ) ;
...
position p;
@@
 const struct link_params * cocci_id@p ;
...
position p;
@@
 struct bnx2x_reg_set cocci_id@p [ ] ;
...
position p;
@@
 struct link_vars cocci_id@p [ MAX_PHYS ] ;
...
position p;
@@
 const struct bnx2x_ets_params * cocci_id@p ;
...
position p;
@@
 struct bnx2x_phy cocci_id@p [ PORT_MAX ] ;
...
position p;
@@
 const struct link_vars * cocci_id@p ;
...
typedef u32;
@@
 u32 cocci_id@p [ I2C_SWITCH_WIDTH ] ;
...
position p;
@@
 char cocci_id@p [ SFP_EEPROM_PART_NO_SIZE + 1 ] ;
...
typedef u8,u16;
@@
 int ( * cocci_id@p ) ( struct bnx2x_phy * phy , struct link_params * params , u8 dev_addr , u16 addr , u8 byte_cnt , u8 * o_buf , u8 ) ;
...
typedef u8;
@@
 u8 cocci_id@p [ DCBX_MAX_NUM_COS ] ;
...
position p;
@@
 const struct bnx2x_phy cocci_id@p ;
...
typedef u8;
@@
 u8 cocci_id@p [ SFP_EEPROM_FC_TX_TECH_ADDR + 1 ] ;
...
typedef u8;
@@
 u8 cocci_id@p [ SFP_EEPROM_OPTIONS_SIZE ] ;
...
position p;
@@
 char cocci_id@p [ SFP_EEPROM_VENDOR_NAME_SIZE + 1 ] ;
...
position p;
@@
 read_sfp_module_eeprom_func_p cocci_id@p ;
