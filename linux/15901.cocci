...
position p;
@@
 struct gigaset_capi_ctr * cocci_id@p ;
...
typedef u32,u16;
@@
 struct gigaset_capi_appl {  struct list_head ctrlist ;  struct gigaset_capi_appl * bcnext ;  u16 id ;  struct capi_register_params rp ;  u16 nextMessageNumber ;  u32 listenInfoMask ;  u32 listenCIPmask ; } cocci_id@p ;
...
position p;
@@
 struct gigaset_capi_appl * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u16,capi_send_handler_t;
@@
 struct {  u16 cmd ;  capi_send_handler_t handler ; } cocci_id@p [ ] ;
...
typedef u8,atomic_t;
@@
 struct gigaset_capi_ctr {  struct capi_ctr ctr ;  struct list_head appls ;  struct sk_buff_head sendqueue ;  atomic_t sendqlen ;  _cmsg hcmsg ;  _cmsg acmsg ;  u8 bc_buf [ MAX_BC_OCTETS + 1 ] ;  u8 hlc_buf [ MAX_HLC_OCTETS + 1 ] ;  u8 cgpty_buf [ MAX_NUMBER_DIGITS + 3 ] ;  u8 cdpty_buf [ MAX_NUMBER_DIGITS + 2 ] ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef capi_send_handler_t;
@@
 capi_send_handler_t cocci_id@p ;
...
position p;
@@
 capi_register_params * cocci_id@p ;
...
typedef u8;
@@
 struct {  u8 * bc ;  u8 * hlc ; } cocci_id@p [ ] ;
...
position p;
@@
 char cocci_id@p [ 3 * MAX_FMT_IE_LEN ] ;
...
position p;
@@
 void ( * cocci_id@p ) ( struct gigaset_capi_ctr * , struct gigaset_capi_appl * , struct sk_buff * ) ;
