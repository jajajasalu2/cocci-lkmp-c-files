...
position p;
@@
 enum channel_type { CH_A = 0 , CH_B = 1 , } cocci_id@p ;
...
position p;
@@
 enum port_type { PORT_MDM = 0 , PORT_DIAG = 1 , PORT_APP1 = 2 , PORT_APP2 = 3 , PORT_CTRL = 4 , PORT_ERROR = - 1 , } cocci_id@p ;
...
position p;
@@
 enum ctrl_port_type cocci_id@p ;
...
position p;
@@
 const struct ctrl_dl * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,u32,u16,wait_queue_head_t;
@@
 struct port {  struct tty_port port ;  u8 update_flow_control ;  struct ctrl_ul ctrl_ul ;  struct ctrl_dl ctrl_dl ;  struct kfifo fifo_ul ;  void __iomem * dl_addr [ 2 ] ;  u32 dl_size [ 2 ] ;  u8 toggle_dl ;  void __iomem * ul_addr [ 2 ] ;  u32 ul_size [ 2 ] ;  u8 toggle_ul ;  u16 token_dl ;  wait_queue_head_t tty_wait ;  struct async_icount tty_icount ;  struct nozomi * dc ; } cocci_id@p ;
...
position p;
@@
 const struct async_icount cocci_id@p ;
...
position p;
@@
 enum ctrl_port_type { CTRL_CMD = 0 , CTRL_MDM = 1 , CTRL_DIAG = 2 , CTRL_APP1 = 3 , CTRL_APP2 = 4 , CTRL_ERROR = - 1 , } cocci_id@p ;
...
position p;
@@
 enum card_state { NOZOMI_STATE_UNKNOWN = 0 , NOZOMI_STATE_ENABLED = 1 , NOZOMI_STATE_ALLOCATED = 2 , NOZOMI_STATE_READY = 3 , } cocci_id@p ;
...
position p;
@@
 struct nozomi * cocci_id@p [ NOZOMI_MAX_CARDS ] ;
...
position p;
@@
 struct config_table cocci_id@p ;
...
position p;
@@
 enum card_type { F32_2 = 2048 , F32_8 = 8192 , } cocci_id@p ;
...
@@
 struct nozomi {  void __iomem * base_addr ;  unsigned long flip ;  void __iomem * reg_iir ;  void __iomem * reg_fcr ;  void __iomem * reg_ier ;  u16 last_ier ;  enum card_type card_type ;  struct config_table config_table ;  struct pci_dev * pdev ;  struct port port [ NOZOMI_MAX_PORTS ] ;  u8 * send_buf ;  spinlock_t spin_mutex ;  unsigned int index_start ;  enum card_state state ;  u32 open_ttys ; } cocci_id@p ;
...
position p;
@@
 const struct ctrl_ul * cocci_id@p ;
...
position p;
@@
 const struct nozomi * cocci_id@p ;
...
position p;
@@
 const struct tty_struct * cocci_id@p ;
