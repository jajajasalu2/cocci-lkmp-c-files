...
position p;
@@
 struct cbe_iommu cocci_id@p [ NR_IOMMUS ] ;
...
position p;
@@
 struct iommu_window * cocci_id@p ;
...
position p;
@@
 struct iommu_window {  struct list_head list ;  struct cbe_iommu * iommu ;  unsigned long offset ;  unsigned long size ;  unsigned int ioid ;  struct iommu_table table ; } cocci_id@p ;
...
typedef u64;
@@
 u64 cocci_id@p ( struct device * dev ) ;
...
position p;
@@
 struct cbe_iommu {  int nid ;  char name [ NAMESIZE ] ;  void __iomem * xlate_regs ;  void __iomem * cmd_regs ;  unsigned long * stab ;  unsigned long * ptab ;  void * pad_page ;  struct list_head windows ; } cocci_id@p ;
