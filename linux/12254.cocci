...
position p;
@@
 enum { LVDS_BIT_MAP_SPWG , LVDS_BIT_MAP_JEIDA , } cocci_id@p ;
...
position p;
@@
 const struct imx_ldb_bit_mapping cocci_id@p [ ] ;
...
position p;
@@
 struct imx_ldb_channel * cocci_id@p ;
...
position p;
@@
 struct imx_crtc_state * cocci_id@p ;
...
position p;
@@
 struct bus_mux {  int reg ;  int shift ;  int mask ; } cocci_id@p ;
...
typedef u32;
@@
 struct imx_ldb_channel {  struct imx_ldb * ldb ;  struct drm_connector connector ;  struct drm_encoder encoder ;  struct drm_panel * panel ;  struct drm_bridge * bridge ;  struct device_node * child ;  struct i2c_adapter * ddc ;  int chno ;  void * edid ;  int edid_len ;  struct drm_display_mode mode ;  int mode_valid ;  u32 bus_format ;  u32 bus_flags ; } cocci_id@p ;
...
position p;
@@
 const struct bus_mux * cocci_id@p ;
...
typedef u32;
@@
 struct imx_ldb_bit_mapping {  u32 bus_format ;  u32 datawidth ;  const char * const mapping ; } cocci_id@p ;
...
typedef u32;
@@
 struct imx_ldb {  struct regmap * regmap ;  struct device * dev ;  struct imx_ldb_channel channel [ 2 ] ;  struct clk * clk [ 2 ] ;  struct clk * clk_sel [ 4 ] ;  struct clk * clk_parent [ 4 ] ;  struct clk * clk_pll [ 2 ] ;  u32 ldb_ctrl ;  const struct bus_mux * lvds_mux ; } cocci_id@p ;
