...
typedef FILE;
@@
 size_t cocci_id@p ( struct trace * trace , FILE * fp ) ;
...
position p;
@@
 struct thread_trace * cocci_id@p ;
...
typedef bool,u16;
@@
 struct bpf_map_syscall_entry {  bool enabled ;  u16 string_args_len [ 6 ] ; } cocci_id@p ;
...
position p;
@@
 struct bpf_map_syscall_entry * cocci_id@p ;
...
position p;
@@
 struct augmented_arg * cocci_id@p ;
...
position p;
@@
 struct evsel_trace {  struct syscall_tp sc ;  struct syscall_arg_fmt * fmt ; } cocci_id@p ;
...
position p;
@@
 struct ordered_event * cocci_id@p ;
...
position p;
@@
 struct syscall_stats * cocci_id@p ;
...
position p;
@@
 struct syscall_arg_fmt cocci_id@p ;
...
position p;
@@
 const struct syscall_arg_fmt * cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct trace * trace , struct evsel * evsel , union perf_event * event , struct perf_sample * sample ) ;
...
position p;
@@
 const struct syscall_fmt * cocci_id@p ;
...
typedef u64,u32;
@@
 struct syscall_stats {  struct stats stats ;  u64 nr_failures ;  int max_errno ;  u32 * errnos ; } cocci_id@p ;
