...
position p;
@@
 int cocci_id@p ( struct i2c_client * client , const struct i2c_device_id * id ) ;
...
position p;
@@
 int cocci_id@p ( struct i2c_client * client , struct i2c_board_info * info ) ;
...
position p;
@@
 struct watchdog_info cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct i2c_client * client ) ;
...
typedef u8,u16;
@@
 int cocci_id@p ( struct i2c_client * client , u16 reg , u8 value ) ;
...
position p;
@@
 struct w83793_data * cocci_id@p ( struct device * dev ) ;
...
typedef u8,u16;
@@
 u8 cocci_id@p ( struct i2c_client * client , u16 reg ) ;
...
position p;
@@
 void cocci_id@p ( struct i2c_client * client ) ;
...
typedef s8,u8,u16;
@@
 struct w83793_data {  struct i2c_client * lm75 [ 2 ] ;  struct device * hwmon_dev ;  struct mutex update_lock ;  char valid ;  unsigned long last_updated ;  unsigned long last_nonvolatile ;  u8 bank ;  u8 vrm ;  u8 vid [ 2 ] ;  u8 in [ 10 ] [ 3 ] ;  u8 in_low_bits [ 3 ] ;  u16 has_fan ;  u16 fan [ 12 ] ;  u16 fan_min [ 12 ] ;  s8 temp [ 6 ] [ 5 ] ;  u8 temp_low_bits ;  u8 temp_mode [ 2 ] ;  u8 temp_critical ;  u8 temp_fan_map [ 6 ] ;  u8 has_pwm ;  u8 has_temp ;  u8 has_vid ;  u8 pwm_enable ;  u8 pwm_uptime ;  u8 pwm_downtime ;  u8 pwm_default ;  u8 pwm [ 8 ] [ 3 ] ;  u8 pwm_stop_time [ 8 ] ;  u8 temp_cruise [ 6 ] ;  u8 alarms [ 5 ] ;  u8 beeps [ 5 ] ;  u8 beep_enable ;  u8 tolerance [ 3 ] ;  u8 sf2_pwm [ 6 ] [ 7 ] ;  u8 sf2_temp [ 6 ] [ 7 ] ;  struct i2c_client * client ;  struct mutex watchdog_lock ;  struct list_head list ;  struct kref kref ;  struct miscdevice watchdog_miscdev ;  unsigned long watchdog_is_open ;  char watchdog_expect_close ;  char watchdog_name [ 10 ] ;  unsigned int watchdog_caused_reboot ;  int watchdog_timeout ; } cocci_id@p ;
