...
position p;
@@
 struct set_schib_struct cocci_id@p ;
...
@@
 struct copy_block_struct {  wait_queue_head_t wait ;  int ret ; } cocci_id@p ;
...
position p;
@@
 struct copy_block_struct cocci_id@p ;
...
@@
 struct set_schib_struct {  u32 mme ;  int mbfc ;  unsigned long address ;  wait_queue_head_t wait ;  int ret ; } cocci_id@p ;
...
typedef u32,u16;
@@
 struct cmb {  u16 ssch_rsch_count ;  u16 sample_count ;  u32 device_connect_time ;  u32 function_pending_time ;  u32 device_disconnect_time ;  u32 control_unit_queuing_time ;  u32 device_active_only_time ;  u32 reserved [ 2 ] ; } cocci_id@p ;
...
typedef spinlock_t;
@@
 struct cmb_area {  struct cmb * mem ;  struct list_head list ;  int num_channels ;  spinlock_t lock ; } cocci_id@p ;
...
position p;
@@
 struct cmb_operations cocci_id@p ;
...
position p;
@@
 enum cmb_format { CMF_BASIC , CMF_EXTENDED , CMF_AUTODETECT = - 1 , } cocci_id@p ;
...
position p;
@@
 struct ccw_device_private * cocci_id@p ;
...
position p;
@@
 struct cmb_data {  void * hw_block ;  void * last_block ;  int size ;  unsigned long long last_update ; } cocci_id@p ;
