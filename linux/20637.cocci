...
position p;
@@
 struct cpu_perf_ibs cocci_id@p ;
...
typedef u64,u32;
@@
 struct perf_ibs_data {  u32 size ;  union {  u32 data [ 0 ] ;  u32 caps ; } ;  u64 regs [ MSR_AMD64_IBS_REG_COUNT_MAX ] ; } cocci_id@p ;
...
position p;
@@
 enum ibs_states { IBS_ENABLED = 0 , IBS_STARTED = 1 , IBS_STOPPING = 2 , IBS_STOPPED = 3 , IBS_MAX_STATES , } cocci_id@p ;
...
typedef u64;
@@
 struct perf_ibs {  struct pmu pmu ;  unsigned int msr ;  u64 config_mask ;  u64 cnt_mask ;  u64 enable_mask ;  u64 valid_mask ;  u64 max_period ;  unsigned long offset_mask [ 1 ] ;  int offset_max ;  struct cpu_perf_ibs __percpu * pcpu ;  struct attribute * * format_attrs ;  struct attribute_group format_group ;  const struct attribute_group * attr_groups [ 2 ] ;  u64 ( * get_count ) ( u64 config ) ; } cocci_id@p ;
...
position p;
@@
 struct perf_ibs_data cocci_id@p ;
...
position p;
@@
 struct cpu_perf_ibs {  struct perf_event * event ;  unsigned long state [ BITS_TO_LONGS ( IBS_MAX_STATES ) ] ; } cocci_id@p ;
