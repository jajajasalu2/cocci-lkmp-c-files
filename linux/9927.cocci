...
position p;
@@
 struct ath6kl_mbox_info * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct ath6kl * ar ) ;
...
symbol cocci_id;
position p;
typedef u8,bool,wait_queue_head_t,atomic_t,spinlock_t;
@@
 struct ath6kl_sdio {  struct sdio_func * func ;  spinlock_t lock ;  struct list_head bus_req_freeq ;  struct bus_request bus_req [ BUS_REQUEST_MAX_NUM ] ;  struct ath6kl * ar ;  u8 * dma_buffer ;  struct mutex dma_buffer_mutex ;  struct list_head scat_req ;  atomic_t irq_handling ;  wait_queue_head_t irq_wq ;  spinlock_t scat_lock ;  bool scatter_enabled ;  bool is_disabled ;  const struct sdio_device_id * id ;  struct work_struct wr_async_work ;  struct list_head wr_asyncq ;  spinlock_t wr_async_lock ; } cocci_id@p ;
...
position p;
@@
 struct hif_scatter_item cocci_id@p ;
