...
position p;
@@
 enum { RX_LARGE_BUF = 1 << 0 , RX_UNMAPPED_BUF = 1 << 1 , } cocci_id@p ;
...
typedef dma_addr_t;
@@
 dma_addr_t cocci_id@p [ MAX_SKB_FRAGS + 1 ] ;
...
position p;
@@
 const struct ulptx_sge_pair * cocci_id@p ;
...
position p;
@@
 struct tx_sw_desc {  struct sk_buff * skb ;  struct ulptx_sgl * sgl ; } cocci_id@p ;
...
position p;
@@
 const struct ulptx_sgl * cocci_id@p ;
