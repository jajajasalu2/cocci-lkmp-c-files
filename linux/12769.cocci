...
typedef u8;
@@
 struct glink_defer_cmd {  struct list_head node ;  struct glink_msg msg ;  u8 data [ ] ; } cocci_id@p ;
...
position p;
@@
 struct glink_channel * cocci_id@p ;
...
@@
 struct qcom_glink {  struct device * dev ;  const char * name ;  struct mbox_client mbox_client ;  struct mbox_chan * mbox_chan ;  struct qcom_glink_pipe * rx_pipe ;  struct qcom_glink_pipe * tx_pipe ;  int irq ;  struct work_struct rx_work ;  spinlock_t rx_lock ;  struct list_head rx_queue ;  spinlock_t tx_lock ;  spinlock_t idr_lock ;  struct idr lcids ;  struct idr rcids ;  unsigned long features ;  bool intentless ; } cocci_id@p ;
...
position p;
@@
 struct glink_defer_cmd * cocci_id@p ;
...
position p;
@@
 enum { GLINK_STATE_CLOSED , GLINK_STATE_OPENING , GLINK_STATE_OPEN , GLINK_STATE_CLOSING , } cocci_id@p ;
...
typedef __le32;
@@
 struct intent_pair {  __le32 size ;  __le32 iid ; } cocci_id@p ;
...
typedef bool,u32;
@@
 struct glink_core_rx_intent {  void * data ;  u32 id ;  size_t size ;  bool reuse ;  bool in_use ;  u32 offset ;  struct list_head node ; } cocci_id@p ;
...
position p;
@@
 struct glink_core_rx_intent * cocci_id@p ;
...
position p;
@@
 struct qcom_glink_pipe * cocci_id@p ;
...
typedef bool,spinlock_t;
@@
 struct glink_channel {  struct rpmsg_endpoint ept ;  struct rpmsg_device * rpdev ;  struct qcom_glink * glink ;  struct kref refcount ;  spinlock_t recv_lock ;  char * name ;  unsigned int lcid ;  unsigned int rcid ;  spinlock_t intent_lock ;  struct idr liids ;  struct idr riids ;  struct work_struct intent_work ;  struct list_head done_intents ;  struct glink_core_rx_intent * buf ;  int buf_offset ;  int buf_size ;  struct completion open_ack ;  struct completion open_req ;  struct mutex intent_req_lock ;  bool intent_req_result ;  struct completion intent_req_comp ; } cocci_id@p ;
