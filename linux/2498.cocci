...
symbol cocci_id;
position p;
typedef u64,bool,atomic_t;
@@
 struct ppl_io_unit {  struct ppl_log * log ;  struct page * header_page ;  unsigned int entries_count ;  unsigned int pp_size ;  u64 seq ;  struct list_head log_sibling ;  struct list_head stripe_list ;  atomic_t pending_stripes ;  atomic_t pending_flushes ;  bool submitted ;  struct bio bio ;  struct bio_vec biovec [ PPL_IO_INLINE_BVECS ] ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u32,atomic64_t,mempool_t,spinlock_t;
@@
 struct ppl_conf {  struct mddev * mddev ;  struct ppl_log * child_logs ;  int count ;  int block_size ;  u32 signature ;  atomic64_t seq ;  struct kmem_cache * io_kc ;  mempool_t io_pool ;  struct bio_set bs ;  struct bio_set flush_bs ;  int recovered_entries ;  int mismatch_count ;  struct list_head no_mem_stripes ;  spinlock_t no_mem_stripes_lock ;  unsigned short write_hint ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,sector_t,spinlock_t;
@@
 struct ppl_log {  struct ppl_conf * ppl_conf ;  struct md_rdev * rdev ;  struct mutex io_mutex ;  struct ppl_io_unit * current_io ;  spinlock_t io_list_lock ;  struct list_head io_list ;  sector_t next_io_sector ;  unsigned int entry_space ;  bool use_multippl ;  bool wb_cache_on ;  unsigned long disk_flush_bitmap ; } cocci_id@p ;
...
position p;
@@
 struct ppl_header_entry * cocci_id@p ;
