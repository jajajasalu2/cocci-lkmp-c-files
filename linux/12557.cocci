...
position p;
@@
 const struct edp_pixel_clk_div cocci_id@p [ 2 ] [ EDP_PIXEL_CLK_NUM ] ;
...
@@
 struct edp_ctrl {  struct platform_device * pdev ;  void __iomem * base ;  struct regulator * vdda_vreg ;  struct regulator * lvl_vreg ;  struct clk * aux_clk ;  struct clk * pixel_clk ;  struct clk * ahb_clk ;  struct clk * link_clk ;  struct clk * mdp_core_clk ;  struct gpio_desc * panel_en_gpio ;  struct gpio_desc * panel_hpd_gpio ;  struct completion idle_comp ;  struct mutex dev_mutex ;  struct work_struct on_work ;  struct work_struct off_work ;  struct workqueue_struct * workqueue ;  spinlock_t irq_lock ;  bool edp_connected ;  bool power_on ;  struct edid * edid ;  struct drm_dp_aux * drm_aux ;  u8 dpcd [ DP_RECEIVER_CAP_SIZE ] ;  u8 link_rate ;  u8 lane_cnt ;  u8 v_level ;  u8 p_level ;  u8 interlaced ;  u32 pixel_rate ;  u32 color_depth ;  struct edp_aux * aux ;  struct edp_phy * phy ; } cocci_id@p ;
...
position p;
@@
 enum edp_color_depth cocci_id@p ;
...
position p;
@@
 const struct edp_pixel_clk_div * cocci_id@p ;
...
typedef u32;
@@
 struct edp_pixel_clk_div {  u32 rate ;  u32 m ;  u32 n ; } cocci_id@p ;
