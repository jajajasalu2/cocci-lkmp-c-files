...
position p;
@@
 struct cls_bpf_prog cocci_id@p ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_BPF_MAX + 1 ] ;
...
@@
 struct cls_bpf_prog {  struct bpf_prog * filter ;  struct list_head link ;  struct tcf_result res ;  bool exts_integrated ;  u32 gen_flags ;  unsigned int in_hw_count ;  struct tcf_exts exts ;  u32 handle ;  u16 bpf_num_ops ;  struct sock_filter * bpf_ops ;  const char * bpf_name ;  struct tcf_proto * tp ;  struct rcu_work rwork ; } cocci_id@p ;
...
position p;
@@
 struct tc_cls_bpf_offload cocci_id@p ;
...
position p;
@@
 const struct cls_bpf_prog * cocci_id@p ;
...
position p;
@@
 struct cls_bpf_head {  struct list_head plist ;  struct idr handle_idr ;  struct rcu_head rcu ; } cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_BPF_MAX + 1 ] ;
...
position p;
@@
 struct cls_bpf_head * cocci_id@p ;
