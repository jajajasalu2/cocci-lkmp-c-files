...
position p;
@@
 enum cc_key_type { CC_UNPROTECTED_KEY , CC_HW_PROTECTED_KEY , CC_POLICY_PROTECTED_KEY , CC_INVALID_PROTECTED_KEY , } cocci_id@p ;
...
position p;
@@
 struct cipher_req_ctx cocci_id@p ;
...
position p;
@@
 struct cc_cipher_ctx cocci_id@p ;
...
position p;
@@
 struct cc_hw_key_info {  enum cc_hw_crypto_key key1_slot ;  enum cc_hw_crypto_key key2_slot ; } cocci_id@p ;
...
position p;
@@
 enum cc_hw_crypto_key cocci_id@p ;
...
position p;
@@
 struct cc_cipher_handle * cocci_id@p ;
...
position p;
@@
 const struct cc_alg_template cocci_id@p [ ] ;
...
position p;
@@
 struct cc_cipher_ctx {  struct cc_drvdata * drvdata ;  int keylen ;  int key_round_number ;  int cipher_mode ;  int flow_mode ;  unsigned int flags ;  enum cc_key_type key_type ;  struct cc_user_key_info user ;  union {  struct cc_hw_key_info hw ;  struct cc_cpp_key_info cpp ; } ;  struct crypto_shash * shash_tfm ; } cocci_id@p ;
...
@@
 struct cc_user_key_info {  u8 * key ;  dma_addr_t key_dma_addr ; } cocci_id@p ;
...
position p;
@@
 struct cc_hkey_info cocci_id@p ;
...
position p;
@@
 struct cc_hw_desc cocci_id@p [ MAX_SKCIPHER_SEQ_LEN ] ;
...
typedef u8;
@@
 struct tdes_keys {  u8 key1 [ DES_KEY_SIZE ] ;  u8 key2 [ DES_KEY_SIZE ] ;  u8 key3 [ DES_KEY_SIZE ] ; } cocci_id@p ;
...
position p;
@@
 struct cc_cipher_handle {  struct list_head alg_list ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct device * dev , void * cc_req , int err ) ;
...
typedef u8;
@@
 struct cc_cpp_key_info {  u8 slot ;  enum cc_cpp_alg alg ; } cocci_id@p ;
