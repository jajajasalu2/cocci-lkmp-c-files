...
position p;
@@
 const struct serio_device_id cocci_id@p [ ] ;
...
position p;
@@
 struct cec_log_addrs * cocci_id@p ;
...
position p;
@@
 struct serio_driver cocci_id@p ;
...
typedef u8,bool;
@@
 struct pulse8 {  struct device * dev ;  struct serio * serio ;  struct cec_adapter * adap ;  unsigned int vers ;  struct completion cmd_done ;  struct work_struct work ;  struct delayed_work ping_eeprom_work ;  struct cec_msg rx_msg ;  u8 data [ DATA_SIZE ] ;  unsigned int len ;  u8 buf [ DATA_SIZE ] ;  unsigned int idx ;  bool escape ;  bool started ;  struct mutex config_lock ;  struct mutex write_lock ;  bool config_pending ;  bool restoring_config ;  bool autonomous ; } cocci_id@p ;
