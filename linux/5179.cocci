...
position p;
@@
 enum GPIO_REG { GPIO_USE_SEL = 0 , GPIO_IO_SEL , GPIO_LVL , GPO_BLINK , } cocci_id@p ;
...
position p;
@@
 struct lpc_ich_info * cocci_id@p ;
...
@@
 struct {  spinlock_t lock ;  struct device * dev ;  struct gpio_chip chip ;  struct resource * gpio_base ;  struct resource * pm_base ;  struct ichx_desc * desc ;  u32 orig_gpio_ctrl ;  u8 use_gpio ;  int outlvl_cache [ 3 ] ; } cocci_id@p ;
...
typedef u8,bool,u32;
@@
 struct ichx_desc {  uint ngpio ;  const u8 * regs [ 3 ] ;  const u8 * reglen ;  bool have_blink ;  bool uses_gpe0 ;  u32 use_sel_ignore [ 3 ] ;  int ( * request ) ( struct gpio_chip * chip , unsigned offset ) ;  int ( * get ) ( struct gpio_chip * chip , unsigned offset ) ;  bool use_outlvl_cache ; } cocci_id@p ;
