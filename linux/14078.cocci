...
position p;
@@
 struct jpu_buffer {  struct v4l2_m2m_buffer buf ;  unsigned short compr_quality ;  unsigned char subsampling ; } cocci_id@p ;
...
position p;
@@
 const unsigned int cocci_id@p [ JPU_MAX_QUALITY ] [ QTBL_SIZE ] ;
...
position p;
@@
 struct jpeg_buffer {  void * end ;  void * curr ; } cocci_id@p ;
...
typedef bool;
@@
 struct jpu_ctx {  struct jpu * jpu ;  bool encoder ;  unsigned short compr_quality ;  struct jpu_q_data out_q ;  struct jpu_q_data cap_q ;  struct v4l2_fh fh ;  struct v4l2_ctrl_handler ctrl_handler ; } cocci_id@p ;
...
position p;
@@
 struct jpu_q_data {  struct jpu_fmt * fmtinfo ;  struct v4l2_pix_format_mplane format ;  unsigned int sequence ; } cocci_id@p ;
...
position p;
@@
 const unsigned int cocci_id@p [ HDCTBL_SIZE ] ;
...
typedef u8,u32,u16;
@@
 struct jpu_fmt {  u32 fourcc ;  u32 colorspace ;  u8 bpp [ 2 ] ;  u8 h_align ;  u8 v_align ;  u8 subsampling ;  u8 num_planes ;  u16 types ; } cocci_id@p ;
...
position p;
@@
 unsigned char cocci_id@p [ JPU_MAX_QUALITY ] [ JPU_JPEG_HDR_SIZE ] ;
...
typedef spinlock_t;
@@
 struct jpu {  struct mutex mutex ;  spinlock_t lock ;  struct v4l2_device v4l2_dev ;  struct video_device vfd_encoder ;  struct video_device vfd_decoder ;  struct v4l2_m2m_dev * m2m_dev ;  struct jpu_ctx * curr ;  void __iomem * regs ;  unsigned int irq ;  struct clk * clk ;  struct device * dev ;  int ref_count ; } cocci_id@p ;
...
position p;
@@
 const unsigned int cocci_id@p [ HACTBL_SIZE ] ;
