...
position p;
@@
 struct clk_set_value cocci_id@p ;
...
typedef u32;
@@
 struct scpi_xfer {  u32 slot ;  u32 cmd ;  u32 status ;  const void * tx_buf ;  void * rx_buf ;  unsigned int tx_len ;  unsigned int rx_len ;  struct list_head node ;  struct completion done ; } cocci_id@p ;
...
position p;
@@
 const struct scpi_opp * cocci_id@p ;
...
position p;
@@
 enum scpi_drv_cmds { CMD_SCPI_CAPABILITIES = 0 , CMD_GET_CLOCK_INFO , CMD_GET_CLOCK_VALUE , CMD_SET_CLOCK_VALUE , CMD_GET_DVFS , CMD_SET_DVFS , CMD_GET_DVFS_INFO , CMD_SENSOR_CAPABILITIES , CMD_SENSOR_INFO , CMD_SENSOR_VALUE , CMD_SET_DEVICE_PWR_STATE , CMD_GET_DEVICE_PWR_STATE , CMD_MAX_COUNT , } cocci_id@p ;
...
position p;
@@
 struct scpi_dvfs_info * cocci_id@p ;
...
position p;
@@
 struct scpi_sensor_info * cocci_id@p ;
...
position p;
@@
 struct _scpi_sensor_info cocci_id@p ;
...
position p;
@@
 enum scpi_error_codes { SCPI_SUCCESS = 0 , SCPI_ERR_PARAM = 1 , SCPI_ERR_ALIGN = 2 , SCPI_ERR_SIZE = 3 , SCPI_ERR_HANDLER = 4 , SCPI_ERR_ACCESS = 5 , SCPI_ERR_RANGE = 6 , SCPI_ERR_TIMEOUT = 7 , SCPI_ERR_NOMEM = 8 , SCPI_ERR_PWRSTATE = 9 , SCPI_ERR_SUPPORT = 10 , SCPI_ERR_DEVICE = 11 , SCPI_ERR_BUSY = 12 , SCPI_ERR_MAX , } cocci_id@p ;
...
position p;
@@
 struct legacy_clk_set_value cocci_id@p ;
...
position p;
@@
 struct clk_get_info cocci_id@p ;
...
position p;
@@
 struct scp_capabilities cocci_id@p ;
...
typedef u8;
@@
 struct _scpi_sensor_info {  __le16 sensor_id ;  u8 class ;  u8 trigger_type ;  char name [ 20 ] ; } cocci_id@p ;
...
@@
 struct scpi_drvinfo {  u32 protocol_version ;  u32 firmware_version ;  bool is_legacy ;  int num_chans ;  int * commands ; 
	 DECLARE_BITMAP ( cmd_priority , LEGACY_SCPI_CMD_COUNT ) 
		 ;  atomic_t next_chan ;  struct scpi_ops * scpi_ops ;  struct scpi_chan * channels ;  struct scpi_dvfs_info * dvfs [ MAX_DVFS_DOMAINS ] ; } cocci_id@p ;
...
position p;
@@
 struct scpi_drvinfo * cocci_id@p ;
...
@@
 struct scpi_chan {  struct mbox_client cl ;  struct mbox_chan * chan ;  void __iomem * tx_payload ;  void __iomem * rx_payload ;  struct list_head rx_pending ;  struct list_head xfers_list ;  struct scpi_xfer * xfers ;  spinlock_t rx_lock ;  struct mutex xfers_lock ;  u8 token ; } cocci_id@p ;
...
position p;
@@
 struct dev_pstate_set cocci_id@p ;
