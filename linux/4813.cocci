...
position p;
@@
 int cocci_id@p ( struct iw_cm_id * cm_id , struct iw_cm_event * event ) ;
...
position p;
@@
 struct rdma_nl_cbs cocci_id@p [ RDMA_NL_IWPM_NUM_OPS ] ;
...
position p;
@@
 struct iwcm_work {  struct work_struct work ;  struct iwcm_id_private * cm_id ;  struct list_head list ;  struct iw_cm_event event ;  struct list_head free_list ; } cocci_id@p ;
...
position p;
@@
 struct iwcm_id_private * cocci_id@p ;
