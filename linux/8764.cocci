...
position p;
@@
 unsigned char cocci_id@p [ QLCNIC_ILB_PKT_SIZE ] ;
...
position p;
@@
 const struct qlcnic_stats cocci_id@p [ ] ;
...
position p;
@@
 struct qlcnic_esw_statistics cocci_id@p ;
...
position p;
@@
 struct qlcnic_stats {  char stat_string [ ETH_GSTRING_LEN ] ;  int sizeof_stat ;  int stat_offset ; } cocci_id@p ;
...
position p;
@@
 struct qlcnic_tx_queue_stats cocci_id@p ;
