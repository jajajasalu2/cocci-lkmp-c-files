...
position p;
@@
 struct xen_memory_map cocci_id@p ;
...
position p;
@@
 const struct e820_entry * cocci_id@p ;
...
position p;
@@
 struct callback_register cocci_id@p ;
...
position p;
@@
 struct {  unsigned long next_area_mfn ;  unsigned long target_pfn ;  unsigned long size ;  unsigned long mfns [ REMAP_SIZE ] ; } cocci_id@p ;
...
position p;
@@
 unsigned long ( * cocci_id@p ) ( unsigned long start_pfn , unsigned long end_pfn , unsigned long nr_pages , unsigned long last_val ) ;
...
position p;
@@
 struct xen_memory_reservation cocci_id@p ;
