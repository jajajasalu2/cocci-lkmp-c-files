...
@@
 struct pri_queue {  atomic_t inflight ;  bool finish ;  int status ; } cocci_id@p ;
...
position p;
@@
 struct device_state * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u16,wait_queue_head_t,spinlock_t,atomic_t;
@@
 struct device_state {  struct list_head list ;  u16 devid ;  atomic_t count ;  struct pci_dev * pdev ;  struct pasid_state * * states ;  struct iommu_domain * domain ;  int pasid_levels ;  int max_pasids ;  amd_iommu_invalid_ppr_cb inv_ppr_cb ;  amd_iommu_invalidate_ctx inv_ctx_cb ;  spinlock_t lock ;  wait_queue_head_t wq ; } cocci_id@p ;
...
position p;
@@
 amd_iommu_invalid_ppr_cb cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,wait_queue_head_t,spinlock_t,atomic_t;
@@
 struct pasid_state {  struct list_head list ;  atomic_t count ;  unsigned mmu_notifier_count ;  struct mm_struct * mm ;  struct mmu_notifier mn ;  struct pri_queue pri [ PRI_QUEUE_SIZE ] ;  struct device_state * device_state ;  int pasid ;  bool invalid ;  spinlock_t lock ;  wait_queue_head_t wq ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct device_state * dev_state ) ;
...
typedef u64,u16;
@@
 struct fault {  struct work_struct work ;  struct device_state * dev_state ;  struct pasid_state * state ;  struct mm_struct * mm ;  u64 address ;  u16 devid ;  u16 pasid ;  u16 tag ;  u16 finish ;  u16 flags ; } cocci_id@p ;
...
position p;
@@
 amd_iommu_invalidate_ctx cocci_id@p ;
