...
symbol cocci_id;
position p;
typedef u8,dma_cap_mask_t,dma_addr_t;
@@
 struct arasan_cf_dev {  struct ata_host * host ;  struct clk * clk ;  dma_addr_t pbase ;  void __iomem * vbase ;  int irq ;  u8 dma_status ;  u8 card_present ;  struct completion cf_completion ;  struct completion dma_completion ;  struct dma_chan * dma_chan ;  dma_cap_mask_t mask ;  struct work_struct work ;  struct delayed_work dwork ;  struct ata_queued_cmd * qc ; } cocci_id@p ;
...
position p;
@@
 struct arasan_cf_dev * cocci_id@p ;
...
position p;
@@
 struct arasan_cf_pdata * cocci_id@p ;
