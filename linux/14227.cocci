...
position p;
@@
 const struct hfi_ops cocci_id@p ;
...
position p;
@@
 struct hfi_msg_sys_coverage_pkt * cocci_id@p ;
...
position p;
@@
 struct hfi_queue_table_header cocci_id@p ;
...
@@
 struct mem_desc {  dma_addr_t da ;  void * kva ;  u32 size ;  unsigned long attrs ; } cocci_id@p ;
...
typedef u32;
@@
 struct hfi_queue_header {  u32 status ;  u32 start_addr ;  u32 type ;  u32 q_size ;  u32 pkt_size ;  u32 pkt_drop_cnt ;  u32 rx_wm ;  u32 tx_wm ;  u32 rx_req ;  u32 tx_req ;  u32 rx_irq_status ;  u32 tx_irq_status ;  u32 read_idx ;  u32 write_idx ; } cocci_id@p ;
...
position p;
@@
 const struct venus_resources * cocci_id@p ;
...
typedef u8,bool,u32;
@@
 struct venus_hfi_device {  struct venus_core * core ;  u32 irq_status ;  u32 last_packet_type ;  bool power_enabled ;  bool suspended ;  enum venus_state state ;  struct mutex lock ;  struct completion pwr_collapse_prep ;  struct completion release_resource ;  struct mem_desc ifaceq_table ;  struct mem_desc sfr ;  struct iface_queue queues [ IFACEQ_NUM ] ;  u8 pkt_buf [ IFACEQ_VAR_HUGE_PKT_SIZE ] ;  u8 dbg_buf [ IFACEQ_VAR_HUGE_PKT_SIZE ] ; } cocci_id@p ;
...
position p;
@@
 const struct reg_val * cocci_id@p ;
...
position p;
@@
 struct iface_queue {  struct hfi_queue_header * qhdr ;  struct mem_desc qmem ; } cocci_id@p ;
...
position p;
@@
 enum venus_state { VENUS_STATE_DEINIT = 1 , VENUS_STATE_INIT , } cocci_id@p ;
...
typedef u8;
@@
 u8 cocci_id@p [ IFACEQ_VAR_LARGE_PKT_SIZE ] ;
...
position p;
@@
 struct hfi_queue_header cocci_id@p ;
...
position p;
@@
 struct hfi_msg_sys_debug_pkt * cocci_id@p ;
...
typedef u32;
@@
 struct hfi_queue_table_header {  u32 version ;  u32 size ;  u32 qhdr0_offset ;  u32 qhdr_size ;  u32 num_q ;  u32 num_active_q ; } cocci_id@p ;
...
position p;
@@
 struct hfi_msg_event_notify_pkt * cocci_id@p ;
...
position p;
@@
 struct venus_hfi_device * cocci_id@p ;
...
typedef u8;
@@
 u8 cocci_id@p [ IFACEQ_VAR_SMALL_PKT_SIZE ] ;
