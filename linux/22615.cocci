...
position p;
@@
 struct pcm_substream * cocci_id@p ;
...
typedef u8;
@@
 struct pcm_urb {  struct hiface_chip * chip ;  struct urb instance ;  struct usb_anchor submitted ;  u8 * buffer ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,snd_pcm_uframes_t,spinlock_t;
@@
 struct pcm_substream {  spinlock_t lock ;  struct snd_pcm_substream * instance ;  bool active ;  snd_pcm_uframes_t dma_off ;  snd_pcm_uframes_t period_off ; } cocci_id@p ;
...
@@
 struct pcm_runtime {  struct hiface_chip * chip ;  struct snd_pcm * instance ;  struct pcm_substream playback ;  bool panic ;  struct pcm_urb out_urbs [ PCM_N_URBS ] ;  struct mutex stream_mutex ;  u8 stream_state ;  u8 extra_freq ;  wait_queue_head_t stream_wait_queue ;  bool stream_wait_cond ; } cocci_id@p ;
...
position p;
@@
 enum { STREAM_DISABLED , STREAM_STARTING , STREAM_RUNNING , STREAM_STOPPING , } cocci_id@p ;
