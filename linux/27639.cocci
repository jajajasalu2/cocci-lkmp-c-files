...
position p;
@@
 void ( * cocci_id@p ) ( struct iomap_ioend * ioend , struct iomap_ioend * next ) ;
...
position p;
@@
 struct iomap_writepage_ctx * cocci_id@p ;
...
@@
 struct iomap_page {  atomic_t read_count ;  atomic_t write_count ;  spinlock_t uptodate_lock ; 
...
position p;
@@
 const struct iomap_writeback_ops * cocci_id@p ;
...
position p;
@@
 struct iomap_readpage_ctx cocci_id@p ;
...
position p;
@@
 enum { IOMAP_WRITE_F_UNSHARE = ( 1 << 0 ) , } cocci_id@p ;
...
typedef bool;
@@
 struct iomap_readpage_ctx {  struct page * cur_page ;  bool cur_page_in_bio ;  bool is_readahead ;  struct bio * bio ;  struct list_head * pages ; } cocci_id@p ;
