...
position p;
@@
 struct {  int fs ;  int ir ; } cocci_id@p [ ] ;
...
position p;
@@
 struct sta32x_platform_data * cocci_id@p ;
...
typedef u32;
@@
 struct sta32x_priv {  struct regmap * regmap ;  struct clk * xti_clk ;  struct regulator_bulk_data supplies [ ARRAY_SIZE ( sta32x_supply_names ) ] ;  struct snd_soc_component * component ;  struct sta32x_platform_data * pdata ;  unsigned int mclk ;  unsigned int format ;  u32 coef_shadow [ STA32X_COEF_COUNT ] ;  struct delayed_work watchdog_work ;  int shutdown ;  struct gpio_desc * gpiod_nreset ;  struct mutex coeff_lock ; } cocci_id@p ;
