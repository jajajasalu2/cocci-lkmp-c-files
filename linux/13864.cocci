...
position p;
@@
 const struct {  unsigned char kind ;  unsigned char data ;  unsigned short code ; } cocci_id@p [ ] ;
...
position p;
@@
 struct ati_receiver_type * cocci_id@p ;
...
position p;
@@
 struct ati_receiver_type {  const char * default_keymap ;  const char * ( * get_default_keymap ) ( struct usb_interface * interface ) ; } cocci_id@p ;
...
position p;
@@
 const struct accel_times cocci_id@p [ ] ;
...
position p;
@@
 struct accel_times {  const char value ;  unsigned int msecs ; } cocci_id@p ;
...
position p;
@@
 const struct ati_receiver_type cocci_id@p ;
...
symbol cocci_id;
position p;
typedef wait_queue_head_t,dma_addr_t;
@@
 struct ati_remote {  struct input_dev * idev ;  struct rc_dev * rdev ;  struct usb_device * udev ;  struct usb_interface * interface ;  struct urb * irq_urb ;  struct urb * out_urb ;  struct usb_endpoint_descriptor * endpoint_in ;  struct usb_endpoint_descriptor * endpoint_out ;  unsigned char * inbuf ;  unsigned char * outbuf ;  dma_addr_t inbuf_dma ;  dma_addr_t outbuf_dma ;  unsigned char old_data ;  unsigned long old_jiffies ;  unsigned long acc_jiffies ;  unsigned long first_jiffies ;  unsigned int repeat_count ;  char rc_name [ NAME_BUFSIZE ] ;  char rc_phys [ NAME_BUFSIZE ] ;  char mouse_name [ NAME_BUFSIZE ] ;  char mouse_phys [ NAME_BUFSIZE ] ;  wait_queue_head_t wait ;  int send_flags ;  int users ;  struct mutex open_mutex ; } cocci_id@p ;
