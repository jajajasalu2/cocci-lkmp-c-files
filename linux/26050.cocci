...
position p;
@@
 struct lpm_mt_test_info * cocci_id@p ;
...
position p;
@@
 struct lpm_mt_test_info cocci_id@p [ 4 ] ;
...
typedef __u32;
@@
 struct lpm_mt_test_info {  int cmd ;  int iter ;  int map_fd ;  struct {  __u32 prefixlen ;  __u32 data ; } key [ MAX_TEST_KEYS ] ; } cocci_id@p ;
...
typedef uint8_t;
@@
 struct tlpm_node {  struct tlpm_node * next ;  size_t n_bits ;  uint8_t key [ ] ; } cocci_id@p ;
