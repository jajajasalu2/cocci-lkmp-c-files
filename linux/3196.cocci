...
position p;
@@
 enum chips { lm85 , lm96000 , adm1027 , adt7463 , adt7468 , emc6d100 , emc6d102 , emc6d103 , emc6d103s , } cocci_id@p ;
...
typedef s8,u8;
@@
 struct lm85_zone {  s8 limit ;  u8 hyst ;  u8 range ;  s8 critical ;  u8 max_desired ; } cocci_id@p ;
...
typedef u8;
@@
 struct lm85_autofan {  u8 config ;  u8 min_pwm ;  u8 min_off ; } cocci_id@p ;
...
@@
 struct lm85_data {  struct i2c_client * client ;  const struct attribute_group * groups [ 6 ] ;  const int * freq_map ;  unsigned int freq_map_size ;  enum chips type ;  bool has_vid5 ;  struct mutex update_lock ;  int valid ;  unsigned long last_reading ;  unsigned long last_config ;  u8 in [ 8 ] ;  u8 in_max [ 8 ] ;  u8 in_min [ 8 ] ;  s8 temp [ 3 ] ;  s8 temp_min [ 3 ] ;  s8 temp_max [ 3 ] ;  u16 fan [ 4 ] ;  u16 fan_min [ 4 ] ;  u8 pwm [ 3 ] ;  u8 pwm_freq [ 3 ] ;  u8 temp_ext [ 3 ] ;  u8 in_ext [ 8 ] ;  u8 vid ;  u8 vrm ;  u32 alarms ;  u8 cfg5 ;  struct lm85_autofan autofan [ 3 ] ;  struct lm85_zone zone [ 3 ] ; } cocci_id@p ;
