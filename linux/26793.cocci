...
position p;
@@
 struct old_itimerspec32 * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u64,bool,wait_queue_head_t,spinlock_t,ktime_t;
@@
 struct timerfd_ctx {  union {  struct hrtimer tmr ;  struct alarm alarm ; } t ;  ktime_t tintv ;  ktime_t moffs ;  wait_queue_head_t wqh ;  u64 ticks ;  int clockid ;  short unsigned expired ;  short unsigned settime_flags ;  struct rcu_head rcu ;  struct list_head clist ;  spinlock_t cancel_lock ;  bool might_cancel ; } cocci_id@p ;
...
position p;
@@
 enum alarmtimer_restart cocci_id@p ;
...
position p;
@@
 const struct itimerspec64 * cocci_id@p ;
...
position p;
@@
 const struct old_itimerspec32 * cocci_id@p ;
