...
position p;
@@
 struct loopback_pcm cocci_id@p ;
...
position p;
@@
 struct loopback_pcm {  struct loopback * loopback ;  struct snd_pcm_substream * substream ;  struct loopback_cable * cable ;  unsigned int pcm_buffer_size ;  unsigned int buf_pos ;  unsigned int silent_size ;  unsigned int pcm_period_size ;  unsigned int pcm_bps ;  unsigned int pcm_salign ;  unsigned int pcm_rate_shift ;  unsigned int period_update_pending : 1 ;  unsigned int irq_pos ;  unsigned int period_size_frac ;  unsigned int last_drift ;  unsigned long last_jiffies ;  struct timer_list timer ; } cocci_id@p ;
...
position p;
@@
 struct loopback_cable cocci_id@p ;
...
position p;
@@
 struct loopback {  struct snd_card * card ;  struct mutex cable_lock ;  struct loopback_cable * cables [ MAX_PCM_SUBSTREAMS ] [ 2 ] ;  struct snd_pcm * pcm [ 2 ] ;  struct loopback_setup setup [ MAX_PCM_SUBSTREAMS ] [ 2 ] ;  const char * timer_source ; } cocci_id@p ;
...
position p;
@@
 struct loopback_setup {  unsigned int notify : 1 ;  unsigned int rate_shift ;  unsigned int format ;  unsigned int rate ;  unsigned int channels ;  struct snd_ctl_elem_id active_id ;  struct snd_ctl_elem_id format_id ;  struct snd_ctl_elem_id rate_id ;  struct snd_ctl_elem_id channels_id ; } cocci_id@p ;
...
position p;
@@
 struct snd_timer_id * cocci_id@p ;
...
position p;
@@
 struct loopback_ops cocci_id@p ;
...
position p;
@@
 struct loopback_ops {  int ( * open ) ( struct loopback_pcm * dpcm ) ;  int ( * start ) ( struct loopback_pcm * dpcm ) ;  int ( * stop ) ( struct loopback_pcm * dpcm ) ;  int ( * stop_sync ) ( struct loopback_pcm * dpcm ) ;  int ( * close_substream ) ( struct loopback_pcm * dpcm ) ;  int ( * close_cable ) ( struct loopback_pcm * dpcm ) ;  unsigned int ( * pos_update ) ( struct loopback_cable * cable ) ;  void ( * dpcm_info ) ( struct loopback_pcm * dpcm , struct snd_info_buffer * buffer ) ; } cocci_id@p ;
...
position p;
@@
 struct snd_timer_instance * cocci_id@p ;
...
typedef spinlock_t;
@@
 struct loopback_cable {  spinlock_t lock ;  struct loopback_pcm * streams [ 2 ] ;  struct snd_pcm_hardware hw ;  unsigned int valid ;  unsigned int running ;  unsigned int pause ;  struct loopback_ops * ops ;  struct {  int stream ;  struct snd_timer_id id ;  struct tasklet_struct event_tasklet ;  struct snd_timer_instance * instance ; } snd_timer ; } cocci_id@p ;
...
position p;
@@
 struct platform_device * cocci_id@p [ SNDRV_CARDS ] ;
...
position p;
@@
 struct loopback_setup * cocci_id@p ;
