...
position p;
@@
 struct packet_offload * cocci_id@p ;
...
typedef bool;
@@
 struct geneve_dev {  struct geneve_dev_node hlist4 ; #if IS_ENABLED(CONFIG_IPV6)  struct geneve_dev_node hlist6 ; #endif  struct net * net ;  struct net_device * dev ;  struct ip_tunnel_info info ;  struct geneve_sock __rcu * sock4 ; #if IS_ENABLED(CONFIG_IPV6)  struct geneve_sock __rcu * sock6 ; #endif  struct list_head next ;  struct gro_cells gro_cells ;  bool collect_md ;  bool use_udp6_rx_checksums ;  bool ttl_inherit ;  enum ifla_geneve_df df ; } cocci_id@p ;
...
position p;
@@
 enum ifla_geneve_df cocci_id@p ;
...
position p;
@@
 struct geneve_dev_node * cocci_id@p ;
...
position p;
@@
 const struct packet_offload * cocci_id@p ;
...
position p;
@@
 struct geneve_net {  struct list_head geneve_list ;  struct list_head sock_list ; } cocci_id@p ;
...
typedef bool;
@@
 struct geneve_sock {  bool collect_md ;  struct list_head list ;  struct socket * sock ;  struct rcu_head rcu ;  int refcnt ;  struct hlist_head vni_list [ VNI_HASH_SIZE ] ; } cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ IFLA_GENEVE_MAX + 1 ] ;
...
position p;
@@
 struct geneve_dev_node {  struct hlist_node hlist ;  struct geneve_dev * geneve ; } cocci_id@p ;
