...
position p;
@@
 struct xfrm_trans_cb {  union {  struct inet_skb_parm h4 ; #if IS_ENABLED(CONFIG_IPV6)  struct inet6_skb_parm h6 ; #endif } header ;  int ( * finish ) ( struct net * net , struct sock * sk , struct sk_buff * skb ) ; } cocci_id@p ;
...
position p;
@@
 struct ip_beet_phdr * cocci_id@p ;
...
position p;
@@
 const struct xfrm_input_afinfo * cocci_id@p ;
...
position p;
@@
 struct xfrm_trans_tasklet {  struct tasklet_struct tasklet ;  struct sk_buff_head queue ; } cocci_id@p ;
...
position p;
@@
 struct xfrm_trans_cb * cocci_id@p ;
...
position p;
@@
 const __rcu struct xfrm_input_afinfo * cocci_id@p [ AF_INET6 + 1 ] ;
...
position p;
@@
 struct xfrm_trans_tasklet * cocci_id@p ;
