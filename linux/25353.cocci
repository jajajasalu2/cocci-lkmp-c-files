...
position p;
@@
 int ( * cocci_id@p ) ( struct evsel * , struct machine * ) ;
...
position p;
@@
 struct callchain_result cocci_id@p [ ] ;
...
typedef u64;
@@
 struct callchain_result {  u64 nr ;  struct {  const char * dso ;  const char * sym ; } node [ 10 ] ; } cocci_id@p ;
...
typedef u64,u32;
@@
 struct sample {  u32 pid ;  u64 ip ;  struct thread * thread ;  struct map * map ;  struct symbol * sym ; } cocci_id@p ;
...
typedef u64;
@@
 struct result {  u64 children ;  u64 self ;  const char * comm ;  const char * dso ;  const char * sym ; } cocci_id@p ;
