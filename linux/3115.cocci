...
position p;
@@
 void cocci_id@p ( struct resource_info * , struct drbd_resource * ) ;
...
position p;
@@
 struct drbd_device * constcocci_id ;
...
position p;
@@
 struct new_c_uuid_parms cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct netlink_callback * cb ) ;
...
position p;
@@
 struct start_ov_parms cocci_id@p ;
...
position p;
@@
 struct detach_parms cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct device_info * , struct drbd_device * ) ;
...
typedef u64,u32,s32;
@@
 struct md_offsets_and_sizes {  u64 last_agreed_sect ;  u64 md_offset ;  s32 al_offset ;  s32 bm_offset ;  u32 md_size_sect ;  u32 al_stripes ;  u32 al_stripe_size_4k ; } cocci_id@p ;
...
position p;
@@
 enum drbd_peer_state cocci_id@p ;
...
position p;
@@
 struct drbd_resource * constcocci_id ;
...
position p;
@@
 struct drbd_helper_info cocci_id@p ;
...
position p;
@@
 struct device_statistics cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct sk_buff * skb , struct netlink_callback * cb ) ;
...
position p;
@@
 struct drbd_config_context * cocci_id@p ;
...
position p;
@@
 struct timeout_parms cocci_id@p ;
...
position p;
@@
 struct resize_parms cocci_id@p ;
...
position p;
@@
 struct resource_statistics cocci_id@p ;
...
position p;
@@
 struct peer_device_statistics * cocci_id@p ;
...
position p;
@@
 struct drbd_connection * constcocci_id ;
...
position p;
@@
 enum mdf_peer_flag { MDF_PEER_CONNECTED = 1 << 0 , MDF_PEER_OUTDATED = 1 << 1 , MDF_PEER_FENCING = 1 << 2 , MDF_PEER_FULL_SYNC = 1 << 3 , } cocci_id@p ;
...
position p;
@@
 struct set_role_parms cocci_id@p ;
...
position p;
@@
 struct crypto {  struct crypto_shash * verify_tfm ;  struct crypto_shash * csums_tfm ;  struct crypto_shash * cram_hmac_tfm ;  struct crypto_shash * integrity_tfm ; } cocci_id@p ;
...
position p;
@@
 struct drbd_genlmsghdr * cocci_id@p ;
...
position p;
@@
 struct drbd_peer_device * constcocci_id ;
...
position p;
@@
 const struct sib_info * cocci_id@p ;
...
position p;
@@
 struct request_queue * constcocci_id ;
...
position p;
@@
 struct drbd_notification_header cocci_id@p ;
...
position p;
@@
 enum { SINGLE_RESOURCE , ITERATE_RESOURCES , } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct sk_buff * skb , struct genl_info * info ) ;
...
position p;
@@
 struct connection_statistics cocci_id@p ;
...
position p;
@@
 struct disconnect_parms cocci_id@p ;
