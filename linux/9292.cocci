...
position p;
@@
 void cocci_id@p ( struct lbs_private * priv ) ;
...
position p;
@@
 const struct lbs_debugfs_files cocci_id@p [ ] ;
...
typedef umode_t;
@@
 struct lbs_debugfs_files {  const char * name ;  umode_t perm ;  struct file_operations fops ; } cocci_id@p ;
...
typedef u32;
@@
 struct debug_data {  char name [ 32 ] ;  u32 size ;  size_t addr ; } cocci_id@p ;
...
position p;
@@
 struct mrvl_ie_thresholds * cocci_id@p ;
...
position p;
@@
 struct cmd_ds_802_11_subscribe_event * cocci_id@p ;
