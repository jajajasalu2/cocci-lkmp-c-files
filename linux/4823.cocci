...
typedef u16;
@@
 struct find_gid_index_context {  u16 vlan_id ;  enum ib_gid_type gid_type ; } cocci_id@p ;
...
position p;
@@
 union rdma_network_hdr * cocci_id@p ;
...
position p;
@@
 const union rdma_network_hdr * cocci_id@p ;
...
position p;
@@
 struct ib_drain_cqe {  struct ib_cqe cqe ;  struct completion done ; } cocci_id@p ;
...
position p;
@@
 struct find_gid_index_context cocci_id@p ;
...
position p;
@@
 enum rdma_transport_type cocci_id@p ;
...
position p;
@@
 const struct {  int valid ;  enum ib_qp_attr_mask req_param [ IB_QPT_MAX ] ;  enum ib_qp_attr_mask opt_param [ IB_QPT_MAX ] ; } cocci_id@p [ IB_QPS_ERR + 1 ] [ IB_QPS_ERR + 1 ] ;
...
position p;
@@
 int cocci_id@p ( struct ib_device * device , struct rdma_ah_attr * ah_attr ) ;
...
position p;
@@
 struct ib_drain_cqe * cocci_id@p ;
...
position p;
@@
 struct ib_qp_security * cocci_id@p ;
...
typedef u64;
@@
 int ( * cocci_id@p ) ( struct ib_mr * , u64 ) ;
...
position p;
@@
 struct ib_mr_status * cocci_id@p ;
