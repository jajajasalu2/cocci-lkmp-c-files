...
position p;
@@
 struct kretprobe_blackpoint cocci_id@p [ ] ;
...
position p;
@@
 struct swap_insn_args cocci_id@p ;
...
position p;
@@
 struct kprobe_ctlblk * cocci_id@p ;
...
position p;
@@
 struct swap_insn_args {  struct kprobe * p ;  unsigned int arm_kprobe : 1 ; } cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ PAGE_SIZE ] __aligned ( PAGE_SIZE ) ;
