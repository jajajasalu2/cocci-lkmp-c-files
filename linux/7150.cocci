...
position p;
@@
 struct gnttab_unmap_grant_ref cocci_id@p [ VSCSI_GRANT_BATCH ] ;
...
symbol cocci_id;
position p;
typedef grant_handle_t,uint16_t,uint32_t,uint8_t;
@@
 struct vscsibk_pend {  uint16_t rqid ;  uint8_t cmnd [ VSCSIIF_MAX_COMMAND_SIZE ] ;  uint8_t cmd_len ;  uint8_t sc_data_direction ;  uint16_t n_sg ;  uint16_t n_grants ;  uint32_t data_len ;  uint32_t result ;  struct vscsibk_info * info ;  struct v2p_entry * v2p ;  struct scatterlist * sgl ;  uint8_t sense_buffer [ VSCSIIF_SENSE_BUFFERSIZE ] ;  grant_handle_t grant_handles [ VSCSI_MAX_GRANTS ] ;  struct page * pages [ VSCSI_MAX_GRANTS ] ;  struct se_cmd se_cmd ;  struct completion tmr_done ; } cocci_id@p ;
...
typedef u16;
@@
 struct scsiback_tpg {  u16 tport_tpgt ;  int tv_tpg_port_count ;  int tv_tpg_fe_count ;  struct list_head tv_tpg_list ;  struct mutex tv_tpg_mutex ;  struct scsiback_nexus * tpg_nexus ;  struct scsiback_tport * tport ;  struct se_portal_group se_tpg ;  char param_alias [ VSCSI_NAMELEN ] ;  struct list_head info_list ; } cocci_id@p ;
...
position p;
@@
 const struct xenbus_device_id cocci_id@p [ ] ;
...
position p;
@@
 struct xenbus_driver cocci_id@p ;
...
position p;
@@
 struct vscsibk_info * cocci_id@p ;
...
position p;
@@
 struct vscsiif_sring * cocci_id@p ;
...
position p;
@@
 struct vscsibk_pend * cocci_id@p ;
...
position p;
@@
 struct scsiback_tpg cocci_id@p ;
...
typedef u8,u64;
@@
 struct scsiback_tport {  u8 tport_proto_id ;  u64 tport_wwpn ;  char tport_name [ VSCSI_NAMELEN ] ;  struct se_wwn tport_wwn ; } cocci_id@p ;
...
position p;
@@
 struct scsiback_tport cocci_id@p ;
...
position p;
@@
 struct gnttab_map_grant_ref cocci_id@p [ VSCSI_GRANT_BATCH ] ;
...
position p;
@@
 struct vscsiif_response * cocci_id@p ;
...
position p;
@@
 unsigned char cocci_id@p [ VSCSI_NAMELEN ] ;
...
position p;
@@
 struct vscsiif_request * cocci_id@p ;
...
position p;
@@
 struct gnttab_map_grant_ref * cocci_id@p ;
...
position p;
@@
 struct scsiif_request_segment cocci_id@p ;
...
position p;
@@
 struct vscsiif_back_ring * cocci_id@p ;
...
position p;
@@
 struct scsiback_nexus {  struct se_session * tvn_se_sess ; } cocci_id@p ;
...
position p;
@@
 enum tcm_tmreq_table cocci_id@p ;
...
position p;
@@
 struct ids_tuple {  unsigned int hst ;  unsigned int chn ;  unsigned int tgt ;  unsigned int lun ; } cocci_id@p ;
...
position p;
@@
 struct page * cocci_id@p [ VSCSI_GRANT_BATCH ] ;
...
position p;
@@
 struct scsiback_nexus * cocci_id@p ;
...
position p;
@@
 struct xenbus_device * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef wait_queue_head_t,domid_t,atomic_t,spinlock_t;
@@
 struct vscsibk_info {  struct xenbus_device * dev ;  domid_t domid ;  unsigned int irq ;  struct vscsiif_back_ring ring ;  int ring_error ;  spinlock_t ring_lock ;  atomic_t nr_unreplied_reqs ;  spinlock_t v2p_lock ;  struct list_head v2p_entry_lists ;  wait_queue_head_t waiting_to_free ; } cocci_id@p ;
...
position p;
@@
 struct v2p_entry {  struct ids_tuple v ;  struct scsiback_tpg * tpg ;  unsigned int lun ;  struct kref kref ;  struct list_head l ; } cocci_id@p ;
