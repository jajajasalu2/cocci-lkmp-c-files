...
position p;
@@
 unsigned long cocci_id@p ( unsigned long , unsigned long , unsigned long , unsigned long ) ;
...
position p;
@@
 int ( * cocci_id@p ) ( const struct device_node * ) ;
...
symbol cocci_id;
position p;
typedef psci_initcall_t;
@@
 psci_initcall_t cocci_id@p ;
...
position p;
@@
 enum arm_smccc_conduit cocci_id@p ;
...
position p;
@@
 struct psci_operations cocci_id@p ;
...
position p;
@@
 enum psci_function { PSCI_FN_CPU_SUSPEND , PSCI_FN_CPU_ON , PSCI_FN_CPU_OFF , PSCI_FN_MIGRATE , PSCI_FN_MAX , } cocci_id@p ;
