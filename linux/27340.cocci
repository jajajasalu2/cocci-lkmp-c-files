...
position p;
@@
 struct mnt_namespace * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct mount * mnt ) ;
...
position p;
@@
 enum umount_tree_flags cocci_id@p ;
...
typedef bool;
@@
 bool cocci_id@p ( const struct super_block * sb , int * new_mnt_flags ) ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct vfsmount * , void * ) ;
...
position p;
@@
 void cocci_id@p ( struct mnt_namespace * ) ;
...
position p;
@@
 enum umount_tree_flags { UMOUNT_SYNC = 1 , UMOUNT_PROPAGATE = 2 , UMOUNT_CONNECTED = 4 , } cocci_id@p ;
...
typedef bool;
@@
 struct mnt_namespace * cocci_id@p ( struct user_namespace * , bool ) ;
