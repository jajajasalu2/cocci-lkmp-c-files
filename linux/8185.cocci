...
typedef u32;
@@
 u32 cocci_id@p [ CGX_LINK_SPEED_MAX ] ;
...
position p;
@@
 struct cgx_link_event cocci_id@p ;
...
typedef bool;
@@
 int cocci_id@p ( struct cgx * cgx , int lmac_id , bool en ) ;
...
symbol cocci_id;
position p;
typedef u64,u8,bool,wait_queue_head_t,spinlock_t;
@@
 struct lmac {  wait_queue_head_t wq_cmd_cmplt ;  struct mutex cmd_lock ;  u64 resp ;  struct cgx_link_user_info link_info ;  struct cgx_event_cb event_cb ;  spinlock_t event_cb_lock ;  bool cmd_pend ;  struct cgx * cgx ;  u8 lmac_id ;  char * name ; } cocci_id@p ;
...
typedef u8;
@@
 struct cgx {  void __iomem * reg_base ;  struct pci_dev * pdev ;  u8 cgx_id ;  u8 lmac_count ;  struct lmac * lmac_idmap [ MAX_LMAC_PER_CGX ] ;  struct work_struct cgx_cmd_work ;  struct workqueue_struct * cgx_cmd_workq ;  struct list_head cgx_list ; } cocci_id@p ;
...
position p;
@@
 struct cgx_event_cb * cocci_id@p ;
