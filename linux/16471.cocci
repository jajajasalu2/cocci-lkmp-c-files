...
@@
 struct dj_receiver_dev {  struct hid_device * mouse ;  struct hid_device * keyboard ;  struct hid_device * hidpp ;  struct dj_device * paired_dj_devices [ DJ_MAX_PAIRED_DEVICES + DJ_DEVICE_INDEX_MIN ] ;  struct list_head list ;  struct kref kref ;  struct work_struct work ;  struct kfifo notif_fifo ;  unsigned long last_query ;  bool ready ;  enum recvr_type type ;  unsigned int unnumbered_application ;  spinlock_t lock ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct dj_receiver_dev * djrcv_dev ) ;
...
typedef u8,u64;
@@
 struct dj_workitem {  u8 type ;  u8 device_index ;  u8 device_type ;  u8 quad_id_msb ;  u8 quad_id_lsb ;  u64 reports_supported ; } cocci_id@p ;
...
position p;
@@
 struct dj_receiver_dev * cocci_id@p ;
...
typedef u8,u64;
@@
 struct dj_device {  struct hid_device * hdev ;  struct dj_receiver_dev * dj_receiver_dev ;  u64 reports_supported ;  u8 device_index ; } cocci_id@p ;
...
typedef u8;
@@
 struct dj_report {  u8 report_id ;  u8 device_index ;  u8 report_type ;  u8 report_params [ DJREPORT_SHORT_LENGTH - 3 ] ; } cocci_id@p ;
...
typedef u8;
@@
 const u8 cocci_id@p [ NUMBER_OF_HID_REPORTS ] ;
