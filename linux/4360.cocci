...
position p;
@@
 struct si_sm_data {  enum smic_states state ;  struct si_sm_io * io ;  unsigned char write_data [ MAX_SMIC_WRITE_SIZE ] ;  int write_pos ;  int write_count ;  int orig_write_count ;  unsigned char read_data [ MAX_SMIC_READ_SIZE ] ;  int read_pos ;  int truncated ;  unsigned int error_retries ;  long smic_timeout ; } cocci_id@p ;
...
position p;
@@
 enum smic_states { SMIC_IDLE , SMIC_START_OP , SMIC_OP_OK , SMIC_WRITE_START , SMIC_WRITE_NEXT , SMIC_WRITE_END , SMIC_WRITE2READ , SMIC_READ_START , SMIC_READ_NEXT , SMIC_READ_END , SMIC_HOSED , } cocci_id@p ;
...
position p;
@@
 const struct si_sm_handlers cocci_id@p ;
