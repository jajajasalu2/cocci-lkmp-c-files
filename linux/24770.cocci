...
typedef spinlock_t;
@@
 spinlock_t cocci_id@p [ CONNTRACK_LOCKS ] ;
...
position p;
@@
 struct conntrack_gc_work * cocci_id@p ;
...
typedef bool,u32;
@@
 struct conntrack_gc_work {  struct delayed_work dwork ;  u32 last_bucket ;  bool exiting ;  bool early_drop ;  long next_gc_run ; } cocci_id@p ;
...
position p;
@@
 struct {  __be16 sport ;  __be16 dport ; } cocci_id@p ;
...
position p;
@@
 struct nf_conntrack * cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct nf_conn * i , void * data ) ;
...
position p;
@@
 struct ip_conntrack_stat cocci_id@p ;
