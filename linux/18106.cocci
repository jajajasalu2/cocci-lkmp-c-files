...
position p;
@@
 bL_switch_completion_handler cocci_id@p ;
...
position p;
@@
 struct bL_thread cocci_id@p [ NR_CPUS ] ;
...
position p;
@@
 unsigned int cocci_id@p [ NR_CPUS ] ;
...
position p;
@@
 unsigned int cocci_id@p [ MAX_CPUS_PER_CLUSTER ] [ MAX_NR_CLUSTERS ] ;
...
@@
 struct bL_thread {  spinlock_t lock ;  struct task_struct * task ;  wait_queue_head_t wq ;  int wanted_cluster ;  struct completion started ;  bL_switch_completion_handler completer ;  void * completer_cookie ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( void ( * fn ) ( void * ) , void * arg , void * sp ) ;
