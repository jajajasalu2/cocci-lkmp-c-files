...
position p;
@@
 struct goldfish_pipe_dev cocci_id@p ;
...
typedef u32;
@@
 struct signalled_pipe_buffer {  u32 id ;  u32 flags ; } cocci_id@p ;
...
@@
 struct goldfish_pipe_dev {  void * magic ;  spinlock_t lock ;  struct goldfish_pipe * * pipes ;  u32 pipes_capacity ;  struct goldfish_pipe_dev_buffers * buffers ;  struct goldfish_pipe * first_signalled_pipe ;  struct device * pdev_dev ;  int irq ;  int version ;  unsigned char __iomem * base ;  struct tasklet_struct irq_tasklet ;  struct miscdevice miscdev ; } cocci_id@p ;
...
typedef u64,u32;
@@
 struct open_command_param {  u64 command_buffer_ptr ;  u32 rw_params_max_count ; } cocci_id@p ;
...
position p;
@@
 enum { MAX_BUFFERS_PER_COMMAND = 336 , MAX_SIGNALLED_PIPES = 64 , INITIAL_PIPES_CAPACITY = 64 , } cocci_id@p ;
...
position p;
@@
 struct goldfish_pipe_dev_buffers * cocci_id@p ;
...
position p;
@@
 struct goldfish_pipe * cocci_id@p ;
...
position p;
@@
 enum { PIPE_DRIVER_VERSION = 2 , PIPE_CURRENT_DEVICE_VERSION = 2 , } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct platform_device * pdev , struct goldfish_pipe_dev * dev ) ;
...
position p;
@@
 struct goldfish_pipe_command * cocci_id@p ;
...
typedef u64,u32,s32;
@@
 struct goldfish_pipe_command {  s32 cmd ;  s32 id ;  s32 status ;  s32 reserved ;  union {  struct {  u32 buffers_count ;  s32 consumed_size ;  u64 ptrs [ MAX_BUFFERS_PER_COMMAND ] ;  u32 sizes [ MAX_BUFFERS_PER_COMMAND ] ; } rw_params ; } ; } cocci_id@p ;
...
position p;
@@
 struct goldfish_pipe_dev_buffers {  struct open_command_param open_command_params ;  struct signalled_pipe_buffer signalled_pipe_buffers [ MAX_SIGNALLED_PIPES ] ; } cocci_id@p ;
...
position p;
@@
 struct page * cocci_id@p [ MAX_BUFFERS_PER_COMMAND ] ;
...
@@
 struct goldfish_pipe {  u32 id ;  unsigned long flags ;  unsigned long signalled_flags ;  struct goldfish_pipe_command * command_buffer ;  struct goldfish_pipe * prev_signalled ;  struct goldfish_pipe * next_signalled ;  struct mutex lock ;  wait_queue_head_t wake_queue ;  struct goldfish_pipe_dev * dev ;  struct page * pages [ MAX_BUFFERS_PER_COMMAND ] ; } cocci_id@p ;
