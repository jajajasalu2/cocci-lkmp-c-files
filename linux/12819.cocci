...
position p;
@@
 struct coresight_desc cocci_id@p ;
...
typedef phys_addr_t;
@@
 struct channel_space {  void __iomem * base ;  phys_addr_t phys ;  unsigned long * guaranteed ; } cocci_id@p ;
...
position p;
@@
 struct coresight_platform_data * cocci_id@p ;
...
position p;
@@
 const struct coresight_ops_source cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,u32,spinlock_t,local_t;
@@
 struct stm_drvdata {  void __iomem * base ;  struct clk * atclk ;  struct coresight_device * csdev ;  spinlock_t spinlock ;  struct channel_space chs ;  struct stm_data stm ;  local_t mode ;  u8 traceid ;  u32 write_bytes ;  u32 stmsper ;  u32 stmspscr ;  u32 numsp ;  u32 stmheer ;  u32 stmheter ;  u32 stmhebsr ; } cocci_id@p ;
...
position p;
@@
 struct coresight_device * cocci_id@p ;
...
position p;
@@
 const struct coresight_ops cocci_id@p ;
...
position p;
@@
 enum stm_pkt_type { STM_PKT_TYPE_DATA = 0x98 , STM_PKT_TYPE_FLAG = 0xE8 , STM_PKT_TYPE_TRIG = 0xF8 , } cocci_id@p ;
