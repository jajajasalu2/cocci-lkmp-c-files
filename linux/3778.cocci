...
symbol cocci_id;
position p;
typedef u64,u8,bool,u32;
@@
 struct s5p_hash_reqctx {  struct s5p_aes_dev * dd ;  bool op_update ;  u64 digcnt ;  u8 digest [ SHA256_DIGEST_SIZE ] ;  unsigned int nregs ;  u32 engine ;  struct scatterlist * sg ;  unsigned int sg_len ;  struct scatterlist sgl [ 2 ] ;  unsigned int skip ;  unsigned int total ;  bool finup ;  bool error ;  u32 bufcnt ;  u8 buffer [ 0 ] ; } cocci_id@p ;
...
position p;
@@
 const struct scatterlist * cocci_id@p ;
...
position p;
@@
 struct s5p_hash_reqctx cocci_id@p ;
...
position p;
@@
 struct s5p_hash_ctx {  struct s5p_aes_dev * dd ;  unsigned long flags ;  struct crypto_shash * fallback ; } cocci_id@p ;
...
position p;
@@
 struct s5p_aes_reqctx cocci_id@p ;
...
position p;
@@
 const struct platform_device * cocci_id@p ;
...
position p;
@@
 struct s5p_hash_ctx cocci_id@p ;
...
typedef u8;
@@
 struct s5p_aes_ctx {  struct s5p_aes_dev * dev ;  u8 aes_key [ AES_MAX_KEY_SIZE ] ;  u8 nonce [ CTR_RFC3686_NONCE_SIZE ] ;  int keylen ; } cocci_id@p ;
...
position p;
@@
 const struct samsung_aes_variant cocci_id@p ;
...
position p;
@@
 const struct s5p_hash_reqctx * cocci_id@p ;
...
position p;
@@
 struct s5p_aes_reqctx {  unsigned long mode ; } cocci_id@p ;
...
position p;
@@
 struct samsung_aes_variant {  unsigned int aes_offset ;  unsigned int hash_offset ;  const char * clk_names [ 2 ] ; } cocci_id@p ;
