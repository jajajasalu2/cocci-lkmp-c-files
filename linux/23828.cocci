...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_U32_MAX + 1 ] ;
...
position p;
@@
 struct tc_u_common {  struct tc_u_hnode __rcu * hlist ;  void * ptr ;  int refcnt ;  struct idr handle_idr ;  struct hlist_node hnode ;  long knodes ; } cocci_id@p ;
...
position p;
@@
 struct {  struct tc_u_knode * knode ;  unsigned int off ; } cocci_id@p [ TC_U32_MAXDEPTH ] ;
...
position p;
@@
 const struct tc_u32_sel * cocci_id@p ;
...
typedef bool,u32;
@@
 struct tc_u_hnode {  struct tc_u_hnode __rcu * next ;  u32 handle ;  u32 prio ;  int refcnt ;  unsigned int divisor ;  struct idr handle_idr ;  bool is_root ;  struct rcu_head rcu ;  u32 flags ;  struct tc_u_knode __rcu * ht [ 1 ] ; } cocci_id@p ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_U32_MAX + 1 ] ;
