...
typedef __le32;
@@
 union ehci_shadow {  struct ehci_qh * qh ;  __le32 * hw_next ;  void * ptr ; } cocci_id@p ;
...
position p;
@@
 enum ehci_timer_action { TIMER_IO_WATCHDOG , TIMER_IAA_WATCHDOG , TIMER_ASYNC_SHRINK , TIMER_ASYNC_OFF , } cocci_id@p ;
...
@@
 struct oxu_hcd {  unsigned int is_otg : 1 ;  u8 qh_used [ QHEAD_NUM ] ;  u8 qtd_used [ QTD_NUM ] ;  u8 db_used [ BUFFER_NUM ] ;  u8 murb_used [ MURB_NUM ] ;  struct oxu_onchip_mem __iomem * mem ;  spinlock_t mem_lock ;  struct timer_list urb_timer ;  struct ehci_caps __iomem * caps ;  struct ehci_regs __iomem * regs ;  u32 hcs_params ;  spinlock_t lock ;  struct ehci_qh * async ;  struct ehci_qh * reclaim ;  unsigned int reclaim_ready : 1 ;  unsigned int scanning : 1 ;  unsigned int periodic_size ;  __le32 * periodic ;  dma_addr_t periodic_dma ;  unsigned int i_thresh ;  union ehci_shadow * pshadow ;  int next_uframe ;  unsigned int periodic_sched ;  unsigned long reset_done [ EHCI_MAX_ROOT_PORTS ] ;  unsigned long bus_suspended ;  unsigned long companion_ports ;  struct timer_list watchdog ;  unsigned long actions ;  unsigned int stamp ;  unsigned long next_statechange ;  u32 command ;  struct list_head urb_list ;  struct oxu_murb * murb_pool ;  unsigned int urb_len ;  u8 sbrn ; } cocci_id@p ;
...
typedef u8;
@@
 struct oxu_murb {  struct urb urb ;  struct urb * main ;  u8 last ; } cocci_id@p ;
...
position p;
@@
 struct oxu_info {  struct usb_hcd * hcd [ 2 ] ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct oxu_hcd * oxu ) ;
...
position p;
@@
 enum ehci_timer_action cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct oxu_hcd * oxu , struct ehci_qh * qh ) ;
...
position p;
@@
 const struct ehci_qh * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct oxu_hcd * oxu , struct ehci_qh * qh ) ;
...
typedef u16;
@@
 int cocci_id@p ( struct usb_hcd * hcd , u16 typeReq , u16 wValue , u16 wIndex , char * buf , u16 wLength ) ;
