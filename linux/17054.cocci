...
position p;
@@
 struct analog_types cocci_id@p [ ] ;
...
position p;
@@
 struct analog {  struct input_dev * dev ;  int mask ;  short * buttons ;  char name [ ANALOG_MAX_NAME_LENGTH ] ;  char phys [ ANALOG_MAX_PHYS_LENGTH ] ; } cocci_id@p ;
...
position p;
@@
 struct analog_port {  struct gameport * gameport ;  struct analog analog [ 2 ] ;  unsigned char mask ;  char saitek ;  char cooked ;  int bads ;  int reads ;  int speed ;  int loop ;  int fuzz ;  int axes [ 4 ] ;  int buttons ;  int initial [ 4 ] ;  int axtime ; } cocci_id@p ;
...
position p;
@@
 struct analog_types {  char * name ;  int value ; } cocci_id@p ;
