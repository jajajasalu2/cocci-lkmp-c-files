...
position p;
@@
 enum apm_suspend_state { SUSPEND_NONE , SUSPEND_PENDING , SUSPEND_READ , SUSPEND_ACKED , SUSPEND_ACKTO , SUSPEND_WAIT , SUSPEND_DONE , } cocci_id@p ;
...
position p;
@@
 struct apm_user {  struct list_head list ;  unsigned int suser : 1 ;  unsigned int writer : 1 ;  unsigned int reader : 1 ;  int suspend_result ;  enum apm_suspend_state suspend_state ;  struct apm_queue queue ; } cocci_id@p ;
...
typedef apm_event_t;
@@
 struct apm_queue {  unsigned int event_head ;  unsigned int event_tail ;  apm_event_t events [ APM_MAX_EVENTS ] ; } cocci_id@p ;
...
position p;
@@
 void ( * cocci_id@p ) ( struct apm_power_info * ) ;
