...
position p;
@@
 struct ec_event_queue {  int capacity ;  int head ;  int tail ;  struct ec_event * entries [ 0 ] ; } cocci_id@p ;
...
position p;
@@
 struct ec_event_queue * cocci_id@p ;
...
@@
 struct event_device_data {  struct ec_event_queue * events ;  spinlock_t queue_lock ;  wait_queue_head_t wq ;  struct device dev ;  struct cdev cdev ;  bool exist ;  atomic_t available ; } cocci_id@p ;
...
position p;
@@
 struct event_device_data * cocci_id@p ;
