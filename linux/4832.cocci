...
symbol cocci_id;
position p;
typedef u8,u16,atomic_t,spinlock_t;
@@
 struct mcast_group {  struct ib_sa_mcmember_rec rec ;  struct rb_node node ;  struct mcast_port * port ;  spinlock_t lock ;  struct work_struct work ;  struct list_head pending_list ;  struct list_head active_list ;  struct mcast_member * last_join ;  int members [ NUM_JOIN_MEMBERSHIP_TYPES ] ;  atomic_t refcount ;  enum mcast_group_state state ;  struct ib_sa_query * query ;  u16 pkey_index ;  u8 leave_state ;  int retries ; } cocci_id@p ;
...
position p;
@@
 enum mcast_group_state { MCAST_IDLE , MCAST_BUSY , MCAST_GROUP_ERROR , MCAST_PKEY_EVENT , } cocci_id@p ;
...
position p;
@@
 struct mcast_device * cocci_id@p ;
...
position p;
@@
 enum mcast_group_state cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( int status , struct ib_sa_multicast * multicast ) ;
...
typedef atomic_t;
@@
 struct mcast_member {  struct ib_sa_multicast multicast ;  struct ib_sa_client * client ;  struct mcast_group * group ;  struct list_head list ;  enum mcast_state state ;  atomic_t refcount ;  struct completion comp ; } cocci_id@p ;
...
position p;
@@
 enum { MCAST_INVALID_PKEY_INDEX = 0xFFFF , } cocci_id@p ;
...
position p;
@@
 enum mcast_state { MCAST_JOINING , MCAST_MEMBER , MCAST_ERROR , } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,atomic_t,spinlock_t;
@@
 struct mcast_port {  struct mcast_device * dev ;  spinlock_t lock ;  struct rb_root table ;  atomic_t refcount ;  struct completion comp ;  u8 port_num ; } cocci_id@p ;
...
position p;
@@
 struct mcast_member cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( int status , struct ib_sa_mcmember_rec * rec , void * context ) ;
...
position p;
@@
 struct mcast_device {  struct ib_device * device ;  struct ib_event_handler event_handler ;  int start_port ;  int end_port ;  struct mcast_port port [ 0 ] ; } cocci_id@p ;
