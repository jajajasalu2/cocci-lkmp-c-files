...
position p;
@@
 struct lowpan_btle_dev * * cocci_id@p ;
...
position p;
@@
 struct skb_cb {  struct in6_addr addr ;  struct in6_addr gw ;  struct l2cap_chan * chan ; } cocci_id@p ;
...
typedef atomic_t;
@@
 struct lowpan_btle_dev {  struct list_head list ;  struct hci_dev * hdev ;  struct net_device * netdev ;  struct list_head peers ;  atomic_t peer_count ;  struct work_struct delete_netdev ;  struct delayed_work notify_peers ; } cocci_id@p ;
...
typedef bool;
@@
 struct set_enable {  struct work_struct work ;  bool flag ; } cocci_id@p ;
...
position p;
@@
 struct lowpan_peer {  struct list_head list ;  struct rcu_head rcu ;  struct l2cap_chan * chan ;  unsigned char lladdr [ ETH_ALEN ] ;  struct in6_addr peer_addr ; } cocci_id@p ;
