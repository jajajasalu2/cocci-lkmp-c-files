...
@@
 struct olpc_ec_priv {  struct olpc_ec_driver * drv ;  u8 version ;  struct work_struct worker ;  struct mutex cmd_lock ;  struct regulator_dev * dcon_rdev ;  bool dcon_enabled ;  struct list_head cmd_q ;  spinlock_t cmd_q_lock ;  struct dentry * dbgfs_dir ;  u16 ec_wakeup_mask ;  bool suspended ; } cocci_id@p ;
...
position p;
@@
 unsigned char cocci_id@p [ EC_MAX_CMD_REPLY ] ;
...
position p;
@@
 unsigned int cocci_id@p [ EC_MAX_CMD_ARGS ] ;
...
position p;
@@
 unsigned char cocci_id@p [ EC_MAX_CMD_ARGS ] ;
...
position p;
@@
 struct olpc_ec_priv cocci_id@p ;
