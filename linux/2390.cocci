...
position p;
@@
 struct pch_dma_desc * cocci_id@p ;
...
position p;
@@
 struct pch_dma {  struct dma_device dma ;  void __iomem * membase ;  struct dma_pool * pool ;  struct pch_dma_regs regs ;  struct pch_dma_desc_regs ch_regs [ MAX_CHAN_NR ] ;  struct pch_dma_chan channels [ MAX_CHAN_NR ] ; } cocci_id@p ;
...
typedef u32;
@@
 struct pch_dma_desc_regs {  u32 dev_addr ;  u32 mem_addr ;  u32 size ;  u32 next ; } cocci_id@p ;
...
position p;
@@
 struct pch_dma_desc {  struct pch_dma_desc_regs regs ;  struct dma_async_tx_descriptor txd ;  struct list_head desc_node ;  struct list_head tx_list ; } cocci_id@p ;
...
position p;
@@
 struct pch_dma_chan cocci_id@p ;
...
typedef spinlock_t;
@@
 struct pch_dma_chan {  struct dma_chan chan ;  void __iomem * membase ;  enum dma_transfer_direction dir ;  struct tasklet_struct tasklet ;  unsigned long err_status ;  spinlock_t lock ;  struct list_head active_list ;  struct list_head queue ;  struct list_head free_list ;  unsigned int descs_allocated ; } cocci_id@p ;
...
typedef u32;
@@
 struct pch_dma_regs {  u32 dma_ctl0 ;  u32 dma_ctl1 ;  u32 dma_ctl2 ;  u32 dma_ctl3 ;  u32 dma_sts0 ;  u32 dma_sts1 ;  u32 dma_sts2 ;  u32 reserved3 ;  struct pch_dma_desc_regs desc [ MAX_CHAN_NR ] ; } cocci_id@p ;
...
position p;
@@
 struct pch_dma_regs * cocci_id@p ;
