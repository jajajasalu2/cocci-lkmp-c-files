...
position p;
@@
 struct preempt_client cocci_id@p [ 4 ] ;
...
position p;
@@
 enum { RING_START_IDX = 0 , RING_TAIL_IDX , MAX_IDX , } cocci_id@p ;
...
position p;
@@
 const struct phase {  const char * name ;  unsigned int flags ; } cocci_id@p [ ] ;
...
position p;
@@
 struct i915_request * cocci_id@p [ 3 ] ;
...
position p;
@@
 struct i915_request * cocci_id@p [ ARRAY_SIZE ( client ) ] ;
...
position p;
@@
 struct preempt_smoke cocci_id@p ;
...
typedef u32;
@@
 const struct {  u32 reg ;  u32 offset ;  const char * name ; } cocci_id@p [ ] ;
...
position p;
@@
 struct intel_context * cocci_id@p [ 2 ] ;
...
position p;
@@
 struct task_struct * cocci_id@p [ I915_NUM_ENGINES ] ;
...
position p;
@@
 struct preempt_smoke cocci_id@p [ I915_NUM_ENGINES ] ;
...
position p;
@@
 struct i915_gem_context * cocci_id@p [ 16 ] ;
...
position p;
@@
 struct preempt_client {  struct igt_spinner spin ;  struct i915_gem_context * ctx ; } cocci_id@p ;
...
position p;
@@
 struct preempt_smoke {  struct intel_gt * gt ;  struct i915_gem_context * * contexts ;  struct intel_engine_cs * engine ;  struct drm_i915_gem_object * batch ;  unsigned int ncontext ;  struct rnd_state prng ;  unsigned long count ; } cocci_id@p ;
...
position p;
@@
 struct preempt_client cocci_id@p ;
...
position p;
@@
 struct i915_request * cocci_id@p [ 16 ] ;
...
position p;
@@
 struct intel_context * cocci_id@p [ 16 ] ;
...
position p;
@@
 struct i915_request * cocci_id@p [ MAX_ENGINE_INSTANCE + 1 ] ;
...
position p;
@@
 struct igt_live_test cocci_id@p ;
...
position p;
@@
 struct live_preempt_cancel cocci_id@p ;
...
position p;
@@
 struct intel_engine_cs * cocci_id@p [ MAX_ENGINE_INSTANCE + 1 ] ;
...
position p;
@@
 const struct i915_subtest cocci_id@p [ ] ;
...
position p;
@@
 struct i915_request * cocci_id@p [ 2 ] ;
