...
position p;
@@
 const struct sched_attr * cocci_id@p ;
...
position p;
@@
 struct uclamp_bucket * cocci_id@p ;
...
position p;
@@
 struct cfs_schedulable_data * cocci_id@p ;
...
position p;
@@
 struct sched_rt_entity * * cocci_id@p ;
...
position p;
@@
 struct migration_arg * cocci_id@p ;
...
position p;
@@
 struct migration_arg {  struct task_struct * task ;  int dest_cpu ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct cgroup_subsys_state * css ) ;
...
position p;
@@
 enum { cpuset , possible , fail , } cocci_id@p ;
...
position p;
@@
 unsigned int cocci_id@p [ UCLAMP_CNT ] ;
...
position p;
@@
 const struct sched_param * cocci_id@p ;
...
position p;
@@
 struct migration_swap_arg cocci_id@p ;
...
position p;
@@
 struct uclamp_request cocci_id@p ;
...
position p;
@@
 struct uclamp_se cocci_id@p [ UCLAMP_CNT ] ;
...
typedef atomic_t;
@@
 struct tick_work {  int cpu ;  atomic_t state ;  struct delayed_work work ; } cocci_id@p ;
...
typedef u64;
@@
 int cocci_id@p ( struct task_group * tg , u64 period , u64 runtime ) ;
