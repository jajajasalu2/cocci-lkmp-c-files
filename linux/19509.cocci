...
position p;
@@
 struct xlp_msi_data * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef uint64_t,spinlock_t,uint32_t;
@@
 struct xlp_msi_data {  struct nlm_soc_info * node ;  uint64_t lnkbase ;  uint32_t msi_enabled_mask ;  uint32_t msi_alloc_mask ;  uint32_t msix_alloc_mask ;  spinlock_t msi_lock ; } cocci_id@p ;
...
position p;
@@
 struct nlm_soc_info * cocci_id@p ;
