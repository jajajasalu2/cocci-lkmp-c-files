...
position p;
@@
 const struct sysinfo * cocci_id@p ;
...
typedef uint64_t;
@@
 struct ttm_mem_zone {  struct kobject kobj ;  struct ttm_mem_global * glob ;  const char * name ;  uint64_t zone_mem ;  uint64_t emer_mem ;  uint64_t max_mem ;  uint64_t swap_limit ;  uint64_t used_mem ; } cocci_id@p ;
...
position p;
@@
 struct ttm_mem_zone * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct ttm_mem_global * glob ) ;
