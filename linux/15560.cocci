...
@@
 struct gasket_coherent_page_entry {  dma_addr_t paddr ;  u64 user_virt ;  u64 kernel_virt ;  u32 in_use ; } cocci_id@p ;
...
position p;
@@
 struct gasket_page_table * * cocci_id@p ;
...
typedef u64,uint;
@@
 struct gasket_page_table {  struct gasket_page_table_config config ;  uint num_simple_entries ;  uint num_extended_entries ;  struct gasket_page_table_entry * entries ;  uint num_active_pages ;  u64 __iomem * base_slot ;  u64 __iomem * extended_offset_reg ;  struct device * device ;  struct pci_dev * pci_dev ;  u64 extended_flag ;  struct mutex mutex ;  int num_coherent_pages ;  struct gasket_coherent_page_entry * coherent_pages ; } cocci_id@p ;
...
position p;
@@
 enum pte_status { PTE_FREE , PTE_INUSE , } cocci_id@p ;
...
typedef dma_addr_t;
@@
 struct gasket_page_table_entry {  enum pte_status status ;  int offset ;  dma_addr_t dma_addr ;  struct page * page ;  struct gasket_page_table_entry * sublevel ; } cocci_id@p ;
...
position p;
@@
 struct gasket_page_table_entry * cocci_id@p ;
...
position p;
@@
 const struct gasket_page_table_config * cocci_id@p ;
...
position p;
@@
 const struct gasket_bar_data * cocci_id@p ;
