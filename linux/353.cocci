...
position p;
@@
 struct bpf_lru_node cocci_id@p ;
...
position p;
@@
 const struct bpf_htab * cocci_id@p ;
...
@@
 struct bucket {  struct hlist_nulls_head head ;  raw_spinlock_t lock ; } cocci_id@p ;
...
@@
 struct bpf_htab {  struct bpf_map map ;  struct bucket * buckets ;  void * elems ;  union {  struct pcpu_freelist freelist ;  struct bpf_lru lru ; } ;  struct htab_elem * __percpu * extra_elems ;  atomic_t count ;  u32 n_buckets ;  u32 elem_size ;  u32 hashrnd ; } cocci_id@p ;
...
typedef bool;
@@
 bool cocci_id@p ( void * arg , struct bpf_lru_node * node ) ;
...
position p;
@@
 struct pcpu_freelist_node * cocci_id@p ;
