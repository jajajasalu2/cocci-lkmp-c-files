...
position p;
@@
 enum parse_mode { PARSE_MEMMAP , PARSE_EFI , } cocci_id@p ;
...
position p;
@@
 struct slot_area {  unsigned long addr ;  int num ; } cocci_id@p ;
...
position p;
@@
 enum mem_avoid_index { MEM_AVOID_ZO_RANGE = 0 , MEM_AVOID_INITRD , MEM_AVOID_CMDLINE , MEM_AVOID_BOOTPARAMS , MEM_AVOID_MEMMAP_BEGIN , MEM_AVOID_MEMMAP_END = MEM_AVOID_MEMMAP_BEGIN + MAX_MEMMAP_REGIONS - 1 , MEM_AVOID_MAX , } cocci_id@p ;
...
position p;
@@
 struct slot_area cocci_id@p [ MAX_SLOT_AREA ] ;
...
position p;
@@
 struct setup_indirect * cocci_id@p ;
...
position p;
@@
 struct mem_vector cocci_id@p [ MEM_AVOID_MAX ] ;
