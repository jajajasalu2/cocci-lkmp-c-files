...
typedef bool;
@@
 int cocci_id@p ( struct sdma_engine * sde , struct iowait_work * wait , struct sdma_txreq * stx , unsigned int seq , bool pkts_sent ) ;
...
position p;
@@
 const struct rvt_operation_params cocci_id@p [ RVT_OPERATION_MAX ] ;
...
position p;
@@
 void cocci_id@p ( struct iowait * wait ) ;
