...
position p;
@@
 enum dss_clk_source cocci_id@p ;
...
position p;
@@
 struct dispc_clock_info * cocci_id@p ;
...
position p;
@@
 const struct dispc_ops cocci_id@p ;
...
position p;
@@
 const enum dispc_feature_id cocci_id@p [ ] ;
...
position p;
@@
 enum dispc_feat_reg_field { FEAT_REG_FIRHINC , FEAT_REG_FIRVINC , FEAT_REG_FIFOHIGHTHRESHOLD , FEAT_REG_FIFOLOWTHRESHOLD , FEAT_REG_FIFOSIZE , FEAT_REG_HORIZONTALACCU , FEAT_REG_VERTICALACCU , } cocci_id@p ;
...
position p;
@@
 enum mgr_reg_fields { DISPC_MGR_FLD_ENABLE , DISPC_MGR_FLD_STNTFT , DISPC_MGR_FLD_GO , DISPC_MGR_FLD_TFTDATALINES , DISPC_MGR_FLD_STALLMODE , DISPC_MGR_FLD_TCKENABLE , DISPC_MGR_FLD_TCKSELECTION , DISPC_MGR_FLD_CPR , DISPC_MGR_FLD_FIFOHANDCHECK , DISPC_MGR_FLD_NUM , } cocci_id@p ;
...
position p;
@@
 const struct dispc_reg_field * cocci_id@p ;
...
typedef dma_addr_t;
@@
 struct i734_buf {  size_t size ;  dma_addr_t paddr ;  void * vaddr ; } cocci_id@p ;
...
position p;
@@
 enum omap_color_component cocci_id@p ;
...
position p;
@@
 enum omap_dss_load_mode cocci_id@p ;
...
position p;
@@
 enum dispc_feature_id { FEAT_LCDENABLEPOL , FEAT_LCDENABLESIGNAL , FEAT_PCKFREEENABLE , FEAT_FUNCGATED , FEAT_MGR_LCD2 , FEAT_MGR_LCD3 , FEAT_LINEBUFFERSPLIT , FEAT_ROWREPEATENABLE , FEAT_RESIZECONF , FEAT_CORE_CLK_DIV , FEAT_HANDLE_UV_SEPARATE , FEAT_ATTR2 , FEAT_CPR , FEAT_PRELOAD , FEAT_FIR_COEF_V , FEAT_ALPHA_FIXED_ZORDER , FEAT_ALPHA_FREE_ZORDER , FEAT_FIFO_MERGE , FEAT_OMAP3_DSI_FIFO_BUG , FEAT_BURST_2D , FEAT_MFLAG , } cocci_id@p ;
...
position p;
@@
 unsigned long cocci_id@p ( struct dispc_device * dispc , enum omap_channel channel ) ;
...
position p;
@@
 const struct csc_coef_yuv2rgb * cocci_id@p ;
...
position p;
@@
 const struct csc_coef_rgb2yuv cocci_id@p ;
...
position p;
@@
 const struct omap_dss_cpr_coefs * cocci_id@p ;
...
position p;
@@
 const struct dss_reg_field cocci_id@p [ ] ;
...
position p;
@@
 enum omap_burst_size { BURST_SIZE_X2 = 0 , BURST_SIZE_X4 = 1 , BURST_SIZE_X8 = 2 , } cocci_id@p ;
...
position p;
@@
 enum omap_overlay_caps cocci_id@p ;
...
typedef u32;
@@
 const struct {  const char * name ;  u32 vsync_irq ;  u32 framedone_irq ;  u32 sync_lost_irq ;  struct dispc_gamma_desc gamma ;  struct dispc_reg_field reg_desc [ DISPC_MGR_FLD_NUM ] ; } cocci_id@p [ ] ;
...
position p;
@@
 const struct dispc_coef * cocci_id@p ;
...
position p;
@@
 enum dispc_feat_reg_field cocci_id@p ;
...
position p;
@@
 enum omap_dss_trans_key_type cocci_id@p ;
...
position p;
@@
 const struct dispc_errata_i734_data {  struct videomode vm ;  struct omap_overlay_info ovli ;  struct omap_overlay_manager_info mgri ;  struct dss_lcd_mgr_config lcd_conf ; } cocci_id@p ;
...
position p;
@@
 const struct omap_overlay_info * cocci_id@p ;
...
typedef u32;
@@
 void cocci_id@p ( struct dispc_device * dispc , u32 mask ) ;
...
position p;
@@
 const struct accu cocci_id@p [ 4 ] ;
...
position p;
@@
 const struct dispc_clock_info * cocci_id@p ;
...
position p;
@@
 const struct soc_device_attribute cocci_id@p [ ] ;
...
position p;
@@
 const struct dispc_features cocci_id@p ;
...
position p;
@@
 enum dss_writeback_channel cocci_id@p ;
...
position p;
@@
 const struct omap_dss_writeback_info * cocci_id@p ;
...
position p;
@@
 enum mgr_reg_fields cocci_id@p ;
...
position p;
@@
 const struct drm_color_lut cocci_id@p [ ] ;
...
position p;
@@
 const struct videomode * cocci_id@p ;
...
position p;
@@
 const struct omap_overlay_manager_info * cocci_id@p ;
...
position p;
@@
 const enum omap_overlay_caps cocci_id@p [ ] ;
...
position p;
@@
 struct dss_lcd_mgr_config cocci_id@p ;
...
@@
 struct dispc_gamma_desc {  u32 len ;  u32 bits ;  u16 reg ;  bool has_index ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,u32,irq_handler_t;
@@
 struct dispc_device {  struct platform_device * pdev ;  void __iomem * base ;  struct dss_device * dss ;  struct dss_debugfs_entry * debugfs ;  int irq ;  irq_handler_t user_handler ;  void * user_data ;  unsigned long core_clk_rate ;  unsigned long tv_pclk_rate ;  u32 fifo_size [ DISPC_MAX_NR_FIFOS ] ;  int fifo_assignment [ DISPC_MAX_NR_FIFOS ] ;  bool ctx_valid ;  u32 ctx [ DISPC_SZ_REGS / sizeof ( u32 ) ] ;  u32 * gamma_table [ DISPC_MAX_CHANNEL_GAMMA ] ;  const struct dispc_features * feat ;  bool is_enabled ;  struct regmap * syscon_pol ;  u32 syscon_pol_offset ; } cocci_id@p ;
...
position p;
@@
 enum dss_io_pad_mode cocci_id@p ;
...
position p;
@@
 struct omap_overlay_info cocci_id@p ;
...
position p;
@@
 unsigned long cocci_id@p ( struct dispc_device * dispc , enum omap_plane_id plane ) ;
...
typedef u8,u16;
@@
 struct dispc_reg_field {  u16 reg ;  u8 high ;  u8 low ; } cocci_id@p ;
...
position p;
@@
 dispc_div_calc_func cocci_id@p ;
...
position p;
@@
 const enum omap_plane_id cocci_id@p ;
...
position p;
@@
 struct dispc_device cocci_id@p ;
...
position p;
@@
 enum omap_dss_rotation_type cocci_id@p ;
...
position p;
@@
 enum omap_burst_size cocci_id@p ;
...
position p;
@@
 unsigned long cocci_id@p ( struct dispc_device * dispc ) ;
...
position p;
@@
 enum omap_color_component { DISPC_COLOR_COMPONENT_RGB_Y = 1 << 0 , DISPC_COLOR_COMPONENT_UV = 1 << 1 , } cocci_id@p ;
...
position p;
@@
 enum dispc_feature_id cocci_id@p ;
...
position p;
@@
 const struct dispc_gamma_desc * cocci_id@p ;
...
position p;
@@
 const struct dss_lcd_mgr_config * cocci_id@p ;
