...
typedef u32;
@@
 struct cls_cgroup_head {  u32 handle ;  struct tcf_exts exts ;  struct tcf_ematch_tree ematches ;  struct tcf_proto * tp ;  struct rcu_work rwork ; } cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_CGROUP_MAX + 1 ] ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_CGROUP_MAX + 1 ] ;
...
position p;
@@
 struct cls_cgroup_head * cocci_id@p ;
