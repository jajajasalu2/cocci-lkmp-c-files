...
position p;
@@
 struct other_inode {  unsigned long orig_ino ;  struct ext4_inode * raw_inode ; } cocci_id@p ;
...
position p;
@@
 struct ext4_map_blocks cocci_id@p ;
...
symbol cocci_id;
position p;
typedef ext4_io_end_t;
@@
 ext4_io_end_t * cocci_id@p ;
...
position p;
@@
 struct mpage_da_data cocci_id@p ;
...
position p;
@@
 struct ext4_xattr_ibody_header * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct page * page , unsigned int len ) ;
...
position p;
@@
 int cocci_id@p ( struct inode * inode , int lblocks , int pextents ) ;
...
typedef pgoff_t;
@@
 struct mpage_da_data {  struct inode * inode ;  struct writeback_control * wbc ;  pgoff_t first_page ;  pgoff_t next_page ;  pgoff_t last_page ;  struct ext4_map_blocks map ;  struct ext4_io_submit io_submit ;  unsigned int do_map : 1 ; } cocci_id@p ;
...
position p;
@@
 const struct iomap_ops cocci_id@p ;
...
position p;
@@
 struct extent_status cocci_id@p ;
...
position p;
@@
 struct ext4_io_submit cocci_id@p ;
...
position p;
@@
 struct ext4_io_end_vec * cocci_id@p ;
...
position p;
@@
 struct ext4_xattr_inode_array * cocci_id@p ;
