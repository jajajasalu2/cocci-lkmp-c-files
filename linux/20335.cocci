...
symbol cocci_id;
position p;
typedef u64,u32,arch_spinlock_t;
@@
 union hpet_lock {  struct {  arch_spinlock_t lock ;  u32 value ; } ;  u64 lockval ; } cocci_id@p ;
...
position p;
@@
 struct hpet_channel cocci_id@p ;
...
position p;
@@
 struct hpet_channel {  struct clock_event_device evt ;  unsigned int num ;  unsigned int cpu ;  unsigned int irq ;  unsigned int in_use ;  enum hpet_mode mode ;  unsigned int boot_cfg ;  char name [ 10 ] ; } cocci_id@p ;
...
position p;
@@
 enum hpet_mode { HPET_MODE_UNUSED , HPET_MODE_LEGACY , HPET_MODE_CLOCKEVT , HPET_MODE_DEVICE , } cocci_id@p ;
...
position p;
@@
 struct hpet_base {  unsigned int nr_channels ;  unsigned int nr_clockevents ;  unsigned int boot_cfg ;  struct hpet_channel * channels ; } cocci_id@p ;
