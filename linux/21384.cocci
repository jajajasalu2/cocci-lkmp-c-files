...
symbol cocci_id;
position p;
typedef u64,uint32_t,csky_pmu_t;
@@
 struct csky_pmu_t {  struct pmu pmu ;  struct pmu_hw_events __percpu * hw_events ;  struct platform_device * plat_device ;  uint32_t count_width ;  uint32_t hpcr ;  u64 max_period ; } cocci_id@p ;
...
position p;
@@
 struct pmu_hw_events {  struct perf_event * events [ CSKY_PMU_MAX_EVENTS ] ;  unsigned long used_mask [ BITS_TO_LONGS ( CSKY_PMU_MAX_EVENTS ) ] ; } cocci_id@p ;
...
position p;
@@
 const int cocci_id@p [ C ( MAX ) ] [ C ( OP_MAX ) ] [ C ( RESULT_MAX ) ] ;
...
position p;
@@
 const int cocci_id@p [ PERF_COUNT_HW_MAX ] ;
...
typedef uint64_t;
@@
 void ( * cocci_id@p [ CSKY_PMU_MAX_EVENTS ] ) ( uint64_t val ) ;
