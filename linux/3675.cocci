...
position p;
@@
 const struct efivar_operations cocci_id@p ;
...
@@
 struct gsmi_buf {  u8 * start ;  size_t length ;  dma_addr_t handle ;  u32 address ; } cocci_id@p ;
...
position p;
@@
 struct gsmi_log_entry_type_1 cocci_id@p ;
...
typedef u32;
@@
 struct {  u32 percentage ;  u32 data_type ; } cocci_id@p ;
...
position p;
@@
 struct gsmi_get_next_var_param cocci_id@p ;
...
position p;
@@
 struct gsmi_nvram_var_param cocci_id@p ;
...
position p;
@@
 struct gsmi_set_eventlog_param cocci_id@p ;
...
@@
 struct gsmi_device {  struct platform_device * pdev ;  struct gsmi_buf * name_buf ;  struct gsmi_buf * data_buf ;  struct gsmi_buf * param_buf ;  spinlock_t lock ;  u16 smi_cmd ;  int handshake_type ;  struct dma_pool * dma_pool ; } cocci_id@p ;
