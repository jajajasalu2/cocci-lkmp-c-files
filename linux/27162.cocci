...
position p;
@@
 const struct fscrypt_mode * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,refcount_t;
@@
 struct fscrypt_direct_key {  struct hlist_node dk_node ;  refcount_t dk_refcount ;  const struct fscrypt_mode * dk_mode ;  struct crypto_skcipher * dk_ctfm ;  u8 dk_descriptor [ FSCRYPT_KEY_DESCRIPTOR_SIZE ] ;  u8 dk_raw [ FSCRYPT_MAX_KEY_SIZE ] ; } cocci_id@p ;
...
typedef u8;
@@
 const u8 cocci_id@p [ FSCRYPT_KEY_DESCRIPTOR_SIZE ] ;
...
position p;
@@
 const struct fscrypt_key * cocci_id@p ;
...
typedef u8;
@@
 const u8 cocci_id@p [ FS_KEY_DERIVATION_NONCE_SIZE ] ;
...
position p;
@@
 struct fscrypt_direct_key * cocci_id@p ;
...
position p;
@@
 const struct fscrypt_info * cocci_id@p ;
