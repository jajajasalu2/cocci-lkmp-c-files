...
position p;
@@
 const struct unw_table_entry * cocci_id@p ;
...
position p;
@@
 struct unw_state_record * cocci_id@p ;
...
position p;
@@
 const struct unw_frame_info * cocci_id@p ;
...
position p;
@@
 struct unw_table_entry * cocci_id@p ;
...
position p;
@@
 struct unw_reg_info * * cocci_id@p ;
...
position p;
@@
 struct unw_labeled_state * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef unw_hash_index_t;
@@
 unw_hash_index_t cocci_id@p ;
...
position p;
@@
 const unsigned char cocci_id@p [ 3 ] ;
...
typedef spinlock_t;
@@
 struct {  spinlock_t lock ;  struct unw_table * tables ;  unsigned long r0 ;  const unsigned char save_order [ 8 ] ;  unsigned short sw_off [ sizeof ( struct unw_frame_info ) / 8 ] ;  unsigned short lru_head ;  unsigned short lru_tail ;  unsigned short preg_index [ UNW_NUM_REGS ] ;  short pt_regs_offsets [ 32 ] ;  struct unw_table kernel_table ;  size_t gate_table_size ;  unsigned long * gate_table ;  unsigned short hash [ UNW_HASH_SIZE ] ;  struct unw_script cache [ UNW_CACHE_SIZE ] ; # ifdef UNW_DEBUG  const char * preg_name [ UNW_NUM_REGS ] ; # endif # if UNW_STATS  struct {  struct {  int lookups ;  int hinted_hits ;  int normal_hits ;  int collision_chain_traversals ; } cache ;  struct {  unsigned long build_time ;  unsigned long run_time ;  unsigned long parse_time ;  int builds ;  int news ;  int collisions ;  int runs ; } script ;  struct {  unsigned long init_time ;  unsigned long unwind_time ;  int inits ;  int unwinds ; } api ; } stat ; # endif } cocci_id@p ;
...
position p;
@@
 struct unw_reg_info * cocci_id@p [ 3 ] ;
...
position p;
@@
 enum unw_insn_opcode cocci_id@p ;
...
position p;
@@
 struct unw_reg_state * cocci_id@p ;
