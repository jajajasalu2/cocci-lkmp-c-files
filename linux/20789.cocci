...
typedef u8;
@@
 struct x64_jit_data {  struct bpf_binary_header * header ;  int * addrs ;  u8 * image ;  int proglen ;  struct jit_context ctx ; } cocci_id@p ;
...
position p;
@@
 struct btf_func_model * cocci_id@p ;
...
position p;
@@
 struct exception_table_entry cocci_id@p ;
...
typedef u8;
@@
 u8 cocci_id@p [ BPF_MAX_INSN_SIZE + BPF_INSN_SAFETY ] ;
...
position p;
@@
 struct jit_context {  int cleanup_addr ; } cocci_id@p ;
...
position p;
@@
 struct x64_jit_data * cocci_id@p ;
