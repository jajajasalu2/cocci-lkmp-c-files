...
position p;
@@
 struct alua_queue_data * cocci_id@p ;
...
typedef spinlock_t;
@@
 struct alua_dh_data {  struct list_head node ;  struct alua_port_group __rcu * pg ;  int group_id ;  spinlock_t pg_lock ;  struct scsi_device * sdev ;  int init_error ;  struct mutex init_mutex ; } cocci_id@p ;
...
position p;
@@
 struct alua_queue_data {  struct list_head entry ;  activate_complete callback_fn ;  void * callback_data ; } cocci_id@p ;
...
position p;
@@
 struct alua_dh_data * cocci_id@p ;
...
position p;
@@
 struct alua_port_group cocci_id@p ;
...
typedef bool;
@@
 bool cocci_id@p ( struct alua_port_group * pg , struct scsi_device * sdev , struct alua_queue_data * qdata , bool force ) ;
...
typedef bool;
@@
 void cocci_id@p ( struct scsi_device * sdev , bool force ) ;
...
typedef u8;
@@
 u8 cocci_id@p [ MAX_COMMAND_SIZE ] ;
