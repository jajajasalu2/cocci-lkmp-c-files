...
position p;
@@
 enum br_ctx_signal_state cocci_id@p ;
...
position p;
@@
 struct nfp_insn_meta * cocci_id@p ;
...
position p;
@@
 struct nfp_insn_ur_regs cocci_id@p ;
...
position p;
@@
 struct nfp_bpf_vnic * cocci_id@p ;
...
typedef u8,s32,bool;
@@
 int ( * cocci_id@p ) ( struct nfp_prog * nfp_prog , u8 gpr , u8 gpr_byte , s32 off , unsigned int size , bool first , bool new_gpr , bool last , bool lm3 , bool needs_inc ) ;
...
typedef instr_cb_t;
@@
 const instr_cb_t cocci_id@p [ 256 ] ;
...
typedef bool;
@@
 const struct jmp_code_map {  enum br_mask br_mask ;  bool swap ; } cocci_id@p [ ] ;
...
position p;
@@
 const struct jmp_code_map * cocci_id@p ;
...
position p;
@@
 struct nfp_bpf_cap_adjust_head * cocci_id@p ;
...
position p;
@@
 struct nfp_insn_re_regs cocci_id@p ;
...
position p;
@@
 struct reciprocal_value_adv cocci_id@p ;
