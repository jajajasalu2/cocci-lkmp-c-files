...
position p;
@@
 struct mxs_lradc_adc * cocci_id@p ;
...
position p;
@@
 struct mxs_lradc_scale {  unsigned int integer ;  unsigned int nano ; } cocci_id@p ;
...
typedef u32;
@@
 const u32 cocci_id@p [ ] [ LRADC_MAX_TOTAL_CHANS ] ;
...
position p;
@@
 enum mxs_lradc_divbytwo { MXS_LRADC_DIV_DISABLED = 0 , MXS_LRADC_DIV_ENABLED , } cocci_id@p ;
...
position p;
@@
 struct mxs_lradc_scale * cocci_id@p ;
...
@@
 struct mxs_lradc_adc {  struct mxs_lradc * lradc ;  struct device * dev ;  void __iomem * base ;  u32 buffer [ 10 ] ;  struct iio_trigger * trig ;  struct completion completion ;  spinlock_t lock ;  const u32 * vref_mv ;  struct mxs_lradc_scale scale_avail [ LRADC_MAX_TOTAL_CHANS ] [ 2 ] ;  unsigned long is_divided ; } cocci_id@p ;
