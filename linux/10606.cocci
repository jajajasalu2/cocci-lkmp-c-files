...
typedef u32;
@@
 struct hang {  struct intel_gt * gt ;  struct drm_i915_gem_object * hws ;  struct drm_i915_gem_object * obj ;  struct i915_gem_context * ctx ;  u32 * seqno ;  u32 * batch ; } cocci_id@p ;
...
position p;
@@
 struct evict_vma {  struct completion completion ;  struct i915_vma * vma ; } cocci_id@p ;
...
position p;
@@
 struct i915_gpu_error * cocci_id@p ;
...
position p;
@@
 struct active_engine * cocci_id@p ;
...
position p;
@@
 const typeof ( * igt_atomic_phases ) * cocci_id@p ;
...
position p;
@@
 struct i915_gem_context * cocci_id@p [ ARRAY_SIZE ( rq ) ] ;
...
position p;
@@
 const struct {  const char * name ;  unsigned int flags ; } cocci_id@p [ ] ;
...
position p;
@@
 const struct igt_atomic_section * cocci_id@p ;
...
position p;
@@
 struct active_engine {  struct task_struct * task ;  struct intel_engine_cs * engine ;  unsigned long resets ;  unsigned int flags ; } cocci_id@p ;
...
position p;
@@
 struct tasklet_struct * const cocci_id@p ;
...
position p;
@@
 struct active_engine cocci_id@p [ I915_NUM_ENGINES ] ;
...
position p;
@@
 const struct i915_vma * cocci_id@p ;
...
position p;
@@
 struct i915_request * cocci_id@p [ 8 ] ;
