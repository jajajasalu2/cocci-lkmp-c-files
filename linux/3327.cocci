...
position p;
@@
 enum chips { max1110 , max1111 , max1112 , max1113 , } cocci_id@p ;
...
typedef uint8_t;
@@
 struct max1111_data {  struct spi_device * spi ;  struct device * hwmon_dev ;  struct spi_message msg ;  struct spi_transfer xfer [ 2 ] ;  uint8_t tx_buf [ MAX1111_TX_BUF_SIZE ] ;  uint8_t rx_buf [ MAX1111_RX_BUF_SIZE ] ;  struct mutex drvdata_lock ;  int sel_sh ;  int lsb ; } cocci_id@p ;
...
position p;
@@
 struct max1111_data * cocci_id@p ;
