...
position p;
@@
 char cocci_id@p [ ACPI_OEM_TABLE_ID_SIZE ] ;
...
position p;
@@
 char cocci_id@p [ ACPI_OEM_ID_SIZE ] ;
...
symbol cocci_id;
position p;
typedef u8,u16,phys_addr_t;
@@
 struct mcfg_entry {  struct list_head list ;  phys_addr_t addr ;  u16 segment ;  u8 bus_start ;  u8 bus_end ; } cocci_id@p ;
...
typedef u32,u16;
@@
 struct mcfg_fixup {  char oem_id [ ACPI_OEM_ID_SIZE + 1 ] ;  char oem_table_id [ ACPI_OEM_TABLE_ID_SIZE + 1 ] ;  u32 oem_revision ;  u16 segment ;  struct resource bus_range ;  struct pci_ecam_ops * ops ;  struct resource cfgres ; } cocci_id@p ;
