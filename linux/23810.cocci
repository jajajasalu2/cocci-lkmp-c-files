...
typedef u64,u32;
@@
 struct fq_flow {  struct rb_root t_root ;  struct sk_buff * head ;  union {  struct sk_buff * tail ;  unsigned long age ; } ;  struct rb_node fq_node ;  struct sock * sk ;  int qlen ;  int credit ;  u32 socket_hash ;  struct fq_flow * next ;  struct rb_node rate_node ;  u64 time_next_packet ; } cocci_id@p ;
...
position p;
@@
 struct fq_flow_head * cocci_id@p ;
...
position p;
@@
 struct fq_flow_head {  struct fq_flow * first ;  struct fq_flow * last ; } cocci_id@p ;
...
position p;
@@
 struct tc_fq_qd_stats cocci_id@p ;
...
typedef u8,u64,u32;
@@
 struct fq_sched_data {  struct fq_flow_head new_flows ;  struct fq_flow_head old_flows ;  struct rb_root delayed ;  u64 time_next_delayed_flow ;  unsigned long unthrottle_latency_ns ;  struct fq_flow internal ;  u32 quantum ;  u32 initial_quantum ;  u32 flow_refill_delay ;  u32 flow_plimit ;  unsigned long flow_max_rate ;  u64 ce_threshold ;  u32 orphan_mask ;  u32 low_rate_threshold ;  struct rb_root * fq_root ;  u8 rate_enable ;  u8 fq_trees_log ;  u32 flows ;  u32 inactive_flows ;  u32 throttled_flows ;  u64 stat_gc_flows ;  u64 stat_internal_packets ;  u64 stat_throttled ;  u64 stat_ce_mark ;  u64 stat_flows_plimit ;  u64 stat_pkts_too_long ;  u64 stat_allocation_errors ;  struct qdisc_watchdog watchdog ; } cocci_id@p ;
...
typedef u64;
@@
 struct fq_skb_cb {  u64 time_to_send ; } cocci_id@p ;
...
position p;
@@
 struct fq_sched_data cocci_id@p ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_FQ_MAX + 1 ] ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_FQ_MAX + 1 ] ;
...
position p;
@@
 const struct fq_flow * cocci_id@p ;
