...
position p;
@@
 struct brcmf_pcie_dhi_ringinfo cocci_id@p ;
...
@@
 struct brcmf_pcie_shared_info {  u32 tcm_base_address ;  u32 flags ;  struct brcmf_pcie_ringbuf * commonrings [ BRCMF_NROF_COMMON_MSGRINGS ] ;  struct brcmf_pcie_ringbuf * flowrings ;  u16 max_rxbufpost ;  u16 max_flowrings ;  u16 max_submissionrings ;  u16 max_completionrings ;  u32 rx_dataoffset ;  u32 htod_mb_data_addr ;  u32 dtoh_mb_data_addr ;  u32 ring_info_addr ;  struct brcmf_pcie_console console ;  void * scratch ;  dma_addr_t scratch_dmahandle ;  void * ringupd ;  dma_addr_t ringupd_dmahandle ;  u8 version ; } cocci_id@p ;
...
position p;
@@
 struct brcmf_pciedev_info * cocci_id@p ;
...
position p;
@@
 struct brcmf_pciedev * cocci_id@p ;
...
position p;
@@
 struct brcmf_pcie_ringbuf * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct device * dev , int ret , struct brcmf_fw_request * fwreq ) ;
...
position p;
@@
 enum brcmf_pcie_state { BRCMFMAC_PCIE_STATE_DOWN , BRCMFMAC_PCIE_STATE_UP , } cocci_id@p ;
...
typedef u32;
@@
 struct brcmf_pcie_core_info {  u32 base ;  u32 wrapbase ; } cocci_id@p ;
...
typedef u8,u32;
@@
 struct brcmf_pcie_console {  u32 base_addr ;  u32 buf_addr ;  u32 bufsize ;  u32 read_idx ;  u8 log_str [ 256 ] ;  u8 log_idx ; } cocci_id@p ;
...
position p;
@@
 struct brcmf_fw_request * cocci_id@p ( struct brcmf_pciedev_info * devinfo ) ;
...
typedef __le32;
@@
 struct brcmf_pcie_dhi_ringinfo {  __le32 ringmem ;  __le32 h2d_w_idx_ptr ;  __le32 h2d_r_idx_ptr ;  __le32 d2h_w_idx_ptr ;  __le32 d2h_r_idx_ptr ;  struct msgbuf_buf_addr h2d_w_idx_hostaddr ;  struct msgbuf_buf_addr h2d_r_idx_hostaddr ;  struct msgbuf_buf_addr d2h_w_idx_hostaddr ;  struct msgbuf_buf_addr d2h_r_idx_hostaddr ;  __le16 max_flowrings ;  __le16 max_submissionrings ;  __le16 max_completionrings ; } cocci_id@p ;
...
@@
 struct brcmf_pcie_ringbuf {  struct brcmf_commonring commonring ;  dma_addr_t dma_handle ;  u32 w_idx_addr ;  u32 r_idx_addr ;  struct brcmf_pciedev_info * devinfo ;  u8 id ; } cocci_id@p ;
...
typedef u32;
@@
 const u32 cocci_id@p [ BRCMF_NROF_COMMON_MSGRINGS ] ;
...
position p;
@@
 struct brcmf_pcie_shared_info * cocci_id@p ;
...
position p;
@@
 struct brcmf_pcie_console * cocci_id@p ;
