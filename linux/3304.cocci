...
position p;
@@
 struct via686a_data * cocci_id@p ;
...
typedef u8,u16;
@@
 struct via686a_data {  unsigned short addr ;  const char * name ;  struct device * hwmon_dev ;  struct mutex update_lock ;  char valid ;  unsigned long last_updated ;  u8 in [ 5 ] ;  u8 in_max [ 5 ] ;  u8 in_min [ 5 ] ;  u8 fan [ 2 ] ;  u8 fan_min [ 2 ] ;  u16 temp [ 3 ] ;  u8 temp_over [ 3 ] ;  u8 temp_hyst [ 3 ] ;  u8 fan_div [ 2 ] ;  u16 alarms ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct via686a_data * data ) ;
...
position p;
@@
 struct via686a_data * cocci_id@p ( struct device * dev ) ;
