...
position p;
@@
 struct gdrom_id {  char mid ;  char modid ;  char verid ;  char padA [ 13 ] ;  char mname [ 16 ] ;  char modname [ 16 ] ;  char firmver [ 16 ] ;  char padB [ 16 ] ; } cocci_id@p ;
...
position p;
@@
 const struct {  int sense_key ;  const char * const text ; } cocci_id@p [ ] ;
...
position p;
@@
 int cocci_id@p ( short * bufstring ) ;
...
position p;
@@
 int cocci_id@p ( struct cdrom_device_info * cd_info ) ;
...
position p;
@@
 int cocci_id@p ( struct cdrom_device_info * cd_info , struct packet_command * command ) ;
...
position p;
@@
 struct gdrom_unit {  struct gendisk * disk ;  struct cdrom_device_info * cd_info ;  int status ;  int pending ;  int transfer ;  char disk_type ;  struct gdromtoc * toc ;  struct request_queue * gdrom_rq ;  struct blk_mq_tag_set tag_set ; } cocci_id@p ;
