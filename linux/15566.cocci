...
position p;
@@
 struct scsipending {  struct uiscmdrsp cmdrsp ;  void * sent ;  char cmdtype ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u64,bool,wait_queue_head_t,atomic_t,spinlock_t;
@@
 struct visorhba_devdata {  struct Scsi_Host * scsihost ;  struct visor_device * dev ;  struct list_head dev_info_list ;  struct scsipending pending [ MAX_PENDING_REQUESTS ] ;  unsigned int nextinsert ;  spinlock_t privlock ;  bool serverdown ;  bool serverchangingstate ;  unsigned long long acquire_failed_cnt ;  unsigned long long interrupts_rcvd ;  unsigned long long interrupts_notme ;  unsigned long long interrupts_disabled ;  u64 __iomem * flags_addr ;  atomic_t interrupt_rcvd ;  wait_queue_head_t rsp_queue ;  struct visordisk_info head ;  unsigned int max_buff_len ;  int devnum ;  struct task_struct * thread ;  int thread_wait_ms ;  struct idr idr ;  struct dentry * debugfs_dir ;  struct dentry * debugfs_info ; } cocci_id@p ;
...
position p;
@@
 struct visordisk_info * cocci_id@p ;
...
position p;
@@
 struct vhba_config_max cocci_id@p ;
...
position p;
@@
 struct visorhba_devices_open {  struct visorhba_devdata * devdata ; } cocci_id@p ;
...
@@
 struct visordisk_info {  struct scsi_device * sdev ;  u32 valid ;  atomic_t ios_threshold ;  atomic_t error_count ;  struct visordisk_info * next ; } cocci_id@p ;
...
position p;
@@
 enum task_mgmt_types cocci_id@p ;
...
position p;
@@
 struct visorhba_devdata * cocci_id@p ;
