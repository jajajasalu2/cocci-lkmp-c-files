...
position p;
@@
 struct smp_cmd_security_req cocci_id@p ;
...
position p;
@@
 struct smp_cmd_sign_info cocci_id@p ;
...
typedef u8,bool;
@@
 struct smp_dev {  bool local_oob ;  u8 local_pk [ 64 ] ;  u8 local_rand [ 16 ] ;  bool debug_key ;  struct crypto_shash * tfm_cmac ;  struct crypto_kpp * tfm_ecdh ; } cocci_id@p ;
...
position p;
@@
 struct smp_cmd_pairing cocci_id@p ;
...
position p;
@@
 struct smp_cmd_ident_info cocci_id@p ;
...
position p;
@@
 enum { SMP_FLAG_TK_VALID , SMP_FLAG_CFM_PENDING , SMP_FLAG_MITM_AUTH , SMP_FLAG_COMPLETE , SMP_FLAG_INITIATOR , SMP_FLAG_SC , SMP_FLAG_REMOTE_PK , SMP_FLAG_DEBUG_KEY , SMP_FLAG_WAIT_USER , SMP_FLAG_DHKEY_PENDING , SMP_FLAG_REMOTE_OOB , SMP_FLAG_LOCAL_OOB , SMP_FLAG_CT2 , } cocci_id@p ;
...
position p;
@@
 struct smp_cmd_encrypt_info cocci_id@p ;
...
position p;
@@
 struct smp_cmd_master_ident cocci_id@p ;
...
position p;
@@
 struct smp_cmd_pairing_confirm cocci_id@p ;
...
position p;
@@
 struct smp_cmd_dhkey_check * cocci_id@p ;
...
position p;
@@
 struct smp_cmd_keypress_notify * cocci_id@p ;
...
position p;
@@
 struct smp_cmd_ident_addr_info * cocci_id@p ;
...
position p;
@@
 const struct l2cap_ops cocci_id@p ;
...
position p;
@@
 struct smp_cmd_public_key * cocci_id@p ;
...
typedef u8,bdaddr_t;
@@
 struct smp_chan {  struct l2cap_conn * conn ;  struct delayed_work security_timer ;  unsigned long allow_cmd ;  u8 preq [ 7 ] ;  u8 prsp [ 7 ] ;  u8 prnd [ 16 ] ;  u8 rrnd [ 16 ] ;  u8 pcnf [ 16 ] ;  u8 tk [ 16 ] ;  u8 rr [ 16 ] ;  u8 lr [ 16 ] ;  u8 enc_key_size ;  u8 remote_key_dist ;  bdaddr_t id_addr ;  u8 id_addr_type ;  u8 irk [ 16 ] ;  struct smp_csrk * csrk ;  struct smp_csrk * slave_csrk ;  struct smp_ltk * ltk ;  struct smp_ltk * slave_ltk ;  struct smp_irk * remote_irk ;  u8 * link_key ;  unsigned long flags ;  u8 method ;  u8 passkey_round ;  u8 local_pk [ 64 ] ;  u8 remote_pk [ 64 ] ;  u8 dhkey [ 32 ] ;  u8 mackey [ 16 ] ;  struct crypto_shash * tfm_cmac ;  struct crypto_kpp * tfm_ecdh ; } cocci_id@p ;
