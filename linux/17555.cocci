...
position p;
@@
 struct pmc_cntrl_data cocci_id@p [ NUM_THREADS ] [ NR_PHYS_CTRS ] ;
...
typedef u8,u16;
@@
 struct pm_signal {  u16 cpu ;  u16 sub_unit ;  short int signal_group ;  u8 bus_word ;  u8 bit ; } cocci_id@p ;
...
position p;
@@
 unsigned long cocci_id@p [ MAX_NUMNODES * NUM_SPUS_PER_NODE ] ;
...
position p;
@@
 struct op_counter_config * cocci_id@p ;
...
position p;
@@
 struct op_system_config * cocci_id@p ;
...
typedef u8,u16;
@@
 struct pm_cntrl {  u16 enable ;  u16 stop_at_max ;  u16 trace_mode ;  u16 freeze ;  u16 count_mode ;  u16 spu_addr_trace ;  u8 trace_buf_ovflw ; } cocci_id@p ;
...
position p;
@@
 struct pmc_cntrl_data {  unsigned long vcntr ;  unsigned long evnts ;  unsigned long masks ;  unsigned long enabled ; } cocci_id@p ;
...
position p;
@@
 struct op_powerpc_model cocci_id@p ;
...
position p;
@@
 struct pm_signal cocci_id@p [ NUM_SPUS_PER_NODE ] ;
...
position p;
@@
 unsigned char cocci_id@p [ NUM_INPUT_BUS_WORDS ] ;
...
typedef u32;
@@
 struct {  u32 group_control ;  u32 debug_bus_control ;  struct pm_cntrl pm_cntrl ;  u32 pm07_cntrl [ NR_PHYS_CTRS ] ; } cocci_id@p ;
...
position p;
@@
 struct pm_signal cocci_id@p [ NR_PHYS_CTRS ] ;
...
position p;
@@
 enum { SUBFUNC_RESET = 1 , SUBFUNC_ACTIVATE = 2 , SUBFUNC_DEACTIVATE = 3 , PASSTHRU_IGNORE = 0 , PASSTHRU_ENABLE = 1 , PASSTHRU_DISABLE = 2 , } cocci_id@p ;
