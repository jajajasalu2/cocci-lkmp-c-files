...
position p;
@@
 struct guillemot {  struct gameport * gameport ;  struct input_dev * dev ;  int bads ;  int reads ;  struct guillemot_type * type ;  unsigned char length ;  char phys [ 32 ] ; } cocci_id@p ;
...
position p;
@@
 struct guillemot_type cocci_id@p [ ] ;
...
typedef u8;
@@
 u8 cocci_id@p [ GUILLEMOT_MAX_LENGTH ] ;
...
position p;
@@
 struct {  int x ;  int y ; } cocci_id@p [ 16 ] ;
...
position p;
@@
 struct guillemot_type {  unsigned char id ;  short * abs ;  short * btn ;  int hat ;  char * name ; } cocci_id@p ;
