...
position p;
@@
 struct intel_bts_queue cocci_id@p ;
...
position p;
@@
 struct intel_bts_synth {  struct perf_tool dummy_tool ;  struct perf_session * session ; } cocci_id@p ;
...
typedef u64;
@@
 struct branch {  u64 from ;  u64 to ;  u64 misc ; } cocci_id@p ;
...
position p;
@@
 struct intel_bts_synth cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u64,bool,u32,pid_t;
@@
 struct intel_bts_queue {  struct intel_bts * bts ;  unsigned int queue_nr ;  struct auxtrace_buffer * buffer ;  bool on_heap ;  bool done ;  pid_t pid ;  pid_t tid ;  int cpu ;  u64 time ;  struct intel_pt_insn intel_pt_insn ;  u32 sample_flags ; } cocci_id@p ;
