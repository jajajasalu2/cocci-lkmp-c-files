...
position p;
@@
 struct be_cmd_resp_seeprom_read * cocci_id@p ;
...
position p;
@@
 struct be_ethtool_stat {  char desc [ ETH_GSTRING_LEN ] ;  int type ;  int size ;  int offset ; } cocci_id@p ;
...
position p;
@@
 enum { DRVSTAT_TX , DRVSTAT_RX , DRVSTAT , } cocci_id@p ;
...
position p;
@@
 const struct be_ethtool_stat cocci_id@p [ ] ;
...
typedef u8;
@@
 u8 cocci_id@p [ RSS_INDIR_TABLE_LEN ] ;
