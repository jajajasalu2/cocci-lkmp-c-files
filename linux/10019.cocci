...
position p;
@@
 void cocci_id@p ( struct request_context * ctx ) ;
...
position p;
@@
 struct hermes_response * cocci_id@p ;
...
position p;
@@
 struct request_context cocci_id@p ;
...
typedef u8;
@@
 u8 cocci_id@p [ MICHAEL_MIC_LEN + 1 ] ;
...
position p;
@@
 struct orinoco_private * cocci_id@p ;
...
position p;
@@
 enum ezusb_state { EZUSB_CTX_START , EZUSB_CTX_QUEUED , EZUSB_CTX_REQ_SUBMITTED , EZUSB_CTX_REQ_COMPLETE , EZUSB_CTX_RESP_RECEIVED , EZUSB_CTX_REQ_TIMEOUT , EZUSB_CTX_REQ_FAILED , EZUSB_CTX_RESP_TIMEOUT , EZUSB_CTX_REQSUBMIT_FAIL , EZUSB_CTX_COMPLETE , } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct ezusb_priv * upriv ) ;
...
position p;
@@
 const struct hermes_ops cocci_id@p ;
...
position p;
@@
 struct ezusb_packet * cocci_id@p ;
...
typedef u8,u16;
@@
 struct ez_usb_fw {  u16 size ;  const u8 * code ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u16,refcount_t;
@@
 struct request_context {  struct list_head list ;  refcount_t refcount ;  struct completion done ;  int killed ;  struct urb * outurb ;  struct ezusb_priv * upriv ;  struct ezusb_packet * buf ;  int buf_length ;  struct timer_list timer ;  enum ezusb_state state ;  u16 out_rid ;  u16 in_rid ; } cocci_id@p ;
...
@@
 struct ezusb_priv {  struct usb_device * udev ;  struct net_device * dev ;  struct mutex mtx ;  spinlock_t req_lock ;  struct list_head req_pending ;  struct list_head req_active ;  spinlock_t reply_count_lock ;  u16 hermes_reg_fake [ 0x40 ] ;  u8 * bap_buf ;  struct urb * read_urb ;  int read_pipe ;  int write_pipe ;  u8 reply_count ; } cocci_id@p ;
