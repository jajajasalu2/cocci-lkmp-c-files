...
position p;
@@
 struct ltq_etop_chan {  int idx ;  int tx_free ;  struct net_device * netdev ;  struct napi_struct napi ;  struct ltq_dma_channel dma ;  struct sk_buff * skb [ LTQ_DESC_NUM ] ; } cocci_id@p ;
...
typedef spinlock_t;
@@
 struct ltq_etop_priv {  struct net_device * netdev ;  struct platform_device * pdev ;  struct ltq_eth_data * pldata ;  struct resource * res ;  struct mii_bus * mii_bus ;  struct ltq_etop_chan ch [ MAX_DMA_CHAN ] ;  int tx_free [ MAX_DMA_CHAN >> 1 ] ;  spinlock_t lock ; } cocci_id@p ;
...
position p;
@@
 struct ltq_etop_priv * cocci_id@p ;
...
position p;
@@
 struct ltq_dma_desc * cocci_id@p ;
...
position p;
@@
 struct ltq_etop_chan cocci_id@p ;
