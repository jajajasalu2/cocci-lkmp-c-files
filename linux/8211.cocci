...
symbol cocci_id;
position p;
typedef bool,wait_queue_head_t;
@@
 struct rocker_wait {  wait_queue_head_t wait ;  bool done ;  bool nowait ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef rocker_cmd_prep_cb_t;
@@
 rocker_cmd_prep_cb_t cocci_id@p ;
...
position p;
@@
 const struct rocker_tlv * cocci_id@p ;
...
position p;
@@
 const struct rocker_dma_ring_info * cocci_id@p ;
...
position p;
@@
 const struct rocker_tlv * cocci_id@p [ ROCKER_TLV_EVENT_MAC_VLAN_MAX + 1 ] ;
...
position p;
@@
 int cocci_id@p ( struct rocker_port * rocker_port , const unsigned char * addr , __be16 vlan_id ) ;
...
position p;
@@
 const struct rocker_tlv * cocci_id@p [ ROCKER_TLV_EVENT_LINK_CHANGED_MAX + 1 ] ;
...
position p;
@@
 struct rocker_switchdev_event_work {  struct work_struct work ;  struct switchdev_notifier_fdb_info fdb_info ;  struct rocker_port * rocker_port ;  unsigned long event ; } cocci_id@p ;
...
position p;
@@
 struct rocker_walk_data * cocci_id@p ;
...
position p;
@@
 const struct rocker_tlv * cocci_id@p [ ROCKER_TLV_RX_MAX + 1 ] ;
...
position p;
@@
 const struct rocker_tlv * cocci_id@p [ ROCKER_TLV_TX_FRAG_ATTR_MAX + 1 ] ;
...
position p;
@@
 struct rocker_walk_data {  struct rocker * rocker ;  struct rocker_port * port ; } cocci_id@p ;
...
position p;
@@
 struct rocker_switchdev_event_work cocci_id@p ;
...
position p;
@@
 struct rocker_fib_event_work * cocci_id@p ;
...
position p;
@@
 const struct rocker_tlv * cocci_id@p [ ROCKER_TLV_CMD_PORT_SETTINGS_MAX + 1 ] ;
...
position p;
@@
 const struct rocker * cocci_id@p ;
...
position p;
@@
 struct rocker_dma_ring_info * cocci_id@p ;
...
position p;
@@
 const struct rocker_desc_info * cocci_id@p ;
...
position p;
@@
 struct rocker_port_stats {  char str [ ETH_GSTRING_LEN ] ;  int type ; } cocci_id@p [ ] ;
...
position p;
@@
 const struct rocker_tlv * cocci_id@p [ ROCKER_TLV_CMD_PORT_STATS_MAX + 1 ] ;
...
position p;
@@
 const struct rocker_tlv * cocci_id@p [ ROCKER_TLV_TX_MAX + 1 ] ;
...
position p;
@@
 const struct rocker_tlv * cocci_id@p [ ROCKER_TLV_CMD_MAX + 1 ] ;
...
position p;
@@
 struct port_name {  char * buf ;  size_t len ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef rocker_cmd_proc_cb_t;
@@
 rocker_cmd_proc_cb_t cocci_id@p ;
...
position p;
@@
 struct switchdev_notifier_port_attr_info * cocci_id@p ;
...
position p;
@@
 struct rocker_fib_event_work {  struct work_struct work ;  union {  struct fib_entry_notifier_info fen_info ;  struct fib_rule_notifier_info fr_info ; } ;  struct rocker * rocker ;  unsigned long event ; } cocci_id@p ;
...
position p;
@@
 const struct rocker_tlv * cocci_id@p [ ROCKER_TLV_EVENT_MAX + 1 ] ;
