...
symbol cocci_id;
position p;
typedef vmlogrdr_priv_t,spinlock_t,atomic_t;
@@
 struct vmlogrdr_priv_t {  char system_service [ 8 ] ;  char internal_name [ 8 ] ;  char recording_name [ 8 ] ;  struct iucv_path * path ;  int connection_established ;  int iucv_path_severed ;  struct iucv_message local_interrupt_buffer ;  atomic_t receive_ready ;  int minor_num ;  char * buffer ;  char * current_position ;  int remaining ;  ulong residual_length ;  int buffer_free ;  int dev_in_use ;  spinlock_t priv_lock ;  struct device * device ;  struct device * class_device ;  int autorecording ;  int autopurge ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef vmlogrdr_priv_t;
@@
 struct vmlogrdr_priv_t cocci_id@p [ ] ;
...
typedef u8;
@@
 void cocci_id@p ( struct iucv_path * , u8 * ipuser ) ;
