...
typedef __le32,u8;
@@
 struct __packed ems_cpc_msg {  u8 type ;  u8 length ;  u8 msgid ;  __le32 ts_sec ;  __le32 ts_nsec ;  union {  u8 generic [ 64 ] ;  struct cpc_can_msg can_msg ;  struct cpc_can_params can_params ;  struct cpc_confirm confirmation ;  struct cpc_overrun overrun ;  struct cpc_can_error error ;  struct cpc_can_err_counter err_counter ;  u8 can_state ; } msg ; } cocci_id@p ;
...
position p;
@@
 struct ems_tx_urb_context * cocci_id@p ;
...
typedef __le32,u8;
@@
 struct cpc_can_msg {  __le32 id ;  u8 length ;  u8 msg [ 8 ] ; } cocci_id@p ;
...
typedef u8;
@@
 struct cpc_sja1000_params {  u8 mode ;  u8 acc_code0 ;  u8 acc_code1 ;  u8 acc_code2 ;  u8 acc_code3 ;  u8 acc_mask0 ;  u8 acc_mask1 ;  u8 acc_mask2 ;  u8 acc_mask3 ;  u8 btr0 ;  u8 btr1 ;  u8 outp_contr ; } cocci_id@p ;
...
typedef u8;
@@
 struct cpc_can_err_counter {  u8 rx ;  u8 tx ; } cocci_id@p ;
...
position p;
@@
 struct cpc_can_params cocci_id@p ;
...
typedef u8;
@@
 struct cpc_can_params {  u8 cc_type ;  union {  struct cpc_sja1000_params sja1000 ; } cc_params ; } cocci_id@p ;
...
typedef u8;
@@
 struct cpc_confirm {  u8 error ; } cocci_id@p ;
...
typedef u8;
@@
 struct cpc_overrun {  u8 event ;  u8 count ; } cocci_id@p ;
...
typedef u8,u32;
@@
 struct ems_tx_urb_context {  struct ems_usb * dev ;  u32 echo_index ;  u8 dlc ; } cocci_id@p ;
...
typedef u8;
@@
 struct cpc_sja1000_can_error {  u8 ecc ;  u8 rxerr ;  u8 txerr ; } cocci_id@p ;
...
typedef u8;
@@
 struct cpc_can_error {  u8 ecode ;  struct {  u8 cc_type ;  union {  struct cpc_sja1000_can_error sja1000 ; } regs ; } cc ; } cocci_id@p ;
...
position p;
@@
 struct cpc_sja1000_params * cocci_id@p ;
...
typedef u8,atomic_t;
@@
 struct ems_usb {  struct can_priv can ;  struct sk_buff * echo_skb [ MAX_TX_URBS ] ;  struct usb_device * udev ;  struct net_device * netdev ;  atomic_t active_tx_urbs ;  struct usb_anchor tx_submitted ;  struct ems_tx_urb_context tx_contexts [ MAX_TX_URBS ] ;  struct usb_anchor rx_submitted ;  struct urb * intr_urb ;  u8 * tx_msg_buffer ;  u8 * intr_in_buffer ;  unsigned int free_slots ;  struct ems_cpc_msg active_params ; } cocci_id@p ;
