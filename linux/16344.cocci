...
typedef u64;
@@
 const struct sample_resolution_info {  const char * sample_res ;  int bytes ;  u64 formats ; } cocci_id@p [ ] ;
...
position p;
@@
 struct snd_pcm_runtime * const cocci_id@p ;
...
typedef bool;
@@
 struct sound_adapter {  struct list_head dev_list ;  struct most_interface * iface ;  struct snd_card * card ;  struct list_head list ;  bool registered ;  int pcm_dev_idx ; } cocci_id@p ;
...
position p;
@@
 struct channel * const cocci_id@p ;
...
position p;
@@
 struct sound_adapter * cocci_id@p ;
