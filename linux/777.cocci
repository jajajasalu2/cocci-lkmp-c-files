...
@@
 struct vmd_dev {  struct pci_dev * dev ;  spinlock_t cfg_lock ;  char __iomem * cfgbar ;  int msix_count ;  struct vmd_irq_list * irqs ;  struct pci_sysdata sysdata ;  struct resource resources [ 3 ] ;  struct irq_domain * irq_domain ;  struct pci_bus * bus ;  u8 busn_start ;  struct dma_map_ops dma_ops ;  struct dma_domain dma_domain ; } cocci_id@p ;
...
position p;
@@
 struct vmd_irq_list * cocci_id@p ;
...
position p;
@@
 enum vmd_features { VMD_FEAT_HAS_MEMBAR_SHADOW = ( 1 << 0 ) , VMD_FEAT_HAS_BUS_RESTRICTIONS = ( 1 << 1 ) , } cocci_id@p ;
...
position p;
@@
 struct vmd_irq_list {  struct list_head irq_list ;  struct srcu_struct srcu ;  unsigned int count ; } cocci_id@p ;
...
typedef bool;
@@
 struct vmd_irq {  struct list_head node ;  struct vmd_irq_list * irq ;  bool enabled ;  unsigned int virq ; } cocci_id@p ;
