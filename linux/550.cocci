...
position p;
@@
 struct crypto_report_comp cocci_id@p ;
...
position p;
@@
 const int cocci_id@p [ CRYPTO_NR_MSGTYPES ] ;
...
position p;
@@
 struct nlattr * cocci_id@p [ CRYPTOCFGA_MAX + 1 ] ;
...
position p;
@@
 const struct crypto_link {  int ( * doit ) ( struct sk_buff * , struct nlmsghdr * , struct nlattr * * ) ;  int ( * dump ) ( struct sk_buff * , struct netlink_callback * ) ;  int ( * done ) ( struct netlink_callback * ) ; } cocci_id@p [ CRYPTO_NR_MSGTYPES ] ;
...
position p;
@@
 struct crypto_dump_info * cocci_id@p ;
...
position p;
@@
 const struct crypto_link * cocci_id@p ;
...
position p;
@@
 struct crypto_user_alg * cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ CRYPTOCFGA_MAX + 1 ] ;
...
typedef u32,u16;
@@
 struct crypto_dump_info {  struct sk_buff * in_skb ;  struct sk_buff * out_skb ;  u32 nlmsg_seq ;  u16 nlmsg_flags ; } cocci_id@p ;
...
position p;
@@
 struct crypto_report_cipher cocci_id@p ;
...
position p;
@@
 struct crypto_report_larval cocci_id@p ;
