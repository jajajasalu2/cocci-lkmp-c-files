...
typedef u64,u32;
@@
 struct cpu_port {  u64 mpidr ;  u32 port ; } cocci_id@p ;
...
position p;
@@
 struct cci_nb_ports {  unsigned int nb_ace ;  unsigned int nb_ace_lite ; } cocci_id@p ;
...
position p;
@@
 struct cci_ace_port * cocci_id@p ;
...
position p;
@@
 struct cpu_port cocci_id@p [ NR_CPUS ] ;
...
position p;
@@
 const struct of_dev_auxdata cocci_id@p [ ] ;
...
position p;
@@
 const struct cci_nb_ports * cocci_id@p ;
...
position p;
@@
 enum cci_ace_port_type { ACE_INVALID_PORT = 0x0 , ACE_PORT , ACE_LITE_PORT , } cocci_id@p ;
...
position p;
@@
 struct cci_ace_port {  void __iomem * base ;  unsigned long phys ;  enum cci_ace_port_type type ;  struct device_node * dn ; } cocci_id@p ;
