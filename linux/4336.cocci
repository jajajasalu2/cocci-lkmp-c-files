...
position p;
@@
 enum { MLXBF_TMFIFO_VRING_RX , MLXBF_TMFIFO_VRING_TX , MLXBF_TMFIFO_VRING_MAX , } cocci_id@p ;
...
position p;
@@
 struct mlxbf_tmfifo_irq_info * cocci_id@p ;
...
position p;
@@
 struct mlxbf_tmfifo cocci_id@p ;
...
@@
 struct mlxbf_tmfifo_vring {  void * va ;  dma_addr_t dma ;  struct virtqueue * vq ;  struct vring_desc * desc ;  struct vring_desc * desc_head ;  int cur_len ;  int rem_len ;  u32 pkt_len ;  u16 next_avail ;  int num ;  int align ;  int index ;  int vdev_id ;  struct mlxbf_tmfifo * fifo ; } cocci_id@p ;
...
position p;
@@
 struct mlxbf_tmfifo_vdev * cocci_id@p ;
...
@@
 struct mlxbf_tmfifo {  struct mlxbf_tmfifo_vdev * vdev [ MLXBF_TMFIFO_VDEV_MAX ] ;  struct mutex lock ;  void __iomem * rx_base ;  void __iomem * tx_base ;  int rx_fifo_size ;  int tx_fifo_size ;  unsigned long pend_events ;  struct mlxbf_tmfifo_irq_info irq_info [ MLXBF_TM_MAX_IRQ ] ;  struct work_struct work ;  struct timer_list timer ;  struct mlxbf_tmfifo_vring * vring [ 2 ] ;  spinlock_t spin_lock ;  bool is_ready ; } cocci_id@p ;
...
position p;
@@
 struct mlxbf_tmfifo_irq_info {  struct mlxbf_tmfifo * fifo ;  int irq ;  int index ; } cocci_id@p ;
...
position p;
@@
 struct mlxbf_tmfifo_msg_hdr cocci_id@p ;
...
typedef u64,u8;
@@
 struct mlxbf_tmfifo_vdev {  struct virtio_device vdev ;  u8 status ;  u64 features ;  struct mlxbf_tmfifo_vring vrings [ MLXBF_TMFIFO_VRING_MAX ] ;  union {  struct virtio_console_config cons ;  struct virtio_net_config net ; } config ;  struct circ_buf tx_buf ; } cocci_id@p ;
...
position p;
@@
 struct mlxbf_tmfifo_vring * cocci_id@p ;
...
position p;
@@
 enum { MLXBF_TM_RX_LWM_IRQ , MLXBF_TM_RX_HWM_IRQ , MLXBF_TM_TX_LWM_IRQ , MLXBF_TM_TX_HWM_IRQ , MLXBF_TM_MAX_IRQ , } cocci_id@p ;
