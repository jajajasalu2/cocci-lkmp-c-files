...
@@
 struct tiger_hw {  struct list_head list ;  struct pci_dev * pdev ;  char name [ MISDN_MAX_IDLEN ] ;  enum nj_types typ ;  int irq ;  u32 irqcnt ;  u32 base ;  size_t base_s ;  dma_addr_t dma ;  void * dma_p ;  spinlock_t lock ;  struct isac_hw isac ;  struct tiger_dma send ;  struct tiger_dma recv ;  struct tiger_ch bc [ 2 ] ;  u8 ctrlreg ;  u8 dmactrl ;  u8 auxd ;  u8 last_is0 ;  u8 irqmask0 ;  char log [ LOG_SIZE ] ; } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct tiger_ch * ) ;
...
position p;
@@
 enum nj_types { NETJET_S_TJ300 , NETJET_S_TJ320 , ENTERNOW__TJ320 , } cocci_id@p ;
...
typedef u8,u16;
@@
 struct tiger_ch {  struct bchannel bch ;  struct tiger_hw * nj ;  int idx ;  int free ;  int lastrx ;  u16 rxstate ;  u16 txstate ;  struct isdnhdlc_vars hsend ;  struct isdnhdlc_vars hrecv ;  u8 * hsbuf ;  u8 * hrbuf ; } cocci_id@p ;
...
typedef u32;
@@
 struct tiger_dma {  size_t size ;  u32 * start ;  int idx ;  u32 dmastart ;  u32 dmairq ;  u32 dmaend ;  u32 dmacur ; } cocci_id@p ;
