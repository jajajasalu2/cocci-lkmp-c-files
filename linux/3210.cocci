...
position p;
@@
 enum WATCHDOG_RESOLUTION { seconds = 1 , minutes = 60 , } cocci_id@p ;
...
typedef u8,bool;
@@
 struct fts_data {  struct i2c_client * client ;  struct mutex update_lock ;  struct mutex access_lock ;  unsigned long last_updated ;  struct watchdog_device wdd ;  enum WATCHDOG_RESOLUTION resolution ;  bool valid ;  u8 volt [ FTS_NO_VOLT_SENSORS ] ;  u8 temp_input [ FTS_NO_TEMP_SENSORS ] ;  u8 temp_alarm ;  u8 fan_present ;  u8 fan_input [ FTS_NO_FAN_SENSORS ] ;  u8 fan_source [ FTS_NO_FAN_SENSORS ] ;  u8 fan_alarm ; } cocci_id@p ;
...
position p;
@@
 enum WATCHDOG_RESOLUTION cocci_id@p ;
