...
typedef uint32_t;
@@
 struct lpc32xx_nand_cfg_mlc {  uint32_t tcea_delay ;  uint32_t busy_delay ;  uint32_t nand_ta ;  uint32_t rd_high ;  uint32_t rd_low ;  uint32_t wr_high ;  uint32_t wr_low ;  int wp_gpio ;  struct mtd_partition * parts ;  unsigned num_parts ; } cocci_id@p ;
...
position p;
@@
 struct lpc32xx_nand_cfg_mlc * cocci_id@p ;
...
position p;
@@
 struct lpc32xx_nand_host * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef uint8_t,uint32_t,dma_addr_t;
@@
 struct lpc32xx_nand_host {  struct platform_device * pdev ;  struct nand_chip nand_chip ;  struct lpc32xx_mlc_platform_data * pdata ;  struct clk * clk ;  void __iomem * io_base ;  int irq ;  struct lpc32xx_nand_cfg_mlc * ncfg ;  struct completion comp_nand ;  struct completion comp_controller ;  uint32_t llptr ;  dma_addr_t oob_buf_phy ;  uint8_t * oob_buf ;  dma_addr_t io_base_phy ;  struct completion comp_dma ;  struct dma_chan * dma_chan ;  struct dma_slave_config dma_slave_config ;  struct scatterlist sgl ;  uint8_t * dma_buf ;  uint8_t * dummy_buf ;  int mlcsubpages ; } cocci_id@p ;
