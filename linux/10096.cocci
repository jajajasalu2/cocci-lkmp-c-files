...
position p;
@@
 struct tun_page {  struct page * page ;  int count ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct net_device * dev , struct ethtool_link_ksettings * cmd ) ;
...
typedef u8;
@@
 struct {  u8 u [ ETH_ALEN ] ; } * cocci_id@p ;
...
typedef u32;
@@
 struct tun_flow_entry {  struct hlist_node hash_link ;  struct rcu_head rcu ;  struct tun_struct * tun ;  u32 rxhash ;  u32 rps_rxhash ;  int queue_index ;  unsigned long updated ____cacheline_aligned_in_smp ; } cocci_id@p ;
...
position p;
@@
 struct veth {  __be16 h_vlan_proto ;  __be16 h_vlan_TCI ; } cocci_id@p ;
...
typedef bool,u16;
@@
 struct tun_file {  struct sock sk ;  struct socket socket ;  struct tun_struct __rcu * tun ;  struct fasync_struct * fasync ;  unsigned int flags ;  union {  u16 queue_index ;  unsigned int ifindex ; } ;  struct napi_struct napi ;  bool napi_enabled ;  bool napi_frags_enabled ;  struct mutex napi_mutex ;  struct list_head next ;  struct tun_struct * detached ;  struct ptr_ring tx_ring ;  struct xdp_rxq_info xdp_rxq ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u32,u64_stats_t;
@@
 struct tun_pcpu_stats {  u64_stats_t rx_packets ;  u64_stats_t rx_bytes ;  u64_stats_t tx_packets ;  u64_stats_t tx_bytes ;  struct u64_stats_sync syncp ;  u32 rx_dropped ;  u32 tx_dropped ;  u32 rx_frame_errors ; } cocci_id@p ;
...
position p;
@@
 const struct tun_file * cocci_id@p ;
...
position p;
@@
 struct tun_pcpu_stats cocci_id@p ;
...
position p;
@@
 struct tun_flow_entry * cocci_id@p ;
...
position p;
@@
 struct tun_prog {  struct rcu_head rcu ;  struct bpf_prog * prog ; } cocci_id@p ;
...
typedef u32;
@@
 struct tap_filter {  unsigned int count ;  u32 mask [ 2 ] ;  unsigned char addr [ FLT_EXACT_COUNT ] [ ETH_ALEN ] ; } cocci_id@p ;
