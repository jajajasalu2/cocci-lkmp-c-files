...
typedef u64,u16;
@@
 struct carm_port {  unsigned int port_no ;  struct gendisk * disk ;  struct carm_host * host ;  u64 capacity ;  char name [ 41 ] ;  u16 dev_geom_head ;  u16 dev_geom_sect ;  u16 dev_geom_cyl ; } cocci_id@p ;
...
position p;
@@
 struct carm_msg_allocbuf * cocci_id@p ;
...
position p;
@@
 struct carm_msg_ioctl cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct block_device * bdev , struct hd_geometry * geo ) ;
...
position p;
@@
 unsigned int ( * cocci_id@p ) ( struct carm_host * , unsigned int , void * ) ;
...
position p;
@@
 struct carm_array_info cocci_id@p ;
...
position p;
@@
 struct carm_msg_sync_time * cocci_id@p ;
...
position p;
@@
 struct carm_response * cocci_id@p ;
...
position p;
@@
 struct carm_request * cocci_id@p ;
...
position p;
@@
 struct carm_request {  int n_elem ;  unsigned int msg_type ;  unsigned int msg_subtype ;  unsigned int msg_bucket ;  struct scatterlist sg [ CARM_MAX_REQ_SG ] ; } cocci_id@p ;
...
position p;
@@
 struct carm_msg_get_fw_ver * cocci_id@p ;
...
position p;
@@
 enum { CARM_SG_BOUNDARY = 0xffffUL , } cocci_id@p ;
...
position p;
@@
 enum scatter_gather_types { SGT_32BIT = 0 , SGT_64BIT = 1 , } cocci_id@p ;
...
position p;
@@
 enum host_states { HST_INVALID , HST_ALLOC_BUF , HST_ERROR , HST_PORT_SCAN , HST_DEV_SCAN_START , HST_DEV_SCAN , HST_DEV_ACTIVATE , HST_PROBE_FINISHED , HST_PROBE_START , HST_SYNC_TIME , HST_GET_FW_VER , } cocci_id@p ;
...
@@
 struct carm_host {  unsigned long flags ;  void __iomem * mmio ;  void * shm ;  dma_addr_t shm_dma ;  int major ;  int id ;  char name [ 32 ] ;  spinlock_t lock ;  struct pci_dev * pdev ;  unsigned int state ;  u32 fw_ver ;  struct blk_mq_tag_set tag_set ;  struct request_queue * oob_q ;  unsigned int n_oob ;  unsigned int hw_sg_used ;  unsigned int resp_idx ;  unsigned int wait_q_prod ;  unsigned int wait_q_cons ;  struct request_queue * wait_q [ CARM_MAX_WAIT_Q ] ;  void * msg_base ;  dma_addr_t msg_dma ;  int cur_scan_dev ;  unsigned long dev_active ;  unsigned long dev_present ;  struct carm_port port [ CARM_MAX_PORTS ] ;  struct work_struct fsm_task ;  struct completion probe_comp ; } cocci_id@p ;
