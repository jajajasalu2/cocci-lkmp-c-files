...
position p;
@@
 struct f81534_port_private cocci_id@p ;
...
typedef u8;
@@
 struct f81534_serial_private {  u8 conf_data [ F81534_DEF_CONF_SIZE ] ;  int tty_idx [ F81534_NUM_PORT ] ;  u8 setting_idx ;  int opened_port ;  struct mutex urb_mutex ; } cocci_id@p ;
...
position p;
@@
 struct f81534_serial_private * cocci_id@p ;
...
@@
 struct f81534_port_private {  struct mutex mcr_mutex ;  struct mutex lcr_mutex ;  struct work_struct lsr_work ;  struct usb_serial_port * port ;  unsigned long tx_empty ;  spinlock_t msr_lock ;  u32 baud_base ;  u8 shadow_mcr ;  u8 shadow_lcr ;  u8 shadow_msr ;  u8 shadow_clk ;  u8 phy_num ; } cocci_id@p ;
...
position p;
@@
 const struct f81534_port_out_pin cocci_id@p [ ] ;
...
position p;
@@
 struct f81534_port_out_pin {  struct f81534_pin_data pin [ 3 ] ; } cocci_id@p ;
...
typedef u8,u16;
@@
 struct f81534_pin_data {  const u16 reg_addr ;  const u8 reg_mask ; } cocci_id@p ;
...
typedef u8;
@@
 u8 cocci_id@p [ F81534_MAX_DATA_BLOCK ] ;
