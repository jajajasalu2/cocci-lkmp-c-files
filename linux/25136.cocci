...
position p;
@@
 struct usbtest_param cocci_id@p ;
...
position p;
@@
 union {  char buf [ 255 ] ;  struct usb_interface_descriptor intf ; } cocci_id@p ;
...
position p;
@@
 struct usbtest_param {  unsigned test_num ;  unsigned iterations ;  unsigned length ;  unsigned vary ;  unsigned sglen ;  struct timeval duration ; } cocci_id@p ;
...
position p;
@@
 enum usb_device_speed { USB_SPEED_UNKNOWN = 0 , USB_SPEED_LOW , USB_SPEED_FULL , USB_SPEED_HIGH , } cocci_id@p ;
...
typedef pthread_t;
@@
 struct testdev {  struct testdev * next ;  char * name ;  pthread_t thread ;  enum usb_device_speed speed ;  unsigned ifnum : 8 ;  unsigned forever : 1 ;  int test ;  struct usbtest_param param ; } cocci_id@p ;
