...
position p;
@@
 struct mvebu_sei_caps cocci_id@p ;
...
@@
 struct mvebu_sei {  struct device * dev ;  void __iomem * base ;  struct resource * res ;  struct irq_domain * sei_domain ;  struct irq_domain * ap_domain ;  struct irq_domain * cp_domain ;  const struct mvebu_sei_caps * caps ;  struct mutex cp_msi_lock ; 
	 DECLARE_BITMAP ( cp_msi_bitmap , SEI_IRQ_COUNT ) 
		 ;  raw_spinlock_t mask_lock ; } cocci_id@p ;
...
typedef u32;
@@
 struct mvebu_sei_interrupt_range {  u32 first ;  u32 size ; } cocci_id@p ;
...
position p;
@@
 struct mvebu_sei_caps {  struct mvebu_sei_interrupt_range ap_range ;  struct mvebu_sei_interrupt_range cp_range ; } cocci_id@p ;
