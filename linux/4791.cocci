...
position p;
@@
 void cocci_id@p ( struct ib_cq * cq , struct ib_wc * wc , const char * opname ) ;
...
position p;
@@
 struct srp_map_state cocci_id@p ;
...
position p;
@@
 struct srp_rport_identifiers cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ IB_DEVICE_NAME_MAX + 8 ] ;
...
position p;
@@
 struct srp_tsk_mgmt * cocci_id@p ;
...
position p;
@@
 struct srp_cred_req * cocci_id@p ;
...
position p;
@@
 struct srp_cred_rsp cocci_id@p ;
...
position p;
@@
 struct scatterlist cocci_id@p [ 1 ] ;
...
position p;
@@
 struct srp_login_req_rdma cocci_id@p ;
...
position p;
@@
 struct srp_direct_buf cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct ib_cm_id * cm_id , const struct ib_cm_event * event ) ;
...
position p;
@@
 struct srp_indirect_buf cocci_id@p ;
...
position p;
@@
 const struct srp_login_rsp * cocci_id@p ;
...
position p;
@@
 struct srp_login_rej * cocci_id@p ;
...
position p;
@@
 struct ib_fmr_pool_param cocci_id@p ;
...
position p;
@@
 struct srp_login_req cocci_id@p ;
...
position p;
@@
 struct srp_target_port * cocci_id@p ;
...
position p;
@@
 struct srp_function_template cocci_id@p ;
...
position p;
@@
 struct {  struct rdma_conn_param rdma_param ;  struct srp_login_req_rdma rdma_req ;  struct ib_cm_req_param ib_param ;  struct srp_login_req ib_req ; } * cocci_id@p ;
...
position p;
@@
 const struct ib_device_attr * cocci_id@p ;
...
position p;
@@
 enum { SRP_OPT_ERR = 0 , SRP_OPT_ID_EXT = 1 << 0 , SRP_OPT_IOC_GUID = 1 << 1 , SRP_OPT_DGID = 1 << 2 , SRP_OPT_PKEY = 1 << 3 , SRP_OPT_SERVICE_ID = 1 << 4 , SRP_OPT_MAX_SECT = 1 << 5 , SRP_OPT_MAX_CMD_PER_LUN = 1 << 6 , SRP_OPT_IO_CLASS = 1 << 7 , SRP_OPT_INITIATOR_EXT = 1 << 8 , SRP_OPT_CMD_SG_ENTRIES = 1 << 9 , SRP_OPT_ALLOW_EXT_SG = 1 << 10 , SRP_OPT_SG_TABLESIZE = 1 << 11 , SRP_OPT_COMP_VECTOR = 1 << 12 , SRP_OPT_TL_RETRY_COUNT = 1 << 13 , SRP_OPT_QUEUE_SIZE = 1 << 14 , SRP_OPT_IP_SRC = 1 << 15 , SRP_OPT_IP_DEST = 1 << 16 , SRP_OPT_TARGET_CAN_QUEUE = 1 << 17 , SRP_OPT_MAX_IT_IU_SIZE = 1 << 18 , } cocci_id@p ;
