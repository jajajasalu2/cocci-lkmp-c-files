...
typedef u32;
@@
 struct vmpipe_proto_header {  u32 pkt_type ;  u32 data_size ; } cocci_id@p ;
...
position p;
@@
 struct hvs_send_buf * cocci_id@p ;
...
position p;
@@
 struct vmpipe_proto_header cocci_id@p ;
...
position p;
@@
 union hvs_service_id cocci_id@p ;
...
position p;
@@
 struct hvs_recv_buf * cocci_id@p ;
...
typedef u8;
@@
 struct hvs_recv_buf {  struct vmpipe_proto_header hdr ;  u8 data [ HVS_MTU_SIZE ] ; } cocci_id@p ;
...
typedef guid_t;
@@
 union hvs_service_id {  guid_t srv_id ;  struct {  unsigned int svm_port ;  unsigned char b [ sizeof ( guid_t ) - sizeof ( unsigned int ) ] ; } ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,u32,guid_t;
@@
 struct hvsock {  struct vsock_sock * vsk ;  guid_t vm_srv_id ;  guid_t host_srv_id ;  struct vmbus_channel * chan ;  struct vmpacket_descriptor * recv_desc ;  u32 recv_data_len ;  u32 recv_data_off ;  bool fin_sent ; } cocci_id@p ;
...
typedef u8;
@@
 struct hvs_send_buf {  struct vmpipe_proto_header hdr ;  u8 data [ HVS_SEND_BUF_SIZE ] ; } cocci_id@p ;
