...
position p;
@@
 enum filter_op_ids { OPS , } cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( const char * str , void * data , int pos , struct filter_parse_error * pe , struct filter_pred * * pred ) ;
...
position p;
@@
 enum { TOO_MANY_CLOSE = - 1 , TOO_MANY_OPEN = - 2 , MISSING_QUOTE = - 3 , } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef test_filter_data_t;
@@
 struct test_filter_data_t * cocci_id@p ;
...
position p;
@@
 enum { INVERT = 1 , PROCESS_AND = 2 , PROCESS_OR = 4 , } cocci_id@p ;
...
position p;
@@
 struct function_filter_data cocci_id@p ;
...
position p;
@@
 struct function_filter_data {  struct ftrace_ops * ops ;  int first_filter ;  int first_notrace ; } cocci_id@p ;
...
position p;
@@
 struct event_filter * * cocci_id@p ;
...
position p;
@@
 struct filter_parse_error {  int lasterr ;  int lasterr_pos ; } cocci_id@p ;
...
position p;
@@
 struct trace_subsystem_dir * cocci_id@p ;
...
position p;
@@
 struct filter_parse_error * * cocci_id@p ;
...
position p;
@@
 struct event_subsystem * cocci_id@p ;
...
position p;
@@
 struct filter_list {  struct list_head list ;  struct event_filter * filter ; } cocci_id@p ;
...
position p;
@@
 struct prog_entry {  int target ;  int when_to_branch ;  struct filter_pred * pred ; } cocci_id@p ;
...
typedef test_filter_data_t;
@@
 struct test_filter_data_t {  char * filter ;  struct trace_event_raw_ftrace_test_filter rec ;  int match ;  char * not_visited ; } cocci_id@p [ ] ;
...
symbol cocci_id;
position p;
typedef filter_pred_fn_t;
@@
 filter_pred_fn_t cocci_id@p ;
