...
position p;
@@
 struct vpu_ipi_desc * cocci_id@p ;
...
typedef dma_addr_t;
@@
 struct vpu_mem {  void * va ;  dma_addr_t pa ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef ipi_handler_t;
@@
 ipi_handler_t cocci_id@p ;
...
typedef u32,s32;
@@
 struct share_obj {  s32 id ;  u32 len ;  unsigned char share_buf [ SHARE_BUF_SIZE ] ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,u32,wait_queue_head_t;
@@
 struct mtk_vpu {  struct vpu_mem extmem [ 2 ] ;  struct vpu_regs reg ;  struct vpu_run run ;  struct vpu_wdt wdt ;  struct vpu_ipi_desc ipi_desc [ IPI_MAX ] ;  struct share_obj * recv_buf ;  struct share_obj * send_buf ;  struct device * dev ;  struct clk * clk ;  bool fw_loaded ;  bool enable_4GB ;  struct mutex vpu_mutex ;  u32 wdt_refcnt ;  wait_queue_head_t ack_wq ;  bool ipi_id_ack [ IPI_MAX ] ; } cocci_id@p ;
...
position p;
@@
 struct vpu_wdt {  struct vpu_wdt_handler handler [ VPU_RST_MAX ] ;  struct work_struct ws ;  struct workqueue_struct * wq ; } cocci_id@p ;
...
position p;
@@
 struct vpu_wdt_handler {  void ( * reset_func ) ( void * ) ;  void * priv ; } cocci_id@p ;
...
position p;
@@
 struct vpu_wdt_handler * cocci_id@p ;
...
@@
 struct vpu_run {  u32 signaled ;  char fw_ver [ VPU_FW_VER_LEN ] ;  unsigned int dec_capability ;  unsigned int enc_capability ;  wait_queue_head_t wq ; } cocci_id@p ;
...
position p;
@@
 struct vpu_regs {  void __iomem * tcm ;  void __iomem * cfg ;  int irq ; } cocci_id@p ;
...
position p;
@@
 enum vpu_fw_type { P_FW , D_FW , } cocci_id@p ;
...
typedef ipi_handler_t;
@@
 struct vpu_ipi_desc {  ipi_handler_t handler ;  const char * name ;  void * priv ; } cocci_id@p ;
