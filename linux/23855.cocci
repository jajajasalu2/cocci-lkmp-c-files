...
position p;
@@
 struct fl_flow_tmplt {  struct fl_flow_key dummy_key ;  struct fl_flow_key mask ;  struct flow_dissector dissector ;  struct tcf_chain * chain ; } cocci_id@p ;
...
position p;
@@
 struct rhashtable_params cocci_id@p ;
...
position p;
@@
 struct fl_flow_mask * cocci_id@p ;
...
position p;
@@
 struct flow_dissector_key cocci_id@p [ FLOW_DISSECTOR_KEY_MAX ] ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_FLOWER_KEY_ENC_OPT_GENEVE_MAX + 1 ] ;
...
position p;
@@
 struct cls_fl_filter * cocci_id@p ;
...
position p;
@@
 struct fl_flow_mask_range {  unsigned short int start ;  unsigned short int end ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,u32,refcount_t;
@@
 struct cls_fl_filter {  struct fl_flow_mask * mask ;  struct rhash_head ht_node ;  struct fl_flow_key mkey ;  struct tcf_exts exts ;  struct tcf_result res ;  struct fl_flow_key key ;  struct list_head list ;  struct list_head hw_list ;  u32 handle ;  u32 flags ;  u32 in_hw_count ;  struct rcu_work rwork ;  struct net_device * hw_dev ;  refcount_t refcnt ;  bool deleted ; } cocci_id@p ;
...
position p;
@@
 const struct fl_flow_mask * cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_FLOWER_MAX + 1 ] ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_FLOWER_KEY_ENC_OPT_VXLAN_MAX + 1 ] ;
...
position p;
@@
 struct fl_flow_tmplt * cocci_id@p ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_FLOWER_KEY_ENC_OPT_VXLAN_MAX + 1 ] ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_FLOWER_KEY_ENC_OPT_GENEVE_MAX + 1 ] ;
...
typedef spinlock_t;
@@
 struct cls_fl_head {  struct rhashtable ht ;  spinlock_t masks_lock ;  struct list_head masks ;  struct list_head hw_filters ;  struct rcu_work rwork ;  struct idr handle_idr ; } cocci_id@p ;
...
@@
 struct fl_flow_mask {  struct fl_flow_key key ;  struct fl_flow_mask_range range ;  u32 flags ;  struct rhash_head ht_node ;  struct rhashtable ht ;  struct rhashtable_params filter_ht_params ;  struct flow_dissector dissector ;  struct list_head filters ;  struct rcu_work rwork ;  struct list_head list ;  refcount_t refcnt ; } cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_FLOWER_KEY_ENC_OPTS_MAX + 1 ] ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_FLOWER_KEY_ENC_OPT_ERSPAN_MAX + 1 ] ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_FLOWER_KEY_ENC_OPT_ERSPAN_MAX + 1 ] ;
