...
position p;
@@
 struct blk_major_name * cocci_id@p ;
...
position p;
@@
 char * cocci_id@p [ ARRAY_SIZE ( disk_uevents ) + 1 ] ;
...
position p;
@@
 struct class_dev_iter cocci_id@p ;
...
position p;
@@
 const  cocci_id@p ( events_poll_msecs , 0644 , 
...
position p;
@@
 void cocci_id@p ( struct gendisk * disk ) ;
...
position p;
@@
 struct blk_major_name {  struct blk_major_name * next ;  int major ;  char name [ 16 ] ; } * cocci_id@p [ BLKDEV_MAJOR_HASH_SIZE ] ;
...
typedef dev_t;
@@
 struct kobject * ( * cocci_id@p ) ( dev_t , int * , void * ) ;
...
position p;
@@
 struct disk_part_tbl * cocci_id@p ;
...
typedef spinlock_t;
@@
 struct disk_events {  struct list_head node ;  struct gendisk * disk ;  spinlock_t lock ;  struct mutex block_mutex ;  int block ;  unsigned int pending ;  unsigned int clearing ;  long poll_msecs ;  struct delayed_work dwork ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct disk_events * ev , unsigned int * clearing_ptr ) ;
