...
symbol cocci_id;
position p;
typedef kuid_t,time64_t,kgid_t;
@@
 struct hypfs_sb_info {  kuid_t uid ;  kgid_t gid ;  struct dentry * update_file ;  time64_t last_update ;  struct mutex lock ; } cocci_id@p ;
...
position p;
@@
 enum { Opt_uid , Opt_gid , } cocci_id@p ;
...
position p;
@@
 struct hypfs_sb_info cocci_id@p ;
...
position p;
@@
 struct dentry * cocci_id@p ( struct dentry * dir ) ;
