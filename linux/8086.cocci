...
position p;
@@
 struct vlan_pcpu_stats cocci_id@p ;
...
position p;
@@
 enum macvlan_macaddr_mode cocci_id@p ;
...
position p;
@@
 const struct macvlan_dev * cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ IFLA_MACVLAN_MAX + 1 ] ;
...
position p;
@@
 struct macvlan_source_entry cocci_id@p ;
...
position p;
@@
 struct macvlan_port * cocci_id@p ;
...
position p;
@@
 struct macvlan_skb_cb {  const struct macvlan_dev * src ; } cocci_id@p ;
...
position p;
@@
 struct macvlan_skb_cb * cocci_id@p ;
...
typedef u32;
@@
 struct macvlan_port {  struct net_device * dev ;  struct hlist_head vlan_hash [ MACVLAN_HASH_SIZE ] ;  struct list_head vlans ;  struct sk_buff_head bc_queue ;  struct work_struct bc_work ;  u32 flags ;  int count ;  struct hlist_head vlan_source_hash [ MACVLAN_HASH_SIZE ] ; 
	 DECLARE_BITMAP ( mc_filter , MACVLAN_MC_FILTER_SZ ) 
		 ;  unsigned char perm_addr [ ETH_ALEN ] ; } cocci_id@p ;
...
position p;
@@
 const struct macvlan_port * cocci_id@p ;
