...
position p;
@@
 const struct dm1105_subid cocci_id@p [ ] ;
...
position p;
@@
 const struct dm1105_board cocci_id@p [ ] ;
...
typedef u32,u16;
@@
 struct dm1105_subid {  u16 subvendor ;  u16 subdevice ;  u32 card ; } cocci_id@p ;
...
typedef u32;
@@
 struct infrared {  struct rc_dev * dev ;  char input_phys [ 32 ] ;  struct work_struct work ;  u32 ir_command ; } cocci_id@p ;
...
position p;
@@
 struct stv0288_config cocci_id@p ;
...
position p;
@@
 struct si21xx_config cocci_id@p ;
...
position p;
@@
 const struct stv0299_config cocci_id@p ;
...
@@
 struct dm1105_dev {  struct pci_dev * pdev ;  u8 __iomem * io_mem ;  struct infrared ir ;  struct dmx_frontend hw_frontend ;  struct dmx_frontend mem_frontend ;  struct dmxdev dmxdev ;  struct dvb_adapter dvb_adapter ;  struct dvb_demux demux ;  struct dvb_frontend * fe ;  struct dvb_net dvbnet ;  unsigned int full_ts_users ;  unsigned int boardnr ;  int nr ;  struct i2c_adapter i2c_adap ;  struct i2c_adapter i2c_bb_adap ;  struct i2c_algo_bit_data i2c_bit ;  struct work_struct work ;  struct workqueue_struct * wq ;  char wqn [ 16 ] ;  dma_addr_t dma_addr ;  unsigned char * ts_buf ;  u32 wrp ;  u32 nextwrp ;  u32 buffer_size ;  unsigned int PacketErrorCount ;  unsigned int dmarst ;  spinlock_t lock ; } cocci_id@p ;
