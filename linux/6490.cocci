...
position p;
@@
 const struct supply_info * cocci_id@p ;
...
position p;
@@
 struct field {  int reg ;  int shift ;  int mask ; } cocci_id@p ;
...
position p;
@@
 const struct supply_info cocci_id@p [ N_REGULATORS ] ;
...
position p;
@@
 struct tps6524x {  struct device * dev ;  struct spi_device * spi ;  struct mutex lock ;  struct regulator_desc desc [ N_REGULATORS ] ; } cocci_id@p ;
