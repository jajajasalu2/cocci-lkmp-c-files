...
position p;
@@
 struct amd_iommu_event_desc {  struct kobj_attribute attr ;  const char * event ; } cocci_id@p ;
...
position p;
@@
 struct perf_amd_iommu * cocci_id@p ;
...
position p;
@@
 struct amd_iommu_event_desc * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,u64,raw_spinlock_t;
@@
 struct perf_amd_iommu {  struct list_head list ;  struct pmu pmu ;  struct amd_iommu * iommu ;  char name [ IOMMU_NAME_SIZE ] ;  u8 max_banks ;  u8 max_counters ;  u64 cntr_assign_mask ;  raw_spinlock_t lock ; } cocci_id@p ;
