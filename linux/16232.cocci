...
typedef u16;
@@
 void cocci_id@p ( const u16 * in_buf , int count ) ;
...
position p;
@@
 enum { edge_top = 1 , edge_bottom , edge_left , edge_right , edge_quiet , } cocci_id@p ;
...
position p;
@@
 const struct st_bits_data * cocci_id@p ;
...
position p;
@@
 enum { CT_Off = 0 , CT_On , CT_Highlight , CT_Window , CT_Max , } cocci_id@p ;
...
typedef st_spk_t;
@@
 struct st_spk_t * cocci_id@p [ MAX_NR_CONSOLES ] ;
...
position p;
@@
 void ( * cocci_id@p ) ( struct vc_data * ) ;
...
position p;
@@
 struct keyboard_notifier_param * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct vc_data * vc , int command ) ;
...
position p;
@@
 struct st_var_header * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct notifier_block * , unsigned long code , void * param ) ;
...
position p;
@@
 enum { RA_NOTHING , RA_NEXT_SENT , RA_PREV_LINE , RA_NEXT_LINE , RA_PREV_SENT , RA_DOWN_ARROW , RA_TIMER , RA_FIND_NEXT_SENT , RA_FIND_PREV_SENT , } cocci_id@p ;
