...
position p;
@@
 struct external_name cocci_id@p ;
...
position p;
@@
 struct name_snapshot * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef dentry_stat_t;
@@
 struct dentry_stat_t cocci_id@p ;
...
position p;
@@
 struct check_mount {  struct vfsmount * mnt ;  unsigned int mounted ; } cocci_id@p ;
...
position p;
@@
 enum d_walk_ret ( * cocci_id@p ) ( void * , struct dentry * ) ;
...
typedef atomic_t;
@@
 struct external_name {  union {  atomic_t count ;  struct rcu_head head ; } u ;  unsigned char name [ ] ; } cocci_id@p ;
...
position p;
@@
 struct hlist_bl_head * cocci_id@p ;
...
position p;
@@
 enum d_walk_ret { D_WALK_CONTINUE , D_WALK_QUIT , D_WALK_NORETRY , D_WALK_SKIP , } cocci_id@p ;
...
position p;
@@
 struct hlist_bl_head cocci_id@p [ 1 << IN_LOOKUP_SHIFT ] ;
...
position p;
@@
 struct select_data {  struct dentry * start ;  union {  long found ;  struct dentry * victim ; } ;  struct list_head dispose ; } cocci_id@p ;
