...
typedef u8,u16;
@@
 struct u132_endp {  struct kref kref ;  u8 udev_number ;  u8 endp_number ;  u8 usb_addr ;  u8 usb_endp ;  struct u132 * u132 ;  struct list_head endp_ring ;  struct u132_ring * ring ;  unsigned toggle_bits : 2 ;  unsigned active : 1 ;  unsigned delayed : 1 ;  unsigned input : 1 ;  unsigned output : 1 ;  unsigned pipetype : 2 ;  unsigned dequeueing : 1 ;  unsigned edset_flush : 1 ;  unsigned spare_bits : 14 ;  unsigned long jiffies ;  struct usb_host_endpoint * hep ;  struct u132_spin queue_lock ;  u16 queue_size ;  u16 queue_last ;  u16 queue_next ;  struct urb * urb_list [ ENDP_QUEUE_SIZE ] ;  struct list_head urb_more ;  struct delayed_work scheduler ; } cocci_id@p ;
...
typedef u8;
@@
 struct u132_addr {  u8 address ; } cocci_id@p ;
...
position p;
@@
 struct u132_platform_data * cocci_id@p ;
...
position p;
@@
 struct u132_port {  struct u132 * u132 ;  int reset ;  int enable ;  int power ;  int Status ; } cocci_id@p ;
...
typedef u8;
@@
 void ( * cocci_id@p ) ( void * endp , struct urb * urb , u8 * buf , int len , int toggle_bits , int error_count , int condition_code , int repeat_number , int halted , int skipped , int actual , int non_null ) ;
...
typedef u8;
@@
 struct u132_udev {  struct kref kref ;  struct usb_device * usb_device ;  u8 enumeration ;  u8 udev_number ;  u8 usb_addr ;  u8 portnumber ;  u8 endp_number_in [ 16 ] ;  u8 endp_number_out [ 16 ] ; } cocci_id@p ;
...
typedef u8;
@@
 struct u132_ring {  unsigned in_use : 1 ;  unsigned length : 7 ;  u8 number ;  struct u132 * u132 ;  struct u132_endp * curr_endp ;  struct delayed_work scheduler ; } cocci_id@p ;
...
position p;
@@
 struct u132_urbq {  struct list_head urb_more ;  struct urb * urb ; } cocci_id@p ;
...
typedef spinlock_t;
@@
 struct u132_spin {  spinlock_t slock ; } cocci_id@p ;
