...
position p;
@@
 struct zd_reg_alpha2_map cocci_id@p [ ] ;
...
position p;
@@
 const struct tx_retry_rate cocci_id@p [ ] ;
...
position p;
@@
 void cocci_id@p ( struct zd_mac * mac ) ;
...
position p;
@@
 void cocci_id@p ( struct zd_mac * mac , unsigned int short_preamble ) ;
...
typedef bool;
@@
 int cocci_id@p ( struct ieee80211_hw * hw , struct sk_buff * beacon , bool in_intr ) ;
...
position p;
@@
 const struct rx_status * cocci_id@p ;
...
typedef u32;
@@
 struct zd_reg_alpha2_map {  u32 reg ;  char alpha2 [ 2 ] ; } cocci_id@p ;
