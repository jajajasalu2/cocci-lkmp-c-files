...
typedef u16;
@@
 const u16 cocci_id@p [ NR_HW_IRQS ] [ IPROC_MSI_REG_SIZE ] ;
...
symbol cocci_id;
position p;
typedef bool,u16,phys_addr_t,dma_addr_t;
@@
 struct iproc_msi {  struct iproc_pcie * pcie ;  const  u16 ( * reg_offsets ) ;  struct iproc_msi_grp * grps ;  int nr_irqs ;  int nr_cpus ;  bool has_inten_reg ;  unsigned long * bitmap ;  struct mutex bitmap_lock ;  unsigned int nr_msi_vecs ;  struct irq_domain * inner_domain ;  struct irq_domain * msi_domain ;  unsigned int nr_eq_region ;  unsigned int nr_msi_region ;  void * eq_cpu ;  dma_addr_t eq_dma ;  phys_addr_t msi_addr ; } cocci_id@p ;
...
position p;
@@
 struct iproc_msi_grp * cocci_id@p ;
...
position p;
@@
 struct iproc_msi_grp {  struct iproc_msi * msi ;  int gic_irq ;  unsigned int eq ; } cocci_id@p ;
...
position p;
@@
 enum iproc_msi_reg { IPROC_MSI_EQ_PAGE = 0 , IPROC_MSI_EQ_PAGE_UPPER , IPROC_MSI_PAGE , IPROC_MSI_PAGE_UPPER , IPROC_MSI_CTRL , IPROC_MSI_EQ_HEAD , IPROC_MSI_EQ_TAIL , IPROC_MSI_INTS_EN , IPROC_MSI_REG_SIZE , } cocci_id@p ;
