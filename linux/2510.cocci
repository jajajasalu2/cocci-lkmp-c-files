...
position p;
@@
 struct frame {  struct dm_block * b ;  struct btree_node * n ;  unsigned level ;  unsigned nr_children ;  unsigned current_child ; } cocci_id@p ;
...
position p;
@@
 struct del_stack {  struct dm_btree_info * info ;  struct dm_transaction_manager * tm ;  int top ;  struct frame spine [ MAX_SPINE_DEPTH ] ; } cocci_id@p ;
...
position p;
@@
 struct dm_btree_cursor * cocci_id@p ;
...
position p;
@@
 struct shadow_spine * cocci_id@p ;
