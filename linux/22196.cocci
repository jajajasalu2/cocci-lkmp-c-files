...
position p;
@@
 struct kmemleak_object * cocci_id@p ;
...
position p;
@@
 const struct vm_struct * cocci_id@p ;
...
position p;
@@
 const struct kmemleak_object * cocci_id@p ;
...
position p;
@@
 struct kmemleak_object cocci_id@p [ CONFIG_DEBUG_KMEMLEAK_MEM_POOL_SIZE ] ;
...
position p;
@@
 struct kmemleak_scan_area * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u32,atomic_t,spinlock_t,pid_t;
@@
 struct kmemleak_object {  spinlock_t lock ;  unsigned int flags ;  struct list_head object_list ;  struct list_head gray_list ;  struct rb_node rb_node ;  struct rcu_head rcu ;  atomic_t use_count ;  unsigned long pointer ;  size_t size ;  unsigned long excess_ref ;  int min_count ;  int count ;  u32 checksum ;  struct hlist_head area_list ;  unsigned long trace [ MAX_TRACE ] ;  unsigned int trace_len ;  unsigned long jiffies ;  pid_t pid ;  char comm [ TASK_COMM_LEN ] ; } cocci_id@p ;
...
position p;
@@
 struct kmemleak_scan_area {  struct hlist_node node ;  unsigned long start ;  size_t size ; } cocci_id@p ;
