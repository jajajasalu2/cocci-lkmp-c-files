...
position p;
@@
 enum cfg_options_flags cocci_id@p ;
...
position p;
@@
 struct stats_record {  struct record stats ;  struct record * rxq ; } cocci_id@p ;
...
position p;
@@
 enum cfg_options_flags { NO_TOUCH = 0x0U , READ_MEM = 0x1U , SWAP_MAC = 0x2U , } cocci_id@p ;
...
typedef __u32;
@@
 struct config {  __u32 action ;  int ifindex ;  __u32 options ; } cocci_id@p ;
...
typedef __u64;
@@
 struct datarec {  __u64 processed ;  __u64 issue ; } cocci_id@p ;
...
position p;
@@
 char cocci_id@p [ XDP_ACTION_MAX_STRLEN + 1 ] ;
