...
position p;
@@
 const struct perf_event_map cocci_id@p [ ] ;
...
position p;
@@
 const struct sparc_pmu cocci_id@p ;
...
position p;
@@
 struct sparc_stackf32 cocci_id@p ;
...
position p;
@@
 struct perf_event_map cocci_id@p [ PERF_COUNT_HW_CACHE_MAX ] [ PERF_COUNT_HW_CACHE_OP_MAX ] [ PERF_COUNT_HW_CACHE_RESULT_MAX ] ;
...
typedef u64;
@@
 struct cpu_hw_events {  int n_events ;  int n_added ;  struct perf_event * event [ MAX_HWEVENTS ] ;  unsigned long events [ MAX_HWEVENTS ] ;  int current_idx [ MAX_HWEVENTS ] ;  u64 pcr [ MAX_HWEVENTS ] ;  int enabled ;  unsigned int txn_flags ; } cocci_id@p ;
