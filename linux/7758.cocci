...
position p;
@@
 struct ltq_uart_port * cocci_id@p [ MAXPORTS ] ;
...
position p;
@@
 struct ltq_uart_port * cocci_id@p ;
...
typedef spinlock_t;
@@
 struct ltq_uart_port {  struct uart_port port ;  struct clk * freqclk ;  struct clk * clk ;  unsigned int tx_irq ;  unsigned int rx_irq ;  unsigned int err_irq ;  unsigned int common_irq ;  spinlock_t lock ;  const struct ltq_soc_data * soc ; } cocci_id@p ;
...
position p;
@@
 const struct ltq_soc_data cocci_id@p ;
...
position p;
@@
 struct ltq_soc_data {  int ( * fetch_irq ) ( struct device * dev , struct ltq_uart_port * ltq_port ) ;  int ( * request_irq ) ( struct uart_port * port ) ;  void ( * free_irq ) ( struct uart_port * port ) ; } cocci_id@p ;
