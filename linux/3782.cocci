...
position p;
@@
 struct cryp_init_vector_value cocci_id@p ;
...
position p;
@@
 enum cryp_algo_mode cocci_id@p ;
...
position p;
@@
 struct cryp_algo_template cocci_id@p ;
...
position p;
@@
 enum cryp_init_vector_index cocci_id@p ;
...
position p;
@@
 struct cryp_device_data * * cocci_id@p ;
...
typedef u32;
@@
 u32 cocci_id@p [ CRYP_MAX_KEY_SIZE / 4 ] ;
...
position p;
@@
 struct cryp_platform_data * cocci_id@p ;
...
position p;
@@
 enum cryp_key_reg_index cocci_id@p ;
...
position p;
@@
 struct cryp_protection_config cocci_id@p ;
...
typedef u32;
@@
 u32 cocci_id@p [ AES_BLOCK_SIZE / 4 ] ;
...
position p;
@@
 struct cryp_driver_data {  struct klist device_list ;  struct semaphore device_allocation ; } cocci_id@p ;
...
position p;
@@
 struct cryp_algo_template {  enum cryp_algo_mode algomode ;  struct skcipher_alg skcipher ; } cocci_id@p ;
...
typedef u8,u32;
@@
 struct cryp_ctx {  struct cryp_config config ;  u8 key [ CRYP_MAX_KEY_SIZE ] ;  u32 keylen ;  u8 * iv ;  const u8 * indata ;  u8 * outdata ;  u32 datalen ;  u32 outlen ;  u32 blocksize ;  u8 updated ;  struct cryp_device_context dev_ctx ;  struct cryp_device_data * device ;  u32 session_id ; } cocci_id@p ;
...
position p;
@@
 struct cryp_key_value cocci_id@p ;
...
position p;
@@
 struct cryp_driver_data cocci_id@p ;
