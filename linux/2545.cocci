...
position p;
@@
 enum clone_metadata_mode { CM_WRITE , CM_READ_ONLY , CM_FAIL , } cocci_id@p ;
...
typedef spinlock_t;
@@
 struct hash_table_bucket {  struct hlist_head head ;  spinlock_t lock ; } cocci_id@p ;
...
position p;
@@
 struct dm_clone_region_hydration * cocci_id@p ;
...
position p;
@@
 enum clone_metadata_mode cocci_id@p ;
...
position p;
@@
 struct hash_table_bucket cocci_id@p ;
...
position p;
@@
 struct batch_info {  struct dm_clone_region_hydration * head ;  unsigned int nr_batched_regions ; } cocci_id@p ;
...
@@
 struct dm_clone_region_hydration {  struct clone * clone ;  unsigned long region_nr ;  struct bio * overwrite_bio ;  bio_end_io_t * overwrite_bio_end_io ;  struct bio_list deferred_bios ;  blk_status_t status ;  struct list_head list ;  struct hlist_node h ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef wait_queue_head_t,sector_t,spinlock_t,mempool_t,atomic_t;
@@
 struct clone {  struct dm_target * ti ;  struct dm_target_callbacks callbacks ;  struct dm_dev * metadata_dev ;  struct dm_dev * dest_dev ;  struct dm_dev * source_dev ;  unsigned long nr_regions ;  sector_t region_size ;  unsigned int region_shift ;  struct mutex commit_lock ;  struct dm_clone_metadata * cmd ;  struct bio flush_bio ;  struct hash_table_bucket * ht ;  atomic_t ios_in_flight ;  wait_queue_head_t hydration_stopped ;  mempool_t hydration_pool ;  unsigned long last_commit_jiffies ;  spinlock_t lock ;  struct bio_list deferred_bios ;  struct bio_list deferred_discard_bios ;  struct bio_list deferred_flush_bios ;  struct bio_list deferred_flush_completions ;  unsigned int hydration_threshold ;  unsigned int hydration_batch_size ;  unsigned long hydration_offset ;  atomic_t hydrations_in_flight ;  unsigned int nr_ctr_args ;  const char * * ctr_args ;  struct workqueue_struct * wq ;  struct work_struct worker ;  struct delayed_work waker ;  struct dm_kcopyd_client * kcopyd_client ;  enum clone_metadata_mode mode ;  unsigned long flags ; } cocci_id@p ;
