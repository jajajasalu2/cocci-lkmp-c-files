...
position p;
@@
 enum crb_ctrl_sts { CRB_CTRL_STS_ERROR = BIT ( 0 ) , CRB_CTRL_STS_TPM_IDLE = BIT ( 1 ) , } cocci_id@p ;
...
position p;
@@
 struct resource_win cocci_id@p ;
...
position p;
@@
 struct tpm2_crb_smc cocci_id@p ;
...
position p;
@@
 struct acpi_table_tpm2 * cocci_id@p ;
...
position p;
@@
 struct crb_regs_tail cocci_id@p ;
...
position p;
@@
 enum crb_loc_ctrl { CRB_LOC_CTRL_REQUEST_ACCESS = BIT ( 0 ) , CRB_LOC_CTRL_RELINQUISH = BIT ( 1 ) , } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,u32,regs_t;
@@
 struct crb_priv {  u32 sm ;  const char * hid ;  struct crb_regs_head __iomem * regs_h ;  struct crb_regs_tail __iomem * regs_t ;  u8 __iomem * cmd ;  u8 __iomem * rsp ;  u32 cmd_size ;  u32 smc_func_id ; } cocci_id@p ;
...
position p;
@@
 struct resource cocci_id@p [ TPM_CRB_MAX_RESOURCES + 1 ] ;
...
position p;
@@
 enum crb_loc_state { CRB_LOC_STATE_LOC_ASSIGNED = BIT ( 1 ) , CRB_LOC_STATE_TPM_REG_VALID_STS = BIT ( 7 ) , } cocci_id@p ;
...
position p;
@@
 const struct tpm_class_ops cocci_id@p ;
...
position p;
@@
 enum crb_status { CRB_DRV_STS_COMPLETE = BIT ( 0 ) , } cocci_id@p ;
...
position p;
@@
 enum crb_defaults { CRB_ACPI_START_REVISION_ID = 1 , CRB_ACPI_START_INDEX = 1 , } cocci_id@p ;
...
typedef u8,u32,u16;
@@
 struct tpm2_crb_smc {  u32 interrupt ;  u8 interrupt_flags ;  u8 op_flags ;  u16 reserved2 ;  u32 smc_func_id ; } cocci_id@p ;
...
position p;
@@
 enum crb_cancel { CRB_CANCEL_INVOKE = BIT ( 0 ) , } cocci_id@p ;
...
position p;
@@
 enum crb_ctrl_req { CRB_CTRL_REQ_CMD_READY = BIT ( 0 ) , CRB_CTRL_REQ_GO_IDLE = BIT ( 1 ) , } cocci_id@p ;
...
position p;
@@
 enum crb_start { CRB_START_INVOKE = BIT ( 0 ) , } cocci_id@p ;
