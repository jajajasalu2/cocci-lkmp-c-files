...
position p;
@@
 struct mtd_info * cocci_id@p ( struct map_info * map ) ;
...
position p;
@@
 const struct cfi_early_fixup cocci_id@p [ ] ;
...
typedef uint16_t;
@@
 struct cfi_early_fixup {  uint16_t mfr ;  uint16_t id ;  void ( * fixup ) ( struct cfi_private * cfi ) ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct cfi_ident * ) ;
...
position p;
@@
 int cocci_id@p ( struct map_info * map , struct cfi_private * cfi ) ;
...
typedef __u32;
@@
 int cocci_id@p ( struct map_info * map , __u32 base , unsigned long * chip_map , struct cfi_private * cfi ) ;
