...
position p;
@@
 struct nda_cacheinfo cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,u16,__be32;
@@
 struct vxlan_fdb {  struct hlist_node hlist ;  struct rcu_head rcu ;  unsigned long updated ;  unsigned long used ;  struct list_head remotes ;  u8 eth_addr [ ETH_ALEN ] ;  u16 state ;  __be32 vni ;  u16 flags ; } cocci_id@p ;
...
position p;
@@
 struct udp_tunnel_sock_cfg cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct vxlan_dev * vxlan ) ;
...
position p;
@@
 const struct vxlan_rdst * cocci_id@p ;
...
position p;
@@
 struct vxlanhdr_gbp * cocci_id@p ;
...
position p;
@@
 const struct vxlan_dev * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct vxlan_dev * vxlan ) ;
...
position p;
@@
 const struct ifla_vxlan_port_range * cocci_id@p ;
...
position p;
@@
 struct ifla_vxlan_port_range cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ IFLA_VXLAN_MAX + 1 ] ;
...
position p;
@@
 const struct vxlan_config * cocci_id@p ;
...
position p;
@@
 struct vxlan_dev_node * cocci_id@p ;
...
typedef spinlock_t;
@@
 struct vxlan_net {  struct list_head vxlan_list ;  struct hlist_head sock_list [ PORT_HASH_SIZE ] ;  spinlock_t sock_lock ; } cocci_id@p ;
...
position p;
@@
 struct udp_port_cfg cocci_id@p ;
...
position p;
@@
 struct vxlanhdr_gpe * cocci_id@p ;
...
typedef u8;
@@
 const u8 cocci_id@p [ ETH_ALEN + 2 ] ;
...
position p;
@@
 const struct vxlan_fdb * cocci_id@p ;
