...
position p;
@@
 struct vmcs_host_state * cocci_id@p ;
...
typedef bool;
@@
 bool cocci_id@p ( struct kvm_vcpu * vcpu ) ;
...
position p;
@@
 struct perf_guest_switch_msr * cocci_id@p ;
...
position p;
@@
 struct vmx_capability * cocci_id@p ;
...
position p;
@@
 struct vmcs_controls_shadow cocci_id@p ;
...
position p;
@@
 struct shared_msr_entry * cocci_id@p ;
...
position p;
@@
 struct hv_partition_assist_pg * * cocci_id@p ;
...
position p;
@@
 struct hv_enlightened_vmcs * cocci_id@p ;
...
typedef bool;
@@
 bool cocci_id@p ( struct vcpu_vmx * vmx , unsigned long * regs , bool launched ) ;
...
position p;
@@
 struct hv_guest_mapping_flush_list * cocci_id@p ;
...
position p;
@@
 struct msr_autoload * cocci_id@p ;
...
position p;
@@
 const struct kvm_vmx_segment_field {  unsigned selector ;  unsigned base ;  unsigned limit ;  unsigned ar_bytes ; } cocci_id@p [ ] ;
...
position p;
@@
 enum vmx_l1d_flush_state cocci_id@p ;
...
typedef bool;
@@
 const struct {  const char * option ;  bool for_parse ; } cocci_id@p [ ] ;
...
typedef u32;
@@
 u32 cocci_id@p ( struct kvm_segment * var ) ;
...
position p;
@@
 const struct kvm_vmx_segment_field * cocci_id@p ;
...
typedef u64;
@@
 struct {  u64 pcid ;  u64 gla ; } cocci_id@p ;
...
typedef u32;
@@
 void cocci_id@p ( unsigned long * msr_bitmap , u32 msr , int type ) ;
...
position p;
@@
 struct hv_vp_assist_page * cocci_id@p ;
