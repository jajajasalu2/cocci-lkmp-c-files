...
typedef u64;
@@
 struct dmac_map {  u64 vf_map ;  u64 dmac ; } cocci_id@p ;
...
typedef u8,bool;
@@
 struct bgx {  u8 bgx_id ;  struct lmac lmac [ MAX_LMAC_PER_BGX ] ;  u8 lmac_count ;  u8 max_lmac ;  u8 acpi_lmac_idx ;  void __iomem * reg_base ;  struct pci_dev * pdev ;  bool is_dlm ;  bool is_rgx ; } cocci_id@p ;
...
position p;
@@
 enum MCAST_MODE { MCAST_MODE_REJECT = 0x0 , MCAST_MODE_ACCEPT = 0x1 , MCAST_MODE_CAM_FILTER = 0x2 , RSVD = 0x3 , } cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct lmac * lmac ) ;
...
position p;
@@
 struct bgx * cocci_id@p [ MAX_BGX_THUNDER ] ;
...
typedef u8,bool;
@@
 struct lmac {  struct bgx * bgx ;  u8 dmacs_cfg ;  u8 dmacs_count ;  struct dmac_map * dmacs ;  u8 mac [ ETH_ALEN ] ;  u8 lmac_type ;  u8 lane_to_sds ;  bool use_training ;  bool autoneg ;  bool link_up ;  int lmacid ;  int lmacid_bd ;  struct net_device netdev ;  struct phy_device * phydev ;  unsigned int last_duplex ;  unsigned int last_link ;  unsigned int last_speed ;  bool is_sgmii ;  struct delayed_work dwork ;  struct workqueue_struct * check_link ; } cocci_id@p ;
...
position p;
@@
 struct bgx_link_status * cocci_id@p ;
