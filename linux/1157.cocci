...
position p;
@@
 struct usbhsh_ep {  struct usbhs_pipe * pipe ;  struct usbhsh_device * udev ;  struct usb_host_endpoint * ep ;  struct list_head ep_list ;  unsigned int counter ; } cocci_id@p ;
...
position p;
@@
 struct usbhsh_hpriv cocci_id@p ;
...
position p;
@@
 struct usbhsh_device {  struct usb_device * usbv ;  struct list_head ep_list_head ; } cocci_id@p ;
...
position p;
@@
 struct renesas_usbhs_driver_pipe_config * cocci_id@p ;
...
typedef u32;
@@
 struct usbhsh_hpriv {  struct usbhs_mod mod ;  struct usbhs_pipe * dcp ;  struct usbhsh_device udev [ USBHSH_DEVICE_MAX ] ;  u32 port_stat ;  struct completion setup_ack_done ; } cocci_id@p ;
...
position p;
@@
 struct usbhsh_request cocci_id@p ;
...
position p;
@@
 struct usbhs_irq_state * cocci_id@p ;
...
position p;
@@
 struct usbhsh_device * cocci_id@p ( struct usbhsh_hpriv * hpriv , struct urb * urb ) ;
...
position p;
@@
 struct usbhsh_request {  struct urb * urb ;  struct usbhs_pkt pkt ; } cocci_id@p ;
...
position p;
@@
 struct usbhsh_device * cocci_id@p ;
