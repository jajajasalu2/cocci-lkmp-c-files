...
position p;
@@
 struct OpalHMIEvent * cocci_id@p ;
...
position p;
@@
 struct OpalHmiEvtNode * cocci_id@p ;
...
position p;
@@
 const struct xstop_reason cocci_id@p [ ] ;
...
typedef uint32_t;
@@
 struct xstop_reason {  uint32_t xstop_reason ;  const char * unit_failed ;  const char * description ; } cocci_id@p ;
...
position p;
@@
 struct OpalHmiEvtNode {  struct list_head list ;  struct OpalHMIEvent hmi_evt ; } cocci_id@p ;
