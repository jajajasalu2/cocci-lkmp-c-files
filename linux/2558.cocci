...
position p;
@@
 struct dirty_map {  unsigned long * dirty_words ;  unsigned long * dirty_regions ;  unsigned int changed ; } cocci_id@p ;
...
position p;
@@
 struct dm_clone_metadata * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,__u8,dm_block_t,sector_t,spinlock_t;
@@
 struct dm_clone_metadata {  struct block_device * bdev ;  sector_t target_size ;  sector_t region_size ;  unsigned long nr_regions ;  unsigned long nr_words ;  spinlock_t bitmap_lock ;  struct dirty_map dmap [ 2 ] ;  struct dirty_map * current_dmap ;  struct dirty_map * committing_dmap ;  unsigned long * region_map ;  unsigned int read_only ;  struct dm_block_manager * bm ;  struct dm_space_map * sm ;  struct dm_transaction_manager * tm ;  struct rw_semaphore lock ;  struct dm_disk_bitset bitset_info ;  dm_block_t bitset_root ;  __u8 metadata_space_map_root [ SPACE_MAP_ROOT_SIZE ] ;  bool hydration_done : 1 ;  bool fail_io : 1 ; } cocci_id@p ;
