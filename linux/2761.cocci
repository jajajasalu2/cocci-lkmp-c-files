...
position p;
@@
 const struct i2c_adapter * cocci_id@p ;
...
position p;
@@
 const struct itco_wdt_platform_data cocci_id@p ;
...
position p;
@@
 const struct i801_priv * cocci_id@p ;
...
position p;
@@
 const struct dmi_onboard_device_info cocci_id@p [ ] ;
...
position p;
@@
 struct i2c_mux_gpio_platform_data cocci_id@p ;
...
position p;
@@
 struct i801_mux_config {  char * gpio_chip ;  unsigned values [ 3 ] ;  int n_values ;  unsigned classes [ 3 ] ;  unsigned gpios [ 2 ] ;  int n_gpios ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,bool,wait_queue_head_t;
@@
 struct i801_priv {  struct i2c_adapter adapter ;  unsigned long smba ;  unsigned char original_hstcfg ;  unsigned char original_slvcmd ;  struct pci_dev * pci_dev ;  unsigned int features ;  wait_queue_head_t waitq ;  u8 status ;  u8 cmd ;  bool is_read ;  int count ;  int len ;  u8 * data ; #if IS_ENABLED(CONFIG_I2C_MUX_GPIO) && defined CONFIG_DMI  const struct i801_mux_config * mux_drvdata ;  struct platform_device * mux_pdev ;  struct gpiod_lookup_table * lookup ; #endif  struct platform_device * tco_pdev ;  bool acpi_reserved ;  struct mutex acpi_lock ; } cocci_id@p ;
...
position p;
@@
 struct i801_mux_config cocci_id@p ;
...
position p;
@@
 const struct {  const char * dmi_product_name ;  unsigned short i2c_addr ; } cocci_id@p [ ] ;
...
typedef u8;
@@
 struct dmi_onboard_device_info {  const char * name ;  u8 type ;  unsigned short i2c_addr ;  const char * i2c_type ; } cocci_id@p ;
...
position p;
@@
 const struct i801_mux_config * cocci_id@p ;
