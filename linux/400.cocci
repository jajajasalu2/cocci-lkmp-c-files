...
position p;
@@
 struct blacklist_entry {  struct list_head next ;  char * buf ; } cocci_id@p ;
...
position p;
@@
 struct blacklist_entry * cocci_id@p ;
...
position p;
@@
 const char * cocci_id@p [ MAX_INIT_ENVS + 2 ] ;
...
position p;
@@
 const char * cocci_id@p [ MAX_INIT_ARGS + 2 ] ;
...
position p;
@@
 const struct obs_kernel_param cocci_id@p [ ] ;
