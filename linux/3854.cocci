...
position p;
@@
 struct dcp_async_ctx cocci_id@p ;
...
position p;
@@
 struct dcp_aes_req_ctx {  unsigned int enc : 1 ;  unsigned int ecb : 1 ; } cocci_id@p ;
...
position p;
@@
 struct dcp_export_state cocci_id@p ;
...
position p;
@@
 struct dcp_aes_req_ctx cocci_id@p ;
...
position p;
@@
 struct dcp_dma_desc * cocci_id@p ;
...
typedef uint32_t;
@@
 struct dcp_dma_desc {  uint32_t next_cmd_addr ;  uint32_t control0 ;  uint32_t control1 ;  uint32_t source ;  uint32_t destination ;  uint32_t size ;  uint32_t payload ;  uint32_t status ; } cocci_id@p ;
...
position p;
@@
 enum dcp_chan { DCP_CHAN_HASH_SHA = 0 , DCP_CHAN_CRYPTO = 2 , } cocci_id@p ;
...
typedef uint8_t;
@@
 struct dcp_coherent_block {  uint8_t aes_in_buf [ DCP_BUF_SZ ] ;  uint8_t aes_out_buf [ DCP_BUF_SZ ] ;  uint8_t sha_in_buf [ DCP_BUF_SZ ] ;  uint8_t sha_out_buf [ DCP_SHA_PAY_SZ ] ;  uint8_t aes_key [ 2 * AES_KEYSIZE_128 ] ;  struct dcp_dma_desc desc [ DCP_MAX_CHANS ] ; } cocci_id@p ;
...
position p;
@@
 struct dcp_sha_req_ctx {  unsigned int init : 1 ;  unsigned int fini : 1 ; } cocci_id@p ;
...
position p;
@@
 struct dcp_sha_req_ctx * cocci_id@p ;
...
position p;
@@
 const struct dcp_export_state * cocci_id@p ;
...
position p;
@@
 struct dcp_export_state {  struct dcp_sha_req_ctx req_ctx ;  struct dcp_async_ctx async_ctx ; } cocci_id@p ;
...
@@
 struct dcp_async_ctx {  enum dcp_chan chan ;  uint32_t fill ;  struct mutex mutex ;  uint32_t alg ;  unsigned int hot : 1 ;  struct crypto_sync_skcipher * fallback ;  unsigned int key_len ;  uint8_t key [ AES_KEYSIZE_128 ] ; } cocci_id@p ;
...
@@
 struct dcp {  struct device * dev ;  void __iomem * base ;  uint32_t caps ;  struct dcp_coherent_block * coh ;  struct completion completion [ DCP_MAX_CHANS ] ;  spinlock_t lock [ DCP_MAX_CHANS ] ;  struct task_struct * thread [ DCP_MAX_CHANS ] ;  struct crypto_queue queue [ DCP_MAX_CHANS ] ;  struct clk * dcp_clk ; } cocci_id@p ;
