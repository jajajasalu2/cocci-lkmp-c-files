...
position p;
@@
 struct dln2_platform_data * cocci_id@p ;
...
position p;
@@
 struct dln2_response {  struct dln2_header hdr ;  __le16 result ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,bool,wait_queue_head_t,spinlock_t;
@@
 struct dln2_dev {  struct usb_device * usb_dev ;  struct usb_interface * interface ;  u8 ep_in ;  u8 ep_out ;  struct urb * rx_urb [ DLN2_MAX_URBS ] ;  void * rx_buf [ DLN2_MAX_URBS ] ;  struct dln2_mod_rx_slots mod_rx_slots [ DLN2_HANDLES ] ;  struct list_head event_cb_list ;  spinlock_t event_cb_lock ;  bool disconnect ;  int active_transfers ;  wait_queue_head_t disconnect_wq ;  spinlock_t disconnect_lock ; } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u16,dln2_event_cb_t;
@@
 struct dln2_event_cb_entry {  struct list_head list ;  u16 id ;  struct platform_device * pdev ;  dln2_event_cb_t callback ; } cocci_id@p ;
...
position p;
@@
 struct dln2_event_cb_entry * cocci_id@p ;
...
position p;
@@
 struct dln2_rx_context * cocci_id@p ;
...
position p;
@@
 struct dln2_mod_rx_slots * cocci_id@p ;
...
@@
 struct dln2_mod_rx_slots { 
	 DECLARE_BITMAP ( bmap , DLN2_MAX_RX_SLOTS ) 
		 ;  wait_queue_head_t wq ;  struct dln2_rx_context slots [ DLN2_MAX_RX_SLOTS ] ;  spinlock_t lock ; } cocci_id@p ;
...
typedef bool;
@@
 struct dln2_rx_context {  struct completion done ;  struct urb * urb ;  bool in_use ; } cocci_id@p ;
...
position p;
@@
 struct dln2_header {  __le16 size ;  __le16 id ;  __le16 echo ;  __le16 handle ; } cocci_id@p ;
...
position p;
@@
 enum dln2_handle { DLN2_HANDLE_EVENT = 0 , DLN2_HANDLE_CTRL , DLN2_HANDLE_GPIO , DLN2_HANDLE_I2C , DLN2_HANDLE_SPI , DLN2_HANDLES , } cocci_id@p ;
...
symbol cocci_id;
position p;
typedef dln2_event_cb_t;
@@
 dln2_event_cb_t cocci_id@p ;
...
position p;
@@
 struct dln2_response * cocci_id@p ;
