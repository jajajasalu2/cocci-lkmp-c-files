...
position p;
@@
 struct telem_device_data cocci_id@p ;
...
typedef atomic_t;
@@
 struct telem_device_data {  struct device dev ;  struct cdev cdev ;  struct wilco_ec_device * ec ;  atomic_t available ; } cocci_id@p ;
...
position p;
@@
 struct wilco_ec_telem_request * cocci_id@p ;
...
typedef u8,bool;
@@
 struct telem_session_data {  struct telem_device_data * dev_data ;  struct wilco_ec_telem_request request ;  u8 response [ TELEM_RESPONSE_SIZE ] ;  bool has_msg ; } cocci_id@p ;
...
position p;
@@
 struct telem_session_data * cocci_id@p ;
