...
typedef u32;
@@
 u32 cocci_id@p [ ATH9K_NUM_DMA_DEBUG_REGS ] ;
...
position p;
@@
 struct ath_antenna_stats * cocci_id@p ;
...
position p;
@@
 struct ath_hw_antcomb_conf cocci_id@p ;
...
position p;
@@
 struct ath9k_nfcal_hist * cocci_id@p ;
...
position p;
@@
 struct {  const char * name ;  unsigned int val ; } cocci_id@p [ ] ;
...
typedef u32;
@@
 const struct reg_hole {  u32 start ;  u32 end ; } cocci_id@p [ ] ;
...
position p;
@@
 struct ath_rx_status * cocci_id@p ;
...
position p;
@@
 const char * const cocci_id@p [ __RESET_TYPE_MAX ] ;
