...
position p;
@@
 struct rsxx_dma_ctrl cocci_id@p ;
...
position p;
@@
 struct dma_tracker {  int next_tag ;  struct rsxx_dma * dma ; } cocci_id@p ;
...
position p;
@@
 enum rsxx_hw_cmd { HW_CMD_BLK_DISCARD = 0x70 , HW_CMD_BLK_WRITE = 0x80 , HW_CMD_BLK_READ = 0xC0 , HW_CMD_BLK_RECON_READ = 0xE0 , } cocci_id@p ;
...
position p;
@@
 int cocci_id@p [ RSXX_MAX_TARGETS ] ;
...
position p;
@@
 struct rsxx_cardinfo * cocci_id@p ;
...
@@
 struct rsxx_dma {  struct list_head list ;  u8 cmd ;  unsigned int laddr ;  struct {  u32 off ;  u32 cnt ; } sub_page ;  dma_addr_t dma_addr ;  struct page * page ;  unsigned int pg_off ;  rsxx_dma_cb cb ;  void * cb_data ; } cocci_id@p ;
...
position p;
@@
 enum rsxx_dma_status { DMA_SW_ERR = 0x1 , DMA_HW_FAULT = 0x2 , DMA_CANCELLED = 0x4 , } cocci_id@p ;
...
position p;
@@
 enum rsxx_hw_status { HW_STATUS_CRC = 0x01 , HW_STATUS_HARD_ERR = 0x02 , HW_STATUS_SOFT_ERR = 0x04 , HW_STATUS_FAULT = 0x08 , } cocci_id@p ;
...
position p;
@@
 struct list_head cocci_id@p [ RSXX_MAX_TARGETS ] ;
...
position p;
@@
 struct dma_tracker_list * cocci_id@p ;
...
typedef spinlock_t;
@@
 struct dma_tracker_list {  spinlock_t lock ;  int head ;  struct dma_tracker list [ 0 ] ; } cocci_id@p ;
