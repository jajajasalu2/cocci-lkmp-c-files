...
position p;
@@
 const struct sfb_bucket * cocci_id@p ;
...
position p;
@@
 struct sfb_sched_data * cocci_id@p ;
...
position p;
@@
 struct tc_sfb_xstats cocci_id@p ;
...
typedef u8,bool,u32;
@@
 struct sfb_sched_data {  struct Qdisc * qdisc ;  struct tcf_proto __rcu * filter_list ;  struct tcf_block * block ;  unsigned long rehash_interval ;  unsigned long warmup_time ;  u32 max ;  u32 bin_size ;  u32 increment ;  u32 decrement ;  u32 limit ;  u32 penalty_rate ;  u32 penalty_burst ;  u32 tokens_avail ;  unsigned long rehash_time ;  unsigned long token_time ;  u8 slot ;  bool double_buffering ;  struct sfb_bins bins [ 2 ] ;  struct {  u32 earlydrop ;  u32 penaltydrop ;  u32 bucketdrop ;  u32 queuedrop ;  u32 childdrop ;  u32 marked ; } stats ; } cocci_id@p ;
...
position p;
@@
 const struct tc_sfb_qopt * cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_SFB_MAX + 1 ] ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_SFB_MAX + 1 ] ;
...
@@
 struct sfb_bins {  siphash_key_t perturbation ;  struct sfb_bucket bins [ SFB_LEVELS ] [ SFB_NUMBUCKETS ] ; } cocci_id@p ;
...
position p;
@@
 const struct sfb_sched_data * cocci_id@p ;
...
typedef u32;
@@
 struct sfb_skb_cb {  u32 hashes [ 2 ] ; } cocci_id@p ;
...
typedef u16;
@@
 struct sfb_bucket {  u16 qlen ;  u16 p_mark ; } cocci_id@p ;
