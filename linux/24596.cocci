...
position p;
@@
 const struct skb_checksum_ops cocci_id@p ;
...
position p;
@@
 typeof ( skb -> transport_header ) cocci_id@p ;
...
position p;
@@
 struct sk_buff_fclones cocci_id@p ;
...
position p;
@@
 __sum16 ( * cocci_id@p ) ( struct sk_buff * skb ) ;
...
position p;
@@
 struct napi_alloc_cache {  struct page_frag_cache page ;  unsigned int skb_count ;  void * skb_cache [ NAPI_SKB_CACHE_SIZE ] ; } cocci_id@p ;
...
position p;
@@
 struct skb_seq_state * cocci_id@p ;
...
position p;
@@
 struct partial_page cocci_id@p [ MAX_SKB_FRAGS ] ;
...
position p;
@@
 struct nf_bridge_info cocci_id@p ;
...
position p;
@@
 struct page * cocci_id@p [ MAX_SKB_FRAGS ] ;
...
position p;
@@
 struct napi_alloc_cache * cocci_id@p ;
...
position p;
@@
 typeof ( skb -> mac_header ) cocci_id@p ;
...
position p;
@@
 const struct splice_pipe_desc * cocci_id@p ;
