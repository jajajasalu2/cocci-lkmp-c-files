...
typedef u64,bool;
@@
 struct perf_inject {  struct perf_tool tool ;  struct perf_session * session ;  bool build_ids ;  bool sched_stat ;  bool have_auxtrace ;  bool strip ;  bool jit_mode ;  const char * input_name ;  struct perf_data output ;  u64 bytes_written ;  u64 aux_id ;  struct list_head samples ;  struct itrace_synth_opts itrace_synth_opts ;  char event_copy [ PERF_SAMPLE_MAX_SIZE ] ; } cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct perf_tool * tool , union perf_event * event , struct perf_sample * sample , struct evsel * evsel , struct machine * machine ) ;
...
typedef u32;
@@
 struct event_entry {  struct list_head node ;  u32 tid ;  union perf_event event [ 0 ] ; } cocci_id@p ;
