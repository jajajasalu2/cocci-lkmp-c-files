...
position p;
@@
 const char * cocci_id@p [ CS_ETMV4_PRIV_MAX ] ;
...
position p;
@@
 struct cs_etm_recording * cocci_id@p ;
...
typedef bool;
@@
 bool cocci_id@p ( struct auxtrace_record * itr , int cpu ) ;
...
position p;
@@
 const char * cocci_id@p [ CS_ETM_PRIV_MAX ] ;
...
typedef bool;
@@
 struct cs_etm_recording {  struct auxtrace_record itr ;  struct perf_pmu * cs_etm_pmu ;  struct evlist * evlist ;  int wrapped_cnt ;  bool * wrapped ;  bool snapshot_mode ;  size_t snapshot_size ; } cocci_id@p ;
