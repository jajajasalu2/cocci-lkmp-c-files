...
position p;
@@
 struct debug_percpu_free * cocci_id@p ;
...
position p;
@@
 struct self_test {  unsigned long dummy1 [ 6 ] ;  int static_init ;  unsigned long dummy2 [ 3 ] ; } cocci_id@p ;
...
position p;
@@
 const char * cocci_id@p [ ODEBUG_STATE_MAX ] ;
...
typedef bool;
@@
 bool ( * cocci_id@p ) ( void * addr , enum debug_obj_state state ) ;
...
position p;
@@
 struct debug_obj * cocci_id@p [ ODEBUG_BATCH_SIZE ] ;
...
@@
 struct debug_bucket {  struct hlist_head list ;  raw_spinlock_t lock ; } cocci_id@p ;
...
position p;
@@
 struct debug_percpu_free {  struct hlist_head free_objs ;  int obj_free ; } cocci_id@p ;
...
position p;
@@
 struct debug_bucket cocci_id@p [ ODEBUG_HASH_SIZE ] ;
...
position p;
@@
 struct debug_bucket * cocci_id@p ;
