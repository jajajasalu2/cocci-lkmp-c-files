...
position p;
@@
 struct zstd_workspace_manager cocci_id@p ;
...
position p;
@@
 size_t cocci_id@p [ ZSTD_BTRFS_MAX_LEVEL ] ;
...
@@
 struct zstd_workspace_manager {  const struct btrfs_compress_op * ops ;  spinlock_t lock ;  struct list_head lru_list ;  struct list_head idle_ws [ ZSTD_BTRFS_MAX_LEVEL ] ;  unsigned long active_map ;  wait_queue_head_t wait ;  struct timer_list timer ; } cocci_id@p ;
...
position p;
@@
 struct workspace {  void * mem ;  size_t size ;  char * buf ;  unsigned int level ;  unsigned int req_level ;  unsigned long last_used ;  struct list_head list ;  struct list_head lru_list ;  ZSTD_inBuffer in_buf ;  ZSTD_outBuffer out_buf ; } cocci_id@p ;
