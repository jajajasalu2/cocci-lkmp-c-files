...
position p;
@@
 struct rc_map_table cocci_id@p [ ] ;
...
position p;
@@
 struct lirc_scancode cocci_id@p ;
...
position p;
@@
 struct rc_filter_attribute cocci_id@p ;
...
position p;
@@
 const struct rc_map_table * cocci_id@p ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct rc_dev * dev , struct rc_scancode_filter * filter ) ;
...
typedef u64;
@@
 const struct {  u64 type ;  const char * name ;  const char * module_name ; } cocci_id@p [ ] ;
...
position p;
@@
 enum rc_driver_type cocci_id@p ;
...
typedef bool;
@@
 struct rc_filter_attribute {  struct device_attribute attr ;  enum rc_filter_type type ;  bool mask ; } cocci_id@p ;
...
position p;
@@
 const struct rc_map * cocci_id@p ;
...
position p;
@@
 const struct {  const char * name ;  unsigned int repeat_period ;  unsigned int scancode_bits ; } cocci_id@p [ ] ;
