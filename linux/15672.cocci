...
position p;
@@
 struct gb_loopback_stats cocci_id@p ;
...
typedef bool;
@@
 void cocci_id@p ( struct gb_loopback * gb , bool error ) ;
...
position p;
@@
 char cocci_id@p [ DEBUGFS_NAMELEN ] ;
...
position p;
@@
 struct gb_loopback_async_operation * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u32,wait_queue_head_t,spinlock_t;
@@
 struct gb_loopback_device {  struct dentry * root ;  u32 count ;  size_t size_max ;  spinlock_t lock ;  wait_queue_head_t wq ; } cocci_id@p ;
...
position p;
@@
 struct gb_loopback_device cocci_id@p ;
...
position p;
@@
 struct gb_loopback_transfer_request cocci_id@p ;
...
typedef u64,u32;
@@
 struct gb_loopback_stats {  u32 min ;  u32 max ;  u64 sum ;  u32 count ; } cocci_id@p ;
...
typedef ktime_t;
@@
 struct gb_loopback_async_operation {  struct gb_loopback * gb ;  struct gb_operation * operation ;  ktime_t ts ;  int ( * completion ) ( struct gb_loopback_async_operation * op_async ) ; } cocci_id@p ;
...
position p;
@@
 struct gb_loopback_transfer_response * cocci_id@p ;
...
position p;
@@
 struct gb_operation_msg_hdr cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct gb_loopback * gb ) ;
