...
typedef cstate_t;
@@
 cstate_t cocci_id@p [ AMD_FAM14H_STATE_NUM ] ;
...
position p;
@@
 struct cpuidle_monitor cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( unsigned int id , unsigned long long * count , unsigned int cpu ) ;
...
position p;
@@
 enum amd_fam14h_states { NON_PC0 = 0 , PC1 , PC6 , NBP1 , AMD_FAM14H_STATE_NUM , } cocci_id@p ;
...
position p;
@@
 unsigned long long * cocci_id@p [ AMD_FAM14H_STATE_NUM ] ;
...
position p;
@@
 int cocci_id@p ( unsigned int self_id , double * percent , unsigned int cpu ) ;
