...
typedef bool;
@@
 struct icmp_control {  bool ( * handler ) ( struct sk_buff * skb ) ;  short error ; } cocci_id@p ;
...
position p;
@@
 const struct icmp_control cocci_id@p [ NR_ICMP_TYPES + 1 ] ;
...
position p;
@@
 const struct icmp_err cocci_id@p [ ] ;
...
typedef __be32;
@@
 struct icmp_bxm {  struct sk_buff * skb ;  int offset ;  int data_len ;  struct {  struct icmphdr icmph ;  __be32 times [ 3 ] ; } data ;  int head_len ;  struct ip_options_data replyopts ; } cocci_id@p ;
...
@@
 struct {  spinlock_t lock ;  u32 credit ;  u32 stamp ; } cocci_id@p ;
