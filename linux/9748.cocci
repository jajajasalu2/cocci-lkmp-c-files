...
position p;
@@
 struct rs_tx_column cocci_id@p ;
...
position p;
@@
 const struct rs_init_rate_info * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef u8,bool,allow_column_func_t;
@@
 struct rs_tx_column {  enum rs_column_mode mode ;  u8 ant ;  bool sgi ;  enum rs_column next_columns [ MAX_NEXT_COLUMNS ] ;  allow_column_func_t checks [ MAX_COLUMN_CHECKS ] ; } cocci_id@p ;
...
typedef u16;
@@
 const u16 cocci_id@p [ 4 ] [ IWL_RATE_COUNT ] ;
...
position p;
@@
 const struct rate_control_ops cocci_id@p ;
...
position p;
@@
 const struct iwl_rs_rate_info cocci_id@p [ IWL_RATE_COUNT ] ;
...
position p;
@@
 const struct iwl_rate_mcs_info cocci_id@p [ IWL_RATE_COUNT ] ;
...
typedef bool;
@@
 void cocci_id@p ( struct iwl_lq_sta * lq_sta , bool force_search ) ;
...
position p;
@@
 struct iwl_scale_tbl_info * cocci_id@p ;
...
typedef u16;
@@
 const u16 * cocci_id@p [ IWL_RATE_COUNT ] ;
...
position p;
@@
 enum iwl_table_type cocci_id@p ;
...
typedef s8,u8;
@@
 struct rs_init_rate_info {  s8 rssi ;  u8 rate_idx ; } cocci_id@p ;
...
position p;
@@
 enum rs_column_mode { RS_INVALID = 0 , RS_LEGACY , RS_SISO , RS_MIMO2 , } cocci_id@p ;
...
position p;
@@
 struct iwl_rate_scale_data * cocci_id@p ;
...
position p;
@@
 struct ieee80211_tx_rate_control * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct iwl_mvm * mvm , struct ieee80211_sta * sta , struct iwl_lq_sta * lq_sta , const struct rs_rate * initial_rate ) ;
...
position p;
@@
 const struct rs_rate * cocci_id@p ;
...
position p;
@@
 const struct rs_tx_column cocci_id@p [ ] ;
...
position p;
@@
 struct rs_rate_stats * cocci_id@p ;
...
typedef bool;
@@
 void cocci_id@p ( struct iwl_mvm * mvm , struct ieee80211_sta * sta , struct iwl_lq_sta * lq_sta , int tid , bool ndp ) ;
...
symbol cocci_id;
position p;
typedef allow_column_func_t;
@@
 allow_column_func_t cocci_id@p ;
...
position p;
@@
 struct rs_bfer_active_iter_data * cocci_id@p ;
...
position p;
@@
 enum rs_action { RS_ACTION_STAY = 0 , RS_ACTION_DOWNSCALE = - 1 , RS_ACTION_UPSCALE = 1 , } cocci_id@p ;
...
position p;
@@
 enum tpc_action { TPC_ACTION_STAY , TPC_ACTION_DECREASE , TPC_ACTION_INCREASE , TPC_ACTION_NO_RESTIRCTION , } cocci_id@p ;
...
position p;
@@
 struct rs_bfer_active_iter_data {  struct ieee80211_sta * exclude_sta ;  struct iwl_mvm_sta * bfer_mvmsta ; } cocci_id@p ;
...
typedef bool;
@@
 bool ( * cocci_id@p ) ( struct iwl_mvm * mvm , struct ieee80211_sta * sta , struct rs_rate * rate , const struct rs_tx_column * next_col ) ;
