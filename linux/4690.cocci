...
position p;
@@
 struct phantom_device cocci_id@p ;
...
position p;
@@
 unsigned char cocci_id@p [ PHANTOM_MAX_MINORS ] ;
...
symbol cocci_id;
position p;
typedef u32,wait_queue_head_t,spinlock_t,atomic_t;
@@
 struct phantom_device {  unsigned int opened ;  void __iomem * caddr ;  u32 __iomem * iaddr ;  u32 __iomem * oaddr ;  unsigned long status ;  atomic_t counter ;  wait_queue_head_t wait ;  struct cdev cdev ;  struct mutex open_lock ;  spinlock_t regs_lock ;  struct phm_regs oregs ;  u32 ctl_reg ; } cocci_id@p ;
