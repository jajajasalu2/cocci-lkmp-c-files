...
position p;
@@
 struct bh_accounting {  int nr ;  int ratelimit ; } cocci_id@p ;
...
position p;
@@
 struct decrypt_bh_ctx cocci_id@p ;
...
position p;
@@
 struct decrypt_bh_ctx {  struct work_struct work ;  struct buffer_head * bh ; } cocci_id@p ;
...
typedef spinlock_t;
@@
 int cocci_id@p ( spinlock_t * lock , struct list_head * list ) ;
...
position p;
@@
 int cocci_id@p ( int op , int op_flags , struct buffer_head * bh , enum rw_hint hint , struct writeback_control * wbc ) ;
...
position p;
@@
 struct bh_lru {  struct buffer_head * bhs [ BH_LRU_SIZE ] ; } cocci_id@p ;
