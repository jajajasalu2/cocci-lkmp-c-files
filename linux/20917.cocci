...
@@
 struct uml_gre_data {  uint32_t rx_key ;  uint32_t tx_key ;  uint32_t sequence ;  bool ipv6 ;  bool has_sequence ;  bool pin_sequence ;  bool checksum ;  bool key ;  struct gre_minimal_header expected_header ;  uint32_t checksum_offset ;  uint32_t key_offset ;  uint32_t sequence_offset ; } cocci_id@p ;
...
position p;
@@
 struct uml_gre_data * cocci_id@p ;
...
position p;
@@
 struct uml_l2tpv3_data * cocci_id@p ;
...
typedef uint16_t;
@@
 struct gre_minimal_header {  uint16_t header ;  uint16_t arptype ; } cocci_id@p ;
...
@@
 struct uml_l2tpv3_data {  uint64_t rx_cookie ;  uint64_t tx_cookie ;  uint64_t rx_session ;  uint64_t tx_session ;  uint32_t counter ;  bool udp ;  bool ipv6 ;  bool has_counter ;  bool pin_counter ;  bool cookie ;  bool cookie_is_64 ;  uint32_t cookie_offset ;  uint32_t session_offset ;  uint32_t counter_offset ; } cocci_id@p ;
