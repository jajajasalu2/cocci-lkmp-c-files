...
position p;
@@
 struct serio_device_id cocci_id@p [ ] ;
...
position p;
@@
 int cocci_id@p ( const char * val , const struct kernel_param * ) ;
...
position p;
@@
 const struct psmouse_protocol cocci_id@p [ ] ;
...
position p;
@@
 struct psmouse_attribute * cocci_id@p ;
...
typedef bool;
@@
 int ( * cocci_id@p ) ( struct psmouse * , bool ) ;
