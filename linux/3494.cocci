...
typedef u32;
@@
 struct intel_host {  u32 dsm_fns ;  u32 hs_caps ; } cocci_id@p ;
...
position p;
@@
 struct sdhci_acpi_uid_slot {  const char * hid ;  const char * uid ;  const struct sdhci_acpi_slot * slot ; } cocci_id@p ;
...
position p;
@@
 enum { INTEL_DSM_FNS = 0 , INTEL_DSM_V18_SWITCH = 3 , INTEL_DSM_V33_SWITCH = 4 , INTEL_DSM_HS_CAPS = 8 , } cocci_id@p ;
...
typedef bool;
@@
 struct sdhci_acpi_host {  struct sdhci_host * host ;  const struct sdhci_acpi_slot * slot ;  struct platform_device * pdev ;  bool use_runtime_pm ;  unsigned long private [ 0 ] ____cacheline_aligned ; } cocci_id@p ;
...
position p;
@@
 const struct sdhci_acpi_slot cocci_id@p ;
...
position p;
@@
 enum { SDHCI_ACPI_SD_CD = BIT ( 0 ) , SDHCI_ACPI_RUNTIME_PM = BIT ( 1 ) , SDHCI_ACPI_SD_CD_OVERRIDE_LEVEL = BIT ( 2 ) , } cocci_id@p ;
...
position p;
@@
 const struct sdhci_acpi_uid_slot cocci_id@p [ ] ;
...
position p;
@@
 struct sdhci_acpi_host * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef mmc_pm_flag_t;
@@
 struct sdhci_acpi_chip {  const struct sdhci_ops * ops ;  unsigned int quirks ;  unsigned int quirks2 ;  unsigned long caps ;  unsigned int caps2 ;  mmc_pm_flag_t pm_caps ; } cocci_id@p ;
...
position p;
@@
 const struct sdhci_acpi_chip cocci_id@p ;
