...
position p;
@@
 struct hh_flow_state cocci_id@p ;
...
position p;
@@
 struct wdrr_bucket {  struct sk_buff * head ;  struct sk_buff * tail ;  struct list_head bucketchain ;  int deficit ; } cocci_id@p ;
...
position p;
@@
 struct hhf_sched_data cocci_id@p ;
...
position p;
@@
 enum wdrr_bucket_idx { WDRR_BUCKET_FOR_HH = 0 , WDRR_BUCKET_FOR_NON_HH = 1 , } cocci_id@p ;
...
typedef u32;
@@
 struct hh_flow_state {  u32 hash_id ;  u32 hit_timestamp ;  struct list_head flowchain ; } cocci_id@p ;
...
position p;
@@
 struct tc_hhf_xstats cocci_id@p ;
...
position p;
@@
 const struct nla_policy cocci_id@p [ TCA_HHF_MAX + 1 ] ;
...
symbol cocci_id;
position p;
typedef u32,siphash_key_t;
@@
 struct hhf_sched_data {  struct wdrr_bucket buckets [ WDRR_BUCKET_CNT ] ;  siphash_key_t perturbation ;  u32 quantum ;  u32 drop_overlimit ;  struct list_head * hh_flows ;  u32 hh_flows_limit ;  u32 hh_flows_overlimit ;  u32 hh_flows_total_cnt ;  u32 hh_flows_current_cnt ;  u32 * hhf_arrays [ HHF_ARRAYS_CNT ] ;  u32 hhf_arrays_reset_timestamp ;  unsigned long * hhf_valid_bits [ HHF_ARRAYS_CNT ] ;  struct list_head new_buckets ;  struct list_head old_buckets ;  u32 hhf_reset_timeout ;  u32 hhf_admit_bytes ;  u32 hhf_evict_timeout ;  u32 hhf_non_hh_weight ; } cocci_id@p ;
...
position p;
@@
 enum wdrr_bucket_idx cocci_id@p ;
...
position p;
@@
 struct nlattr * cocci_id@p [ TCA_HHF_MAX + 1 ] ;
