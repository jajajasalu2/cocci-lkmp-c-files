...
position p;
@@
 struct max1027_chip_info {  const struct iio_chan_spec * channels ;  unsigned int num_channels ;  const unsigned long * available_scan_masks ; } cocci_id@p ;
...
typedef u8;
@@
 struct max1027_state {  const struct max1027_chip_info * info ;  struct spi_device * spi ;  struct iio_trigger * trig ;  __be16 * buffer ;  struct mutex lock ;  u8 reg ____cacheline_aligned ; } cocci_id@p ;
...
position p;
@@
 const struct max1027_chip_info cocci_id@p [ ] ;
...
position p;
@@
 struct max1027_state * cocci_id@p ;
...
position p;
@@
 enum max1027_id { max1027 , max1029 , max1031 , max1227 , max1229 , max1231 , } cocci_id@p ;
