...
position p;
@@
 struct stripe_head * cocci_id@p [ MAX_STRIPE_BATCH ] ;
...
position p;
@@
 enum async_tx_flags cocci_id@p ;
...
position p;
@@
 struct r5worker_group * * cocci_id@p ;
...
position p;
@@
 const struct r5pending_data * cocci_id@p ;
...
position p;
@@
 struct raid5_percpu cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct r5conf * conf , int cnt , int * group_cnt , int * worker_cnt_per_group , struct r5worker_group * * worker_groups ) ;
...
position p;
@@
 struct raid5_plug_cb * cocci_id@p ;
...
position p;
@@
 struct r5pending_data * cocci_id@p ;
...
typedef sector_t;
@@
 void cocci_id@p ( sector_t stripe , struct r5conf * conf , int previous , struct stripe_head * sh ) ;
...
position p;
@@
 void cocci_id@p ( struct stripe_head * head_sh , unsigned long handle_flags ) ;
...
position p;
@@
 void cocci_id@p ( struct r5conf * conf ) ;
...
position p;
@@
 struct raid5_plug_cb {  struct blk_plug_cb cb ;  struct list_head list ;  struct list_head temp_inactive_list [ NR_STRIPE_HASH_LOCKS ] ; } cocci_id@p ;
