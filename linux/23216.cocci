...
position p;
@@
 const struct {  int value ;  int ratio ; } cocci_id@p [ ] ;
...
position p;
@@
 const char * cocci_id@p [ WM8523_NUM_SUPPLIES ] ;
...
position p;
@@
 struct wm8523_priv {  struct regmap * regmap ;  struct regulator_bulk_data supplies [ WM8523_NUM_SUPPLIES ] ;  unsigned int sysclk ;  unsigned int rate_constraint_list [ WM8523_NUM_RATES ] ;  struct snd_pcm_hw_constraint_list rate_constraint ; } cocci_id@p ;
...
position p;
@@
 const struct {  int value ;  int ratio ; } cocci_id@p [ WM8523_NUM_RATES ] ;
