...
position p;
@@
 const  cocci_id@p ( ssm260x_outmix_tlv , 
							    0 , 47 , 
...
typedef u8,u32;
@@
 struct ssm2602_coeff {  u32 mclk ;  u32 rate ;  u8 srate ; } cocci_id@p ;
...
position p;
@@
 const struct reg_default cocci_id@p [ SSM2602_CACHEREGNUM ] ;
...
position p;
@@
 struct ssm2602_priv * cocci_id@p ;
...
position p;
@@
 const struct ssm2602_coeff cocci_id@p [ ] ;
...
position p;
@@
 struct ssm2602_priv {  unsigned int sysclk ;  const struct snd_pcm_hw_constraint_list * sysclk_constraints ;  struct regmap * regmap ;  enum ssm2602_type type ;  unsigned int clk_out_pwr ; } cocci_id@p ;
