...
position p;
@@
 struct vmu_block {  unsigned int num ;  unsigned int ofs ; } cocci_id@p ;
...
position p;
@@
 enum maple_file_errors cocci_id@p ;
...
position p;
@@
 struct maple_driver cocci_id@p ;
...
typedef u32,u16;
@@
 struct memcard {  u16 tempA ;  u16 tempB ;  u32 partitions ;  u32 blocklen ;  u32 writecnt ;  u32 readcnt ;  u32 removeable ;  int partition ;  int read ;  unsigned char * blockread ;  struct vmupart * parts ;  struct mtd_info * mtd ; } cocci_id@p ;
...
position p;
@@
 struct mdev_part {  struct maple_device * mdev ;  int partition ; } cocci_id@p ;
...
position p;
@@
 struct maple_device * cocci_id@p ;
...
typedef u16;
@@
 struct vmupart {  u16 user_blocks ;  u16 root_block ;  u16 numblocks ;  char * name ;  struct vmu_cache * pcache ; } cocci_id@p ;
...
position p;
@@
 struct vmu_cache {  unsigned char * buffer ;  unsigned int block ;  unsigned long jiffies_atc ;  int valid ; } cocci_id@p ;
