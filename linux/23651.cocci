...
position p;
@@
 struct asm_data_cmd_read_v2_done * cocci_id@p ;
...
position p;
@@
 struct audio_client cocci_id@p ;
...
position p;
@@
 struct asm_session_cmd_run_v2 * cocci_id@p ;
...
@@
 struct q6asm {  struct apr_device * adev ;  struct device * dev ;  struct q6core_svc_api_info ainfo ;  wait_queue_head_t mem_wait ;  spinlock_t slock ;  struct audio_client * session [ MAX_SESSIONS + 1 ] ; } cocci_id@p ;
...
position p;
@@
 struct audio_port_data * cocci_id@p ;
...
position p;
@@
 struct audio_buffer * cocci_id@p ;
...
position p;
@@
 struct avs_cmd_shared_mem_unmap_regions * cocci_id@p ;
...
position p;
@@
 struct asm_stream_cmd_open_read_v3 * cocci_id@p ;
...
typedef u8;
@@
 u8 cocci_id@p [ PCM_MAX_NUM_CHANNEL ] ;
...
position p;
@@
 struct avs_cmd_shared_mem_map_regions * cocci_id@p ;
...
position p;
@@
 struct asm_data_cmd_write_v2 * cocci_id@p ;
...
position p;
@@
 struct asm_flac_fmt_blk_v2 * cocci_id@p ;
...
position p;
@@
 struct apr_resp_pkt * cocci_id@p ;
...
position p;
@@
 struct asm_multi_channel_pcm_fmt_blk_v2 * cocci_id@p ;
...
typedef uint32_t;
@@
 struct audio_port_data {  struct audio_buffer * buf ;  uint32_t num_periods ;  uint32_t dsp_buf ;  uint32_t mem_map_handle ; } cocci_id@p ;
...
position p;
@@
 struct asm_data_cmd_read_v2 * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef aprv2_ibasic_rsp_result_t;
@@
 struct aprv2_ibasic_rsp_result_t * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef wait_queue_head_t,spinlock_t,uint32_t,aprv2_ibasic_rsp_result_t;
@@
 struct audio_client {  int session ;  q6asm_cb cb ;  void * priv ;  uint32_t io_mode ;  struct apr_device * adev ;  struct mutex cmd_lock ;  spinlock_t lock ;  struct kref refcount ;  struct audio_port_data port [ 2 ] ;  wait_queue_head_t cmd_wait ;  struct aprv2_ibasic_rsp_result_t result ;  int perf_mode ;  int stream_id ;  struct q6asm * q6asm ;  struct device * dev ; } cocci_id@p ;
...
position p;
@@
 struct q6asm_flac_cfg * cocci_id@p ;
...
typedef u32;
@@
 struct asm_data_cmd_read_v2_done {  u32 status ;  u32 buf_addr_lsw ;  u32 buf_addr_msw ; } cocci_id@p ;
...
position p;
@@
 struct asm_enc_cfg_blk_param_v2 cocci_id@p ;
...
position p;
@@
 struct asm_stream_cmd_open_write_v3 * cocci_id@p ;
...
position p;
@@
 struct avs_shared_map_region_payload * cocci_id@p ;
...
position p;
@@
 struct asm_multi_channel_pcm_enc_cfg_v2 * cocci_id@p ;
...
symbol cocci_id;
position p;
typedef phys_addr_t,uint32_t;
@@
 struct audio_buffer {  phys_addr_t phys ;  uint32_t size ; } cocci_id@p ;
