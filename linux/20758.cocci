...
position p;
@@
 struct xen_memory_exchange cocci_id@p ;
...
position p;
@@
 const struct mmuext_op * cocci_id@p ;
...
position p;
@@
 const struct flush_tlb_info * cocci_id@p ;
...
position p;
@@
 struct multicall_space cocci_id@p ;
...
position p;
@@
 unsigned long cocci_id@p [ 1 << MAX_CONTIG_ORDER ] ;
...
position p;
@@
 int ( * cocci_id@p ) ( struct mm_struct * mm , struct page * , enum pt_level ) ;
...
symbol cocci_id;
position p;
typedef bool,xen_pfn_t,pgprot_t;
@@
 struct remap_data {  xen_pfn_t * pfn ;  bool contiguous ;  bool no_translate ;  pgprot_t prot ;  struct mmu_update * mmu_update ; } cocci_id@p ;
...
position p;
@@
 const struct pv_mmu_ops cocci_id@p ;
...
position p;
@@
 const struct mmu_update * cocci_id@p ;
...
position p;
@@
 struct xen_machphys_mapping cocci_id@p ;
...
position p;
@@
 struct xen_platform_parameters cocci_id@p ;
...
position p;
@@
 struct mmu_update cocci_id@p [ REMAP_BATCH_SIZE ] ;
