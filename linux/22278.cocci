...
position p;
@@
 struct selinux_kernel_status cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct selinux_fs_info * fsi ) ;
...
position p;
@@
 enum sel_inos { SEL_ROOT_INO = 2 , SEL_LOAD , SEL_ENFORCE , SEL_CONTEXT , SEL_ACCESS , SEL_CREATE , SEL_RELABEL , SEL_USER , SEL_POLICYVERS , SEL_COMMIT_BOOLS , SEL_MLS , SEL_DISABLE , SEL_MEMBER , SEL_CHECKREQPROT , SEL_COMPAT_NET , SEL_REJECT_UNKNOWN , SEL_DENY_UNKNOWN , SEL_STATUS , SEL_POLICY , SEL_VALIDATE_TRANS , SEL_INO_NEXT , } cocci_id@p ;
...
position p;
@@
 struct policy_load_memory * cocci_id@p ;
...
position p;
@@
 struct avc_cache_stats * cocci_id@p ;
...
position p;
@@
 struct selinux_fs_info * cocci_id@p ;
...
position p;
@@
 struct dentry * cocci_id@p ( struct dentry * dir , const char * name , unsigned long * ino ) ;
...
position p;
@@
 struct policy_load_memory {  size_t len ;  void * data ; } cocci_id@p ;
