...
position p;
@@
 const struct rot_variant * cocci_id@p ;
...
position p;
@@
 enum rot_irq_status { ROT_IRQ_STATUS_COMPLETE = 8 , ROT_IRQ_STATUS_ILLEGAL = 9 , } cocci_id@p ;
...
position p;
@@
 struct rot_variant {  const struct exynos_drm_ipp_formats * formats ;  unsigned int num_formats ; } cocci_id@p ;
...
position p;
@@
 enum rot_irq_status cocci_id@p ;
...
position p;
@@
 struct rot_context {  struct exynos_drm_ipp ipp ;  struct drm_device * drm_dev ;  struct device * dev ;  void __iomem * regs ;  struct clk * clock ;  const struct exynos_drm_ipp_formats * formats ;  unsigned int num_formats ;  struct exynos_drm_ipp_task * task ; } cocci_id@p ;
