...
position p;
@@
 struct streamzap_ir cocci_id@p ;
...
symbol cocci_id;
position p;
typedef bool,ktime_t,dma_addr_t;
@@
 struct streamzap_ir {  struct rc_dev * rdev ;  struct device * dev ;  struct usb_device * usbdev ;  struct usb_interface * interface ;  struct usb_endpoint_descriptor * endpoint ;  struct urb * urb_in ;  unsigned char * buf_in ;  dma_addr_t dma_in ;  unsigned int buf_in_len ;  enum StreamzapDecoderState decoder_state ;  bool idle ;  unsigned long sum ;  ktime_t signal_last ;  ktime_t signal_start ;  bool timeout_enabled ;  char name [ 128 ] ;  char phys [ 64 ] ; } cocci_id@p ;
...
position p;
@@
 enum StreamzapDecoderState { PulseSpace , FullPulse , FullSpace , IgnorePulse , } cocci_id@p ;
