...
position p;
@@
 enum { TUMBLER_MUTE_HP , TUMBLER_MUTE_AMP , TUMBLER_MUTE_LINE , } cocci_id@p ;
...
position p;
@@
 enum { VOL_IDX_PCM , VOL_IDX_PCM2 , VOL_IDX_ADC , VOL_IDX_LAST_MIX , } cocci_id@p ;
...
typedef u8;
@@
 struct pmac_tumbler {  struct pmac_keywest i2c ;  struct pmac_gpio audio_reset ;  struct pmac_gpio amp_mute ;  struct pmac_gpio line_mute ;  struct pmac_gpio line_detect ;  struct pmac_gpio hp_mute ;  struct pmac_gpio hp_detect ;  int headphone_irq ;  int lineout_irq ;  unsigned int save_master_vol [ 2 ] ;  unsigned int master_vol [ 2 ] ;  unsigned int save_master_switch [ 2 ] ;  unsigned int master_switch [ 2 ] ;  unsigned int mono_vol [ VOL_IDX_LAST_MONO ] ;  unsigned int mix_vol [ VOL_IDX_LAST_MIX ] [ 2 ] ;  int drc_range ;  int drc_enable ;  int capture_source ;  int anded_reset ;  int auto_mute_notify ;  int reset_on_sleep ;  u8 acs ; } cocci_id@p ;
...
position p;
@@
 enum { VOL_IDX_PCM_MONO , VOL_IDX_BASS , VOL_IDX_TREBLE , VOL_IDX_LAST_MONO , } cocci_id@p ;
...
typedef u8;
@@
 struct pmac_gpio {  unsigned int addr ;  u8 active_val ;  u8 inactive_val ;  u8 active_state ; } cocci_id@p ;
...
position p;
@@
 struct tumbler_mono_vol cocci_id@p ;
...
position p;
@@
 struct tumbler_mono_vol {  int index ;  int reg ;  int bytes ;  unsigned int max ;  unsigned int * table ; } cocci_id@p ;
...
position p;
@@
 struct pmac_keywest * cocci_id@p ;
...
position p;
@@
 struct pmac_tumbler * cocci_id@p ;
