...
position p;
@@
 struct usb_cdc_ncm_nth16 cocci_id@p ;
...
position p;
@@
 const struct cdc_ncm_stats cocci_id@p [ ] ;
...
position p;
@@
 void cocci_id@p ( unsigned long param ) ;
...
position p;
@@
 struct usb_cdc_ncm_dpe16 * cocci_id@p ;
...
position p;
@@
 struct usb_cdc_ncm_ndp16 * cocci_id@p ;
...
typedef u32;
@@
 void cocci_id@p ( struct usbnet * dev , u32 new_rx , u32 new_tx ) ;
...
position p;
@@
 enum hrtimer_restart cocci_id@p ( struct hrtimer * hr_timer ) ;
...
position p;
@@
 struct usb_cdc_speed_change * cocci_id@p ;
...
position p;
@@
 struct cdc_ncm_stats {  char stat_string [ ETH_GSTRING_LEN ] ;  int sizeof_stat ;  int stat_offset ; } cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct cdc_ncm_ctx * ctx ) ;
