...
position p;
@@
 enum nvme_rdma_queue_flags { NVME_RDMA_Q_ALLOCATED = 0 , NVME_RDMA_Q_LIVE = 1 , NVME_RDMA_Q_TR_READY = 2 , } cocci_id@p ;
...
typedef u64;
@@
 struct nvme_rdma_qe {  struct ib_cqe cqe ;  void * data ;  u64 dma ; } cocci_id@p ;
...
position p;
@@
 const struct nvme_rdma_cm_rej * cocci_id@p ;
...
@@
 struct nvme_rdma_request {  struct nvme_request req ;  struct ib_mr * mr ;  struct nvme_rdma_qe sqe ;  union nvme_result result ;  __le16 status ;  refcount_t ref ;  struct ib_sge sge [ 1 + NVME_RDMA_MAX_INLINE_SEGMENTS ] ;  u32 num_sge ;  int nents ;  struct ib_reg_wr reg_wr ;  struct ib_cqe reg_cqe ;  struct nvme_rdma_queue * queue ;  struct sg_table sg_table ;  struct scatterlist first_sgl [ ] ; } cocci_id@p ;
...
position p;
@@
 struct nvme_rdma_qe * cocci_id@p ;
...
position p;
@@
 struct nvme_rdma_device * cocci_id@p ;
...
position p;
@@
 int cocci_id@p ( struct rdma_cm_id * cm_id , struct rdma_cm_event * event ) ;
...
position p;
@@
 struct nvme_rdma_ctrl cocci_id@p ;
...
typedef bool,u32;
@@
 struct nvme_rdma_ctrl {  struct nvme_rdma_queue * queues ;  struct blk_mq_tag_set tag_set ;  struct work_struct err_work ;  struct nvme_rdma_qe async_event_sqe ;  struct delayed_work reconnect_work ;  struct list_head list ;  struct blk_mq_tag_set admin_tag_set ;  struct nvme_rdma_device * device ;  u32 max_fr_pages ;  struct sockaddr_storage addr ;  struct sockaddr_storage src_addr ;  struct nvme_ctrl ctrl ;  bool use_inline_data ;  u32 io_queues [ HCTX_MAX_TYPES ] ; } cocci_id@p ;
...
position p;
@@
 struct nvme_rdma_queue cocci_id@p ;
...
position p;
@@
 struct nvme_rdma_request * cocci_id@p ;
...
position p;
@@
 struct nvme_rdma_queue {  struct nvme_rdma_qe * rsp_ring ;  int queue_size ;  size_t cmnd_capsule_len ;  struct nvme_rdma_ctrl * ctrl ;  struct nvme_rdma_device * device ;  struct ib_cq * ib_cq ;  struct ib_qp * qp ;  unsigned long flags ;  struct rdma_cm_id * cm_id ;  int cm_error ;  struct completion cm_done ; } cocci_id@p ;
...
position p;
@@
 enum ib_poll_context cocci_id@p ;
...
position p;
@@
 struct nvme_rdma_device {  struct ib_device * dev ;  struct ib_pd * pd ;  struct kref ref ;  struct list_head entry ;  unsigned int num_inline_segments ; } cocci_id@p ;
