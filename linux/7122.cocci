...
position p;
@@
 struct pci_dev_entry * cocci_id@p ;
...
position p;
@@
 struct vpci_dev_data {  struct list_head dev_list [ PCI_SLOT_MAX ] ;  struct mutex lock ; } cocci_id@p ;
...
position p;
@@
 struct vpci_dev_data * cocci_id@p ;
...
position p;
@@
 publish_pci_root_cb cocci_id@p ;
...
position p;
@@
 const struct xen_pcibk_backend cocci_id@p ;
