...
position p;
@@
 struct deferred_unmap_info * cocci_id@p ;
...
position p;
@@
 const struct qset_params * cocci_id@p ;
...
position p;
@@
 const struct sg_ent * cocci_id@p ;
...
typedef s8,u8;
@@
 struct tx_sw_desc {  struct sk_buff * skb ;  u8 eop ;  u8 addr_idx ;  u8 fragidx ;  s8 sflit ; } cocci_id@p ;
...
typedef __be64;
@@
 struct tx_desc {  __be64 flit [ TX_DESC_FLITS ] ; } cocci_id@p ;
...
position p;
@@
 const struct deferred_unmap_info * cocci_id@p ;
...
typedef dma_addr_t;
@@
 struct deferred_unmap_info {  struct pci_dev * pdev ;  dma_addr_t addr [ MAX_SKB_FRAGS + 1 ] ; } cocci_id@p ;
...
position p;
@@
 enum { TXQ_ETH , TXQ_OFLD , TXQ_CTRL , } cocci_id@p ;
...
position p;
@@
 const struct rsp_desc * cocci_id@p ;
...
typedef __be32;
@@
 struct rx_desc {  __be32 addr_lo ;  __be32 len_gen ;  __be32 gen2 ;  __be32 addr_hi ; } cocci_id@p ;
...
position p;
@@
 struct sk_buff * cocci_id@p [ RX_BUNDLE_SIZE ] ;
...
position p;
@@
 enum { TXQ_RUNNING = 1 << 0 , TXQ_LAST_PKT_DB = 1 << 1 , } cocci_id@p ;
...
typedef u8,__be32;
@@
 struct rsp_desc {  struct rss_header rss_hdr ;  __be32 flags ;  __be32 len_cq ;  u8 imm_data [ 47 ] ;  u8 intr_gen ; } cocci_id@p ;
...
position p;
@@
 unsigned int cocci_id@p [ SGE_TXQ_PER_SET ] ;
...
position p;
@@
 struct rx_sw_desc {  union {  struct sk_buff * skb ;  struct fl_pg_chunk pg_chunk ; } ;  DEFINE_DMA_UNMAP_ADDR ( dma_addr ) ; } cocci_id@p ;
