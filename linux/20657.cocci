...
position p;
@@
 struct x86_pmu_quirk * cocci_id@p ;
...
position p;
@@
 struct perf_callchain_entry_ctx * cocci_id@p ;
...
position p;
@@
 struct sched_state {  int weight ;  int event ;  int counter ;  int unassigned ;  int nr_gp ;  unsigned long used [ BITS_TO_LONGS ( X86_PMC_IDX_MAX ) ] ; } cocci_id@p ;
...
position p;
@@
 struct perf_pmu_events_ht_attr * cocci_id@p ;
...
position p;
@@
 void cocci_id@p ( struct perf_event * event , int flags ) ;
...
position p;
@@
 struct x86_perf_task_context cocci_id@p ;
...
position p;
@@
 unsigned long cocci_id@p [ BITS_TO_LONGS ( X86_PMC_IDX_MAX ) ] ;
...
position p;
@@
 int cocci_id@p [ X86_PMC_IDX_MAX ] ;
...
position p;
@@
 struct stack_frame_ia32 cocci_id@p ;
...
position p;
@@
 struct perf_sched {  int max_weight ;  int max_events ;  int max_gp ;  int saved_states ;  struct event_constraint * * constraints ;  struct sched_state state ;  struct sched_state saved [ SCHED_STATES_MAX ] ; } cocci_id@p ;
